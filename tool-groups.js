(function(){
  ...
  function renderSelectionOptions(body,group,activeSubTool){
    ...
    if(activeSubTool&&activeSubTool.id==='magic-wand'&&window.MagicWandSelection){
      ...
      const toleranceField=document.createElement('div');toleranceField.className='selection-option-field';const toleranceLabel=document.createElement('label');toleranceLabel.className='selection-option-label';toleranceLabel.textContent='Tolerance';const toleranceControls=document.createElement('div');toleranceControls.className='magic-wand-tolerance-controls';const tolerance=document.createElement('input');tolerance.type='range';tolerance.className='ts-range';tolerance.min=0;tolerance.max=255;tolerance.step=1;tolerance.value=settings.tolerance;toleranceLabel.htmlFor='magic-wand-tolerance';tolerance.id='magic-wand-tolerance';const toleranceValue=document.createElement('output');toleranceValue.className='ts-num magic-wand-tolerance-value';toleranceValue.setAttribute('for',tolerance.id);toleranceValue.textContent=settings.tolerance;tolerance.oninput=()=>{toleranceValue.textContent=tolerance.value;MagicWandSelection.updateSettings({tolerance:+tolerance.value});};toleranceControls.append(tolerance,toleranceValue);toleranceField.append(toleranceLabel,toleranceControls);wandSection.appendChild(toleranceField);
      function checkbox(labelText,key){const row=document.createElement('label');row.className='magic-wand-checkbox-row';const input=document.createElement('input');input.type='checkbox';input.className='ts-check';input.checked=!!settings[key];input.onchange=()=>MagicWandSelection.updateSettings({[key]:input.checked});const text=document.createElement('span');text.textContent=labelText;row.append(input,text);wandSection.appendChild(row);}
      checkbox('Contiguous','contiguous');
      const sampleField=document.createElement('label');sampleField.className='selection-option-field';const sampleLabel=document.createElement('span');sampleLabel.className='selection-option-label';sampleLabel.textContent='Sample';const sample=document.createElement('select');sample.className='ts-select magic-wand-sample-select';[['current','Current Layer'],['all','All Visible Layers']].forEach(([value,text])=>{const option=document.createElement('option');option.value=value;option.textContent=text;sample.appendChild(option);});sample.value=settings.sample;sample.onchange=()=>MagicWandSelection.updateSettings({sample:sample.value});sampleField.append(sampleLabel,sample);wandSection.appendChild(sampleField);
      checkbox('Include Alpha','includeAlpha');checkbox('Anti-alias','antiAlias');layout.appendChild(wandSection);
    }
