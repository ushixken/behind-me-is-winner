...
test("A. Windows desktop + pen hold on canvas -> no context menu", () => {
  ...
  assert.strictEqual(
    allowed,
    false,
    "Desktop pen hold on canvas must NOT allow context menu",
  );
  resetPointerDown();
});
...
test("B. Windows desktop + pen hold on other UI element (layers/brush/palette) -> blocked", () => {
  ...
  const ctxEvent = {
    ...
    button: 2,
    buttons: 0,
    target: { className: "layer-row" },
  };
  const allowed = shouldAllowContextMenu(ctxEvent, { env: winDesktopEnv });
  assert.strictEqual(
    allowed,
    false,
    "Desktop pen hold on layer row must NOT allow context menu",
  );
  resetPointerDown();
});
...
test("C. Windows desktop + mouse right-click on canvas -> context menu allowed", () => {
  ...
  assert.strictEqual(
    allowed,
    true,
    "Desktop mouse right-click on canvas must be allowed",
  );
  resetPointerDown();
});
...
test("D. Windows desktop + mouse right-click on other UI element -> context menu allowed", () => {
  ...
  const ctxEvent = {
    ...
    button: 2,
    buttons: 2,
    target: { className: "bp-item" },
  };
  const allowed = shouldAllowContextMenu(ctxEvent, { env: winDesktopEnv });
  assert.strictEqual(
    allowed,
    true,
    "Desktop mouse right-click on UI element must be allowed",
  );
  resetPointerDown();
});
...
test("F. macOS desktop + pen hold -> no context menu", () => {
  ...
  assert.strictEqual(
    allowed,
    false,
    "macOS desktop pen hold must NOT allow context menu",
  );
  resetPointerDown();
});
...
test("G. Linux desktop + pen hold -> no context menu", () => {
  ...
  assert.strictEqual(
    allowed,
    false,
    "Linux desktop pen hold must NOT allow context menu",
  );
  resetPointerDown();
});
...
test("H. Android tablet + pen hold -> allowed", () => {
  ...
  assert.strictEqual(
    allowed,
    true,
    "Android tablet stylus hold must be allowed",
  );
  resetPointerDown();
});
...
test("I. Android tablet + touch long-press -> preserved", () => {
  ...
  assert.strictEqual(
    allowed,
    true,
    "Android tablet touch long-press must be allowed",
  );
  resetPointerDown();
});
...
test("J. desktop pen barrel button click -> preserved", () => {
  ...
  assert.strictEqual(
    allowed,
    true,
    "Desktop pen barrel button click must be allowed",
  );
  resetPointerDown();
});
...
test("K. iPadOS tablet (MacIntel + maxTouchPoints > 1) -> tablet classification", () => {
  ...
  assert.strictEqual(
    isTabletOrMobileDevice(ipadEnv),
    true,
    "iPadOS must classify as tablet",
  );
});
...
test("L. Windows touchscreen laptop (Win32 + maxTouchPoints > 0) -> desktop classification", () => {
  ...
  assert.strictEqual(
    isTabletOrMobileDevice(winTouchLaptop),
    false,
    "Windows touchscreen laptop must classify as desktop",
  );
});
...
test("M. global capturing interceptor stops propagation for desktop pen hold", () => {
  ...
  assert.strictEqual(
    ...
    true,
    "Immediate propagation must be stopped",
  );
  resetPointerDown();
});
...
test("N. integration & structural check in platform-input.js, layers.js and index.html", () => {
  const platformSrc = fs.readFileSync(
    path.join(__dirname, "platform-input.js"),
    "utf8",
  );
  ...
  assert.match(
    platformSrc,
    /document\.addEventListener\('contextmenu',\s*function\s*\(e\)\s*\{/,
  );
  ...
  assert.match(
    indexSrc,
    /<script\s+src="src\/core\/platform-input\.js"><\/script>/,
  );
});
