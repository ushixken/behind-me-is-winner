(function () {
  ...
  function render(layer) {
    layer.raf = 0;
    const geometry = resize(layer);
    clear(layer);
    if (!layer.visible || typeof layer.draw !== "function") return;
    layer.context.save();
    layer.context.setTransform(geometry.dpr, 0, 0, geometry.dpr, 0, 0);
    layer.context.globalAlpha = 1;
    layer.context.globalCompositeOperation = "source-over";
    layer.draw(layer.context, geometry);
    layer.context.restore();
  }
  function request(layer) {
    if (!layer.raf)
      layer.raf = requestAnimationFrame(function () {
        render(layer);
      });
  }
  function invalidateAll() {
    layers.forEach(request);
  }
  ...
  function create(id, options) {
    ...
    canvas.style.cssText =
      "position:absolute;inset:0;width:100%;height:100%;display:none;pointer-events:" +
      (options.pointerEvents || "none") +
      ";z-index:" +
      (options.zIndex == null ? 4 : options.zIndex) +
      ";";
    ...
    if (!resizeObserver && window.ResizeObserver) {
      resizeObserver = new ResizeObserver(invalidateAll);
  ...
  if (window.visualViewport)
    visualViewport.addEventListener("resize", invalidateAll);
  window.EditorOverlayRenderer = {
    create: create,
