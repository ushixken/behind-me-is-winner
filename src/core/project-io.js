...
(function(){
  ...
  async function exportProject(){
    ...
    const manifest={format:FORMAT,version:VERSION,createdAt:new Date().toISOString(),document:{name:currentName(),width:CW,height:CH,totalFrames:TOTAL,maxFps:MAX_FPS,framesPerSecond:typeof getFPS==='function'?getFPS():Number(fpsTl.value)||24,backgroundColor:bgColor,currentFrame:curFrame,currentLayer:curLayer,rangeStart,rangeEnd,loopRange:!!loopRange},groups:clone(groups||[]),layers:savedLayers,timeline:{frameLabelOffset:typeof frameLabelOffset==='number'?frameLabelOffset:0},palette:paletteState()};
  ...
  async function commit(staged){
    const old={CW,CH,TOTAL,MAX_FPS,curFrame,curLayer,rangeStart,rangeEnd,loopRange,bgColor,layers,groups,palette:paletteState(),labelOffset:typeof frameLabelOffset==='number'?frameLabelOffset:0},doc=staged.manifest.document;
    try{
      ...
      undoStack=[];redoStack=[];clipboard=null;styleClipboard=null;initCanvas();applyUi(Number(doc.framesPerSecond));if(staged.manifest.palette&&window.PaletteDocker)window.PaletteDocker.load(staged.manifest.palette,{persist:false});loadFrame(curLayer,curFrame);renderLayerPanel();renderTimeline();updateOnion();updateStatus();fitCanvasToView();window.dispatchEvent(new CustomEvent('project-loaded',{detail:{format:FORMAT,version:VERSION,name:window._projectName}}));
    }catch(error){CW=old.CW;CH=old.CH;TOTAL=old.TOTAL;MAX_FPS=old.MAX_FPS;curFrame=old.curFrame;curLayer=old.curLayer;rangeStart=old.rangeStart;rangeEnd=old.rangeEnd;loopRange=old.loopRange;bgColor=old.bgColor;layers=old.layers;groups=old.groups;if(typeof frameLabelOffset==='number')frameLabelOffset=old.labelOffset;initCanvas();if(old.palette&&window.PaletteDocker)window.PaletteDocker.load(old.palette,{persist:false});applyUi(Math.min(Number(fpsTl.value)||24,MAX_FPS));loadFrame(curLayer,curFrame);renderLayerPanel();renderTimeline();throw error;}
  }
