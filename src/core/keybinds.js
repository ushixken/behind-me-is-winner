...
const KEYBIND_DEFAULTS = {
  ...
  flipperBypass: {
    label: "Flipper: Enable Bypass (hold)",
    key: "Shift",
    ctrl: false,
    shift: false,
    alt: false,
  ...
  try {
    ...
    if (raw) {
      ...
      for (const action in saved) {
        ...
        if (
          action === "toolCurve" &&
          _matchesStoredBind(saved[action], {
            key: "c",
            ctrl: false,
            shift: false,
            alt: false,
        ...
        if (legacy && _matchesStoredBind(saved[action], legacy)) {
          migrated = true;
          continue;
        }
...
try {
  _expandedToolGroups =
    JSON.parse(localStorage.getItem(TOOL_GROUP_EXPANDED_KEY) || "{}") || {};
} catch (_) {
  _expandedToolGroups = {};
}
function _toolSubActionId(groupId, subToolId) {
  return "toolSubTool." + groupId + "." + subToolId;
}
function _toolSubAction(group, subTool) {
  return subTool.shortcutActionId || _toolSubActionId(group.id, subTool.id);
}
function _saveExpandedToolGroups() {
  try {
    localStorage.setItem(
      TOOL_GROUP_EXPANDED_KEY,
      JSON.stringify(_expandedToolGroups),
    );
  } catch (_) {}
}
function syncToolGroupKeybindCommands() {
  ...
  let stored = {};
  try {
    stored = JSON.parse(localStorage.getItem(KEYBIND_STORE_KEY) || "{}") || {};
  } catch (_) {}
  ToolGroups.getGroups().forEach((group) => {
    if (group.shortcutActionId && !KEYBIND_DEFAULTS[group.shortcutActionId])
      KEYBIND_DEFAULTS[group.shortcutActionId] = {
        label: group.name + " Tool",
        key: "",
        ctrl: false,
        shift: false,
        alt: false,
      };
    if (group.shortcutActionId && !keybinds[group.shortcutActionId])
      keybinds[group.shortcutActionId] = Object.assign(
        {},
        KEYBIND_DEFAULTS[group.shortcutActionId],
        stored[group.shortcutActionId] || {},
      );
    if (
      group.shortcutActionId === "toolSelection" &&
      keybinds[group.shortcutActionId] &&
      keybinds[group.shortcutActionId].key === ""
    )
      keybinds[group.shortcutActionId].key = "g";
    group.subTools.forEach((subTool) => {
      const action = _toolSubAction(group, subTool),
        definition = {
          label: subTool.name,
          key: "",
          ctrl: false,
      ...
      if (!KEYBIND_DEFAULTS[action]) KEYBIND_DEFAULTS[action] = definition;
      if (!keybinds[action])
        keybinds[action] = Object.assign({}, definition, stored[action] || {});
    });
  });
}
window.addEventListener("tool-groups-ready", () => {
  syncToolGroupKeybindCommands();
});
function handleToolGroupKeybind(event) {
  syncToolGroupKeybindCommands();
  if (!window.ToolGroups || typeof ToolGroups.getGroups !== "function")
    return false;
  const groups = ToolGroups.getGroups();
  for (const group of groups) {
    for (const subTool of group.subTools) {
      const action = _toolSubAction(group, subTool);
      if (keybinds[action] && matchBind(event, action)) {
        ToolGroups.activateSubTool(group.id, subTool.id, {
          fromShortcut: true,
        });
        return true;
      }
  ...
  for (const group of groups) {
    if (
      group.shortcutActionId &&
      keybinds[group.shortcutActionId] &&
      matchBind(event, group.shortcutActionId)
    ) {
      ToolGroups.activateGroup(group.id);
      return true;
    }
  }
  return false;
...
function matchBind(e, action) {
  ...
  let ctrlHeld = KEYBIND_IS_MAC ? !!e.metaKey : !!e.ctrlKey,
    shiftHeld = !!e.shiftKey,
    altHeld = !!e.altKey;
  if (action !== "flipperBypass" && window._flipperBypassHeld) {
    const bypassKey = (keybinds["flipperBypass"] || {}).key || "";
    const rawCtrl = KEYBIND_IS_MAC ? !!e.metaKey : !!e.ctrlKey,
      rawShift = !!e.shiftKey,
      rawAlt = !!e.altKey;
    const bypassModRaw =
      bypassKey === "Shift"
    ...
    if (bypassModRaw) {
      ...
      const rawMatchesThis =
        !!b.ctrl === rawCtrl && !!b.shift === rawShift && !!b.alt === rawAlt;
      if (!rawMatchesThis) {
        ...
        const exactOtherWantsModifier = Object.keys(keybinds).some((a) => {
          ...
          const ob = keybinds[a];
          const obk = _normKey(
            ob.key.length === 1 ? ob.key.toLowerCase() : ob.key,
          );
          return (
            obk === bk &&
            !!ob.ctrl === rawCtrl &&
...
function matchPointerBind(e, action, allowSelectionModes) {
  ...
  if (
    !allowSelectionModes &&
    (!!b.shift !== !!e.shiftKey || !!b.alt !== !!e.altKey)
  )
    return false;
  return true;
}
...
function findBindConflict(action, combo) {
  for (const a in keybinds) {
    ...
    const b = keybinds[a];
    if (
      _normKey(b.key.toLowerCase()) === _normKey(combo.key.toLowerCase()) &&
      !!b.ctrl === !!combo.ctrl &&
      !!b.shift === !!combo.shift &&
      !!b.alt === !!combo.alt
    )
...
function renderKeybindsList() {
  ...
  const matches = (action) => {
    ...
    return (
      ...
      (!q ||
        ...
        formatBind(b).toLowerCase().includes(q))
    );
  };
  function buildRow(action, options) {
    ...
    row.className =
      "modal-row keybinds-row" +
      (opts.child ? " keybinds-tool-child" : "") +
      (opts.disabled ? " disabled" : "");
    const label = document.createElement("span");
    label.className = "keybinds-row-label";
    ...
    meta.className = "keybinds-row-meta";
    if (b.note) {
      const info = document.createElement("span");
      info.className = "keybinds-row-info";
      info.textContent = "ⓘ";
      info.title = b.note;
    ...
    if (opts.status) {
      ...
      status.className = "keybinds-row-status";
      status.textContent = opts.status;
      meta.appendChild(status);
    }
    row.appendChild(meta);
    const wrap = document.createElement("span");
    wrap.className = "keybinds-shortcut-wrap";
    const btn = document.createElement("button");
    btn.className = "modal-btn keybinds-shortcut-button";
    btn.textContent = formatBind(b);
    btn.title = opts.disabled
      ? "Shortcut unavailable until this sub-tool is implemented"
      : "Click, then press a new key combo";
    ...
    if (b.key && !opts.disabled) {
      const clear = document.createElement("button");
      clear.className = "keybinds-clear-button";
      clear.textContent = "✕";
      clear.title = "Remove this shortcut";
      clear.onclick = (event) => {
        event.stopPropagation();
        cancelRebind();
        keybinds[action] = Object.assign({}, keybinds[action], {
          key: "",
          ctrl: false,
          shift: false,
          alt: false,
  ...
  function buildSection(name, actions) {
    const visible = actions.filter(
      (action) => keybinds[action] && matches(action),
    );
    if (!visible.length) return false;
    const forceOpen = !!q,
      collapsed = !forceOpen && !!_collapsedCats[name];
    const header = document.createElement("div");
    header.className = "keybinds-cat-header";
    const arrow = document.createElement("span");
    arrow.className = "keybinds-cat-chevron";
    arrow.textContent = collapsed ? "\u25b6" : "\u25bc";
    const title = document.createElement("span");
    title.textContent = name;
    const count = document.createElement("span");
    count.className = "keybinds-cat-count";
    ...
    if (!collapsed)
      visible.forEach((action) => list.appendChild(buildRow(action)));
    return true;
  }
  function buildToolGroups() {
    if (!window.ToolGroups || typeof ToolGroups.getGroups !== "function")
      return false;
    const visibleGroups = ToolGroups.getGroups()
      .map((group) => {
        const mainAction = group.shortcutActionId,
          children = group.subTools.map((subTool) => ({
            subTool,
            action: _toolSubAction(group, subTool),
          })),
          visibleChildren = children.filter((item) => matches(item.action)),
          parentMatches = mainAction && matches(mainAction);
        return { group, mainAction, children, visibleChildren, parentMatches };
      })
      .filter(
        (item) =>
          item.mainAction &&
          keybinds[item.mainAction] &&
          (!q || item.parentMatches || item.visibleChildren.length),
      );
    if (!visibleGroups.length) return false;
    const categoryExpanded =
        !!q || _expandedToolGroups.__toolGroupsCategory !== false,
    ...
    const title = document.createElement("span");
    title.textContent = "Tool Groups";
    const count = document.createElement("span");
    count.className = "keybinds-cat-count";
    count.textContent = visibleGroups.length;
    header.append(arrow, title, count);
    header.onclick = () => {
      _expandedToolGroups.__toolGroupsCategory = !categoryExpanded;
      _saveExpandedToolGroups();
      renderKeybindsList();
    };
    list.appendChild(header);
    if (!categoryExpanded) return true;
    visibleGroups.forEach(
      ({ group, mainAction, children, visibleChildren, parentMatches }) => {
        const expanded = !!q || _expandedToolGroups[group.id] === true,
          parent = document.createElement("div");
        parent.className = "modal-row keybinds-row keybinds-tool-parent";
        const toggle = document.createElement("button");
        toggle.type = "button";
        toggle.className = "keybinds-tool-toggle";
        toggle.setAttribute("aria-expanded", String(expanded));
        const chevron = document.createElement("span");
        chevron.className = "keybinds-tool-chevron";
        chevron.textContent = expanded ? "\u25bc" : "\u25b6";
        const name = document.createElement("span");
        name.className = "keybinds-tool-name";
        name.textContent = keybinds[mainAction].label;
        toggle.append(chevron, name);
        toggle.onclick = () => {
          _expandedToolGroups[group.id] = !expanded;
          _saveExpandedToolGroups();
          renderKeybindsList();
        };
        const shortcutWrap = document.createElement("span");
        shortcutWrap.className = "keybinds-shortcut-wrap";
        const shortcut = document.createElement("button");
        shortcut.className = "modal-btn keybinds-shortcut-button";
        shortcut.textContent = formatBind(keybinds[mainAction]);
        shortcut.title = "Click, then press a new key combo";
        shortcut.onclick = (event) => {
          event.stopPropagation();
          startRebind(mainAction, shortcut);
        };
        shortcutWrap.appendChild(shortcut);
        if (keybinds[mainAction].key) {
          const clear = document.createElement("button");
          clear.className = "keybinds-clear-button";
          clear.textContent = "✕";
          clear.title = "Remove this shortcut";
          clear.onclick = (event) => {
            event.stopPropagation();
            cancelRebind();
            keybinds[mainAction] = Object.assign({}, keybinds[mainAction], {
              key: "",
              ctrl: false,
              shift: false,
              alt: false,
          ...
          shortcutWrap.appendChild(clear);
        }
        parent.append(toggle, shortcutWrap);
        list.appendChild(parent);
        const displayedChildren =
          q && parentMatches ? children : visibleChildren;
        if (expanded)
          displayedChildren.forEach(({ subTool, action }) => {
            const disabled = subTool.status !== "implemented",
              status =
                subTool.id === "style-select"
                  ? "Smart Raster Only"
                  : disabled
                    ? "Coming Soon"
                    : "";
            list.appendChild(
              buildRow(action, { child: true, disabled, status }),
            );
          });
      },
    );
  ...
  let anyVisible = buildToolGroups(),
    seen = new Set();
  if (window.ToolGroups && typeof ToolGroups.getGroups === "function")
    ToolGroups.getGroups().forEach((group) => {
      if (group.shortcutActionId) seen.add(group.shortcutActionId);
      group.subTools.forEach((subTool) =>
        seen.add(_toolSubAction(group, subTool)),
      );
    });
  KEYBIND_CATEGORIES.forEach((category) => {
    category.actions.forEach((action) => {
      if (keybinds[action]) seen.add(action);
    });
    if (buildSection(category.name, category.actions)) anyVisible = true;
  });
  const leftover = Object.keys(keybinds).filter((action) => !seen.has(action));
  if (leftover.length && buildSection("Other", leftover)) anyVisible = true;
  if (!anyVisible) {
    const empty = document.createElement("div");
    empty.className = "keybinds-empty";
...
function startRebind(action, btn) {
  ...
  btn.textContent = pointerAction
    ? "Press modifiers + click\u2026"
    : "Press a key\u2026";
  ...
  _rebindListener = (e) => {
    ...
    const combo = {
      key: e.key,
      ctrl: KEYBIND_IS_MAC ? e.metaKey : e.ctrlKey,
      shift: e.shiftKey,
      alt: e.altKey,
    };
  ...
  if (pointerAction) {
    ...
    window.addEventListener("pointerdown", _rebindPointerListener, {
      capture: true,
    });
  }
}
...
function cancelRebind() {
  if (_rebindListener)
    window.removeEventListener("keydown", _rebindListener, { capture: true });
  if (_rebindPointerListener)
    window.removeEventListener("pointerdown", _rebindPointerListener, {
      capture: true,
    });
...
const KEYBIND_MENU_LABELS = {
  ...
  copyLayer: "dd-copy",
  pasteLayer: "dd-paste",
  duplicateLayer: "dd-duplicate",
  pasteImage: "dd-paste-image",
  clearFrame: "dd-clear",
  rotateReset: "dd-reset-rotation",
  toggleDrawMode: "dd-toggle-draw-mode",
};
...
document.getElementById("modal-keybinds-reset").onclick = async () => {
  if (
    !(await siteConfirm("Reset all keybinds to their defaults?", {
...
document
  .getElementById("modal-keybinds-import-file")
  .addEventListener("change", (e) => {
    ...
    reader.onload = async () => {
      ...
      const incoming =
        parsed && parsed.keybinds && typeof parsed.keybinds === "object"
          ? parsed.keybinds
          : parsed;
      if (!incoming || typeof incoming !== "object") {
        await siteAlert("That file doesn't look like a keybinds export.", {
          title: "Import Failed",
        });
        return;
      }
      ...
      for (const action in incoming) {
        if (!KEYBIND_DEFAULTS[action] && !keybinds[action]) {
          unknown.push(action);
          continue;
        }
      ...
      if (!importedCount) {
        await siteAlert("No recognizable keybinds were found in that file.", {
          title: "Import Failed",
        });
        return;
      }
      ...
      if (
        !(await siteConfirm(msg, {
          title: "Import Keybinds",
          okText: "Import",
        }))
      )
        return;
      for (const action in clean) {
        if (!keybinds[action])
          keybinds[action] = Object.assign({ label: action }, clean[action]);
    ...
    reader.onerror = async () =>
      await siteAlert("Could not read that file.", { title: "Import Failed" });
...
(function initPrefPsNav() {
  const navItems = document.querySelectorAll(
    "#pref-ps-sidebar .pref-ps-nav-item",
  );
  ...
  function activate(panelId) {
    ...
    if (
      panelId === "storage" &&
      typeof window._renderLocalStorage === "function"
    )
...
document.getElementById("dd-preferences").onclick = () => {
  ...
  (cursorRadio || document.getElementById("pref-cursor-crosshair")).checked =
    true;
  ...
  const storagePanel = document.querySelector(
    '.pref-ps-panel[data-panel="storage"]',
  );
  if (
    storagePanel &&
    storagePanel.classList.contains("active") &&
    typeof window._renderLocalStorage === "function"
  ) {
    window._renderLocalStorage();
  }
...
document.getElementById("pref-cursor-crosshair").onchange = (e) => {
  if (e.target.checked) _setCursorStyle("crosshair");
};
document.getElementById("pref-cursor-point").onchange = (e) => {
  if (e.target.checked) _setCursorStyle("point");
};
document.getElementById("pref-cursor-brush").onchange = (e) => {
  if (e.target.checked) _setCursorStyle("brush");
};
document.getElementById("pref-cursor-brush-shape").onchange = (e) => {
  if (e.target.checked) _setCursorStyle("brush-shape");
};
