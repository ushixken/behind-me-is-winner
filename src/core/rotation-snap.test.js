...
test("A. slow rotation still never snaps (completely free)", () => {
  ...
  for (const raw of slowSequence) {
    ...
    assert.strictEqual(
      res.displayAngle,
      raw,
      `Slow rotation at ${raw}° must remain free`,
    );
    assert.strictEqual(
      res.isSnapped,
      false,
      `Slow rotation at ${raw}° isSnapped must be false`,
    );
    assert.strictEqual(
      res.snapTarget,
      null,
      `Slow rotation at ${raw}° snapTarget must be null`,
    );
...
test("B. flick that slightly overshoots 0° still chooses 0°", () => {
  ...
  assert.strictEqual(
    res.snapTarget,
    0,
    "Overshoot to -18° must catch 0° target",
  );
  assert.strictEqual(
    res.displayAngle,
    -18,
    "Display angle during drag tracks raw angle 1:1",
  );
});
...
test("C. flick that overshoots 45° still chooses 45°", () => {
  ...
  assert.strictEqual(
    res.snapTarget,
    45,
    "Overshoot to 61° must catch 45° target",
  );
});
...
test("H. projected target remains continuous at 360°", () => {
  ...
  assert.strictEqual(
    res.snapTarget,
    360,
    "Flick overshooting 360° to 372° must target 360°",
  );
});
...
test("L. springProgress properties: starts at 0, finishes at 1, has subtle overshoot", () => {
  ...
  assert.ok(
    maxProgress > 1.02 && maxProgress < 1.25,
    `Overshoot should be subtle (got ${maxProgress})`,
  );
});
...
test("O. integration & structural check in core-state.js", () => {
  const coreStateSrc = fs.readFileSync(
  ...
  assert.match(
    coreStateSrc,
    /function\s+_startRotateSpringSettle\(startAngle,\s*targetAngle,\s*pivotX,\s*pivotY\)/,
  );

  // Verify cancel is called on drag starts and reset
  assert.match(
    coreStateSrc,
    /_spaceDragStart[^{]*\{[\s\S]*?_cancelRotateSettle\(\);/,
  );
  assert.match(
    coreStateSrc,
    /_spaceDragStartXY[^{]*\{[\s\S]*?_cancelRotateSettle\(\);/,
  );
  assert.match(
    coreStateSrc,
    /resetRotation[^{]*\{[\s\S]*?_cancelRotateSettle\(\);/,
  );
  ...
  assert.match(
    coreStateSrc,
    /_startRotateSpringSettle\(currentRot,\s*settleTarget,\s*settlePivotX,\s*settlePivotY\)/,
  );

  // Verify rotateCanvasTo receives the captured fixed pivot
  assert.match(
    coreStateSrc,
    /rotateCanvasTo\(currentAngle,\s*pivotX,\s*pivotY\)/,
  );
});
