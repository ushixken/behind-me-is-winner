...
const uiControlsSrc = fs.readFileSync(
  path.join(__dirname, "..", "ui", "ui-controls.js"),
  "utf8",
);
...
function createZoomEnvironment(store = {}) {
  ...
  function setZoomSpeedLevel(level) {
    ...
    try {
      mockLocalStorage.setItem("animator_zoom_speed", String(zoomSpeedLevel));
    } catch (_) {}
...
test("H. zoom-to-cursor / camera anchor logic math is preserved", () => {
  ...
  assert.ok(
    Math.abs(canvasX0 - canvasX1) < 1e-10,
    "Canvas X under cursor fixed",
  );
  assert.ok(
    Math.abs(canvasY0 - canvasY1) < 1e-10,
    "Canvas Y under cursor fixed",
  );
});
...
test("J. UI and structural bindings match specification", () => {
  ...
  assert.match(
    indexHtml,
    /id="zoom-speed-input"[^>]*min="1"[^>]*max="10"[^>]*step="1"/,
  );
  // layers.js loads zoomSpeedLevel into zoom-speed-input
  assert.match(
    layersSrc,
    /document\.getElementById\('zoom-speed-input'\)\.value\s*=\s*zoomSpeedLevel;/,
  );
  ...
  assert.match(
    coreStateSrc,
    /window\.clampZoomSpeedLevel\s*=\s*clampZoomSpeedLevel/,
  );
  assert.match(
    coreStateSrc,
    /window\.setZoomSpeedLevel\s*=\s*setZoomSpeedLevel/,
  );
});
