(function () {
  ...
  function makeAdvancedStyle(rgba, id, name) {
    ...
    const safeRgba = [0, 1, 2, 3].map((i) =>
      Math.max(
        0,
        Math.min(
          255,
          Math.round(Number(values[i] ?? (i === 3 ? 255 : 0)) || 0),
        ),
      ),
    );
    return {
      id: id || makeId("style"),
      type: "style",
      name: String(name || "").trim() || nextAdvancedStyleName(),
      rgba: safeRgba,
      locked: false,
      visible: true,
    };
  }
  function makeAdvancedSeparator(id) {
    return { id: id || makeId("advsep"), type: "separator" };
  }
  function isAdvancedSeparator(item) {
    return !!item && item.type === "separator";
  }
  function styleHex(style) {
    const c = style && Array.isArray(style.rgba) ? style.rgba : [0, 0, 0, 255];
  ...
  function sanitizeAdvancedStyles(input) {
    ...
    return list.map((style, i) => {
      ...
      const made = makeAdvancedStyle(
        style && style.rgba,
        style && style.id,
        (style && style.name) || "color_" + (i + 1),
      );
  ...
  function nextAdvancedStyleName() {
    ...
    const names = new Set(
      advancedStyles
        .filter((s) => !isAdvancedSeparator(s))
        .map((s) => String(s.name || "").toLowerCase()),
    );
    while (names.has(("color_" + n).toLowerCase())) n++;
    return "color_" + n;
  }
  function activeAdvancedStyle() {
    return (
      advancedStyles.find(
        (s) => !isAdvancedSeparator(s) && s.id === activeAdvancedStyleId,
      ) ||
      advancedStyles.find((s) => !isAdvancedSeparator(s)) ||
      null
    );
  }
  function activeLayerUsesAdvancedPalette() {
  ...
  function isTransparentHex(hex) {
    return (
      /^#[0-9a-f]{8}$/i.test(String(hex || "").trim()) &&
      String(hex).slice(7, 9).toLowerCase() === "00"
    );
  }
  function isValidHex(text) {
    return /^#[0-9a-f]{6}([0-9a-f]{2})?$/i.test(String(text || "").trim());
  }
  ...
  function rgbaArray(hex) {
    ...
    return [
      ...
      rgba.g,
      rgba.b,
      Math.round((Number.isFinite(+rgba.a) ? rgba.a : 1) * 255),
    ];
  }
  ...
  function currentForegroundHex() {
    return normalizeHex(typeof color !== "undefined" ? color : "#000000").slice(
      0,
      7,
    );
  }
  ...
  function makeSwatch(hex, id, name) {
    ...
    const swatch = {
      ...
      type: "swatch",
      hex: safeHex,
      rgba: hexToRgba(safeHex),
    };
    const cleanName = String(name || "").trim();
    if (cleanName) swatch.name = cleanName;
    return swatch;
  }
  function makeSeparator(id) {
    return { id: id || makeId("sep"), type: "separator" };
  }
  function makeSpacer(id) {
    return { id: id || makeId("space"), type: "spacer" };
  }
  function isSeparator(item) {
    return !!item && item.type === "separator";
  }
  function isSpacer(item) {
    return !!item && item.type === "spacer";
  }
  function isSwatch(item) {
    return !!item && item.type !== "separator" && item.type !== "spacer";
  }
  function cloneSwatch(swatch) {
    return isSeparator(swatch)
      ? makeSeparator()
      : isSpacer(swatch)
        ? makeSpacer()
        : makeSwatch(swatch.hex, null, swatch && swatch.name);
  }
  function defaultSwatches() {
    return defaultHexes.map((hex) => makeSwatch(hex));
  }
  function createDefaultPaletteState() {
    ...
    return {
      ...
      advancedPalette: {
        version: ADVANCED_PALETTE_VERSION,
        enabled: false,
        styles: [],
        activeStyleId: null,
      },
      view: {
        normalPaletteSwatchSize: swatchSize,
        advancedPaletteSwatchSize,
        scrollTop: 0,
        toolbarPaletteAttachment: null,
        toolbarPaletteVisible: false,
        advancedPaletteEnabled: false,
      },
    };
  }
  function sanitizePaletteItem(item) {
    ...
    const hex =
      item.hex ||
      rgbaToHex(
        item.rgba && item.rgba[0],
        item.rgba && item.rgba[1],
        item.rgba && item.rgba[2],
        item.rgba && item.rgba[3],
      );
  ...
  function nextPaletteName() {
    let n = 1;
    const names = new Set(
      palettes.map((p) => String(p.name || "").toLowerCase()),
    );
  ...
  function defaultAdvancedPaletteName(name) {
    ...
    return generated
      ? "Advanced Palette " + generated[1]
      : normalName || "Advanced Palette 1";
  }
  ...
  function activePaletteMode() {
    return activeLayerUsesAdvancedPalette() ? "advanced" : "normal";
  }
  function uniquePaletteName(name, excludeId, mode) {
    ...
    const names = new Set(
      palettes
        ...
        .map((p) =>
          String(
            paletteNameForMode(p, advanced ? "advanced" : "normal") || "",
          ).toLowerCase(),
        ),
    );
  ...
  function makePalette(
    ...
    const selected =
      selection && cleanSwatches.some((s) => s.id === selection)
        ? selection
        : cleanSwatches[0]
          ? cleanSwatches[0].id
          : null;
    const normalName = String(name || "").trim() || nextPaletteName();
    const storedAdvancedName =
      String(advancedName || "").trim() ||
      defaultAdvancedPaletteName(normalName);
    return {
      id: id || makeId("palette"),
      name: normalName,
      advancedName: storedAdvancedName,
  ...
  function activePalette() {
    return (
      palettes.find((p) => p.id === activePaletteId) || palettes[0] || null
    );
  }
  function loadAdvancedColorHistories() {
    try {
      const saved = JSON.parse(
        localStorage.getItem(ADVANCED_COLOR_HISTORY_KEY) || "{}",
      );
  ...
  function advancedColorHistory(paletteId) {
    ...
    return Array.isArray(histories[paletteId])
      ? histories[paletteId]
          .filter(isValidHex)
          .map(normalizeHex)
          .slice(0, ADVANCED_COLOR_HISTORY_LIMIT)
      : [];
  }
  function pushAdvancedColorHistory(paletteId, hex) {
    const clean = normalizeHex(hex);
    if (!paletteId || !isValidHex(clean)) return false;
    const histories = loadAdvancedColorHistories();
    const history = Array.isArray(histories[paletteId])
      ? histories[paletteId].filter(isValidHex).map(normalizeHex)
      : [];
    if (history[0] === clean) return false;
    const duplicate = history.indexOf(clean);
    if (duplicate >= 0) history.splice(duplicate, 1);
    history.unshift(clean);
    histories[paletteId] = history.slice(0, ADVANCED_COLOR_HISTORY_LIMIT);
    try {
      localStorage.setItem(
        ADVANCED_COLOR_HISTORY_KEY,
        JSON.stringify(histories),
      );
    } catch (e) {}
    advancedColorHistorySubscribers.forEach((callback) => {
      try {
        callback(histories[paletteId].slice());
      } catch (e) {}
    });
    return true;
  }
  const AdvancedColorHistory = {
    record(rgba) {
      const hex = Array.isArray(rgba)
        ? rgbaToHex(rgba[0], rgba[1], rgba[2], rgba[3])
        : normalizeHex(rgba);
      return pushAdvancedColorHistory(
        advancedColorPanelPaletteId || activePaletteId,
        hex,
      );
    },
    getAll() {
      return advancedColorHistory(
        advancedColorPanelPaletteId || activePaletteId,
      ).slice();
    },
    subscribe(callback) {
      if (typeof callback !== "function") return () => {};
      advancedColorHistorySubscribers.add(callback);
      return () => advancedColorHistorySubscribers.delete(callback);
    },
  };
  ...
  function syncActiveRefs() {
    ...
    selectedId =
      active.selectedId && swatches.some((s) => s.id === active.selectedId)
        ? active.selectedId
        : null;
  ...
  function persist(options) {
    ...
    if (!options || options.markDirty !== false) {
      if (typeof window.markProjectDirty === "function")
        window.markProjectDirty("palette");
    }
  }
  function persistView() {
    ...
    try {
      ...
      localStorage.setItem(
        ADVANCED_SWATCH_SIZE_KEY,
        String(advancedPaletteSwatchSize),
      );
      localStorage.setItem(
        VIEW_KEY,
        JSON.stringify({
          ...
          toolbarPaletteAttachment: toolbarPaletteAttachment
            ? Object.assign({}, toolbarPaletteAttachment)
            : null,
          toolbarPaletteVisible,
          advancedPaletteEnabled,
        }),
      );
    } catch (e) {}
  }
  function loadView() {
    try {
      ...
      const hasViewNormal =
        view && Number.isFinite(+view.normalPaletteSwatchSize);
      if (hasStoredNormal) swatchSize = clampSwatchSize(+storedNormal);
      else if (hasViewNormal)
        swatchSize = clampSwatchSize(+view.normalPaletteSwatchSize);
      else if (view && Number.isFinite(+view.swatchSize))
        swatchSize = clampSwatchSize(+view.swatchSize);
      if (Number.isFinite(+storedAdvanced))
        advancedPaletteSwatchSize = clampSwatchSize(+storedAdvanced);
      else if (view && Number.isFinite(+view.advancedPaletteSwatchSize))
        advancedPaletteSwatchSize = clampSwatchSize(
          +view.advancedPaletteSwatchSize,
        );
      if (view && Number.isFinite(+view.scrollTop))
        savedScrollTop = Math.max(0, +view.scrollTop);
      toolbarPaletteAttachment =
        view && view.toolbarPaletteAttachment
          ? Object.assign({}, view.toolbarPaletteAttachment)
          : null;
      toolbarPaletteVisible = !!(
        toolbarPaletteAttachment &&
        (view &&
        Object.prototype.hasOwnProperty.call(view, "toolbarPaletteVisible")
          ? view.toolbarPaletteVisible
          : true)
      );
  ...
  function serialize() {
    ...
    return {
      ...
      palettes: palettes.map((p) => ({
        id: p.id,
        name: p.name,
        advancedName: p.advancedName,
        swatches: p.swatches.map(exportSwatchData),
        selectedId: p.selectedId || null,
      })),
      activePaletteId,
      advancedPalette: {
        version: ADVANCED_PALETTE_VERSION,
        enabled: advancedPaletteEnabled,
        styles: advancedStyles,
        activeStyleId: activeAdvancedStyleId,
      },
      view: {
        normalPaletteSwatchSize: swatchSize,
        advancedPaletteSwatchSize,
  ...
  function load(data, options) {
    ...
    if (payload && Array.isArray(payload.palettes)) {
      palettes = payload.palettes
        ...
        .filter(Boolean);
      if (!palettes.length)
        palettes = [makePalette("Palette 1", defaultSwatches())];
      activePaletteId = palettes.some((p) => p.id === payload.activePaletteId)
        ? payload.activePaletteId
        : palettes[0].id;
    } else {
      palettes = [
        makePalette(
          "Palette 1",
          payload && Array.isArray(payload.swatches) ? payload.swatches : null,
          null,
          payload && payload.selectedId,
          false,
        ),
      ];
    ...
    if (payload && payload.view) {
      if (Number.isFinite(+payload.view.normalPaletteSwatchSize))
        swatchSize = clampSwatchSize(+payload.view.normalPaletteSwatchSize);
      else if (Number.isFinite(+payload.view.swatchSize))
        swatchSize = clampSwatchSize(+payload.view.swatchSize);
      if (Number.isFinite(+payload.view.advancedPaletteSwatchSize))
        advancedPaletteSwatchSize = clampSwatchSize(
          +payload.view.advancedPaletteSwatchSize,
        );
    }
    if (payload && payload.view && Number.isFinite(+payload.view.scrollTop))
      savedScrollTop = Math.max(0, +payload.view.scrollTop);
    if (
      payload &&
      payload.view &&
      ...
      toolbarPaletteAttachment = payload.view.toolbarPaletteAttachment
        ? Object.assign({}, payload.view.toolbarPaletteAttachment)
        : null;
    if (
      payload &&
      payload.view &&
      ...
      toolbarPaletteVisible = !!(
        payload.view.toolbarPaletteVisible && toolbarPaletteAttachment
      );
    else if (
      payload &&
      payload.view &&
      ...
      toolbarPaletteVisible = !!toolbarPaletteAttachment;
    const advancedPayload = payload && payload.advancedPalette;
    advancedPaletteEnabled =
      !!(advancedPayload && advancedPayload.enabled) ||
      (payload && payload.view && !!payload.view.advancedPaletteEnabled);
    advancedPaletteVersion = Number.isFinite(
      +(advancedPayload && advancedPayload.version),
    )
      ? +advancedPayload.version
      : 0;
    advancedStyles = sanitizeAdvancedStyles(
      advancedPayload && advancedPayload.styles,
    );
    activeAdvancedStyleId = advancedPayload && advancedPayload.activeStyleId;
    if (advancedPaletteEnabled && !advancedStyles.length)
      initializeAdvancedPaletteFromActivePalette();
    migrateAdvancedPaletteIfNeeded();
    syncAdvancedRefs();
    restoreScrollPending = true;
    applyViewSettings(false);
    synchronizeActiveContext(false, {
      markDirty: options ? options.markDirty : false,
    });
    render();
  ...
  function loadPersisted() {
    ...
    if (
      saved &&
      (Array.isArray(saved.swatches) || Array.isArray(saved.palettes))
    ) {
      load(saved, { persist: false, markDirty: false });
      return;
    }
  ...
  function setForeground(hex, openPicker, skipPaletteRender) {
    ...
    if (typeof window.setForegroundColorFromPalette === "function") {
      ...
      try {
        window.setForegroundColorFromPalette(
          safeHex,
          !!openPicker,
          !!skipPaletteRender,
        );
      } finally {
        isSyncingColorPanel = wasSyncing;
      }
    } else {
  ...
  function setForegroundFromSample(hex) {
    setForeground(hex, false, true);
  }
  ...
  function syncPaletteSelectionToForeground(hex, source) {
    ...
    const match = swatches.find(
      (item) => isSwatch(item) && normalizeHex(item.hex) === safeHex,
    );
  ...
  function activePaletteSwatchSize() {
    return activePaletteMode() === "advanced"
      ? advancedPaletteSwatchSize
      : swatchSize;
  }
  function applyViewSettings(keepSelectedVisible) {
    ...
    if (body) {
      ...
      body.style.setProperty(
        "--advanced-palette-swatch-size",
        advancedPaletteSwatchSize + "px",
      );
    }
  ...
  function stepSwatchSize(delta) {
    setSwatchSize(activePaletteSwatchSize() + delta * SWATCH_SIZE_STEP, true);
  }
  function ensureSelectedVisible() {
    const grid = document.getElementById("palette-grid");
    if (!grid || !selectedId) return;
    const selected = grid.querySelector(
      '.palette-swatch[data-id="' + CSS.escape(selectedId) + '"]',
    );
    if (selected)
  ...
  function toolbarPaletteEnabled() {
    return !!(
      toolbarPaletteVisible &&
      toolbarPaletteAttachment &&
      palettes.some((p) => p.id === toolbarPaletteAttachment.paletteId) &&
      (toolbarPaletteAttachment.mode === "normal" ||
        toolbarPaletteAttachment.mode === "advanced")
    );
  }
  function currentPaletteDockerMode() {
    const body = document.getElementById("palette-body");
    return body && body.classList.contains("advanced-mode")
      ? "advanced"
      : "normal";
  }
  function currentToolbarPaletteAttachment() {
    return { paletteId: activePaletteId, mode: currentPaletteDockerMode() };
  }
  function currentPaletteIsAttached() {
    const current = currentToolbarPaletteAttachment();
    return (
      toolbarPaletteEnabled() &&
      toolbarPaletteAttachment.paletteId === current.paletteId &&
      toolbarPaletteAttachment.mode === current.mode
    );
  }
  ...
  function renderToolbarPalette() {
    ...
    if (toolbarPaletteAttachment.mode === "advanced") {
      ...
      advancedStyles.forEach((style) => {
        ...
        btn.addEventListener("pointerdown", (event) => {
          if (
            window.LinkedPixelSelection &&
            window.LinkedPixelSelection.handleStylePointer(event, style.id)
          )
            btn._linkedSelectionClick = true;
        });
        btn.addEventListener("click", (event) => {
          event.preventDefault();
          if (btn._linkedSelectionClick) {
            btn._linkedSelectionClick = false;
            return;
          }
    ...
    const attached =
      palettes.find((p) => p.id === toolbarPaletteAttachment.paletteId) || null;
    const items =
      attached && Array.isArray(attached.swatches) ? attached.swatches : [];
    items.forEach((item) => {
      if (isSeparator(item)) {
        const gap = document.createElement("span");
        gap.className = "toolbar-palette-separator";
      ...
      btn.addEventListener("click", (event) => {
        ...
        if (attached.id === activePaletteId) {
          selectedId = item.id;
          syncSelectionClasses();
        }
  ...
  function selectedSwatch() {
    const item = swatches.find((s) => s.id === selectedId) || null;
    return isSwatch(item) ? item : null;
  }
  function selectedItem() {
    return swatches.find((s) => s.id === selectedId) || null;
  }
  function swatchIndex(id) {
    return swatches.findIndex((s) => s.id === id);
  }
  ...
  function activatePaletteSwatch(swatchId, options) {
    ...
    if (opts.setForeground && isSwatch(swatch))
      setForeground(swatch.hex, false, !!opts.skipPaletteRender);
  ...
  function selectSwatch(swatch, applyColor) {
    ...
    advancedColorPanelPaletteId = null;
    if (!swatch) return;
    activatePaletteSwatch(swatch.id, {
      select: true,
      setForeground: !!applyColor,
    });
    render();
  }
  function sideMenu() {
    return document.getElementById("palette-side-menu");
  }
  function toolbarPaletteSubmenu() {
    return document.getElementById("palette-toolbar-submenu");
  }
  function toolbarPaletteTrigger() {
    return document.getElementById("palette-toolbar-toggle");
  }
  function setToolbarPaletteAttachment(paletteId, mode) {
    if (
      !palettes.some((p) => p.id === paletteId) ||
  ...
  function renderToolbarPaletteSubmenu() {
    ...
    palettes.forEach((palette) => {
      const on =
        toolbarPaletteEnabled() &&
        toolbarPaletteAttachment.paletteId === palette.id &&
        toolbarPaletteAttachment.mode === "normal";
      addButton(
        palette.name,
        () => setToolbarPaletteAttachment(palette.id, "normal"),
        on,
      );
    });
    addSeparator();
    ...
    palettes.forEach((palette) => {
      const on =
        ...
        toolbarPaletteAttachment.mode === "advanced";
      addButton(
        palette.advancedName,
        () => setToolbarPaletteAttachment(palette.id, "advanced"),
        on,
      );
    });
  }
  function positionToolbarPaletteSubmenu() {
    ...
    const maxTop = Math.max(
      0,
      window.innerHeight - menuRect.top - submenu.offsetHeight - 6,
    );
  ...
  function toggleToolbarPaletteSubmenu() {
    toolbarSubmenuOpen
      ? closeToolbarPaletteSubmenu()
      : openToolbarPaletteSubmenu();
  }
  function paletteDropdown() {
    return document.getElementById("palette-dropdown");
  }
  function settingsButton() {
    return document.getElementById("palette-settings");
  }
  function activeDisplay() {
    return document.getElementById("palette-active-display");
  }
  function positionPaletteDropdown() {
    ...
    if (!menu || !button || !panel || panel.classList.contains("fp-hidden")) {
    ...
    const top = Math.max(
      margin,
      Math.min(window.innerHeight - height - margin, rect.bottom + 2),
    );
  ...
  function requestPaletteDropdownPosition() {
    ...
    dropdownFrame = requestAnimationFrame(() => {
      dropdownFrame = null;
  ...
  function closePaletteDropdown() {
    ...
    dropdownOpen = false;
    if (dropdownFrame) {
      cancelAnimationFrame(dropdownFrame);
      dropdownFrame = null;
    }
    if (menu) menu.classList.add("hidden");
  }
  function togglePaletteDropdown() {
    dropdownOpen ? closePaletteDropdown() : openPaletteDropdown();
  }
  function positionSideMenu() {
    ...
    if (!menu || !panel || panel.classList.contains("fp-hidden")) {
      closeSideMenu();
      return;
    }
    ...
    const margin = 6;
    const useLeft =
      panelRect.right + menuWidth + gap > window.innerWidth - margin &&
      panelRect.left - menuWidth - gap >= margin;
    const left = useLeft
      ? Math.max(margin, panelRect.left - menuWidth - gap)
      : Math.min(window.innerWidth - menuWidth - margin, panelRect.right + gap);
    const idealTop = buttonRect.top;
    const top = Math.max(
      margin,
      Math.min(window.innerHeight - menuHeight - margin, idealTop),
    );
    menu.style.left = left + "px";
    menu.style.top = top + "px";
  ...
  function requestSideMenuPosition() {
    ...
    sideMenuFrame = requestAnimationFrame(() => {
      sideMenuFrame = null;
      positionSideMenu();
    });
  }
  ...
  function syncStyleLayeringControl() {
    ...
    if (!input) return;
    const state = window.SmartRasterStyleLayering
      ? window.SmartRasterStyleLayering.getActiveState()
      : {
          enabled: false,
          available: false,
          reason: "Style Layering is unavailable.",
        };
    ...
    if (control)
      control.title = state.enabled
        ? "Render this Smart Raster layer in Advanced Palette order."
        : state.available
          ? "Render this Smart Raster layer in Advanced Palette order."
          : "Unavailable: " +
            String(state.reason || "the active layer is not compatible.");
  }
  function closeSideMenu() {
    ...
    sideMenuOpen = false;
    if (sideMenuFrame) {
      cancelAnimationFrame(sideMenuFrame);
      sideMenuFrame = null;
    }
    if (menu) menu.classList.add("hidden");
    closeToolbarPaletteSubmenu();
    ...
    window.TransientManager.register({
      ...
      contains: (target) => {
        ...
        return !!(
          ...
          (settings && settings.contains(target))
        );
      },
    });
    window.TransientManager.register({
      id: "palette-dropdown",
      isOpen: () => !!dropdownOpen,
      close: () => closePaletteDropdown(),
      contains: (target) => {
        if (paletteDragState) return true;
        const dropdown = paletteDropdown();
        const display = activeDisplay();
        return !!(
          (dropdown && dropdown.contains(target)) ||
          (display && display.contains(target))
        );
      },
    });
    window.TransientManager.register({
      id: "palette-context-menu",
      isOpen: () => !!document.getElementById("palette-context-menu"),
      close: () => hideContextMenu(),
      contains: (target) =>
        !!(
          document.getElementById("palette-context-menu") &&
          document.getElementById("palette-context-menu").contains(target)
        ),
    });
  }
  ...
  function bindPalettePopupListeners() {
    ...
    document.addEventListener("pointermove", () => {
      ...
      requestPaletteDropdownPosition();
    });
  ...
  function renderPaletteList() {
    ...
    palettes.forEach((p) => {
      ...
      item.addEventListener("pointerdown", (event) =>
        beginPaletteListPointer(event, p.id, item),
      );
  ...
  function onPaletteListPointerMove(event) {
    if (!paletteDragState || event.pointerId !== paletteDragState.pointerId)
      return;
    ...
    if (!paletteDragState.active) {
      ...
      if (paletteDragState.sourceEl && !paletteDragState.captured) {
        try {
          paletteDragState.sourceEl.setPointerCapture(event.pointerId);
          paletteDragState.captured = true;
        } catch (e) {}
      }
      if (paletteDragState.sourceEl)
        paletteDragState.sourceEl.classList.add("dragging");
    }
    event.preventDefault();
    clearPaletteListIndicators();
    const target = document.elementFromPoint(event.clientX, event.clientY);
    const item =
      target && target.closest ? target.closest(".palette-list-item") : null;
    if (!item || item.dataset.id === paletteDragState.id) return;
    const rect = item.getBoundingClientRect();
    paletteDragState.overId = item.dataset.id;
    paletteDragState.side =
      event.clientY < rect.top + rect.height / 2 ? "before" : "after";
    item.classList.add(
      paletteDragState.side === "before" ? "insert-before" : "insert-after",
    );
  }
  ...
  function finishPaletteListPointer(event, cancelled) {
    ...
    if (state.sourceEl && state.captured) {
      try {
        if (
          !state.sourceEl.hasPointerCapture ||
          state.sourceEl.hasPointerCapture(state.pointerId)
        )
    ...
    clearPaletteListIndicators();
    document
      .querySelectorAll(".palette-list-item.dragging")
      .forEach((node) => node.classList.remove("dragging"));
    if (state.active && !cancelled) {
      if (event) event.preventDefault();
      if (state.overId && state.overId !== state.id)
        reorderPalette(state.id, state.overId, state.side);
      return;
    }
    if (cancelled) return;
  ...
  function clearPaletteListIndicators() {
    document
      .querySelectorAll(
        ".palette-list-item.insert-before,.palette-list-item.insert-after",
      )
      .forEach((node) =>
  ...
  function reorderPalette(id, overId, side) {
    ...
    activePaletteId = palettes.some((p) => p.id === keepActiveId)
      ? keepActiveId
      : id;
    syncActiveRefs();
    render();
  ...
  function renderPaletteSelector() {
    ...
    if (display) {
      ...
      display.setAttribute(
        "aria-label",
        active ? "Active palette: " + activeName : "Active palette",
      );
    }
    ...
    if (advancedToggle) {
      ...
      advancedToggle.textContent =
        "Advanced Palette: " + (advancedPaletteEnabled ? "On" : "Off");
    }
    renderPaletteList();
    requestSideMenuPosition();
  }
  function setAdvancedPaletteEnabled(enabled) {
    ...
    if (nextEnabled && !advancedPaletteEnabled)
      initializeAdvancedPaletteFromActivePalette();
  ...
  function toggleAdvancedPalette() {
    setAdvancedPaletteEnabled(!advancedPaletteEnabled);
  }
  function normalPaletteColorCount() {
    const active = activePalette();
    return active && Array.isArray(active.swatches)
      ? active.swatches.filter(isSwatch).length
      : 0;
  }
  function isDefaultBlackWhiteAdvancedStyles(styles) {
    const list = Array.isArray(styles)
      ? styles.filter((s) => !isAdvancedSeparator(s))
      : [];
    if (list.length !== 2 || styles.length !== 2) return false;
    const first = list[0],
      second = list[1];
    return (
      String(first.name || "").toLowerCase() === "color_1" &&
  ...
  function initializeAdvancedPaletteFromActivePalette(force) {
    ...
    const active = activePalette();
    const source =
      active && Array.isArray(active.swatches) ? active.swatches : [];
    let colorIndex = 1;
    advancedStyles = source.reduce((items, item) => {
      if (isSeparator(item)) {
        items.push(makeAdvancedSeparator());
        return items;
      }
      if (!isSwatch(item)) return items;
      items.push(
        makeAdvancedStyle(rgbaArray(item.hex), null, "color_" + colorIndex++),
      );
      return items;
    }, []);
    if (!advancedStyles.some((s) => !isAdvancedSeparator(s))) {
      defaultSwatches().forEach((item) =>
        advancedStyles.push(
          makeAdvancedStyle(rgbaArray(item.hex), null, "color_" + colorIndex++),
        ),
      );
  ...
  function migrateAdvancedPaletteIfNeeded() {
    ...
    if (!shouldMigrateAdvancedPaletteDefaults()) {
      advancedPaletteVersion = Math.max(
        advancedPaletteVersion || 0,
        ADVANCED_PALETTE_VERSION,
      );
      return false;
    }
  ...
  function syncAdvancedStyleToColorPanel(style) {
    ...
    isSyncingColorPanel = true;
    try {
      setForeground(styleHex(style), false, true);
    } finally {
      isSyncingColorPanel = false;
    }
  }
  function scheduleAdvancedSelectionPersist(options) {
    if (options && options.persist === false) return;
    if (advancedSelectionPersistTimer !== null)
      clearTimeout(advancedSelectionPersistTimer);
  ...
  function selectStyle(id, setBrush, paletteId, options) {
    const style = advancedStyles.find(
      (s) => !isAdvancedSeparator(s) && s.id === id,
    );
    if (!style) return;
    ...
    const next =
      grid &&
      grid.querySelector(
        '.palette-style-card[data-id="' + CSS.escape(style.id) + '"]',
      );
      ...
      if (activeLayerUsesAdvancedPalette()) {
        ...
        if (swatch)
          activatePaletteSwatch(
            swatch.id,
            Object.assign(
              { select: true, setForeground: true, skipPaletteRender: true },
              syncOptions,
            ),
          );
      }
  ...
  function selectMatchingRgba(red, green, blue, alpha) {
    ...
    if (currentPaletteDockerMode() === "advanced") {
      syncAdvancedRefs();
      const match = advancedStyles.find(
        (style) => !isAdvancedSeparator(style) && equal(style.rgba),
      );
      if (!match) return false;
      selectStyle(match.id, true, activePaletteId);
      render();
      return true;
    }
    ...
    const match =
      palette &&
      palette.swatches.find(
        (item) => isSwatch(item) && equal(rgbaArray(item.hex)),
      );
    if (!match) {
      if (selectedId !== null) {
        selectedId = null;
  ...
  function selectAdvancedStyleById(styleId) {
    ...
    syncAdvancedRefs();
    const style = advancedStyles.find(
      (item) => !isAdvancedSeparator(item) && item.id === styleId,
    );
  ...
  function createAdvancedStyle() {
    syncAdvancedRefs();
    const style = makeAdvancedStyle(
      rgbaArray(currentForegroundHex()),
      null,
      nextAdvancedStyleName(),
    );
  ...
  function findAdvancedStyleById(id) {
    return (
      advancedStyles.find((s) => !isAdvancedSeparator(s) && s.id === id) || null
    );
  }
  function refreshAdvancedStyleChange(style, colorChanged) {
    if (style && style.id === activeAdvancedStyleId)
      syncAdvancedStyleToColorPanel(style);
    render();
    renderToolbarPalette();
    persist();
    if (colorChanged) {
      window.dispatchEvent(
        new CustomEvent("advanced-palette-style-color-changed", {
          detail: { styleId: style.id },
        }),
      );
    }
  }
  function dispatchPendingAdvancedStyleColors() {
    ...
    styleIds.forEach((id) =>
      window.dispatchEvent(
        new CustomEvent("advanced-palette-style-color-changed", {
          detail: { styleId: id, interactive: true },
        }),
      ),
    );
    clearTimeout(advancedColorSettleTimer);
    advancedColorSettleTimer = setTimeout(() => {
      advancedColorSettleTimer = null;
      render();
      persist();
    }, 120);
  }
  function queueAdvancedStyleColorChange(styleId) {
    ...
    activeAdvancedColorStyleIds.add(styleId);
    if (advancedColorFrame !== null || advancedColorTimer !== null) return;
    const delay = Math.max(
      0,
      33 - (performance.now() - advancedColorLastDispatch),
    );
    if (delay > 1)
      advancedColorTimer = setTimeout(() => {
        advancedColorTimer = null;
        advancedColorFrame = requestAnimationFrame(
          dispatchPendingAdvancedStyleColors,
        );
      }, delay);
    else
      advancedColorFrame = requestAnimationFrame(
  ...
  function flushAdvancedStyleColorChange() {
    if (advancedColorFrame !== null) {
      cancelAnimationFrame(advancedColorFrame);
      advancedColorFrame = null;
    }
    if (advancedColorTimer !== null) {
      clearTimeout(advancedColorTimer);
      advancedColorTimer = null;
    }
    advancedColorLastDispatch = performance.now();
    const styleIds = Array.from(activeAdvancedColorStyleIds);
    pendingAdvancedColorStyleIds.clear();
    activeAdvancedColorStyleIds.clear();
    styleIds.forEach((id) =>
      window.dispatchEvent(
        new CustomEvent("advanced-palette-style-color-changed", {
          detail: { styleId: id, interactive: false },
        }),
      ),
    );
    if (advancedColorSettleTimer !== null) {
      clearTimeout(advancedColorSettleTimer);
      advancedColorSettleTimer = null;
    }
    render();
  ...
  function updateActiveAdvancedStyleFromColorPanel(hex) {
    if (
      isSyncingColorPanel ||
      !activeLayerUsesAdvancedPalette() ||
      !activeAdvancedStyleId
    )
      return false;
  ...
  function startInlineAdvancedStyleRename(style) {
    ...
    const card =
      grid &&
      grid.querySelector(
        '.palette-style-card[data-id="' + CSS.escape(target.id) + '"]',
      );
    const label = card && card.querySelector(".palette-style-name");
    if (!card || !label) return;
    const previous = target.name || "";
    const input = document.createElement("input");
    input.type = "text";
    input.className = "palette-style-name-input";
  ...
  function renameAdvancedStyle(style) {
  ...
  function deleteAdvancedSeparator(separator) {
    ...
    const idx = advancedStyles.findIndex(
      (item) => isAdvancedSeparator(item) && item.id === separator.id,
    );
    if (idx < 0) return;
  ...
  function moveAdvancedItemToEdge(item, toEnd) {
    if (!item) return;
    const idx = advancedStyles.findIndex(
      (entry) => entry && entry.id === item.id && entry.type === item.type,
    );
    if (idx < 0) return;
  ...
  async function deleteAdvancedStyle(style) {
    if (
      !style ||
      isAdvancedSeparator(style) ||
      advancedStyles.filter((s) => !isAdvancedSeparator(s)).length <= 1
    )
      return false;
    ...
    const used = !!(
      ...
      typeof window.SmartRasterLayer.isStyleUsed === "function" &&
      window.SmartRasterLayer.isStyleUsed(style.id)
    );
    if (used) {
      const confirmed = await requestPaletteConfirm(
        "Delete Style",
        "This style is used by existing Smart Raster pixels. Deleting it will orphan their style ownership while preserving their current painted appearance. Continue?",
        "Delete Style",
      );
      if (!confirmed) return false;
    }
    const previous = advancedStyles
      .slice(0, idx)
      .reverse()
      .find((item) => !isAdvancedSeparator(item));
    const next = advancedStyles
      .slice(idx + 1)
      .find((item) => !isAdvancedSeparator(item));
    advancedStyles.splice(idx, 1);
    if (typeof window.markStyleDeleted === "function")
      window.markStyleDeleted(style.id);
    if (activeAdvancedStyleId === style.id) {
      ...
      advancedColorPanelPaletteId = activeAdvancedStyleId
        ? activePaletteId
        : null;
  ...
  function showAdvancedStyleContextMenu(x, y, style) {
    ...
    const pixelSelection =
      window.PixelSelection && PixelSelection.getState
        ? PixelSelection.getState()
        : null;
    const canReplaceSelected = !!(
      ...
      pixelSelection &&
      pixelSelection.active &&
      pixelSelection.count > 0 &&
      pixelSelection.layerIndex === curLayer &&
      window.LinkedPixelSelection &&
      LinkedPixelSelection.replaceSelectedWithStyle
    );
    [
      menuItem(
        "Edit Style",
        () => editAdvancedStyle(style),
        false,
        !!style.locked,
      ),
      menuItem(
        "Replace Selected With This Style",
        () =>
          LinkedPixelSelection.replaceSelectedWithStyle(style.id, style.rgba),
        false,
        !canReplaceSelected,
      ),
      ...
      menuItem(
        "Delete Style",
        () => deleteAdvancedStyle(style),
        true,
        advancedStyles.filter((s) => !isAdvancedSeparator(s)).length <= 1,
      ),
      menuItem("-"),
      menuItem(style.locked ? "Unlock Style" : "Lock Style", () =>
        toggleAdvancedStyleLock(style),
      ),
    ].forEach((item) => menu.appendChild(item));
    document.body.appendChild(menu);
    menu.style.left =
      Math.min(x, window.innerWidth - menu.offsetWidth - 8) + "px";
    menu.style.top =
      Math.min(y, window.innerHeight - menu.offsetHeight - 8) + "px";
    setTimeout(
      () =>
        document.addEventListener("pointerdown", hideContextMenu, {
          once: true,
        }),
      0,
    );
  ...
  function showAdvancedSeparatorContextMenu(x, y, separator) {
    ...
    [
      menuItem(
        "Delete Separator",
        () => deleteAdvancedSeparator(separator),
        true,
      ),
      menuItem("-"),
  ...
  function clearAdvancedStyleInsert() {
    document
      .querySelectorAll(
        ".palette-style-card.advanced-insert-before,.palette-style-card.advanced-insert-after,.palette-style-separator.advanced-insert-before,.palette-style-separator.advanced-insert-after",
      )
      .forEach((card) =>
        card.classList.remove(
          "advanced-insert-before",
          "advanced-insert-after",
        ),
      );
  }
  function advancedStyleDropTarget(clientX, clientY) {
    const grid = document.getElementById("palette-grid"),
      state = advancedStyleDrag;
    if (!grid || !state) return null;
    const gridRect = grid.getBoundingClientRect();
    const rowGap =
      parseFloat(getComputedStyle(grid).rowGap || getComputedStyle(grid).gap) ||
      0;
    const separators = Array.from(
      grid.querySelectorAll(".palette-style-separator"),
    ).filter(
      (separator) =>
        separator.dataset.id !== state.id && separator.getClientRects().length,
    );
    for (const separator of separators) {
      ...
      if (
        ...
        clientY >= rect.top - rowGap &&
        clientY <= rect.bottom + rowGap
      ) {
        const sourceIndex = advancedStyles.findIndex(
          (item) => item && item.id === state.id,
        );
        const targetIndex = advancedStyles.findIndex(
          (item) => item && item.id === separator.dataset.id,
        );
        return {
          id: separator.dataset.id,
          side:
    ...
    const cards = Array.from(
      grid.querySelectorAll(".palette-style-card,.palette-style-separator"),
    ).filter(
      (card) => card.dataset.id !== state.id && card.getClientRects().length,
    );
  ...
  function activateAdvancedStyleDrag(state) {
    ...
    state.oldUserSelect = document.documentElement.style.userSelect || "";
    state.oldCursor = document.documentElement.style.cursor || "";
    ...
    if (state.lastMove && !state.rafPending) {
      state.rafPending = true;
      requestAnimationFrame(processAdvancedStyleDragMove);
    }
  }
  function processAdvancedStyleDragMove() {
    ...
    const target = advancedStyleDropTarget(event.clientX, event.clientY);
    if (!target) {
      clearAdvancedStyleInsert();
      return;
    }
    if (
      ...
      state.target.side !== target.side
    ) {
      const targetCard = grid.querySelector(
        '.palette-style-card[data-id="' + CSS.escape(target.id) + '"]',
      );
      const rect = targetCard && targetCard.getBoundingClientRect();
      if (rect && Math.abs(event.clientY - (rect.top + rect.height / 2)) < 2)
        target.side = state.target.side;
    }
    state.target = target;
    clearAdvancedStyleInsert();
    ...
    if (card) card.classList.add("advanced-insert-" + target.side);
  }
  function finishAdvancedStyleDrag(event, cancelled) {
    ...
    if (
      state.captured &&
      state.source.hasPointerCapture &&
    ...
    if (state.active) {
      document.documentElement.style.userSelect = state.oldUserSelect;
      document.documentElement.style.cursor = state.oldCursor;
    }
    if (state.active && !cancelled && state.target) {
      const from = advancedStyles.findIndex(
        (item) => item && item.id === state.id,
      );
      const over = advancedStyles.findIndex(
        (item) => item && item.id === state.target.id,
      );
      if (from >= 0 && over >= 0) {
        const beforeOrder = advancedStyles.map((item) => item.id);
        const moved = advancedStyles.splice(from, 1)[0];
        let to = advancedStyles.findIndex(
          (item) => item && item.id === state.target.id,
        );
        ...
        if (
          layers.some(
            (layer) => layer && layer.renderMode === "style-layering",
          ) &&
          typeof undoStack !== "undefined"
        ) {
          undoStack.push({
            type: "smart-raster-style-order",
            before: beforeOrder,
            after: advancedStyles.map((item) => item.id),
          });
          if (typeof redoStack !== "undefined") redoStack.length = 0;
        }
      }
    }
    if (state.active || advancedStyleSuppressClick)
      setTimeout(() => {
        advancedStyleSuppressClick = false;
      }, 0);
  }
  function onAdvancedStylePointerUp(event) {
    finishAdvancedStyleDrag(event, false);
  }
  function onAdvancedStylePointerCancel(event) {
    finishAdvancedStyleDrag(event, true);
  }
  function beginAdvancedStyleDrag(event, style, card) {
    if (!isPrimaryPalettePointer(event) || event.target.closest("input"))
      return;
    ...
    advancedStyleDrag = {
      ...
      startY: event.clientY,
      source: card,
      selectable: !isAdvancedSeparator(style),
      active: false,
      captured: false,
      target: null,
      rafPending: false,
    ...
    try {
      ...
      advancedStyleDrag.captured = true;
    } catch (e) {}
    const pendingDrag = advancedStyleDrag;
    advancedStyleDrag.holdTimer = setTimeout(
      () => activateAdvancedStyleDrag(pendingDrag),
      PALETTE_REORDER_HOLD_MS,
    );
    document.addEventListener("pointermove", updateAdvancedStyleDrag, {
      passive: false,
    });
  ...
  function renderAdvancedPalette(grid) {
    ...
    advancedStyles.forEach((style) => {
      if (isAdvancedSeparator(style)) {
        const separator = document.createElement("div");
        separator.className =
          "palette-style-separator advanced-palette-separator";
        separator.dataset.id = style.id;
        separator.setAttribute("role", "separator");
        separator.title = "Separator";
        separator.addEventListener("pointerdown", (event) =>
          beginAdvancedStyleDrag(event, style, separator),
        );
        separator.addEventListener("dragstart", (event) =>
          event.preventDefault(),
        );
        separator.addEventListener("contextmenu", (event) => {
          event.preventDefault();
          event.stopPropagation();
          if (advancedStyleDrag) finishAdvancedStyleDrag(null, true);
          showAdvancedSeparatorContextMenu(event.clientX, event.clientY, style);
        });
      ...
      card.addEventListener("contextmenu", (event) => {
        ...
        advancedColorPanelPaletteId = activePaletteId;
        render();
        showAdvancedStyleContextMenu(event.clientX, event.clientY, style);
      });
  ...
  function render() {
    ...
    if (useAdvanced && !advancedStyles.length)
      initializeAdvancedPaletteFromActivePalette();
  ...
  function updateToolbarState() {
    ...
    if (remove) {
      ...
      remove.disabled = advanced
        ? !activeAdvancedStyle() ||
          advancedStyles.filter((style) => !isAdvancedSeparator(style))
            .length <= 1
        : !item;
    }
  }
  function syncSelectionClasses() {
    updateToolbarState();
    document
      .querySelectorAll("#palette-grid .palette-swatch")
      .forEach((node) => {
  ...
  function removeEscapedNewlineArtifacts() {
    ...
    Array.from(body.childNodes || []).forEach((node) => {
      ...
      if (
        node.nodeType === 3 &&
        artifacts.some((mark) => text.indexOf(mark) !== -1)
      )
        node.remove();
    });
  }
  ...
  function applyInlineRename() {
    ...
    const mode = activePaletteMode();
    if (mode === "advanced")
      active.advancedName = uniquePaletteName(clean, active.id, mode);
  ...
  function showDeleteConfirm() {
    ...
    if (text)
      text.textContent =
        'Delete "' + paletteNameForMode(active, activePaletteMode()) + '"?';
    box.classList.remove("hidden");
  ...
  function confirmDeletePalette() {
    ...
    if (
      toolbarPaletteAttachment &&
      toolbarPaletteAttachment.paletteId === active.id
    ) {
      toolbarPaletteAttachment = null;
      toolbarPaletteVisible = false;
    }
  ...
  function duplicatePalette() {
    ...
    const copiedSelected =
      selectedIndex >= 0 && copiedSwatches[selectedIndex]
        ? copiedSwatches[selectedIndex].id
        : null;
    const copy = makePalette(
      uniqueCopyName(active.name),
      copiedSwatches,
      null,
      copiedSelected,
      true,
      uniquePaletteName(active.advancedName + " Copy", null, "advanced"),
    );
    palettes.push(copy);
  ...
  function exportSwatchData(swatch) {
    ...
    return {
      id: swatch.id,
      type: "swatch",
      hex: normalizeHex(swatch.hex).toUpperCase(),
      rgba: rgbaArray(swatch.hex),
  ...
  function activePaletteExportData() {
    ...
    return {
      version: 1,
      name: active.name,
      advancedName: active.advancedName,
      swatches: (active.swatches || []).map(exportSwatchData),
    };
  }
  ...
  function exportAllPalettesJson() {
    downloadText(
      "palettes.json",
      JSON.stringify(allPalettesExportData(), null, 2),
      "application/json",
    );
    closeSideMenu();
  }
  function exportActivePaletteGpl() {
    ...
    (active.swatches || []).filter(isSwatch).forEach((s) => {
      ...
      lines.push(
        String(c.r).padStart(3, " ") +
          " " +
          String(c.g).padStart(3, " ") +
          " " +
          String(c.b).padStart(3, " ") +
          label,
      );
    });
    downloadText(
      safeFilename(active.name, "gpl"),
      lines.join("\n") + "\n",
      "text/plain",
    );
    closeSideMenu();
  ...
  function swatchFromImport(item) {
    ...
    if (!hex && Array.isArray(item.rgba) && item.rgba.length >= 3)
      hex = rgbToHex(item.rgba[0], item.rgba[1], item.rgba[2]);
    if (!hex) return null;
    return makeSwatch(hex, null, item.name);
  }
  function paletteFromImport(
    name,
    swatchItems,
    selectedImportId,
    advancedName,
    ...
    source.forEach((item) => {
      ...
      if (!swatch) return;
      if (item && item.id === selectedImportId)
        selectedIndex = swatchList.length;
      swatchList.push(swatch);
    });
    if (!swatchList.length) return null;
    const selected =
      selectedIndex >= 0 && swatchList[selectedIndex]
        ? swatchList[selectedIndex].id
        : null;
    return makePalette(
      uniqueImportedPaletteName(name),
      swatchList,
      null,
      selected,
      true,
  ...
  function parsePaletteJson(text) {
    ...
    if (Array.isArray(data.palettes)) {
      ...
      data.palettes.forEach((p, i) => {
        const palette = paletteFromImport(
          (p && p.name) || "Palette " + (i + 1),
          p && p.swatches,
          p && p.selectedId,
          p && p.advancedName,
      ...
      if (!imported.length)
        throw new Error("No colors were found in this palette file.");
      if (!imported.activeId) imported.activeId = imported[0].id;
      return imported;
    }
    const one = paletteFromImport(
      data.name || "Imported Palette",
      data.swatches,
      data.selectedId,
      data.advancedName,
    );
  ...
  function importPalettes(imported) {
    if (!Array.isArray(imported) || !imported.length)
      throw new Error("No palettes were imported.");
    rememberSelection();
    palettes = palettes.concat(imported);
    activePaletteId =
      imported.activeId && imported.some((p) => p.id === imported.activeId)
        ? imported.activeId
        : imported[0].id;
    restoreScrollPending = true;
    hidePaletteWarning();
  ...
  function handleImportFile(file) {
    ...
    reader.onload = () => {
      try {
        ...
        const imported = /\.gpl$/i.test(file.name)
          ? parseGpl(text)
          : parsePaletteJson(text);
        importPalettes(imported);
      } catch (e) {
        showPaletteWarning(
          e && e.message ? e.message : "Invalid palette file.",
        );
      }
    };
    reader.onerror = () =>
      showPaletteWarning("Could not read that palette file.");
  ...
  function handleGridContextMenu(event) {
    const swatchEl =
      event.target && event.target.closest
        ? event.target.closest(".palette-swatch")
        : null;
    const grid = document.getElementById("palette-grid");
  ...
  function menuItem(label, fn, danger, disabled) {
    ...
    const item = document.createElement("div");
    item.className =
      "ctx-item" + (danger ? " danger" : "") + (disabled ? " disabled" : "");
  ...
  function showEditColorModal(swatch, onApply) {
    ...
    const currentPreview = modal.querySelector(
      ".palette-color-preview.current",
    );
    ...
    const historySwatches = modal.querySelector(
      ".palette-color-history-swatches",
    );
    let dragging = "",
      dragStartHex = "";
    function currentRgb() {
    ...
    function paintPreview(el, hex) {
      if (isTransparentHex(hex)) el.classList.add("transparent");
      else el.classList.remove("transparent");
      el.style.background = isTransparentHex(hex) ? "" : displayHex(hex);
    }
    function setRgb(r, g, b, nextAlpha) {
      ...
      if (nextAlpha !== undefined)
        alpha = Math.max(0, Math.min(255, Math.round(+nextAlpha || 0)));
      hsv = rgbToHsv(rgb.r, rgb.g, rgb.b);
      syncAll();
    }
    function setHsv(h, s, v, nextAlpha) {
      hsv = {
        h: Math.max(0, Math.min(359, +h || 0)),
        s: Math.max(0, Math.min(1, (+s || 0) / 100)),
        v: Math.max(0, Math.min(1, (+v || 0) / 100)),
      };
      if (nextAlpha !== undefined)
        alpha = Math.max(0, Math.min(255, Math.round(+nextAlpha || 0)));
      syncAll();
    ...
    function field(label, value, min, max, step, cls) {
      return (
        "<label>" +
        label +
        '<div class="palette-field-row"><input class="' +
        cls +
        '-range" type="range" min="' +
        min +
        '" max="' +
        max +
        '" step="' +
    ...
    function renderFields() {
      ...
      modal
        .querySelectorAll(".palette-mode-tabs button")
        .forEach((btn) =>
          btn.classList.toggle("active", btn.dataset.mode === mode),
        );
      if (mode === "RGB")
        fields.innerHTML =
          field("R", rgb.r, 0, 255, 1, "edit-r") +
          field("G", rgb.g, 0, 255, 1, "edit-g") +
          field("B", rgb.b, 0, 255, 1, "edit-b") +
          field("A", alpha, 0, 255, 1, "edit-a");
      else if (mode === "HSV")
        fields.innerHTML =
          field("H", Math.round(hsv.h), 0, 359, 1, "edit-h") +
          field("S", Math.round(hsv.s * 100), 0, 100, 1, "edit-s") +
          field("V", Math.round(hsv.v * 100), 0, 100, 1, "edit-v") +
          field("A", alpha, 0, 255, 1, "edit-a");
      else if (mode === "HSL")
        fields.innerHTML =
          field("H", Math.round(hsl.h), 0, 359, 1, "edit-hsl-h") +
          field("S", Math.round(hsl.s * 100), 0, 100, 1, "edit-hsl-s") +
          field("L", Math.round(hsl.l * 100), 0, 100, 1, "edit-hsl-l") +
          field("A", alpha, 0, 255, 1, "edit-a");
      else
        fields.innerHTML =
          '<label>HEX <input class="edit-hex" type="text" spellcheck="false" value="' +
    ...
    function bindFields() {
      if (mode === "RGB") {
        const fn = () =>
          setRgb(
            fields.querySelector(".edit-r").value,
            fields.querySelector(".edit-g").value,
            fields.querySelector(".edit-b").value,
            fields.querySelector(".edit-a").value,
          );
        ["edit-r", "edit-g", "edit-b", "edit-a"].forEach((cls) =>
          bindPair(cls, fn),
        );
      } else if (mode === "HSV") {
        const fn = () =>
          setHsv(
            fields.querySelector(".edit-h").value,
            fields.querySelector(".edit-s").value,
            fields.querySelector(".edit-v").value,
            fields.querySelector(".edit-a").value,
          );
        ["edit-h", "edit-s", "edit-v", "edit-a"].forEach((cls) =>
          bindPair(cls, fn),
        );
      } else if (mode === "HSL") {
        const fn = () =>
          setHsl(
            fields.querySelector(".edit-hsl-h").value,
            fields.querySelector(".edit-hsl-s").value,
            fields.querySelector(".edit-hsl-l").value,
            fields.querySelector(".edit-a").value,
          );
        ["edit-hsl-h", "edit-hsl-s", "edit-hsl-l", "edit-a"].forEach((cls) =>
          bindPair(cls, fn),
        );
      } else {
    ...
    function renderColorHistory() {
      ...
      AdvancedColorHistory.getAll().forEach((hex) => {
        const button = document.createElement("button");
        button.type = "button";
        button.className = "palette-color-history-swatch";
        button.title = hex.toUpperCase();
        if (isTransparentHex(hex)) button.classList.add("transparent");
        else button.style.background = displayHex(hex);
    ...
    function syncAll() {
      paintPreview(nextPreview, colorHex());
      syncWheel();
      renderFields();
    }
    ...
    const unsubscribeHistory = isAdvancedEdit
      ? AdvancedColorHistory.subscribe(renderColorHistory)
      : () => {};
    function close() {
      unsubscribeHistory();
      document.removeEventListener("keydown", onKey);
    ...
    function ok() {
      finalizeHistory("");
      if (typeof onApply === "function") onApply(colorHex());
      else updateSwatchColor(swatch, colorHex());
      close();
    }
    function cancel() {
      close();
    ...
    modal.querySelectorAll(".palette-mode-tabs button").forEach((btn) =>
    ...
    wheel.addEventListener("pointerdown", (event) => {
      if (event.target.closest(".palette-inner-sv")) return;
      dragStartHex = colorHex();
      dragging = "hue";
    ...
    sv.addEventListener("pointermove", (event) => {
      if (dragging === "sv") setSvFromEvent(event);
    });
    ...
    modal
      .querySelector(".palette-color-cancel")
      .addEventListener("click", cancel);
    modal
      .querySelector(".palette-color-close")
      .addEventListener("click", cancel);
  ...
  function hideEditConfirmBar() {
    const modal = document.getElementById("palette-color-modal");
    if (modal) modal.remove();
    const bar = document.getElementById("palette-edit-confirm");
    if (bar) bar.remove();
  }
  ...
  function deleteSwatch(swatch) {
    ...
    selectedId = swatches.length
      ? swatches[Math.min(idx, swatches.length - 1)].id
      : null;
    rememberSelection();
    render();
    persist();
  ...
  function addTransparentSwatch() {
    insertItemAfterSelection(makeSwatch("#00000000"));
  }
  function addAdvancedSeparator() {
    const idx = activeAdvancedStyleId
      ? advancedStyles.findIndex(
          (item) =>
            !isAdvancedSeparator(item) && item.id === activeAdvancedStyleId,
        )
      : -1;
    advancedStyles.splice(
      idx >= 0 ? idx + 1 : advancedStyles.length,
      0,
      makeAdvancedSeparator(),
    );
    render();
    persist();
  }
  ...
  async function clearAll() {
    ...
    const ok = await requestPaletteConfirm(
      "Clear Palette",
      "Clear every color in this palette?",
      "Clear",
    );
    if (!ok) return;
    swatches = [];
  ...
  function beginDrag(event, swatch, el) {
    ...
    dragState = {
      ...
      targetSeparatorSide: null,
      targetSeparatorId: null,
      rafPending: false,
      lastMove: null,
      pointerId: event.pointerId,
      sourceEl: el,
    ...
    dragState.holdTimer = setTimeout(
      () => activatePaletteDrag(pendingDrag),
      PALETTE_REORDER_HOLD_MS,
    );
  ...
  function separatorDropSide(separatorEl, centerY) {
    ...
    const previous =
      dragState && dragState.overId === separatorEl.dataset.id
        ? dragState.side
        : null;
    const deadZone = dragState && dragState.pointerType === "pen" ? 10 : 8;
    if (
      previous &&
      centerY >= center - deadZone &&
      centerY <= center + deadZone
    )
      return previous;
  ...
  function activatePaletteDrag(state) {
    ...
    if (state.sourceEl && !state.captured) {
      try {
        state.sourceEl.setPointerCapture(state.pointerId);
        state.captured = true;
      } catch (e) {}
    }
    const dragged = document.querySelector(
      '.palette-swatch[data-id="' + CSS.escape(state.id) + '"]',
    );
  ...
  function processDragMove() {
    ...
    const insideGrid =
      ...
      event.clientY >= gridRect.top &&
      event.clientY <= gridRect.bottom;
    const target = paletteDropTargetAt(
      grid,
      dragCenterX,
      dragCenterY,
      event.clientX,
  ...
  function finishPaletteDrag(event, cancelled) {
    ...
    document
      .querySelectorAll(".palette-swatch.dragging")
      .forEach((node) => node.classList.remove("dragging"));
    if (state.active && !cancelled) {
      if (event) event.preventDefault();
      if (
        state.lineMode === "slot" &&
        Number.isInteger(state.targetGroupStart) &&
        Number.isInteger(state.targetGroupEnd) &&
        Number.isInteger(state.targetLocalSlot)
      )
        reorderSwatchToSlot(
          state.id,
          state.targetGroupStart,
          state.targetGroupEnd,
          state.targetLocalSlot,
        );
      else if (state.overId && state.overId !== state.id)
        reorderSwatch(state.id, state.overId, state.side);
      setTimeout(() => {
        suppressClick = false;
      }, 0);
      return;
  ...
  function paletteDropItems(grid) {
    return Array.from(
      grid.querySelectorAll(".palette-swatch:not(.dragging)"),
    ).filter((node) => !node.classList.contains("palette-drop-placeholder"));
  }
  function paletteDropTargetAt(grid, clientX, clientY, pointerX, pointerY) {
    const items = paletteDropItems(grid).filter(
      (node) => node.dataset.id !== dragState.id,
    );
    const allSeps = items.filter((n) => n.classList.contains("separator"));
    const swatchNodes = items.filter(
      (n) =>
        !n.classList.contains("spacer") && !n.classList.contains("separator"),
    );
    ...
    function edgeInGroup(sepAbove, px, py) {
      ...
      const groupSwatches = swatchNodes.filter((n) => {
        ...
        const aboveOk = sepAbove
          ? cy > sepAbove.getBoundingClientRect().bottom
          : true;
        const belowOk = nextSep
          ? cy < nextSep.getBoundingClientRect().top
          : true;
    ...
    if (
      item &&
      (!grid.contains(item) ||
        item.dataset.id === dragState.id ||
        item.classList.contains("spacer"))
    )
      item = null;
    ...
    if (item && item.classList.contains("separator")) {
      ...
      return (
        edgeInGroup(sepAbove, pointerX, pointerY) || {
          el: item,
          side: "after",
          mode: "edge",
        }
    ...
    if (item) {
      ...
      const rect = item.getBoundingClientRect();
      const previousSame =
        dragState.overId === item.dataset.id ? dragState.side : null;
      const mid = rect.left + rect.width / 2;
      const threshold = dragState.pointerType === "pen" ? 8 : 5;
      const side =
        previousSame && Math.abs(pointerX - mid) < threshold
          ? previousSame
          : pointerX < mid
            ? "before"
            : "after";
  ...
  function paletteSlotTargetAt(grid, clientX, clientY, groupY) {
    ...
    for (let i = 0; i < groups.length; i++) {
      ...
      const deadZone = dragState && dragState.pointerType === "pen" ? 10 : 8;
      if (
        groupProbeY >= center - deadZone &&
        groupProbeY <= center + deadZone
      ) {
        const stable = Number.isInteger(dragState && dragState.targetGroupStart)
          ? groups.find(
              (candidate) =>
                candidate.startIndex === dragState.targetGroupStart,
            )
          : null;
        if (stable) {
          group = stable;
      ...
      if (groupProbeY < center) {
        group = groups[Math.max(0, i - 1)];
        break;
      }
      group = groups[i];
    }
    ...
    else if (group.separatorBefore)
      groupTop = group.separatorBefore.getBoundingClientRect().bottom + gap;
    ...
    const rowEntries = group.items
      .map((node, index) => ({ node, index }))
      .filter((entry) => entry.index >= rowStart && entry.index < rowEnd);
    const rowSwatches = rowEntries.filter(
      (entry) => !entry.node.classList.contains("spacer"),
    );
    ...
    if (rowSwatches.length) {
      ...
      const column = Math.max(
        0,
        Math.min(columnCount - 1, Math.floor(localX / cellWidth)),
      );
      localSlot = rowStart + column;
    }
    const column = Math.max(0, Math.min(columnCount - 1, localSlot - rowStart));
    const afterRowSwatch =
      rowSwatches.length &&
      localSlot === rowSwatches[rowSwatches.length - 1].index + 1;
    const anchor = afterRowSwatch
      ? rowSwatches[rowSwatches.length - 1].node
      : null;
    return {
      mode: "slot",
      group,
  ...
  function showPaletteSlotBox(target) {
    ...
    line.style.left =
      target.groupLeft +
      target.column * (swatchSize + target.gap) -
      gridRect.left +
      grid.scrollLeft +
      "px";
    line.style.top =
      target.groupTop +
      target.row * (swatchSize + target.gap) -
      gridRect.top +
      grid.scrollTop +
      "px";
  ...
  function showPaletteDropLine(item, side, mode) {
    ...
    if (mode === "bottom") {
      ...
      const nextX =
        rect.right + gap + swatchSize <= gridRect.right
          ? rect.right + gap
          : gridLeft;
    ...
    const neighbor =
      side === "before" ? previousPaletteItem(item) : nextPaletteItem(item);
    let edge;
    if (neighbor && !neighbor.classList.contains("separator")) {
      const nr = neighbor.getBoundingClientRect();
      const sameRow =
        Math.abs((nr.top + nr.bottom) / 2 - (rect.top + rect.bottom) / 2) <
        Math.max(rect.height, nr.height) / 2;
      edge = sameRow
        ? side === "before"
          ? (rect.left + nr.right) / 2
          : (rect.right + nr.left) / 2
        : side === "before"
          ? rect.left
          : rect.right;
    } else {
  ...
  function previousPaletteItem(item) {
    ...
    while (
      ...
      (node.classList.contains("dragging") ||
        node.classList.contains("palette-drop-line") ||
        node.classList.contains("palette-group-drop-line") ||
        node.classList.contains("palette-drop-placeholder"))
    )
  ...
  function clearInsertIndicators() {
    document
      .querySelectorAll(
        ".palette-swatch.insert-before,.palette-swatch.insert-after",
      )
      .forEach((node) =>
        node.classList.remove("insert-before", "insert-after"),
      );
  ...
  function bindResizeObserver() {
    const panel = document.getElementById("palette-panel");
    if (!panel || typeof ResizeObserver === "undefined" || resizeObserver)
      return;
  ...
  function bind() {
    ...
    const exportActiveGplBtn = document.getElementById(
      "palette-export-active-gpl",
    );
    ...
    const toolbarToggle = document.getElementById("palette-toolbar-toggle");
    const advancedToggle = document.getElementById("palette-advanced-toggle");
    const styleLayeringToggle = document.getElementById(
      "palette-style-layering",
    );
    ...
    if (exportActiveBtn)
      exportActiveBtn.addEventListener("click", exportActivePaletteJson);
    if (exportActiveGplBtn)
      exportActiveGplBtn.addEventListener("click", exportActivePaletteGpl);
    if (exportAllBtn)
      exportAllBtn.addEventListener("click", exportAllPalettesJson);
    if (importFile)
    ...
    if (toolbarToggle)
      toolbarToggle.addEventListener("click", (event) => {
        event.stopPropagation();
        toggleToolbarPaletteSubmenu();
      });
    if (advancedToggle)
      advancedToggle.addEventListener("click", toggleAdvancedPalette);
    if (styleLayeringToggle)
      styleLayeringToggle.addEventListener("change", () => {
        const layer = layers[curLayer],
          requested = styleLayeringToggle.checked,
          before = !!(layer && layer.renderMode === "style-layering"),
          beforeLayer =
            typeof _deepCopyLayer === "function" ? _deepCopyLayer(layer) : null;
        const result =
          window.SmartRasterStyleLayering &&
          window.SmartRasterStyleLayering.setEnabled(layer, requested);
        if (!result || !result.success) styleLayeringToggle.checked = before;
        else if (result.changed && typeof undoStack !== "undefined") {
          undoStack.push({
            type: "smart-raster-style-layering-toggle",
            layerIndex: curLayer,
            beforeLayer,
            afterLayer:
              typeof _deepCopyLayer === "function"
                ? _deepCopyLayer(layer)
                : null,
          });
          if (typeof redoStack !== "undefined") redoStack.length = 0;
        }
    ...
    if (renameInput)
      renameInput.addEventListener("keydown", (event) => {
        if (event.key === "Enter") applyInlineRename();
        if (event.key === "Escape") {
          hidePaletteInlinePanels();
          positionSideMenu();
        }
  ...
  document.addEventListener("visibilitychange", () => {
    if (document.hidden) cancelInterruptedPaletteInteraction();
  });
  function getAdvancedStyleOrder() {
    return advancedStyles
      .filter(
        (item) => item && !isAdvancedSeparator(item) && item.type === "style",
      )
      .map((item) => item.id);
  }
  function restoreAdvancedStyleOrder(snapshot) {
    const byId = new Map(advancedStyles.map((item) => [item.id, item])),
      ordered = (Array.isArray(snapshot) ? snapshot : [])
        .map((id) => byId.get(id))
        .filter(Boolean),
      seen = new Set(ordered.map((item) => item.id));
    advancedStyles = ordered.concat(
      advancedStyles.filter((item) => !seen.has(item.id)),
    );
    syncAdvancedRefs();
    notifyAdvancedStyleOrderChanged();
    render();
  ...
  function notifyAdvancedStyleOrderChanged() {
    window.dispatchEvent(
      new CustomEvent("advanced-palette-order-changed", {
        detail: { styleIds: getAdvancedStyleOrder() },
      }),
    );
  }
  function isAdvancedPalettePaintingEnabled() {
    return activeLayerUsesAdvancedPalette();
  }
  function getActiveAdvancedPaletteStyleId() {
    const style = activeAdvancedStyle();
    return activeLayerUsesAdvancedPalette() && style ? style.id : null;
  }
  ...
  function getActiveAdvancedStyleColorForHistory() {
    const style =
      advancedColorPanelStyleId &&
      advancedColorPanelStyleId === activeAdvancedStyleId
        ? findAdvancedStyleById(activeAdvancedStyleId)
        : null;
    return style && !style.locked && Array.isArray(style.rgba)
      ? style.rgba.slice()
      : null;
  }
  window.PaletteDocker = {
    ...
    load,
    reset() {
      load(null, { persist: false, markDirty: false });
    },
    renderCurrentColors: render,
    refresh: render,
    synchronizeActiveContext,
    isAdvancedPalettePaintingEnabled,
    getActiveAdvancedPaletteStyleId,
    getActiveAdvancedStyleColorForHistory,
    findAdvancedStyleById,
    getAdvancedStyleOrder,
    restoreAdvancedStyleOrder,
    updateActiveAdvancedStyleFromColorPanel,
    flushAdvancedStyleColorChange,
    selectAdvancedStyleById,
    selectMatchingRgba,
    setForegroundFromSample,
  };
  window.addEventListener("foreground-color-changed", (event) => {
    ...
    syncPaletteSelectionToForeground(
      event.detail && event.detail.color,
      "external",
    );
  });
  window.addEventListener("active-artwork-changed", () => {
    synchronizeActiveContext(true, { markDirty: false });
    syncStyleLayeringControl();
  });
  window.addEventListener("active-layer-changed", syncStyleLayeringControl);
  window.addEventListener(
    "smart-raster-style-layering-changed",
    syncStyleLayeringControl,
  );
