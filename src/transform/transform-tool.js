...
const tfActionControls = document.createElement("div");
tfActionControls.className = "tf-floating-actions";
tfActionControls.hidden = true;
const tfConfirmButton = document.createElement("button"),
  tfCancelButton = document.createElement("button");
tfConfirmButton.type = tfCancelButton.type = "button";
tfConfirmButton.className = "tf-floating-action tf-floating-confirm";
tfCancelButton.className = "tf-floating-action tf-floating-cancel";
tfConfirmButton.textContent = "\u2713";
tfCancelButton.textContent = "\u00d7";
tfConfirmButton.title = "Confirm transform";
tfCancelButton.title = "Cancel transform";
tfConfirmButton.setAttribute("aria-label", "Confirm transform");
tfCancelButton.setAttribute("aria-label", "Cancel transform");
tfActionControls.append(tfConfirmButton, tfCancelButton);
document.getElementById("canvas-area").appendChild(tfActionControls);
...
function _tfResizeGuideCanvas() {
  ...
  if (tfUiC.width !== w) tfUiC.width = w;
  if (tfUiC.height !== h) tfUiC.height = h;
  perspGuideCtx.setTransform(dpr, 0, 0, dpr, 0, 0);
  tfUiCtx.setTransform(dpr, 0, 0, dpr, 0, 0);
  return { width: r.width, height: r.height, dpr };
}
function _tfClearUi() {
  tfUiCtx.setTransform(1, 0, 0, 1, 0, 0);
  tfUiCtx.clearRect(0, 0, tfUiC.width, tfUiC.height);
...
function _tfSyncGuideCanvasActive() {
  perspGuideC.classList.toggle(
    "tf-persp-active",
    !!(tfActive && tfPerspective),
  );
...
function _tfAnalysisToViewport(analysis) {
  ...
  return {
    ...
    vanishingPoints: analysis.vanishingPoints.map((v) =>
      Object.assign({}, v, vp(v)),
    ),
    horizon: analysis.horizon
      ? { p0: vp(analysis.horizon.p0), p1: vp(analysis.horizon.p1) }
      : null,
  };
}
...
let tfAntialiasingEnabled = true,
  tfAntialiasingQuality = "medium",
  tfAntialiasing = "medium";
try {
  const legacy = localStorage.getItem(TF_ANTIALIAS_KEY),
    savedEnabled = localStorage.getItem(TF_ANTIALIAS_ENABLED_KEY),
    savedQuality = localStorage.getItem(TF_ANTIALIAS_QUALITY_KEY);
  if (["weak", "medium", "strong"].includes(savedQuality))
    tfAntialiasingQuality = savedQuality;
  else if (["weak", "medium", "strong"].includes(legacy))
    tfAntialiasingQuality = legacy;
  tfAntialiasingEnabled =
    savedEnabled === null ? legacy !== "none" : savedEnabled === "true";
...
function _tfPersistAntialiasing() {
  ...
  try {
    localStorage.setItem(
      TF_ANTIALIAS_ENABLED_KEY,
      String(tfAntialiasingEnabled),
    );
    localStorage.setItem(TF_ANTIALIAS_QUALITY_KEY, tfAntialiasingQuality);
    localStorage.setItem(TF_ANTIALIAS_KEY, tfAntialiasing);
  } catch (_) {}
...
function _tfCenter(state) {
  state = state || tfState;
  return {
    x: tfBox.x + tfBox.w / 2 + state.tx,
    y: tfBox.y + tfBox.h / 2 + state.ty,
  };
}

function _tfConfigureSmoothing(context) {
  const enabled = tfAntialiasing !== "none";
  context.imageSmoothingEnabled = enabled;
  if (enabled && "imageSmoothingQuality" in context)
    context.imageSmoothingQuality =
      tfAntialiasing === "weak"
        ? "low"
        : tfAntialiasing === "strong"
          ? "high"
          : "medium";
}
...
function _tfIsIntegerTranslation(state) {
  ...
  return (
    Math.abs(state.scale - 1) < 1e-9 &&
    (rotation < 1e-9 || Math.abs(rotation - 360) < 1e-9) &&
    Math.abs(state.tx - Math.round(state.tx)) < 1e-6 &&
    Math.abs(state.ty - Math.round(state.ty)) < 1e-6
  );
}
function _tfDrawFreeSource(context, source, state) {
  state = state || tfState;
  const originX = Number(source && source._tfOriginX) || 0,
    originY = Number(source && source._tfOriginY) || 0;
  if (_tfIsIntegerTranslation(state)) {
    context.imageSmoothingEnabled = false;
    context.drawImage(
      source,
      originX + Math.round(state.tx),
      originY + Math.round(state.ty),
    );
    return;
  }
  const center = _tfCenter(state);
  context.save();
  context.translate(center.x, center.y);
  context.rotate((state.rotation * Math.PI) / 180);
  context.scale(state.scale, state.scale);
  context.translate(-(tfBox.x + tfBox.w / 2), -(tfBox.y + tfBox.h / 2));
  _tfConfigureSmoothing(context);
  context.drawImage(source, originX, originY);
  context.restore();
}
...
function _tfCropViewportPreviewSource(source) {
  const originX = Number(source && source._tfOriginX) || 0,
    originY = Number(source && source._tfOriginY) || 0;
  const worldX = Math.max(originX, Math.floor(tfBox.x)),
    worldY = Math.max(originY, Math.floor(tfBox.y));
  const right = Math.min(originX + source.width, Math.ceil(tfBox.x + tfBox.w)),
    bottom = Math.min(originY + source.height, Math.ceil(tfBox.y + tfBox.h));
  const width = Math.max(1, right - worldX),
    height = Math.max(1, bottom - worldY),
  ...
  return { canvas, x: worldX, y: worldY };
}
function _tfDrawViewportFreePreview() {
  if (
    tfPerspective ||
    !tfViewportPreviewEntries ||
    !tfViewportPreviewEntries.length
  )
    return;
  const hasLiveTransform =
    tfState &&
    (Math.abs(tfState.tx) > 1e-9 ||
      Math.abs(tfState.ty) > 1e-9 ||
      Math.abs(tfState.scale - 1) > 1e-9 ||
      Math.abs(tfState.rotation) > 1e-9);
  if (!hasLiveTransform) return;
  const dpr = Math.max(1, window.devicePixelRatio || 1),
    viewportWidth = tfUiC.width / dpr,
    viewportHeight = tfUiC.height / dpr;
  const documentCorners = [
      { x: 0, y: 0 },
      { x: CW, y: 0 },
      { x: CW, y: CH },
      { x: 0, y: CH },
    ].map(_tfToViewportPoint),
    origin = _tfToViewportPoint({ x: 0, y: 0 }),
    axisX = _tfToViewportPoint({ x: 1, y: 0 }),
    axisY = _tfToViewportPoint({ x: 0, y: 1 });
  const c = tfUiCtx;
  c.save();
  c.beginPath();
  c.rect(0, 0, viewportWidth, viewportHeight);
  c.moveTo(documentCorners[0].x, documentCorners[0].y);
  for (let index = 1; index < documentCorners.length; index++)
    c.lineTo(documentCorners[index].x, documentCorners[index].y);
  c.closePath();
  c.clip("evenodd");
  c.transform(
    axisX.x - origin.x,
  ...
  c.rotate((tfState.rotation * Math.PI) / 180);
  c.scale(tfState.scale, tfState.scale);
  c.translate(-(tfBox.x + tfBox.w / 2), -(tfBox.y + tfBox.h / 2));
  _tfConfigureSmoothing(c);
  tfViewportPreviewEntries.forEach((entry) => {
    const layer = entry.layerIndex == null ? null : layers[entry.layerIndex];
    if (
      layer &&
      (!layer.visible ||
    ...
    if (layer)
      c.globalAlpha =
        (layer.opacity ?? 1) *
        (typeof _layerGroupChainOpacity === "function"
          ? _layerGroupChainOpacity(layer)
          : 1);
    c.drawImage(entry.canvas, entry.x, entry.y);
    c.restore();
  });
  c.restore();
}
...
function _tfTransformSelectionMaskFree() {
  ...
  const width = tfPixelSelection.width || CW,
    height = tfPixelSelection.height || CH,
    source = tfPixelSelection.mask,
    output = new Uint8ClampedArray(width * height);
  if (_tfIsIntegerTranslation(tfState)) {
    const dx = Math.round(tfState.tx),
      dy = Math.round(tfState.ty);
    for (let y = 0; y < height; y++)
      for (let x = 0; x < width; x++)
        if (source[y * width + x] === 255) {
          const nx = x + dx,
            ny = y + dy;
  ...
  const boxCenterX = tfBox.x + tfBox.w / 2,
    boxCenterY = tfBox.y + tfBox.h / 2,
    center = _tfCenter(tfState),
    scale = tfState.scale || 1,
    radians = (tfState.rotation * Math.PI) / 180,
  ...
  const corners = [
    ...
    const dx = (point.x - boxCenterX) * scale,
      dy = (point.y - boxCenterY) * scale;
    return {
  ...
  for (let y = minY; y < maxY; y++)
    for (let x = minX; x < maxX; x++) {
      const worldX = x + 0.5 - center.x,
        worldY = y + 0.5 - center.y,
        localX = (worldX * cosR + worldY * sinR) / scale + boxCenterX,
        localY = (-worldX * sinR + worldY * cosR) / scale + boxCenterY;
      const sx = Math.floor(localX),
        sy = Math.floor(localY);
      if (
        sx >= 0 &&
        sy >= 0 &&
        sx < width &&
        sy < height &&
        source[sy * width + sx] === 255
      )
...
function _tfTransformSelectionMaskPerspective() {
  ...
  const width = tfPixelSelection.width || CW,
    ...
    source = tfPixelSelection.mask,
    output = new Uint8ClampedArray(width * height),
    inverse = _tfInvertHomography(
      _tfQuadH(tfCorners[0], tfCorners[1], tfCorners[2], tfCorners[3]),
    );
  if (!inverse) return output;
  const minX = Math.max(
      0,
      Math.floor(Math.min(...tfCorners.map((point) => point.x))),
    ),
    minY = Math.max(
      0,
      Math.floor(Math.min(...tfCorners.map((point) => point.y))),
    ),
    maxX = Math.min(
      width,
      Math.ceil(Math.max(...tfCorners.map((point) => point.x))),
    ),
    maxY = Math.min(
      height,
      Math.ceil(Math.max(...tfCorners.map((point) => point.y))),
    );
  for (let y = minY; y < maxY; y++)
    for (let x = minX; x < maxX; x++) {
      const px = x + 0.5,
        py = y + 0.5,
        denominator = inverse.g * px + inverse.h * py + inverse.i;
      if (!Number.isFinite(denominator) || Math.abs(denominator) < 1e-10)
        continue;
      const u = (inverse.a * px + inverse.b * py + inverse.c) / denominator,
        v = (inverse.d * px + inverse.e * py + inverse.f) / denominator;
      if (
        !Number.isFinite(u) ||
        !Number.isFinite(v) ||
        u < 0 ||
        ...
        continue;
      const sx = Math.floor(tfBox.x + u * tfBox.w),
        sy = Math.floor(tfBox.y + v * tfBox.h);
      if (
        sx >= 0 &&
        sy >= 0 &&
        sx < width &&
...
function _tfCurrentSelectionMask() {
  return tfPerspective
    ? _tfTransformSelectionMaskPerspective()
    : _tfTransformSelectionMaskFree();
}
function _tfPreviewSelectionMask() {
  if (tfPixelSelection && window.PixelSelection) {
    const mask = _tfCurrentSelectionMask();
    if (mask)
      PixelSelection.setTransformPreview(
        mask,
        tfPixelSelection.width || CW,
        tfPixelSelection.height || CH,
      );
  }
}
...
function _tfWorldToLocal(world, state, box) {
  ...
  const lx = (dx * cosR - dy * sinR) / s,
    ly = (dx * sinR + dy * cosR) / s;
  return { x: bx + lx, y: by + ly };
}
// Current on-screen position of the pivot handle.
function _tfPivotWorld(state) {
  return _tfLocalToWorld(tfPivot, state);
}
...
function _tfQuadH(d0, d1, d2, d3) {
  const dx1 = d1.x - d2.x,
    dx2 = d3.x - d2.x,
    dx3 = d0.x - d1.x + d2.x - d3.x;
  const dy1 = d1.y - d2.y,
    dy2 = d3.y - d2.y,
    dy3 = d0.y - d1.y + d2.y - d3.y;
  let a, b, c, d, e, f, g, h;
...
function _tfDrawTri(dctx, img, s0, s1, s2, d0, d1, d2) {
  ...
  const cx = (d0.x + d1.x + d2.x) / 3,
    cy = (d0.y + d1.y + d2.y) / 3;
  const e0 = _tfOutset(d0, cx, cy, TF_TRI_BLEED),
    e1 = _tfOutset(d1, cx, cy, TF_TRI_BLEED),
    e2 = _tfOutset(d2, cx, cy, TF_TRI_BLEED);
  dctx.save();
  dctx.beginPath();
  dctx.moveTo(e0.x, e0.y);
  dctx.lineTo(e1.x, e1.y);
...
function _tfDrawPerspective(dctx, img, sx, sy, sw, sh, corners, gridN) {
  ...
  for (let j = 0; j < n; j++) {
    ...
    for (let i = 0; i < n; i++) {
      const u0 = i / n,
        u1 = (i + 1) / n;
      const p00 = _tfApplyQuadH(H, u0, v0),
        p10 = _tfApplyQuadH(H, u1, v0);
      const p11 = _tfApplyQuadH(H, u1, v1),
        p01 = _tfApplyQuadH(H, u0, v1);
      const s00 = { x: sx + u0 * sw, y: sy + v0 * sh },
        s10 = { x: sx + u1 * sw, y: sy + v0 * sh };
      const s11 = { x: sx + u1 * sw, y: sy + v1 * sh },
        s01 = { x: sx + u0 * sw, y: sy + v1 * sh };
...
function _computeOpaqueBBox(canvas) {
  ...
  const originX = Number(canvas && canvas._tfOriginX) || 0,
    originY = Number(canvas && canvas._tfOriginY) || 0;
...
function _tfCaptureSmartMove(li, fi, bounds, styleId) {
  ...
  if (!layer || layer.type !== "smart-raster" || !window.SmartRasterLayer)
    return null;
  ...
  return {
    ...
    contribution:
      styleId && typeof window.SmartRasterV4CaptureStyleTransform === "function"
        ? SmartRasterV4CaptureStyleTransform(styleId, layer, fi)
        : null,
  };
}
...
function _tfSampleCoverageBilinear(coverage, width, height, x, y) {
  ...
  const top = value(x0, y0) * (1 - tx) + value(x0 + 1, y0) * tx,
    bottom = value(x0, y0 + 1) * (1 - tx) + value(x0 + 1, y0 + 1) * tx;
  return Math.max(0, Math.min(65535, Math.round(top * (1 - ty) + bottom * ty)));
}
...
function _tfCommitSmartFreeTransform() {
  ...
  const outputIds = tfPixelSelection
    ? source.styleIds.slice()
    : new Uint16Array(source.width * source.height);
  const contribution = source.contribution,
    outputCoverage = contribution
      ? new Uint16Array(source.width * source.height)
      : null;
  if (tfPixelSelection)
    for (let p = 0; p < tfPixelSelection.mask.length; p++)
      if (tfPixelSelection.mask[p] === 255) outputIds[p] = 0;
  ...
  const minX = Math.max(
    0,
    Math.floor(Math.min(...transformedCorners.map((p) => p.x))) - 2,
  );
  const minY = Math.max(
    0,
    Math.floor(Math.min(...transformedCorners.map((p) => p.y))) - 2,
  );
  const maxX = Math.min(
    source.width,
    Math.ceil(Math.max(...transformedCorners.map((p) => p.x))) + 2,
  );
  const maxY = Math.min(
    source.height,
    Math.ceil(Math.max(...transformedCorners.map((p) => p.y))) + 2,
  );

  for (let y = minY; y < maxY; y++)
    for (let x = minX; x < maxX; x++) {
      const destinationOffset = y * source.width + x;
      if (!contribution && outputRgba[destinationOffset * 4 + 3] === 0)
        continue;
      const worldX = x + 0.5 - center.x,
        worldY = y + 0.5 - center.y;
      ...
      if (
        localX < tfBox.x ||
        localX >= tfBox.x + tfBox.w ||
        localY < tfBox.y ||
        localY >= tfBox.y + tfBox.h
      )
        continue;
      const sourceX = Math.floor(localX),
        sourceY = Math.floor(localY);
      if (
        sourceX < 0 ||
      ...
      if (contribution) {
        outputCoverage[destinationOffset] = _tfSampleCoverageBilinear(
      ...
      if (tfPixelSelection && tfPixelSelection.mask[sourceOffset] !== 255)
        continue;
  ...
  if (
    contribution &&
    typeof window.SmartRasterV4ReplaceStyleTransform === "function" &&
    SmartRasterV4ReplaceStyleTransform(
      contribution,
      layer,
      source.frame,
      outputCoverage,
...
function _tfSampleRgba(src, width, height, bounds, sx, sy) {
  ...
  function combine(samples, weights) {
    ...
    for (let i = 0; i < samples.length; i++) {
      const value = samples[i],
        weight = weights[i];
      alpha += value[3] * weight;
      red += value[0] * value[3] * weight;
      green += value[1] * value[3] * weight;
      blue += value[2] * value[3] * weight;
    }
    alpha = Math.max(0, Math.min(1, alpha));
    if (alpha <= 1e-8) return [0, 0, 0, 0];
    return [
      Math.max(0, Math.min(255, red / alpha)),
      Math.max(0, Math.min(255, green / alpha)),
      Math.max(0, Math.min(255, blue / alpha)),
      alpha,
    ];
  }
  const nearest = pixel(Math.round(sx), Math.round(sy));
  if (tfAntialiasing === "none") return nearest;
  ...
  if (tfAntialiasing === "weak")
    return combine([nearest, bilinear], [0.65, 0.35]);
  ...
  for (let yy = -1; yy <= 2; yy++)
    for (let xx = -1; xx <= 2; xx++) {
      samples.push(pixel(x0 + xx, y0 + yy));
      weights.push(cubicWeight(xx - tx) * cubicWeight(yy - ty));
    }
  return combine(samples, weights);
}

function _tfDrawSmartPerspectiveInverse(
  destinationContext,
  sourceCanvas,
  sourceBounds,
  corners,
  ...
  const source =
    tfSmartMove && tfSmartMove.previewSourceImage
      ? tfSmartMove.previewSourceImage
      : sourceCanvas
          .getContext("2d", { willReadFrequently: true })
  ...
  const output = cachedBackground
    ? new ImageData(
        cachedBackground.data.slice(),
    ...
    : backgroundCanvas
      ? backgroundCanvas
          .getContext("2d", { willReadFrequently: true })
          .getImageData(0, 0, CW, CH)
      : destinationContext.createImageData(CW, CH);
  ...
  for (let y = minY; y < maxY; y++)
    for (let x = minX; x < maxX; x++) {
      ...
      if (
        ...
        continue;
      const sx = sourceBounds.x + u * sourceBounds.w - 0.5,
        sy = sourceBounds.y + v * sourceBounds.h - 0.5;
      const sampled = _tfSampleRgba(
          src,
          source.width,
          source.height,
          sourceBounds,
      ...
      const offset = (y * CW + x) * 4,
        destinationAlpha = dst[offset + 3] / 255,
        outputAlpha = alpha + destinationAlpha * (1 - alpha);
      dst[offset] = Math.round(
        (red * alpha + dst[offset] * destinationAlpha * (1 - alpha)) /
          outputAlpha,
      );
      dst[offset + 1] = Math.round(
        (green * alpha + dst[offset + 1] * destinationAlpha * (1 - alpha)) /
          outputAlpha,
      );
      dst[offset + 2] = Math.round(
        (blue * alpha + dst[offset + 2] * destinationAlpha * (1 - alpha)) /
          outputAlpha,
      );
      dst[offset + 3] = Math.round(outputAlpha * 255);
    }
  destinationContext.clearRect(0, 0, CW, CH);
  destinationContext.putImageData(output, 0, 0);
  return true;
}

function _tfDrawRasterPerspectiveInverse(
  destinationContext,
  corners,
  backgroundCanvas,
) {
  const cached = tfRasterPerspectivePreview;
  if (!cached) return false;
  const homography = _tfQuadH(corners[0], corners[1], corners[2], corners[3]);
  const inverse = _tfInvertHomography(homography);
  if (!inverse) return false;
  const output = backgroundCanvas
    ? backgroundCanvas
        .getContext("2d", { willReadFrequently: true })
        .getImageData(0, 0, CW, CH)
    : destinationContext.createImageData(CW, CH);
  const src = cached.image.data,
    dst = output.data,
    sourceBounds = { x: 0, y: 0, w: cached.width, h: cached.height };
  const minX = Math.max(0, Math.floor(Math.min(...corners.map((p) => p.x)))),
    minY = Math.max(0, Math.floor(Math.min(...corners.map((p) => p.y))));
  const maxX = Math.min(CW, Math.ceil(Math.max(...corners.map((p) => p.x)))),
    maxY = Math.min(CH, Math.ceil(Math.max(...corners.map((p) => p.y))));
  _tfInverseMapRgba(
    src,
    cached.width,
    cached.height,
...
function _tfInverseMapRgba(
  ...
  for (let y = bounds.minY; y < bounds.maxY; y++)
    for (let x = bounds.minX; x < bounds.maxX; x++) {
      const px = (x + 0.5) / destinationScale,
        py = (y + 0.5) / destinationScale,
        denominator = inverse.g * px + inverse.h * py + inverse.i;
      if (!Number.isFinite(denominator) || Math.abs(denominator) < 1e-10)
        continue;
      const u = (inverse.a * px + inverse.b * py + inverse.c) / denominator,
      ...
      const sx = sourceBounds.x + u * sourceBounds.w - 0.5,
        sy = sourceBounds.y + v * sourceBounds.h - 0.5,
        sampled = _tfSampleRgba(src, srcWidth, srcHeight, sourceBounds, sx, sy);
      const red = sampled[0],
        green = sampled[1],
        blue = sampled[2],
        alpha = sampled[3];
      ...
      if (composite) {
        const destinationAlpha = dst[offset + 3] / 255,
          outputAlpha = alpha + destinationAlpha * (1 - alpha);
        dst[offset] = Math.round(
          (red * alpha + dst[offset] * destinationAlpha * (1 - alpha)) /
            outputAlpha,
        ...
        dst[offset] = Math.round(red);
        dst[offset + 1] = Math.round(green);
        dst[offset + 2] = Math.round(blue);
        dst[offset + 3] = Math.round(alpha * 255);
      }
    }
}

function _tfDrawFastPerspectiveInverse(
  destinationContext,
  source,
  sourceBounds,
  corners,
) {
  const preview = tfPerspectiveFastPreview;
  if (!preview) return false;
  const inverse = _tfInvertHomography(
    _tfQuadH(corners[0], corners[1], corners[2], corners[3]),
  );
  if (!inverse) return false;
  const scale = preview.scale,
    minX = Math.max(
      0,
      Math.floor(Math.min(...corners.map((p) => p.x)) * scale),
    ),
    minY = Math.max(
      0,
      Math.floor(Math.min(...corners.map((p) => p.y)) * scale),
    );
  const maxX = Math.min(
      preview.canvas.width,
      Math.ceil(Math.max(...corners.map((p) => p.x)) * scale),
    ),
    maxY = Math.min(
      preview.canvas.height,
      Math.ceil(Math.max(...corners.map((p) => p.y)) * scale),
    );
  const current = { minX, minY, maxX, maxY },
    previous = preview.dirty || current,
    dirty = {
      minX: Math.min(current.minX, previous.minX),
      minY: Math.min(current.minY, previous.minY),
      maxX: Math.max(current.maxX, previous.maxX),
      maxY: Math.max(current.maxY, previous.maxY),
    };
  const data = preview.image.data;
  for (let y = dirty.minY; y < dirty.maxY; y++)
    data.fill(
      0,
      (y * preview.canvas.width + dirty.minX) * 4,
      (y * preview.canvas.width + dirty.maxX) * 4,
    );
  _tfInverseMapRgba(
    source.data,
    source.width,
  ...
  preview.dirty = current;
  destinationContext.save();
  _tfConfigureSmoothing(destinationContext);
  destinationContext.drawImage(preview.canvas, 0, 0, CW, CH);
...
function _tfCommitSmartPerspectiveTransform() {
  ...
  const minX = Math.max(
    0,
    Math.floor(Math.min(...tfCorners.map((p) => p.x))) - 2,
  );
  const minY = Math.max(
    0,
    Math.floor(Math.min(...tfCorners.map((p) => p.y))) - 2,
  );
  const maxX = Math.min(
    source.width,
    Math.ceil(Math.max(...tfCorners.map((p) => p.x))) + 2,
  );
  const maxY = Math.min(
    source.height,
    Math.ceil(Math.max(...tfCorners.map((p) => p.y))) + 2,
  );
    ...
    for (let x = minX; x < maxX; x++) {
      ...
      if (contribution) {
        outputCoverage[destinationOffset] = _tfSampleCoverageBilinear(
          ...
          source.width,
          source.height,
          source.bounds.x + u * source.bounds.w,
          source.bounds.y + v * source.bounds.h,
        );
        continue;
      }
      if (tfPixelSelection && tfPixelSelection.mask[sourceOffset] !== 255)
...
function _tfResolveDrawingFrameIndex(layerIndex, frameIndex) {
  const info =
    typeof getPreviousVisibleDrawingKey === "function"
      ? getPreviousVisibleDrawingKey(layerIndex, frameIndex)
      : null;
  if (info && Number.isInteger(info.frameIndex)) return info.frameIndex;
  const layer = layers[layerIndex],
    held =
      layer && typeof getHeldKey === "function"
        ? getHeldKey(layerIndex, frameIndex)
        : null;
  if (layer && held) {
    const key = Object.keys(layer.frames || {}).find(
      (value) => layer.frames[value] === held,
    );
...
function _tfDiscardSessionForContextChange() {
  ...
  const sameOwner =
    !!tfContext &&
    curLayer === tfContext.layerIndex &&
    curFrame === tfContext.frameIndex;
  ...
  if (sameOwner && !tfGroupMode && tfSnapshot) {
    ctx.clearRect(0, 0, CW, CH);
    ctx.drawImage(
      tfSnapshot,
      Number(tfSnapshot._tfOriginX) || 0,
      Number(tfSnapshot._tfOriginY) || 0,
    );
  }
  if (tfPixelSelection && window.PixelSelection) {
    PixelSelection.clearTransformPreview();
    PixelSelection.setOverlayVisible(true);
  }
...
function _tfPrepareForArtworkChange(nextLayer, nextFrame) {
  ...
  tfContextRefreshPerspective = !!tfPerspective;
  tfContextRefreshPending = true;
...
window.addEventListener("active-artwork-changed", (event) => {
  ...
  if (tfActive) {
    const drawingFrame = _tfResolveDrawingFrameIndex(
      detail.layerIndex,
      detail.frameIndex,
    );
    if (
      tfContext &&
      tfContext.layerIndex === detail.layerIndex &&
      tfContext.frameIndex === detail.frameIndex &&
      tfContext.drawingFrameIndex === drawingFrame
    )
      return;
    tfContextRefreshPerspective = !!tfPerspective;
    _tfDiscardSessionForContextChange();
  }
  const restorePerspective =
    tfContextRefreshPending && tfContextRefreshPerspective;
  if (window.PixelSelection && PixelSelection.isActive())
    PixelSelection.clear();
  tfContextRefreshPending = false;
  enterTransformTool();
  if (restorePerspective && tfActive) _tfSetPerspective(true);
});
function enterTransformTool() {
  if (window.LightTable && window.LightTable.transformMode) return;
  if (
    window.LightTable &&
    typeof window.LightTable.exitTransformMode === "function"
  ) {
  ...
  tfContext = {
    layerIndex: curLayer,
    frameIndex: curFrame,
    drawingFrameIndex: _tfResolveDrawingFrameIndex(curLayer, curFrame),
  };
  ...
  if (activeGroupId)
    _allDescendantGroupIds(activeGroupId).forEach((id) => groupIdSet.add(id));
  if (typeof _selectedGroupsFullIdSet === "function")
    _selectedGroupsFullIdSet().forEach((id) => groupIdSet.add(id));

  const memberSet = new Set(
    typeof selectedLayerIndices !== "undefined" ? selectedLayerIndices : [],
  );
  layers.forEach((l, i) => {
    if (l.groupId && groupIdSet.has(l.groupId)) memberSet.add(i);
  });
  ...
  if (tfGroupMode) {
    ...
    if (!tfMemberIdx.length) {
      if (typeof showInfo === "function")
        showInfo("Nothing to transform.", "Transform");
      setTool("brush", "Brush");
      return;
    }
    tfMembers = tfMemberIdx.map((li) => {
      const heldInfo =
          typeof getPreviousVisibleDrawingKey === "function"
            ? getPreviousVisibleDrawingKey(li, curFrame)
            : null,
        held = (heldInfo && heldInfo.canvas) || getHeldKey(li, curFrame),
        drawingFrameIndex =
          heldInfo && Number.isInteger(heldInfo.frameIndex)
            ? heldInfo.frameIndex
            : _tfResolveDrawingFrameIndex(li, curFrame),
        extended =
          typeof getExtendedLayerFrame === "function"
            ? getExtendedLayerFrame(li, drawingFrameIndex)
            : null;
      let base;
      if (extended) {
        base = document.createElement("canvas");
        base.width = extended.canvas.width;
        base.height = extended.canvas.height;
        base.getContext("2d").drawImage(extended.canvas, 0, 0);
        base._tfOriginX = extended.x;
        base._tfOriginY = extended.y;
      } else {
        base = mkLayerCanvas();
      ...
      return { li, base, frameIndex: drawingFrameIndex };
    });
    const memberBoxes = tfMembers.map((member) =>
        _computeOpaqueBBox(member.base),
      ),
      minMemberX = Math.min(...memberBoxes.map((box) => box.x)),
      minMemberY = Math.min(...memberBoxes.map((box) => box.y)),
      maxMemberX = Math.max(...memberBoxes.map((box) => box.x + box.w)),
      maxMemberY = Math.max(...memberBoxes.map((box) => box.y + box.h));
    tfBox = {
      x: minMemberX,
      y: minMemberY,
      w: maxMemberX - minMemberX,
      h: maxMemberY - minMemberY,
    };
    tfViewportPreviewEntries = tfMembers.map((member) =>
      Object.assign(
        { layerIndex: member.li },
        _tfCropViewportPreviewSource(member.base),
      ),
    );
    ...
    const pixelState =
      window.PixelSelection && PixelSelection.isActive()
        ? PixelSelection.getState()
        : null;
    const extended =
      !pixelState && typeof getExtendedLayerFrame === "function"
        ? getExtendedLayerFrame(curLayer, tfContext.drawingFrameIndex)
        : null;
    if (extended) {
      tfSnapshot = document.createElement("canvas");
      tfSnapshot.width = extended.canvas.width;
      tfSnapshot.height = extended.canvas.height;
      tfSnapshot.getContext("2d").drawImage(extended.canvas, 0, 0);
      tfSnapshot._tfOriginX = extended.x;
      tfSnapshot._tfOriginY = extended.y;
    } else {
      tfSnapshot = mkLayerCanvas();
      tfSnapshot.getContext("2d").drawImage(activeC, 0, 0);
      tfSnapshot._tfOriginX = 0;
      tfSnapshot._tfOriginY = 0;
    }
    if (pixelState && pixelState.layerIndex === curLayer && pixelState.bounds) {
      const selected = mkLayerCanvas(),
        background = mkLayerCanvas();
      ...
      selectedCtx.drawImage(pixelState.maskCanvas, 0, 0);
      ...
      backgroundCtx.drawImage(pixelState.maskCanvas, 0, 0);
      backgroundCtx.globalCompositeOperation = "source-over";
      tfPixelSelection = {
        mask: pixelState.mask.slice(),
        width: pixelState.width,
        height: pixelState.height,
        source: selected,
        background: background,
      };
      tfBox = {
        x: pixelState.bounds.x,
        y: pixelState.bounds.y,
        w: pixelState.bounds.width,
        h: pixelState.bounds.height,
      };
      PixelSelection.clearTransformPreview();
      PixelSelection.setOverlayVisible(true);
    } else tfBox = _computeOpaqueBBox(tfSnapshot);
    tfSmartMove = _tfCaptureSmartMove(
      curLayer,
      pixelState && pixelState.frameIndex != null
        ? pixelState.frameIndex
        : tfContext.drawingFrameIndex,
      tfBox,
      pixelState && pixelState.styleId,
    );
    if (tfSmartMove) {
      if (tfSmartMove.contribution && tfPixelSelection) {
        tfPixelSelection.source = _tfCanvasFromRgba(
          tfSmartMove.contribution.sourceRgba,
          CW,
          CH,
        );
        tfPixelSelection.background = _tfCanvasFromRgba(
          tfSmartMove.contribution.backgroundRgba,
          CW,
          CH,
        );
        tfPixelSelection.above = _tfCanvasFromRgba(
          tfSmartMove.contribution.aboveRgba,
          CW,
          CH,
        );
      }
      const previewSource = tfPixelSelection
        ? tfPixelSelection.source
        : tfSnapshot;
      tfSmartMove.previewSourceImage = previewSource
        .getContext("2d", { willReadFrequently: true })
        .getImageData(0, 0, CW, CH);
      tfSmartMove.previewBackgroundImage = tfPixelSelection
        ? tfPixelSelection.background
            .getContext("2d", { willReadFrequently: true })
            .getImageData(0, 0, CW, CH)
        : null;
    } else {
      const previewSource = tfPixelSelection
        ? tfPixelSelection.source
        : tfSnapshot;
      const x = Math.floor(tfBox.x),
        y = Math.floor(tfBox.y);
      ...
      canvas.height = height;
      const sourceOriginX =
          Number(previewSource && previewSource._tfOriginX) || 0,
        sourceOriginY = Number(previewSource && previewSource._tfOriginY) || 0;
      canvas
        .getContext("2d")
        .drawImage(
          previewSource,
    ...
    const viewportSource = tfPixelSelection
      ? tfPixelSelection.source
      : tfSnapshot;
...
function _tfStoreFullTransform(layerIndex, frameIndex, source, options) {
  if (!source || typeof setExtendedLayerFrame !== "function") return null;
  options = options || {};
  const perspective = !!options.perspective,
    corners = perspective ? tfCorners : _tfCorners(),
    padding = perspective ? 3 : 2;
  let minX = Math.floor(Math.min(...corners.map((point) => point.x))) - padding,
  ...
  if (options.background) {
    ...
    maxX = Math.max(maxX, CW);
    maxY = Math.max(maxY, CH);
  }
  const full = document.createElement("canvas");
  full.width = Math.max(1, maxX - minX);
  full.height = Math.max(1, maxY - minY);
  const fullContext = full.getContext("2d");
  fullContext.translate(-minX, -minY);
  if (options.background) fullContext.drawImage(options.background, 0, 0);
  if (perspective) {
    const originX = Number(source._tfOriginX) || 0,
      originY = Number(source._tfOriginY) || 0,
  ...
  } else _tfDrawFreeSource(fullContext, source, tfState);
  if (options.above) fullContext.drawImage(options.above, 0, 0);
  setExtendedLayerFrame(layerIndex, frameIndex, full, minX, minY);
  return { canvas: full, x: minX, y: minY };
}

function commitTransformTool(options) {
  if (!tfActive) return;
  _tfHideFloatingActions();
  if (window.DEBUG_TOOL_LIFECYCLE)
    console.log("[ToolLifecycle] commitTransformTool", {
      activeTool: tool,
      repeatable: !!(
        window.RepeatableTransformController &&
        RepeatableTransformController.active
      ),
      stack: new Error("Transform commit").stack,
    });
  ...
  if (tfGroupMode) {
    ...
    tfMembers.forEach((m) => {
      _tfStoreFullTransform(m.li, m.frameIndex, m.base, {
        perspective: tfPerspective,
      });
      if (m.li === tfContext.layerIndex) {
        ctx.clearRect(0, 0, CW, CH);
        const key = layers[m.li].frames[m.frameIndex];
        if (key) ctx.drawImage(key, 0, 0);
      }
    });
    _tfHiddenLayers = new Set();
    recomposite(curLayer, curFrame);
  ...
  if (tfPerspective) {
    if (tfSmartMove) {
      const perspectiveSource = tfPixelSelection
        ? tfPixelSelection.source
        : tfSnapshot;
      _tfDrawSmartPerspectiveInverse(
        ctx,
        perspectiveSource,
        tfBox,
        tfCorners,
        tfPixelSelection ? tfPixelSelection.background : null,
      );
      tfPerspectivePreviewExact = true;
    } else {
      _tfDrawRasterPerspectiveInverse(
        ctx,
        tfCorners,
        tfPixelSelection ? tfPixelSelection.background : null,
      );
  ...
  const storageSource =
      tfPerspective && tfRasterPerspectivePreview
        ? tfRasterPerspectivePreview.canvas
        : tfPixelSelection
          ? tfPixelSelection.source
          : tfSnapshot,
    storageOptions = {
      perspective: tfPerspective,
      background: tfPixelSelection && tfPixelSelection.background,
      above: tfPixelSelection && tfPixelSelection.above,
    };
  if (tfPerspective && tfRasterPerspectivePreview)
    storageOptions.sourceRect = {
      x: 0,
      y: 0,
      w: tfRasterPerspectivePreview.width,
      h: tfRasterPerspectivePreview.height,
    };
  const targetLayer = tfContext ? tfContext.layerIndex : curLayer,
    targetFrame = tfContext ? tfContext.drawingFrameIndex : curFrame;
  const storedExtended = !!_tfStoreFullTransform(
    targetLayer,
    targetFrame,
    storageSource,
    storageOptions,
  ...
  renderTimeline();
  if (tfPixelSelection && window.PixelSelection) {
    const transformedMask = _tfCurrentSelectionMask(),
      selectionIdentity =
        tfSmartMove && tfSmartMove.contribution
          ? {
              layerIndex: tfSmartMove.layer,
              frameIndex: tfSmartMove.frame,
              styleId: tfSmartMove.contribution.styleId,
            }
          : null;
    PixelSelection.clearTransformPreview();
    if (transformedMask)
      PixelSelection.replaceMask(
        transformedMask,
        tfPixelSelection.width || CW,
        tfPixelSelection.height || CH,
        "selection-transform",
...
function _tfScheduleFreePreview() {
  ...
  tfFreePreviewRaf = requestAnimationFrame(() => {
    tfFreePreviewRaf = 0;
    if (tfActive && !tfPerspective) _tfRedraw(false);
  });
}
function _tfCancelFreePreview(flush) {
  if (tfFreePreviewRaf) {
    cancelAnimationFrame(tfFreePreviewRaf);
    tfFreePreviewRaf = 0;
  }
...
function _tfRedraw(fastPerspectivePreview) {
  ...
  if (tfGroupMode) {
    ...
    if (tfPerspective) _tfDrawHandlesPerspective(false);
    else _tfDrawHandles(false);
  } else {
    _tfResetPreviewContext(ctx);
    ctx.clearRect(0, 0, CW, CH);
    if (tfPixelSelection) ctx.drawImage(tfPixelSelection.background, 0, 0);
    const transformSource = tfPixelSelection
      ? tfPixelSelection.source
      : tfSnapshot;
    if (tfPerspective) {
      if (fastPerspectivePreview) {
        if (tfSmartMove)
          _tfDrawFastPerspectiveInverse(
            ctx,
            tfSmartMove.previewSourceImage,
            tfBox,
            tfCorners,
          );
        else if (tfRasterPerspectivePreview)
          _tfDrawFastPerspectiveInverse(
            ctx,
            tfRasterPerspectivePreview.image,
            {
              x: 0,
              y: 0,
              w: tfRasterPerspectivePreview.width,
    ...
    if (tfPixelSelection && tfPixelSelection.above)
      ctx.drawImage(tfPixelSelection.above, 0, 0);
    _scheduleRecomposite();
    _tfPreviewSelectionMask();
    if (tfPerspective) _tfDrawHandlesPerspective(true);
    else _tfDrawHandles(true);
  }
}
...
function _tfDrawGroupPreview(fastPerspectivePreview) {
  ...
  tfMembers.forEach((m) => {
    ...
    if (
      !l.visible ||
      (typeof _layerGroupChainVisible === "function" &&
        !_layerGroupChainVisible(l))
    )
      return;
    const layerAlpha =
      (l.opacity ?? 1) *
      (typeof _layerGroupChainOpacity === "function"
        ? _layerGroupChainOpacity(l)
        : 1);
    tfCtx.save();
    tfCtx.globalAlpha = layerAlpha;
    if (tfPerspective) {
      const originX = Number(m.base._tfOriginX) || 0,
        originY = Number(m.base._tfOriginY) || 0;
      _tfDrawPerspective(
        tfCtx,
        m.base,
        tfBox.x - originX,
...
function _tfCorners() {
  const c = _tfCenter();
  const hw = (tfBox.w / 2) * tfState.scale,
    hh = (tfBox.h / 2) * tfState.scale;
  const rad = (tfState.rotation * Math.PI) / 180;
...
function _tfHideFloatingActions() {
  tfActionControls.hidden = true;
}
function _tfPositionFloatingActions(points) {
  if (!tfActive || tfAwaitingRepeat || !points || !points.length) {
    _tfHideFloatingActions();
    return;
  }
  const area = document.getElementById("canvas-area"),
    ...
    controlHeight = 32,
    gap = 10;
  let left = (minX + maxX - controlWidth) / 2,
    top = maxY + gap;
  if (top + controlHeight > height - 6) top = minY - controlHeight - gap;
  tfActionControls.style.left =
    Math.max(6, Math.min(width - controlWidth - 6, left)) + "px";
  tfActionControls.style.top =
    Math.max(6, Math.min(height - controlHeight - 6, top)) + "px";
  tfActionControls.hidden = false;
}

function _tfDrawHandles(clearFirst) {
  _tfResizeGuideCanvas();
  const corners = _tfCorners().map(_tfToViewportPoint);
  _tfClearUi();
  const idle = tfAwaitingRepeat,
    c = tfUiCtx;
  c.save();
  c.globalAlpha = idle ? 0.38 : 1;
  c.strokeStyle = idle ? "#9a9aa6" : tfGroupMode ? "#ff9f4d" : "#4da3ff";
  c.lineWidth = 1.5;
  c.lineJoin = "round";
  c.lineCap = "round";
  c.setLineDash([6, 4]);
  ...
  const hr = TF_HANDLE_R;
  c.fillStyle = idle ? "#8c8c96" : "#fff";
  corners.forEach((p) => {
    c.beginPath();
    c.rect(p.x - hr / 2, p.y - hr / 2, hr, hr);
    c.fill();
    c.stroke();
  });
  const mids = _tfPolyEdgeMidpoints(corners),
    er = hr * 0.42;
  mids.forEach((p) => {
    c.beginPath();
    c.rect(p.x - er, p.y - er, er * 2, er * 2);
    c.fill();
    c.stroke();
  });
  _tfDrawPivotHandle();
...
function _tfDrawPivotHandle(targetCtx, targetPivotWorld, options) {
  ...
  if (!c) return;
  const p = _tfToViewportPoint(pWorld),
    hr = TF_HANDLE_R;
  const idle = options && options.idle;
  c.save();
  c.strokeStyle = idle || tfAwaitingRepeat ? "#92929c" : "#ffd24d";
  c.fillStyle =
    idle || tfAwaitingRepeat ? "rgba(146,146,156,.2)" : "rgba(255,210,77,0.25)";
  c.lineWidth = 1.5;
  c.lineCap = "round";
  c.beginPath();
  c.arc(p.x, p.y, hr * 0.7, 0, Math.PI * 2);
  c.fill();
  c.stroke();
  c.beginPath();
  c.moveTo(p.x - hr / 2, p.y);
  c.lineTo(p.x + hr / 2, p.y);
  c.moveTo(p.x, p.y - hr / 2);
  c.lineTo(p.x, p.y + hr / 2);
  c.stroke();
  c.restore();
}

function _tfDrawHandlesPerspective(clearFirst) {
  const geometry = _tfResizeGuideCanvas();
  _tfClearUi();
  perspGuideCtx.setTransform(1, 0, 0, 1, 0, 0);
  perspGuideCtx.clearRect(0, 0, perspGuideC.width, perspGuideC.height);
  perspGuideCtx.setTransform(geometry.dpr, 0, 0, geometry.dpr, 0, 0);
  if (tfOptionValues.perspectiveGuidesEnabled) {
    const analysis = PerspectiveController.analyze(tfCorners),
      viewAnalysis = _tfAnalysisToViewport(analysis);
    PerspectiveController.draw(perspGuideCtx, viewAnalysis, {
      scale: 1,
      width: geometry.width,
      height: geometry.height,
    });
  }
  const corners = tfCorners.map(_tfToViewportPoint),
    idle = tfAwaitingRepeat,
    c = tfUiCtx,
    hr = TF_HANDLE_R;
  c.save();
  c.globalAlpha = idle ? 0.38 : 1;
  c.strokeStyle = idle ? "#9a9aa6" : tfGroupMode ? "#ff9f4d" : "#a24dff";
  c.lineWidth = 1.5;
  c.lineJoin = "round";
  c.lineCap = "round";
  c.setLineDash([6, 4]);
...
const TF_ROTATE_CURSOR =
  'url("data:image/svg+xml,%3Csvg xmlns=%27http://www.w3.org/2000/svg%27 width=%2724%27 height=%2724%27 viewBox=%270 0 24 24%27%3E%3Cpath d=%27M6 8a7 7 0 1 1-1 7%27 fill=%27none%27 stroke=%27white%27 stroke-width=%273%27/%3E%3Cpath d=%27M3 5v6h6%27 fill=%27none%27 stroke=%27white%27 stroke-width=%273%27/%3E%3Cpath d=%27M6 8a7 7 0 1 1-1 7%27 fill=%27none%27 stroke=%27000%27 stroke-width=%271.2%27/%3E%3Cpath d=%27M3 5v6h6%27 fill=%27none%27 stroke=%27000%27 stroke-width=%271.2%27/%3E%3C/svg%3E") 12 12, crosshair';
function _tfResizeCursor(angle) {
...
function _tfHitCursor(hit) {
  if (!hit) return "default";
  if (hit.mode === "pivot") return "crosshair";
  if (hit.mode === "move" || hit.mode === "pmove") return "move";
  if (hit.mode === "rotate" || hit.mode === "protate") return TF_ROTATE_CURSOR;
  if (hit.cursorAngle != null) return _tfResizeCursor(hit.cursorAngle);
  return hit.mode === "vp"
    ? "crosshair"
    : hit.mode === "horizon"
      ? "ns-resize"
...
function _tfHitTestGeneric(
  ...
  for (let i = 0; i < corners.length; i++)
    if (_tfDist(p.x, p.y, corners[i].x, corners[i].y) <= hitR)
      return {
        mode: "scale",
        cornerIndex: i,
        cursorAngle: rotation + (i === 0 || i === 2 ? 45 : 135),
      };
  const mids = _tfPolyEdgeMidpoints(corners);
  for (let i = 0; i < mids.length; i++)
    if (_tfDist(p.x, p.y, mids[i].x, mids[i].y) <= hitR)
      return {
        mode: "scale",
        edgeIndex: i,
        cursorAngle: rotation + (i % 2 === 0 ? 90 : 0),
      };
  if (pivotWorld) {
    if (_tfDist(p.x, p.y, pivotWorld.x, pivotWorld.y) <= hitR)
      return { mode: "pivot" };
  }
  const rad = (-rotation * Math.PI) / 180,
    cosR = Math.cos(rad),
...
function _tfHitTestPerspective(p) {
  ...
  for (let i = 0; i < tfCorners.length; i++) {
    if (_tfDist(p.x, p.y, tfCorners[i].x, tfCorners[i].y) <= hitR) {
      const previous = tfCorners[(i + tfCorners.length - 1) % tfCorners.length],
        next = tfCorners[(i + 1) % tfCorners.length];
      return {
        mode: "pcorner",
        cornerIndex: i,
        cursorAngle:
          (Math.atan2(next.y - previous.y, next.x - previous.x) * 180) /
          Math.PI,
      };
    }
  }
  if (tfPivot) {
    const pivot = _tfPivotWorld();
    if (_tfDist(p.x, p.y, pivot.x, pivot.y) <= hitR) return { mode: "pivot" };
  }
  ...
  if (tfOptionValues.perspectiveGuidesEnabled) {
    ...
    if (PerspectiveController.hitTestHorizon(p, analysis, hitR))
      return { mode: "horizon" };
  }
  ...
  for (let i = 0; i < mids.length; i++) {
    if (_tfDist(p.x, p.y, mids[i].x, mids[i].y) <= hitR) {
      const a = tfCorners[i],
        b = tfCorners[(i + 1) % tfCorners.length];
      return {
        mode: "pedge",
        edgeIndex: i,
        cursorAngle: (Math.atan2(b.y - a.y, b.x - a.x) * 180) / Math.PI + 90,
      };
    }
  }
...
function _tfPerspPointerDown(e) {
  ...
  if (hit.mode === "pcorner") {
    ...
    tfDragInfo = {
      startP: p,
      startCorner: Object.assign({}, tfCorners[hit.cornerIndex]),
    };
  } else if (hit.mode === "pedge") {
    const i0 = hit.edgeIndex,
      i1 = (hit.edgeIndex + 1) % tfCorners.length;
    tfDragInfo = {
      startP: p,
      edgeIndex: hit.edgeIndex,
      startA: Object.assign({}, tfCorners[i0]),
      startB: Object.assign({}, tfCorners[i1]),
    };
  } else if (hit.mode === "vp") {
    ...
    const analysis = PerspectiveController.analyze(tfCorners);
    const otherVP = analysis.vanishingPoints.find(
      (v) => v.axisId !== hit.axisId,
    );
    tfDragInfo = {
      axisId: hit.axisId,
      startP: p,
      startCorners: tfCorners.map((c) => ({ x: c.x, y: c.y })),
  ...
  } else if (hit.mode === "protate") {
    const pivot = tfPivot
      ? _tfPivotWorld()
      : { x: tfBox.x + tfBox.w / 2, y: tfBox.y + tfBox.h / 2 };
    tfDragInfo = {
      startP: p,
      startCorners: tfCorners.map((c) => ({ x: c.x, y: c.y })),
      pivot,
...
function _tfFreePointerDown(e) {
  ...
  tfDragInfo = {
    ...
    startAngle: Math.atan2(
      p.y - _tfPivotWorld(tfState).y,
      p.x - _tfPivotWorld(tfState).x,
    ),
...
function _tfPerspPointerMoveDrag(e) {
  ...
  if (tfDrag === "pivot") {
    ...
    _tfSyncStateFields();
    _tfDrawHandlesPerspective(true);
    return;
  }
  ...
  if (tfDrag === "pcorner") {
    ...
    // side, untouched.
    const dx = p.x - tfDragInfo.startP.x,
      dy = p.y - tfDragInfo.startP.y;
    const i0 = tfDragInfo.edgeIndex,
      i1 = (tfDragInfo.edgeIndex + 1) % tfCorners.length;
    candidate = tfCorners.map((c, i) => {
      if (i === i0)
        return { x: tfDragInfo.startA.x + dx, y: tfDragInfo.startA.y + dy };
      if (i === i1)
        return { x: tfDragInfo.startB.x + dx, y: tfDragInfo.startB.y + dy };
      return c;
    });
    ...
    const dx = p.x - tfDragInfo.startP.x,
      dy = p.y - tfDragInfo.startP.y;
    candidate = tfCorners.map((c, i) => ({
      x: tfDragInfo.startCorners[i].x + dx,
      y: tfDragInfo.startCorners[i].y + dy,
    }));
  } else if (tfDrag === "vp") {
    ...
    candidate = PerspectiveController.dragAxisVP(
      tfDragInfo.startCorners,
      tfDragInfo.axisId,
      p,
      tfDragInfo.fixedOtherVP,
    );
  } else if (tfDrag === "horizon") {
    const dy = p.y - tfDragInfo.startP.y;
    candidate = PerspectiveController.dragHorizon(
      tfDragInfo.startCorners,
      tfDragInfo.startVP1,
      tfDragInfo.startVP2,
      dy,
    );
  } else if (tfDrag === "protate") {
    const angle =
        Math.atan2(p.y - tfDragInfo.pivot.y, p.x - tfDragInfo.pivot.x) -
        tfDragInfo.startAngle,
      cos = Math.cos(angle),
      sin = Math.sin(angle);
    candidate = tfDragInfo.startCorners.map((point) => {
      const dx = point.x - tfDragInfo.pivot.x,
        dy = point.y - tfDragInfo.pivot.y;
      return {
        x: tfDragInfo.pivot.x + dx * cos - dy * sin,
        y: tfDragInfo.pivot.y + dx * sin + dy * cos,
      };
    });
  }
...
function _tfFreePointerMoveDrag(e) {
  ...
  if (tfDrag === "move") {
    ...
    const d = _tfDist(
      p.x,
      p.y,
      tfDragInfo.startCenter.x,
      tfDragInfo.startCenter.y,
    );
    const ratio = tfDragInfo.startDist > 1 ? d / tfDragInfo.startDist : 1;
    const newScale = Math.max(
      0.02,
      Math.min(50, tfDragInfo.startState.scale * ratio),
    );
    ...
    _tfSetStateForPivot(
      tfDragInfo.startPivotWorld,
      tfDragInfo.startState.rotation,
      newScale,
    );
  } else if (tfDrag === "rotate") {
    const ang = Math.atan2(
      p.y - tfDragInfo.startPivotWorld.y,
      p.x - tfDragInfo.startPivotWorld.x,
    );
...
function _tfEndDrag(e) {
  ...
  if (transformC.hasPointerCapture && transformC.hasPointerCapture(e.pointerId))
    transformC.releasePointerCapture(e.pointerId);
  if (
    perspGuideC.hasPointerCapture &&
    perspGuideC.hasPointerCapture(e.pointerId)
  )
    perspGuideC.releasePointerCapture(e.pointerId);
  if (tfUiC.hasPointerCapture && tfUiC.hasPointerCapture(e.pointerId))
    tfUiC.releasePointerCapture(e.pointerId);
  tfDrag = null;
  tfDragInfo = null;
  tfCornerDrag = null;
  if (settlePerspective) {
    _tfCancelPerspectivePreview();
    _tfRedraw(false);
  } else _tfCancelFreePreview(true);
}
...
function _tfFreePointerHover(e) {
  ...
  const hit = _tfHitTest(getPos(e)),
    cursor = _tfHitCursor(hit);
  transformC.style.cursor = cursor;
  tfUiC.style.cursor = cursor;
}
...
function _tfRememberCurrentFreeOperation() {
  const boxCenter = { x: tfBox.x + tfBox.w / 2, y: tfBox.y + tfBox.h / 2 },
    pivotOffset = { x: tfPivot.x - boxCenter.x, y: tfPivot.y - boxCenter.y },
    hasLiveOperation =
      Math.abs(tfState.tx) > 1e-9 ||
      Math.abs(tfState.ty) > 1e-9 ||
      Math.abs(tfState.scale - 1) > 1e-9 ||
      Math.abs(tfState.rotation) > 1e-9 ||
      Math.abs(pivotOffset.x) > 1e-9 ||
      Math.abs(pivotOffset.y) > 1e-9;
  if (hasLiveOperation)
    tfLastCommittedOperation = {
      state: Object.assign({}, tfState),
      pivotOffset,
    };
  return hasLiveOperation;
}
function _tfShowRepeatedFreePreview() {
  if (!tfLastCommittedOperation || !tfActive || tfPerspective) return false;
  const boxCenter = { x: tfBox.x + tfBox.w / 2, y: tfBox.y + tfBox.h / 2 };
  tfState = Object.assign({}, tfLastCommittedOperation.state);
  tfPivot = {
    x: boxCenter.x + tfLastCommittedOperation.pivotOffset.x,
    y: boxCenter.y + tfLastCommittedOperation.pivotOffset.y,
  };
  tfAwaitingRepeat = false;
  _tfRedraw(false);
  return true;
...
function _tfConfirmAction() {
  if (!tfActive) return false;
  if (!tfPerspective && tfAwaitingRepeat) return _tfShowRepeatedFreePreview();
  const wasPerspective = tfPerspective;
  if (
    !wasPerspective &&
    !_tfRememberCurrentFreeOperation() &&
    tfLastCommittedOperation
  )
    return _tfShowRepeatedFreePreview();
  if (!wasPerspective && tfPendingRepeatUndo) {
    undoStack.push(tfPendingRepeatUndo);
    if (undoStack.length > 40) undoStack.shift();
    redoStack = [];
    tfPendingRepeatUndo = null;
  }
  commitTransformTool();
  if (tool === "transform") {
    enterTransformTool();
    if (!wasPerspective) {
      tfPendingRepeatUndo = undoStack.pop() || null;
      tfAwaitingRepeat = true;
      _tfHideFloatingActions();
      _tfDrawHandles(true);
    }
  }
...
function _tfCancelAction() {
  if (!tfActive || tfAwaitingRepeat) return false;
  const wasPerspective = tfPerspective;
  cancelTransformTool();
  if (tool === "transform") {
    enterTransformTool();
    if (!wasPerspective) {
      tfPendingRepeatUndo = undoStack.pop() || tfPendingRepeatUndo;
      tfAwaitingRepeat = true;
      _tfHideFloatingActions();
      _tfDrawHandles(true);
    }
...
tfConfirmButton.addEventListener("pointerdown", (event) =>
  event.stopPropagation(),
);
tfCancelButton.addEventListener("pointerdown", (event) =>
  event.stopPropagation(),
);
tfConfirmButton.addEventListener("click", (event) => {
  event.preventDefault();
  event.stopPropagation();
  _tfConfirmAction();
});
tfCancelButton.addEventListener("click", (event) => {
  event.preventDefault();
  event.stopPropagation();
  _tfCancelAction();
});
...
const TF_MODE_OPTIONS = {
  ...
  perspective: [
    {
      id: "perspectiveGuidesEnabled",
      label: "Perspective Guides (Beta)",
      default: true,
    },
  ],
};
let tfOptionValues = {};
Object.values(TF_MODE_OPTIONS).forEach((list) =>
  list.forEach((o) => {
    tfOptionValues[o.id] = o.default;
  }),
);
...
function _tfCurrentModeKey() {
  return tfPerspective ? "perspective" : "free";
}
...
function _tfStateFieldValue(key) {
  ...
  if (key === "pivotX" || key === "pivotY") {
    const pivot = tfPivot ? _tfPivotWorld(tfState) : { x: 0, y: 0 },
      center = tfBox
        ? { x: tfBox.x + tfBox.w / 2, y: tfBox.y + tfBox.h / 2 }
        : { x: 0, y: 0 };
    return key === "pivotX" ? pivot.x - center.x : pivot.y - center.y;
  }
  return key === "translateX"
    ? tfState.tx
    : key === "translateY"
      ? tfState.ty
      : tfState.rotation;
...
function _tfSyncStateFields() {
  ...
  _tfOptionsBody.querySelectorAll("[data-tf-state-field]").forEach((input) => {
    ...
    input.value = _tfFormatStateField(
      input.dataset.tfStateField,
      _tfStateFieldValue(input.dataset.tfStateField),
    );
  });
}
function _tfApplyStateField(key, raw) {
  ...
  if (key === "translateX") tfState.tx = parsed;
    ...
    _tfSetStateForPivot(_tfPivotWorld(tfState), parsed, tfState.scale);
  else if (key === "scale" && parsed > 0)
    _tfSetStateForPivot(_tfPivotWorld(tfState), tfState.rotation, parsed / 100);
  else if ((key === "pivotX" || key === "pivotY") && tfPivot && tfBox) {
    const pivotWorld = _tfPivotWorld(tfState),
      center = { x: tfBox.x + tfBox.w / 2, y: tfBox.y + tfBox.h / 2 };
    pivotWorld[key === "pivotX" ? "x" : "y"] =
      (key === "pivotX" ? center.x : center.y) + parsed;
...
function _tfAppendStateFields(root) {
  ...
  [
    ["Panning X", "translateX"],
    ["Panning Y", "translateY"],
    ["Scale", "scale"],
    ["Angle", "rotation"],
    ["Pivot X", "pivotX"],
    ["Pivot Y", "pivotY"],
    ...
    row.className = "tf-option-row tf-state-row";
    const text = document.createElement("span");
    text.textContent = label;
    const input = document.createElement("input");
    input.type = "text";
    input.className = "tf-state-input";
    input.dataset.tfStateField = key;
    input.value = _tfFormatStateField(key, _tfStateFieldValue(key));
...
function _tfRenderOptionsPanel() {
  ...
  if (
    window.ToolSettingsUI &&
    typeof ToolSettingsUI.antialiasing === "function"
  )
    ToolSettingsUI.antialiasing(_tfOptionsBody, {
      enabled: tfAntialiasingEnabled,
      quality: tfAntialiasingQuality,
      onEnabledChange: (enabled) => {
        tfAntialiasingEnabled = !!enabled;
        _tfPersistAntialiasing();
      },
      onQualityChange: (quality) => {
        if (["weak", "medium", "strong"].includes(quality))
          tfAntialiasingQuality = quality;
        _tfPersistAntialiasing();
      },
    });
  opts.forEach((o) => {
    ...
    row.style.cssText =
      "display:flex;align-items:center;gap:6px;font-size:11px;color:var(--text);cursor:pointer;user-select:none;";
...
if (_tfBtnFree)
  _tfBtnFree.onclick = () => {
    if (window.LightTable && window.LightTable.transformMode) {
      if (typeof window.LightTable.exitTransformMode === "function")
        window.LightTable.exitTransformMode();
...
function _tfSyncToggleUI() {
  ...
  if (_tfBtnFree)
    _tfBtnFree.classList.toggle("active", !isLt && !tfPerspective);
  if (_tfBtnPersp)
    _tfBtnPersp.classList.toggle("active", !isLt && tfPerspective);
  _tfRenderOptionsPanel();
}
...
function _tfSetPerspective(on) {
  ...
  if (on) {
    if (!tfPerspectiveFastPreview) {
      const scale = 1,
        canvas = document.createElement("canvas");
      canvas.width = Math.max(1, Math.ceil(CW * scale));
      canvas.height = Math.max(1, Math.ceil(CH * scale));
      const context = canvas.getContext("2d");
      tfPerspectiveFastPreview = {
        scale,
        canvas,
        context,
    ...
    if (tfSmartMove)
      tfSmartMove.sourceCorners = tfCorners.map((p) => ({ x: p.x, y: p.y }));
  } else {
    tfCorners = null;
  }
  tfPerspective = on;
