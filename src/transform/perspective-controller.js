...
const PerspectiveController = (() => {
  ...
  function analyze(corners) {
    ...
    const vanishingPoints = axes.map((a) => {
      ...
      const mx = (a.nearA.x + a.nearB.x) / 2,
        my = (a.nearA.y + a.nearB.y) / 2;
      ...
      return {
        axisId: a.id,
        x: mx + dx * VP_PLACEHOLDER_DIST,
        y: my + dy * VP_PLACEHOLDER_DIST,
        converged: false,
      };
    });
    let horizon = null;
    ...
    if (converged.length === 2) {
      horizon = {
        p0: { x: converged[0].x, y: converged[0].y },
      ...
      horizon = {
        p0: { x: vp.x - 4000, y: vp.y },
        p1: { x: vp.x + 4000, y: vp.y },
      };
    }
  ...
  function _segsIntersect(p1, p2, p3, p4) {
    const d1 = _orient(p3, p4, p1),
      d2 = _orient(p3, p4, p2);
    const d3 = _orient(p1, p2, p3),
      d4 = _orient(p1, p2, p4);
    return (
      ((d1 > 0 && d2 < 0) || (d1 < 0 && d2 > 0)) &&
      ((d3 > 0 && d4 < 0) || (d3 < 0 && d4 > 0))
  ...
  function draw(ctx, analysis, opts) {
    ...
    if (
      !analysis ||
      !analysis.vanishingPoints ||
      !analysis.vanishingPoints.length
    )
      return;
    ...
    analysis.axes.forEach((axis) => {
      if (!axis.vp) return;
      ctx.beginPath();
      ctx.moveTo(axis.farA.x, axis.farA.y);
      ctx.lineTo(axis.vp.x, axis.vp.y);
      ctx.moveTo(axis.farB.x, axis.farB.y);
      ctx.lineTo(axis.vp.x, axis.vp.y);
      ctx.stroke();
    });
    ...
    const sortedForLabel = analysis.vanishingPoints
      .slice()
      .sort(
        (a, b) => (axisOrder[a.axisId] ?? 99) - (axisOrder[b.axisId] ?? 99),
      );
  ...
  function _pcRotateTo(anchor, target, length) {
    ...
    return {
      x: anchor.x + (dx / len) * length,
      y: anchor.y + (dy / len) * length,
    };
  }
  ...
  function _pcDist(a, b) {
    return Math.hypot(a.x - b.x, a.y - b.y);
  }
  function _pcLineAtAngle(origin, theta) {
    return {
      p0: origin,
      p1: { x: origin.x + Math.cos(theta), y: origin.y + Math.sin(theta) },
    };
  }
  function _pcAngle(a, b) {
  ...
  function dragAxisVP(corners, axisId, targetVP, otherVP) {
    ...
    const _selfVP = analyze(corners).vanishingPoints.find(
      (v) => v.axisId === axisId,
    );
    ...
    if (axisId === "horizontal") {
      ...
      const leftMid = _pcPointOnSegment(TL, BL, 0.5),
        rightMid = _pcPointOnSegment(TR, BR, 0.5);
      ...
      if (pivotLeft) {
        ...
        const solved = _pcSolveMovableSide(
          topLine,
          bottomLine,
          otherVP,
          TR,
    ...
    if (axisId === "vertical") {
      if (!otherVP) return _pcCloneCorners(corners);
      const topMid = _pcPointOnSegment(TL, TR, 0.5),
        bottomMid = _pcPointOnSegment(BL, BR, 0.5);
  ...
  function _pcSolveFromVPs(corners, vpH, vpV) {
    ...
    const ah = axes.find((a) => a.id === "horizontal"),
      av = axes.find((a) => a.id === "vertical");
    const anchor = ah.nearA; // shared corner (same object for both axes)
    const newFarAH = _pcRotateTo(
      anchor,
      vpH,
      Math.hypot(ah.farA.x - anchor.x, ah.farA.y - anchor.y),
    );
    const newFarAV = _pcRotateTo(
      anchor,
      vpV,
      Math.hypot(av.farA.x - anchor.x, av.farA.y - anchor.y),
    );
  ...
  function dragHorizon(corners, vp1, vp2, dy) {
    ...
    return _pcSolveFromVPs(
      corners,
      { x: targetH.x, y: targetH.y + dy },
      { x: targetV.x, y: targetV.y + dy },
    );
  }
