...
(function () {
  ...
  function _cloneMeta(meta) {
    if (!meta) return null;
    var idxToId = {};
    Object.entries(meta.indexToStyleId || {}).forEach(function (e) {
      idxToId[Number(e[0])] = e[1];
    });
    var idToIdx = {};
    Object.entries(meta.styleIdToIndex || {}).forEach(function (e) {
      var n = Number(e[1]);
      if (n > 0) idToIdx[e[0]] = n;
    });
    return {
  ...
  function _encodePixel(data, offset, index, coverage) {
    ...
    data[offset + 2] =
      coverage == null ? 255 : Math.max(1, Math.min(255, coverage));
    data[offset + 3] = 255;
  }
  ...
  function ensureStyleIndex(li, fi, styleId) {
    ...
    if (!bundle || !styleId) return 0;
    var meta = bundle.meta;
    var existing = meta.styleIdToIndex.hasOwnProperty(styleId)
      ? Number(meta.styleIdToIndex[styleId])
      : 0;
    if (existing > 0) {
      if (
        !meta.indexToStyleId.hasOwnProperty(existing) ||
    ...
    var maxExisting = Object.keys(meta.indexToStyleId).reduce(function (m, k) {
  ...
  function commitBrushMask(
    ...
    var sctx = bundle.canvas.getContext("2d", {
      willReadFrequently: true,
      colorSpace: "srgb",
    });
    ...
    for (var y = 0; y < CH; y++) {
      ...
      while (x < CW) {
        ...
        for (var runX = 0; runX < runWidth; runX++) {
          ...
          if (
            brushBlendMode === "draw-behind" &&
            previousPixel.index &&
            previousPixel.index !== index
          )
            continue;
          var coverage = Math.min(
            255,
            incoming + Math.round((previous * (255 - incoming)) / 255),
          );
          _encodePixel(
            smart,
            smartOffset,
  ...
  function applyDiff(li, fi, beforeImage, styleId) {
    ...
    var before = beforeImage.data,
      now = after.data;
    var idx = ensureStyleIndex(li, fi, styleId);
    if (!idx) return;
    var bundle = ensureFrame(li, fi);
    if (!bundle) return;
    var sctx = bundle.canvas.getContext("2d", {
  ...
  function clearWhereTransparent(li, fi) {
    ...
    var sctx = l.indexFrames[fi].getContext("2d", {
      willReadFrequently: true,
      colorSpace: "srgb",
    });
    var img = sctx.getImageData(0, 0, CW, CH);
  ...
  function renderFrame(li, fi, targetCanvas) {
    ...
    var target =
      targetCanvas ||
      (li === curLayer && fi === curFrame
        ? activeC
        : layer.frames && layer.frames[fi]);
    if (!meta || !target) return false;
    var sctx = layer.indexFrames[fi].getContext("2d", {
      willReadFrequently: true,
      colorSpace: "srgb",
    });
    ...
    for (var i = 0; i < smart.length; i += 4) {
      ...
      out[i + 2] = rgba[2];
      out[i + 3] = Math.round(
        (coverage * (rgba[3] == null ? 255 : rgba[3])) / 255,
      );
    }
  ...
  function rerenderAll() {
    layers.forEach(function (layer, li) {
      ...
      Object.keys(layer.indexFrames).forEach(function (fi) {
        ...
        var target =
          li === curLayer && frameIndex === curFrame
            ? activeC
            : layer.frames && layer.frames[frameIndex];
  ...
  function rerenderStyle(styleId) {
    ...
    layers.forEach(function (layer, li) {
      ...
      Object.keys(layer.indexFrames).forEach(function (fi) {
        ...
        if (!meta || !meta.styleIdToIndex || !meta.styleIdToIndex[styleId])
          return;
        var frameIndex = Number(fi);
    ...
    if (activeAffected && typeof recomposite === "function")
  ...
  function resizeAllFrames(nw, nh, offsetX, offsetY) {
    layers.forEach(function (layer) {
      ...
      Object.keys(layer.indexFrames).forEach(function (fi) {
        ...
        var dy = Number.isFinite(offsetY)
          ? Math.round(offsetY)
  ...
  function markDeleted(styleId) {
    ...
    layers.forEach(function (layer) {
      ...
      Object.keys(layer.indexMeta).forEach(function (fi) {
        ...
        if (!idx) return;
        if (meta.indexToStyleId)
          meta.indexToStyleId[idx] = "__deleted__:" + styleId;
  ...
  function serializeLayer(layer) {
    ...
    keys.forEach(function (fi) {
      var c = layer.indexFrames[fi];
      if (!c) return;
      try {
        frames[fi] = c.toDataURL("image/png");
      } catch (e) {}
    });
    var meta = {};
    if (layer.indexMeta) {
  ...
  function deserializeLayer(layer, data) {
    ...
    frameKeys.forEach(function (fi) {
      ...
      img.onload = function () {
        ...
        tmp.width = img.width || CW;
        tmp.height = img.height || CH;
        ...
        if (data.meta && data.meta[fi])
          layer.indexMeta[fi] = _cloneMeta(data.meta[fi]);
        else
          layer.indexMeta[fi] = {
            indexToStyleId: {},
            styleIdToIndex: {},
  ...
  function _migrateExistingLayers() {
    ...
    layers.forEach(function (layer) {
      ...
      var hasIndexData = !!(
        layer.indexFrames && Object.keys(layer.indexFrames).length > 0
      );
  ...
  window.addEventListener(
    ...
    function (event) {
      ...
      if (
        styleId &&
        window.SmartRasterLayer &&
        typeof window.SmartRasterLayer.rerenderStyle === "function"
      )
        window.SmartRasterLayer.rerenderStyle(styleId);
    },
  );
