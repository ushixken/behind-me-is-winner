...
const cursor = fs.readFileSync(
  path.join(__dirname, "..", "brush", "brush-size-cursor.js"),
  "utf8",
);
const smartSources = ["smart-raster-typed.js", "smart-raster-v4-shadow.js"]
...
test("one pending or in-flight preview coalesces input into one newest-state follow-up", () => {
  ...
  assert.match(
    engine,
    /if\(_hardRoundPreviewRAF !== null\)\{[\s\S]*?_hardRoundPreviewRAFSession===_activeStrokeSession/,
  );
  assert.match(
    engine,
  ...
  assert.match(
    engine,
    /needsFollowup&&requestedRenderer&&_hardRoundPreviewSession===_activeStrokeSession&&_inStroke/,
  );
});
...
test("pending geometry is drained without dropping and follow-up is session safe", () => {
  assert.match(
    engine,
    /_hardRoundPendingRenderSegments\.splice\(0,_hardRoundPendingRenderSegments\.length\)/,
  );
  assert.match(
    engine,
    /_hardRoundPreviewNeedsFollowup\|\|_hardRoundPendingRenderSegments\.length>0/,
  );
