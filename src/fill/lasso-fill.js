(function(){
  'use strict';
  var active=false,pointerId=null,points=[],preview=null,previewContext=null,previewRaf=0,maskCanvas=null,maskContext=null;
  ...
  function ensureMask(){if(!maskCanvas){maskCanvas=document.createElement('canvas');maskContext=maskCanvas.getContext('2d',{willReadFrequently:true});}if(maskCanvas.width!==CW||maskCanvas.height!==CH){maskCanvas.width=CW;maskCanvas.height=CH;maskContext=maskCanvas.getContext('2d',{willReadFrequently:true});}}
  ...
  function commit(){
    if(!meaningful()){cancel();return;}var rect=bounds();if(!rect||!rect.w||!rect.h){cancel();return;}ensureMask();maskContext.clearRect(0,0,CW,CH);maskContext.fillStyle='#fff';maskContext.beginPath();maskContext.moveTo(points[0].x,points[0].y);for(var i=1;i<points.length;i++)maskContext.lineTo(points[i].x,points[i].y);
    // Self-crossing paths use Canvas 2D's nonzero winding rule.
    maskContext.closePath();maskContext.fill('nonzero');cancel();if(window.FillMaskEngine)FillMaskEngine.applyMask(maskCanvas,rect,{source:'lasso-fill'});
  }
