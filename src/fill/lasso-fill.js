(function () {
  ...
  function bounds() {
    ...
    var x = Math.max(0, Math.floor(minX - 1)),
      y = Math.max(0, Math.floor(minY - 1)),
      ex = Math.min(CW, Math.ceil(maxX + 1)),
      ey = Math.min(CH, Math.ceil(maxY + 1));
  ...
  function commit() {
    ...
    var samples = points.map(function (point) {
      ...
      path = window.FreehandClosedPath && FreehandClosedPath.build(samples);
    cancel();
    if (path && window.FillMaskEngine)
      FillMaskEngine.applyPath(path, rect, { source: "lasso-fill" });
  }
  function pointerDown(event) {
    if (
      ...
      panning ||
      spaceHeld ||
      (typeof isLayerLocked === "function" && isLayerLocked(curLayer))
    )
      return;
    if (
