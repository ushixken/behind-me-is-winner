(function () {
  ...
  function simplifyOpen(points, tolerance) {
    ...
    for (var i = 1; i < last; i++) {
      var distance = pointSegmentDistance(points[i], points[0], points[last]);
    ...
    var left = simplifyOpen(points.slice(0, index + 1), tolerance),
      right = simplifyOpen(points.slice(index), tolerance);
  ...
  function simplifyClosed(points, tolerance) {
    ...
    var first = simplifyOpen(points.slice(0, anchor + 1), tolerance),
      second = simplifyOpen(
        points.slice(anchor).concat([points[0]]),
        tolerance,
      );
  ...
  function turnAngle(a, b, c) {
    ...
    return (
      (Math.acos(Math.max(-1, Math.min(1, (ax * bx + ay * by) / (al * bl)))) *
        180) /
      Math.PI
    );
  }
  function append(commands, rawPoints) {
    var points = simplifyClosed(rawPoints, 0.6),
      count = points.length;
    if (count < 3) return;
    var start = {
    ...
    for (var i = 0; i < count; i++) {
      var previous = points[(i - 1 + count) % count],
        ...
        end = { x: (current.x + next.x) / 2, y: (current.y + next.y) / 2 };
      if (turnAngle(previous, current, next) >= 65)
        commands.push(
          { type: "line", x: current.x, y: current.y },
