(function () {
  ...
  function available() {
    return window.PixelSelection && PixelSelection.isActive();
  }
  ...
  function updateHover(event) {
    ...
    setMoveCursor(
      !!(available() && modifier(event) && PixelSelection.containsEvent(event)),
    );
  }
  ...
  function release(pointerId) {
    try {
      if (activeC.hasPointerCapture(pointerId))
        activeC.releasePointerCapture(pointerId);
    } catch (_) {}
  }
  function cleanup() {
    if (!gesture) return;
    var pointerId = gesture.pointerId;
    gesture = null;
    release(pointerId);
    setMoveCursor(false);
  }
  ...
  function pointerDown(event) {
    ...
    var point = getPos(event),
      inside = PixelSelection.containsEvent(event);
    gesture = {
  ...
  function pointerMove(event) {
    ...
    if (gesture.dragged && gesture.inside && !gesture.started)
      gesture.started = true;
    if (gesture.started)
      PixelSelection.setOverlayOffset(
  ...
  function pointerUp(event) {
    ...
    if (current.started) {
      ...
      LinkedPixelSelection.handleCanvasPointer
    ) {
  ...
  function cancel(event) {
    if (
      ...
      return;
    if (gesture.started) PixelSelection.setOverlayOffset(0, 0);
    cleanup();
  ...
  canvasArea.addEventListener("pointerleave", function () {
    if (!gesture) setMoveCursor(false);
  });
  document.addEventListener(
    ...
    function (event) {
      ...
      if ((event.key === "Control" || event.key === "Meta") && lastHoverEvent)
        updateHover({
          clientX: lastHoverEvent.clientX,
          clientY: lastHoverEvent.clientY,
  ...
  document.addEventListener(
    "keyup",
    function (event) {
      if (event.key === "Control" || event.key === "Meta") setMoveCursor(false);
    },
    true,
  );
  window.addEventListener("blur", function () {
    if (gesture) cancel({});
    else setMoveCursor(false);
  });
  window.addEventListener("tool-changed", function () {
    if (gesture) cancel({});
    lastHoverEvent = null;
    setMoveCursor(false);
  });
