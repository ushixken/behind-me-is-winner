(function () {
  ...
  function ensurePreview() {
    ...
    preview = EditorOverlayRenderer.create("rectangle-selection-preview", {
      zIndex: 7,
      draw: function (context, geometry) {
        var r = rect();
        if (!active || !r || !r.w || !r.h) return;
        var p0 = geometry.worldToScreen({ x: r.x, y: r.y }),
          p1 = geometry.worldToScreen({ x: r.x + r.w, y: r.y }),
          p2 = geometry.worldToScreen({ x: r.x + r.w, y: r.y + r.h }),
          p3 = geometry.worldToScreen({ x: r.x, y: r.y + r.h });
  ...
  function commit(event) {
    ...
    var r = rect(),
      ...
      x1 = Math.min(CW, Math.ceil(r.x + r.w)),
      y1 = Math.min(CH, Math.ceil(r.y + r.h));
    if (x1 <= x0 || y1 <= y0) {
      cancel();
      return;
    }
    var incoming = new Uint8ClampedArray(CW * CH);
    for (var y = y0; y < y1; y++) incoming.fill(255, y * CW + x0, y * CW + x1);
    var selectedMode = mode;
    cancel();
    if (window.PixelSelection)
      PixelSelection.applyMask(incoming, CW, CH, selectedMode, "rectangle");
  }
  function pointerDown(event) {
    if (tool !== "rectangle-select" || activeGroupId || panning || spaceHeld)
      return;
    if (
    ...
    mode = window.SelectionToolSettings
      ? SelectionToolSettings.modeFromEvent("rectangle-select", event)
  ...
  window.addEventListener("tool-changed", function (event) {
    if (active && event.detail && event.detail.tool !== "rectangle-select")
      cancel();
  });
  window.RectangleSelection = {
    cancel: cancel,
    isActive: function () {
      return active;
    },
