(function () {
  ...
  try {
    ...
    if (saved) {
      ...
      settings.combine =
        ["add", "replace", "subtract", "intersect"].indexOf(saved.combine) >= 0
          ? saved.combine
          : "add";
  ...
  function normalizeSettings() {
    settings.tolerance = clampNumber(settings.tolerance, 0, 255, 0);
    settings.edgeExpansion = clampNumber(settings.edgeExpansion, -20, 20, 0);
    settings.gapWidth = clampNumber(settings.gapWidth, 1, 10, 1);
    settings.sample = settings.sample === "all" ? "all" : "current";
    if (
      ["add", "replace", "subtract", "intersect"].indexOf(settings.combine) < 0
    )
      settings.combine = "add";
  }
  function persist() {
    try {
  ...
  function update(values) {
    ...
    normalizeSettings();
    persist();
    window.dispatchEvent(new CustomEvent("magic-wand-settings-changed"));
  }
  ...
  function normalizedPixel(data, offset) {
  ...
  function matches(data, offset, target, tolerance, includeAlpha) {
    ...
    if (!includeAlpha && (candidate[3] === 0) !== (target[3] === 0))
      return false;
    var distance = Math.max(
      Math.abs(candidate[0] - target[0]),
      Math.abs(candidate[1] - target[1]),
      Math.abs(candidate[2] - target[2]),
    );
    if (includeAlpha)
      distance = Math.max(distance, Math.abs(candidate[3] - target[3]));
    return distance <= tolerance;
  }
  function ensureWork(count) {
    if (!workVisited || workVisited.length !== count) {
      workVisited = new Uint8Array(count);
      workQueue = new Int32Array(count);
      workA = new Uint8Array(count);
      workB = new Uint8Array(count);
    } else {
      workVisited.fill(0);
      workA.fill(0);
      workB.fill(0);
    ...
    for (var y = 0; y < height; y++)
      for (var x = 0; x < width; x++) {
        ...
        for (
          ...
          yy <= Math.min(height - 1, y + 1) && !found;
          yy++
        )
          for (
  ...
  function repeatMorphology(source, width, height, steps, dilate) {
    ...
    for (var step = 0; step < steps; step++) {
      morphology(from, to, width, height, dilate);
      var swap = from;
      from = to;
      to = swap;
      to.fill(0);
  ...
  function keepSeedComponent(source, width, height, start) {
    var count = width * height,
      result = new Uint8ClampedArray(count);
    workVisited.fill(0);
    var head = 0,
      tail = 0;
    if (start < 0 || start >= count || source[start] !== 255) return result;
    workVisited[start] = 1;
    workQueue[tail++] = start;
    while (head < tail) {
      var pixel = workQueue[head++],
        x = pixel % width,
        y = (pixel / width) | 0,
        next;
      result[pixel] = 255;
      if (x > 0) {
        next = pixel - 1;
        if (source[next] === 255 && !workVisited[next]) {
          workVisited[next] = 1;
          workQueue[tail++] = next;
        }
      }
  ...
  function buildMask(image, startX, startY) {
    var data = image.data,
      ...
      target = normalizedPixel(data, start * 4);
    ensureWork(count);
    ...
    for (var pixel = 0, offset = 0; pixel < count; pixel++, offset += 4)
      if (
        matches(data, offset, target, settings.tolerance, settings.includeAlpha)
      )
        raw[pixel] = 255;
    var connectivity =
      settings.contiguous && settings.gapBridging
        ? bridgeSmallGaps(raw, width, height, settings.gapWidth)
        : raw;
    var result = settings.contiguous
      ? keepSeedComponent(connectivity, width, height, start)
      : new Uint8ClampedArray(raw);
    if (settings.edgeExpansion)
      result = repeatMorphology(
        result,
        width,
        height,
        Math.abs(settings.edgeExpansion),
        settings.edgeExpansion > 0,
      );
  ...
  function pointerDown(event) {
    ...
    var modified = event.shiftKey || event.altKey,
      mode =
        modified && window.PixelSelection
    ...
    function calculate() {
      ...
      if (token === operationToken && window.PixelSelection)
        PixelSelection.applyMask(mask, CW, CH, mode, "magic-wand");
      if (typeof _refreshActiveCursor === "function") _refreshActiveCursor();
    }
    if (CW * CH >= 1000000) {
      activeC.style.cursor = "wait";
  ...
  document.addEventListener(
    "keydown",
    function (event) {
      if (event.key === "Escape" && tool === "magic-wand") {
        operationToken++;
        if (typeof _refreshActiveCursor === "function") _refreshActiveCursor();
      }
    },
  ...
  window.addEventListener("tool-changed", function (event) {
    if (!event.detail || event.detail.tool !== "magic-wand") operationToken++;
  });
  activeC.addEventListener("pointerdown", pointerDown);
  window.MagicWandSelection = {
