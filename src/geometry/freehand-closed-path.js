(function () {
  ...
  function isIntentionalCorner(points, index) {
    ...
    var outerPrevious = points[(index - 2 + count) % count],
      outerNext = points[(index + 2) % count];
  ...
  function build(points) {
    ...
    var start = midpoint(points[count - 1], points[0]);
    commands.push({ type: "move", x: start.x, y: start.y });
    for (var i = 0; i < count; i++) {
      var current = points[i],
        next = points[(i + 1) % count],
        end = midpoint(current, next);
      if (isIntentionalCorner(points, i)) {
        commands.push({ type: "line", x: current.x, y: current.y });
