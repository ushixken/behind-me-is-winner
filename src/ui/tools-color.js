...
const onionSkinSettings = {
  ...
  previousOpacity: Array.from(
    { length: ONION_EXPOSURE_COUNT },
    (_, i) => (ONION_EXPOSURE_COUNT - i) / ONION_EXPOSURE_COUNT,
  ),
  nextOpacity: Array.from(
    { length: ONION_EXPOSURE_COUNT },
    (_, i) => (ONION_EXPOSURE_COUNT - i) / ONION_EXPOSURE_COUNT,
  ),
  currentOpacity: 1,
};
let onionTintCanvas = null,
  onionTintContext = null;

function _normalizeOnionSettings(saved) {
  if (!saved || typeof saved !== "object") return;
  const savedType = saved.type === "frame" ? "drawings" : saved.type;
  if (["drawings", "keyframe", "inbetween", "breakdown"].includes(savedType))
    onionSkinSettings.type = savedType;
  if (/^#[0-9a-f]{6}$/i.test(saved.previousColor || ""))
    onionSkinSettings.previousColor = saved.previousColor;
  if (/^#[0-9a-f]{6}$/i.test(saved.nextColor || ""))
    onionSkinSettings.nextColor = saved.nextColor;
  for (const side of ["previousEnabled", "nextEnabled"]) {
    if (Array.isArray(saved[side]))
      onionSkinSettings[side] = Array.from(
        { length: ONION_EXPOSURE_COUNT },
        (_, i) => saved[side][i] !== false,
      );
  }
  for (const side of ["previousOpacity", "nextOpacity"]) {
    if (Array.isArray(saved[side]))
      onionSkinSettings[side] = Array.from(
        { length: ONION_EXPOSURE_COUNT },
        (_, i) => Math.max(0, Math.min(1, Number(saved[side][i]) || 0)),
      );
  }
  if (Number.isFinite(saved.currentOpacity))
    onionSkinSettings.currentOpacity = Math.max(
      0,
      Math.min(1, saved.currentOpacity),
    );
  else if (
    Number.isFinite(saved.currentUpper) ||
    Number.isFinite(saved.currentLower)
  )
    onionSkinSettings.currentOpacity = Math.max(
      0,
      Math.min(
        1,
        ((Number(saved.currentUpper) || 0) +
          (Number(saved.currentLower) || 0)) /
          2,
      ),
    );
}
function _saveOnionSettings() {
  try {
    localStorage.setItem(
      ONION_SKIN_SETTINGS_KEY,
      JSON.stringify(onionSkinSettings),
    );
  } catch (_) {}
}
function _onionDrawingMatches(li, fi) {
  return (
    onionSkinSettings.type === "drawings" ||
    (typeof getDrawingMark === "function" &&
      getDrawingMark(li, fi) === onionSkinSettings.type)
  );
}
function _collectOnionDrawings(li, startFrame, direction) {
  const layer = layers[li],
    found = [];
  if (!layer || !layer.frames) return found;
  if (
    onionSkinSettings.type === "drawings" &&
    typeof getHeldKey === "function"
  ) {
    const currentDrawing = getHeldKey(li, curFrame),
      seen = new Set(currentDrawing ? [currentDrawing] : []);
    for (
      let frame = startFrame;
      frame >= 0 && frame < TOTAL && found.length < ONION_EXPOSURE_COUNT;
      frame += direction
    ) {
      const canvas = getHeldKey(li, frame);
      if (!canvas || seen.has(canvas)) continue;
      seen.add(canvas);
      found.push({ frameIndex: frame, canvas });
    }
    return found;
  }
  for (
    ...
    if (!layer.frames[frame]) continue;
    if (
      typeof isDrawingFrameHidden === "function" &&
      isDrawingFrameHidden(li, frame)
    )
      continue;
    if (!_onionDrawingMatches(li, frame)) continue;
    found.push({ frameIndex: frame, canvas: layer.frames[frame] });
  }
  return found;
}
function _drawTintedOnion(canvas, color, alpha) {
  if (
    !onionTintCanvas ||
    onionTintCanvas.width !== CW ||
    onionTintCanvas.height !== CH
  ) {
    onionTintCanvas = document.createElement("canvas");
    onionTintCanvas.width = CW;
    onionTintCanvas.height = CH;
    onionTintContext = onionTintCanvas.getContext("2d");
  }
  onionTintContext.setTransform(1, 0, 0, 1, 0, 0);
  onionTintContext.globalAlpha = 1;
  onionTintContext.globalCompositeOperation = "source-over";
  onionTintContext.clearRect(0, 0, CW, CH);
  onionTintContext.drawImage(canvas, 0, 0);
  onionTintContext.globalCompositeOperation = "source-in";
  onionTintContext.fillStyle = color;
  onionTintContext.fillRect(0, 0, CW, CH);
  onionTintContext.globalCompositeOperation = "source-over";
  octx.globalAlpha = alpha;
  octx.drawImage(onionTintCanvas, 0, 0);
  octx.globalAlpha = 1;
}
function updateOnion() {
  const trace = window.DEBUG_TIMELINE_ONION === true,
    masterEnabled = _onionMasterEnabled();
  if (trace)
    console.debug("[TimelineOnion] renderer-start", {
      masterEnabled,
  ...
  if (trace)
    console.debug("[TimelineOnion] canvas-cleared", { width: CW, height: CH });
  if (!masterEnabled) {
    if (trace)
      console.debug("[TimelineOnion] renderer-stop", {
        reason: "master-disabled",
      });
    if (typeof refreshDisplayComposite === "function")
  ...
  for (let layerIndex = 0; layerIndex < layers.length; layerIndex++) {
    ...
    if (trace)
      console.debug("[TimelineOnion] layer-read", {
        layerIndex,
        name: layer && layer.name,
        onionSkin: layer && layer.onionSkin,
        visible: layer && layer.visible,
        groupVisible,
      });
    if (
      !layer ||
      layer.onionSkin !== true ||
      layer.visible === false ||
      !groupVisible
    )
      continue;
    renderedLayers++;
    const previous = _collectOnionDrawings(layerIndex, curFrame - 1, -1),
      next = _collectOnionDrawings(layerIndex, curFrame + 1, 1);
    for (
      let i = Math.min(previous.length, ONION_EXPOSURE_COUNT) - 1;
      i >= 0;
      i--
    ) {
      if (previous[i] && onionSkinSettings.previousEnabled[i]) {
        _drawTintedOnion(
          previous[i].canvas,
          onionSkinSettings.previousColor,
          0.32 *
            onionSkinSettings.previousOpacity[i] *
            onionSkinSettings.currentOpacity,
        );
        drawCalls++;
      }
    }
    for (let i = Math.min(next.length, ONION_EXPOSURE_COUNT) - 1; i >= 0; i--) {
      if (next[i] && onionSkinSettings.nextEnabled[i]) {
        _drawTintedOnion(
          next[i].canvas,
          onionSkinSettings.nextColor,
          0.24 *
            onionSkinSettings.nextOpacity[i] *
            onionSkinSettings.currentOpacity,
        );
        drawCalls++;
      }
  ...
  if (trace)
    console.debug("[TimelineOnion] renderer-complete", {
      renderedLayers,
      drawCalls,
    });
  if (typeof refreshDisplayComposite === "function") refreshDisplayComposite();
}
function _onionMasterEnabled() {
  return (
    document.getElementById("btn-onion-skin")?.getAttribute("aria-pressed") ===
    "true"
  );
}
function setOnionSkinEnabled(enabled, { persist = true } = {}) {
  ...
  if (persist) {
    try {
      localStorage.setItem(ONION_SKIN_PREF_KEY, enabled ? "true" : "false");
    } catch (_) {}
  }
  updateOnion();
  if (typeof _syncOnionMasterState === "function") _syncOnionMasterState();
  if (typeof _renderOnionExposureGraph === "function")
    _renderOnionExposureGraph();
}
function toggleOnionSkin() {
  setOnionSkinEnabled(!_onionMasterEnabled());
}
let onionExposurePaint = null;
function _defaultOnionExposure(distance) {
  return (ONION_EXPOSURE_COUNT - distance + 1) / ONION_EXPOSURE_COUNT;
}
function _setOnionExposureEnabled(side, distance, enabled) {
  const key = side === "previous" ? "previousEnabled" : "nextEnabled";
  onionSkinSettings[key][distance - 1] = !!enabled;
  _saveOnionSettings();
  _renderOnionExposureGraph();
  updateOnion();
}
function _syncOnionMasterState() {
  const enabled = _onionMasterEnabled(),
    panel = document.getElementById("onion-skin-panel"),
    graph = document.getElementById("onion-exposure-graph"),
    previous = document.getElementById("onion-previous-color"),
    next = document.getElementById("onion-next-color");
  panel && panel.classList.toggle("onion-master-off", !enabled);
  graph && graph.classList.toggle("is-master-off", !enabled);
  if (previous) previous.disabled = !enabled;
  if (next) next.disabled = !enabled;
}
function _renderOnionExposureGraph() {
  const graph = document.getElementById("onion-exposure-graph");
  if (!graph) return;
  graph.replaceChildren();
  const masterEnabled = _onionMasterEnabled();
  graph.classList.toggle("is-master-off", !masterEnabled);
  const addExposure = (side, distance, label) => {
    const current = side === "current",
      ...
      enabled = current
        ? _onionMasterEnabled()
        : onionSkinSettings[enabledKey][distance - 1],
      opacity = current
        ? onionSkinSettings.currentOpacity
        : onionSkinSettings[opacityKey][distance - 1],
      color = current
        ? "var(--accent)"
        : side === "previous"
          ? onionSkinSettings.previousColor
    ...
    item.className =
      "onion-exposure-item" + (current ? " onion-exposure-current" : "");
    item.dataset.side = side;
    item.dataset.distance = String(distance);
    item.style.setProperty("--onion-color", color);
    item.style.setProperty("--onion-opacity", String(opacity));
    item.classList.toggle("is-off", !enabled);
    item.classList.toggle("is-master-disabled", !current && !masterEnabled);
    const track = document.createElement("div");
    track.className = "onion-exposure-track";
    track.tabIndex = 0;
    track.setAttribute("role", "slider");
    track.setAttribute(
      "aria-label",
      (current ? "Frame 0" : side + " " + distance) + " opacity",
    );
    track.setAttribute("aria-valuemin", "0");
    track.setAttribute("aria-valuemax", "100");
    track.setAttribute("aria-valuenow", String(Math.round(opacity * 100)));
    const bar = document.createElement("div");
    bar.className = "onion-exposure-bar";
    const updateValue = (value) => {
      value = Math.max(0, Math.min(1, value));
      if (current) onionSkinSettings.currentOpacity = value;
      else onionSkinSettings[opacityKey][distance - 1] = value;
      item.style.setProperty("--onion-opacity", String(value));
      track.setAttribute("aria-valuenow", String(Math.round(value * 100)));
      updateOnion();
    };
    const updateFromPointer = (event) => {
      const rect = track.getBoundingClientRect();
      updateValue((rect.bottom - event.clientY) / Math.max(1, rect.height));
    };
    track.addEventListener("pointerdown", (event) => {
      if (!current && !masterEnabled) return;
    ...
    track.addEventListener("keydown", (event) => {
      let value = current
        ? onionSkinSettings.currentOpacity
        : onionSkinSettings[opacityKey][distance - 1];
      if (event.key === "ArrowUp" || event.key === "ArrowRight") value += 0.05;
    ...
    track.addEventListener("dblclick", (event) => {
      if (!current && !masterEnabled) return;
      event.preventDefault();
      const value = current ? 1 : _defaultOnionExposure(distance);
      if (current) onionSkinSettings.currentOpacity = value;
      else onionSkinSettings[opacityKey][distance - 1] = value;
      _saveOnionSettings();
      _renderOnionExposureGraph();
    ...
    toggle.type = "button";
    toggle.className = "onion-exposure-toggle";
    toggle.textContent = label;
    toggle.setAttribute("aria-pressed", String(enabled));
    if (!current && !masterEnabled) toggle.disabled = true;
    toggle.title =
      (enabled ? "Disable " : "Enable ") +
      (current ? "onion skin" : side + " " + distance);
    toggle.addEventListener("pointerdown", (event) => {
      ...
      onionExposurePaint = {
        ...
        enabled: !onionSkinSettings[enabledKey][distance - 1],
      };
      _setOnionExposureEnabled(side, distance, onionExposurePaint.enabled);
    });
    if (!current)
      toggle.addEventListener("pointerenter", (event) => {
        if (
          onionExposurePaint &&
          event.buttons &&
          event.pointerId === onionExposurePaint.pointerId
        )
          _setOnionExposureEnabled(side, distance, onionExposurePaint.enabled);
      });
  ...
  for (let distance = 10; distance >= 1; distance--)
    addExposure("previous", distance, String(distance));
  addExposure("current", 0, "0");
  for (let distance = 1; distance <= 10; distance++)
    addExposure("next", distance, String(distance));
}
document.addEventListener("pointerup", (event) => {
  if (onionExposurePaint && event.pointerId === onionExposurePaint.pointerId)
    onionExposurePaint = null;
});
document.addEventListener("pointercancel", (event) => {
  if (onionExposurePaint && event.pointerId === onionExposurePaint.pointerId)
    onionExposurePaint = null;
});
const ONION_TYPE_LABELS = {
  drawings: "Drawings",
  keyframe: "Keyframe",
  inbetween: "Inbetween",
  breakdown: "Breakdown",
};
let onionTypeMenu = null,
  onionTypeMenuOpen = false;
function _setOnionType(value) {
  if (!ONION_TYPE_LABELS[value]) return;
  onionSkinSettings.type = value;
  _saveOnionSettings();
  _syncOnionPanel();
  updateOnion();
}
function _buildOnionTypeMenu() {
  const trigger = document.getElementById("onion-type-select");
  if (onionTypeMenu || !trigger) return;
  onionTypeMenu = document.createElement("div");
  onionTypeMenu.id = "onion-type-menu";
  onionTypeMenu.className = "ts-brush-blend-mode-menu onion-type-menu";
  onionTypeMenu.role = "listbox";
  onionTypeMenu.setAttribute("aria-label", "Onion skin drawing type");
  onionTypeMenu.hidden = true;
  Object.entries(ONION_TYPE_LABELS).forEach(([value, label]) => {
    ...
    option.addEventListener("click", () => {
      _setOnionType(value);
      _closeOnionTypeMenu(true);
    });
    option.addEventListener("keydown", (event) => {
      const options = [...onionTypeMenu.querySelectorAll("button")],
      ...
      if (event.key === "ArrowDown" || event.key === "ArrowUp") {
        ...
        event.preventDefault();
        _closeOnionTypeMenu(true);
      } else if (event.key === "Tab") _closeOnionTypeMenu(false);
    });
    onionTypeMenu.appendChild(option);
  });
  document.body.appendChild(onionTypeMenu);
  trigger.setAttribute("aria-controls", onionTypeMenu.id);
}
function _positionOnionTypeMenu() {
  const trigger = document.getElementById("onion-type-select");
  if (!onionTypeMenuOpen || !onionTypeMenu || !trigger) return;
  const rect = trigger.getBoundingClientRect(),
    gap = 6,
    spaceBelow = innerHeight - rect.bottom - gap,
    openUp = spaceBelow < onionTypeMenu.offsetHeight && rect.top > spaceBelow;
  onionTypeMenu.style.width = Math.max(110, rect.width) + "px";
  onionTypeMenu.style.left =
    Math.max(
      gap,
      Math.min(rect.left, innerWidth - onionTypeMenu.offsetWidth - gap),
    ) + "px";
  onionTypeMenu.style.top =
    (openUp
      ? Math.max(gap, rect.top - onionTypeMenu.offsetHeight - 4)
      : rect.bottom + 4) + "px";
}
function _openOnionTypeMenu() {
  ...
  _buildOnionTypeMenu();
  if (!onionTypeMenu || !trigger) return;
  onionTypeMenuOpen = true;
  onionTypeMenu.hidden = false;
  trigger.setAttribute("aria-expanded", "true");
  _syncOnionTypeMenu();
  _positionOnionTypeMenu();
  const selected =
    onionTypeMenu.querySelector(".is-selected") ||
    onionTypeMenu.querySelector("button");
  selected && selected.focus({ preventScroll: true });
}
function _closeOnionTypeMenu(focus) {
  const trigger = document.getElementById("onion-type-select");
  if (!onionTypeMenu) return;
  onionTypeMenuOpen = false;
  onionTypeMenu.hidden = true;
...
function _syncOnionTypeMenu() {
  const value = document.getElementById("onion-type-value");
  if (value)
    value.textContent = ONION_TYPE_LABELS[onionSkinSettings.type] || "Drawings";
  if (onionTypeMenu)
    onionTypeMenu
      .querySelectorAll(".ts-brush-blend-mode-option")
      .forEach((option) => {
        const selected = option.dataset.mode === onionSkinSettings.type;
        option.classList.toggle("is-selected", selected);
        option.setAttribute("aria-selected", String(selected));
      });
}
function _syncOnionPanel() {
  const type = document.getElementById("onion-type-select"),
    previous = document.getElementById("onion-previous-color"),
    next = document.getElementById("onion-next-color");
  _syncOnionTypeMenu();
  if (previous) previous.value = onionSkinSettings.previousColor;
  if (next) next.value = onionSkinSettings.nextColor;
  _syncOnionMasterState();
  _renderOnionExposureGraph();
}
window.setOnionSkinEnabled = setOnionSkinEnabled;
window.toggleOnionSkin = toggleOnionSkin;
window.updateOnionSkinPanel = _syncOnionPanel;
(function initOnionSkinPreference() {
  let enabled = false;
  try {
    const saved = localStorage.getItem(ONION_SKIN_PREF_KEY);
    if (saved !== null) enabled = saved === "true";
    _normalizeOnionSettings(
      JSON.parse(localStorage.getItem(ONION_SKIN_SETTINGS_KEY) || "null"),
    );
  } catch (_) {}
  _syncOnionPanel();
  setOnionSkinEnabled(enabled, { persist: false });
  document
    .getElementById("btn-onion-skin")
  ...
  if (type) {
    type.addEventListener("click", () =>
      onionTypeMenuOpen ? _closeOnionTypeMenu(false) : _openOnionTypeMenu(),
    );
    type.addEventListener("keydown", (event) => {
      if (["ArrowDown", "ArrowUp", "Enter", " "].includes(event.key)) {
        event.preventDefault();
        _openOnionTypeMenu();
      } else if (event.key === "Escape") _closeOnionTypeMenu(false);
    });
  }
  document.addEventListener(
    ...
    (event) => {
      if (
        onionTypeMenuOpen &&
        !onionTypeMenu.contains(event.target) &&
        !type.contains(event.target)
      )
        _closeOnionTypeMenu(false);
    },
    true,
  );
  document.addEventListener(
    "scroll",
    () => {
      if (onionTypeMenuOpen) _positionOnionTypeMenu();
    },
    true,
  );
  ...
  previous &&
    previous.addEventListener("input", (event) => {
      onionSkinSettings.previousColor = event.currentTarget.value;
      _saveOnionSettings();
      _renderOnionExposureGraph();
      updateOnion();
    });
  next &&
    next.addEventListener("input", (event) => {
      onionSkinSettings.nextColor = event.currentTarget.value;
      _saveOnionSettings();
      _renderOnionExposureGraph();
      updateOnion();
    });
  window.addEventListener("local-storage-preference-removed", (event) => {
    if (event.detail && event.detail.key === ONION_SKIN_PREF_KEY)
      setOnionSkinEnabled(false, { persist: false });
  });
})();
...
function _currentUndoSnapshot() {
  ...
  const selectionSnapshot =
    window.PixelSelection && PixelSelection.capture
      ? PixelSelection.capture()
      : null;
  const layerType =
    layer && layer.type === "smart-raster" ? "smart-raster" : "bitmap";
  const extendedSnapshot =
    typeof getExtendedLayerFrame === "function" &&
    typeof cloneExtendedFrameRecord === "function"
      ? cloneExtendedFrameRecord(getExtendedLayerFrame(curLayer, curFrame))
      : null;
...
function pushUndo() {
  if (typeof window.markProjectDirty === "function")
    window.markProjectDirty("push-undo");
...
function pushUndoAt(layerIndex, frameIndex) {
  if (typeof window.markProjectDirty === "function")
    window.markProjectDirty("push-undo-at");
  const layer = layers[layerIndex];
  const selectionSnapshot =
    window.PixelSelection && PixelSelection.capture
      ? PixelSelection.capture()
  ...
  const extendedSnapshot =
    typeof getExtendedLayerFrame === "function" &&
    typeof cloneExtendedFrameRecord === "function"
      ? cloneExtendedFrameRecord(getExtendedLayerFrame(layerIndex, frameIndex))
      : null;
  let snap = null,
    styleBundle = null;
  if (layerType === "smart-raster")
...
function restoreBitmapUndo(action) {
  ...
  if (action.extendedSnapshot && typeof setExtendedLayerFrame === "function")
    setExtendedLayerFrame(
      action.layer,
      action.frame,
      action.extendedSnapshot.canvas,
      action.extendedSnapshot.x,
      action.extendedSnapshot.y,
    );
  else if (typeof clearExtendedLayerFrame === "function")
    clearExtendedLayerFrame(action.layer, action.frame);
...
function restoreSmartRasterUndo(action) {
  ...
  const layer = layers[action.layer];
  if (!layer) return;
  SmartRasterLayer.restoreFrameBundle(
    action.layer,
    action.frame,
    action.styleBundle,
  );
...
function _restoreUndoAction(action) {
  ...
  if (
    Object.prototype.hasOwnProperty.call(action, "selectionSnapshot") &&
    window.PixelSelection &&
    PixelSelection.restore
  )
    PixelSelection.restore(action.selectionSnapshot);
}
...
function _undoSmartRasterDuplicateLayer(action) {
  if (action.index >= 0 && action.index < layers.length)
...
function _redoSmartRasterDuplicateLayer(action) {
  ...
  if (
    ...
    window.SmartRasterV4Document.restoreLayer(restored);
...
function _restoreStyleLayeringToggle(action, direction) {
  const snapshot =
      direction === "before" ? action.beforeLayer : action.afterLayer,
    index = Math.max(0, Math.min(action.layerIndex, layers.length - 1));
  if (!snapshot || typeof _deepCopyLayer !== "function") return false;
  const restored = _deepCopyLayer(snapshot);
  layers[index] = restored;
  if (
    window.SmartRasterV4Document &&
    typeof window.SmartRasterV4Document.restoreLayer === "function"
  )
  ...
  window.dispatchEvent(
    new CustomEvent("smart-raster-style-layering-changed", {
      detail: {
        layer: restored,
        enabled: restored.renderMode === "style-layering",
      },
    }),
  );
  return true;
...
function _restoreStyleLayeringOrder(snapshot) {
  if (
    ...
    typeof window.PaletteDocker.restoreAdvancedStyleOrder !== "function"
  )
    return false;
  window.PaletteDocker.restoreAdvancedStyleOrder(snapshot);
  recomposite(curLayer, curFrame);
  return true;
}
function _restoreDrawingFrameHidden(action, hidden) {
  if (
    typeof setDrawingFrameHidden !== "function" ||
    !setDrawingFrameHidden(action.layer, action.frame, hidden)
  )
    return false;
...
function _restoreLayerHierarchy(snapshot) {
  ...
  (snapshot.layerGroupIds || []).forEach((groupId, index) => {
    if (layers[index]) layers[index].groupId = groupId || null;
  });
  curLayer = Math.max(
    0,
    Math.min(layers.length - 1, Number(snapshot.curLayer) || 0),
  );
...
function undo() {
  ...
  if (typeof window.markProjectDirty === "function")
    window.markProjectDirty("undo");
  const action = undoStack.pop();
  if (action.type === "audio-clips" && window.AudioClipUI) {
    if (AudioClipUI.restore(action.before)) redoStack.push(action);
    return;
  }
  if (action.type === "camera-state" && window.CameraSystem) {
    CameraSystem.restore(action.before);
    redoStack.push(action);
    if (typeof renderTimeline === "function") renderTimeline();
    return;
  }
  if (action.type === "camera-track" && window.CameraSystem) {
    CameraSystem.restoreTrack(action.before);
    redoStack.push(action);
    if (typeof renderTimeline === "function") renderTimeline();
    return;
  }
  if (action.type === "canvas-resize" && window.CanvasResizeTool) {
    if (CanvasResizeTool.restoreHistory(action.before)) redoStack.push(action);
    return;
  }
  if (action.type === "layer-hierarchy") {
    if (_restoreLayerHierarchy(action.before)) redoStack.push(action);
    return;
  }
  if (action.type === "drawing-frame-hidden") {
    if (_restoreDrawingFrameHidden(action, action.before))
      redoStack.push(action);
    return;
  }
  if (action.type === "smart-raster-style-layering-toggle") {
    if (_restoreStyleLayeringToggle(action, "before")) redoStack.push(action);
    return;
  }
  if (action.type === "smart-raster-style-order") {
    if (_restoreStyleLayeringOrder(action.before)) redoStack.push(action);
    return;
  }
  ...
  if (action.type === "timeline-frames") {
    _restoreFrameMaps(action.before);
    redoStack.push(action);
    selectedKFs.clear();
    loadFrame(curLayer, curFrame);
    recomposite(curLayer, curFrame);
...
function redo() {
  ...
  if (typeof window.markProjectDirty === "function")
    window.markProjectDirty("redo");
  const action = redoStack.pop();
  if (action.type === "audio-clips" && window.AudioClipUI) {
    if (AudioClipUI.restore(action.after)) undoStack.push(action);
    return;
  }
  if (action.type === "camera-state" && window.CameraSystem) {
    CameraSystem.restore(action.after);
    undoStack.push(action);
    if (typeof renderTimeline === "function") renderTimeline();
    return;
  }
  if (action.type === "camera-track" && window.CameraSystem) {
    CameraSystem.restoreTrack(action.after);
    undoStack.push(action);
    if (typeof renderTimeline === "function") renderTimeline();
    return;
  }
  if (action.type === "canvas-resize" && window.CanvasResizeTool) {
    if (CanvasResizeTool.restoreHistory(action.after)) undoStack.push(action);
    return;
  }
  if (action.type === "layer-hierarchy") {
    if (_restoreLayerHierarchy(action.after)) undoStack.push(action);
    return;
  }
  if (action.type === "drawing-frame-hidden") {
    if (_restoreDrawingFrameHidden(action, action.after))
      undoStack.push(action);
    return;
  }
  if (action.type === "smart-raster-style-layering-toggle") {
    if (_restoreStyleLayeringToggle(action, "after")) undoStack.push(action);
    return;
  }
  if (action.type === "smart-raster-style-order") {
    if (_restoreStyleLayeringOrder(action.after)) undoStack.push(action);
    return;
  }
  if (action.type === "layer-cut") {
  ...
  if (action.type === "timeline-frames") {
    _restoreFrameMaps(action.after);
    undoStack.push(action);
    selectedKFs.clear();
    loadFrame(curLayer, curFrame);
    recomposite(curLayer, curFrame);
...
function _syncBrushPresetsDocker(t) {
  ...
  if (nameEl)
    nameEl.textContent = showTransform ? "Transform" : "Brush Presets";
...
function setTool(t, lbl) {
  ...
  if (window.DEBUG_TOOL_LIFECYCLE && previousTool !== t) {
    ...
    console.groupCollapsed(
      "[ToolLifecycle] setTool " + previousTool + " -> " + t,
    );
    console.log({
      functionName: "setTool",
      caller: (stack.split("\\n")[2] || "").trim(),
      previousTool,
      newTool: t,
      label: lbl,
  ...
  if (
    tool === "resize-canvas" &&
    t !== "resize-canvas" &&
    window.CanvasResizeTool &&
    CanvasResizeTool.active
  )
    CanvasResizeTool.cancel();
  if (tool === "camera" && t !== "camera" && window.CameraSystem)
    CameraSystem.exit();
  if (
    tool === "transform" &&
    t !== "transform" &&
    typeof cancelTransformTool === "function"
  ) {
    if (
      window.RepeatableTransformController &&
      RepeatableTransformController.active
  ...
  if (tool === "lasso" && t !== "lasso" && window.LassoSelection)
    LassoSelection.cancel();
  if (tool === "lasso-fill" && t !== "lasso-fill" && window.LassoFill)
    LassoFill.cancel();
  tool = t;
...
colorIn.oninput = (e) => {
  ...
  swatchEl.style.background = color;
  document.getElementById("stat-color").textContent = "Color: " + color;
  if (typeof refreshColorSwatches === "function") refreshColorSwatches();
  if (typeof syncColorPanelSwatches === "function") syncColorPanelSwatches();
  window.dispatchEvent(
    new CustomEvent("foreground-color-changed", { detail: { color } }),
  );
};
...
// ════════════════════════════════════════════════════════════════
document.getElementById("tp-btn-brush").onclick = () =>
  setTool("brush", "Brush");
document.getElementById("tp-btn-eraser").onclick = () =>
...
document.getElementById("tp-btn-eyedropper").onclick = () =>
  setTool("eyedropper", "Eyedropper");
document.getElementById("tp-btn-selection").onclick = () =>
  setTool("lasso", "Lasso Select");
document.getElementById("tp-btn-transform").onclick = () =>
  setTool("transform", "Transform");
document.getElementById("tp-btn-canvas-resize").onclick = () =>
  setTool("resize-canvas", "Resize Canvas");
...
(function () {
  ...
  document.addEventListener("pointerup", (e) => {
    if (dragging && draggedOut) {
      ...
      if (!colorPanel.style.width || parseFloat(colorPanel.style.width) > 400)
        colorPanel.style.width = "200px";
      if (!colorPanel.style.height || parseFloat(colorPanel.style.height) > 500)
        colorPanel.style.height = "240px";
      ...
      if (FloatPanels.setFloatSize) {
        FloatPanels.setFloatSize(
          "color",
          parseFloat(colorPanel.style.width),
          parseFloat(colorPanel.style.height),
        );
      }
...
(function () {
  ...
  function drawSq() {
    ...
    gH.addColorStop(1, `hsl(${mpH},100%,50%)`);
    sqCtx.fillStyle = gH;
    ...
    sqCtx.fillRect(0, 0, SQ, SQ);
    sqCtx.beginPath();
    sqCtx.arc(sqX, sqY, 7, 0, Math.PI * 2);
    sqCtx.strokeStyle = "#fff";
    sqCtx.lineWidth = 2;
    sqCtx.stroke();
    sqCtx.beginPath();
    sqCtx.arc(sqX, sqY, 5, 0, Math.PI * 2);
    sqCtx.strokeStyle = "rgba(0,0,0,0.5)";
    sqCtx.lineWidth = 1.5;
    sqCtx.stroke();
  }
  ...
  function drawHue() {
    ...
    hCtx.fillStyle = g;
    hCtx.fillRect(0, 0, 16, SQ);
    const ty = Math.round(hueY);
    hCtx.fillStyle = "rgba(0,0,0,0.35)";
    hCtx.fillRect(0, ty - 3, 16, 6);
    hCtx.strokeStyle = "#fff";
    hCtx.lineWidth = 2;
    hCtx.strokeRect(1, ty - 3, 14, 6);
  }
  ...
  sqCanvas.addEventListener("pointerdown", (e) => {
    e.preventDefault();
    sqCanvas.setPointerCapture(e.pointerId);
    onSqMove(e);
  ...
  window.openMiniPicker = function (anchorRect, triggerEvent, target) {
    ...
    if (pendingDragPointerId != null) {
      ...
      try {
        prevAnchor.releasePointerCapture(pendingDragPointerId);
      } catch (_) {}
      pendingDragPointerId = null;
    }
    mpTarget = target || null;
    const initial = mpTarget && mpTarget.getColor ? mpTarget.getColor() : color;
    [mpH, mpS, mpV] = hexToHsv(
      initial && initial !== "transparent" ? initial : "#000000",
    );
    syncThumbs();
    drawSq();
    drawHue();
  ...
  if (window.TransientManager) {
    window.TransientManager.register({
      id: "mini-picker",
      isOpen: () => !!mpOpen,
      close: () => closeMiniPicker(),
      contains: (target) => {
        const anchor = (mpTarget && mpTarget.anchorEl) || csStack;
        return !!(
          (mp && mp.contains(target)) ||
  ...
  document.addEventListener(
    ...
    (e) => {
      const anchor = (mpTarget && mpTarget.anchorEl) || csStack;
      if (mpOpen && !mp.contains(e.target) && !anchor.contains(e.target))
        closeMiniPicker();
    },
    { capture: true },
  );
...
const cpL1 = document.getElementById("cp-l1"),
  cpL2 = document.getElementById("cp-l2"),
  cpL3 = document.getElementById("cp-l3");
const cpS1 = document.getElementById("cp-s1"),
  cpS2 = document.getElementById("cp-s2"),
  cpS3 = document.getElementById("cp-s3");
const cpN1 = document.getElementById("cp-n1"),
  cpN2 = document.getElementById("cp-n2"),
  cpN3 = document.getElementById("cp-n3");
const cpRow2 = document.getElementById("cp-row2"),
  cpRow3 = document.getElementById("cp-row3");
...
let cpHue = 0,
  cpSat = 1,
  cpVal = 0;
const WHEEL_R_BASE = 120,
  WHEEL_THICK_BASE = 18;
// Dynamic getters — read current canvas size so drawing always matches scale
function getWheelR() {
  return cpWheelCanvas.width / 2;
}
function getWheelThick() {
  return Math.round(
    WHEEL_THICK_BASE * (cpWheelCanvas.width / (WHEEL_R_BASE * 2)),
  );
}
...
function drawWheel() {
  ...
  if (cpMode === "v") {
    cpWheelCanvas.style.opacity = "0.25";
  } else {
    cpWheelCanvas.style.opacity = "1";
  }
  // marker
  const ha = (cpHue * Math.PI) / 180,
    mr = (r + ri) / 2;
  ctx.beginPath();
  ctx.arc(cx + mr * Math.cos(ha), cy + mr * Math.sin(ha), 6, 0, Math.PI * 2);
  ctx.strokeStyle = "#fff";
  ctx.lineWidth = 2;
  ctx.stroke();
  ctx.strokeStyle = "#000";
...
function drawInner() {
  ...
  if (cpShape === "square") {
    ...
    const cx = WR,
      cy = WR;
    cpSqCanvas.width = side;
    cpSqCanvas.height = side;
    cpSqCanvas.style.left = cx - side / 2 + "px";
    cpSqCanvas.style.top = cy - side / 2 + "px";
    ...
    if (cpMode === "v") {
      ...
      gH.addColorStop(0, "#fff");
      gH.addColorStop(1, `hsl(${cpHue},100%,50%)`);
      ctx.fillStyle = gH;
      ctx.fillRect(0, 0, side, side);
    ...
    const sz = innerR * 2,
      ...
      cy = WR;
    cpSqCanvas.width = sz;
    cpSqCanvas.height = sz;
    cpSqCanvas.style.left = cx - sz / 2 + "px";
    cpSqCanvas.style.top = cy - sz / 2 + "px";
    ...
    const vH = [
      ocx + innerR * Math.cos(angleHue),
      ocy + innerR * Math.sin(angleHue),
    ];
    const vW = [
      ocx + innerR * Math.cos(angleWhite),
      ocy + innerR * Math.sin(angleWhite),
    ];
    const vB = [
      ocx + innerR * Math.cos(angleBlack),
      ocy + innerR * Math.sin(angleBlack),
    ];
    ...
    if (cpMode === "v") {
      ...
      g1.addColorStop(1, `hsl(${cpHue},100%,50%)`);
      ctx2.fillStyle = g1;
      ctx2.fillRect(0, 0, sz, sz);
    ...
    const px =
      vW[0] * (1 - cpSat) * cpVal + vH[0] * cpSat * cpVal + vB[0] * (1 - cpVal);
    const py =
      vW[1] * (1 - cpSat) * cpVal + vH[1] * cpSat * cpVal + vB[1] * (1 - cpVal);
    ctx2.beginPath();
...
function applyModeUI() {
  ...
  cpL1.textContent = cfg.labels[0];
  cpL2.textContent = cfg.labels[1];
  cpL3.textContent = cfg.labels[2];
  cpS1.max = cfg.max[0];
  cpN1.max = cfg.max[0];
  cpS2.max = cfg.max[1];
  cpN2.max = cfg.max[1];
  ...
  [cpModeRGB, cpModeHSV, cpModeHSL, cpModeV].forEach((b) =>
    b.classList.remove("active"),
  );
  if (cpSlidersVisible) {
    ({ rgb: cpModeRGB, hsv: cpModeHSV, hsl: cpModeHSL, v: cpModeV })[
      cpMode
...
function getSliderValues() {
  ...
  if (cpMode === "hsv")
    return [
      Math.round(cpHue),
      Math.round(cpSat * 100),
      Math.round(cpVal * 100),
    ];
  if (cpMode === "hsl") {
...
function _applyColorLive(hex) {
  ...
  if (colorTarget === "fg") {
    if (
      window.PaletteDocker &&
      typeof window.PaletteDocker.updateActiveAdvancedStyleFromColorPanel ===
        "function"
    )
      window.PaletteDocker.updateActiveAdvancedStyleFromColorPanel(hex);
    window.dispatchEvent(
      new CustomEvent("foreground-color-changed", { detail: { color: hex } }),
    );
  }
}
...
function currentEditColor() {
  return colorTarget === "fg" ? color : bgDrawColor;
}
...
window.setForegroundColorFromPalette = function (
  hex,
  openPicker,
  skipPaletteRender,
) {
  ...
  if (openPicker) {
    ...
    if (typeof FloatPanels !== "undefined" && FloatPanels.setVisible)
      FloatPanels.setVisible("color", true);
    if (panel && typeof FloatPanels !== "undefined" && FloatPanels.bringToFront)
      FloatPanels.bringToFront(panel);
  }
  if (
    !skipPaletteRender &&
    window.PaletteDocker &&
    typeof window.PaletteDocker.renderCurrentColors === "function"
  )
    window.PaletteDocker.renderCurrentColors();
};

// ── Slider / number input interaction ──
function _advancedHistoryRgba() {
  return window.PaletteDocker &&
    typeof window.PaletteDocker.getActiveAdvancedStyleColorForHistory ===
      "function"
    ? window.PaletteDocker.getActiveAdvancedStyleColorForHistory()
    : null;
}
function _sameHistoryRgba(a, b) {
  return !!(
    a &&
    b &&
    a.length === 4 &&
...
function _recordAdvancedHistoryAfter(start) {
  ...
  if (
    start &&
    finalRgba &&
    !_sameHistoryRgba(start, finalRgba) &&
    window.AdvancedColorHistory
  )
    window.AdvancedColorHistory.record(finalRgba);
}
function onSliderChange() {
  let rgb;
  if (cpMode === "rgb") {
    rgb = [+cpS1.value, +cpS2.value, +cpS3.value];
  } else if (cpMode === "hsv") {
    [cpHue, cpSat, cpVal] = [+cpS1.value, +cpS2.value / 100, +cpS3.value / 100];
    rgb = hsvToRgb(cpHue, cpSat, cpVal);
  } else if (cpMode === "hsl") {
    rgb = hslToRgb(+cpS1.value, +cpS2.value / 100, +cpS3.value / 100);
    [cpHue, cpSat, cpVal] = rgbToHsv(...rgb);
  } else if (cpMode === "v") {
    const v = +cpS1.value / 100;
    cpSat = 0;
    cpVal = v;
    rgb = hsvToRgb(cpHue, 0, v);
  }
  if (rgb) {
    _applyColorLive(rgbToHex(...rgb));
    cpN1.value = cpS1.value;
    cpN2.value = cpS2.value;
    cpN3.value = cpS3.value;
    drawPicker();
  }
}
function onNumberChange() {
  cpS1.value = cpN1.value;
  cpS2.value = cpN2.value;
  cpS3.value = cpN3.value;
  onSliderChange();
}
...
[cpS1, cpS2, cpS3, cpN1, cpN2, cpN3].forEach((input) => {
  const begin = () => {
    const rgba = _advancedHistoryRgba();
    if (rgba) _colorControlStarts.set(input, rgba);
    else _colorControlStarts.delete(input);
  };
  const commit = () => {
    if (!_colorControlStarts.has(input)) return;
    const start = _colorControlStarts.get(input);
    _colorControlStarts.delete(input);
    _recordAdvancedHistoryAfter(start);
  };
...
(function () {
  ...
  function hitWheel(e) {
    ...
    const x = e.clientX - rect.left - WHEEL_R,
      y = e.clientY - rect.top - WHEEL_R;
    const dist = Math.sqrt(x * x + y * y),
      ri = WHEEL_R - WHEEL_THICK - 2;
    ...
    if (cpMode !== "v")
      cpHue = ((Math.atan2(y, x) * 180) / Math.PI + 360) % 360;
  ...
  function hitInner(e) {
    ...
    if (cpShape === "square") {
      if (cpMode === "v") {
        cpVal = x / cpSqCanvas.width;
        cpSat = 0;
      } else {
        cpSat = x / cpSqCanvas.width;
        cpVal = 1 - y / cpSqCanvas.height;
      }
    } else {
      // Triangle mode: derive sat/val using the same vertex layout as drawInner
      const innerR = WHEEL_R - WHEEL_THICK - 6;
      const ocx = cpSqCanvas.width / 2,
        ocy = cpSqCanvas.height / 2;
      const hueRad = (cpHue * Math.PI) / 180;
      const vH = [
        ocx + innerR * Math.cos(hueRad),
        ocy + innerR * Math.sin(hueRad),
      ];
      const vW = [
        ocx + innerR * Math.cos(hueRad + (2 * Math.PI) / 3),
        ocy + innerR * Math.sin(hueRad + (2 * Math.PI) / 3),
      ];
      const vB = [
        ocx + innerR * Math.cos(hueRad + (4 * Math.PI) / 3),
        ocy + innerR * Math.sin(hueRad + (4 * Math.PI) / 3),
      ];
      // Barycentric coords of click point relative to the three triangle verts
      const denom =
        (vW[1] - vB[1]) * (vH[0] - vB[0]) + (vB[0] - vW[0]) * (vH[1] - vB[1]);
      if (Math.abs(denom) < 0.001) {
        setFromHsv(cpHue, cpSat, cpVal);
        return;
      }
      const lH =
        ((vW[1] - vB[1]) * (x - vB[0]) + (vB[0] - vW[0]) * (y - vB[1])) / denom;
      const lW =
        ((vB[1] - vH[1]) * (x - vB[0]) + (vH[0] - vB[0]) * (y - vB[1])) / denom;
      const lB = 1 - lH - lW;
      // Clamp to inside triangle
      const sum = Math.max(lH, 0) + Math.max(lW, 0) + Math.max(lB, 0) || 1;
      const bH = Math.max(lH, 0) / sum,
        bW = Math.max(lW, 0) / sum;
      if (cpMode === "v") {
        cpVal = clamp(bW + bH, 0, 1);
        cpSat = 0;
      } else {
  ...
  function _showPickCursor(e) {
    _cpPickEl.style.left = e.clientX + "px";
    _cpPickEl.style.top = e.clientY + "px";
    _cpPickEl.style.display = "block";
  }
  function _hidePickCursor() {
    _cpPickEl.style.display = "none";
  }
  ...
  function onWheelMove(e) {
    if (wheelDown) hitWheel(e);
    else if (sqDown) hitInner(e);
    _showPickCursor(e);
  }
  function onWheelUp(e) {
    ...
    if (
      shouldCommit &&
      window.PaletteDocker &&
      typeof window.PaletteDocker.flushAdvancedStyleColorChange === "function"
    )
      window.PaletteDocker.flushAdvancedStyleColorChange();
    if (shouldCommit) _recordAdvancedHistoryAfter(historyStart);
    historyStart = null;
  ...
  cpWheelCanvas.addEventListener("pointerdown", (e) => {
    ...
    if (hitWheel(e)) {
      ...
      if (
        e.clientX >= sqRect.left &&
        e.clientX <= sqRect.right &&
        e.clientY >= sqRect.top &&
        e.clientY <= sqRect.bottom
      ) {
        sqDown = true;
        historyStart = start;
        hitInner(e);
  ...
  cpSqCanvas.addEventListener("pointerdown", (e) => {
    e.preventDefault();
    historyStart = _advancedHistoryRgba();
    sqDown = true;
    hitInner(e);
    _showPickCursor(e);
...
// ── Shape toggle ──
cpShapeSquare.onclick = () => {
  cpShape = "square";
  cpShapeSquare.classList.add("active");
  cpShapeTriangle.classList.remove("active");
  drawPicker();
};
cpShapeTriangle.onclick = () => {
  cpShape = "triangle";
  cpShapeTriangle.classList.add("active");
  cpShapeSquare.classList.remove("active");
  drawPicker();
};
...
function setSlidersVisible(v) {
  ...
  requestAnimationFrame(() => {
    if (window._cpUpdateWheel) _cpUpdateWheel();
  });
}
[
  { btn: cpModeRGB, mode: "rgb" },
  { btn: cpModeHSV, mode: "hsv" },
  { btn: cpModeHSL, mode: "hsl" },
  { btn: cpModeV, mode: "v" },
].forEach(({ btn, mode }) => {
  btn.onclick = () => {
...
cpSwFg.addEventListener("click", () => {
  colorTarget = "fg";
  syncColorPanelInputs();
});
cpSwBg.addEventListener("click", () => {
  colorTarget = "bg";
  syncColorPanelInputs();
});
cpSwTransparent.addEventListener("click", () => {
  setTool("eraser", "Eraser");
  syncColorPanelSwatches();
});
...
(function () {
  ...
  if (typeof FloatPanels !== "undefined" && FloatPanels.setFloatSize) {
    FloatPanels.setFloatSize(
      "color",
      parseFloat(panel.style.width) || INIT_W,
      parseFloat(panel.style.height) || INIT_H,
    );
  }

  function isPanelVisible() {
    return (
      !panel.classList.contains("fp-hidden") && panel.offsetParent !== null
    );
  }
  ...
  function ensureHeightFits() {
    ...
    if (neededH > panel.offsetHeight) {
      ...
      if (typeof FloatPanels !== "undefined" && FloatPanels.setFloatSize)
        FloatPanels.setFloatSize("color", null, h);
    }
  }
  ...
  function applyPanelSize(w, h) {
    ...
    panel.style.height = h + "px";
    if (typeof FloatPanels !== "undefined" && FloatPanels.setFloatSize)
      FloatPanels.setFloatSize("color", w, h);
    updateWheel();
  }
  ...
  EDGES.forEach((side) => {
    ...
    el.addEventListener("pointerdown", (e) => {
      ...
      dockedDrag = !!(
        typeof FloatPanels !== "undefined" &&
        FloatPanels.isDocked &&
        FloatPanels.isDocked("color")
      );
    ...
    el.addEventListener("pointermove", (e) => {
      ...
      if (side.includes("w")) {
        nw = Math.max(MIN_W, Math.min(MAX_W, sw - dx));
        nl = sl + (sw - nw);
      }
      if (side.includes("s")) nh = Math.max(MIN_H, Math.min(MAX_H, sh + dy));
      if (side.includes("n")) {
        nh = Math.max(MIN_H, Math.min(MAX_H, sh - dy));
        nt = st + (sh - nh);
      }
    ...
    function endResize() {
      if (!active) return;
      active = false;
      if (typeof FloatPanels !== "undefined" && FloatPanels.saveLayout)
        FloatPanels.saveLayout();
    }
