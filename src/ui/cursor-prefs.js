...
try {
  cursorStyle = localStorage.getItem("animator_cursor_style") || "crosshair";
} catch (e) {}
...
const _POINT_CURSOR_CSS =
  "url(\"data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='7' height='7'><circle cx='3.5' cy='3.5' r='1.25' fill='black'/></svg>\") 3 3, crosshair";
const _CROSS_CURSOR_CSS =
  "url(\"data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='9' height='9'><path d='M4.5 1.5v6M1.5 4.5h6' fill='none' stroke='black' stroke-width='1'/></svg>\") 4 4, crosshair";
const _EYEDROPPER_CURSOR_CSS =
...
function _baseCursorCSS() {
  if (
    ...
    return "default";
  if (typeof tool !== "undefined" && tool === "eyedropper")
    return _EYEDROPPER_CURSOR_CSS;
  const paintTool =
    typeof tool !== "undefined" && (tool === "brush" || tool === "eraser");
  if (
    paintTool &&
    (cursorStyle === "crosshair" ||
      cursorStyle === "point" ||
      cursorStyle === "brush" ||
      cursorStyle === "brush-shape")
  )
    return "none";
  if (cursorStyle === "point") return _POINT_CURSOR_CSS;
  if (cursorStyle === "brush" || cursorStyle === "brush-shape")
...
function _setCursorStyle(v) {
  cursorStyle = v;
  try {
    localStorage.setItem("animator_cursor_style", v);
  } catch (e) {}
...
function _refreshActiveCursor() {
  ...
  if (window.BrushCursorNativeFlashNoteWrite)
    window.BrushCursorNativeFlashNoteWrite(
      ...
      "cursor-prefs:_refreshActiveCursor",
    );
}
