(function(){
  ...
  function renderCameraOptions(body){
    ...
    const positionLinkRow=document.createElement('label');positionLinkRow.className='magic-wand-checkbox-row camera-position-link-row';const positionLink=document.createElement('input');positionLink.type='checkbox';positionLink.className='ts-check';positionLink.checked=camera.track.positionLinked!==false;positionLink.onchange=()=>{CameraSystem.setPositionLinked(positionLink.checked);if(typeof renderTimeline==='function')renderTimeline();};const positionLinkText=document.createElement('span');positionLinkText.textContent='Link Position X / Y';positionLinkRow.append(positionLink,positionLinkText);transform.appendChild(positionLinkRow);
