(function(){
  ...
  function renderCameraOptions(body){
    ...
    const numeric=(parent,label,read,patch,options={})=>{
      ...
      const commit=()=>{if(committed)return;committed=true;const raw=input.value.trim(),value=Number(raw);if(!raw||!Number.isFinite(value)){sync(CameraSystem.value);return;}CameraSystem.update(patch(internal(value),CameraSystem.value),true);if(options.history!==false)CameraSystem.commit(before||CameraSystem.snapshot());sync(CameraSystem.value);};input.addEventListener('blur',commit,{signal});input.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();commit();input.blur();}else if(event.key==='Escape'){event.preventDefault();if(before)CameraSystem.restore(before);committed=true;input.blur();}},{signal});row.append(text,input);if(options.resetValue!=null){const reset=document.createElement('button');reset.type='button';reset.className='camera-property-reset-btn';reset.textContent='↺';reset.title='Reset '+label;reset.setAttribute('aria-label',reset.title);reset.onpointerdown=event=>event.preventDefault();reset.onclick=event=>{event.preventDefault();const snapshot=CameraSystem.snapshot();CameraSystem.update(patch(options.resetValue,CameraSystem.value),true);CameraSystem.commit(snapshot);sync(CameraSystem.value);};row.appendChild(reset);}parent.appendChild(row);const control={input,sync};controls.push(control);return control;
    };
    const transform=section(basic,'Transform');
    const positionGroup=document.createElement('div');positionGroup.className='camera-position-group';transform.appendChild(positionGroup);
    numeric(positionGroup,'Position X',value=>value.positionX,value=>({positionX:value}),{resetValue:0});
    numeric(positionGroup,'Position Y',value=>value.positionY,value=>({positionY:value}),{resetValue:0});
    const positionBracket=document.createElement('span');positionBracket.className='camera-position-bracket';positionBracket.setAttribute('aria-hidden','true');const positionLink=document.createElement('button');positionLink.type='button';positionLink.className='camera-position-link-btn';const syncPositionLink=value=>{const linked=value.track.positionLinked!==false;positionLink.classList.toggle('active',linked);positionGroup.classList.toggle('is-linked',linked);positionLink.setAttribute('aria-pressed',String(linked));positionLink.title=linked?'Unlock Position X and Y':'Link Position X and Y';positionLink.setAttribute('aria-label',positionLink.title);positionLink.innerHTML=linked?'<svg viewBox="0 0 16 16" aria-hidden="true"><rect x="3.5" y="7" width="9" height="6.5" rx="1.5"></rect><path d="M5.5 7V5a2.5 2.5 0 0 1 5 0v2"></path></svg>':'<svg viewBox="0 0 16 16" aria-hidden="true"><rect x="3.5" y="7" width="9" height="6.5" rx="1.5"></rect><path d="M10.5 7V5a2.5 2.5 0 0 0-4.6-1.4"></path></svg>';};syncPositionLink(camera);positionLink.onclick=()=>{CameraSystem.setPositionLinked(CameraSystem.value.track.positionLinked===false);if(typeof renderTimeline==='function')renderTimeline();};positionGroup.append(positionBracket,positionLink);
    numeric(transform,'Zoom',value=>value.zoom,value=>({zoom:value}),{min:10,max:1600,toDisplay:value=>value*100,toInternal:value=>value/100,resetValue:1});numeric(transform,'Rotation',value=>value.rotation,value=>({rotation:value}),{resetValue:0});    const guideVisibility=section(basic,'Guides'),guideInputs={};
    ...
    const actionRow=document.createElement('div');actionRow.className='tool-setting-action-row camera-view-row';const cameraView=document.createElement('button');cameraView.id='btn-camera-view';cameraView.type='button';cameraView.className='camera-view-settings-btn';cameraView.textContent='Camera View';cameraView.title='Toggle Camera View';cameraView.setAttribute('role','switch');const cameraViewEnabled=!!(window.CameraView&&CameraView.enabled);cameraView.setAttribute('aria-checked',String(cameraViewEnabled));cameraView.setAttribute('aria-pressed',String(cameraViewEnabled));cameraView.classList.toggle('active',cameraViewEnabled);actionRow.appendChild(cameraView);basic.appendChild(actionRow);
