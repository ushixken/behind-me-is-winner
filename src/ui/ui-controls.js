...
(function initFormControlAccessibility() {
  ...
  const contextName = (control) => {
    const mirror = control.closest("[data-mirror-target]")?.dataset
      .mirrorTarget;
    if (mirror)
      return (
        mirror + "-mirror-" + (control.type || control.tagName.toLowerCase())
      );
    if (control.name)
      return control.name + (control.value ? "-" + control.value : "");
    const owner = control.closest("[id],[data-panel],[data-body]");
    const ownerName =
      owner && (owner.id || owner.dataset.panel || owner.dataset.body);
    return (
      (ownerName || "app") +
      "-" +
  ...
  const readableName = (control) => {
    ...
    const row = control.closest(
      ".ts-row,.modal-row,.selection-option-field,.tool-group-option-row,.cp-slider-row,.palette-size-control",
    );
    const text = row
      ?.querySelector(
        ".ts-label,.ts-label-sm,.selection-option-label,label,span",
      )
      ?.textContent?.trim();
    if (text) return text;
  ...
  function normalizeControl(control) {
    ...
    let explicit = document.querySelector(
  ...
  new MutationObserver((records) =>
    records.forEach((record) => record.addedNodes.forEach(normalize)),
...
(function () {
  ...
  rhBottom.addEventListener("pointerdown", (e) => {
    ...
    dragging = true;
    startY = e.clientY;
    startH = timelineArea.offsetHeight;
  ...
  rhBottom.addEventListener("pointerup", () => {
    if (dragging) {
      dragging = false;
      rhBottom.classList.remove("dragging");
    }
  });
  rhBottom.addEventListener("pointercancel", () => {
    if (dragging) {
      dragging = false;
      rhBottom.classList.remove("dragging");
    }
...
function totalToSecFr(total, fps) {
  return { sec: Math.floor(total / fps), fr: total % fps };
}
function secFrToTotal(sec, fr, fps) {
  return sec * fps + fr;
}
...
function openCanvasModal() {
  ...
  document.getElementById("canvas-bg-color-input").value =
    bgColor === "transparent" ? "#ffffff" : bgColor;
  document
    .querySelectorAll(".bg-swatch")
    .forEach((s) => s.classList.toggle("active", s.dataset.color === bgColor));
  document.getElementById("modal-canvas").classList.add("visible");
}
function updateCanvasFramesModalTotal() {
  const fps = Math.max(
    1,
    parseInt(document.getElementById("canvas-maxfps-input").value) || MAX_FPS,
  );
  const sec = Math.max(
    0,
    parseInt(document.getElementById("canvas-frames-sec-input").value) || 0,
  );
  const fr = Math.max(
    0,
    parseInt(document.getElementById("canvas-frames-fr-input").value) || 0,
  );
...
document
  .getElementById("canvas-frames-sec-input")
  .addEventListener("change", updateCanvasFramesModalTotal);
document
  .getElementById("canvas-frames-fr-input")
  .addEventListener("change", updateCanvasFramesModalTotal);
document.getElementById("canvas-maxfps-input").addEventListener("input", () => {
  const fps = Math.max(
    ...
    Math.min(
      120,
      parseInt(document.getElementById("canvas-maxfps-input").value) ||
        PROJECT_DEFAULTS.fps,
    ),
  );
  document.getElementById("canvas-frames-fps-label").textContent = fps;
...
function closeCanvasModal() {
  document.getElementById("modal-canvas").classList.remove("visible");
}
...
document.getElementById("modal-canvas").addEventListener("click", (e) => {
  if (e.target === document.getElementById("modal-canvas")) closeCanvasModal();
});
document.querySelectorAll(".preset-btn").forEach((btn) => {
  btn.onclick = () => {
    document.getElementById("canvas-w-input").value = btn.dataset.w;
    document.getElementById("canvas-h-input").value = btn.dataset.h;
  };
});
document.querySelectorAll(".bg-swatch").forEach((sw) => {
  sw.onclick = () => {
    ...
    if (sw.dataset.color !== "transparent")
      document.getElementById("canvas-bg-color-input").value = sw.dataset.color;
  };
});
document.getElementById("canvas-bg-color-input").oninput = () =>
  document
    .querySelectorAll(".bg-swatch")
    .forEach((s) => s.classList.remove("active"));
document.getElementById("modal-canvas-ok").onclick = () => {
  ...
  if (nw < 8 || nh < 8 || nw > 4096 || nh > 4096) {
    showInfo("Dimensions must be 8–4096.", "Invalid Size");
    return;
  }
  MAX_FPS = Math.max(
    1,
  ...
  const nf = norm.total;
  if (nf < 2 || nf > 9999) {
    showInfo("Frame count must be 2–9999.", "Invalid Frame Count");
    return;
  }
  fpsTl.max = MAX_FPS;
  const cf = Math.min(getFPS(), MAX_FPS);
  fpsTl.value = cf;
  fpsVal.textContent = cf;
  ...
  if (typeof window.markProjectDirty === "function")
    window.markProjectDirty("canvas-settings");
  applyTotalFrames(nf);
  applyCanvasResize(nw, nh);
  closeCanvasModal();
};
function applyCanvasResize(nw, nh) {
  layers.forEach((l) => {
    const nf = {};
    Object.keys(l.frames).forEach((fi) => {
...
function applyTotalFrames(n) {
  TOTAL = n;
  rangeStart = Math.min(rangeStart, TOTAL - 1);
  rangeEnd = Math.min(rangeEnd, TOTAL - 1);
  if (rangeEnd < rangeStart) rangeEnd = rangeStart;
  curFrame = Math.min(curFrame, TOTAL - 1);
  document.getElementById("frame-info").textContent =
    "Frame " + frameLabel(curFrame) + " / " + TOTAL;
  renderTimeline();
}
...
function updateFramesModalTotal() {
  const fps = Math.max(
    1,
    parseInt(document.getElementById("frames-maxfps-input").value) || MAX_FPS,
  );
  const sec = Math.max(
    0,
    parseInt(document.getElementById("frames-sec-input").value) || 0,
  );
  const fr = Math.max(
    0,
    parseInt(document.getElementById("frames-fr-input").value) || 0,
  );
  const norm = normalizeSecFr(sec, fr, fps);
...
document
  .getElementById("frames-sec-input")
  .addEventListener("change", updateFramesModalTotal);
document
  .getElementById("frames-fr-input")
  .addEventListener("change", updateFramesModalTotal);
document.getElementById("frames-maxfps-input").addEventListener("input", () => {
  document.getElementById("frames-fps-label").textContent = Math.max(
    1,
    parseInt(document.getElementById("frames-maxfps-input").value) || MAX_FPS,
  );
  updateFramesModalTotal();
});
document.getElementById("modal-frames-cancel").onclick = () =>
  document.getElementById("modal-frames").classList.remove("visible");
document.getElementById("modal-frames").addEventListener("click", (e) => {
  if (e.target === document.getElementById("modal-frames"))
    document.getElementById("modal-frames").classList.remove("visible");
});
document.getElementById("modal-frames-ok").onclick = () => {
  ...
  applyTotalFrames(n);
  if (typeof window.markProjectDirty === "function")
    window.markProjectDirty("frame-count");
  document.getElementById("modal-frames").classList.remove("visible");
};

// Zoom settings modal
document.getElementById("modal-zoom-cancel").onclick = () =>
  document.getElementById("modal-zoom").classList.remove("visible");
document.getElementById("modal-zoom").addEventListener("click", (e) => {
  if (e.target === document.getElementById("modal-zoom"))
    document.getElementById("modal-zoom").classList.remove("visible");
});
document.getElementById("modal-zoom-ok").onclick = () => {
  ...
  zoomMin = Math.max(
    ...
    Math.min(
      1,
      parseFloat(document.getElementById("zoom-min-input").value) || 0.1,
    ),
  );
  zoomMax = Math.max(
    1,
    Math.min(
      32,
      parseFloat(document.getElementById("zoom-max-input").value) || 16,
    ),
  );
  document.getElementById("modal-zoom").classList.remove("visible");
};
...
document
  .getElementById("draw-mode-exit")
  .addEventListener("click", () => setDrawMode(false));
...
let _temporaryEyedropperRestore = null,
  _temporaryEyedropperActivating = false;
...
function _restoreTemporaryEyedropper() {
  ...
  const previous = _temporaryEyedropperRestore;
  _temporaryEyedropperRestore = null;
  if (tool !== "eyedropper") return;
  if (
    previous.groupId &&
    previous.subToolId &&
    window.ToolGroups &&
    typeof ToolGroups.activateSubTool === "function" &&
    ToolGroups.activateSubTool(previous.groupId, previous.subToolId, {
      fromTemporaryTool: true,
    })
  )
    return;
...
document.addEventListener("keydown", (e) => {
  if (
    e.key !== "Alt" ||
    e.repeat ||
    _temporaryEyedropperRestore ||
    _shortcutTargetIsEditable(e.target) ||
    document.querySelector(".modal-overlay.visible")
  )
    return;
  if (!["brush", "eraser", "fill", "line", "curve"].includes(tool)) return;
  const group =
    window.ToolGroups && ToolGroups.getGroup(ToolGroups.activeGroupId);
  e.preventDefault();
  _temporaryEyedropperRestore = {
    toolId: tool,
    label: document.getElementById("stat-tool").textContent,
    groupId: (group && group.id) || null,
    subToolId: (group && group.activeSubToolId) || null,
  };
  _temporaryEyedropperActivating = true;
  try {
...
window.addEventListener("tool-changed", () => {
  if (_temporaryEyedropperRestore && !_temporaryEyedropperActivating)
    _temporaryEyedropperRestore = null;
});
document.addEventListener("keyup", (e) => {
  if (e.key === "Alt") _restoreTemporaryEyedropper();
});
window.addEventListener("blur", _restoreTemporaryEyedropper);
document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "hidden") _restoreTemporaryEyedropper();
});
...
document.addEventListener("keydown", (e) => {
  ...
  const clipboardShortcutBlocked = !!(
    (shortcutTarget &&
      (shortcutTarget.isContentEditable ||
        shortcutTarget.closest('input,textarea,[contenteditable="true"]'))) ||
    document.querySelector(".modal-overlay.visible")
  );
  if (!clipboardShortcutBlocked) {
    if (matchBind(e, "copyFrame")) {
      e.preventDefault();
      copyFrame();
      return;
    }
    if (matchBind(e, "cutFrame")) {
      e.preventDefault();
      cutFrame();
      return;
    }
    if (matchBind(e, "pasteImage")) {
      e.preventDefault();
      if (typeof pasteImageFromClipboard === "function")
        pasteImageFromClipboard();
      return;
    }
    if (matchBind(e, "pasteFrame")) {
      e.preventDefault();
      pasteFrame();
      return;
    }
    if (matchBind(e, "duplicateFrame")) {
      e.preventDefault();
      duplicateFrame();
      return;
    }
    if (matchBind(e, "clearFrame")) {
      e.preventDefault();
      clearCurrentFrame();
      return;
    }
    if (matchBind(e, "copyLayer")) {
      e.preventDefault();
      copyLayer(curLayer);
      return;
    }
    if (matchBind(e, "cutLayer")) {
      e.preventDefault();
      cutLayer(curLayer);
      return;
    }
    if (matchBind(e, "pasteLayer")) {
      e.preventDefault();
      pasteLayer(curLayer);
      return;
    }
    if (matchBind(e, "duplicateLayer")) {
      e.preventDefault();
      duplicateLayer(curLayer);
      return;
    }
    if (matchBind(e, "deleteLayer")) {
      e.preventDefault();
      deleteLayer(curLayer);
      return;
    }
  }
  const toolShortcutBlocked =
    !!(
      shortcutTarget &&
      (shortcutTarget.isContentEditable ||
  ...
  if (!toolShortcutBlocked && !e.repeat && matchBind(e, "toggleDrawMode")) {
    e.preventDefault();
    toggleDrawMode();
    return;
  }
  if (!toolShortcutBlocked && !e.repeat && matchBind(e, "toggleOnionSkin")) {
    e.preventDefault();
    toggleOnionSkin();
    return;
  }
  if (
    !toolShortcutBlocked &&
    typeof handleToolGroupKeybind === "function" &&
    handleToolGroupKeybind(e)
  ) {
    e.preventDefault();
    return;
  }
  if (
    !toolShortcutBlocked &&
    (!window.ToolGroups || typeof ToolGroups.getGroups !== "function")
  ) {
    const toolMap = {
      toolBrush: ["brush", "Brush"],
      toolEraser: ["eraser", "Eraser"],
      toolFill: ["fill", "Fill"],
      toolLine: ["line", "Line"],
      toolCurve: ["curve", "Curve"],
      toolEyedropper: ["eyedropper", "Eyedropper"],
      toolTransform: ["transform", "Transform"],
    };
    for (const action in toolMap) {
      if (matchBind(e, action)) {
        e.preventDefault();
        setTool(...toolMap[action]);
        return;
      }
    }
  }
  if (matchBind(e, "newFrame")) {
    if (window.CameraTimeline && CameraTimeline.selected)
      CameraTimeline.addOrUpdateKey();
    else {
      createBlankKey();
      loadFrame(curLayer, curFrame);
    }
  }
  if (matchBind(e, "delKeyframe")) {
    e.preventDefault();
    deleteKeyframe();
  }
  if (matchBind(e, "nextFrame")) {
    e.preventDefault();
    kfswNavigate(+1);
  }
  if (matchBind(e, "prevFrame")) {
    e.preventDefault();
    kfswNavigate(-1);
  }
  // Space is reserved for canvas pan — Tab toggles play (handled in timeline.js)
  // Zoom keyboard shortcuts — center of canvas-area
  if (matchBind(e, "zoomIn")) {
    e.preventDefault();
    ...
    doZoom(1, r.width / 2, r.height / 2);
  }
  if (matchBind(e, "zoomOut")) {
    e.preventDefault();
    if (window.CameraView && CameraView.active) {
      CameraView.zoomBy(-1);
      return;
  ...
  if (matchBind(e, "rotateReset")) {
    e.preventDefault();
    resetRotation();
  }
  if (matchBind(e, "flipHorizontal")) {
    e.preventDefault();
    toggleFlipH();
  }
  if (matchBind(e, "flipVertical")) {
    e.preventDefault();
    toggleFlipV();
  }
});
