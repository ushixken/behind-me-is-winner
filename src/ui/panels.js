...
function floodFill(x, y, fc) {
  ...
  const fillPixelAllowed = window.SelectionScope
    ? (pixel) => SelectionScope.allowsPixel(pixel)
    : () => true;
  ...
  while (head < tail) {
    ...
    region[pixel] = 1;
    if (cx < minFillX) minFillX = cx;
    if (cx > maxFillX) maxFillX = cx;
    if (cy < minFillY) minFillY = cy;
    if (cy > maxFillY) maxFillY = cy;
  ...
  const dirtyBounds = {
    x: minFillX,
    y: minFillY,
    w: maxFillX - minFillX + 1,
    h: maxFillY - minFillY + 1,
  };
  const fillSettings = window.FillMaskEngine
    ? FillMaskEngine.getSettings()
    : null;
  if (fillSettings && fillSettings.antialiasing && window.BucketFillAA) {
    BucketFillAA.apply(region, dirtyBounds, fc);
    return;
  }
  for (let row = minFillY; row <= maxFillY; row++)
    for (let col = minFillX; col <= maxFillX; col++)
      if (region[row * CW + col]) {
        const offset = (row * CW + col) * 4;
        d[offset] = fr;
        d[offset + 1] = fg;
        d[offset + 2] = fb;
        d[offset + 3] = 255;
...
function cloneExtendedFrameRecord(record) {
  if (!record || !record.canvas) return null;
  const canvas = document.createElement("canvas");
  canvas.width = record.canvas.width;
  canvas.height = record.canvas.height;
  canvas.getContext("2d").drawImage(record.canvas, 0, 0);
  return { canvas, x: Number(record.x) || 0, y: Number(record.y) || 0 };
}
function getExtendedLayerFrame(li, fi) {
  const layer = layers[li];
  return (layer && layer.extendedFrames && layer.extendedFrames[fi]) || null;
}
function setExtendedLayerFrame(li, fi, canvas, x, y) {
  const layer = layers[li];
  if (!layer || !canvas) return null;
  if (!layer.extendedFrames) layer.extendedFrames = {};
  const record = {
    canvas,
    x: Math.floor(Number(x) || 0),
    y: Math.floor(Number(y) || 0),
  };
  layer.extendedFrames[fi] = record;
  let key = layer.frames[fi];
  if (!key || key.width !== CW || key.height !== CH)
    key = layer.frames[fi] = mkLayerCanvas();
  const keyContext = key.getContext("2d");
  keyContext.clearRect(0, 0, CW, CH);
  keyContext.drawImage(canvas, record.x, record.y);
  return record;
}
function clearExtendedLayerFrame(li, fi) {
  const layer = layers[li];
  if (layer && layer.extendedFrames) delete layer.extendedFrames[fi];
}
window.cloneExtendedFrameRecord = cloneExtendedFrameRecord;
window.getExtendedLayerFrame = getExtendedLayerFrame;
window.setExtendedLayerFrame = setExtendedLayerFrame;
window.clearExtendedLayerFrame = clearExtendedLayerFrame;
...
function mkStyleIndexCanvas() {
  return window.SmartRasterLayer._mkIndexCanvas();
}
function cloneStyleCanvas(src) {
  return window.SmartRasterLayer.cloneIndexCanvas(src);
}
function cloneStyleMeta(meta) {
  return window.SmartRasterLayer.cloneMeta(meta);
}
function makeEmptyStyleMeta() {
  return window.SmartRasterLayer._makeEmptyMeta();
}
...
function resetStyleFrame(li, fi) {
  window.SmartRasterLayer.resetFrame(li, fi);
}
function ensureStyleFrame(li, fi) {
  return window.SmartRasterLayer.ensureFrame(
    li == null ? curLayer : li,
    fi == null ? curFrame : fi,
  );
}
function ensureStyleIndexForFrame(li, fi, styleId) {
  return window.SmartRasterLayer.ensureStyleIndex(li, fi, styleId);
}
function encodeStyleIndexToPixel(data, offset, index) {
  window.SmartRasterLayer._encodePixel(data, offset, index);
}

function activeAdvancedStyleIdForPainting() {
  if (typeof window === "undefined") return null;
  if (typeof window.getActiveAdvancedPaletteStyleId === "function")
    return window.getActiveAdvancedPaletteStyleId();
  if (
    window.PaletteDocker &&
    typeof window.PaletteDocker.getActiveAdvancedPaletteStyleId === "function"
  )
    return window.PaletteDocker.getActiveAdvancedPaletteStyleId();
  return null;
}

function advancedPalettePaintingEnabled() {
  if (typeof window === "undefined") return false;
  if (typeof window.isAdvancedPalettePaintingEnabled === "function")
    return !!window.isAdvancedPalettePaintingEnabled();
  return !!(
    window.PaletteDocker &&
    typeof window.PaletteDocker.isAdvancedPalettePaintingEnabled ===
      "function" &&
    window.PaletteDocker.isAdvancedPalettePaintingEnabled()
  );
}
...
function commitSmartRasterBrush(
  maskCanvas,
  styleId,
  strokeOpacity,
  dirtyRect,
  ...
  if (!advancedPalettePaintingEnabled() || !styleId || !maskCanvas)
    return false;
  return window.SmartRasterLayer.commitBrushMask(
    curLayer,
    curFrame,
    maskCanvas,
    styleId,
...
function commitSmartRasterBrushAt(
  layerIndex,
  frameIndex,
  maskCanvas,
  styleId,
  ...
  if (
    ...
    typeof window.SmartRasterLayer.commitBrushMask !== "function"
  )
    return false;
  return window.SmartRasterLayer.commitBrushMask(
    layerIndex,
...
function deleteStyleFrame(li, fi) {
  window.SmartRasterLayer.resetFrame(li, fi);
}
...
window.getStyleFrameBundle = function (li, fi) {
  return window.SmartRasterLayer.getFrameBundle(li, fi);
};
window.restoreStyleFrameBundle = function (li, fi, bundle) {
  window.SmartRasterLayer.restoreFrameBundle(li, fi, bundle);
};
...
function _ensureScratchCanvases() {
  if (!_scratchMask || _scratchMask.width !== CW || _scratchMask.height !== CH)
    _scratchMask = mkLayerCanvas();
  if (!_scratchTmp || _scratchTmp.width !== CW || _scratchTmp.height !== CH)
    _scratchTmp = mkLayerCanvas();
}
function getExactKey(li, fi) {
  return layers[li].frames[fi] || null;
}
  ...
  for (let f = fi; f >= 0; f--) {
    if (
      ...
      !(
        ...
        isDrawingFrameHidden(li, f)
      )
    )
      return layers[li].frames[f];
  }
  return null;
}
function getPreviousVisibleDrawingKey(li, fi) {
  ...
  for (let f = Math.min(fi, TOTAL - 1); f >= 0; f--) {
    if (
      ...
      return { frameIndex: f, canvas: layer.frames[f] };
  }
  return null;
}
...
function _effectiveLayerClip(l) {
  if (l.stencil !== "none")
    return { stencil: l.stencil, clipTo: l.clipTo, clipToGroup: l.clipToGroup };
  if (l.groupId) {
    ...
    for (const gid of [..._groupChain(l.groupId)].reverse()) {
      ...
      if (g && g.stencil && g.stencil !== "none") {
        if (g.clipToGroup) {
          return {
            stencil: g.stencil === "outside" ? "group-outside" : "group-inside",
            clipTo: null,
            clipToGroup: g.clipToGroup,
          };
        }
        if (g.clipTo != null && layers[g.clipTo]) {
          return {
            stencil: g.stencil === "outside" ? "outside" : "inside",
            clipTo: g.clipTo,
            clipToGroup: null,
          };
        }
...
function _queueDeferredKeyVisualRefresh() {
  if (_deferredKeyVisualRefreshRaf) {
    cancelAnimationFrame(_deferredKeyVisualRefreshRaf);
...
window._scheduleDeferredKeyVisualRefreshAfterPresentation =
  _scheduleDeferredKeyVisualRefreshAfterPresentation;
function ensureKey(options) {
  ...
  const profiler =
      window.BrushLatencyProfiler && window.BrushLatencyProfiler.enabled
        ...
        : null,
    totalStart = profiler ? performance.now() : 0;
  if (!layers[curLayer].frames[curFrame]) {
    let diagnosticStarted =
      window.FirstDabLatencyProbe && window.FirstDabLatencyProbe.enabled
        ? performance.now()
        : 0;
    let started = profiler ? performance.now() : 0;
    const key = mkLayerCanvas();
    if (window.FirstDabLatencyProbe)
      window.FirstDabLatencyProbe.ensureKeyStage(
        "ensureKeyCanvasAllocation",
        diagnosticStarted,
      );
    if (profiler)
      profiler.measure("key-canvas-allocation", started, {
        width: CW,
        height: CH,
      });
    diagnosticStarted =
      window.FirstDabLatencyProbe && window.FirstDabLatencyProbe.enabled
        ? performance.now()
        : 0;
    started = profiler ? performance.now() : 0;
    layers[curLayer].frames[curFrame] = key;
    if (window.FirstDabLatencyProbe)
      window.FirstDabLatencyProbe.ensureKeyStage(
    ...
    if (profiler)
      profiler.measure("key-frame-map-insertion", started, {
        layerIndex: curLayer,
        frameIndex: curFrame,
      });
    diagnosticStarted =
    ...
    if (profiler)
      profiler.measure("key-initialization-clear", started, {
        width: CW,
        height: CH,
        copiedHeldExposure: false,
      });
    diagnosticStarted =
      window.FirstDabLatencyProbe && window.FirstDabLatencyProbe.enabled
        ? performance.now()
    ...
    if (profiler)
      profiler.measure("key-creation-recomposite", started, {
        fullCanvas: true,
      });
    if (deferVisualRefresh) {
      ...
      if (profiler) profiler.measure("key-creation-onion-update", started);
      diagnosticStarted =
        window.FirstDabLatencyProbe && window.FirstDabLatencyProbe.enabled
          ? performance.now()
          : 0;
      ...
      if (profiler) profiler.measure("key-creation-timeline-refresh", started);
      diagnosticStarted =
        window.FirstDabLatencyProbe && window.FirstDabLatencyProbe.enabled
          ? performance.now()
          : 0;
      ...
      if (profiler) profiler.measure("key-creation-status-update", started);
    }
    if (profiler) {
      const noThumbnailStart = performance.now();
      profiler.measure("key-thumbnail-work", noThumbnailStart, {
        performed: false,
        reason: "timeline has no keyframe thumbnail renderer",
      });
      profiler.measure("ensure-key-created-total", totalStart, {
        created: true,
      });
    }
    return true;
  ...
  if (profiler)
    profiler.measure("ensure-key-noop-total", totalStart, { created: false });
  return false;
}
function saveActiveToKey() {
  ...
  if (extended) {
    ...
    fullContext.drawImage(activeC, -minX, -minY);
    setExtendedLayerFrame(curLayer, curFrame, full, minX, minY);
    return;
  }
  const kctx = kf.getContext("2d");
...
function loadFrame(li, fi) {
  if (typeof window.prepareTransformForArtworkChange === "function")
    window.prepareTransformForArtworkChange(li, fi);
  if (typeof window.finishActiveDrawingBeforeArtworkChange === "function")
    window.finishActiveDrawingBeforeArtworkChange(li, fi);
  if (window.CameraSystem && typeof CameraSystem.evaluateAt === "function")
    CameraSystem.evaluateAt(fi);
  ctx.clearRect(0, 0, CW, CH);
  const k = getHeldKey(li, fi);
  if (k) {
    const keyFrame = Object.keys(layers[li].frames).find(
        (frame) => layers[li].frames[frame] === k,
      ),
      extended = keyFrame == null ? null : getExtendedLayerFrame(li, keyFrame);
    if (extended) ctx.drawImage(extended.canvas, extended.x, extended.y);
    else ctx.drawImage(k, 0, 0);
  }
  recomposite(li, fi);
  updateOnion();
  updatePlayhead();
  document.getElementById("frame-info").textContent =
    frameLabel(fi) + " / " + frameLabel(TOTAL - 1);
  updateStatus();
  window.dispatchEvent(
    new CustomEvent("active-artwork-changed", {
      detail: { layerIndex: li, frameIndex: fi },
    }),
  );
}
...
function recomposite(li, fi, dirtyRect) {
  const firstDabDiagnosticStart = window.FirstDabLatencyProbe
    ? window.FirstDabLatencyProbe.recomposeStart()
    : 0;
  const latencyProfiler =
    window.BrushLatencyProfiler && window.BrushLatencyProfiler.enabled
      ...
      : null;
  if (latencyProfiler)
    latencyProfiler.point("recomposite-begins", {
      dirtyRect: dirtyRect || null,
    });
  if (window.BrushRafExperiment)
    window.BrushRafExperiment.noteRecompositeBegin();
  const presentationStart = latencyProfiler
    ? latencyProfiler.presentationStart()
    : 0;
  const compositeStart = presentationStart ? performance.now() : 0;
  const preparationStart = presentationStart ? performance.now() : 0;
  const clip =
    dirtyRect && dirtyRect.w > 0 && dirtyRect.h > 0 ? dirtyRect : null;
  if (clip) {
    compCtx.save();
    compCtx.beginPath();
  ...
  if (presentationStart)
    latencyProfiler.measure("recomposite-preparation", preparationStart, {
      clip,
      layerCount: layers.length,
      canvas: { width: CW, height: CH },
      liveStroke: typeof _inStroke !== "undefined" && _inStroke,
    });
  const clearStart = presentationStart ? performance.now() : 0;
  artworkCompositeCtx.clearRect(
    clip ? clip.x : 0,
    clip ? clip.y : 0,
    clip ? clip.w : CW,
    clip ? clip.h : CH,
  ...
  if (presentationStart)
    latencyProfiler.measure(
      "composite-clear-and-background",
      clearStart,
      latencyProfiler.canvasDetail(compC, compCtx, { clip }),
    );
  const scratchStart = presentationStart ? performance.now() : 0;
  const scratchAllocated =
    !_scratchMask ||
    !_scratchTmp ||
    _scratchMask.width !== CW ||
    _scratchMask.height !== CH ||
    _scratchTmp.width !== CW ||
    _scratchTmp.height !== CH;
  _ensureScratchCanvases();
  if (presentationStart)
    latencyProfiler.measure("composite-scratch-preparation", scratchStart, {
      allocatedOrResized: scratchAllocated,
      width: CW,
      height: CH,
    });
  const curLForMask = layers[li];
  const curIsMaskForCheck =
    curLForMask &&
    (layers.some(
      (ol) => ol !== curLForMask && ol.stencil !== "none" && ol.clipTo === li,
    ) ||
      groups.some((g) => g.clipTo === li && g.stencil && g.stencil !== "none"));
  const layersStart = presentationStart ? performance.now() : 0;
  let persistentLayerDraws = 0,
    livePreviewDraws = 0,
    maskPasses = 0;
  ...
  for (let idx = 0; idx < layers.length; idx++) {
    const l = layers[idx];
    if (typeof _tfHiddenLayers !== "undefined" && _tfHiddenLayers.has(idx))
      continue;
    ...
    if (idx === li) {
      ...
      const isGrpStencil =
        (eff.stencil === "group-inside" || eff.stencil === "group-outside") &&
        eff.clipToGroup;
      const isCurGrpMask =
        l.groupId &&
        (layers.some(
          (ol) =>
            ol.clipToGroup === l.groupId &&
            (ol.stencil === "group-inside" || ol.stencil === "group-outside"),
        ) ||
          _groupUsedAsGroupMaskSource(l.groupId));
      activeC.style.opacity = "0";
      ...
      const hasPendingCpu =
        typeof _hardRoundPendingCpuFinishing === "function" &&
        _hardRoundPendingCpuFinishing();
      if (
        ((typeof _inStroke !== "undefined" && _inStroke) || hasPendingCpu) &&
        typeof _getLiveStrokePreview === "function"
      ) {
        const liveStart = presentationStart ? performance.now() : 0;
        srcCanvas = _getLiveStrokePreview();
        livePreviewDraws++;
        if (presentationStart)
          latencyProfiler.measure("live-preview-draw", liveStart, {
            layerIndex: idx,
    ...
    if (
      ...
      layers.forEach((ml, mi) => {
        ...
        if (ms) {
          mc.globalAlpha = ml.opacity ?? 1;
          mc.drawImage(ms, 0, 0);
          mc.globalAlpha = 1;
        }
      });
      const tmp = _scratchTmp;
      const tc = tmp.getContext("2d");
      tc.clearRect(0, 0, CW, CH);
      tc.drawImage(srcCanvas, 0, 0);
      tc.globalCompositeOperation =
        eff.stencil === "group-inside" ? "destination-in" : "destination-out";
    ...
    if (
      eff.stencil !== "none" &&
      eff.stencil !== "group-inside" &&
      eff.stencil !== "group-outside" &&
      eff.clipTo != null &&
      layers[eff.clipTo]
    ) {
      maskPasses++;
      const maskCanvas =
        eff.clipTo === li ? activeC : getHeldKey(eff.clipTo, fi);
      if (maskCanvas) {
        const tmp = _scratchTmp;
        const tc = tmp.getContext("2d");
        tc.clearRect(0, 0, CW, CH);
        ...
        tc.globalCompositeOperation =
          eff.stencil === "inside" ? "destination-in" : "destination-out";
        tc.drawImage(maskCanvas, 0, 0);
        tc.globalCompositeOperation = "source-over";
        artworkCompositeCtx.globalAlpha = layerAlpha;
        artworkCompositeCtx.drawImage(tmp, 0, 0);
  ...
  if (presentationStart)
    latencyProfiler.measure(
      "persistent-layers-masks-and-clipping",
      layersStart,
      {
        persistentLayerDraws,
        livePreviewDraws,
  ...
  if (typeof _hrStageDiagCaptureE1 === "function")
    _hrStageDiagCaptureE1(artworkCompositeC);
  drawBg();
  ...
  const firstDabDisplayBlitStart = firstDabDiagnosticStart
    ? performance.now()
    : 0;
  ...
  const firstDabDisplayBlitDuration = firstDabDisplayBlitStart
    ? performance.now() - firstDabDisplayBlitStart
    : 0;
  ...
  if (presentationStart)
    latencyProfiler.measure("composite-state-restore", stateRestoreStart, {
      restoredClip: !!clip,
    });
  ...
  if (presentationStart)
    latencyProfiler.measure("layer-composite", compositeStart, {
      dirty: !!clip,
      rect: clip,
    });
  ...
  if (presentationStart)
    latencyProfiler.point("final-composite-copy-ready", {
      canvas: { width: CW, height: CH },
    });
  ...
  if (typeof _hrStageDiagCaptureE3 === "function")
    _hrStageDiagCaptureE3(displayC);
  ...
  if (presentationStart) {
    latencyProfiler.measure("canvas-display-upload", displayUploadStart, {
      width: CW,
      height: CH,
      blur: typeof _displayBlurPx === "number" ? _displayBlurPx : null,
    });
    latencyProfiler.presentationEnd(presentationStart, { dirty: !!clip });
  }
  if (firstDabDiagnosticStart && window.FirstDabLatencyProbe)
    window.FirstDabLatencyProbe.displayComplete(
      firstDabDiagnosticStart,
      firstDabDisplayBlitDuration,
    );
...
const CompositionPrewarm = (function () {
  ...
  let runCount = 0,
    ...
    lastError = "",
    lastPath = "";
  let experimentPending = null,
    experimentSerial = 0;
  const experimentRecords = [];
  function clone(value) {
    return JSON.parse(JSON.stringify(value));
  }
  function stageMap(record) {
    const map = {};
    for (const stage of (record && record.stages) || []) {
      const values = map[stage.name] || (map[stage.name] = []);
      values.push(stage.duration);
    }
  ...
  function markCold(reason) {
    compositionWarm = false;
    lastReason = reason || "unknown";
  }
  function refreshIdleState() {
    if (performance.now() - lastCompositeAt >= idleThresholdMs)
      markCold("idle");
  }
  function noteComposite() {
    lastCompositeAt = performance.now();
    compositionWarm = true;
  }
  function run(reason) {
    warmupPending = false;
    warmupRaf = 0;
    ...
    if (
      ...
      warming ||
      experimentPending ||
      window.__suspendAutomaticCompositionPrewarm ||
      (window.BrushRafExperiment && window.BrushRafExperiment.pending) ||
    ...
    try {
      const livePath =
        typeof window._prewarmLiveStrokeComposition === "function" &&
        window._prewarmLiveStrokeComposition();
      if (!livePath) recomposite(curLayer, curFrame);
      lastPath = livePath ? "live-stroke-clipped" : "settled-frame";
      runCount++;
  ...
  function schedule(reason) {
    ...
    warmupPending = true;
    lastReason = reason || lastReason;
    warmupRaf = requestAnimationFrame(() => run(lastReason));
    return true;
  }
  function beforeStroke() {
    ...
    if (experimentPending && window.BrushLatencyProfiler)
      window.BrushLatencyProfiler.labelNextStroke(
        "experiment-" + experimentPending.mode,
      );
    return compositionWarm;
  }
  function noteStrokeComplete() {
    if (!experimentPending) return;
    const profiler = window.BrushLatencyProfiler,
      stroke = profiler && profiler.latest ? profiler.latest() : null;
    const visible =
      stroke &&
      stroke.points &&
      stroke.points.find((point) => point.name === "first-display-presented");
    experimentPending.stroke = stroke;
    experimentPending.pointerDownToFirstVisibleDab = visible
      ? visible.fromPointerDown
      : null;
    experimentPending.totalFirstStrokeLatency = stroke
      ? stroke.totalToPointerUp
      : null;
    experimentPending.strokeStages = stageMap(stroke);
    experimentPending.completedAt = performance.now();
    experimentRecords.push(experimentPending);
  ...
  function armExperiment(mode) {
    ...
    if (experimentPending)
      throw new Error(
        "Finish the currently armed experiment stroke before starting another mode.",
      );
    const profiler = window.BrushLatencyProfiler;
    if (profiler) profiler.enable(true);
    markCold("experiment-" + mode);
    const record = {
      id: ++experimentSerial,
      mode,
      armedAt: performance.now(),
      coldIdleMs: performance.now() - lastCompositeAt,
      prewarm: null,
      stroke: null,
    };
    ...
    return new Promise((resolve) => {
      ...
      requestAnimationFrame(() => {
        const rafWait = performance.now() - scheduledAt;
        if (profiler) {
          profiler.beginPrewarm({
          ...
          profiler.measure("prewarm-raf-wait", scheduledAt, { mode });
        }
        const started = performance.now();
        let result,
          path,
        ...
        try {
          if (mode === "existing") {
            ...
            path = "existing-empty-1x1";
          } else {
            result = window._prewarmRealDisposableDab
              ? window._prewarmRealDisposableDab()
              : { success: false, error: "Real-dab hook unavailable" };
            path = "real-disposable-dab";
          }
          ...
          if (profiler)
            profiler.endPrewarm({ path, mode, error: error || null, result });
          record.prewarm = {
            mode,
            path,
            requestAnimationFrameWait: rafWait,
            totalDuration: duration,
            result,
            stages:
              profiler && profiler.prewarmResults
                ? stageMap(profiler.prewarmResults().slice(-1)[0])
                : {},
          };
  ...
  function resume(reason) {
    lastVisibilityResumeAt = performance.now();
    markCold(reason);
    schedule(reason);
  }
  document.addEventListener("visibilitychange", () => {
    if (document.hidden) markCold("hidden");
    else resume("visible");
  });
  window.addEventListener("blur", () => markCold("blur"));
  window.addEventListener("focus", () => resume("focus"));
  window.addEventListener("pageshow", () => resume("pageshow"));
  canvasArea.addEventListener(
    "pointerenter",
    (event) => {
      if (!event.buttons) schedule("pointerenter-after-idle");
    },
    { passive: true },
  );
  canvasArea.addEventListener(
    "pointermove",
    (event) => {
      if (!event.buttons) schedule("pointer-hover-after-idle");
    },
    { passive: true },
  );
  return {
    ...
    testNone() {
      return armExperiment("none");
    },
    testExisting() {
      return armExperiment("existing");
    },
    testRealDab() {
      return armExperiment("real-dab");
    },
    compareModes() {
      return {
        instructions: [
          "Wait for the desired cold period.",
          "Call one test method and await it.",
          "Draw exactly one normal stroke.",
          "Repeat for all three modes under the same brush/layer conditions.",
        ],
        methods: [
          "CompositionPrewarm.testNone()",
          "CompositionPrewarm.testExisting()",
          "CompositionPrewarm.testRealDab()",
        ],
        results: experimentRecords.map(clone),
        pending: experimentPending && clone(experimentPending),
      };
    },
    experimentResults() {
      return experimentRecords.map(clone);
    },
    ...
    get lastCompositeAt() {
      return lastCompositeAt;
    },
    get lastVisibilityResumeAt() {
      return lastVisibilityResumeAt;
    },
    get idleThresholdMs() {
      return idleThresholdMs;
    },
    set idleThresholdMs(value) {
      const next = Number(value);
      if (Number.isFinite(next) && next >= 1000) idleThresholdMs = next;
    },
    debug() {
      return {
        compositionWarm,
        ...
        lastError,
        lastPath,
        experimentPending: experimentPending && clone(experimentPending),
      };
    },
  };
})();
...
const BrushRafExperiment = (function () {
  ...
  function hashBytes(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if ((i & 3) === 3 && data[i]) nonEmpty = true;
    }
    return { hash: (hash >>> 0).toString(16), nonEmpty };
  }
  ...
  function smartDigest(layer) {
    if (!layer || layer.type !== "smart-raster") return null;
    if (typeof window.SmartRasterV4DebugDigest === "function") {
      const v4 = window.SmartRasterV4DebugDigest();
      if (v4) return v4;
    }
    const frame = layer.smartStyleFrames && layer.smartStyleFrames[curFrame],
      text = JSON.stringify(frame || null),
      bytes = new TextEncoder().encode(text);
    return { fallback: true, length: text.length, hash: hashBytes(bytes).hash };
  }
  function setupSnapshot() {
    const layer = layers[curLayer],
      key = layer && layer.frames && layer.frames[curFrame],
      artwork = artworkDigest();
    return {
      layerIndex: curLayer,
      frameIndex: curFrame,
      layerType: layer && layer.type,
      renderMode: (layer && layer.renderMode) || null,
      keyExists: !!key,
      nonEmpty: !!key && artwork.nonEmpty,
      artworkDigest: artwork.hash,
      smartDigest: smartDigest(layer),
      canvas: { width: CW, height: CH },
      brush: {
        ...
        blend: window.brushBlendMode || "normal",
      },
  ...
  function signature(s) {
    return JSON.stringify({
      ...
      brush: s.brush,
      zoom: s.zoom,
      artworkDigest: s.artworkDigest,
      smartDigest: s.smartDigest,
    });
  }
  function point(record, name) {
    return (
      record &&
      record.points &&
      record.points.find((item) => item.name === name)
    );
  }
  function stage(record, name) {
    return (
      record &&
      record.stages &&
      record.stages.find((item) => item.name === name)
    );
  }
  function metric(record, name) {
    const item = point(record, name);
    return item ? item.fromPointerDown : null;
  }
  ...
  function stats(values) {
    ...
    if (!values.length)
      return { n: 0, median: null, min: null, max: null, p90: null };
    return {
      n: values.length,
  ...
  async function prepareTrial(nextMode, coldMs = 12000) {
    if (!MODES.has(nextMode))
      throw new Error(
        "Mode must be current, immediate, deadline-aware, or prewarm-current.",
      );
    if (pending || preparing)
      throw new Error(
        "Finish the pending/preparing trial before starting another.",
      );
    installRafObserver();
    window.__suspendAutomaticCompositionPrewarm = true;
    if (window.CompositionPrewarm) window.CompositionPrewarm.cancelPending();
    window.BrushLatencyProfiler.enable(true);
    const setup = setupSnapshot();
    if (!setup.keyExists)
      throw new Error(
        "Controlled trial rejected: the current frame has no exact keyframe.",
      );
    if (!setup.nonEmpty)
      throw new Error(
        "Controlled trial rejected: the exact keyframe is empty.",
      );
    if (!baseline) baseline = setup;
    else if (signature(setup) !== signature(baseline))
      throw new Error(
        "Controlled trial rejected: artwork, Smart Raster ownership, brush, zoom, canvas, or layer type differs from the series baseline. Undo the prior trial first.",
      );
    coldMs = Math.max(1000, Number(coldMs) || 12000);
    preparing = true;
    mode = nextMode;
    const coldStartedAt = performance.now();
    await new Promise((resolve) => setTimeout(resolve, coldMs));
    if (curLayer !== setup.layerIndex || curFrame !== setup.frameIndex) {
      preparing = false;
      mode = "off";
      throw new Error(
        "Controlled trial rejected: layer or frame changed during the cold wait.",
      );
    }
    const actualColdMs =
      performance.now() - window.CompositionPrewarm.lastCompositeAt;
    if (actualColdMs < coldMs - 5) {
      preparing = false;
      mode = "off";
      throw new Error(
        "Controlled trial rejected: a recomposite occurred during the cold wait.",
      );
    }
    let prewarm = null;
    if (mode === "prewarm-current") {
      const scheduled = performance.now();
      prewarm = await new Promise((resolve) =>
        requestAnimationFrame(() => {
          const started = performance.now(),
            success = !!(
              window._prewarmLiveStrokeComposition &&
    ...
    window.BrushLatencyProfiler.labelNextStroke("raf-" + mode);
    return clone(pending);
  }
  function strokeBegins(detail) {
    if (!pending) return;
    pending.strokeStartedAt = performance.now();
    pending.strokeStart = detail;
    pending.undoBefore = undoStack.length;
    pending.redoBefore = redoStack.length;
  }
  function noteKeyCheck(detail) {
    if (pending) pending.keyCheck = detail;
  }
  function noteRecompositeRequest(detail) {
    if (pending)
      pending.events.push(
        Object.assign({ type: "request", time: performance.now() }, detail),
      );
  }
  function noteCoalescedRequest() {
    if (pending) pending.coalescedRequests++;
  }
  function shouldPresentImmediately(detail) {
    return (
      !!pending &&
      detail.firstDab &&
      (mode === "immediate" ||
        (mode === "deadline-aware" && !detail.rafAlreadyPending))
    );
  }
  function rafState() {
    return { executions: rafExecutions };
  }
  function anotherRafRanFirst(state) {
    return state ? rafExecutions > state.executions + 1 : null;
  }
  function noteRafCallback(detail) {
    if (pending) pending.raf = detail;
  }
  function noteRecompositeBegin() {
    if (pending) pending.recompositeBeginAt = performance.now();
  }
  function strokeEnds(detail) {
    if (!pending) return;
    const stroke = window.BrushLatencyProfiler.latest(),
      visible = point(stroke, "first-display-presented"),
      scheduled = point(stroke, "recomposite-raf-scheduled"),
      callback = point(stroke, "recomposite-raf-callback-begin"),
      recomp = stage(stroke, "layer-composite"),
      upload = stage(stroke, "canvas-display-upload");
    const result = Object.assign(pending, {
      stroke,
      metrics: {
        pointerdownToRafScheduling: scheduled
          ? scheduled.fromPointerDown
          : null,
        pointerdownToFirstDabGenerated: metric(stroke, "first-dab-generated"),
        rafSchedulingWait: stage(stroke, "raf-wakeup-wait")?.duration || 0,
        recompositeTime: recomp?.duration || null,
        displayUploadTime: upload?.duration || null,
        pointerdownToFirstVisibleDab: visible ? visible.fromPointerDown : null,
        rafCallbackAt: callback ? callback.fromPointerDown : null,
      },
      safety: {
        autoCreatedKey: !!(pending.keyCheck && pending.keyCheck.autoCreatedKey),
        ensureKeyDuration: pending.keyCheck && pending.keyCheck.duration,
        undoDelta: undoStack.length - pending.undoBefore,
        redoBefore: pending.redoBefore,
        redoAfter: redoStack.length,
        layerUnchanged: curLayer === pending.setup.layerIndex,
        frameUnchanged: curFrame === pending.setup.frameIndex,
        dabCount: detail && detail.dabCount,
        spacing: window.brushSpacing,
        pressurePointerType:
          stroke && stroke.detail && stroke.detail.pointerType,
        firstDab:
          stage(stroke, "dab-rasterization") &&
          stage(stroke, "dab-rasterization").detail,
        laterRequestsCoalesced: pending.coalescedRequests,
        finalArtworkDigest: artworkDigest().hash,
        finalSmartDigest: smartDigest(layers[curLayer]),
      },
  ...
  function table(layerType) {
    const rows = records.filter(
        (r) => !layerType || r.setup.layerType === layerType,
      ),
      names = ["current", "immediate", "deadline-aware", "prewarm-current"];
    return names.map((name) => {
      const group = rows.filter((r) => r.mode === name);
      return {
        mode: name,
        trials: group.length,
        pointerdownToFirstDabGenerated: stats(
          group.map((r) => r.metrics.pointerdownToFirstDabGenerated),
        ),
        rafSchedulingWait: stats(group.map((r) => r.metrics.rafSchedulingWait)),
        recompositeTime: stats(group.map((r) => r.metrics.recompositeTime)),
        displayUploadTime: stats(group.map((r) => r.metrics.displayUploadTime)),
        pointerdownToFirstVisibleDab: stats(
          group.map((r) => r.metrics.pointerdownToFirstVisibleDab),
        ),
      };
    });
  }
  function safetyReport() {
    return records.map((r) => ({
      id: r.id,
      mode: r.mode,
      layerType: r.setup.layerType,
      safety: r.safety,
      valid:
        !r.safety.autoCreatedKey &&
        r.safety.layerUnchanged &&
        r.safety.frameUnchanged &&
        r.safety.undoDelta === 1 &&
        r.safety.redoAfter === 0,
    }));
  }
  function clear() {
    records.length = 0;
  ...
  function stop() {
    ...
    preparing = false;
    mode = "off";
    window.__suspendAutomaticCompositionPrewarm = false;
    uninstallRafObserver();
    return true;
  }
  return {
    prepareTrial,
    ...
    noteRecompositeRequest,
    noteCoalescedRequest,
    shouldPresentImmediately,
    rafState,
    anotherRafRanFirst,
    noteRafCallback,
    noteRecompositeBegin,
    strokeBegins,
    noteKeyCheck,
    strokeEnds,
  };
...
const BrushFirstDabExperiment = (function () {
  ...
  const stage = (stroke, name) =>
    stroke && stroke.stages && stroke.stages.find((item) => item.name === name);
  const point = (stroke, name) =>
    stroke && stroke.points && stroke.points.find((item) => item.name === name);
  const stageTotal = (stroke, name) =>
    stroke && stroke.stages
      ? stroke.stages
  ...
  function setMode(next) {
    if (!MODES.has(next))
      throw new Error(
        "Mode must be current, immediate-first-dab, or deadline-aware-first-dab.",
      );
    mode = next;
    if (window.BrushLatencyProfiler) window.BrushLatencyProfiler.enable(true);
    return mode;
  }
  function strokeBegins(detail) {
    active = {
      ...
      detail,
      presentations: [],
      scheduledDisposition: "none",
    };
  }
  function decide({ firstDab, rafAlreadyPending }) {
    if (!firstDab || mode === "current") return { immediate: false };
    if (mode === "immediate-first-dab") return { immediate: true };
    return { immediate: !rafAlreadyPending };
  }
  function noteRecompositeRequest(detail) {
    if (active)
      (active.requests || (active.requests = [])).push(
        Object.assign({ at: performance.now() }, detail),
      );
  }
  function noteCoalescedRequest() {
    if (active) active.coalescedRequests = (active.coalescedRequests || 0) + 1;
  }
  function noteScheduledDisposition(value) {
    if (active) active.scheduledDisposition = value;
  }
  function notePresentation(detail) {
    if (!active) return;
    active.presentations.push(Object.assign({ at: performance.now() }, detail));
    active.scheduledDisposition =
      detail.scheduledWork || active.scheduledDisposition;
  }
  function rafState() {
    return { executions: rafExecutions };
  }
  function anotherRafRanFirst() {
    return null;
  }
  function noteRafCallback(detail) {
    rafExecutions++;
    if (active) active.raf = detail;
  }
  function strokeEnds(detail) {
    if (!active) return;
    const stroke =
      ...
      visible = point(stroke, "first-display-presented"),
      firstDab = stage(stroke, "first-dab-pipeline-total"),
      sync = stage(stroke, "synchronous-first-dab-recomposite"),
      raf = stage(stroke, "raf-wakeup-wait"),
      scheduled = stage(stroke, "scheduled-recomposite-duration"),
      upload = stage(stroke, "canvas-display-upload");
    active.metrics = {
      pointerdownToFirstVisible: visible ? visible.fromPointerDown : null,
      firstDabRasterization: firstDab ? firstDab.duration : null,
      synchronousFirstDabRecomposite: sync ? sync.duration : 0,
      rafWait: raf ? raf.duration : 0,
      scheduledRecomposite: scheduled ? scheduled.duration : 0,
      displayUpload: upload ? upload.duration : null,
      smartEnsureKey: stageTotal(stroke, "smart-raster-ensure-key"),
      smartStyleResolution: stageTotal(
        stroke,
        "smart-raster-active-style-resolution",
      ),
      smartPreviewState: stageTotal(
        stroke,
        "smart-raster-preview-state-resolution",
      ),
      smartRectClipping: stageTotal(
        stroke,
        "smart-raster-preview-rect-clipping",
      ),
      smartStyleBundleSync: stageTotal(
        stroke,
        "smart-raster-style-bundle-synchronization",
      ),
      smartMaskReadback: stageTotal(
        stroke,
        "smart-raster-preview-mask-readback",
      ),
      smartStyleIndexUpdate: stageTotal(
        stroke,
        "smart-raster-style-index-canvas-update",
      ),
      smartOwnershipResolution: stageTotal(
        stroke,
        "smart-raster-preview-ownership-resolution",
      ),
      smartPreviewUpload: stageTotal(
        stroke,
        "smart-raster-preview-canvas-upload",
      ),
      smartPreviewGeneration: stageTotal(
        stroke,
        "smart-raster-preview-generation-total",
      ),
      smartPreviewCanvasSwap: stageTotal(
        stroke,
        "smart-raster-preview-canvas-swap",
      ),
    };
    active.dabCount = detail && detail.dabCount;
    active.duplicatePresentationCount = Math.max(
      0,
      active.presentations.length - 1,
    );
    active.dirtyRect =
      (active.presentations[0] && active.presentations[0].rect) ||
      (firstDab && firstDab.detail && firstDab.detail.dirtyRect) ||
      null;
    records.push(active);
    active = null;
  }
  function compare() {
    const layerTypes = [
      ...new Set(records.map((row) => row.detail && row.detail.layerType)),
    ];
    return layerTypes.flatMap((layerType) =>
      [...MODES].map((selected) => {
        const rows = records.filter(
          (row) =>
            row.mode === selected &&
            row.detail &&
            row.detail.layerType === layerType,
        );
        return {
          layerType,
          mode: selected,
          trials: rows.length,
          pointerdownToFirstVisible: stats(rows, "pointerdownToFirstVisible"),
          firstDabRasterization: stats(rows, "firstDabRasterization"),
          synchronousFirstDabRecomposite: stats(
            rows,
            "synchronousFirstDabRecomposite",
          ),
          rafWait: stats(rows, "rafWait"),
          scheduledRecomposite: stats(rows, "scheduledRecomposite"),
          displayUpload: stats(rows, "displayUpload"),
          smartEnsureKey: stats(rows, "smartEnsureKey"),
          smartStyleResolution: stats(rows, "smartStyleResolution"),
          smartPreviewState: stats(rows, "smartPreviewState"),
          smartRectClipping: stats(rows, "smartRectClipping"),
          smartStyleBundleSync: stats(rows, "smartStyleBundleSync"),
          smartMaskReadback: stats(rows, "smartMaskReadback"),
          smartStyleIndexUpdate: stats(rows, "smartStyleIndexUpdate"),
          smartOwnershipResolution: stats(rows, "smartOwnershipResolution"),
          smartPreviewUpload: stats(rows, "smartPreviewUpload"),
          smartPreviewGeneration: stats(rows, "smartPreviewGeneration"),
          smartPreviewCanvasSwap: stats(rows, "smartPreviewCanvasSwap"),
          duplicatePresentations: rows.reduce(
            (sum, row) => sum + row.duplicatePresentationCount,
            0,
          ),
        };
      }),
  ...
  function clear() {
    ...
    active = null;
    serial = 0;
    if (window.BrushLatencyProfiler) window.BrushLatencyProfiler.clear();
    return true;
  }
  return {
    clear,
...
const KeyframeLatencyExperiment = (function () {
  const KEY_STATES = new Set(["empty", "precreated"]),
    PRESENTATIONS = new Set(["current", "immediate", "deadline-aware"]);
  let pending = null,
    preparing = false,
    baseline = null,
    original = null,
  ...
  function config() {
    return {
      ...
      canvas: { width: CW, height: CH },
      brush: {
        preset: window._activeBrushPresetId || null,
        size: getBrushSize(),
        hardness: brushHardness,
        opacity: brushOpacity,
        flow: brushFlow,
  ...
  function point(stroke, name) {
    return (
      stroke &&
      stroke.points &&
      stroke.points.find((item) => item.name === name)
    );
  }
  function stage(stroke, name) {
    return (
      stroke &&
      stroke.stages &&
      stroke.stages.find((item) => item.name === name)
    );
  }
  function duration(stroke, name) {
    const item = stage(stroke, name);
    return item ? item.duration : 0;
  }
  function fromPointer(stroke, name) {
    const item = point(stroke, name);
    return item ? item.fromPointerDown : null;
  }
  function percentile(values, p) {
  ...
  async function prepareTrial(keyState, presentation, coldMs = 12000) {
    if (!KEY_STATES.has(keyState) || !PRESENTATIONS.has(presentation))
      throw new Error(
        "Use keyState empty/precreated and presentation current/immediate/deadline-aware.",
      );
    if (pending || preparing)
      throw new Error("Finish the current trial first.");
    const current = config();
    if (current.layerType !== "bitmap")
      throw new Error(
        "This six-condition series requires a Normal Raster layer.",
      );
    if (!baseline) {
      baseline = current;
      const layer = layers[curLayer],
        slot = layer.frames && layer.frames[curFrame],
        slotCopy = slot ? mkLayerCanvas() : null,
        activeCopy = mkLayerCanvas();
      if (slotCopy) slotCopy.getContext("2d").drawImage(slot, 0, 0);
      activeCopy.getContext("2d").drawImage(activeC, 0, 0);
      original = {
        layerIndex: curLayer,
        frameIndex: curFrame,
        hadKey: !!slot,
      ...
      historyBaseline = { undo: undoStack.slice(), redo: redoStack.slice() };
    } else if (signature(current) !== signature(baseline))
      throw new Error(
        "Layer, frame, brush, zoom, or canvas differs from the baseline.",
      );
    preparing = true;
    window.__suspendAutomaticCompositionPrewarm = true;
    if (window.CompositionPrewarm) window.CompositionPrewarm.cancelPending();
    window.BrushLatencyProfiler.enable(true);
    undoStack.splice(0, undoStack.length, ...historyBaseline.undo);
    redoStack.splice(0, redoStack.length, ...historyBaseline.redo);
    const layer = layers[curLayer];
    ctx.clearRect(0, 0, CW, CH);
    if (keyState === "empty") delete layer.frames[curFrame];
    else {
      const blank = mkLayerCanvas();
      blank.getContext("2d").clearRect(0, 0, CW, CH);
      layer.frames[curFrame] = blank;
    }
    recomposite(curLayer, curFrame);
    renderTimeline();
    updateStatus();
    ...
    const coldStartedAt = performance.now();
    await new Promise((resolve) => setTimeout(resolve, coldMs));
    if (curLayer !== current.layerIndex || curFrame !== current.frameIndex) {
      preparing = false;
      throw new Error("Layer or frame changed during cold wait.");
    }
    const actualColdMs =
      performance.now() - window.CompositionPrewarm.lastCompositeAt;
    if (actualColdMs < coldMs - 5) {
      preparing = false;
      throw new Error("A recomposite occurred during the cold wait.");
    }
    const exactKey = !!(
      layers[curLayer].frames && layers[curLayer].frames[curFrame]
    );
    if (exactKey !== (keyState === "precreated")) {
      preparing = false;
      throw new Error("Exact keyframe state changed during the cold wait.");
    }
    pending = {
      id: ++serial,
      keyState,
      presentation,
      condition: keyState + "/" + presentation,
      setup: current,
      coldMs: actualColdMs,
      undoBefore: undoStack.length,
      redoBefore: redoStack.length,
      events: [],
      coalesced: 0,
    };
    preparing = false;
    window.BrushLatencyProfiler.labelNextStroke(
      "keyframe-" + keyState + "-" + presentation,
    );
    return clone(pending);
  }
  function strokeBegins(detail) {
    if (pending) {
      pending.strokeBeginsAt = performance.now();
      pending.strokeDetail = detail;
    }
  }
  function noteKeyCheck(detail) {
    if (pending) pending.keyCheck = detail;
  ...
  function noteRecompositeRequest(detail) {
    if (pending)
      pending.events.push(Object.assign({ time: performance.now() }, detail));
  }
  function noteCoalescedRequest() {
    if (pending) pending.coalesced++;
  }
  function shouldPresentImmediately(detail) {
    return (
      !!pending &&
      detail.firstDab &&
      (pending.presentation === "immediate" ||
        (pending.presentation === "deadline-aware" &&
          !detail.rafAlreadyPending))
    );
  }
  function rafState() {
    return window.BrushRafExperiment
      ? window.BrushRafExperiment.rafState()
      : null;
  }
  function anotherRafRanFirst(state) {
    return window.BrushRafExperiment
      ? window.BrushRafExperiment.anotherRafRanFirst(state)
      : null;
  }
  function noteRafCallback(detail) {
    if (pending) pending.raf = detail;
  ...
  function noteRecompositeBegin() {
    if (pending) pending.recompositeBeginsAt = performance.now();
  }
  function strokeEnds(detail) {
    if (!pending) return;
    const stroke = window.BrushLatencyProfiler.latest(),
    ...
    pending.metrics = {
      pointerdownToPushUndoComplete: fromPointer(stroke, "push-undo-complete"),
      pushUndoDuration: duration(stroke, "undo-snapshot-setup"),
      ensureKeyDuration: pending.keyCheck && pending.keyCheck.duration,
      keyCanvasAllocation: duration(stroke, "key-canvas-allocation"),
      keyInitializationCopy:
        duration(stroke, "key-frame-map-insertion") +
        duration(stroke, "key-initialization-clear"),
      smartRasterInitialization: duration(
        stroke,
        "smart-raster-key-initialization",
      ),
      timelineRefresh: duration(stroke, "key-creation-timeline-refresh"),
      drawingMarkUpdates: duration(stroke, "drawing-mark-lookup-work"),
      thumbnailWork: duration(stroke, "key-thumbnail-work"),
      firstDabGeneration: fromPointer(stroke, "first-dab-generated"),
      rafSchedulingWait: duration(stroke, "raf-wakeup-wait"),
      recomposite: duration(stroke, "layer-composite"),
      displayUpload: duration(stroke, "canvas-display-upload"),
      pointerdownToFirstVisibleDab: visible ? visible.fromPointerDown : null,
    };
    pending.safety = {
      expectedCreation: pending.keyState === "empty",
      actualCreation: !!(pending.keyCheck && pending.keyCheck.autoCreatedKey),
      undoDelta: undoStack.length - pending.undoBefore,
      redoAfter: redoStack.length,
      layerStable: curLayer === pending.setup.layerIndex,
      frameStable: curFrame === pending.setup.frameIndex,
      dabCount: detail && detail.dabCount,
      spacing: window.brushSpacing,
      pointerType: stroke && stroke.detail && stroke.detail.pointerType,
      laterRequestsCoalesced: pending.coalesced,
    };
    pending.completedAt = performance.now();
    ...
    for (const keyState of KEY_STATES)
      for (const presentation of PRESENTATIONS) {
        const group = records.filter(
            (item) =>
              item.keyState === keyState && item.presentation === presentation,
          ),
          row = {
            condition: keyState + "/" + presentation,
            trials: group.length,
        ...
        for (const name of [
          "pointerdownToPushUndoComplete",
          "pushUndoDuration",
          "ensureKeyDuration",
          "keyCanvasAllocation",
          "keyInitializationCopy",
          "smartRasterInitialization",
          "timelineRefresh",
          "drawingMarkUpdates",
          "thumbnailWork",
          "firstDabGeneration",
          ...
          row[name] = stats(group.map((item) => item.metrics[name]));
        rows.push(row);
      }
    return rows;
  }
  ...
  function stop() {
    ...
    preparing = false;
    if (original) {
      const layer = layers[original.layerIndex];
      if (layer && layer.frames) {
        if (original.hadKey) layer.frames[original.frameIndex] = original.slot;
        else delete layer.frames[original.frameIndex];
      }
      if (
        curLayer === original.layerIndex &&
        curFrame === original.frameIndex
      ) {
        ctx.clearRect(0, 0, CW, CH);
        ctx.drawImage(original.active, 0, 0);
        recomposite(curLayer, curFrame);
        renderTimeline();
        updateStatus();
      }
...
function goToFrame(f, addSel, noSel, selectionOnly) {
  if (typeof window.prepareTransformForArtworkChange === "function")
    window.prepareTransformForArtworkChange(curLayer, f);
  saveActiveToKey();
  curFrame = Math.max(0, Math.min(TOTAL - 1, f));
  window.TimelineScrubController?.reset(curFrame);
  if (!noSel) {
    if (!addSel) selectedFrames.clear();
    selectedFrames.add(curFrame);
  }
  loadFrame(curLayer, curFrame);
  window.AudioPlaybackEngine?.frameChanged(curFrame, {
    playing: typeof playing === "boolean" && playing,
    scrubbing: typeof scrubbing === "boolean" && scrubbing,
  });
  if (selectionOnly && typeof refreshTimelineSelection === "function")
...
function switchLayer(li, options) {
  options = options || {};
  if (typeof window.prepareTransformForArtworkChange === "function")
    window.prepareTransformForArtworkChange(li, curFrame);
  saveActiveToKey();
  curLayer = li;
  activeGroupId = null;
  ...
  if (typeof window.clearCameraTimelineSelection === "function")
    window.clearCameraTimelineSelection();
  if (
    !options.preserveTimelineSelection &&
    typeof syncTimelineSelectionToActiveLayer === "function"
  )
  ...
  renderLayerPanel();
  if (
    options.skipTimelineRender &&
    typeof refreshTimelineSelection === "function"
  )
    refreshTimelineSelection();
  else renderTimeline();
  if (
  ...
  window.dispatchEvent(
    new CustomEvent("active-layer-changed", {
      detail: { layerIndex: curLayer, layer: layers[curLayer] },
    }),
  );
}
...
const FloatPanels = (function () {
  ...
  const dz = {
    left: document.getElementById("fp-dz-left"),
    right: document.getElementById("fp-dz-right"),
    top: document.getElementById("fp-dz-top"),
    bottom: document.getElementById("fp-dz-bottom"),
  };
  ...
  const PANEL_CFG = {
    ...
    "keyframe-switcher": {
      ...
      floatResizable: true,
      get minHeight() {
        const p = document.getElementById("keyframe-switcher-panel");
        return (p && parseFloat(p.style.minHeight)) || 120;
      },
    },
    "keyframe-exposure": {
      minSize: 106,
      ...
      floatResizable: true,
      get minHeight() {
        const p = document.getElementById("keyframe-exposure-panel");
        return (p && parseFloat(p.style.minHeight)) || 160;
      },
    },
    "onion-skin": {
      minSize: 250,
    ...
    "drawing-marks": {
      ...
      floatResizable: true,
      get minHeight() {
        const p = document.getElementById("drawing-marks-panel");
        if (!p) return 40;
        const tb = p.querySelector(".fp-titlebar");
        const body = p.querySelector(".fp-body");
        ...
        const kids = body
          ...
          : [];
        const kH = kids.reduce((s, c) => s + c.offsetHeight, 0);
        return tbH + pt + kH + pb;
      },
    },
  };
  function cfgOf(key) {
    return PANEL_CFG[key] || { minSize: 120, maxSize: 500 };
  }

  function panelByKey(key) {
    return allPanels.find((p) => p.dataset.panel === key) || null;
  }
  function registerTitle(panelEl) {
    titles[panelEl.dataset.panel] =
      panelEl.querySelector(".fp-name").textContent;
  }
  ...
  function _isSplitChild(key) {
    return !!splitHost[key];
  }
  function _splitHostOf(key) {
    return splitHost[key] || null;
  }
  function _stackHostOf(key) {
    return splitHost[key] || key;
  }
  function _dockSideOf(key) {
    const hostKey = _stackHostOf(key);
    return (
      ["left", "right", "top", "bottom"].find((s) =>
        dockOrder[s].includes(hostKey),
      ) || null
    );
  }
  function _stackKeys(hostKey) {
    return [hostKey, ..._splitChildrenOf(hostKey)].filter(
      (k) => !hiddenState[k],
    );
  }
  ...
  function _stackLayout(hostKey) {
    ...
    const items = _stackKeys(hostKey)
      .map((key) => {
  ...
  function _stackInsertPoint(hostKey, clientY, activeDrop) {
    ...
    for (let i = 0; i <= layout.items.length; i++) {
      const lineY =
        i < layout.items.length
          ? layout.items[i].rect.top
          : layout.items[layout.items.length - 1].rect.bottom;
      const threshold =
        activeDrop && activeDrop.hostKey === hostKey && activeDrop.atIndex === i
          ? STACK_DOCK_EXIT_THRESHOLD
          : STACK_DOCK_ENTER_THRESHOLD;
  ...
  function hideStackZone() {
    stackZoneEl.style.display = "none";
  }

  function hideZones() {
    Object.values(dz).forEach((z) => z.classList.remove("show"));
    mergeZoneEl.classList.remove("show");
    hideStackZone();
  }
  ...
  function dockPanel(key, side, atIndex) {
    ...
    if (atIndex == null || atIndex < 0 || atIndex > dockOrder[side].length)
      dockOrder[side].push(key);
    ...
    if (dockSize[key] == null) {
      ...
      dockSize[key] =
        key === "tools"
          ? 40
          : Math.min(cfg.maxSize, Math.max(cfg.minSize, 220));
    }
  }
  ...
  function render() {
    ...
    allPanels.forEach((panel) => {
      ...
      if (mode === "floating") {
        ...
        panel.style.height =
          r.h != null ? r.h + "px" : panel.style.height || "";
      }
  ...
  function renderSide(side) {
    ...
    dockOrder[side].forEach((key) => {
      ...
      if (side === "left" || side === "right") {
        ...
        size = Math.max(
          limits.min,
          Math.min(limits.max, Number.isFinite(+size) ? +size : 220),
        );
        dockSize[key] = size;
      }
      ...
      if (
        ...
        _splitChildrenOf(key).length > 0
      ) {
        const children = _splitChildrenOf(key);
        // Sum of all child heights
        const totalChildPx = children.reduce(
          (s, ck) => s + (splitSize[ck] || 180),
          0,
        );
        ...
        children.forEach((ck, ci) => {
          ...
          const prevChildrenPx = children
            .slice(0, ci)
            .reduce((s, k) => s + (splitSize[k] || 180), 0);
    ...
    if (
      ...
      if (tp) {
        ...
        const tbH = tb ? tb.offsetHeight : 0,
          bodyH = bd ? bd.scrollHeight : 0;
        const fitH = tbH + bodyH;
        if (dockSize.tools !== fitH) {
          dockSize.tools = fitH;
          renderSide(side);
          return;
        }
  ...
  function applyDock(panel, side, atIndex) {
    ...
    if (panelMode[key] === "docked") {
      const cur = ["left", "right", "top", "bottom"].find((s) =>
        dockOrder[s].includes(key),
      );
      const curIdx = cur ? dockOrder[cur].indexOf(key) : -1;
      if (
        cur === side &&
        (atIndex == null || atIndex === curIdx || atIndex === curIdx + 1)
      )
  ...
  function _applyStackInsert(panel, hostKey, atIndex) {
    ...
    const hiddenChildren = (splitChildren[hostKey] || []).filter(
      (k) => hiddenState[k] && k !== key,
    );
    const rebuiltStack = [...visibleStack, ...hiddenChildren];
    const newHost = rebuiltStack[0];
    const newChildren = rebuiltStack.slice(1);
    const sharedWidth =
      dockSize[hostKey] ?? dockSize[newHost] ?? cfgOf(newHost).minSize ?? 180;

    _removeFromAllDockOrders(key);

    dockOrder[side] = dockOrder[side].filter(
      (k) => k !== hostKey && !rebuiltStack.includes(k),
    );
    delete splitChildren[hostKey];
    rebuiltStack.forEach((k) => {
      delete splitHost[k];
    });
  ...
  function applyStackDock(panel, targetPanel, pos) {
    ...
    const visibleStack = _stackKeys(hostKey).filter(
      (k) => k !== panel.dataset.panel,
    );
  ...
  function _centerFloatingPanel(key) {
    ...
    const car = canvasArea.getBoundingClientRect();
    const width =
      (floatRect[key] && floatRect[key].w) || panel.offsetWidth || 240;
    const height =
      (floatRect[key] && floatRect[key].h) || panel.offsetHeight || 180;
    ...
    floatRect[key] = Object.assign(floatRect[key] || {}, {
      ...
      y,
      w:
        (floatRect[key] && floatRect[key].w) != null ? floatRect[key].w : width,
      h:
        (floatRect[key] && floatRect[key].h) != null
          ? floatRect[key].h
          : height,
    });
  }
  ...
  function setFloatSize(key, w, h) {
    if (panelMode[key] !== "floating") return;
    floatRect[key] = Object.assign(floatRect[key] || {}, {
      w: w != null ? w : floatRect[key] && floatRect[key].w,
      h: h != null ? h : floatRect[key] && floatRect[key].h,
    });
  }
  ...
  // ── Tab management ────────────────────────────────────────────
  function tabbarOf(panel) {
    return panel.querySelector(".fp-tabbar");
  }
  function bodyWrapOf(panel) {
    return panel.querySelector(".fp-body-wrap");
  }
  function nameOf(panel) {
    return panel.querySelector(".fp-name");
  }
  ...
  function refreshTabbarVisibility(panel) {
    ...
    if (count > 1) {
      ...
      if (activeTab)
        nameOf(panel).textContent =
          titles[activeTab.dataset.key] || activeTab.textContent;
    } else {
  ...
  function activateTab(panel, key) {
    bodyWrapOf(panel)
      .querySelectorAll(".fp-body")
      .forEach((b) => b.classList.toggle("active", b.dataset.body === key));
    tabbarOf(panel)
  ...
  function addTab(hostPanel, key) {
    ...
    tab.dataset.key = key;
    tab.textContent = titles[key] || key;
    tab.title = "Drag out to undock this tab";
    bindTabDrag(tab, hostPanel);
    tab.addEventListener("click", () => {
      if (!tab._justDragged) activateTab(hostPanel, key);
    });
    tb.appendChild(tab);
  }
  ...
  function mergeInto(sourcePanel, targetPanel) {
    ...
    const srcKey = sourcePanel.dataset.panel,
      targetKey = targetPanel.dataset.panel;
    ...
    const existingTargetBodies = [
      ...bodyWrapOf(targetPanel).querySelectorAll(".fp-body"),
    ];
    if (tabbarOf(targetPanel).children.length === 0) {
      existingTargetBodies.forEach((b) => addTab(targetPanel, b.dataset.body));
      const activeExisting =
        existingTargetBodies.find((b) => b.classList.contains("active")) ||
        existingTargetBodies[0];
      if (activeExisting) activateTab(targetPanel, activeExisting.dataset.body);
    }
    bodies.forEach((b) => {
      b.classList.remove("active");
      bodyWrapOf(targetPanel).appendChild(b);
    });
  ...
  function popOutTab(hostPanel, key, x, y) {
    ...
    const body = bodyWrapOf(hostPanel).querySelector(
      `.fp-body[data-body="${key}"]`,
    );
    if (!ownShell || !body) return;
    const tabEl = tabbarOf(hostPanel).querySelector(
      `.fp-tab[data-key="${key}"]`,
    );
    ...
    if (
      remaining.length &&
      !remaining.some((b) => b.classList.contains("active"))
    )
      activateTab(hostPanel, remaining[0].dataset.body);
    clearMerged(key);
  ...
  function setWindowCheck(key, visible, viaMerge) {
    windowCheckHook && windowCheckHook(key, visible, viaMerge);
  }
  ...
  function setVisible(key, visible) {
    ...
    if (visible) {
      ...
      if (panelMode[key] === "merged") {
        ...
        if (wasHidden && panelMode[key] === "floating")
          _centerFloatingPanel(key);
        render();
        bringToFront(shell);
      }
    } else {
      if (panelMode[key] === "merged") {
        ...
        const body = bodyWrapOf(host).querySelector(
          `.fp-body[data-body="${key}"]`,
        );
        const tabEl = tabbarOf(host).querySelector(
          `.fp-tab[data-key="${key}"]`,
        );
        if (tabEl) tabEl.remove();
        if (body) {
          bodyWrapOf(shell).appendChild(body);
          body.classList.add("active");
        }
        ...
        if (
          ...
          activateTab(host, remaining[0].dataset.body);
        clearMerged(key);
      }
  ...
  function resetLayout() {
    allPanels.forEach((panel) => {
      const key = panel.dataset.panel;
      [...tabbarOf(panel).querySelectorAll(".fp-tab")].forEach((t) => {
        if (t.dataset.key !== key) popOutTab(panel, t.dataset.key, 0, 0);
      });
      if (panelMode[key] === "merged")
        popOutTab(panelByKey(mergedHost[key]), key, 0, 0);
    });
    allPanels.forEach((panel) => {
      const key = panel.dataset.panel;
  ...
  function bindDockResize(panel, rh) {
    ...
    rh.addEventListener("pointerdown", (e) => {
      ...
      side = ["left", "right", "top", "bottom"].find((s) =>
        dockOrder[s].includes(hostKey),
      );
      if (!side) return;
      resizing = true;
    ...
    rh.addEventListener("pointermove", (e) => {
      ...
      dockSize[hostKey] = Math.max(
        limits.min,
        Math.min(limits.max, startSize + delta),
      );
      renderSide(side);
      if (typeof centerCanvas === "function") centerCanvas();
    });
    function endResize() {
      ...
      if (
        pointerId != null &&
        rh.hasPointerCapture &&
        rh.hasPointerCapture(pointerId)
      ) {
  ...
  function bindFloatingResize(panel) {
    ...
    Object.keys(EDGE_CSS).forEach((side) => {
      ...
      el.style.cssText =
        "position:absolute;" + EDGE_CSS[side] + "z-index:10;touch-action:none;";
      ...
      el.addEventListener("pointerdown", (e) => {
        ...
        startMinH = cfg.contentFitHeight
          ? _toolsMinHeight()
          : cfg.minHeight || 120;
      ...
      el.addEventListener("pointermove", (e) => {
        ...
        if (side === "right") {
          r.w = Math.max(startMinW, Math.min(cfg.maxSize, startRect.w + dx));
        } else if (side === "left") {
          r.w = Math.max(startMinW, Math.min(cfg.maxSize, startRect.w - dx));
          r.x = startRect.x + (startRect.w - r.w);
        } else if (side === "bottom") {
          r.h = Math.max(minH, Math.min(700, startRect.h + dy));
        } else if (side === "top") {
          r.h = Math.max(minH, Math.min(700, startRect.h - dy));
  ...
  function _syncDockResizeHandle(panel) {
    ...
    if (side === "left" || side === "right") {
      ...
      cursor = "row-resize";
      top =
        side === "top" ? Math.ceil(rect.bottom) : Math.floor(rect.top) - height;
    }
  ...
  function _syncSplitResizeHandle(
    hostKey,
    childKey,
    side,
    colOffset,
  ...
  function _bindSplitResizeDrag(rh, childKey) {
    ...
    rh.addEventListener("pointerdown", (e) => {
      ...
      rh.setPointerCapture(e.pointerId);
      startY = e.clientY;
      startSize = splitSize[childKey] || 180;
    });
    rh.addEventListener("pointermove", (e) => {
      if (!active) return;
      const dy = startY - e.clientY; // drag up = more space for child
      splitSize[childKey] = Math.max(
        60,
        Math.min(window.innerHeight - 100, startSize + dy),
      );
      ...
      renderSide(
        ["left", "right", "top", "bottom"].find((s) =>
          dockOrder[s].includes(currentHost),
        ) || "left",
      );
    ...
    function endDrag(e) {
      ...
      if (moved) {
        ...
        popOutTab(
          hostPanel,
          tabEl.dataset.key,
          e.clientX - car.left - 20,
          e.clientY - car.top - 10,
        );
      }
      moved = false;
    }
  ...
  function bindTitlebarDrag(panel) {
    ...
    document.addEventListener("pointermove", (e) => {
      ...
      const car = canvasArea.getBoundingClientRect();
      let nx = e.clientX - car.left - dx,
        ny = e.clientY - car.top - dy;
      ...
      ["left", "right"].forEach((side) => {
        ...
        dockOrder[side].forEach((hostKey) => {
          ...
          const withinY =
            e.clientY >= layout.columnTop && e.clientY <= layout.columnBottom;
          ...
          for (let i = 0; i <= stack.length; i++) {
            ...
            const linePos =
              side === "left"
                ? car.left + lineOffset
                : side === "right"
                  ? car.right - lineOffset
                  : side === "top"
                    ? car.top + lineOffset
                    : car.bottom - lineOffset;
            const dist = Math.abs(axisPos - linePos);
            const threshold =
              curDock === side && curAtIndex === i
                ? EDGE_DOCK_EXIT_THRESHOLD
                : EDGE_DOCK_ENTER_THRESHOLD;
            ...
            if (i < stack.length) {
              ...
              if (
                hp &&
                panelMode[hostKey] === "docked" &&
                !hiddenState[hostKey] &&
                hp.classList.contains("docked")
              ) {
                const r = hp.getBoundingClientRect();
                colWidth =
                  side === "left" || side === "right" ? r.width : r.height;
              }
      ...
      if (preferEdgeDock) {
        curDock = best.side;
        curAtIndex = best.atIndex;
        curMergeTarget = null;
        curStackDrop = null;
        hideStackZone();
        showEdgeZone(best.side, best.lineOffset);
      } else if (_earlyStackDrop) {
        curStackDrop = _earlyStackDrop;
        curDock = null;
        curMergeTarget = null;
        hideZones();
        showStackZone(
          _earlyStackDrop.left,
          _earlyStackDrop.lineY,
          _earlyStackDrop.width,
        );
      } else if (best) {
        curDock = best.side;
        curAtIndex = best.atIndex;
        ...
        curStackDrop = null;
        {
          ...
          for (const other of allPanels) {
            if (other === panel || other.classList.contains("fp-hidden"))
              continue;
    ...
    document.querySelectorAll(".float-panel").forEach((panel) => {
      ...
      if (panel.classList.contains("docked")) {
        const side = ["left", "right", "top", "bottom"].find((s) =>
          panel.classList.contains("dock-" + s),
        );
        const cssSize =
          side === "left" || side === "right"
            ? panel.offsetWidth
        ...
        undockToFloat(
          key,
          parseFloat(panel.style.left) || r.left - car.left,
          parseFloat(panel.style.top) || r.top - car.top,
        );
      }
  ...
  function _saveLayout() {
    const state = {
      ...
      savedWidths: Object.fromEntries(
        allPanels.map((p) => [p.dataset.panel, p._savedWidth || null]),
      ),
    };
    try {
  ...
  function _restoreLayout() {
    let state;
    try {
      state = JSON.parse(localStorage.getItem(LAYOUT_KEY) || "null");
    } catch (e) {
      return;
    }
    if (!state || !state.panelMode) return;
    ["left", "right", "top", "bottom"].forEach((s) => {
      dockOrder[s] = (state.dockOrder && state.dockOrder[s]) || [];
    });
    ...
    ["left", "right"].forEach((side) =>
      dockOrder[side].forEach((hostKey) => {
        ...
        dockSize[hostKey] = Math.max(
          limits.min,
          Math.min(limits.max, Number.isFinite(width) ? width : 220),
        );
      }),
    );
    allPanels.forEach((p) => {
    ...
    allPanels.forEach((panel) => {
      ...
      if (panelMode[key] === "merged" && mergedHost[key]) {
        ...
        const body =
          bodyWrapOf(panel).querySelector(`.fp-body[data-body="${key}"]`) ||
          bodyWrapOf(panel).firstElementChild;
        if (body) {
          body.classList.remove("active");
          bodyWrapOf(host).appendChild(body);
          addTab(host, key);
        }
      }
    });
    ...
    allPanels.forEach((panel) => {
      ...
      if (
        bodies.length &&
        !bodies.some((b) => b.classList.contains("active"))
      ) {
  ...
  return {
    ...
    isDocked(key) {
      return panelMode[key] === "docked";
    },
    getDockSide(key) {
      return _dockSideOf(key);
    },
    ...
    getStackHost(key) {
      return _stackHostOf(key);
    },
    getDockWidth(key) {
      return dockSize[_splitHostOf(key) || key];
    },
    setDockWidth(key, widthPx) {
      const hostKey = _splitHostOf(key) || key;
      const side = ["left", "right"].find((s) =>
        dockOrder[s].includes(hostKey),
      );
      if (!side) return;
      const limits = _stackWidthLimits(hostKey);
