(function ImageImport() {
  ...
  modal.querySelectorAll(".img-import-opt").forEach((wrap) => {
    ...
    radio.addEventListener("change", () => {
      modal
        .querySelectorAll(".img-import-opt")
  ...
  function openImportModal(img, fileName) {
    ...
    subText.textContent = `Image: ${img.naturalWidth}×${img.naturalHeight}px — where to place it?`;
  ...
  function doImport() {
    const dest =
      (modal.querySelector('input[name="img-import-dest"]:checked') || {})
        .value || "new-layer";
    ...
    if (dest === "new-layer" || dest === "group-layer") {
      ...
      if (dest === "group-layer") {
        const gid =
          activeGroupId ||
          (layers[curLayer] && layers[curLayer].groupId) ||
          null;
        if (gid) {
          ...
          let topIdx = -1;
          layers.forEach((l, i) => {
            if (l.groupId === gid && i > topIdx) topIdx = i;
          });
  ...
  async function pasteImageFromClipboard() {
    if (!navigator.clipboard || !navigator.clipboard.read) {
      showInfo(
        "Clipboard image paste isn't supported in this browser.",
        "Paste Image",
      );
      return;
    }
    ...
    try {
      ...
      showInfo(
        "Couldn't read the clipboard. Your browser may need permission, or there's no image copied.",
        "Paste Image",
      );
      return;
    }
    for (const item of items) {
      ...
      try {
        ...
        showInfo(
          "Could not load the pasted image: " + err.message,
          "Paste Image",
        );
        return;
      }
  ...
  function isImageDrop(dt) {
    ...
    if (fileItems.length) {
      ...
      if (typedItems.length)
        return typedItems.some((item) => item.type.startsWith("image/"));
    }
  ...
  document.addEventListener(
    ...
    (e) => {
      ...
      loadImageFile(imgFile)
        ...
        .catch((err) =>
          showInfo("Could not load image: " + err.message, "Import Error"),
        );
    },
    false,
  );
