...
(function(){
  ...
  function contrastStroke(path,width){
    ...
    ctx.strokeStyle='rgba(0,0,0,.9)';ctx.lineWidth=width+2;ctx.stroke(path);
    ctx.strokeStyle='rgba(255,255,255,.98)';ctx.lineWidth=width;ctx.stroke(path);ctx.restore();
  }
  function drawPoint(){
    const size=7,c=size/2;prepare(size);ctx.beginPath();ctx.arc(c,c,1.25,0,Math.PI*2);ctx.fillStyle='#fff';ctx.fill();
  }
  function drawCross(){
    const size=9,c=size/2;prepare(size);ctx.beginPath();
    ctx.moveTo(c,1.5);ctx.lineTo(c,7.5);ctx.moveTo(1.5,c);ctx.lineTo(7.5,c);
    ctx.strokeStyle='#fff';ctx.lineWidth=1;ctx.lineCap='butt';ctx.stroke();
  }
  ...
  function drawShape(){
    ...
    if(!tip||!tip.width||!tip.height){
      ...
      ctx.strokeStyle='rgba(0,0,0,.9)';ctx.lineWidth=3/Math.max(roundness,.2);ctx.stroke(path);
      ctx.strokeStyle='rgba(255,255,255,.98)';ctx.lineWidth=1/Math.max(roundness,.2);ctx.stroke(path);ctx.restore();return;
    }
  ...
  function update(force){
    ...
    cursorCanvas.style.mixBlendMode=(cursorStyle==='point'||cursorStyle==='crosshair')?'difference':'normal';
    if(force||next!==lastSignature){
      if(cursorStyle==='point')drawPoint();
