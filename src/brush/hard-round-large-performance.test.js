...
test('compatible AA-On stylus Hard Round uses a direct GPU overlay while AA Off stays CPU',()=>{const src=fs.readFileSync(path.join(__dirname,'brush-engine.js'),'utf8');assert.ok(/new window\.PrototypeRenderer\(\{width:w, height:h, preferGpu:true\}\)/.test(src));assert.ok(/const gpuLiveCompatible=hardRoundAaMode!==['"]off['"]&&hardRoundAaMode!==['"]none['"]/.test(src));assert.ok(/hardRoundRenderer\.preferGpu=gpuLiveCompatible/.test(src));assert.ok(/renderer\.isGpuActive[\s\S]*?_hardRoundSetGpuOverlayVisible\(true\);[\s\S]*?return;/.test(src));});
test('GPU live presentation stays GPU-to-GPU without per-frame readback',()=>{const src=fs.readFileSync(path.join(__dirname,'prototype-renderer.js'),'utf8');assert.ok(/else await this\.gpu\.present\(/.test(src));assert.ok(/if \(readback\) await this\.gpu\.resolveInto\(/.test(src));assert.ok(/outputContext\.getCurrentTexture\(\)/.test(src));});
test('GPU preview waits for presentation before its canvas is exposed',()=>{const src=fs.readFileSync(path.join(__dirname,'prototype-renderer.js'),'utf8');assert.ok(/await this\.device\.queue\.onSubmittedWorkDone\(\)/.test(src));assert.ok(/await this\.gpu\.present\(this\._rgb, this\._composite, this\.presentationOpacity\)/.test(src));});
test('GPU live frames avoid readback and pointer-up performs one authoritative readback',()=>{const engine=fs.readFileSync(path.join(__dirname,'brush-engine.js'),'utf8'),renderer=fs.readFileSync(path.join(__dirname,'prototype-renderer.js'),'utf8');assert.ok(/renderer\.endStroke\(\{readback:gpuCommit\}\)/.test(engine));assert.ok(/if \(readback\) await this\.gpu\.resolveInto/.test(renderer));assert.ok(/else await this\.gpu\.present/.test(renderer));});
