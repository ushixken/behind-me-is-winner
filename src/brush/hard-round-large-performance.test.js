...
test('stylus Hard Round uses the verified visible Canvas2D presentation path',()=>{const src=fs.readFileSync(path.join(__dirname,'brush-engine.js'),'utf8');assert.ok(/new window\.PrototypeRenderer\(\{width:w, height:h, preferGpu:false\}\)/.test(src));assert.ok(/hardRoundRenderer\.preferGpu=false/.test(src));});
test('GPU live presentation stays GPU-to-GPU without full-canvas readback',()=>{const src=fs.readFileSync(path.join(__dirname,'prototype-renderer.js'),'utf8');const resolve=src.slice(src.indexOf('async _resolveToOutput()'),src.indexOf('_resultCanvas()',src.indexOf('async _resolveToOutput()')));assert.ok(/this\.gpu\.present\(/.test(resolve));assert.ok(!/this\.gpu\.resolveInto\(|\.mapAsync\(|\.createImageData\(|\.putImageData\(/.test(resolve));assert.ok(/outputContext\.getCurrentTexture\(\)/.test(src));});
test('GPU preview waits for presentation before its canvas is copied',()=>{const src=fs.readFileSync(path.join(__dirname,'prototype-renderer.js'),'utf8');assert.ok(/await this\.device\.queue\.onSubmittedWorkDone\(\)/.test(src));assert.ok(/await this\.gpu\.present\(this\._rgb, this\._composite\)/.test(src));});
