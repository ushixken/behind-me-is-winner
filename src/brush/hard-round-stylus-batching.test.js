...
test('expensive live frames yield to the next RAF without dropping queued segments',()=>{const src=fs.readFileSync(path.join(__dirname,'brush-engine.js'),'utf8');assert.ok(!/rasterMs>8\?Math\.min\(50,rasterMs\):0/.test(src));assert.ok(!/_hardRoundPreviewTimer=setTimeout/.test(src));assert.ok(/_hardRoundPreviewNeedsFollowup\|\|_hardRoundPendingRenderSegments\.length>0/.test(src));assert.ok(/splice\(0,_hardRoundPendingRenderSegments\.length\)/.test(src));});
test('stroke completion cancels scheduled preview and invalidates follow-up state',()=>{const src=fs.readFileSync(path.join(__dirname,'brush-engine.js'),'utf8');const start=src.indexOf('function _hardRoundCancelLivePreview(');const body=src.slice(start,src.indexOf('\n}',start)+2);assert.ok(/cancelAnimationFrame/.test(body));assert.ok(/_hardRoundPreviewRequestedRenderer = null/.test(body));assert.ok(/_hardRoundPreviewNeedsFollowup = false/.test(body));});
