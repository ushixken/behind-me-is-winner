...
(function () {
  ...
  bindRange("ts-opacity", "ts-opacity-val", "", (v) => {
    brushOpacity = v / 100;
  });
  // Flow (per-dab build-up within the stroke)
  bindRange("ts-flow", "ts-flow-val", "", (v) => {
    brushFlow = v / 100;
  });
  // Hardness
  bindRange("ts-hardness", "ts-hardness-val", "", (v) => {
    brushHardness = v / 100;
    _aaDabCache.clear();
  ...
  bindRange("ts-spacing", "ts-spacing-val", "%", (v) => {
    window._tsSpacing = v / 100;
  });
  ...
  const BRUSH_BLEND_MODE_GROUPS = [
    [
      "Normal",
      [
        ["normal", "Normal", "Paints normally over existing pixels."],
        ["draw-behind", "Draw Behind", "Paints behind existing pixels."],
      ],
    ],
    [
      "Darken",
      [
        ["darken", "Darken", "Keeps the darker brush or artwork color."],
        [
          "multiply",
          "Multiply",
          "Darkens by multiplying brush and artwork colors.",
        ],
        [
          "color-burn",
          "Color Burn",
          "Deepens shadows and contrast using the brush color.",
        ],
        ["linear-burn", "Linear Burn", "Coming Soon", false],
        ["darker-color", "Darker Color", "Coming Soon", false],
      ],
    ],
    [
      "Lighten",
      [
        ["lighten", "Lighten", "Keeps the lighter brush or artwork color."],
        ["screen", "Screen", "Lightens by screening brush and artwork colors."],
        [
          "color-dodge",
          "Color Dodge",
          "Brightens artwork toward the brush color.",
        ],
        ["glow-dodge", "Glow Dodge", "Coming Soon", false],
        [
          "add",
          "Add",
          "Adds the brush color to existing artwork, increasing brightness.",
        ],
        [
          "add-glow",
          "Add (Glow)",
          "Creates a stronger luminous additive effect than Add.",
        ],
        ["lighter-color", "Lighter Color", "Coming Soon", false],
      ],
    ],
    [
      "Contrast",
      [
        [
          "overlay",
          "Overlay",
          "Combines Multiply and Screen based on the artwork.",
        ],
        [
          "soft-light",
          "Soft Light",
          "Adds gentle contrast using the brush color.",
        ],
        [
          "hard-light",
          "Hard Light",
          "Adds strong contrast based on the brush color.",
        ],
        ["vivid-light", "Vivid Light", "Coming Soon", false],
        ["linear-light", "Linear Light", "Coming Soon", false],
        ["pin-light", "Pin Light", "Coming Soon", false],
        ["hard-mix", "Hard Mix", "Coming Soon", false],
      ],
    ],
    [
      "Comparison",
      [
        [
          "difference",
          "Difference",
          "Shows the channel difference between brush and artwork.",
        ],
        ["exclusion", "Exclusion", "Creates a softer difference effect."],
      ],
    ],
    [
      "Component",
      [
        [
          "hue",
          "Hue",
          "Uses brush hue while preserving artwork saturation and brightness.",
        ],
        [
          "saturation",
          "Saturation",
          "Uses brush saturation while preserving artwork hue and brightness.",
        ],
        [
          "color",
          "Color",
          "Uses brush hue and saturation while preserving artwork brightness.",
        ],
        [
          "luminosity",
          "Luminosity",
          "Uses brush brightness while preserving artwork color.",
        ],
      ],
    ],
    [
      "Other",
      [
        ["divide", "Divide", "Coming Soon", false],
        ["subtract", "Subtract", "Coming Soon", false],
      ],
    ],
  ];
  const BRUSH_BLEND_MODES = new Set(
    BRUSH_BLEND_MODE_GROUPS.flatMap((g) =>
      g[1].filter((m) => m[3] !== false).map((m) => m[0]),
    ),
  );
  const BRUSH_BLEND_MODE_LABELS = new Map(
    BRUSH_BLEND_MODE_GROUPS.flatMap((g) => g[1].map((m) => [m[0], m[1]])),
  );
  const brushBlendModeEl = document.getElementById("ts-brush-blend-mode");
  const brushBlendModeValueEl = document.getElementById(
    "ts-brush-blend-mode-value",
  );
  const brushBlendModeField = document.getElementById(
    "ts-brush-blend-mode-field",
  );
  let savedBrushBlendMode = "normal";
  try {
    const stored = localStorage.getItem(BRUSH_BLEND_MODE_STORAGE_KEY);
    savedBrushBlendMode = BRUSH_BLEND_MODES.has(stored)
      ? stored
      : localStorage.getItem("brushDrawBehind") === "true"
        ? "draw-behind"
        : "normal";
    localStorage.setItem(BRUSH_BLEND_MODE_STORAGE_KEY, savedBrushBlendMode);
    localStorage.removeItem("brushDrawBehind");
  } catch (_) {}
  window.brushBlendMode = savedBrushBlendMode;
  let blendMenu = null,
    blendOptions = [],
    blendMenuOpen = false;
  function styleLayeringActive() {
    return !!(
      layers[curLayer] &&
  ...
  function updateBlendModeUI() {
    if (styleLayeringActive() && window.brushBlendMode !== "normal") {
      window.brushBlendMode = "normal";
      try {
        localStorage.setItem(BRUSH_BLEND_MODE_STORAGE_KEY, "normal");
      } catch (_) {}
    }
    const mode = BRUSH_BLEND_MODES.has(window.brushBlendMode)
        ? window.brushBlendMode
        : "normal",
      locked = styleLayeringActive();
    if (brushBlendModeValueEl)
      brushBlendModeValueEl.textContent =
        BRUSH_BLEND_MODE_LABELS.get(mode) || "Normal";
    if (brushBlendModeEl) {
      brushBlendModeEl.disabled = locked;
      brushBlendModeEl.setAttribute("aria-disabled", String(locked));
      brushBlendModeEl.title = locked
        ? "Style Layering uses Normal brush compositing. Draw Behind is unavailable."
        : "Controls how brush paint combines with existing artwork.";
    }
    blendOptions.forEach((el) => {
      const on = el.dataset.mode === mode;
      el.classList.toggle("is-selected", on);
      el.setAttribute("aria-selected", String(on));
      el.disabled = locked && el.dataset.mode !== "normal";
    });
  }
  function setBlendMode(mode) {
    window.brushBlendMode = styleLayeringActive()
      ? "normal"
      : BRUSH_BLEND_MODES.has(mode)
        ? mode
        : "normal";
    try {
      localStorage.setItem(BRUSH_BLEND_MODE_STORAGE_KEY, window.brushBlendMode);
    } catch (_) {}
    updateBlendModeUI();
  }
  function buildBlendMenu() {
    if (blendMenu || !brushBlendModeEl) return;
    blendMenu = document.createElement("div");
    blendMenu.id = "ts-brush-blend-mode-menu";
    blendMenu.className = "ts-brush-blend-mode-menu";
    blendMenu.role = "listbox";
    blendMenu.setAttribute("aria-label", "Brush blend mode");
    blendMenu.tabIndex = -1;
    blendMenu.hidden = true;
    BRUSH_BLEND_MODE_GROUPS.forEach(([label, modes]) => {
      const h = document.createElement("div");
      h.className = "ts-brush-blend-mode-group";
      h.textContent = label;
      blendMenu.appendChild(h);
      modes.forEach(([value, name, description, enabled = true]) => {
        const option = document.createElement(enabled ? "button" : "div");
        option.className = "ts-brush-blend-mode-option";
        ...
        if (!enabled) {
          ...
          option.innerHTML =
            '<span></span><span class="ts-brush-blend-mode-status">Coming Soon</span>';
          option.firstElementChild.textContent = name;
          blendMenu.appendChild(option);
          return;
        }
        option.type = "button";
        option.role = "option";
        option.innerHTML =
          '<span></span><span class="ts-brush-blend-mode-check" aria-hidden="true">✓</span>';
        option.firstElementChild.textContent = name;
        option.onclick = () => {
          setBlendMode(value);
          closeBlendMenu(true);
        };
        option.onkeydown = blendOptionKeydown;
        blendOptions.push(option);
        blendMenu.appendChild(option);
      });
    });
    document.body.appendChild(blendMenu);
    brushBlendModeEl.setAttribute("aria-controls", blendMenu.id);
    updateBlendModeUI();
  }
  function positionBlendMenu() {
    if (!blendMenuOpen || !blendMenu) return;
    const r = brushBlendModeEl.getBoundingClientRect();
    if (!r.width || brushBlendModeField?.hidden || tool !== "brush") {
      closeBlendMenu(false);
      return;
    }
    const gap = 8,
      below = innerHeight - r.bottom - gap,
      above = r.top - gap,
      up = below < 280 && above > below;
    blendMenu.style.maxHeight =
      Math.max(120, Math.min(340, (up ? above : below) - 4)) + "px";
    blendMenu.style.width = Math.max(210, r.width) + "px";
    blendMenu.style.left =
      Math.max(
        gap,
        Math.min(r.left, innerWidth - blendMenu.offsetWidth - gap),
      ) + "px";
    blendMenu.style.top =
      (up ? Math.max(gap, r.top - blendMenu.offsetHeight - 4) : r.bottom + 4) +
      "px";
  }
  function openBlendMenu() {
    buildBlendMenu();
    if (
      !blendMenu ||
      brushBlendModeField?.hidden ||
      tool !== "brush" ||
      styleLayeringActive()
    )
      return;
    blendMenuOpen = true;
    blendMenu.hidden = false;
    brushBlendModeEl.setAttribute("aria-expanded", "true");
    positionBlendMenu();
    const selected =
      blendOptions.find((el) => el.dataset.mode === window.brushBlendMode) ||
      blendOptions[0];
    selected?.focus({ preventScroll: true });
    selected?.scrollIntoView({ block: "nearest" });
  }
  function closeBlendMenu(focus) {
    if (!blendMenu) return;
    blendMenuOpen = false;
    blendMenu.hidden = true;
    brushBlendModeEl?.setAttribute("aria-expanded", "false");
    if (focus) brushBlendModeEl?.focus({ preventScroll: true });
  }
  if (window.TransientManager) {
    window.TransientManager.register({
      id: "brush-blend-mode-menu",
      isOpen: () => !!blendMenuOpen,
      close: () => closeBlendMenu(false),
      contains: (target) =>
        !!(
          (blendMenu && blendMenu.contains(target)) ||
          (brushBlendModeEl && brushBlendModeEl.contains(target))
        ),
    });
    window.TransientManager.register({
      id: "eraser-mode-menu",
      isOpen: () => !!eraserModeOpen,
      close: () => closeEraserModeMenu(false),
      contains: (target) =>
        !!(
          (eraserModeMenu && eraserModeMenu.contains(target)) ||
          (eraserModeEl && eraserModeEl.contains(target))
        ),
    });
  }
  function blendOptionKeydown(e) {
    const i = Math.max(0, blendOptions.indexOf(e.currentTarget));
    let next = null;
    if (e.key === "ArrowDown" || e.key === "ArrowUp") {
      e.preventDefault();
      next =
        blendOptions[
          (i + (e.key === "ArrowDown" ? 1 : -1) + blendOptions.length) %
            blendOptions.length
        ];
      ...
      next =
        e.key === "Home"
          ? blendOptions[0]
          : blendOptions[blendOptions.length - 1];
    } else if (e.key === "Escape") {
      e.preventDefault();
      closeBlendMenu(true);
    } else if (e.key === "Tab") closeBlendMenu(false);
    next?.focus({ preventScroll: true });
    next?.scrollIntoView({ block: "nearest" });
  }
  if (brushBlendModeEl) {
    brushBlendModeEl.onclick = () =>
      blendMenuOpen ? closeBlendMenu(false) : openBlendMenu();
    brushBlendModeEl.onkeydown = (e) => {
      if (["ArrowDown", "ArrowUp", "Enter", " "].includes(e.key)) {
        e.preventDefault();
        openBlendMenu();
      } else if (e.key === "Escape") closeBlendMenu(false);
    };
  }
  document.addEventListener(
    ...
    (e) => {
      if (
        blendMenuOpen &&
        !blendMenu?.contains(e.target) &&
        !brushBlendModeEl?.contains(e.target)
      )
        closeBlendMenu(false);
    },
    true,
  ...
  document.addEventListener(
    "scroll",
    () => {
      if (blendMenuOpen) positionBlendMenu();
    },
    true,
  );
  ...
  updateBlendModeUI();
  function syncBrushBlendModeVisibility() {
    if (brushBlendModeField) brushBlendModeField.hidden = tool !== "brush";
    if (tool !== "brush") closeBlendMenu(false);
  }
  window.addEventListener("tool-changed", syncBrushBlendModeVisibility);
  syncBrushBlendModeVisibility();
  window.addEventListener("smart-raster-style-layering-changed", () => {
    closeBlendMenu(false);
    updateBlendModeUI();
  });
  ...
  try {
    savedSmartEraserMode =
      localStorage.getItem(ERASER_MODE_STORAGE_KEY) === "color"
        ? "color"
        : "normal";
  } catch (_) {}
  window.eraserMode = "normal";
  function eraserColorModeAvailable() {
    return !!(layers[curLayer] && layers[curLayer].type === "smart-raster");
  }
  window.eraserColorModeAvailable = eraserColorModeAvailable;
  let eraserModeMenu = null,
    eraserModeOpen = false;
  function updateEraserModeUI() {
    if (eraserModeValueEl)
      eraserModeValueEl.textContent =
        window.eraserMode === "color" ? "Color" : "Normal";
  }
  function setEraserMode(mode) {
    if (!eraserColorModeAvailable()) mode = "normal";
    window.eraserMode = mode === "color" ? "color" : "normal";
    if (eraserColorModeAvailable()) {
      savedSmartEraserMode = window.eraserMode;
      try {
        localStorage.setItem(ERASER_MODE_STORAGE_KEY, savedSmartEraserMode);
      } catch (_) {}
    }
    updateEraserModeUI();
  }
  function closeEraserModeMenu(focus) {
    if (!eraserModeMenu) return;
    eraserModeOpen = false;
    eraserModeMenu.hidden = true;
    eraserModeEl?.setAttribute("aria-expanded", "false");
    if (focus) eraserModeEl?.focus({ preventScroll: true });
  }
  function positionEraserModeMenu() {
    if (!eraserModeOpen || !eraserModeMenu || !eraserModeEl) return;
    const r = eraserModeEl.getBoundingClientRect(),
      gap = 8;
    eraserModeMenu.style.width = r.width + "px";
    eraserModeMenu.style.left =
      Math.max(gap, Math.min(r.left, innerWidth - r.width - gap)) + "px";
    eraserModeMenu.style.top = r.bottom + 4 + "px";
  }
  function buildEraserModeMenu() {
    if (eraserModeMenu || !eraserModeEl) return;
    eraserModeMenu = document.createElement("div");
    eraserModeMenu.id = "ts-eraser-mode-menu";
    eraserModeMenu.className = "ts-brush-blend-mode-menu";
    eraserModeMenu.role = "listbox";
    eraserModeMenu.setAttribute("aria-label", "Eraser mode");
    eraserModeMenu.hidden = true;
    [
      ["normal", "Normal"],
      ["color", "Color"],
      ...
      option.onclick = () => {
        setEraserMode(value);
        eraserModeMenu
          .querySelectorAll(".ts-brush-blend-mode-option")
          .forEach((item) => {
            const selected = item.dataset.mode === window.eraserMode;
            item.classList.toggle("is-selected", selected);
            item.setAttribute("aria-selected", String(selected));
          });
        closeEraserModeMenu(true);
      };
      eraserModeMenu.appendChild(option);
    });
    document.body.appendChild(eraserModeMenu);
    eraserModeEl.setAttribute("aria-controls", eraserModeMenu.id);
  }
  function openEraserModeMenu() {
    if (
      tool !== "eraser" ||
      eraserModeField?.hidden ||
      !eraserColorModeAvailable()
    )
      return;
    buildEraserModeMenu();
    eraserModeOpen = true;
    eraserModeMenu.hidden = false;
    eraserModeEl.setAttribute("aria-expanded", "true");
    eraserModeMenu
      .querySelectorAll(".ts-brush-blend-mode-option")
      .forEach((item) => {
        const selected = item.dataset.mode === window.eraserMode;
    ...
    positionEraserModeMenu();
  }
  if (eraserModeEl) {
    eraserModeEl.onclick = () =>
      eraserModeOpen ? closeEraserModeMenu(false) : openEraserModeMenu();
    eraserModeEl.onkeydown = (event) => {
      if (["ArrowDown", "ArrowUp", "Enter", " "].includes(event.key)) {
        event.preventDefault();
        openEraserModeMenu();
        eraserModeMenu?.querySelector(".is-selected")?.focus();
      } else if (event.key === "Escape") closeEraserModeMenu(false);
    };
  }
  document.addEventListener(
    "pointerdown",
    (event) => {
      if (
        eraserModeOpen &&
        !eraserModeMenu?.contains(event.target) &&
        !eraserModeEl?.contains(event.target)
      )
        closeEraserModeMenu(false);
    },
    true,
  );
  function syncEraserModeVisibility() {
    ...
    if (eraserModeEl) {
      eraserModeEl.disabled = !available;
      eraserModeEl.setAttribute("aria-disabled", String(!available));
    }
    if (!available) {
      window.eraserMode = "normal";
      closeEraserModeMenu(false);
    } else if (tool === "eraser") window.eraserMode = savedSmartEraserMode;
    if (tool !== "eraser") closeEraserModeMenu(false);
    updateEraserModeUI();
  }
  window.addEventListener("tool-changed", syncEraserModeVisibility);
  window.addEventListener("active-layer-changed", syncEraserModeVisibility);
  window.addEventListener("active-artwork-changed", syncEraserModeVisibility);
  window.addEventListener("resize", positionEraserModeMenu);
  syncEraserModeVisibility();
  ...
  bindRange("ts-airbrush-rate", "ts-airbrush-rate-val", "", (v) => {
    window._tsAirbrushRate = v / 100;
  });
  // Dynamics
  bindRange("ts-min-size", "ts-min-size-val", "%");
  bindRange("ts-start-taper", "ts-start-taper-val", "%", (v) => {
    if (
      typeof captureLiveState === "function" &&
      (tool === "brush" || tool === "eraser")
    )
      captureLiveState(tool);
  });
  ...
  function syncTaperMode() {
    ...
    ["ts-start-taper", "ts-end-taper"].forEach((id) => {
  ...
  syncTaperMode();
  bindRange("ts-angle", "ts-angle-val", "\u00B0", (v) => {
    window._tsBrushAngle = v;
  ...
  function syncRotation() {
    window._tsRotationMode = rotationModeEl
      ? rotationModeEl.value
      : "fixed-rotation";
    window._tsBrushAngle = angleEl ? +angleEl.value : 0;
    if (angleEl) angleEl.disabled = window._tsRotationMode !== "fixed-rotation";
  }
  if (rotationModeEl) {
    rotationModeEl.addEventListener("input", syncRotation);
    rotationModeEl.addEventListener("change", syncRotation);
  }
  syncRotation();
  ...
  function syncTipShape() {
    ...
    if (typeof window._setBrushTipShape === "function")
      window._setBrushTipShape(
        roundness,
        tipFlipXEl?.checked,
        tipFlipYEl?.checked,
      );
  ...
  bindRange("ts-size-jitter", "ts-size-jitter-val", "%", (v) => {
    window.brushTipSizeJitter = v / 100;
  });
  bindRange("ts-angle-jitter", "ts-angle-jitter-val", "%", (v) => {
    window.brushTipAngleJitter = v / 100;
  });
  bindRange("ts-round-jitter", "ts-round-jitter-val", "%", (v) => {
    window.brushTipRoundnessJitter = v / 100;
    _stampCache.clear();
  });
  bindRange("ts-tip-min-roundness", "ts-tip-min-roundness-val", "%", (v) => {
    window.brushTipMinimumRoundness = v / 100;
    _tipDabCache.clear();
    _stampCache.clear();
  });
  bindRange("ts-scatter-amount", "ts-scatter-amount-val", "%", (v) => {
    window._tsScatterAmount = v / 100;
  });
  bindRange("ts-scatter-count", "ts-scatter-count-val", "", (v) => {
    window._tsScatterCount = Math.min(50, Math.max(1, Math.round(v)));
  });
  ...
  function syncScatter() {
    ...
    window._tsScatterAmount = scatterAmountEl
      ? +scatterAmountEl.value / 100
      : 0;
    window._tsScatterCount = scatterCountEl
      ? Math.min(50, Math.max(1, Math.round(+scatterCountEl.value)))
      : 1;
  ...
  bindRange("ts-stabilization", "ts-stabilization-val", "", (v) => {
    window._tsStabilization = v / 100;
  });
  ...
  if (leashEl)
    leashEl.addEventListener("input", () => {
      window._tsLeashEnabled = leashEl.checked;
    });
  ...
  function syncDynamicsMinimums() {
    ...
    if (minSizeRow)
      minSizeRow.style.display =
        sizeControl && sizeControl.value === "pressure" ? "" : "none";
    if (minFlowRow)
      minFlowRow.style.display =
        flowControl && flowControl.value === "pressure" ? "" : "none";
  }
  const sizeControl = document.getElementById("ts-size-control");
  const flowControl = document.getElementById("ts-flow-control");
  ...
  let _lastEnabledAAMode =
    window.brushAAMode && window.brushAAMode !== "none"
      ? window.brushAAMode
      : "medium";
  function _syncAAModeUI() {
    ...
    if (advSelect) {
      advSelect.value = _lastEnabledAAMode;
      advSelect.disabled = !brushAA;
    }
  }
  function _setBrushAA(on) {
    ...
    if (changed && !nextAA) {
      _preAA = {
        opacityCtrl: ctrlEl ? ctrlEl.value : "pressure",
        hardness: brushHardness,
      };
      if (ctrlEl) ctrlEl.value = "off";
      brushHardness = 1;
      const hEl = document.getElementById("ts-hardness");
      if (hEl) hEl.value = 100;
      const hVal = document.getElementById("ts-hardness-val");
      ...
      const hEl = document.getElementById("ts-hardness");
      if (hEl) hEl.value = hardnessPercent;
      const hVal = document.getElementById("ts-hardness-val");
      if (hVal) hVal.textContent = hardnessPercent;
      _preAA = null;
    }
    brushAA = nextAA;
  ...
  function _setBrushAAMode(mode, forceEnable = true) {
    const m =
      mode === "weak" || mode === "medium" || mode === "strong"
        ? mode
        : "medium";
    _lastEnabledAAMode = m;
    window.brushAAMode = m;
    if (forceEnable && !brushAA) _setBrushAA(true);
    else {
      _aaDabCache.clear();
      _stampCache.clear();
      _tipDabCache.clear();
  ...
  window._tsAirbrushRate =
    window._tsAirbrushRate !== undefined ? window._tsAirbrushRate : 0.55;
  window._brushContinuousSpraying = window._brushContinuousSpraying === true;
  function _setAirbrush(on) {
    window._brushAirbrush = !!on;
    const cb = document.getElementById("ts-airbrush");
    if (cb) cb.checked = window._brushAirbrush;
    if (
      !window._brushAirbrush &&
      typeof window._stopAirbrushSpray === "function"
    )
      window._stopAirbrushSpray();
  }
  ...
  if (tsAirbrush) {
    ...
    tsAirbrush.onchange = () => {
      _setAirbrush(tsAirbrush.checked);
    };
  }
  ...
  (function initPsNav() {
    ...
    function activatePanel(panelId) {
      ...
      try {
        sessionStorage.setItem("tsPsPanel", panelId);
      } catch (e) {}
    }
    ...
    try {
      lastPanel = sessionStorage.getItem("tsPsPanel") || "tip-image";
    } catch (e) {}
  ...
  (function initPsCheckGating() {
    ...
    function panelFor(id) {
      return document.querySelector('.ts-ps-panel[data-panel="' + id + '"]');
    }
    ...
    function setGateState(id, on) {
      ...
      if (!on) {
        ...
        selects.forEach((sel) => {
          if (_preGate[id][sel.id] !== undefined)
            sel.value = _preGate[id][sel.id];
        });
        delete _preGate[id];
      }
    }
  ...
  (function initSimpleVisibilityEyes() {
    ...
    let vis = {};
    try {
      vis = JSON.parse(localStorage.getItem(EYE_KEY) || "{}");
    } catch (e) {
      vis = {};
    }
    if (Object.prototype.hasOwnProperty.call(vis, "density")) {
      delete vis.density;
      try {
        localStorage.setItem(EYE_KEY, JSON.stringify(vis));
      } catch (e) {}
    }

    function isVisible(key) {
      return vis[key] !== undefined
        ? vis[key]
    ...
    function applyField(key) {
      const fields = document.querySelectorAll(
        '.ts-field[data-eye="' + key + '"]',
      );
      const buttons = document.querySelectorAll(
        '.ts-eye-btn[data-eye-btn="' + key + '"]',
      );
      const on = isVisible(key);
      fields.forEach((field) =>
        field.classList.toggle("ts-simple-hidden", !on),
      );
      buttons.forEach((btn) => {
        btn.classList.toggle("ts-eye-off", !on);
        btn.setAttribute("aria-pressed", String(on));
        btn.title = on
          ? "Visible in Simple tab — click to hide"
          : "Hidden in Simple tab — click to show";
      });
    }
    function applyAllFields() {
      const keys = new Set([...Object.keys(DEFAULTS), ...Object.keys(vis)]);
      document
        .querySelectorAll("[data-eye],[data-eye-btn]")
        .forEach((element) =>
          keys.add(element.dataset.eye || element.dataset.eyeBtn),
        );
      keys.forEach((key) => {
        if (key) applyField(key);
      });
    }
    ...
    if (dockedFields)
      new MutationObserver(applyAllFields).observe(dockedFields, {
        childList: true,
      });
    ...
    document.addEventListener("click", (e) => {
      const btn =
        e.target.closest && e.target.closest(".ts-eye-btn[data-eye-btn]");
      if (!btn) return;
      e.preventDefault();
      e.stopPropagation();
  ...
  (function initSimpleSectionEyes() {
    ...
    let vis = {};
    try {
      vis = JSON.parse(localStorage.getItem(SECTION_EYE_KEY) || "{}");
    } catch (e) {
      vis = {};
    }
    ...
    try {
      const saved = JSON.parse(
        localStorage.getItem(SECTION_ORDER_KEY) || "null",
      );
      if (Array.isArray(saved) && saved.length === ALL_SECTIONS.length)
        order = saved;
    } catch (e) {}
    ...
    function applySection(key) {
      const panel = document.querySelector(
        '.ts-ps-panel[data-panel="' + key + '"]',
      );
      const btn = document.querySelector(
        '.ts-eye-btn[data-eye-section="' + key + '"]',
      );
      const on = isVisible(key);
      ...
      if (btn) {
        btn.classList.toggle("ts-eye-off", !on);
        btn.title = on
          ? "Shown in Simple tab — click to hide"
          : "Hidden from Simple tab — click to show";
      }
    }
    ...
    function applyOrder() {
      ...
      const basicPanel = container.querySelector(
        '.ts-ps-panel[data-panel="basic"]',
      );
      const importPanel = container.querySelector(
        '.ts-ps-panel[data-panel="import"]',
      );
    ...
    document.addEventListener("click", (e) => {
      const btn =
        e.target.closest && e.target.closest(".ts-eye-btn[data-eye-section]");
      if (!btn) return;
      e.preventDefault();
      e.stopPropagation();
      ...
      vis[key] = !isVisible(key);
      try {
        localStorage.setItem(SECTION_EYE_KEY, JSON.stringify(vis));
      } catch (err) {}
      applySection(key);
    });
    ...
    document.addEventListener("pointerdown", (e) => {
      const handle =
        e.target.closest && e.target.closest(".ts-section-draghandle");
      if (!handle) return;
    ...
    function onSectionDragMove(e) {
      ...
      const panels = Array.from(
        document.querySelectorAll(".ts-ps-panels .ts-ps-panel[data-panel]"),
      ).filter(
        (p) =>
          p.dataset.panel !== "basic" &&
          p.dataset.panel !== "import" &&
          p !== dragEl,
      );
      ...
      if (target) {
        target.classList.add(
          above ? "ts-section-drop-above" : "ts-section-drop-below",
        );
      }
    }
    ...
    function onSectionDragUp(e) {
      ...
      let targetKey = null,
        above = true;
      panels.forEach((p) => {
        if (p.classList.contains("ts-section-drop-above")) {
          targetKey = p.dataset.panel;
          above = true;
        }
        if (p.classList.contains("ts-section-drop-below")) {
          targetKey = p.dataset.panel;
          above = false;
        }
        p.classList.remove("ts-section-drop-above", "ts-section-drop-below");
  ...
  if (curveCanvas) {
    const curves = (typeof window !== "undefined" &&
      window.PRESSURE_CURVES) || {
      linear: [
        [0, 1],
        [1, 0],
      ],
    ...
    let activeCurve =
      (typeof window !== "undefined" && window._tsPressureCurve) || "linear";
    ...
    ["linear", "soft", "hard", "s"].forEach((k) => {
      ...
      btn.onclick = () => {
        ...
        document
          .querySelectorAll("#tool-settings-body .ts-pill")
  ...
  const dockedSimpleSettings = document.getElementById(
    "brush-tool-settings-content",
  );
  const basicSettings = document.querySelector(
    '#tool-settings-body .ts-ps-panel[data-panel="basic"] .ts-section-body',
  );
  if (dockedSimpleSettings && basicSettings) {
    ...
    if (typeof window._applySimpleToolVisibility === "function")
      window._applySimpleToolVisibility();
  }
  ...
  if (tsBtnOpen)
    tsBtnOpen.onclick = () => {
      setTsMode("advanced");
      if (typeof window._syncAdvancedBrushSettings === "function")
        window._syncAdvancedBrushSettings();
      if (typeof window._applySimpleToolVisibility === "function")
        window._applySimpleToolVisibility();
      document
        .getElementById("tool-settings-modal-overlay")
  ...
  let tsMode = "simple";
  try {
    tsMode = localStorage.getItem(TS_MODE_KEY) || "simple";
  } catch (e) {}
  function applyTsMode() {
    ...
    const sBtn = document.getElementById("ts-mode-simple"),
      aBtn = document.getElementById("ts-mode-advanced");
    ...
    if (
      tsMode === "advanced" &&
      typeof window._tsPsActivateDefault === "function"
    )
      window._tsPsActivateDefault();
  }
  function setTsMode(m) {
    tsMode = m;
    try {
      localStorage.setItem(TS_MODE_KEY, tsMode);
    } catch (e) {}
    applyTsMode();
  }
  document
    .getElementById("ts-mode-simple")
    ?.addEventListener("click", () => setTsMode("simple"));
  document
    .getElementById("ts-mode-advanced")
    ?.addEventListener("click", () => setTsMode("advanced"));
  applyTsMode();

  (function initAdvancedMainControlMirrors() {
    const mirrorSyncers = [];
    document
      .querySelectorAll(".ts-advanced-mirror-row[data-mirror-target]")
      .forEach((row) => {
        const source = document.getElementById(row.dataset.mirrorTarget);
        const mirror = row.querySelector(
    ...
    window._syncAdvancedBrushSettings = () =>
      mirrorSyncers.forEach((sync) => sync());
  })();
  ...
  (function initSpacingMode() {
    ...
    if (!mode) return;
    const sync = () => {
      if (!["fixed", "velocity", "pen-pressure"].includes(mode.value))
        mode.value = "fixed";
      const manualAvailable =
        mode.value === "fixed" || mode.value === "velocity";
      const simpleRow = document.getElementById("ts-spacing-manual-row");
      const advancedRow = document.getElementById(
        "ts-advanced-spacing-manual-row",
      );
      if (simpleRow) {
        simpleRow.style.display = manualAvailable ? "" : "none";
        simpleRow.title = "";
      }
      if (advancedRow) {
        advancedRow.style.display = manualAvailable ? "" : "none";
        advancedRow.title = "";
      }
      const spacing = document.getElementById("ts-spacing");
      if (spacing) {
        spacing.disabled = !manualAvailable;
        spacing.title = "";
  ...
  (function initEditableToolValues() {
    document.querySelectorAll(".ts-row").forEach((row) => {
      ...
      if (!slider || !display || display.classList.contains("size-val-edit"))
        return;
    ...
    document.addEventListener("click", (event) => {
      const display =
        event.target.closest && event.target.closest(".ts-value-edit");
      if (display) beginEdit(display);
    });
    document.addEventListener("keydown", (event) => {
      if (
        (event.key === "Enter" || event.key === " ") &&
        event.target.classList?.contains("ts-value-edit")
      ) {
        event.preventDefault();
  ...
  (function initPressureCurveEditors() {
    ...
    const presets = window.PRESSURE_CURVES || {
      linear: [
        [0, 1],
        [1, 0],
      ],
    ...
    function draw(canvas, curve, handles) {
      ...
      for (let i = 0; i <= 64; i++) {
        const x = i / 64,
          y = _evalPressureCurveYFromPoints(curve, x);
    ...
    function _evalPressureCurveYFromPoints(curve, x) {
      ...
      for (let i = 0; i < 20; i++) {
        const mid = (lo + hi) / 2;
        if (_bezierPointAt(curve, mid)[0] < x) lo = mid;
        else hi = mid;
      }
      return _bezierPointAt(curve, (lo + hi) / 2)[1];
    }
    function refreshPreviews() {
      document
        .querySelectorAll(".ts-pressure-preview")
        .forEach((canvas) =>
          draw(canvas, curvePoints(canvas.dataset.pressureSetting), false),
        );
    }
    function openEditor(setting, anchor) {
      activeSetting = setting;
      points = curvePoints(setting);
      popup.classList.add("open");
      popup.setAttribute("aria-hidden", "false");
      const rect = anchor.getBoundingClientRect();
      popup.style.left =
        Math.min(window.innerWidth - popup.offsetWidth - 8, rect.right + 8) +
        "px";
      popup.style.top =
        Math.max(
          8,
          Math.min(window.innerHeight - popup.offsetHeight - 8, rect.top),
        ) + "px";
      draw(editor, points, true);
    }
    ...
    window._closePressureCurveEditor = closeEditor;
    document
      .querySelectorAll(".ts-pressure-preview")
      .forEach((canvas) =>
        canvas.addEventListener("click", () =>
          openEditor(canvas.dataset.pressureSetting, canvas),
        ),
      );
    document.querySelectorAll('[id$="-pressure-curve"]').forEach((select) =>
      select.addEventListener("input", () => {
        const setting = select.id
          .replace("ts-", "")
          .replace("-pressure-curve", "");
        if (select.value !== "custom")
          delete window._tsCustomPressureCurves[setting];
        refreshPreviews();
      }),
    );
    editor.addEventListener("pointerdown", (event) => {
      const rect = editor.getBoundingClientRect(),
        pad = 12,
        x = (event.clientX - rect.left - pad) / (rect.width - pad * 2),
        y = (event.clientY - rect.top - pad) / (rect.height - pad * 2);
      let best = Infinity;
    ...
    editor.addEventListener("pointermove", (event) => {
      ...
      let x = Math.max(
          ...
          Math.min(
            1,
            (event.clientX - rect.left - pad) / (rect.width - pad * 2),
          ),
        ),
        y = Math.max(
          0,
          Math.min(
            1,
            (event.clientY - rect.top - pad) / (rect.height - pad * 2),
          ),
        );
      if (dragIndex === 0) x = 0;
      if (dragIndex === points.length - 1) x = 1;
      if (dragIndex > 0) x = Math.max(points[dragIndex - 1][0] + 0.01, x);
      if (dragIndex < points.length - 1)
        x = Math.min(points[dragIndex + 1][0] - 0.01, x);
      points[dragIndex] = [x, y];
      window._tsCustomPressureCurves[activeSetting] = points.map((point) =>
        point.slice(),
      );
      const select = document.getElementById(
        "ts-" + activeSetting + "-pressure-curve",
      );
      if (select) {
        select.value = "custom";
    ...
    document
      .getElementById("ts-pressure-reset")
      .addEventListener("click", () => {
        ...
        if (select && select.value === "custom") {
          select.value = "linear";
          delete window._tsCustomPressureCurves[activeSetting];
          select.dispatchEvent(new Event("input", { bubbles: true }));
        }
        points = curvePoints(activeSetting);
        draw(editor, points, true);
        refreshPreviews();
      });
    document
      .getElementById("ts-pressure-done")
  ...
  (function initSimpleSettingsPopup() {
    ...
    const configs = {
      size: {
        title: "Size Settings",
        controls: [
          ["Control", "ts-size-control"],
          ["Minimum Size", "ts-min-size"],
          ["Pressure Curve", "ts-size-pressure-curve"],
        ],
      },
      flow: {
        title: "Flow Settings",
        controls: [
          ["Control", "ts-flow-control"],
          ["Minimum Flow", "ts-min-flow"],
          ["Pressure Curve", "ts-flow-pressure-curve"],
        ],
      },
      opacity: {
        title: "Opacity Settings",
        controls: [
          ["Control", "ts-opacity-control"],
          ["Pressure Curve", "ts-opacity-pressure-curve"],
        ],
      },
      stabilization: {
        title: "Stabilization Settings",
        controls: [["Leash", "ts-leash-toggle"]],
      },
    };
    function closePopup() {
      popup.classList.remove("open");
      popup.setAttribute("aria-hidden", "true");
      document
        .querySelectorAll(".ts-simple-settings-btn.active")
        .forEach((btn) => btn.classList.remove("active"));
    }
    window.closeSimpleSettingsPopup = closePopup;
    if (window.TransientManager) {
      window.TransientManager.register({
        ...
        contains: (target) =>
          !!(
            ...
            (target &&
              target.closest &&
              target.closest(".ts-simple-settings-btn")) ||
            (document.getElementById("ts-pressure-editor-popup") &&
              document
                .getElementById("ts-pressure-editor-popup")
                .contains(target))
          ),
      });
      window.TransientManager.register({
        id: "pressure-curve-editor",
        isOpen: () =>
          !!document
            .getElementById("ts-pressure-editor-popup")
            ?.classList.contains("open"),
        close: () => window._closePressureCurveEditor?.(),
        contains: (target) =>
          !!(
            (document.getElementById("ts-pressure-editor-popup") &&
              document
                .getElementById("ts-pressure-editor-popup")
                .contains(target)) ||
            (target && target.closest && target.closest(".ts-pressure-preview"))
          ),
      });
    }
    function addControl(labelText, sourceId) {
      ...
      if (sourceId.endsWith("-pressure-curve")) {
        ...
        preview.className = "ts-pressure-preview";
        preview.width = 92;
        preview.height = 38;
        preview.dataset.pressureSetting = sourceId
          .replace("ts-", "")
          .replace("-pressure-curve", "");
        preview.title = "Click to edit pressure curve";
        preview.addEventListener("click", () =>
          window._openPressureCurveEditor?.(
            preview.dataset.pressureSetting,
            preview,
          ),
        );
        row.appendChild(preview);
        setTimeout(() => window._refreshPressureCurvePreviews?.(), 0);
      }
    ...
    function openPopup(button, key) {
      ...
      title.className = "ts-simple-popup-title";
      title.textContent = config
        ? config.title
        : key[0].toUpperCase() + key.slice(1) + " Settings";
      popup.appendChild(title);
      if (config)
        config.controls.forEach((control) =>
          addControl(control[0], control[1]),
        );
      else {
        const empty = document.createElement("div");
        empty.className = "ts-simple-popup-empty";
        empty.textContent = "No extra settings yet.";
      ...
      popup.style.top =
        Math.max(
          ...
          Math.min(
            ...
            modalOpen ? rect.top + 52 : rect.top,
          ),
        ) + "px";
    }
    document.addEventListener("click", (event) => {
      const button =
        event.target.closest && event.target.closest(".ts-simple-settings-btn");
      if (button) {
        event.preventDefault();
        event.stopPropagation();
        if (button.classList.contains("active")) closePopup();
        else openPopup(button, button.dataset.simpleSettings);
        return;
      }
    ...
    document
      .getElementById("tool-settings-modal-close")
      ?.addEventListener("click", closePopup);
  })();
  ...
  if (importBtn)
    importBtn.onclick = () => {
      const fi = document.createElement("input");
      ...
      fi.accept = ".json,.abr,.sut,.kpp,.tpl";
      fi.onchange = () => {
        const f = fi.files[0];
        if (!f) return;
          ...
          r.onload = (ev) => {
            try {
              ...
              if (!brushes || !brushes.length) {
                showInfo(
                  "No readable brush tips found in this ABR file. Only sampled-image tips are supported currently.",
                  "ABR Import",
                );
                return;
              }
              showABRImportResults(brushes, f.name, () =>
                showABRPicker(brushes, f.name),
              );
            } catch (e) {
              showInfo(
                "Could not parse the ABR file: " + e.message,
                "ABR Import Error",
              );
            }
          };
        ...
        r.onload = (ev) => {
          try {
            ...
            showInfo(
              "Could not parse preset. Make sure it is a JSON export from this app, or use the Paste JSON option for converted presets.",
              "Import Error",
            );
          }
        };
  ...
  if (pasteBtn)
    pasteBtn.onclick = async () => {
      try {
        ...
        showInfo(
          "Could not read clipboard or parse JSON. Copy a valid JSON preset first.",
          "Paste Error",
        );
      }
    };
  if (exportBtn)
    exportBtn.onclick = () => {
      ...
      a.download = "brush-preset.json";
      a.click();
    };

  // Flush display values on load
  document.getElementById("ts-opacity-val").textContent =
    document.getElementById("ts-opacity").value;
  document.getElementById("ts-flow-val").textContent =
    document.getElementById("ts-flow").value;
  document.getElementById("ts-hardness-val").textContent =
    document.getElementById("ts-hardness").value;
  document.getElementById("ts-spacing-val").textContent =
    document.getElementById("ts-spacing").value + "%";
  ...
  (function () {
    ...
    function _drawTipPreview() {
      ...
      if (!window.brushTipCanvas) {
        ...
        ctx2.beginPath();
        ctx2.arc(32, 32, 26, 0, Math.PI * 2);
        ctx2.fill();
        ctx2.globalAlpha = 1;
        ctx2.strokeStyle = "var(--border2)";
        ctx2.lineWidth = 1;
        ctx2.beginPath();
        ctx2.arc(32, 32, 26, 0, Math.PI * 2);
        ctx2.stroke();
      ...
      const shapedW = tipW * (compressWidth ? roundness : 1),
        shapedH = tipH * (compressWidth ? 1 : roundness);
      const rotatedW =
        Math.abs(shapedW * Math.cos(rotation)) +
        Math.abs(shapedH * Math.sin(rotation));
      const rotatedH =
        Math.abs(shapedW * Math.sin(rotation)) +
        Math.abs(shapedH * Math.cos(rotation));
      const scale = Math.min(
        56 / Math.max(1, rotatedW),
        56 / Math.max(1, rotatedH),
      );
      ...
      img.onload = () => {
        ...
        c.getContext(
          ...
          window.TipReadbackExperiment
            ? window.TipReadbackExperiment.contextOptions()
            : undefined,
        ).drawImage(img, 0, 0);
        if (typeof window.setBrushTip === "function")
          window.setBrushTip(c, null, "manual-tip-upload");
        ...
        window._syncTipUI();
        if (typeof window._captureActiveBrushPreset === "function")
          window._captureActiveBrushPreset(true);
      };
      img.onerror = () => {
        URL.revokeObjectURL(url);
      };
  ...
  (function () {
    ...
    window._syncTextureUI = function () {
      ...
      const brightN =
        window.brushTextureBrightness != null
      ...
      const contrastN =
        window.brushTextureContrast != null ? window.brushTextureContrast : 0;
      ...
      const pct = Math.round(
        (window.brushTextureStrength != null
      ...
      const buildupPct = Math.round(
        (window.brushTextureBuildup != null
          ? window.brushTextureBuildup
          : 1.0) * 100,
      );
    ...
    function _loadTextureFromFile(file) {
      ...
      img.onload = () => {
        ...
        if (typeof window.setBrushTexture === "function")
          window.setBrushTexture(c);
    ...
    if (texClearBtn) {
      texClearBtn.onclick = () => {
        if (typeof window.clearBrushTexture === "function")
          window.clearBrushTexture();
        const fn = document.getElementById("ts-texture-filename");
        if (fn) {
          fn.textContent = "";
          fn.style.display = "none";
    ...
    if (texInvertEl) {
      texInvertEl.onchange = () => {
        ...
        if (typeof window._invalidateTextureCache === "function")
          window._invalidateTextureCache();
        if (typeof window._captureActiveBrushPreset === "function")
          window._captureActiveBrushPreset(false);
      };
    }
    ...
    (function () {
      const texPanel = document.querySelector(
        '.ts-ps-panel[data-panel="texture"]',
      );
...
function getToolPreset(options) {
  ...
  const ids = [
    ...
    "ts-end-taper",
    "ts-size-control",
    "ts-size-pressure-curve",
    "ts-flow-control",
    "ts-flow-pressure-curve",
    "ts-opacity-control",
    "ts-opacity-pressure-curve",
    "ts-min-flow",
    "ts-texture-scale",
    "ts-texture-strength",
    "ts-texture-buildup",
    "ts-texture-brightness",
    "ts-texture-contrast",
    "ts-texture-invert",
    "ts-texture-each",
    "ts-texture-mode",
  ...
  out["ts-aa-mode"] =
    window.brushAAMode && window.brushAAMode !== "none"
      ? window.brushAAMode
      : "medium";
  out["ts-pressure-curves"] = JSON.parse(
    JSON.stringify(window._tsCustomPressureCurves || {}),
  );
  ...
  if (includeImages && window.brushTipCanvas) {
    try {
      out["ts-tip-dataurl"] = window.brushTipCanvas.toDataURL("image/png");
    } catch (e) {}
    ...
    if (
      Number.isFinite(Number(window.brushTipReferenceDiameter)) &&
      Number(window.brushTipReferenceDiameter) > 0
    )
      out["ts-tip-reference-diameter"] = Number(
        window.brushTipReferenceDiameter,
      );
    out["ts-tip-roundness"] = Math.round(
      (window.brushTipRoundness == null ? 1 : window.brushTipRoundness) * 100,
    );
    out["ts-tip-min-roundness"] = Math.round(
      (window.brushTipMinimumRoundness || 0) * 100,
    );
  ...
  if (includeImages && window.brushTextureCanvas) {
    try {
      out["ts-texture-dataurl"] =
        window.brushTextureCanvas.toDataURL("image/png");
    } catch (e) {}
    out["ts-texture-strength"] = Math.round(
      (window.brushTextureStrength != null
        ? window.brushTextureStrength
        : 1.0) * 100,
    );
    out["ts-texture-buildup-custom"] = Math.round(
      (window.brushTextureBuildup != null ? window.brushTextureBuildup : 1.0) *
        100,
    );
    out["ts-texture-scale"] = Math.round(
      (window.brushTextureScale || 1.0) * 100,
    );
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      const context = c.getContext(
        "2d",
        brushTipReadback && window.TipReadbackExperiment
          ? window.TipReadbackExperiment.contextOptions()
          : undefined,
      );
      context.drawImage(img, 0, 0);
      if (brushTipReadback && window.TipReadbackExperiment)
        window.TipReadbackExperiment.record("tip-canvas-created", {
          width: c.width,
          height: c.height,
          duration: performance.now() - started,
          requestedWillReadFrequently:
            window.TipReadbackExperiment.mode === "A",
          actualWillReadFrequently:
            typeof context.getContextAttributes === "function" &&
            context.getContextAttributes().willReadFrequently,
        });
      resolve(c);
    };
...
function applyToolPreset(json) {
  Object.entries(json).forEach(([id, val]) => {
    ...
    if (
      id === "ts-tip-dataurl" ||
      id === "ts-texture-dataurl" ||
      id === "ts-tip-mode" ||
      id === "ts-tip-soft-alpha" ||
      id === "ts-tip-reference-diameter" ||
      id === "ts-tip-roundness-dynamics" ||
      id === "ts-texture-strength" ||
      id === "ts-texture-buildup-custom" ||
      id === "ts-pressure-curves"
    )
      return;
    const el = document.getElementById(id);
    if (!el) return;
  ...
  window._tsCustomPressureCurves = JSON.parse(
    JSON.stringify(json["ts-pressure-curves"] || {}),
  );
  if (typeof window._refreshPressureCurvePreviews === "function")
    window._refreshPressureCurvePreviews();

  //  Restore tip image
  window._tsScatterBothAxes = json["ts-scatter-both-axes"] !== false;
  window.brushTipRoundness = Math.max(
    0.01,
    Math.min(1, (json["ts-tip-roundness"] ?? 100) / 100),
  );
  window.brushTipMinimumRoundness = Math.max(
    0,
    Math.min(1, (json["ts-tip-min-roundness"] ?? 0) / 100),
  );
  ...
  if (json["ts-tip-dataurl"]) {
    _dataURLToCanvas(json["ts-tip-dataurl"], true)
      .then((c) => {
        if (window._brushTipLoadGeneration !== tipLoadGeneration) return;
        if (typeof window.setBrushTip === "function")
          window.setBrushTip(
            c,
            json["ts-tip-reference-diameter"],
            "json-tip-import",
          );
        if (json["ts-tip-mode"]) window.brushTipMode = json["ts-tip-mode"];
        if (json["ts-tip-soft-alpha"] !== undefined)
          window.brushTipSoftAlpha = !!json["ts-tip-soft-alpha"];
  ...
  if (json["ts-texture-dataurl"]) {
    _dataURLToCanvas(json["ts-texture-dataurl"]).then(
      (c) => {
        if (window._brushTextureLoadGeneration !== textureLoadGeneration)
          return;
        if (typeof window.setBrushTexture === "function")
          window.setBrushTexture(c);
        const strengthValue =
          json["ts-texture-strength"] ??
          json["ts-texture-depth-custom"] ??
          json["ts-texture-depth"];
        window.brushTextureStrength =
          strengthValue != null ? +strengthValue / 100 : 1.0;
        window.brushTextureBuildup =
          json["ts-texture-buildup-custom"] != null
            ? +json["ts-texture-buildup-custom"] / 100
            : 1.0;
        window.brushTextureScale =
          json["ts-texture-scale"] != null
            ? +json["ts-texture-scale"] / 100
            : 1.0;
        window.brushTextureInvert = !!json["ts-texture-invert"];
        window.brushTextureBrightness =
          json["ts-texture-brightness"] != null
            ? +json["ts-texture-brightness"]
            : 0;
        window.brushTextureContrast =
          json["ts-texture-contrast"] != null
            ? +json["ts-texture-contrast"]
            : 0;
        if (typeof window._invalidateTextureCache === "function")
          window._invalidateTextureCache();
        if (typeof window._syncTextureUI === "function")
          window._syncTextureUI();
      },
      () => {
        if (window._brushTextureLoadGeneration !== textureLoadGeneration)
...
(function () {
  ...
  function normalizeTextureSettings(settings) {
    ...
    if (
      normalized["ts-texture-strength"] == null &&
      normalized["ts-texture-depth-custom"] != null
    ) {
      normalized["ts-texture-strength"] = normalized["ts-texture-depth-custom"];
    }
    if (
      normalized["ts-texture-strength"] == null &&
      normalized["ts-texture-depth"] != null
    ) {
      normalized["ts-texture-strength"] = normalized["ts-texture-depth"];
    }
    if (
      normalized["ts-texture-buildup-custom"] == null &&
      normalized["ts-texture-buildup"] != null
    ) {
      normalized["ts-texture-buildup-custom"] =
        normalized["ts-texture-buildup"];
    }
    if (
      normalized["ts-texture-brightness"] == null &&
      normalized["ts-brightness"] != null
    ) {
      normalized["ts-texture-brightness"] = normalized["ts-brightness"];
    }
    if (
      normalized["ts-texture-contrast"] == null &&
      normalized["ts-contrast"] != null
    ) {
  ...
  function normalizeBrushSettings(settings) {
    ...
    normalized["ts-spacing"] =
      Number.isFinite(spacing) && spacing > 0 ? spacing : 12;
    delete normalized["ts-density"];
    const mode = normalized["ts-spacing-mode"];
    normalized["ts-spacing-mode"] =
      mode === "velocity" || mode === "pen-pressure" ? mode : "fixed";
    if ("ts-stabilization" in normalized) {
      const stab = Number(normalized["ts-stabilization"]);
      normalized["ts-stabilization"] = Number.isFinite(stab)
        ? Math.max(0, Math.min(100, stab))
        : 0;
    }
    return normalized;
  }
  const builtinBrushSettings = (overrides) =>
    Object.assign(
      {},
      DEFAULT_BUILTIN_BRUSH_SETTINGS,
      { "ts-pressure-curves": {} },
      normalizeBrushSettings(overrides),
    );
  ...
  const _presetSettingsKey = (presetId, toolType = "brush") =>
    toolType + ":" + presetId;
  ...
  function _seedPresetSettings(preset, toolType = "brush") {
    ...
    if (!_presetSettings[key]) {
      _presetSettings[key] = JSON.parse(
        JSON.stringify(normalizeBrushSettings(preset.settings)),
      );
    } else {
  ...
  function _isolatePresetTextureSettings(preset, settings) {
    if (!preset || preset.custom || _isPresetTextureDeclared(preset))
      return settings;
  ...
  function _captureToPreset(
    presetId,
    captureTip = false,
    toolType = tool === "eraser" ? "eraser" : "brush",
  ) {
    if (!presetId) return;
    ...
    const effective =
      typeof getToolPreset === "function"
        ? getToolPreset({ includeImages: captureTip })
        : {};
    Object.keys(effective).forEach((settingKey) => {
      if (
        !captureTip &&
        (settingKey === "ts-tip-dataurl" || settingKey === "ts-texture-dataurl")
      )
        return;
    ...
    if (Number.isFinite(capturedSize)) {
      ...
      if (_activePresetId === presetId)
        _toolState[toolType].size = capturedSize;
      if (_activeTab === toolType) {
        const displaySize = window.formatBrushSize
          ? window.formatBrushSize(capturedSize)
          : String(Math.round(capturedSize * 10) / 10);
        document
          .querySelectorAll(
            ".bp-item[data-preset-id=" +
              CSS.escape(presetId) +
              "] .bp-preset-size",
          )
          .forEach((label) => {
            label.textContent = displaySize;
          });
      }
    }
    if (captureTip) {
      if (window.brushTipCanvas) {
        try {
          settings["ts-tip-dataurl"] =
            window.brushTipCanvas.toDataURL("image/png");
        } catch (e) {}
        ...
        if (
          ...
          settings["ts-tip-reference-diameter"] = Number(
            window.brushTipReferenceDiameter,
          );
        else delete settings["ts-tip-reference-diameter"];
      } else {
        delete settings["ts-tip-dataurl"];
        delete settings["ts-tip-mode"];
        delete settings["ts-tip-soft-alpha"];
        delete settings["ts-tip-reference-diameter"];
      }
      ...
      if (window.brushTextureCanvas) {
        try {
          settings["ts-texture-dataurl"] =
            window.brushTextureCanvas.toDataURL("image/png");
        } catch (e) {}
        settings["ts-texture-strength"] = Math.round(
          (window.brushTextureStrength != null
            ? window.brushTextureStrength
            : 1.0) * 100,
        );
        settings["ts-texture-buildup-custom"] = Math.round(
          (window.brushTextureBuildup != null
            ? window.brushTextureBuildup
            : 1.0) * 100,
        );
        settings["ts-texture-scale"] = Math.round(
          (window.brushTextureScale || 1.0) * 100,
        );
      } else {
        delete settings["ts-texture-dataurl"];
    ...
    if (captureTip) {
      const pending = _strokeTipLoads.get(presetId);
      if (pending) pending.src = "";
      _strokeTipLoads.delete(presetId);
      delete _tipThumbCache[presetId];
      const texturePending = _strokeTextureLoads.get(presetId);
      if (texturePending) texturePending.src = "";
      _strokeTextureLoads.delete(presetId);
      _strokeTextureCache.delete(presetId);
    }
    for (const key of _strokePreviewCache.keys())
      if (key.startsWith(presetId + "|")) _strokePreviewCache.delete(key);
    const selector =
      '.bp-stroke-canvas[data-preset-id="' + CSS.escape(presetId) + '"]';
    document.querySelectorAll(selector).forEach((canvas) => {
      const preset = findPreset(presetId);
      if (preset) _scheduleStrokePreview(canvas, preset);
    });
  }
  ...
  let _toolPresetSizes = {
    brush: { "hard-round": 6 },
    eraser: { "hard-round": 20 },
  };
    ...
    try {
      localStorage.setItem(
        ...
        JSON.stringify({
          ...
          groups: _groups,
          eraserPresetIds: Array.from(_eraserPresetIds),
          toolState: _toolState,
          toolPresetSizes: _toolPresetSizes,
          presetSettings: _presetSettings,
        }),
      );
    } catch (e) {
  ...
  window._captureActiveBrushPreset = (captureTip = false) => {
  ...
  function captureActivePresetFromEvent(event) {
    if (
      _applyingPresetSettings ||
      !event.target ||
      !event.target.closest(
        "#tool-settings-body,#brush-tool-settings-content",
      ) ||
      !event.target.id
    )
      return;
  ...
  function loadPersisted() {
    try {
      ...
      if (data && Array.isArray(data.customPresets))
        _customPresets = data.customPresets.map((p) =>
          Object.assign({}, p, {
            settings: normalizeBrushSettings(p.settings),
          }),
        );
      if (data && Array.isArray(data.groups) && data.groups.length)
        _groups = data.groups;
      if (data && Array.isArray(data.eraserPresetIds)) {
        _eraserPresetIds = new Set(
          DEFAULT_ERASER_PRESET_IDS.concat(
            data.eraserPresetIds.filter((id) => typeof id === "string"),
          ),
        );
      }
      if (data && data.toolState) {
        if (data.toolState.brush) {
          _toolState.brush = Object.assign(
            {},
            _toolState.brush,
            data.toolState.brush,
          );
          delete _toolState.brush.density;
        }
        if (data.toolState.eraser) {
          _toolState.eraser = Object.assign(
            {},
            _toolState.eraser,
            data.toolState.eraser,
          );
          delete _toolState.eraser.density;
        }
      }
      if (data && data.toolPresetSizes) {
        if (data.toolPresetSizes.brush)
          Object.assign(_toolPresetSizes.brush, data.toolPresetSizes.brush);
        if (data.toolPresetSizes.eraser)
          Object.assign(_toolPresetSizes.eraser, data.toolPresetSizes.eraser);
      }
      ...
      if (
        data &&
        data.presetSettings &&
        typeof data.presetSettings === "object"
      ) {
        Object.entries(data.presetSettings).forEach(([key, value]) => {
          if (key.includes(":")) {
            ...
            if (key === "brush:hard-round") {
              ...
              if (Number(restored["ts-hardness"]) === 55)
                restored["ts-hardness"] = 100;
              ...
              if (
                restored["ts-taper-mode"] === "percentage" &&
          ...
          if (key === "hard-round") {
            ...
            if (Number(migrated["ts-spacing"]) === 12)
  ...
  if (!_eraserPresetIds.has(_toolState.eraser.presetId)) {
    ...
    _toolState.eraser.size =
      Number(_toolPresetSizes.eraser["hard-round"]) || 20;
  }
  const generalGroup =
    _groups.find((group) => group.id === "general") ||
    _groups.find((group) => group.default) ||
    _groups[0];
  if (generalGroup) {
  ...
  function allPresets() {
    return BRUSH_PRESETS.concat(_customPresets).concat(_assetPackPresets);
  }
  function findPreset(id) {
    return allPresets().find((p) => p.id === id);
  }
  function presetVisibleInActiveLibrary(preset) {
    return _activeTab !== "eraser" || _eraserPresetIds.has(preset.id);
  }
  ...
  function drawPreview(canvas, cfg) {
    ...
    if (cfg.shape === "square") {
      ...
      c.scale(1, ry / r);
      c.beginPath();
      c.arc(cx, cy * (r / ry), r, 0, Math.PI * 2);
      c.fillStyle = grad;
      c.fill();
      c.restore();
    } else {
  ...
  function _paintTipThumbnail(
    ...
    const pad = 6,
      ...
      maxH = height - pad * 2;
    const rotation =
      ((Number(settings && settings["ts-angle"]) || 0) * Math.PI) / 180;
    const minimumRoundness =
      (Number(settings && settings["ts-tip-min-roundness"]) || 0) / 100;
    const roundness = Math.max(
      minimumRoundness,
      (Number(settings && settings["ts-tip-roundness"]) || 100) / 100,
    );
    const imageWidth = img.width || img.naturalWidth || 1,
      imageHeight = img.height || img.naturalHeight || 1;
    ...
    const rotatedWidth =
      Math.abs(shapedWidth * Math.cos(rotation)) +
      Math.abs(shapedHeight * Math.sin(rotation));
    const rotatedHeight =
      Math.abs(shapedWidth * Math.sin(rotation)) +
      Math.abs(shapedHeight * Math.cos(rotation));
    const scale = Math.min(
      maxW / Math.max(1, rotatedWidth),
      maxH / Math.max(1, rotatedHeight),
    ...
    if (settings && (settings["ts-tip-flip-x"] || settings["ts-tip-flip-y"]))
      context.scale(
        settings["ts-tip-flip-x"] ? -1 : 1,
        settings["ts-tip-flip-y"] ? -1 : 1,
      );
    context.drawImage(
      img,
      (-shapedWidth * scale) / 2,
      (-shapedHeight * scale) / 2,
      shapedWidth * scale,
      shapedHeight * scale,
    );
    context.restore();
  ...
  function _presetPreviewSettings(p) {
    const settings = Object.assign(
      {},
      p.settings || {},
      _presetSettings[_presetSettingsKey(p.id, _activeTab)] || {},
    );
    const liveTool = tool === "eraser" ? "eraser" : "brush";
    if (
      p.id === _activePresetId &&
      liveTool === _activeTab &&
      typeof getToolPreset === "function"
    ) {
      Object.assign(settings, getToolPreset({ includeImages: false }));
      settings["ts-size"] = toolSizes[liveTool];
    }
    return settings;
  }
  ...
  function _requestStrokeTip(p, settings) {
    const src = settings["ts-tip-dataurl"] || settings["ts-tip-url"];
    if (
      !src ||
      (_tipThumbCache[p.id] && _tipThumbCache[p.id].src === src) ||
      _strokeTipLoads.has(p.id)
    )
      return;
    const img = new Image();
    _strokeTipLoads.set(p.id, img);
    img.onload = () => {
      if (_strokeTipLoads.get(p.id) !== img) return;
      _strokeTipLoads.delete(p.id);
      _tipThumbCache[p.id] = { alphaCanvas: _tipImageToAlphaCanvas(img), src };
      for (const key of _strokePreviewCache.keys())
        if (key.startsWith(p.id + "|")) _strokePreviewCache.delete(key);
      document
        .querySelectorAll(
          '.bp-stroke-canvas[data-preset-id="' + CSS.escape(p.id) + '"]',
        )
        .forEach((canvas) => _scheduleStrokePreview(canvas, p));
    };
    img.onerror = () => {
      if (_strokeTipLoads.get(p.id) === img) _strokeTipLoads.delete(p.id);
    };
    img.src = src;
  }
  function _requestStrokeTexture(p, settings) {
    ...
    if (
      !src ||
      (_strokeTextureCache.get(p.id) &&
        _strokeTextureCache.get(p.id).src === src) ||
      _strokeTextureLoads.has(p.id)
    )
      return;
    const img = new Image();
    _strokeTextureLoads.set(p.id, img);
    img.onload = () => {
      if (_strokeTextureLoads.get(p.id) !== img) return;
      _strokeTextureLoads.delete(p.id);
      _strokeTextureCache.set(p.id, { image: img, src });
      for (const key of _strokePreviewCache.keys())
        if (key.startsWith(p.id + "|")) _strokePreviewCache.delete(key);
      document
        .querySelectorAll(
    ...
    img.onerror = () => {
      if (_strokeTextureLoads.get(p.id) === img)
        _strokeTextureLoads.delete(p.id);
    };
    img.src = src;
  }
  function _applyPresetTexture(preview, p, settings) {
    const src = settings["ts-texture-dataurl"] || settings["ts-texture-url"],
      entry = _strokeTextureCache.get(p.id);
    ...
    const mc = mask.getContext("2d"),
      scale = Math.max(
        0.1,
        (Number(settings["ts-texture-scale"]) || 100) / 100,
      ),
      pattern = mc.createPattern(entry.image, "repeat");
    if (!pattern) return;
    if (typeof pattern.setTransform === "function")
      pattern.setTransform(
        new DOMMatrix().scale(scale * (window.devicePixelRatio || 1)),
      );
    mc.fillStyle = pattern;
    mc.fillRect(0, 0, width, height);
    try {
      const image = mc.getImageData(0, 0, width, height),
        data = image.data,
        invert = !!settings["ts-texture-invert"],
        brightness = (Number(settings["ts-texture-brightness"]) || 0) / 100,
        contrast = Math.max(
          0.05,
          1 + (Number(settings["ts-texture-contrast"]) || 0) / 50,
        );
      for (let i = 0; i < data.length; i += 4) {
        ...
        value = Math.max(
          0,
          Math.min(1, 0.5 + (value + brightness - 0.5) * contrast),
        );
    ...
    const strength = Math.max(
        0,
        Math.min(1, (Number(settings["ts-texture-strength"]) || 100) / 100),
      ),
      masked = document.createElement("canvas");
    masked.width = width;
    masked.height = height;
    const xc = masked.getContext("2d");
    xc.drawImage(preview, 0, 0);
    xc.globalCompositeOperation = "destination-in";
    xc.drawImage(mask, 0, 0);
    ...
    pc.globalAlpha = strength;
    pc.drawImage(masked, 0, 0);
    pc.globalAlpha = 1;
  }
  function _renderPresetPreview(p, settings, width, height, dpr, kind) {
    ...
    ctx.imageSmoothingEnabled =
      settings["ts-aa"] !== false && settings["ts-aa-mode"] !== "none";
    ctx.imageSmoothingQuality =
      settings["ts-aa-mode"] === "strong" ? "high" : "medium";
    const rawSize = Math.max(
      1,
      Number(settings["ts-size"]) ||
        Number(p.settings && p.settings["ts-size"]) ||
        12,
    );
    const requestedDiameter = 5 + Math.sqrt(rawSize) * 2.25;
    const spacing =
      typeof window._resolveBrushSpacingFrac === "function"
        ? window._resolveBrushSpacingFrac(settings)
        : Math.max(0.01, (Number(settings["ts-spacing"]) || 12) / 100);
    const hardness = Math.max(
      0,
      Math.min(1, (Number(settings["ts-hardness"]) || 0) / 100),
    );
    const opacity = Math.max(
      0.04,
      Math.min(1, (Number(settings["ts-opacity"]) || 100) / 100),
    );
    const flow = Math.max(
      0.04,
      Math.min(1, (Number(settings["ts-flow"]) || 100) / 100),
    );
    const roundness = Math.max(
      0.08,
      Math.min(1, (Number(settings["ts-tip-roundness"]) || 100) / 100),
    );
    const angle = ((Number(settings["ts-angle"]) || 0) * Math.PI) / 180,
      directionFollowsStroke =
        settings["ts-rotation-mode"] === "stroke-direction";
    const edgeSafety = 3 + Math.max(0, 1 - hardness) * 2,
      rotationWidth = directionFollowsStroke
        ? 1
        : Math.abs(Math.cos(angle)) + roundness * Math.abs(Math.sin(angle)),
      rotationHeight = directionFollowsStroke
        ? 1
        : Math.abs(Math.sin(angle)) + roundness * Math.abs(Math.cos(angle));
    const targetWave = Math.min(6, height * 0.5),
      diameter = Math.max(
        3,
        Math.min(
          requestedDiameter,
          (height - (edgeSafety + targetWave) * 2) /
            Math.max(1, rotationHeight),
        ),
      ),
      step = Math.max(0.75, diameter * spacing);
    const radiusX = (diameter * rotationWidth) / 2,
      radiusY = (diameter * rotationHeight) / 2;
    const scatterAmount = settings["ts-scatter-enabled"]
      ? Math.max(0, Number(settings["ts-scatter-amount"]) || 0) / 100
      : 0;
    const scatterPad = Math.min(width * 0.08, diameter * scatterAmount * 0.5),
      safeX = Math.ceil(radiusX + edgeSafety + scatterPad),
      safeY = Math.ceil(radiusY + edgeSafety + scatterPad);
    const tipSrc = settings["ts-tip-dataurl"] || settings["ts-tip-url"],
      tipEntry = _tipThumbCache[p.id],
      tip = tipEntry && tipEntry.src === tipSrc ? tipEntry.alphaCanvas : null;
    const dab = document.createElement("canvas"),
      dabScale = Math.max(1, dpr),
      dabW = Math.ceil(diameter * dabScale),
      dabH = Math.ceil(diameter * roundness * dabScale);
    dab.width = Math.max(1, dabW);
    dab.height = Math.max(1, dabH);
    const dc = dab.getContext("2d");
    dc.imageSmoothingEnabled = ctx.imageSmoothingEnabled;
    dc.imageSmoothingQuality = ctx.imageSmoothingQuality;
    const previewRgb = _toolPreviewRgb();
    if (tip) {
      dc.drawImage(tip, 0, 0, dab.width, dab.height);
      dc.globalCompositeOperation = "source-in";
      dc.fillStyle = "rgba(" + previewRgb + ",1)";
      dc.fillRect(0, 0, dab.width, dab.height);
    } else {
      const radius = Math.max(dab.width, dab.height) / 2,
        gradient = dc.createRadialGradient(
          dab.width / 2,
          dab.height / 2,
          0,
          dab.width / 2,
      ...
      const previewFalloff =
        typeof window._proceduralBrushFalloff === "function"
          ? (t) =>
              window._proceduralBrushFalloff(
                t,
                hardness,
                diameter / 2,
                settings["ts-aa-mode"] || "medium",
                isAirbrush,
              )
          : (t) =>
              t <= hardness
                ...
                : Math.max(
                    0,
                    1 - (t - hardness) / Math.max(0.0001, 1 - hardness),
                  );
      ...
      for (let stop = 0; stop <= PREVIEW_STOPS; stop++) {
        ...
        gradient.addColorStop(
          t,
          "rgba(" + previewRgb + "," + previewFalloff(t) + ")",
        );
      }
      dc.fillStyle = gradient;
      dc.fillRect(0, 0, dab.width, dab.height);
    }
    const baseAlpha = Math.min(1, opacity * (0.35 + 0.65 * flow));
    if (kind === "stamp") {
      const stampPad = 5,
        stampSize = Math.max(
          2,
          Math.min(
            diameter,
            (width - stampPad * 2) / Math.max(0.01, rotationWidth),
            (height - stampPad * 2) / Math.max(0.01, rotationHeight),
          ),
        );
      ...
      ctx.drawImage(
        dab,
        -stampSize / 2,
        (-stampSize * roundness) / 2,
        stampSize,
        stampSize * roundness,
      );
      ctx.restore();
      _applyPresetTexture(out, p, settings);
      return out;
    }
    const sizePressure = settings["ts-size-control"] === "pressure",
      opacityPressure =
        settings["ts-opacity-control"] === "pressure" ||
        settings["ts-flow-control"] === "pressure";
    const minScale = Math.max(
        0.04,
        Math.min(1, (Number(settings["ts-min-size"]) || 0) / 100),
      ),
      minAlpha = Math.max(
        0.04,
        Math.min(1, (Number(settings["ts-min-flow"]) || 0) / 100),
      );
    const start = Math.min(width / 2, Math.max(6, safeX)),
      end = Math.max(start, width - Math.max(6, safeX)),
      span = Math.max(1, end - start),
      dabCount = Math.max(1, Math.ceil(span / step)),
      waveRoom = Math.max(0, height / 2 - safeY),
      waveAmplitude = Math.min(targetWave, waveRoom);
    for (let dabIndex = 0; dabIndex <= dabCount; dabIndex++) {
      const distance = Math.min(span, dabIndex * step),
        t = distance / span,
        rise = Math.min(1, 0.58 + t * 2.8),
        fall = t < 0.62 ? 1 : Math.max(0.02, 1 - (t - 0.62) / 0.38),
        pressure = Math.min(rise, fall);
      const scale = sizePressure
        ? Math.max(minScale, pressure)
        : 1 - 0.18 * (1 - fall);
      const phase = t * Math.PI * 2,
        wave = -Math.sin(phase),
        waveDerivative = -Math.PI * 2.5 * Math.cos(phase);
      const x = start + distance,
        y = height / 2 + wave * waveAmplitude,
        strokeAngle =
          angle +
          (directionFollowsStroke
            ? Math.atan2(waveDerivative * waveAmplitude, span)
            : 0);
      ctx.globalAlpha =
        baseAlpha * (opacityPressure ? Math.max(minAlpha, pressure) : 1);
      ctx.save();
      ctx.translate(x, y);
      if (strokeAngle) ctx.rotate(strokeAngle);
      ctx.drawImage(
        dab,
        (-diameter * scale) / 2,
        (-diameter * roundness * scale) / 2,
        diameter * scale,
        diameter * roundness * scale,
      );
      ctx.restore();
    }
    _applyPresetTexture(out, p, settings);
  ...
  function _scheduleStrokePreview(canvas, p) {
    if (canvas.dataset.previewPending === "true") return;
    canvas.dataset.previewPending = "true";
    requestAnimationFrame(() => {
      canvas.dataset.previewPending = "false";
      if (!canvas.isConnected) return;
      const rect = canvas.getBoundingClientRect(),
        kind = canvas.dataset.previewKind || "stroke",
        width = Math.max(
          kind === "stamp" ? 30 : 72,
          Math.round(rect.width || (kind === "stamp" ? 38 : 132)),
        ),
        height = Math.max(30, Math.round(rect.height || 40)),
        dpr = Math.max(1, window.devicePixelRatio || 1),
        settings = _presetPreviewSettings(p);
      _requestStrokeTip(p, settings);
      _requestStrokeTexture(p, settings);
      const key = _previewHash(p, settings, width, height, dpr, kind);
      let cached = _strokePreviewCache.get(key);
      if (!cached) {
        cached = _renderPresetPreview(p, settings, width, height, dpr, kind);
        _strokePreviewCache.set(key, cached);
        if (_strokePreviewCache.size > 300)
          _strokePreviewCache.delete(_strokePreviewCache.keys().next().value);
      }
  ...
  const _strokePreviewResizeObserver =
    typeof ResizeObserver === "undefined"
      ? null
      : new ResizeObserver((entries) => {
          entries.forEach((entry) => {
            const preset = _strokePreviewPresetByCanvas.get(entry.target);
            if (preset) _scheduleStrokePreview(entry.target, preset);
          });
        });
  function drawPresetThumb(canvas, p) {
    ...
    img.onload = () => {
      ...
      document
        .querySelectorAll(".bp-item .bp-preview canvas")
        .forEach((liveCanvas) => {
  ...
  function showBpCtxMenu(e, presetId) {
    ...
    const addEraserItem = document.getElementById("bp-ctx-add-eraser"),
      removeEraserItem = document.getElementById("bp-ctx-remove-eraser");
    const inErasers = _eraserPresetIds.has(presetId),
      isEraserTab = _activeTab === "eraser";
    if (addEraserItem) {
      addEraserItem.style.display = isEraserTab ? "none" : "";
      addEraserItem.style.opacity = inErasers ? "0.4" : "1";
      addEraserItem.setAttribute("aria-disabled", String(inErasers));
    }
    if (removeEraserItem) {
      const removable =
        isEraserTab && !DEFAULT_ERASER_PRESET_IDS.includes(presetId);
      removeEraserItem.style.display = isEraserTab ? "" : "none";
      removeEraserItem.style.opacity = removable ? "1" : "0.4";
      removeEraserItem.setAttribute("aria-disabled", String(!removable));
    }
    ["bp-ctx-rename", "bp-ctx-cut", "bp-ctx-paste", "bp-ctx-delete"].forEach(
      (id) => {
        const item = document.getElementById(id);
        if (item) item.style.display = isEraserTab ? "none" : "";
      },
    );
  ...
  if (bpCtxMenu) {
    document.getElementById("bp-ctx-add-eraser").onclick = () => {
      ...
      const brushSettings =
        _presetSettings[_presetSettingsKey(id, "brush")] ||
        preset.settings ||
        {};
      _presetSettings[_presetSettingsKey(id, "eraser")] = JSON.parse(
        JSON.stringify(normalizeBrushSettings(brushSettings)),
      );
      const size = Number(
        _toolPresetSizes.brush[id] ??
          brushSettings["ts-size"] ??
          preset.settings?.["ts-size"],
      );
      if (Number.isFinite(size)) _toolPresetSizes.eraser[id] = size;
      _eraserPresetIds.add(id);
      persist();
    };
    document.getElementById("bp-ctx-remove-eraser").onclick = () => {
      ...
      if (
        !id ||
        DEFAULT_ERASER_PRESET_IDS.includes(id) ||
        !_eraserPresetIds.has(id)
      )
        return;
      _eraserPresetIds.delete(id);
      delete _presetSettings[_presetSettingsKey(id, "eraser")];
      delete _toolPresetSizes.eraser[id];
      if (_toolState.eraser.presetId === id) {
        _toolState.eraser.presetId = "hard-round";
        _activePresetId = "hard-round";
        window._activeBrushPresetId = "hard-round";
    ...
    document.getElementById("bp-ctx-rename").onclick = () => {
      ...
      const item = document.querySelector(
        `.bp-item[data-preset-id="${_bpCtxTargetId}"]`,
      );
      if (!item) return;
    ...
    document.getElementById("bp-ctx-paste").onclick = () => {
      ...
      const pasted = Object.assign(
        {},
        JSON.parse(JSON.stringify(_bpCopiedPreset)),
        { id, name: _bpCopiedPreset.name + " Copy", custom: true },
      );
      _customPresets.push(pasted);
      // Seed the pasted preset's own settings slot (copied from the source preset's saved settings)
      _presetSettings[
        _presetSettingsKey(id, tool === "eraser" ? "eraser" : "brush")
      ] = JSON.parse(
        JSON.stringify(
          _presetSettings[
            _presetSettingsKey(
      ...
      const targetGroupId = _bpCtxTargetId
        ? (
            _groups.find((g) => g.ids.includes(_bpCtxTargetId)) ||
            _groups.find((g) => g.default) ||
            _groups[0]
          ).id
    ...
    document.getElementById("bp-ctx-cut").onclick = () => {
      ...
      _groups.forEach((g) => {
        const i = g.ids.indexOf(_bpCtxTargetId);
        if (i !== -1) g.ids.splice(i, 1);
      });
      persist();
      buildGrid();
  ...
  function _startGroupRename(grpId) {
    ...
    const hd = document.querySelector(
      `.bp-group[data-group-id="${grpId}"] .bp-group-hd`,
    );
    if (!hd) return;
    ...
    nameSpan.addEventListener("keydown", (ev) => {
      ...
      if (ev.key === "Escape") {
        nameSpan.textContent = grp.icon + " " + oldLabel;
        nameSpan.blur();
      }
    });
  }
  ...
  function showBgCtxMenu(e, grpId) {
    ...
    const containsBuiltin = !!(
      targetGroup &&
      targetGroup.ids.some((id) =>
        BRUSH_PRESETS.some((preset) => preset.id === id),
      )
    );
    const canDelete = !!targetGroup && !targetGroup.default && !containsBuiltin;
    if (deleteItem) {
      deleteItem.style.opacity = canDelete ? "1" : "0.4";
      deleteItem.style.pointerEvents = canDelete ? "auto" : "none";
    }
  ...
  if (bgCtxMenu) {
    ...
    document.getElementById("bg-ctx-paste").onclick = () => {
      ...
      _bpCopiedGroup.presetSnapshots.forEach((snap) => {
        if (snap.builtinId) {
          ...
          const cloned = Object.assign({}, JSON.parse(JSON.stringify(snap)), {
            id: newId,
            custom: true,
          });
    ...
    const deleteGroupModal = document.getElementById(
      "modal-delete-brush-group",
    );
    ...
    const deleteBrushGroup = (groupId) => {
      ...
      const containsBuiltin = group.ids.some((id) =>
        BRUSH_PRESETS.some((preset) => preset.id === id),
      );
      ...
      for (const toolType of ["brush", "eraser"]) {
        if (deletedIds.has(_toolState[toolType].presetId))
          _toolState[toolType].presetId = "hard-round";
        for (const id of deletedIds) delete _toolPresetSizes[toolType][id];
      }
      _customPresets = _customPresets.filter(
        (preset) => !deletedIds.has(preset.id),
      );
      for (const id of deletedIds) {
        _eraserPresetIds.delete(id);
        delete _presetSettings[_presetSettingsKey(id, "brush")];
        delete _presetSettings[_presetSettingsKey(id, "eraser")];
      }
    ...
    document.getElementById("delete-brush-group-cancel").onclick =
    ...
    document.addEventListener("keydown", (event) => {
      if (
        event.key === "Escape" &&
        deleteGroupModal.classList.contains("visible")
      )
        closeDeleteGroupModal();
    });
  }
  ...
  function makeBpItem(p, groupId) {
    ...
    item.setAttribute("aria-pressed", String(p.id === _activePresetId));
    item.setAttribute("aria-label", p.name + " brush preset");
    item.title = p.name;
    const previewRow = document.createElement("div");
    previewRow.className = "bp-preview-row";
    const stampWrap = document.createElement("div");
    stampWrap.className = "bp-preview bp-tip-preview";
    const stamp = document.createElement("canvas");
    stamp.className = "bp-stroke-canvas";
    stamp.dataset.presetId = p.id;
    stamp.dataset.previewKind = "stamp";
    stamp.setAttribute("aria-hidden", "true");
    stampWrap.appendChild(stamp);
    const strokeWrap = document.createElement("div");
    strokeWrap.className = "bp-preview bp-line-preview";
    const stroke = document.createElement("canvas");
    stroke.className = "bp-stroke-canvas";
    stroke.dataset.presetId = p.id;
    stroke.dataset.previewKind = "stroke";
    stroke.setAttribute("aria-hidden", "true");
    strokeWrap.appendChild(stroke);
    previewRow.append(stampWrap, strokeWrap);
    const info = document.createElement("div");
    info.className = "bp-preset-info";
    const lbl = document.createElement("div");
    lbl.className = "bp-name";
    lbl.textContent = p.name;
    lbl.title = p.name;
    const size = document.createElement("div");
    size.className = "bp-preset-size";
    const settings = _presetPreviewSettings(p);
    size.textContent = window.formatBrushSize
      ? window.formatBrushSize(
          Number(settings["ts-size"]) ||
            Number(p.settings && p.settings["ts-size"]) ||
            6,
    ...
    info.append(lbl, size);
    item.append(previewRow, info);
    [stamp, stroke].forEach((canvas) => {
      _strokePreviewPresetByCanvas.set(canvas, p);
      if (_strokePreviewResizeObserver)
        _strokePreviewResizeObserver.observe(canvas);
      _scheduleStrokePreview(canvas, p);
    });
    ...
    item.addEventListener("dragstart", (e) => {
      ...
      try {
        e.dataTransfer.setData("text/plain", "item:" + p.id);
      } catch (err) {}
    });
    item.addEventListener("dragend", () => {
      item.classList.remove("dragging");
      document
        .querySelectorAll(".bp-item.drop-before,.bp-item.drop-after")
  ...
  function _bpSearchVal() {
    const el = document.getElementById("bp-search");
    return el ? el.value : "";
  }
  ...
  function buildGrid(searchQuery) {
    ...
    if (q) {
      ...
      const matched = allPresets().filter(
        (p) =>
          presetVisibleInActiveLibrary(p) && p.name.toLowerCase().includes(q),
      );
      if (matched.length === 0) {
    ...
    _groups.forEach((grp) => {
      ...
      const visibleIds = grp.ids.filter((id) => {
        const preset = findPreset(id);
        return preset && presetVisibleInActiveLibrary(preset);
      });
      const hd = document.createElement("div");
      hd.className = "bp-group-hd" + (grp.collapsed ? " collapsed" : "");
      hd.draggable = true;
      hd.innerHTML =
        `<span class="bp-group-grip">⠿</span><span class="bp-group-toggle">▾</span><span class="bp-group-name">${grp.icon} ${grp.label}</span><span class="bp-group-count">${grp.ids.length}</span>` +
        (grp.default
          ? ""
          : '<span class="bp-group-del" title="Delete group">✕</span>');
      const visibleCount = hd.querySelector(".bp-group-count");
      if (visibleCount) visibleCount.textContent = visibleIds.length;
      ...
      if (delBtn) {
        delBtn.addEventListener("click", (e) => {
          e.stopPropagation();
          if (typeof window._openBrushGroupDeleteModal === "function")
            window._openBrushGroupDeleteModal(grp.id);
        });
      }
      ...
      hd.addEventListener("dragend", () => {
        ...
        document
          .querySelectorAll(".bp-group.drop-before,.bp-group.drop-after")
          .forEach((el) => el.classList.remove("drop-before", "drop-after"));
        _drag = null;
      });
      hd.addEventListener("dragover", (e) => {
        ...
        document
          .querySelectorAll(".bp-group.drop-before,.bp-group.drop-after")
          .forEach((el) => {
            if (el !== grpEl) el.classList.remove("drop-before", "drop-after");
          });
      ...
      innerGrid.addEventListener("dragleave", () =>
  ...
  function applyPresetSettings(p) {
    ...
    p.settings = s;
    if (!("ts-rotation-mode" in s) || s["ts-rotation-mode"] === "fixed")
      s["ts-rotation-mode"] = "fixed-rotation";
    ...
    window.brushTipRoundness = Math.max(
      0.01,
      Math.min(1, (s["ts-tip-roundness"] ?? 100) / 100),
    );
    window.brushTipMinimumRoundness = Math.max(
      0,
      Math.min(1, (s["ts-tip-min-roundness"] ?? 0) / 100),
    );
    ...
    window._tsCustomPressureCurves = JSON.parse(
      JSON.stringify(s["ts-pressure-curves"] || {}),
    );
    if (typeof window._refreshPressureCurvePreviews === "function")
      window._refreshPressureCurvePreviews();
    ...
    if (spacingMode) {
      ...
      s["ts-spacing-mode"] =
        savedMode === "velocity" || savedMode === "pen-pressure"
          ? savedMode
          : "fixed";
    }
    ...
    if (spacingMode)
      spacingMode.dispatchEvent(new Event("input", { bubbles: true }));
    const taperMode = document.getElementById("ts-taper-mode");
    if (taperMode)
      taperMode.dispatchEvent(new Event("input", { bubbles: true }));
    const mapping = {
      "ts-size": {
        slider: "ts-size",
        val: "ts-size-val",
      ...
      "ts-roundness": {
        slider: "ts-roundness",
        val: "ts-roundness-val",
        suffix: "",
        extra: (v) => {
          window._tsRoundness = v / 100;
          _aaDabCache.clear();
          _stampCache.clear();
        },
      },
      ...
      "ts-airbrush": null,
      "ts-airbrush-rate": {
        slider: "ts-airbrush-rate",
        val: "ts-airbrush-rate-val",
        suffix: "",
        extra: (v) => {
          window._tsAirbrushRate = v / 100;
        },
    ...
    const targetAAMode =
      ...
      s["ts-aa-mode"] === "medium"
        ...
        : "medium";
    const isAAEnabled =
      s["ts-aa"] !== false && s["ts-aa"] !== 0 && s["ts-aa-mode"] !== "none";
    ...
    Object.entries(s).forEach(([key, val]) => {
      if (
        key === "ts-pressure-curves" ||
        key === "ts-aa" ||
        key === "ts-aa-mode"
      )
        return;
      if (key === "ts-airbrush") {
        if (typeof window._setAirbrush === "function")
          window._setAirbrush(!!val);
        return;
      }
      if (key === "ts-continuous-spraying") {
        if (typeof window._setContinuousSpraying === "function")
          window._setContinuousSpraying(!!val);
        return;
      }
      const m = mapping[key];
      if (m) {
    ...
    Object.keys(s).forEach((key) => {
      ...
      if (source && source.matches("input:not([type=file]),select"))
        source.dispatchEvent(new Event("input", { bubbles: true }));
    });
    if (typeof window._syncAdvancedBrushSettings === "function")
      window._syncAdvancedBrushSettings();
    ...
    if (!("ts-airbrush" in s) && typeof window._setAirbrush === "function")
      window._setAirbrush(false);
    if (
      !("ts-continuous-spraying" in s) &&
      typeof window._setContinuousSpraying === "function"
    )
      window._setContinuousSpraying(false);
    ...
    if (tipSrc) {
      const commitTip = (c) => {
        if (
          activationVersion !== _brushRuntimeActivationVersion ||
          window._brushTipLoadGeneration !== tipLoadGeneration
        )
          return;
        if (typeof window.setBrushTip === "function")
          window.setBrushTip(
            c,
            s["ts-tip-reference-diameter"],
            "preset-activation",
          );
        if (s["ts-tip-mode"]) window.brushTipMode = s["ts-tip-mode"];
        if (s["ts-tip-soft-alpha"] !== undefined)
          window.brushTipSoftAlpha = !!s["ts-tip-soft-alpha"];
        if (typeof window._syncTipUI === "function") window._syncTipUI();
      };
      if (
        runtimeAssets &&
        runtimeAssets.tipSrc === tipSrc &&
        runtimeAssets.tipCanvas
      )
        commitTip(runtimeAssets.tipCanvas);
      else if (typeof _dataURLToCanvas === "function")
        pendingAssets.push(_dataURLToCanvas(tipSrc, true).then(commitTip));
    } else {
      if (typeof window.clearBrushTip === "function") window.clearBrushTip();
      if (typeof window._syncTipUI === "function") window._syncTipUI();
    }
    ...
    if (textureSrc) {
      const commitTexture = (c) => {
        ...
        if (typeof window.setBrushTexture === "function")
          window.setBrushTexture(c);
        const depth =
          s["ts-texture-strength"] != null
            ? +s["ts-texture-strength"] / 100
            : 1.0;
        window.brushTextureStrength = depth;
        const buildup =
          s["ts-texture-buildup-custom"] != null
            ? +s["ts-texture-buildup-custom"] / 100
            : 1.0;
        window.brushTextureBuildup = buildup;
        window.brushTextureScale =
          s["ts-texture-scale"] != null ? +s["ts-texture-scale"] / 100 : 1.0;
        window.brushTextureInvert = !!s["ts-texture-invert"];
        window.brushTextureBrightness =
          s["ts-texture-brightness"] != null ? +s["ts-texture-brightness"] : 0;
        window.brushTextureContrast =
          s["ts-texture-contrast"] != null ? +s["ts-texture-contrast"] : 0;
        if (typeof window._invalidateTextureCache === "function")
          window._invalidateTextureCache();
        if (typeof window._syncTextureUI === "function")
          window._syncTextureUI();
      ...
      if (
        runtimeAssets &&
        runtimeAssets.textureSrc === textureSrc &&
        runtimeAssets.textureCanvas
      )
        commitTexture(runtimeAssets.textureCanvas);
      else if (typeof _dataURLToCanvas === "function")
        pendingAssets.push(
          _dataURLToCanvas(textureSrc).then(commitTexture, failTexture),
        );
    }
    const completeActivation = () => {
      ...
      window.dispatchEvent(
        new CustomEvent("brush-runtime-ready", {
          detail: { presetId: activatedPresetId },
        }),
      );
      return true;
    };
  ...
  function captureLiveState(t) {
    ...
    st.spacing = Math.round(
      (window._tsSpacing != null ? window._tsSpacing : 0.12) * 100,
    );
    st.roundness = Math.round(
      (window._tsRoundness != null ? window._tsRoundness : 1) * 100,
    );
    st.aa = !!brushAA;
  ...
  function restoreLiveState(t) {
    ...
    window._activeBrushPresetId = _activePresetId;
    const savedSettings = _isolatePresetTextureSettings(
      preset,
      normalizeBrushSettings(
        Object.assign(
          {},
          preset.settings || {},
          _presetSettings[_presetSettingsKey(presetId, t)] || {},
        ),
      ),
    );
    ...
    if (!preset.custom && preset.settings) {
      PRESET_STRUCTURAL_KEYS.forEach((k) => {
        if (k in preset.settings) savedSettings[k] = preset.settings[k];
      });
    }
    ...
    try {
      applyPresetSettings({
        ...
        runtimeAssets: preset._runtimeAssets,
        presetId,
      });
      restoreToolPresetSize(t, presetId, savedSettings["ts-size"]);
    } finally {
      _applyingPresetSettings = false;
    }
  }
  ...
  function selectPreset(id) {
    ...
    if (tool !== targetTool)
      setTool(targetTool, targetTool === "brush" ? "Brush" : "Eraser");
    ...
    const savedSettings = _isolatePresetTextureSettings(
      ...
      normalizeBrushSettings(
        Object.assign(
          {},
          p.settings || {},
          _presetSettings[_presetSettingsKey(p.id, targetTool)] || {},
        ),
      ),
    );
    ...
    if (!p.custom && p.settings) {
      PRESET_STRUCTURAL_KEYS.forEach((k) => {
        if (k in p.settings) savedSettings[k] = p.settings[k];
      });
    }
    _activePresetId = id;
    window._activeBrushPresetId = _activePresetId;
    ...
    try {
      applyPresetSettings({
        settings: savedSettings,
        runtimeAssets: p._runtimeAssets,
        presetId: id,
      });
  ...
  function refreshGrid() {
    document.querySelectorAll(".bp-item").forEach((el) => {
      const active = el.dataset.presetId === _activePresetId;
  ...
  async function _getBrushPresetPackSlugs() {
    try {
      const res = await fetch("assets/brush-presets/manifest.json", {
        cache: "no-store",
      });
      ...
      console.warn(
        "[brush-presets] failed to load preset manifest; using fallback list:",
        err,
      );
    }
  ...
  async function _loadBrushPresetPacks() {
    ...
    for (const slug of packSlugs) {
      ...
      try {
        ...
        if (!res.ok)
          throw new Error("settings.json fetch failed: " + res.status);
        const json = await res.json();
        const id = "pack:" + slug;
        if (allPresets().some((p) => p.id === id)) continue; // already loaded (e.g. hot-reload)
        const settings = builtinBrushSettings(
          Object.assign({}, json.settings || {}),
        );
        if (json.hasTip !== false)
          settings["ts-tip-url"] = base + (json.tipFile || "tip.png");
        const hasTexture =
          json.hasTexture === true ||
          (json.hasTexture == null && !!json.textureFile);
        if (hasTexture)
          settings["ts-texture-url"] =
            base + (json.textureFile || "texture.png");
        else {
          delete settings["ts-texture-url"];
          delete settings["ts-texture-dataurl"];
        }
        const preset = {
          ...
          preview: {
            shape:
              json.settings && json.settings["ts-roundness"] < 60
                ? "ellipse"
                : "circle",
            hardness:
              json.settings && json.settings["ts-hardness"] != null
                ? json.settings["ts-hardness"] / 100
                : 0.5,
          },
          settings,
        };
        const tipSrc =
          settings["ts-tip-dataurl"] || settings["ts-tip-url"] || null;
        const textureSrc =
          settings["ts-texture-dataurl"] || settings["ts-texture-url"] || null;
        const [tipCanvas, textureCanvas] = await Promise.all([
          tipSrc
            ? _dataURLToCanvas(tipSrc, true).catch(() => null)
            : Promise.resolve(null),
          textureSrc
            ? _dataURLToCanvas(textureSrc).catch(() => null)
            : Promise.resolve(null),
        ]);
        Object.defineProperty(preset, "_runtimeAssets", {
          value: { tipSrc, tipCanvas, textureSrc, textureCanvas },
          enumerable: false,
        });
        ...
        console.warn(
          '[brush-presets] failed to load preset pack "' + slug + '":',
          err,
        );
      }
    }
    ...
    if (
      restoredPresetId &&
      _activeRuntimePresetId !== restoredPresetId &&
      findPreset(restoredPresetId)
    ) {
  ...
  window.setTool = function (t, lbl) {
    const prevTool = tool;
    if (prevTool === "brush" || prevTool === "eraser")
      captureLiveState(prevTool);
    _origSetTool(t, lbl);
    if (t === "brush" || t === "eraser") {
      if (_activeTab !== t) {
        _activeTab = t;
        document
          .querySelectorAll(".bp-tool-tab")
          .forEach((b) => b.classList.toggle("active", b.dataset.bpTool === t));
        buildGrid();
      }
      activateToolRuntimeState(t);
    }
  ...
  if (addGroupBtn)
    addGroupBtn.onclick = () => {
      const name = (
        prompt("Name for the new brush folder:", "New Group") || ""
      ).trim();
      if (!name) return;
      _groups.push({
        id:
  ...
  if (addBrushBtn)
    addBrushBtn.onclick = () => {
      const name = (
        prompt(
          "Name this brush preset (saves the current size/hardness/flow/etc):",
          "My Brush",
        ) || ""
      ).trim();
      if (!name) return;
      ...
      const sizeEl = document.getElementById("ts-size"),
        spEl = document.getElementById("ts-spacing"),
        rdEl = document.getElementById("ts-roundness");
      ...
      const preset = {
        ...
        custom: true,
        preview: {
          shape: roundness < 60 ? "ellipse" : "circle",
        ...
        settings: {
          ...
          "ts-spacing": spacing,
          "ts-spacing-mode":
            document.getElementById("ts-spacing-mode")?.value || "fixed",
          "ts-rotation-mode":
            document.getElementById("ts-rotation-mode")?.value ||
            "fixed-rotation",
          "ts-angle": +(document.getElementById("ts-angle")?.value || 0),
          "ts-tip-roundness": +(
            document.getElementById("ts-tip-roundness")?.value || 100
          ),
          "ts-tip-flip-x": !!document.getElementById("ts-tip-flip-x")?.checked,
          "ts-tip-flip-y": !!document.getElementById("ts-tip-flip-y")?.checked,
          "ts-scatter-enabled":
            !!document.getElementById("ts-scatter-enabled")?.checked,
          "ts-scatter-amount": +(
            document.getElementById("ts-scatter-amount")?.value || 0
          ),
          "ts-scatter-count": +(
            document.getElementById("ts-scatter-count")?.value || 1
          ),
          "ts-roundness": roundness,
          "ts-aa": !!brushAA,
          "ts-aa-mode":
            window.brushAAMode && window.brushAAMode !== "none"
              ? window.brushAAMode
      ...
      const fi = document.createElement("input");
        ...
        reader.onload = (event) => {
          try {
            ...
            if (!brushes || !brushes.length) {
              showInfo(
                "No readable brush tips found in this ABR file.",
                "ABR Import",
              );
              return;
            }
            const fileBase =
              file.name.replace(/\.abr$/i, "").trim() || "Imported ABR";
            const detectedGroupName = String(brushes.groupName || "").trim();
            const resolvedBaseName = (brush) => {
              const parsed = String(brush.name || "")
                .trim()
                .replace(/\s+/g, " ");
              return !parsed || /^(?:ABR )?Brush\s+\d+$/i.test(parsed)
                ? fileBase
                : parsed;
            };
            ...
            function createPreset(brush, name, group) {
              ...
              const clampedSize = Math.max(
                sizeControl ? +sizeControl.min : 1,
                Math.min(sizeControl ? +sizeControl.max : 2000, brush.size),
              );
              const mapped = _mapABRValuesToSettings(
                brush.values,
                brush.features,
              );
              const preset = {
            ...
            showABRImportResults(brushes, file.name, () => {
              if (brushes.length === 1) {
                showABRPicker(brushes, file.name, (brush) => {
                  const general =
                    _groups.find((group) => group.default) || _groups[0];
            ...
            if (
              /Unsupported ABR version/i.test((error && error.message) || "")
            ) {
              showInfo(
                "This ABR version is not supported yet.",
                "ABR Import Error",
              );
            } else {
              showInfo(
                "Could not parse the ABR file: " + error.message,
                "ABR Import Error",
              );
            }
          }
  ...
  if (bpSz) {
    bpSz.addEventListener("input", () => {
      ...
      if (tsSz) tsSz.value = v;
      if (typeof refreshSizeUI === "function") refreshSizeUI();
      else if (bpSzVal) bpSzVal.textContent = v;
      if (tool === "brush" || tool === "eraser") captureLiveState(tool);
    });
  }

  // expose for external use
  window._brushPresets = {
    selectPreset,
    refreshGrid,
    switchTab,
    BRUSH_PRESETS,
...
const _ABR_FEATURE_MAP = {
  ...
  flipX: {
    label: "Flip X",
    needles: ["Flip X", "flipX"],
    control: "ts-tip-flip-x",
    applied: true,
    uiStatus: "implemented",
  },
  flipY: {
    label: "Flip Y",
    needles: ["Flip Y", "flipY"],
    control: "ts-tip-flip-y",
    applied: true,
    uiStatus: "implemented",
  },
  flow: {
  ...
  angleDynamics: {
    label: "Angle Dynamics",
    needles: ["angleDynamics"],
    control: "ts-rotation-mode",
    applied: false,
    uiStatus: "implemented",
  },
  roundness: {
  ...
  scatterAmount: {
    ...
    needles: ["useScatter", "Spcn"],
    control: "ts-scatter-amount",
    applied: true,
    uiStatus: "implemented",
    valueKey: "scatterAmount",
  },
  scatterBoth: {
    label: "Scatter - Both Axes",
    needles: ["bothAxes"],
    control: null,
    applied: false,
    uiStatus: "none",
    metadataOnly: true,
    valueKey: "scatterBothAxes",
  },
  count: {
    label: "Scatter Count",
    needles: ["Cnt"],
    control: "ts-scatter-count",
    applied: true,
    uiStatus: "implemented",
    valueKey: "scatterCount",
  },
  countJitter: {
    label: "Scatter Count Jitter",
    needles: ["countDynamics"],
    control: null,
    applied: false,
    uiStatus: "none",
    metadataOnly: true,
    valueKey: "countJitter",
  },
  scatterDynamics: {
    label: "Scatter Dynamics",
    needles: ["scatterDynamics"],
    control: null,
    applied: false,
    uiStatus: "none",
    metadataOnly: true,
    valueKey: "scatterDynamics",
  },
  texture: {
    label: "Texture",
    needles: ["Texture"],
  ...
  sizeJitter: {
    label: "Size Jitter (Shape Dynamics)",
    needles: ["Shape Dynamics", "sizeJitter", "useTipDynamics", "szVr"],
    control: "ts-size-jitter",
    applied: true,
    uiStatus: "implemented",
  },
  angleJitter: {
    label: "Angle Jitter",
    needles: ["angleJitter"],
    control: "ts-angle-jitter",
    applied: true,
    uiStatus: "implemented",
  },
  roundJitter: {
    label: "Roundness Jitter",
    needles: ["roundnessJitter"],
    control: "ts-round-jitter",
    applied: true,
    uiStatus: "implemented",
  },
  colorDynamics: {
    label: "Color Dynamics",
    needles: ["Color Dynamics"],
    control: "ts-hue-jitter",
    applied: false,
    uiStatus: "coming_soon",
  },
  transfer: {
    label: "Transfer (Opacity/Flow Jitter)",
    needles: ["Transfer"],
    control: "ts-min-flow",
    applied: false,
    uiStatus: "coming_soon",
  ...
  protectTex: {
    ...
    needles: ["Protect Texture"],
    control: null,
    applied: false,
    uiStatus: "none",
  },
  brushGroup: {
    label: "Brush Pose / Tilt Scale",
    needles: ["Brush Pose", "tiltScale"],
    control: null,
    applied: false,
    uiStatus: "none",
  },
...
function _scanABRFeatures(buffer) {
  ...
  try {
    ...
    for (let i = 0; i < chars.length; i++)
      chars[i] = String.fromCharCode((u[i * 2] << 8) | u[i * 2 + 1]);
    text = chars.join("");
  }
  ...
  let ascii = "";
  try {
    ascii = new TextDecoder("latin1").decode(buffer);
  } catch (e) {}
  const detected = {};
  for (const key in _ABR_FEATURE_MAP) {
    const f = _ABR_FEATURE_MAP[key];
    detected[key] = f.needles.some(
      (n) => text.indexOf(n) !== -1 || (ascii && ascii.indexOf(n) !== -1),
    );
  }
  return detected;
}
...
function _extractABRDescriptorValues(buffer) {
  ...
  function keyAt(off, k) {
    return (
      off >= 0 &&
      off + 4 <= n &&
      bytes[off] === k.charCodeAt(0) &&
      bytes[off + 1] === k.charCodeAt(1) &&
      bytes[off + 2] === k.charCodeAt(2) &&
      bytes[off + 3] === k.charCodeAt(3)
    );
  }
  function valueAfterKey(off) {
    if (keyAt(off, "doub") && off + 12 <= n) {
      const v = dv.getFloat64(off + 4, false);
      return isFinite(v) ? v : null;
    }
    if (keyAt(off, "UntF") && off + 16 <= n) {
      const v = dv.getFloat64(off + 8, false);
      return isFinite(v) ? v : null;
    }
    return null;
  }
...
function _mapABRValuesToSettings(values, features) {
  ...
  const f = features || {};
  const out = {};
  const hasSizeDynamics =
    v.useTipDynamics === true && Number(v.sizeVariation) > 0;
  if (v.useBrushSize === true || hasSizeDynamics)
    out["ts-size-control"] = "pressure";
  else if (v.useBrushSize === false || v.useTipDynamics === false)
    out["ts-size-control"] = "off";
  if (v.minimumDiameter != null)
    out["ts-min-size"] = Math.max(
      0,
      Math.min(100, Math.round(Number(v.minimumDiameter))),
    );
  if (v.roundness != null)
    out["ts-tip-roundness"] = Math.max(1, Math.min(100, Number(v.roundness)));
  if (v.minimumRoundness != null)
    out["ts-tip-min-roundness"] = Math.max(
      0,
      Math.min(100, Number(v.minimumRoundness)),
    );
  if (typeof v.roundnessDynamics === "boolean")
    out["ts-tip-roundness-dynamics"] = v.roundnessDynamics;
  ...
  if (hasSizeDynamics && v.sizeVariation != null)
    out["ts-size-jitter"] = Math.max(0, Math.min(100, Number(v.sizeVariation)));
  if (v.angleJitter != null)
    out["ts-angle-jitter"] = Math.max(0, Math.min(100, Number(v.angleJitter)));
  if (v.roundnessJitter != null)
    out["ts-round-jitter"] = Math.max(
      0,
      Math.min(100, Number(v.roundnessJitter)),
    );
  if (typeof v.flipX === "boolean") out["ts-tip-flip-x"] = v.flipX;
  if (typeof v.flipY === "boolean") out["ts-tip-flip-y"] = v.flipY;
  if (typeof v.useScatter === "boolean")
    out["ts-scatter-enabled"] = v.useScatter;
  if (v.scatterJitter != null || v.scatterAmount != null)
    out["ts-scatter-amount"] = Math.max(
      0,
      Math.min(100, Number(v.scatterJitter ?? v.scatterAmount)),
    );
  if (v.scatterCount != null)
    out["ts-scatter-count"] = Math.max(
      1,
      Math.min(50, Math.round(Number(v.scatterCount)) - 1),
    );
  if (typeof v.scatterBothAxes === "boolean")
    out["ts-scatter-both-axes"] = v.scatterBothAxes;
  ...
  if (v.opacity != null)
    out["ts-opacity"] = Math.max(1, Math.min(100, Math.round(v.opacity)));
  if (v.flow != null)
    out["ts-flow"] = Math.max(1, Math.min(100, Math.round(v.flow)));
  if (v.angle != null)
    out["ts-angle"] = ((-Math.round(v.angle) % 360) + 360) % 360;
  return out;
}
...
function _applyABRSettingsToUI(size, spacing, settings) {
  ...
  const clampedSize = Math.max(
    tsSz ? +tsSz.min : 1,
    Math.min(tsSz ? +tsSz.max : 2000, size),
  );
  if (tsSz) {
    tsSz.value = clampedSize;
    tsSz.dispatchEvent(new Event("input"));
  }
  if (bpSz) {
    bpSz.value = clampedSize;
    bpSz.dispatchEvent(new Event("input"));
  }
  if (typeof toolSizes !== "undefined" && typeof tool !== "undefined")
    toolSizes[tool] = clampedSize;
  ...
  for (const ctrlId in settings || {}) {
    if (ctrlId === "ts-scatter-both-axes") {
      window._tsScatterBothAxes = settings[ctrlId] !== false;
      continue;
    }
    if (ctrlId === "ts-tip-roundness-dynamics") {
      window.brushTipRoundnessDynamics = !!settings[ctrlId];
      continue;
    }
...
function _abrDetectedSettings(brush, mappedSettings) {
  ...
  return {
    ...
    useBrushSize:
      typeof values.useBrushSize === "boolean" ? values.useBrushSize : null,
    useTipDynamics:
      typeof values.useTipDynamics === "boolean" ? values.useTipDynamics : null,
    ...
    roundnessDynamics:
      typeof values.roundnessDynamics === "boolean"
        ? values.roundnessDynamics
        : null,
    scatter:
      typeof values.useScatter === "boolean"
        ? values.useScatter
        : !!features.scatter,
    scatterAmount: values.scatterAmount ?? null,
    scatterBothAxes:
      typeof values.scatterBothAxes === "boolean"
        ? values.scatterBothAxes
        : !!features.scatterBoth,
    scatterCount: values.scatterCount ?? null,
    countJitter: values.countJitter ?? null,
    countDynamics: {
      controlType: values.countDynamicsType ?? null,
      jitter: values.countJitter ?? null,
    },
    scatterDynamics: {
      controlType: values.scatterDynamicsType ?? null,
      jitter: values.scatterJitter ?? null,
    },
...
function parseABR(buffer) {
  ...
  function eof(n) {
    return offset + n > buffer.byteLength;
  }
  function readUint8() {
    if (eof(1)) throw new Error("Unexpected end of file");
    const v = dv.getUint8(offset);
    offset += 1;
    return v;
  }
  function readInt16() {
    if (eof(2)) throw new Error("Unexpected end of file");
    const v = dv.getInt16(offset, false);
    offset += 2;
    return v;
  }
  function readUint16() {
  ...
  function readInt32() {
    if (eof(4)) throw new Error("Unexpected end of file");
  ...
  function grayscaleToTipCanvas(pixels, w, h) {
    ...
    c.height = cropH;
    const ctx2 = c.getContext("2d");
    const id = ctx2.createImageData(cropW, cropH),
      d = id.data;
    for (let y = 0, p = 0; y < cropH; y++)
      for (let x = 0; x < cropW; x++, p += 4) {
  ...
  if (version === 1 || version === 2) {
    for (let i = 0; i < countOrSub && remaining() > 0; i++) {
      try {
        ...
        const width = right - left,
          height = bottom - top;
        if (height > 16384 || width <= 0 || height <= 0) {
          offset = blockEnd;
          continue;
        }
    ...
    let tipNamesFromDesc = [];
    {
      ...
      if (reachSection("desc")) {
        ...
        for (let textOff = 0; textOff < dscLen - 8; textOff++) {
          if (
            dscBytes[textOff] !== 0x54 ||
            dscBytes[textOff + 1] !== 0x45 ||
            dscBytes[textOff + 2] !== 0x58 ||
            dscBytes[textOff + 3] !== 0x54
          )
            continue;
          ...
          if (
            stringLength <= 0 ||
            stringLength > 500 ||
            textOff + 8 + stringLength * 2 > dscLen
          )
            continue;
          let value = "";
          ...
          key = key.trim();
          if (/(?:^|\s)Nm$/.test(key)) {
            if (value && !/^Sampled Brush\s+\d+$/i.test(value))
    ...
    if (!reachSection("samp")) {
      throw new Error(
        'Could not find a sampled-brush ("samp") section in this ABR file.',
      );
    }
    ...
    while (offset < sampEnd - 4) {
      let brushSize;
      try {
        brushSize = readUint32();
      } catch (e) {
        break;
      }
      let padded = brushSize;
      ...
      const fixedOffsets =
        countOrSub === 1 ? [47] : countOrSub === 2 ? [301] : [301, 47];
      let boundsOffset = -1;
      for (const skip of fixedOffsets) {
        const tryOff = headerStart + skip;
        if (tryOff + 19 > nextBrush || tryOff + 19 > buffer.byteLength)
          continue;
        const top = dv.getInt32(tryOff, false),
          left = dv.getInt32(tryOff + 4, false),
          bottom = dv.getInt32(tryOff + 8, false),
          right = dv.getInt32(tryOff + 12, false),
      ...
      if (boundsOffset === -1) {
        ...
        for (let o = headerStart; o <= scanLimit; o++) {
          const top = dv.getInt32(o, false),
            left = dv.getInt32(o + 4, false),
            bottom = dv.getInt32(o + 8, false),
            right = dv.getInt32(o + 12, false),
      ...
      if (boundsOffset !== -1) {
        ...
        try {
          ...
          /* skip this tip, keep scanning the rest of the file */
        }
      }
  ...
  const fileLevelName = String(values.name || "")
    .trim()
    .replace(/\s+/g, " ");
  const groupBaseName =
    fileLevelName.replace(/[\s._-]*\d{3,}[\s._-]*$/, "").trim() ||
    fileLevelName;
  ...
  function resolveTipName(rawName) {
    ...
    if (isFallback)
      return (
        fileLevelName ||
        (groupBaseName ? groupBaseName + " Brush" : "") ||
        "ABR Brush"
      );
    return n;
  }
  ...
  brushes.forEach((b, i) => {
    ...
    if (values.diameter != null && Number(values.diameter) > 0) {
      const photoshopDiameter = Number(values.diameter);
      b.referenceDiameter = photoshopDiameter;
      b.size = photoshopDiameter;
    }
    if (values.spacing != null)
      b.spacing = Math.max(1, Math.min(200, Math.round(values.spacing)));
    b.features = features;
    b.values = values;
  });
...
function showABRImportResults(brushes, filename, onContinue) {
  ...
  for (const key in _ABR_FEATURE_MAP) {
    ...
    if (f.applied) {
      ...
      if (v != null) {
        const unit =
          key === "angle"
            ? " deg"
            : key === "opacity" || key === "hardness" || key === "roundness"
              ? "%"
              : "";
        ...
        partial.push(
          f.label +
            " - detected but no readable value, left at current setting",
        );
      }
      continue;
    }
    ...
    if (f.metadataOnly) {
      const v = values[f.valueKey || key];
      if (v != null || features[key])
        partial.push(f.label + " - stored as metadata for future support");
      continue;
    }
    ...
    if (f.sampledInapplicable) {
      ...
      if (found)
        partial.push(
          f.label +
            " - only applies to Photoshop's procedural round brushes, not image-based tips (left at current setting)",
        );
      continue;
    }
    ...
    if (f.uiStatus === "coming_soon") {
      if (features[key])
        partial.push(
          f.label + " - detected, but this control isn't wired up yet",
        );
      continue;
    }
    unsupported.push(
      features[key]
        ? f.label + " - detected, no equivalent in this app"
        : f.label + " - no equivalent in this app",
    );
  }
  ...
  function section(title, items, color) {
    ...
    const rows = items
      .map((i) => `<li style="padding:2px 0;">${i}</li>`)
      .join("");
...
function showABRPicker(brushes, filename, onImport) {
  ...
  brushes.forEach((b, i) => {
    ...
    tile.title =
      b.name +
      "\n" +
      (window.formatBrushSize ? window.formatBrushSize(b.size) : b.size) +
      "  spacing:" +
      b.spacing +
      "%";
    ...
    sz.textContent = window.formatBrushSize
      ? window.formatBrushSize(b.size)
      : String(b.size);
    ...
    tile.addEventListener("click", () => {
      ...
      if (typeof window.setBrushTip === "function")
        window.setBrushTip(b.canvas, b.referenceDiameter, "abr-tip-selection");
      ...
      showInfo(
        'Loaded "' +
          ...
          '" (' +
          (window.formatBrushSize
            ? window.formatBrushSize(clampedSize)
            : clampedSize) +
          ") from " +
          filename +
          ".",
