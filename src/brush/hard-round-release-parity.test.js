...
function geometry(stabilization){
  const core=new PrototypeStrokeCore({brushSize:70,stabilization,zoom:1});
  const raw=Array.from({length:31},(_,i)=>({x:40+i*8,y:150+Math.sin(i/4)*45,pressure:.7,pointerType:'pen',timeStamp:i*8}));
  const live=[core.beginStroke(raw[0],{})];for(let i=1;i<raw.length;i++)live.push(...core.pushSamples([raw[i]]));
  return{live,finish:core.finishStroke(raw.at(-1)).segments};
}
function adapt(s){return Adapter.resolveSegmentRenderParams(s,{baseSize:70,minSizeFrac:0,curveKey:'linear',matchPrototypePressure:true,rgb:[0,0,0],aaMode:'medium',getEffectiveAlpha:()=>1});}
async function render(segments){const r=new PrototypeRenderer({width:340,height:300}),c=ctx(340,300);r._outCanvas={};r._outCtx=c;r.beginStroke();r.drawSegments(segments.map(adapt));await r.endStroke();return c.data;}
