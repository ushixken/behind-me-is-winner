...
(function () {
  ...
  function sizeLabelText(value) {
    return typeof window.formatBrushSize === "function"
  ...
  function _sizeMin() {
    const s = document.getElementById("ts-size");
    return s ? +s.min : 1;
  }
  function _sizeMax() {
    const s = document.getElementById("ts-size");
  ...
  function _applySize(px) {
    ...
    if (tsSz) tsSz.value = px;
    const bpSz = document.getElementById("bp-sz");
    if (bpSz) bpSz.value = px;
    ...
    if (typeof refreshSizeUI === "function") refreshSizeUI();
      ...
      const v = document.getElementById("ts-size-val");
      if (v) v.textContent = px;
    }
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (_lastClientX || _lastClientY)
      showPreview(_lastClientX, _lastClientY, toolSizes[tool] || 6);
  });
  ...
  canvasArea.addEventListener("pointercancel", _endResizeDrag, {
    capture: true,
  });
