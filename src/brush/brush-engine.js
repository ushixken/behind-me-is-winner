...
const TipReadbackExperiment = (function () {
  const requested =
    new URLSearchParams(location.search).get("tipReadback") || "control";
  const mode = ["control", "A", "B", "C", "D"].includes(requested)
      ? requested
      : "control",
    records = [];
  let strokeSerial = 0,
  ...
  return {
    ...
    contextOptions() {
      return mode === "A" ? { willReadFrequently: true } : undefined;
    },
    cacheHit(version) {
      const key = activeStroke + "|" + version;
      if (reportedHits.has(key)) return;
      reportedHits.add(key);
      record("tip-alpha-cache-hit", { tipVersion: version });
    },
    results() {
      return JSON.parse(JSON.stringify(records));
    },
    labelNextStroke(label) {
      window.BrushLatencyProfiler.enable(true);
      return window.BrushLatencyProfiler.labelNextStroke(label);
    },
    report() {
      return window.BrushLatencyProfiler.results().map((stroke) => {
        const get = (name) =>
          stroke.stages.find((stage) => stage.name === name);
        const display = stroke.points.find(
          (point) => point.name === "first-display-presented",
        );
        return {
          id: stroke.id,
          scenario: stroke.scenario,
          idleMs: stroke.idleMs,
          pointerdownToDisplay: display && display.fromPointerDown,
          tipGetImageData: get("tip-get-image-data")?.duration || 0,
          tipInitialization:
            get("tip-alpha-buffer-initialization")?.duration || 0,
          firstDab: get("first-dab-pipeline-total")?.duration || 0,
          rafWait: get("raf-wakeup-wait")?.duration || 0,
          presentation: get("presentation-total")?.duration || 0,
        };
      });
    },
    clear() {
    ...
    reloadUrls() {
      const base = location.href
        .replace(/([?&])tipReadback=[^&]*&?/, "$1")
        .replace(/[?&]$/, "");
      return Object.fromEntries(
        ["control", "A", "B", "C", "D"].map((value) => [
          value,
          base +
            (base.includes("?") ? "&" : "?") +
            "tipReadback=" +
...
const CustomTipCacheTrace = (function () {
  let enabled =
      new URLSearchParams(location.search).get("tipCacheTrace") === "1",
    strokeSerial = 0,
    activeStroke = 0,
    lastCallAt = 0,
    serial = 0;
  ...
  function objectId(value, prefix) {
    ...
    if (!ids.has(value)) ids.set(value, (prefix || "object") + "-" + ++serial);
  ...
  function tipCall(detail) {
    if (!enabled) return;
    const now = performance.now();
    record(
      "get-tip-alpha-buffer",
      Object.assign(
        { timeSincePreviousCall: lastCallAt ? now - lastCallAt : null },
        detail,
      ),
    );
    lastCallAt = now;
  ...
  function lifecycle(type) {
    record("lifecycle", {
      ...
      tipCanvasId: objectId(window.brushTipCanvas, "tip-canvas"),
    });
  }
  ...
  return {
    ...
    invalidated(detail) {
      record("cache-invalidated", detail);
    },
    results() {
      return JSON.parse(JSON.stringify(records));
    },
    ...
    summary() {
      const calls = records.filter(
        (item) => item.type === "get-tip-alpha-buffer",
      );
      return calls.map((item) => ({
        time: item.time,
        strokeId: item.strokeId,
        presetId: item.presetId,
        tipVersion: item.tipVersion,
        cacheKey: item.cacheKey,
        cacheHit: item.cacheHit,
        invalidationReason: item.invalidationReason,
        alphaBufferId: item.alphaBufferId,
        tipCanvasId: item.tipCanvasId,
        width: item.width,
        height: item.height,
        getImageDataDuration: item.getImageDataDuration,
        alphaExtractionDuration: item.alphaExtractionDuration,
        timeSincePreviousCall: item.timeSincePreviousCall,
    ...
    export() {
      return {
        calls: this.summary(),
        events: this.results(),
        strokes: window.TipReadbackExperiment
          ? window.TipReadbackExperiment.report()
          : [],
      };
    },
    reloadUrl() {
      const url = new URL(location.href);
      url.searchParams.set("tipCacheTrace", "1");
      url.searchParams.set("brushPerf", "1");
      return url.href;
    },
  };
})();
...
const CustomFirstDabTrace = (function () {
  ...
  function objectId(value) {
    if (!value || typeof value !== "object") return null;
    if (!ids.has(value)) ids.set(value, "tip-" + ++objectSerial);
    return ids.get(value);
  }
  function now() {
    return performance.now();
  }
  function beginStroke(detail) {
    if (!enabled) return;
    const t = now();
    active = {
      id: ++strokeSerial,
      label: nextLabel,
      startedAt: detail.entryAt || t,
      brush: detail.tip ? "custom-tip" : "procedural",
      presetId: detail.presetId || null,
      tipId: objectId(window.brushTipCanvas),
      tipVersion: window.brushTipVersion || 0,
      timeline: [
        { name: "pointerdown-handler-entry", at: detail.entryAt || t },
        {
          name: "preset-settings-resolved",
          at: t,
          detail: detail.settings || null,
        },
      ],
      samples: [],
      samplesBeforeFirstDab: 0,
      movementSamplesBeforeFirstDab: 0,
      firstDabDispatched: false,
      dabs: [],
    };
    nextLabel = null;
  }
  function endStroke() {
    if (!active) return;
    const prof =
        window.BrushLatencyProfiler && window.BrushLatencyProfiler.latest
          ? window.BrushLatencyProfiler.latest()
          : null,
      visible =
        prof &&
        prof.points &&
        prof.points.find((point) => point.name === "first-display-presented");
    active.firstVisibleAt = visible
      ? active.startedAt + visible.fromPointerDown
      : null;
    active.pointerdownToFirstVisible = visible ? visible.fromPointerDown : null;
    active.duration = now() - active.startedAt;
    strokes.push(active);
    active = null;
    dab = null;
  }
  function beginDab(detail) {
    if (!active) return;
    dab = {
      index: active.dabs.length + 1,
      startedAt: now(),
      detail,
      stages: [],
    };
    active.dabs.push(dab);
  }
  function endDab() {
    if (!dab) return;
    dab.duration = now() - dab.startedAt;
    dab = null;
  }
  function stage(name, start, detail) {
    if (!dab) return;
    dab.stages.push({ name, duration: now() - start, detail: detail || null });
  }
  function instant(name, detail) {
    if (!dab) return;
    dab.stages.push({ name, duration: 0, detail: detail || null });
  }
  function event(name, detail) {
    if (!active) return;
    if (
      name === "brush-parameters-resolved" &&
      active.timeline.some((item) => item.name === name)
    )
      return;
    const t = now();
    active.timeline.push({
      name,
      at: t,
      fromPointerdown: t - active.startedAt,
      detail: detail || null,
    });
  }
  function sample(detail) {
    if (!active) return;
    const item = Object.assign(
      { at: now(), fromPointerdown: now() - active.startedAt },
      detail || {},
    );
    active.samples.push(item);
    if (!active.firstDabDispatched) {
      active.samplesBeforeFirstDab++;
      if (item.source !== "pointerdown") active.movementSamplesBeforeFirstDab++;
    }
  }
  function firstDabDispatch(detail) {
    if (!active) return;
    active.firstDabDispatched = true;
    active.timeline.push({
      name: "first-dab-dispatch",
      at: now(),
      fromPointerdown: now() - active.startedAt,
      detail: Object.assign(
        {
          samplesBeforeFirstDab: active.samplesBeforeFirstDab,
          movementSamplesBeforeFirstDab: active.movementSamplesBeforeFirstDab,
        },
        detail || {},
      ),
    });
  ...
  function clear() {
    ...
    dab = null;
    enabled = true;
    if (window.BrushLatencyProfiler) {
      window.BrushLatencyProfiler.enable(true);
      window.BrushLatencyProfiler.clear();
    }
    return true;
  }
  function compare() {
    ...
    strokes.forEach((stroke) => {
      const key = stroke.brush + "|" + (stroke.label || "unlabelled"),
        first = stroke.dabs[0];
      if (!first) return;
      const group =
        groups[key] ||
        (groups[key] = {
          brush: stroke.brush,
          label: stroke.label || "unlabelled",
          trials: 0,
          stages: {},
        });
      group.trials++;
      first.stages.forEach((item) =>
        (group.stages[item.name] || (group.stages[item.name] = [])).push(
          item.duration,
        ),
      );
    });
    return Object.values(groups).map((group) => {
      ...
      Object.keys(group.stages).forEach((name) => {
        const a = group.stages[name].slice().sort((x, y) => x - y);
        out.stages[name] = {
          median: a[Math.ceil(a.length * 0.5) - 1],
          min: a[0],
          max: a[a.length - 1],
          p90: a[Math.ceil(a.length * 0.9) - 1],
        };
      });
      return out;
    });
...
const FirstDabLatencyProbe = (function () {
  ...
  function begin(detail) {
    if (!enabled) return;
    const at = (detail && detail.pointerdownAt) || now(),
      idleMs = lastStrokeAt ? at - lastStrokeAt : null;
    current = {
      stroke: ++serial,
      timestamp: new Date().toISOString(),
      trigger: visiblePending
        ? "tab-restore"
        : tipPending
          ? "preset-switch"
          : idleMs != null && idleMs >= 15000
            ? "idle"
            : serial > 1
              ? "warm"
              : "unknown",
      idleMs,
      visibility: document.visibilityState,
      firstAfterVisible: visiblePending,
      firstAfterSetBrushTip: tipPending,
      presetId: window._activeBrushPresetId || null,
      tipVersion: window.brushTipVersion || 0,
      layerType: (detail && detail.layerType) || null,
      renderer: "unknown",
      tipCache: null,
      measurements: {},
      setupDetails: {},
  ...
  function firstDabStart() {
    if (!current) return 0;
    current.capturingDab = true;
    current.firstDabAt = now();
    return current.firstDabAt;
  }
  function firstDabEnd(start) {
    if (!current || !current.capturingDab) return;
    current.measurements.stampDab = now() - start;
    current.firstDabGeneratedAt = now();
    current.capturingDab = false;
  }
  function cache(detail) {
    if (current && current.capturingDab) current.tipCache = detail;
  }
  function renderer(name, detail) {
    if (current && current.capturingDab) {
      current.renderer = name;
      if (detail) Object.assign(current, detail);
    }
  }
  function measure(name, start) {
    if (current && current.capturingDab && start)
      current.measurements[name] = now() - start;
  }
  function setupMeasure(name, start, detail) {
    if (!current || !start) return;
    current.measurements[name] =
      (current.measurements[name] || 0) + (now() - start);
    if (detail) Object.assign(current.setupDetails, detail);
  }
  function ensureKeyStage(name, start) {
    if (current && start)
      current.measurements[name] =
        (current.measurements[name] || 0) + (now() - start);
  }
  function renderTimelineStage(name, start, duration) {
    if (!current || !start) return;
    current.measurements[name] =
      (current.measurements[name] || 0) +
      (duration == null ? now() - start : duration);
  }
  function finishRenderTimeline(start) {
    if (!current || !start) return;
    const total = now() - start,
      names = [
        "renderTimelineCanvasClear",
        "renderTimelineBackgroundDrawing",
        "renderTimelineGridDrawing",
        "renderTimelineFrameDrawing",
        "renderTimelineLayerDrawing",
        "renderTimelineThumbnails",
        "renderTimelineDrawingMarks",
        "renderTimelinePlayhead",
        "renderTimelineSelectionsHighlights",
        "renderTimelineTextRendering",
        "renderTimelineScrollbarRendering",
        "renderTimelineOverlays",
      ];
    names.forEach((name) => {
      if (current.measurements[name] == null) current.measurements[name] = 0;
    });
    const measured = names.reduce(
        (sum, name) => sum + current.measurements[name],
        0,
      ),
      classified = Math.min(total, measured);
    current.measurements.renderTimeline = total;
    current.measurements.renderTimelineClassifiedTotal = classified;
    current.measurements.renderTimelineUnclassified = total - classified;
  }
  function finishEnsureKey(start, created) {
    if (!current || !start) return;
    const total = now() - start,
      names = [
        "ensureKeyCanvasAllocation",
        "ensureKeyFrameMapInsertion",
        "ensureKeyActiveCanvasClear",
        "ensureKeyCanvasCopyDrawImage",
        "ensureKeyRecompose",
        "ensureKeyUpdateOnion",
        "ensureKeyRenderTimeline",
        "ensureKeyUpdateStatus",
      ];
    names.forEach((name) => {
      if (current.measurements[name] == null) current.measurements[name] = 0;
    });
    const classified = names.reduce(
      (sum, name) => sum + current.measurements[name],
      0,
    );
    current.measurements.ensureKey = total;
    current.measurements.ensureKeyClassifiedTotal = classified;
    current.measurements.ensureKeyUnclassified = Math.max(
      0,
      total - classified,
    );
    current.setupDetails.ensureKeyCreatedNewKeyframe = !!created;
  }
  function finalizeSetup(at) {
    if (!current) return;
    const end = at || now(),
      total = end - current.pointerdownAt,
      names = [
        "eventValidationAndPreventDefault",
        "pendingPrewarmCancellation",
        "latencyHooksInitialization",
        "pressureAndStateInitialization",
        "coordinateMappingAndTransforms",
        "setPointerCapture",
        "pushUndo",
        "ensureKey",
        "eraserSetup",
        ...
        "selectionSetup",
        "ensureStrokeCanvas",
        "spacingAndFlowInitialization",
      ],
      classified = names.reduce(
        (sum, name) => sum + (current.measurements[name] || 0),
        0,
      );
    current.measurements.pointerdownToStampDabStart = total;
    current.measurements.unclassifiedSetup = Math.max(0, total - classified);
    current.measurements.classifiedSetupTotal = classified;
  }
  function beforeSchedule() {
    if (!current) return;
    const at = now();
    current.measurements.pointerdownToScheduleRecomposite =
      at - current.pointerdownAt;
    current.measurements.firstDabToScheduleRecomposite =
      current.firstDabGeneratedAt ? at - current.firstDabGeneratedAt : null;
    current.readyForPresentation = true;
  }
  function rafScheduled(at) {
    if (current && current.readyForPresentation) current.rafScheduledAt = at;
  }
  function rafCallback(at) {
    if (current && current.readyForPresentation)
      current.measurements.rafWait = at - (current.rafScheduledAt || at);
  }
  function recomposeStart() {
    return current && current.readyForPresentation ? now() : 0;
  }
  function displayComplete(recomposeStart, displayBlit) {
    ...
    current.measurements.totalToDisplayDrawImageReturn =
      now() - current.pointerdownAt;
    const report = current;
    current = null;
    reports.push(report);
    const cacheState = report.tipCache
        ? report.tipCache.hit
          ? "HIT"
          : "MISS"
        : "N/A",
    ...
    console.log(report);
    console.table(
      Object.assign({ renderer: report.renderer, tipCache: cacheState }, m),
    );
    console.log(
      "[BrushLatencySummary] trigger=" +
        report.trigger +
        " renderer=" +
        report.renderer +
        " tipCache=" +
        cacheState +
        " buildTipStamp=" +
        (m.buildTipStamp || 0).toFixed(3) +
        " stampDraw=" +
        (m.cachedStampDrawImage || 0).toFixed(3) +
        " pointerdownToSchedule=" +
        (m.pointerdownToScheduleRecomposite || 0).toFixed(3) +
        " rafWait=" +
        (m.rafWait || 0).toFixed(3) +
        " recompose=" +
        (m.recompose || 0).toFixed(3) +
        " displayBlit=" +
        (m.compCToDisplayCDrawImage || 0).toFixed(3) +
        " totalToDisplayCall=" +
        (m.totalToDisplayDrawImageReturn || 0).toFixed(3),
    );
    console.groupEnd();
  }
  document.addEventListener("visibilitychange", () => {
    if (!document.hidden) visiblePending = true;
  });
  return {
    enable(value = true) {
      enabled = !!value;
    ...
    displayComplete,
    tipChanged() {
      if (enabled) tipPending = true;
    },
    strokeComplete() {
      if (enabled) lastStrokeAt = now();
    },
    reports() {
...
function _brushDiagPerfBegin(strokeId) {
  if (!window.BrushDebugPerf) {
    _brushDiagPerfActive = null;
    return;
  }
  const custom = !!window.brushTipCanvas,
    hard = !!_hardRoundStrokeActive,
    tipVersion = window.brushTipVersion || 0;
  _brushDiagPerfActive = {
    strokeId,
    startTime: performance.now(),
    route: hard
      ? "migrated-hard-round"
      : custom
        ? "legacy-custom-tip-dabs"
        : "legacy-procedural-dabs",
    backend: hard ? "pending" : custom ? "canvas2d" : "canvas2d",
    customTip: custom,
    texture: !!window.brushTextureEnabled,
    rawSampleCount: 1,
    stabilizedSampleCount: 1,
    resolvedDabCount: 0,
    tinyDabCount: 0,
    normalDabCount: 0,
    radiusCount: 0,
    ...
    stampCacheMisses: 0,
    stampBuilds: 0,
    canvasDrawCallCount: 0,
    canvasGetImageDataCalls: 0,
    canvasPutImageDataCalls: 0,
    temporaryCanvasAllocations: 0,
    gpuInstanceCount: 0,
    gpuBatchCount: 0,
    gpuDrawCallCount: 0,
    gpuInitializationMs: 0,
    totalProcessingMs: 0,
    firstGpuInitializationThisStroke: false,
    cacheInvalidatedThisStroke: tipVersion !== _brushDiagPerfLastTipVersion,
    tipVersion,
    hardRoundRendererCreatedThisStroke: false,
    hardRoundRendererReusedThisStroke: false,
    hardRoundGpuInitThisStroke: false,
    hardRoundPipelineCreatedThisStroke: false,
    hardRoundGpuBuffersAllocatedThisStroke: 0,
    hardRoundGpuTexturesAllocatedThisStroke: 0,
    hardRoundCpuFallbackAllocatedThisStroke: false,
    hardRoundCpuFallbackBytesAllocated: 0,
    hardRoundOverlayConfiguredThisStroke: false,
    hardRoundResizeThisStroke: false,
    hardRoundBeginStrokeMs: 0,
    hardRoundFirstPresentMs: null,
  };
...
function _brushDiagPerfNote(type, detail) {
  const a = _brushDiagPerfActive;
  if (!window.BrushDebugPerf || !a) return;
  const d = detail || {};
  if (type === "raw-samples") a.rawSampleCount += d.count || 0;
  else if (type === "stabilized-sample") a.stabilizedSampleCount++;
  else if (type === "resolved") {
    const n = d.count || 1;
    a.resolvedDabCount += n;
    if (Number.isFinite(d.radius)) {
      a.radiusCount += n;
      a.radiusSum += d.radius * n;
    }
    const lo = Number.isFinite(d.minRadius) ? d.minRadius : d.radius,
      hi = Number.isFinite(d.maxRadius) ? d.maxRadius : d.radius;
    if (Number.isFinite(lo))
      a.minRadius = a.minRadius == null ? lo : Math.min(a.minRadius, lo);
    if (Number.isFinite(hi))
      a.maxRadius = a.maxRadius == null ? hi : Math.max(a.maxRadius, hi);
  } else if (type === "spacing" && Number.isFinite(d.step)) {
    a.spacingCount++;
    a.spacingSum += d.step;
    a.minSpacing =
      a.minSpacing == null ? d.step : Math.min(a.minSpacing, d.step);
    a.maxSpacing =
      a.maxSpacing == null ? d.step : Math.max(a.maxSpacing, d.step);
  } else if (type === "tiny") {
    a.tinyDabCount++;
    a.tinyCoverageCalls++;
  } else if (type === "normal") a.normalDabCount++;
  else if (type === "stamp-cache") {
    if (d.hit) a.stampCacheHits++;
    else a.stampCacheMisses++;
  ...
  else if (type === "gpu-init") {
    a.gpuInitializationMs += d.ms || 0;
    a.firstGpuInitializationThisStroke =
      a.firstGpuInitializationThisStroke || !!d.first;
  } else if (type === "hard-round-renderer") {
    a.hardRoundRendererCreatedThisStroke = !!d.created;
    a.hardRoundRendererReusedThisStroke = !d.created;
  } else if (type === "hard-round-begin") a.hardRoundBeginStrokeMs += d.ms || 0;
  else if (
    type === "hard-round-first-present" &&
    a.hardRoundFirstPresentMs == null
  )
    a.hardRoundFirstPresentMs = d.ms || 0;
  else if (type === "hard-round-cpu-allocation") {
    a.hardRoundCpuFallbackAllocatedThisStroke = true;
    a.hardRoundCpuFallbackBytesAllocated += d.bytes || 0;
  } else if (type === "hard-round-gpu-resources") {
    a.hardRoundGpuInitThisStroke = true;
    a.hardRoundPipelineCreatedThisStroke =
      a.hardRoundPipelineCreatedThisStroke || !!d.pipeline;
    a.hardRoundGpuBuffersAllocatedThisStroke += d.buffers || 0;
    a.hardRoundGpuTexturesAllocatedThisStroke += d.textures || 0;
  } else if (type === "hard-round-overlay-configured")
    a.hardRoundOverlayConfiguredThisStroke = true;
  else if (type === "hard-round-resize") a.hardRoundResizeThisStroke = true;
  else if (type === "gpu-state") {
    if (d.backend) a.backend = d.backend;
  } else if (type === "gpu-work") {
    a.gpuInstanceCount += d.instances || 0;
    a.gpuBatchCount += d.batches || 0;
    a.gpuDrawCallCount += d.drawCalls || 0;
...
function _brushDiagPerfEnd(strokeId) {
  const a = _brushDiagPerfByStroke.get(strokeId);
  if (!a) return;
  a.endTime = performance.now();
  a.strokeDurationMs = a.endTime - a.startTime;
  a.avgRadius = a.radiusCount ? a.radiusSum / a.radiusCount : null;
  a.avgSpacing = a.spacingCount ? a.spacingSum / a.spacingCount : null;
  a.minSpacingDistance = a.minSpacing;
  a.maxSpacingDistance = a.maxSpacing;
  a.avgSpacingDistance = a.avgSpacing;
  a.tinyCoverageCallCount = a.tinyCoverageCalls;
  a.stampBuildCount = a.stampBuilds;
  a.stampCacheHitCount = a.stampCacheHits;
  a.stampCacheMissCount = a.stampCacheMisses;
  a.getImageDataCount = a.canvasGetImageDataCalls;
  a.putImageDataCount = a.canvasPutImageDataCalls;
  a.temporaryCanvasCount = a.temporaryCanvasAllocations;
  a.initializationMs = a.gpuInitializationMs;
  a.strokeProcessingMs = a.totalProcessingMs;
  delete a.radiusSum;
  delete a.spacingSum;
  _brushDiagPerfRecords.push(a);
  if (_brushDiagPerfRecords.length > 24)
    _brushDiagPerfRecords.splice(0, _brushDiagPerfRecords.length - 24);
  _brushDiagPerfByStroke.delete(strokeId);
  if (_brushDiagPerfActive === a) _brushDiagPerfActive = null;
}
window.BrushAnalyzePerf = function () {
  const strokes = _brushDiagPerfRecords.map((x) => Object.assign({}, x));
  return {
    available: true,
    enabled: !!window.BrushDebugPerf,
    count: strokes.length,
    latest: strokes[strokes.length - 1] || null,
    strokes,
  };
};
...
Object.defineProperty(window, "brushTextureDepth", {
  configurable: true,
  get() {
    return window.brushTextureStrength;
  },
  set(value) {
    window.brushTextureStrength = value;
  },
});
...
try {
  const _savedLinePressureMode = localStorage.getItem(
    "animate.linePressureMode.v1",
  );
  if (_savedLinePressureMode === "fixed" || _savedLinePressureMode === "pen")
    _linePressureMode = _savedLinePressureMode;
} catch (_) {}
function getLinePressureMode() {
  return _linePressureMode;
}
function setLinePressureMode(mode) {
  _linePressureMode = mode === "fixed" ? "fixed" : "pen";
  try {
    localStorage.setItem("animate.linePressureMode.v1", _linePressureMode);
  } catch (_) {}
}
...
let _lineAAEnabled = true,
  _lineAAQuality = "medium";
...
try {
  ...
  if (value && typeof value === "object") {
    _lineAAEnabled =
      value.enabled !== false &&
      String(value.quality || "").toLowerCase() !== "none";
...
function _persistLineAA() {
  try {
    localStorage.setItem(
      LINE_AA_STORE_KEY,
      JSON.stringify({ enabled: _lineAAEnabled, quality: _lineAAQuality }),
    );
  } catch (_) {}
}
...
function getLineAASettings() {
  return { enabled: _lineAAEnabled, quality: _lineAAQuality };
}
function setLineAAEnabled(enabled) {
  _lineAAEnabled = !!enabled;
  _persistLineAA();
  _clearLineAACaches();
}
function setLineAAQuality(quality) {
  _lineAAQuality = _normalizeLineAAQuality(quality);
  _persistLineAA();
  _clearLineAACaches();
}
...
let _curveToolGesture = null,
  _curveGuideOverlay = null,
  _curveCommitPointerId = null;
let _linePreviewBounds = null,
  _linePreviewPreviousEndpoint = null;
let _linePreviewFrameId = 0,
  _linePreviewMoveSequence = 0,
  _linePreviewGeneration = 0;
let _lineDiagnosticCurrentT = 0,
  _lineEffectivePressureSamples = [];
...
let _strokeOwnerLayer = -1,
  _strokeOwnerFrame = -1,
  _endingForArtworkChange = false;
let _strokeSessionSerial = 0,
  _activeStrokeSession = 0;
function _traceStrokeLifecycle(event, detail) {
  if (!window.debugStrokeLifecycle) return;
  const trace =
    window.__strokeLifecycleTrace || (window.__strokeLifecycleTrace = []);
  trace.push(
    Object.assign(
      {
        event,
        sessionId: _activeStrokeSession,
        time: performance.now(),
        activeLayer: curLayer,
        activeFrame: curFrame,
      },
      detail || {},
    ),
  );
...
function _brushPerf() {
  return window.BrushLatencyProfiler && window.BrushLatencyProfiler.enabled
    ? window.BrushLatencyProfiler
    : null;
}

function _beginColorEraserStroke() {
  _colorEraserBase = null;
  _colorEraserOwnership = null;
  if (tool !== "eraser" || window.eraserMode !== "color") return;
  const layer = layers[curLayer];
  const colorModeAvailable =
    typeof window.eraserColorModeAvailable === "function"
      ? window.eraserColorModeAvailable()
      : !!(layer && layer.type === "smart-raster");
  if (!colorModeAvailable) {
    window.eraserMode = "normal";
    return;
  }
  _colorEraserBase = ctx.getImageData(0, 0, CW, CH);
  const styleId =
    typeof activeAdvancedStyleIdForPainting === "function"
      ? activeAdvancedStyleIdForPainting()
      : null;
  if (
    layer &&
    ...
    typeof window.SmartRasterLayer.beginStyleErase === "function"
  ) {
    _colorEraserOwnership = window.SmartRasterLayer.beginStyleErase(
      curLayer,
      curFrame,
      styleId,
    );
...
function _colorEraserDabRect(centerX, centerY, radiusX, radiusY) {
  const pad = 3,
    x = Math.max(0, Math.floor(centerX - radiusX - pad)),
    y = Math.max(0, Math.floor(centerY - radiusY - pad));
  const right = Math.min(CW, Math.ceil(centerX + radiusX + pad)),
    bottom = Math.min(CH, Math.ceil(centerY + radiusY + pad));
...
function _captureColorEraserDab(centerX, centerY, radiusX, radiusY) {
  if (
    !_colorEraserBase ||
    tool !== "eraser" ||
    window.eraserMode !== "color" ||
    !layers[curLayer] ||
    layers[curLayer].renderMode !== "style-layering"
  )
    return null;
  const rect = _colorEraserDabRect(centerX, centerY, radiusX, radiusY);
  return rect
    ? { rect, image: ctx.getImageData(rect.x, rect.y, rect.w, rect.h) }
    : null;
}
function _queueLiveColorEraserPreview(ownership, rect) {
  if (
    !ownership ||
    !ownership.lastDabChanged ||
    !ownership.lastDabChanged.length
  )
    return;
  for (let i = 0; i < ownership.lastDabChanged.length; i++)
    ownership.previewIndexes.add(ownership.lastDabChanged[i]);
  const pending = ownership.previewRect,
    right = rect.x + rect.w,
    bottom = rect.y + rect.h;
  if (!pending)
    ownership.previewRect = { x: rect.x, y: rect.y, w: rect.w, h: rect.h };
  else {
    const nextRight = Math.max(pending.x + pending.w, right),
      nextBottom = Math.max(pending.y + pending.h, bottom);
    pending.x = Math.min(pending.x, rect.x);
    pending.y = Math.min(pending.y, rect.y);
    pending.w = nextRight - pending.x;
    pending.h = nextBottom - pending.y;
  }
}
function _flushLiveColorEraserPreview() {
  ...
  if (
    !ownership ||
    !ownership.previewIndexes ||
    !ownership.previewIndexes.size ||
    !ownership.previewRect
  )
    return false;
  if (
    !layers[curLayer] ||
    layers[curLayer].renderMode !== "style-layering" ||
    typeof window.SmartRasterV4LiveColorErasePreview !== "function"
  ) {
    ownership.previewIndexes.clear();
    ownership.previewRect = null;
    return false;
  }
  const indexes = ownership.previewIndexes,
    rect = ownership.previewRect;
  window.SmartRasterV4LiveColorErasePreview({
    styleId: ownership.styleId,
...
function _filterColorEraserRegion(
  centerX,
  centerY,
  radiusX,
  radiusY,
  ...
  if (!_colorEraserBase || tool !== "eraser" || window.eraserMode !== "color")
    return;
  const rect = _colorEraserDabRect(centerX, centerY, radiusX, radiusY);
  if (!rect) return;
  const image = ctx.getImageData(rect.x, rect.y, rect.w, rect.h),
    ownership = _colorEraserOwnership;
  if (
    !ownership ||
    !window.SmartRasterLayer ||
    typeof window.SmartRasterLayer.applyStyleEraseRegion !== "function"
  )
    return;
  const exactBefore =
    beforeDab &&
    beforeDab.rect.x === rect.x &&
    beforeDab.rect.y === rect.y &&
    beforeDab.rect.w === rect.w &&
    beforeDab.rect.h === rect.h
      ? beforeDab.image
      : null;
  window.SmartRasterLayer.applyStyleEraseRegion(
    ownership,
    rect,
    image,
    _colorEraserBase,
  ...
  if (layers[curLayer] && layers[curLayer].renderMode === "style-layering")
    _queueLiveColorEraserPreview(ownership, rect);
}
function _endColorEraserStroke() {
  _flushLiveColorEraserPreview();
  if (
    _colorEraserOwnership &&
    window.SmartRasterLayer &&
    typeof window.SmartRasterLayer.finishStyleErase === "function"
  )
    window.SmartRasterLayer.finishStyleErase(_colorEraserOwnership);
  _colorEraserBase = null;
  _colorEraserOwnership = null;
}
...
function _beginEndTaperCapture() {
  ...
  if ((_getStartTaper() <= 0 && _getEndTaper() <= 0) || tool !== "eraser")
    return;
...
function _ensureStrokeCanvas() {
  ...
  const perf = _brushPerf(),
    started = perf ? performance.now() : 0;
  const w = activeC.width,
    h = activeC.height;
  const allocatedOrResized =
    !_strokeCanvas || _strokeCanvas.width !== w || _strokeCanvas.height !== h;
  if (
    !_strokeCanvas ||
  ...
  if (typeof _resetTexturedStrokeCanvas === "function")
    _resetTexturedStrokeCanvas();
  if (perf)
    perf.measure(
      "scratch-canvas-preparation",
      started,
      perf.canvasDetail(_strokeCanvas, _strokeCtx, {
        allocatedOrResized,
...
function _usesBrushPaintPipeline() {
  return tool === "brush" || tool === "line" || tool === "curve";
}
...
function _hrCaptureCanvas(canvas, label) {
  ...
  try {
    const cctx =
      canvas.getContext &&
      (canvas.getContext("2d") || (canvas.getContext("webgpu") && null));
    if (cctx && cctx.getImageData) {
...
function _hrStageDiagPixelDiff(canvasA, canvasB) {
  ...
  if (canvasA.width !== canvasB.width || canvasA.height !== canvasB.height) {
    return {
      sizeMismatch: true,
      aSize: [canvasA.width, canvasA.height],
      bSize: [canvasB.width, canvasB.height],
    };
  }
  ...
  try {
    ...
    rb.width = w;
    rb.height = h;
    const rca = ra.getContext("2d"),
      rcb = rb.getContext("2d");
    rca.drawImage(canvasA, 0, 0);
    rcb.drawImage(canvasB, 0, 0);
    da = rca.getImageData(0, 0, w, h).data;
    db = rcb.getImageData(0, 0, w, h).data;
  } catch (err) {
  ...
  for (let i = 0; i < da.length; i += 4) {
    const dr = Math.abs(da[i] - db[i]),
      dg = Math.abs(da[i + 1] - db[i + 1]),
      dbch = Math.abs(da[i + 2] - db[i + 2]),
      dal = Math.abs(da[i + 3] - db[i + 3]);
...
function _hrStageDiagFullCompare(
  labelA,
  captureA,
  canvasA,
  labelB,
...
function _hrStageDiagCaptureE1(artworkCompositeCanvas) {
  ...
  cap.E1 = _hrStageCaptureFull(
    artworkCompositeCanvas,
    "E1-artworkCompositeC-post-layer-draw",
  );
...
function _hrStageDiagCaptureE3(displayCanvas) {
  ...
  cap.diffsE = {
    DtoE1: {
      from: "D-activeC-post-commit",
      to: "E1-artworkCompositeC-post-layer-draw",
      summary: _hrDiffCaptures(cap.D, cap.E1),
    },
    E1toE2: _hrStageDiagFullCompare(
      "E1-artworkCompositeC-post-layer-draw",
      cap.E1,
      cap._E1canvas,
      "E2-compC-post-artwork-composite",
      cap.E2,
      cap._E2canvas,
    ),
    E2toE3: _hrStageDiagFullCompare(
      "E2-compC-post-artwork-composite",
      cap.E2,
      cap._E2canvas,
      "E3-displayC-final-visible",
      cap.E3,
      cap._E3canvas,
    ),
  };
  console.log(
    "[HR-STAGE-DIAG 11A.37] D->E1->E2->E3 captured. Inspect window.HardRoundStageCapture (E1/E2/E3/diffsE) or call window.HardRoundStageDiagSummary().",
  );
  ...
  try {
    _hrStageDiagE2E3BrushRegion(cap);
  } catch (err) {
    console.warn("[HR-STAGE-DIAG 11A.38] brush-region compare failed", err);
  }
}
function _hrStageDiagE2E3BrushRegion(cap) {
  ...
  const boundsSrc =
    cap.D && cap.D.bounds ? cap.D : cap.E1 && cap.E1.bounds ? cap.E1 : null;
  if (!boundsSrc || !boundsSrc.bounds) {
    console.warn("[HR-STAGE-DIAG 11A.38] no stroke bounds available on D/E1");
    return null;
  }
  ...
  const off = boundsSrc.region
    ? { x: boundsSrc.region.rx, y: boundsSrc.region.ry }
    : { x: 0, y: 0 };
  const b = boundsSrc.bounds;
  const pad = 4;
  const e2c = cap._E2canvas,
    e3c = cap._E3canvas;
  const W = e2c.width,
    H = e2c.height;
  ...
  const rw = rMaxX - rx + 1,
    rh = rMaxY - ry + 1;
  if (rw <= 0 || rh <= 0) {
    console.warn("[HR-STAGE-DIAG 11A.38] empty brush region");
    return null;
  }
  let e2Data, e3Data;
  try {
    ...
    console.warn("[HR-STAGE-DIAG 11A.38] region read failed", err);
    return null;
  }
  let differingPixelCount = 0,
    maxChannelDiff = 0;
  ...
  for (let yy = 0; yy < rh; yy++) {
    for (let xx = 0; xx < rw; xx++) {
      ...
      const dr = Math.abs(e2Data[idx] - e3Data[idx]),
        dg = Math.abs(e2Data[idx + 1] - e3Data[idx + 1]),
        dbch = Math.abs(e2Data[idx + 2] - e3Data[idx + 2]),
        dal = Math.abs(e2Data[idx + 3] - e3Data[idx + 3]);
      const pixelMax = Math.max(dr, dg, dbch, dal);
      if (pixelMax > 0) {
        differingPixelCount++;
        if (pixelMax > maxChannelDiff) maxChannelDiff = pixelMax;
  ...
  const diffBoundsGlobal =
    maxX >= minX
      ? { minX: rx + minX, minY: ry + minY, maxX: rx + maxX, maxY: ry + maxY }
      : null;
  ...
  const fullCanvasCount =
    cap.diffsE && cap.diffsE.E2toE3 && cap.diffsE.E2toE3.pixels
      ? cap.diffsE.E2toE3.pixels.differingPixelCount
      : null;
  const allDiffsInBrushRegion =
    fullCanvasCount != null ? differingPixelCount === fullCanvasCount : null;
  ...
  if (cap._E1canvas) {
    try {
      ...
      for (let i = 0; i < e1Data.length; i += 4) {
        if (e1Data[i + 3] === 255) {
          opaquePixels++;
          const dr = Math.abs(e1Data[i] - e2Data[i]),
            dg = Math.abs(e1Data[i + 1] - e2Data[i + 1]),
            dbc = Math.abs(e1Data[i + 2] - e2Data[i + 2]);
      ...
      e1OpaqueCheck = {
        ...
        opaqueMismatches,
        opaqueMaxDiff,
        note: "Compares E1 vs E2 RGB only where E1 alpha=255 (background cannot show through there, so this is a valid raw comparison; partial-alpha edge pixels are excluded since those genuinely need background compositing to compare fairly).",
      };
    } catch (err) {
      e1OpaqueCheck = { error: String(err) };
    }
  }
  ...
  console.log(
    "[HR-STAGE-DIAG 11A.38] brush-region E2->E3 compare done. Print with: window.HardRoundStageCapture.brushRegionE2E3",
  );
...
function _hrHashActiveCRegion(x, y, w, h, label) {
  ...
  try {
    ...
    console.warn("[HR-ACTIVEC-PROBE] read failed", label, err);
...
function _hrHashCanvas(canvas, label) {
  ...
  try {
    ...
    console.warn("[11A.20] hash failed", label, err);
    return { label, error: String(err) };
  }
  const w = canvas.width,
    h = canvas.height;
...
function _hrDiffHashes(a, b) {
  if (!a || !b) return null;
  if (a.error || b.error)
    return { error: true, aError: a.error, bError: b.error };
  if (a.w !== b.w || a.h !== b.h)
    return { sizeMismatch: true, aSize: [a.w, a.h], bSize: [b.w, b.h] };
  return {
    hashEqual: a.hash === b.hash,
...
function _hrRunCaptureSummary() {
  ...
  const bc = _hrDiffCaptures(c.B, c.C),
    cd = _hrDiffCaptures(c.C, c.D),
    presentVsResolve = _hrDiffCaptures(c.A1_present, c.A2_resolve);
...
function _hrStageDiagEnabled() {
  return !!window.HardRoundStageDiag;
}
function _hrStageDiagReset() {
  window.HardRoundStageCapture = {
    A: null,
    B: null,
    C: null,
    D: null,
...
function _hrStageDiagCaptureA(canvas) {
  ...
  window.HardRoundStageCapture.A = _hrStageCaptureFull(
    canvas,
    "A-gpu-resolved-pre-copy",
  );
}
function _hrStageDiagCaptureB(canvas) {
  if (!_hrStageDiagEnabled()) return;
  if (!window.HardRoundStageCapture) _hrStageDiagReset();
  window.HardRoundStageCapture.B = _hrStageCaptureFull(
    canvas,
    "B-strokeCanvas-pre-commit",
  );
}
...
function _hrStageDiagCaptureC(activeCanvas, rect) {
  if (!_hrStageDiagEnabled()) return;
  if (!window.HardRoundStageCapture) _hrStageDiagReset();
  window.HardRoundStageCapture.C = _hrStageCaptureRegion(
    activeCanvas,
    "C-activeC-pre-commit",
    rect,
  );
}
function _hrStageDiagCaptureD(activeCanvas, rect, detail) {
  if (!_hrStageDiagEnabled()) return;
  if (!window.HardRoundStageCapture) _hrStageDiagReset();
  window.HardRoundStageCapture.D = _hrStageCaptureRegion(
    activeCanvas,
    "D-activeC-post-commit",
    rect,
  );
...
function _hrStageDiagMaybeCaptureE() {
  ...
  cap.pendingE = false;
  const displayCanvas =
    typeof window.HardRoundStageDiagDisplayCanvas === "function"
...
(function _hrStageDiagPatchRecomposite() {
  ...
  const wrapped = function (...args) {
    ...
    try {
      _hrStageDiagMaybeCaptureE();
    } catch (err) {
      console.warn("[HR-STAGE-DIAG] stage E capture failed", err);
    }
    return result;
  };
  ...
  try {
    ...
    /* recomposite may be const in some builds */
  }
  try {
    window.recomposite = wrapped;
  } catch (err) {}
})();
window.HardRoundStageDiagSummary = function () {
  ...
  if (!cap) {
    console.log(
      "[HR-STAGE-DIAG] no capture yet -- enable window.HardRoundStageDiag and draw a stroke",
    );
    return null;
  }
  ...
  console.log(
    "[HR-STAGE-DIAG] E (display, post-recomposite, legacy monkey-patch path):",
    cap.E,
  );
  console.log(
    "[HR-STAGE-DIAG 11A.37] E1 (artworkCompositeC, post layer-draw):",
    cap.E1,
  );
  console.log(
    "[HR-STAGE-DIAG 11A.37] E2 (compC, post artwork-composite):",
    cap.E2,
  );
  console.log(
    "[HR-STAGE-DIAG 11A.37] E3 (displayC, final visible surface):",
    cap.E3,
  );
  console.log(
    "[HR-STAGE-DIAG] commit detail (globalAlpha/compositeOp/etc used by _commitStrokeCanvas):",
    cap.commitDetail,
  );
  console.log("[HR-STAGE-DIAG] diffs A->B / C->D / D->E (legacy):", cap.diffs);
  console.log(
    "[HR-STAGE-DIAG 11A.37] diffs D->E1 / E1->E2 / E2->E3:",
    cap.diffsE,
  );
  return cap;
};
function _commitStrokeCanvas() {
  ...
  const commitSession = _activeStrokeSession,
    commitLayer = curLayer,
    commitFrame = curFrame;
  _traceStrokeLifecycle("commit-start", {
    sessionId: commitSession,
    sourceLayer: commitLayer,
    sourceFrame: commitFrame,
    dirtyRect: _strokeDirty
      ? {
          minX: _strokeDirty.minX,
          minY: _strokeDirty.minY,
          maxX: _strokeDirty.maxX,
          maxY: _strokeDirty.maxY,
        }
      : null,
  });
  ...
  const styleId =
    typeof activeAdvancedStyleIdForPainting === "function"
      ? activeAdvancedStyleIdForPainting()
      : null;
  const brushBlendMode =
    _usesBrushPaintPipeline() && typeof window.brushBlendMode === "string"
      ? window.brushBlendMode
      : "normal";
  const smartOwnership =
    _usesBrushPaintPipeline() &&
    styleId &&
    typeof advancedPalettePaintingEnabled === "function" &&
    advancedPalettePaintingEnabled() &&
    layers[curLayer] &&
    layers[curLayer].type === "smart-raster";
  const ownershipDirtyRect = smartOwnership ? _consumeStrokeDirtyRect() : null;
  const ownershipBefore = smartOwnership
    ? ownershipDirtyRect
      ? ctx.getImageData(
          ownershipDirtyRect.x,
          ownershipDirtyRect.y,
          ownershipDirtyRect.w,
          ownershipDirtyRect.h,
        )
  ...
  // empty test layer).
  const _hrStage36Rect = _strokeDirty
    ? {
        minX: _strokeDirty.minX,
        minY: _strokeDirty.minY,
        maxX: _strokeDirty.maxX,
  ...
  _hrStageDiagCaptureD(
    activeC,
    _hrStage36Rect,
    window.HardRoundStageCapture && window.HardRoundStageCapture.commitDetail,
  );
  if (smartOwnership && typeof commitSmartRasterBrush === "function") {
    _traceStrokeLifecycle("smart-metadata-start", {
      sessionId: commitSession,
      sourceLayer: commitLayer,
      sourceFrame: commitFrame,
      dirtyRect: ownershipDirtyRect,
    });
    commitSmartRasterBrush(
      src,
      styleId,
      brushOpacity,
      ownershipDirtyRect,
    ...
    _traceStrokeLifecycle("smart-metadata-end", {
      sessionId: commitSession,
      sourceLayer: commitLayer,
      sourceFrame: commitFrame,
      dirtyRect: ownershipDirtyRect,
  ...
  _traceStrokeLifecycle("commit-end", {
    sessionId: commitSession,
    sourceLayer: commitLayer,
    sourceFrame: commitFrame,
    smartRaster: !!(
      layers[commitLayer] && layers[commitLayer].type === "smart-raster"
    ),
  });
  _traceStrokeLifecycle("temporary-stroke-cleared", {
    sessionId: commitSession,
    sourceLayer: commitLayer,
    sourceFrame: commitFrame,
  });
...
function _getLiveStrokePreview() {
  const perf = _brushPerf(),
    previewTotalStart = perf ? performance.now() : 0;
  const w = activeC.width,
    h = activeC.height;
  const previewAllocated =
    !_strokePreviewCanvas ||
  ...
  if (perf)
    perf.measure(
      "live-preview-buffer-preparation",
      previewBufferStart,
      perf.canvasDetail(_strokePreviewCanvas, _strokePreviewCtx, {
        allocatedOrResized: previewAllocated,
        source: "activeC",
        getImageData: false,
      }),
    );
  if (
    ...
    _hardRoundPendingCpuFinishingContexts.size > 0
  ) {
    for (const finishingCtx of _hardRoundPendingCpuFinishingContexts.values()) {
      const finSrc =
        finishingCtx.resolvedCanvas || finishingCtx.finishingPreviewCanvas;
      if (finSrc) {
        _strokePreviewCtx.save();
        _strokePreviewCtx.globalAlpha =
          finishingCtx.opacity != null
            ? Math.max(0, Math.min(1, finishingCtx.opacity))
            : 1;
        _strokePreviewCtx.globalCompositeOperation =
          finishingCtx.compositeOperation || "source-over";
        _strokePreviewCtx.drawImage(finSrc, 0, 0);
        if (finishingCtx.blendMode === "add-glow") {
          _strokePreviewCtx.globalAlpha =
            (finishingCtx.opacity != null ? finishingCtx.opacity : 1) * 0.65;
          _strokePreviewCtx.drawImage(finSrc, 0, 0);
        }
  ...
  if (_inStroke && _strokeCanvas && !currentStrokeAlreadyRepresented) {
    const textureStart = perf ? performance.now() : 0,
      textureCanvasExisted = !!_texturedStrokeCanvas;
    const src = _getTexturedStrokeCanvas(_strokeCanvas, false);
    if (perf)
      perf.measure("texture-mask-processing", textureStart, {
        canvas: "textured-stroke",
        width: src.width,
        height: src.height,
        enabled: !!window.brushTextureEnabled,
        cacheHit: !window.brushTextureEnabled || textureCanvasExisted,
        getImageData: !!window.brushTextureEnabled,
      });
    ...
    const styleId =
      typeof activeAdvancedStyleIdForPainting === "function"
        ? activeAdvancedStyleIdForPainting()
        : null;
    if (perf && layers[curLayer] && layers[curLayer].type === "smart-raster")
      perf.measure("smart-raster-active-style-resolution", smartStyleStart, {
        styleId,
      });
    const isSmartRaster =
      _usesBrushPaintPipeline() &&
    ...
    if (
      ...
      layers[curLayer] &&
      layers[curLayer].renderMode === "style-layering" &&
      typeof window.SmartRasterV4LivePaintPreview === "function"
    ) {
      let liveMask = src;
      const selectionStart = perf ? performance.now() : 0;
      if (window.SelectionScope) liveMask = SelectionScope.clipCanvas(liveMask);
      if (perf)
        perf.measure("selection-clipping", selectionStart, {
          active: !!window.SelectionScope,
          width: w,
          height: h,
          getImageData: false,
        });
      ...
      const rect = dirty
        ? {
            x: Math.max(0, Math.floor(dirty.minX)),
            y: Math.max(0, Math.floor(dirty.minY)),
            w:
              Math.min(w, Math.ceil(dirty.maxX)) -
              Math.max(0, Math.floor(dirty.minX)),
            h:
              Math.min(h, Math.ceil(dirty.maxY)) -
              Math.max(0, Math.floor(dirty.minY)),
          }
        : null;
      const smartStart = perf ? performance.now() : 0;
      const live = window.SmartRasterV4LivePaintPreview({
        maskCanvas: liveMask,
        targetCanvas: _strokePreviewCanvas,
        styleId,
        opacity: brushOpacity,
        rect,
        nonDestructive: _disposableDabPrewarm,
      });
      if (perf)
        perf.measure("smart-raster-contribution-preview", smartStart, {
          styleId,
          rect,
          getImageData: true,
          tiles: (live && live.tiles && live.tiles.length) || 0,
        });
      const previewSwapStart = perf ? performance.now() : 0;
      if (live && live.success) {
        if (perf) {
          perf.measure("smart-raster-preview-canvas-swap", previewSwapStart, {
            target: "stroke-preview",
            tiles: (live && live.tiles && live.tiles.length) || 0,
          });
          perf.measure("live-preview-total", previewTotalStart, {
            path: "smart-raster-v4",
            width: w,
            height: h,
          });
        }
    ...
    if (isSmartRaster) {
      ...
      let rgba = null;
      if (
        window.PaletteDocker &&
        typeof window.PaletteDocker.findAdvancedStyleById === "function"
      ) {
      ...
      if (rgba) {
        ...
        if (
          ...
          _srPreviewTintCanvas.width = w;
          _srPreviewTintCanvas.height = h;
          _srPreviewTintCtx = _srPreviewTintCanvas.getContext("2d", {
            willReadFrequently: true,
          });
        } else {
    ...
    _strokePreviewCtx.restore();
    if (perf)
      perf.measure("live-stroke-compositing", blendStart, {
        blendMode: _usesBrushPaintPipeline() ? window.brushBlendMode : "eraser",
        opacity: brushOpacity,
        width: w,
        height: h,
      });
...
window.addEventListener("project-loaded", () => {
  _aaDabCache.clear();
  _softRoundMaskCache.clear();
  _tipDabCache.clear();
  _stampCache.clear();
  if (typeof window._invalidateTextureCache === "function")
    window._invalidateTextureCache();
...
function _prewarmLiveStrokeComposition() {
  if (
    ...
    drawing ||
    !activeC ||
    typeof _getLiveStrokePreview !== "function"
  )
    return false;
  ...
  try {
    ...
    const x = Math.max(
      0,
      Math.min(activeC.width - 1, Math.floor(activeC.width / 2)),
    );
    const y = Math.max(
      0,
      Math.min(activeC.height - 1, Math.floor(activeC.height / 2)),
    );
    ...
    if (_strokeCtx && _strokeCanvas)
      _strokeCtx.clearRect(0, 0, _strokeCanvas.width, _strokeCanvas.height);
    if (_strokePreviewCtx && _strokePreviewCanvas)
      _strokePreviewCtx.clearRect(
        0,
        0,
        _strokePreviewCanvas.width,
        _strokePreviewCanvas.height,
      );
    if (_srPreviewTintCtx && _srPreviewTintCanvas)
      _srPreviewTintCtx.clearRect(
        0,
        0,
        _srPreviewTintCanvas.width,
        _srPreviewTintCanvas.height,
      );
    if (_texturedStrokeCtx && _texturedStrokeCanvas)
      _texturedStrokeCtx.clearRect(
        0,
        0,
        _texturedStrokeCanvas.width,
        _texturedStrokeCanvas.height,
      );
  }
}
...
function _prewarmRealDisposableDab() {
  if (
    ...
    tool !== "brush" ||
    !activeC ||
    typeof recomposite !== "function"
  )
    return {
      success: false,
      error: "Select Brush and finish the active stroke first.",
    };
  const perf = _brushPerf(),
    totalStart = performance.now();
  const saved = {
    frameDirty: _frameDirty,
    strokeDirty: _strokeDirty,
    texPending: _texPendingRect,
    inStroke: _inStroke,
    flowSpacing: _flowSpacingRatio,
    isPen: _isDrawingWithPen,
    currentPressure,
    _smoothedPressure,
    lastKnownPressure: _lastKnownPressure,
    strokeFirstSample: _strokeFirstSample,
    strokeDabCount: _strokeDabCount,
    strokeDist: _strokeDistSoFar,
    autoPrev: _autoHardRoundPrevDab,
    rotationValid: _rotationPrevValid,
    rotationPrevX: _rotationPrevX,
    rotationPrevY: _rotationPrevY,
    rotationDirection: _rotationDirection,
    disposablePrewarm: _disposableDabPrewarm,
  };
  let rect = null,
    alphaPixels = 0;
  try {
    ...
    currentPressure = 1;
    _smoothedPressure = 1;
    _lastKnownPressure = 1;
    _strokeFirstSample = true;
    _strokeDabCount = 0;
    _strokeDistSoFar = 0;
    ...
    const size = Math.max(1, getBrushSize()),
      margin = Math.min(
        Math.max(4, Math.ceil(size * 2)),
        Math.max(4, Math.floor(Math.min(activeC.width, activeC.height) / 2)),
      );
    const x = Math.max(
        margin,
        Math.min(activeC.width - margin, Math.floor(activeC.width / 2)),
      ),
      y = Math.max(
        margin,
        Math.min(activeC.height - margin, Math.floor(activeC.height / 2)),
      );
    _flowSpacingRatio = _initialDabSpacingRatio(synthetic, 1);
    if (perf)
      perf.measure("real-dab-preparation", prepareStart, {
        size,
        hardness: brushHardness,
        opacity: brushOpacity,
        flow: brushFlow,
        spacing: window.brushSpacing,
    ...
    if (!_strokeDirty)
      throw new Error("Production stamp produced no dirty bounds.");
    rect = {
      x: Math.max(0, Math.floor(_strokeDirty.minX)),
      y: Math.max(0, Math.floor(_strokeDirty.minY)),
    };
    rect.w = Math.min(activeC.width, Math.ceil(_strokeDirty.maxX)) - rect.x;
    rect.h = Math.min(activeC.height, Math.ceil(_strokeDirty.maxY)) - rect.y;
    const verifyStart = perf ? performance.now() : 0,
      pixels = _strokeCtx.getImageData(rect.x, rect.y, rect.w, rect.h).data;
    for (let i = 3; i < pixels.length; i += 4) if (pixels[i]) alphaPixels++;
    if (perf)
      perf.measure("real-dab-alpha-verification", verifyStart, {
        rect,
        alphaPixels,
        getImageData: true,
      });
    if (!alphaPixels)
      throw new Error("Production stamp did not produce non-zero alpha.");
    ...
    if (layers[curLayer] && layers[curLayer].type === "smart-raster") {
      const preview = _getLiveStrokePreview(),
        src = _getTexturedStrokeCanvas(_strokeCanvas, false),
        blendStart = perf ? performance.now() : 0;
      _strokePreviewCtx.save();
      _strokePreviewCtx.globalAlpha = Math.max(0, Math.min(1, brushOpacity));
      _drawBrushComposite(_strokePreviewCtx, src);
      _strokePreviewCtx.restore();
      if (perf)
        perf.measure("live-stroke-compositing", blendStart, {
          path: "smart-raster-disposable-explicit",
          width: preview.width,
    ...
    _frameDirty = saved.frameDirty;
    _strokeDirty = saved.strokeDirty;
    _texPendingRect = saved.texPending;
    _inStroke = saved.inStroke;
    _flowSpacingRatio = saved.flowSpacing;
    _isDrawingWithPen = saved.isPen;
    currentPressure = saved.currentPressure;
    _smoothedPressure = saved.smoothedPressure;
    _lastKnownPressure = saved.lastKnownPressure;
    _strokeFirstSample = saved.strokeFirstSample;
    _strokeDabCount = saved.strokeDabCount;
    _strokeDistSoFar = saved.strokeDist;
    _autoHardRoundPrevDab = saved.autoPrev;
    _rotationPrevValid = saved.rotationValid;
    _rotationPrevX = saved.rotationPrevX;
    _rotationPrevY = saved.rotationPrevY;
    _rotationDirection = saved.rotationDirection;
    _disposableDabPrewarm = saved.disposablePrewarm;
    if (perf)
      perf.measure("real-dab-prewarm-total", totalStart, { rect, alphaPixels });
  }
}
...
function _currentAAMode() {
  if (tool === "line" || tool === "curve")
    return _lineAAEnabled ? _lineAAQuality : "none";
  return _normalizeAAMode(
    typeof window !== "undefined" ? window.brushAAMode : null,
  );
}
...
function _quant(v, step) {
  return Math.round(v / step) * step;
}
function _quantAlpha(v) {
  return _quant(v, v < 0.1 ? _Q_ALPHA_LOW : _Q_ALPHA);
}
...
function _isStandardProceduralSoftRound() {
  return (
    tool === "brush" &&
    window._activeBrushPresetId === "soft-round" &&
...
function _buildSoftRoundMask(rRaw) {
  ...
  const hardness =
    Math.round(Math.max(0, Math.min(1, brushHardness)) * 1000) / 1000;
  ...
  const softPerf = _brushPerf();
  if (softPerf)
    softPerf.point(
      cached ? "soft-round-mask-cache-hit" : "soft-round-mask-cache-miss",
      { key },
    );
  ...
  for (let y = 0; y < size; y++)
    for (let x = 0; x < size; x++, offset += 4) {
      const dx = x + 0.5 - center,
        dy = y + 0.5 - center;
      const coverage = _roundBrushFalloff(
  ...
  if (_softRoundMaskCache.size >= _SOFT_ROUND_MASK_CACHE_MAX)
    _softRoundMaskCache.delete(_softRoundMaskCache.keys().next().value);
  _softRoundMaskCache.set(key, stamp);
  return stamp;
}
function _drawSoftRoundMask(x, y, r, rgb, alpha, composite) {
  const stamp = _buildSoftRoundMask(r);
  if (!_softRoundTintCanvas)
    _softRoundTintCanvas = document.createElement("canvas");
  const tint = _softRoundTintCanvas;
  if (tint.width !== stamp.w || tint.height !== stamp.h) {
    tint.width = stamp.w;
    tint.height = stamp.h;
  }
...
function _buildTipStamp(rRaw, rgb, alphaRaw, composite, hardnessRaw) {
  const latencyProbe = window.FirstDabLatencyProbe,
    latencyBuildStart =
      latencyProbe && latencyProbe.enabled ? performance.now() : 0;
  const trace = window.CustomFirstDabTrace,
    lookupStart = trace && trace.enabled ? performance.now() : 0,
    generationStart = lookupStart;
  ...
  const baseRoundness =
    typeof _activeDabRoundness !== "undefined" && _activeDabRoundness != null
      ? _activeDabRoundness
      : window.brushTipRoundness == null
        ? 1
        : window.brushTipRoundness;
  const tipRoundness = Math.max(
    window.brushTipMinimumRoundness || 0,
    Math.min(1, baseRoundness),
  );
  ...
  const r = _quant(rRaw, rQuantStep),
    alpha = _quant(alphaRaw, _Q_ALPHA);
  const hardness =
    Math.round(Math.max(0, Math.min(0.99, hardnessRaw)) * 100) / 100;
  const key =
    r.toFixed(2) +
    "|" +
    rgb.join(",") +
    ...
    tipMode +
    "|rd" +
    tipRoundness.toFixed(3);
  const hit = _tipDabCache.get(key);
  _brushDiagPerfNote("stamp-cache", { hit: !!hit });
  if (latencyProbe && latencyProbe.enabled)
    latencyProbe.cache({
      key,
      hit: !!hit,
      sizeBeforeLookup: _tipDabCache.size,
    });
  if (trace && trace.enabled) {
    trace.stage("custom-tip-cache-lookup", lookupStart, {
      tipId: trace.objectId(tipC),
      tipVersion: tipV,
      invalidationReason: hit
        ? null
        : _tipAlphaInvalidationReason || "stamp-key-miss",
    });
    trace.instant("stamp-cache-lookup", {
      hit: !!hit,
      key,
      scaleBucket: r.toFixed(2),
      rotationBucket: Math.round((_viewAdjustedTipRotation() * 180) / Math.PI),
      roundnessBucket: tipRoundness.toFixed(3),
    });
  }
  const tipPerf = _brushPerf();
  if (tipPerf)
    tipPerf.point(hit ? "tip-stamp-cache-hit" : "tip-stamp-cache-miss", {
      key,
    });
  if (hit) {
    if (latencyProbe && latencyProbe.enabled) {
      latencyProbe.renderer("custom-stamp", {
        effectiveRadius: r,
        stampWidth: hit.w,
        stampHeight: hit.h,
      });
      latencyProbe.measure("buildTipStamp", latencyBuildStart);
    }
    return hit;
  }
  ...
  const dabW = Math.max(
    1,
    tipNativeW * tipScale * (compressWidth ? tipRoundness : 1),
  );
  const dabH = Math.max(
    1,
    tipNativeH * tipScale * (compressWidth ? 1 : tipRoundness),
  );

  const pad = 2;
  const w = Math.ceil(dabW) + pad * 2 + 1,
    h = Math.ceil(dabH) + pad * 2 + 1;
  const cx = w / 2,
    cy = h / 2;
  ...
  const scaleStart = trace && trace.enabled ? performance.now() : 0;
  maskCtx.drawImage(
    tipC,
    0,
    0,
  ...
  if (trace && trace.enabled)
    trace.stage("tip-scaling-resampling", scaleStart, {
      sourceWidth: tipNativeW,
      sourceHeight: tipNativeH,
      dabWidth: dabW,
      dabHeight: dabH,
      stampWidth: w,
      stampHeight: h,
  ...
  const maskReadStart = trace && trace.enabled ? performance.now() : 0,
    maskData = maskCtx.getImageData(0, 0, w, h).data;
  if (trace && trace.enabled)
    trace.stage("get-image-data", maskReadStart, {
      source: "scaled-mask",
      width: w,
      height: h,
    });
  ...
  try {
    ...
    if (trace && trace.enabled)
      trace.stage("get-image-data", sourceReadStart, {
        source: "tip-source",
        width: tipNativeW,
        height: tipNativeH,
      });
    if (window.CustomTipCacheTrace)
      window.CustomTipCacheTrace.record("direct-tip-source-read", {
        path: "_buildTipStamp",
        tipVersion: window.brushTipVersion || 0,
        tipCanvasId: window.CustomTipCacheTrace.objectId(tipC, "tip-canvas"),
        width: tipNativeW,
        height: tipNativeH,
        getImageDataDuration: performance.now() - sourceReadStart,
      });
    ...
    for (let p = 0; p < sourceData.length; p += 4) {
      ...
      const sourceLuminance =
        ...
        255;
      if (sourceLuminance > maximumVisibleLuminance)
        maximumVisibleLuminance = sourceLuminance;
    }
    legacyAlphaOnlyMask = maximumVisibleLuminance < 0.01;
  } catch (error) {
    legacyAlphaOnlyMask = false;
  }
  const output = tc.createImageData(w, h);
  ...
  const semiW = Math.max(0.5, dabW / 2),
    semiH = Math.max(0.5, dabH / 2);
  ...
  const outputCopyStart = trace && trace.enabled ? performance.now() : 0;
  tc.putImageData(output, 0, 0);
  if (trace && trace.enabled)
    trace.stage("temporary-canvas-copy", outputCopyStart, {
  ...
  if (_tipDabCache.size >= _TIP_DAB_CACHE_MAX)
    _tipDabCache.delete(_tipDabCache.keys().next().value);
  ...
  if (trace && trace.enabled)
    trace.stage("stamp-generation", generationStart, {
      key,
      stampWidth: w,
      stampHeight: h,
      scaleBucket: r.toFixed(2),
...
function _getTipPixelsForStamp(w, h) {
  ...
  tc.drawImage(
    ...
    0,
    0,
    tipC.width || tipC.naturalWidth || w,
    tipC.height || tipC.naturalHeight || h,
    0,
    0,
    w,
    h,
  ...
  const readStart = performance.now(),
    data = tc.getImageData(0, 0, w, h).data;
  if (window.CustomTipCacheTrace)
    window.CustomTipCacheTrace.record("direct-tip-source-read", {
      path: "_getTipPixelsForStamp",
      tipVersion: window.brushTipVersion || 0,
      tipCanvasId: window.CustomTipCacheTrace.objectId(tipC, "tip-canvas"),
      width: w,
      height: h,
      getImageDataDuration: performance.now() - readStart,
    });
  return data;
}
function _buildAAStamp(rRaw, rgb, alphaRaw, composite, hardnessRaw) {
  const r = _quant(rRaw, _Q_R),
    alpha = _quantAlpha(alphaRaw);
  const hardness =
    Math.round(Math.max(0, Math.min(1, hardnessRaw)) * 100) / 100;
  ...
  const aaPerf = _brushPerf();
  if (aaPerf)
    aaPerf.point(
      hit ? "analytic-stamp-cache-hit" : "analytic-stamp-cache-miss",
      { key },
    );
  if (hit) return hit;
  const rr = Math.max(0.05, r);
  ...
  const supersample = window.brushTipCanvas
    ? 1
    : rr <= 64 && hardness < 0.25
      ? 4
      : rr <= 256
        ? 2
        : 1;
  const sampleW = w * supersample,
    sampleH = h * supersample;
  const sampleR = rr * supersample,
    cx = sampleW / 2,
    cy = sampleH / 2;
  ...
  const d = id.data;
  const cr = composite === "erase" ? 0 : rgb[0],
    cg = composite === "erase" ? 0 : rgb[1],
    cb = composite === "erase" ? 0 : rgb[2];
  ...
  for (let py = 0; py < sampleH; py++) {
    for (let px = 0; px < sampleW; px++, p += 4) {
      ...
      let a =
        alpha * _proceduralBrushFalloff(t, hardness, rr, aaMode, isAirbrush);
      if (a <= 0) continue;
      if (tipPixels) {
      ...
      d[p + 1] = cg;
      d[p + 2] = cb;
      d[p + 3] = Math.round(Math.min(1, a) * 255);
    }
  }
  ...
  const stamp = { canvas: tmp, w, h };
  if (_aaDabCache.size >= _AA_DAB_CACHE_MAX)
    _aaDabCache.delete(_aaDabCache.keys().next().value); // evict oldest
...
function _drawUnifiedTipStamp(x, y, r, rgb, alpha, composite) {
  const trace = window.CustomFirstDabTrace,
    identityStart = trace && trace.enabled ? performance.now() : 0;
  const dc = _inStroke && composite !== "erase" ? _strokeCtx : ctx;
  if (trace && trace.enabled)
    trace.stage("custom-tip-identity-lookup", identityStart, {
      tipId: trace.objectId(window.brushTipCanvas),
      tipVersion: window.brushTipVersion || 0,
    });
  ...
  if (window.brushTipFlipX || window.brushTipFlipY)
    dc.scale(window.brushTipFlipX ? -1 : 1, window.brushTipFlipY ? -1 : 1);
  if (trace && trace.enabled)
    trace.stage("transformed-tip-generation", transformStart, {
      rotation: adjustedRotation,
      rotationBucket: Math.round((adjustedRotation * 180) / Math.PI),
      stampWidth: stamp.w,
      stampHeight: stamp.h,
    });
  const latencyDrawStart =
    window.FirstDabLatencyProbe && window.FirstDabLatencyProbe.enabled
      ? performance.now()
      : 0;
  const blendStart = trace && trace.enabled ? performance.now() : 0;
  dc.drawImage(stamp.canvas, -stamp.w / 2, -stamp.h / 2);
  if (window.FirstDabLatencyProbe && window.FirstDabLatencyProbe.enabled)
    window.FirstDabLatencyProbe.measure(
      "cachedStampDrawImage",
      latencyDrawStart,
    );
  if (trace && trace.enabled)
    trace.stage("blend-dab-into-stroke-canvas", blendStart, {
      operation: "drawImage",
      stampWidth: stamp.w,
      stampHeight: stamp.h,
    });
...
function _dabAAGpu(x, y, r, rgb, alpha, composite) {
  ...
  const aaMode = _currentAAMode();
  {
    ...
    if (inner > 0.0001) {
      grad.addColorStop(
        Math.min(0.9999, inner),
        `rgba(${c0[0]},${c0[1]},${c0[2]},${alpha})`,
      );
    }
    ...
    for (let i = 1; i <= STOPS; i++) {
      ...
      grad.addColorStop(
        Math.min(1, t),
        `rgba(${c0[0]},${c0[1]},${c0[2]},${Math.max(0, a)})`,
      );
    }
  }
  dc.fillStyle = grad;
  dc.beginPath();
  dc.arc(x, y, rr, 0, Math.PI * 2);
  dc.fill();
  dc.restore();
}
...
function _dabAACpu(x, y, r, rgb, alpha, composite) {
  ...
  const pad = Math.max(1, Math.ceil(_AA_MODE_EDGE_MAX_PX[aaModeCpu] || 1)),
    ir = Math.ceil(rr) + pad;
  const sx = Math.max(0, Math.floor(x - ir)),
    sy = Math.max(0, Math.floor(y - ir));
  const ex = Math.min(cw, Math.ceil(x + ir)),
    ey = Math.min(ch, Math.ceil(y + ir));
  const rw = ex - sx,
    rh = ey - sy;
  if (rw <= 0 || rh <= 0) return;
  ...
  for (let py = 0; py < rh; py++) {
    ...
    for (let px = 0; px < rw; px++, p += 4) {
      ...
      let a =
        alpha *
        _proceduralBrushFalloff(t, brushHardness, rr, aaModeCpu, isAirbrush);
...
function _dabAATinyCoverage(x, y, r, rgb, alpha, composite) {
  ...
  const rr = Math.max(0.05, r),
    pad = 1;
  const sx = Math.max(0, Math.floor(x - rr - pad)),
    sy = Math.max(0, Math.floor(y - rr - pad));
  const ex = Math.min(dc.canvas.width, Math.ceil(x + rr + pad)),
    ey = Math.min(dc.canvas.height, Math.ceil(y + rr + pad));
  const width = ex - sx,
    height = ey - sy;
  if (width <= 0 || height <= 0) return;
  const _hrRtGidStart = window.HardRoundDebugRoutingTrace
    ? performance.now()
    : null;
  _brushDiagPerfNote("get-image-data");
  const image = dc.getImageData(sx, sy, width, height),
    data = image.data;
  if (_hrRtGidStart != null)
    _hrRtNoteDabAATinyCoverage(performance.now() - _hrRtGidStart);
  const inner = _effectiveInnerFrac(rr, brushHardness, _currentAAMode());
  const samples = 4,
    invSamples = 1 / (samples * samples);
  for (let py = 0; py < height; py++) {
    for (let px = 0; px < width; px++) {
      ...
      const sourceAlpha = Math.max(
        0,
        Math.min(1, alpha * coverage * invSamples),
      );
...
let _tipAlphaBuf = null,
  _tipAlphaBufVersion = -1,
  _tipAlphaBufW = 0,
  _tipAlphaBufH = 0,
  _tipAlphaBufLegacy = false,
  _tipAlphaSeedPixels = null,
  _tipReadbackCanvas = null,
  _tipReadbackCtx = null,
  _tipAlphaInvalidationReason = "initial-unbuilt";
function _tipAlphaFromPixels(d, w, h) {
  ...
  for (let i = 0, p = 0; p < d.length; p += 4, i++) {
    const a = d[p + 3] / 255,
      lum = (d[p] * 0.2126 + d[p + 1] * 0.7152 + d[p + 2] * 0.0722) / 255;
    buf[i] = legacy ? a : a * lum;
  }
  return { buf, legacy };
}
function _getTipAlphaBuffer() {
  const alphaTrace = window.CustomFirstDabTrace,
    alphaLookupStart = alphaTrace && alphaTrace.enabled ? performance.now() : 0;
  const callStarted = performance.now(),
    tipC = window.brushTipCanvas;
  if (!tipC) return null;
  const v = window.brushTipVersion || 0,
    w = tipC.width || 1,
    h = tipC.height || 1,
    trace = window.CustomTipCacheTrace,
    tipCanvasId = trace ? trace.objectId(tipC, "tip-canvas") : null,
    cacheKey = v + "|" + tipCanvasId + "|" + w + "x" + h;
  const hit =
      _tipAlphaBufVersion === v && _tipAlphaBufW === w && _tipAlphaBufH === h,
    priorBuffer = _tipAlphaBuf,
    priorVersion = _tipAlphaBufVersion;
  let getImageDataDuration = 0,
    alphaExtractionDuration = 0,
    source = "cached",
    allocated = false,
    resized = false;
  if (!hit) {
    const perf = _brushPerf(),
      experiment = window.TipReadbackExperiment,
      totalStart = performance.now();
    let d;
    if (
      experiment &&
      experiment.mode === "D" &&
      _tipAlphaSeedPixels &&
      _tipAlphaSeedPixels.version === v &&
      _tipAlphaSeedPixels.w === w &&
      _tipAlphaSeedPixels.h === h
    ) {
      d = _tipAlphaSeedPixels.data;
      source = "normalization-image-data";
    } else if (experiment && experiment.mode === "B") {
      source = "dedicated-readback";
      allocated = !_tipReadbackCanvas;
      if (!_tipReadbackCanvas) {
        _tipReadbackCanvas = document.createElement("canvas");
        _tipReadbackCtx = _tipReadbackCanvas.getContext("2d", {
          willReadFrequently: true,
        });
      }
      resized =
        _tipReadbackCanvas.width !== w || _tipReadbackCanvas.height !== h;
      if (resized) {
        _tipReadbackCanvas.width = w;
        _tipReadbackCanvas.height = h;
      }
      const copyStart = performance.now();
      _tipReadbackCtx.clearRect(0, 0, w, h);
      _tipReadbackCtx.drawImage(tipC, 0, 0);
      const readStart = performance.now();
      d = _tipReadbackCtx.getImageData(0, 0, w, h).data;
      getImageDataDuration = performance.now() - readStart;
      if (perf) {
        perf.measure("tip-readback-copy", copyStart, {
          width: w,
          height: h,
          dedicated: true,
        });
        perf.recordDuration("tip-get-image-data", getImageDataDuration, {
          width: w,
          height: h,
          source,
          willReadFrequently: true,
      ...
      const contextStart = performance.now(),
        sctx = tipC.getContext("2d", { willReadFrequently: true }),
        attributes =
          typeof sctx.getContextAttributes === "function"
            ? sctx.getContextAttributes()
            : null,
        readStart = performance.now();
      d = sctx.getImageData(0, 0, w, h).data;
      getImageDataDuration = performance.now() - readStart;
      if (perf) {
        perf.measure("tip-readback-context-access", contextStart, {
          width: w,
          height: h,
          requestedWillReadFrequently: true,
          actualWillReadFrequently: attributes && attributes.willReadFrequently,
        });
        perf.recordDuration("tip-get-image-data", getImageDataDuration, {
          width: w,
          height: h,
    ...
    const processStart = performance.now(),
      built = _tipAlphaFromPixels(d, w, h);
    alphaExtractionDuration = performance.now() - processStart;
    _tipAlphaBuf = built.buf;
    _tipAlphaBufW = w;
    _tipAlphaBufH = h;
    _tipAlphaBufVersion = v;
    _tipAlphaBufLegacy = built.legacy;
    if (perf) {
      perf.recordDuration(
        "tip-alpha-buffer-processing",
        alphaExtractionDuration,
        { width: w, height: h, pixels: w * h },
      );
      perf.measure("tip-alpha-buffer-initialization", totalStart, {
        width: w,
        height: h,
        source,
        mode: (experiment && experiment.mode) || "control",
        allocated,
        resized,
      });
    }
    if (experiment)
      experiment.record("tip-alpha-buffer-initialized", {
        tipVersion: v,
        width: w,
        height: h,
        source,
  ...
  const alphaBufferId = trace
    ? trace.objectId(_tipAlphaBuf, "alpha-buffer")
    : null;
  if (alphaTrace && alphaTrace.enabled)
    alphaTrace.stage("alpha-buffer-cache-lookup", alphaLookupStart, {
      hit,
      tipVersion: v,
      width: w,
      height: h,
      bufferId: alphaTrace.objectId(_tipAlphaBuf),
      invalidationReason: hit ? null : _tipAlphaInvalidationReason,
      getImageDataDuration,
      alphaExtractionDuration,
    });
  if (trace)
    trace.tipCall({
      ...
      cacheHit: hit,
      invalidationReason: hit ? null : _tipAlphaInvalidationReason,
      priorTipVersion: priorVersion,
      priorAlphaBufferId: trace.objectId(priorBuffer, "alpha-buffer"),
      alphaBufferId,
      alphaBufferReplaced: priorBuffer !== _tipAlphaBuf,
      tipCanvasId,
      width: w,
      height: h,
      getImageDataDuration,
      alphaExtractionDuration,
      source,
      totalCallDuration: performance.now() - callStarted,
    });
  if (hit && window.TipReadbackExperiment)
    window.TipReadbackExperiment.cacheHit(v);
  else _tipAlphaInvalidationReason = null;
...
function _tinyTipCoverageTileFor(dc, px, py) {
  if (_tinyTipCoverageTileCtx && _tinyTipCoverageTileCtx !== dc)
    _flushTinyTipCoverageTiles();
  ...
  if (!tile) {
    const w = Math.min(_TINY_TIP_TILE_SIZE, dc.canvas.width - tx),
      h = Math.min(_TINY_TIP_TILE_SIZE, dc.canvas.height - ty);
    _brushDiagPerfNote("get-image-data");
    tile = {
      x: tx,
      y: ty,
      w,
      h,
      image: dc.getImageData(tx, ty, w, h),
      dirty: false,
    };
...
function _flushTinyTipCoverageTiles() {
  ...
  if (brushDiagStart)
    _brushDiagPerfNote("processing", {
      ms: performance.now() - brushDiagStart,
    });
}
function _dabTipTinyCoverageBatched(x, y, r, rgb, alpha, composite, tipInfo) {
  ...
  if (!_inStroke || composite === "erase" || dc !== _strokeCtx) {
    _flushTinyTipCoverageTiles();
    return false;
  }
  const rr = Math.max(0.05, r),
    softAlpha = !!window.brushTipSoftAlpha,
    tipMode = window.brushTipMode || "multiply";
  const baseRoundness =
    typeof _activeDabRoundness !== "undefined" && _activeDabRoundness != null
      ? _activeDabRoundness
      : window.brushTipRoundness == null
  ...
  const tipNativeW = tipInfo.w,
    tipNativeH = tipInfo.h,
    tipScale = (2 * rr) / Math.max(tipNativeW, tipNativeH),
    compressWidth = tipNativeW < tipNativeH;
  const dabW = Math.max(
    0.02,
    tipNativeW * tipScale * (compressWidth ? tipRoundness : 1),
  );
  ...
  const semiW = dabW / 2,
    semiH = dabH / 2,
    rotation = _viewAdjustedTipRotation(),
    cosR = Math.cos(-rotation),
    sinR = Math.sin(-rotation);
  const flipXsign = window.brushTipFlipX ? -1 : 1,
    flipYsign = window.brushTipFlipY ? -1 : 1,
    pad = 1,
    halfSpan = Math.max(semiW, semiH) + pad;
  const sx = Math.max(0, Math.floor(x - halfSpan)),
    sy = Math.max(0, Math.floor(y - halfSpan));
  const ex = Math.min(dc.canvas.width, Math.ceil(x + halfSpan)),
    ey = Math.min(dc.canvas.height, Math.ceil(y + halfSpan));
  if (ex <= sx || ey <= sy) return true;
  const applyFalloff = softAlpha && tipMode !== "replace";
  const inner = applyFalloff
    ? _effectiveInnerFrac(rr, brushHardness, _currentAAMode())
    : 1;
  const samples = 4,
    invSamples = 1 / (samples * samples),
    cr = composite === "erase" ? 0 : rgb[0],
    cg = composite === "erase" ? 0 : rgb[1],
    cb = composite === "erase" ? 0 : rgb[2],
    tipBuf = tipInfo.data;
  for (let py = sy; py < ey; py++)
    for (let px = sx; px < ex; px++) {
      ...
      for (let sampleY = 0; sampleY < samples; sampleY++)
        for (let sampleX = 0; sampleX < samples; sampleX++) {
          const wx = px + (sampleX + 0.5) / samples,
            wy = py + (sampleY + 0.5) / samples;
          let rx = wx - x,
            ry = wy - y;
          if (rotation) {
            const rrx = rx * cosR - ry * sinR,
              rry = rx * sinR + ry * cosR;
            rx = rrx;
            ry = rry;
          }
          rx *= flipXsign;
          ry *= flipYsign;
          const tipA = _sampleTipAlphaBilinear(
            tipBuf,
            tipNativeW,
            tipNativeH,
            (rx / dabW + 0.5) * tipNativeW,
            (ry / dabH + 0.5) * tipNativeH,
          );
          if (tipA <= 0) continue;
          coverage += applyFalloff
            ? tipA *
              _roundBrushFalloff(
                Math.sqrt(
                  (rx / Math.max(0.02, semiW)) ** 2 +
                    (ry / Math.max(0.02, semiH)) ** 2,
                ),
                inner,
                brushHardness,
              )
      ...
      if (sourceAlpha <= 0) continue;
      const tile = _tinyTipCoverageTileFor(dc, px, py),
        offset = ((py - tile.y) * tile.w + (px - tile.x)) * 4,
...
function _dabTipTinyCoverage(x, y, r, rgb, alpha, composite) {
  _brushDiagPerfNote("tiny");
  if (window.FirstDabLatencyProbe && window.FirstDabLatencyProbe.enabled)
    window.FirstDabLatencyProbe.renderer("tiny-custom-tip", {
      effectiveRadius: r,
    });
  const tipInfo = _getTipAlphaBuffer();
  if (!tipInfo) {
    _flushTinyTipCoverageTiles();
    _dabAATinyCoverage(x, y, r, rgb, alpha, composite);
    return;
  }
  if (_dabTipTinyCoverageBatched(x, y, r, rgb, alpha, composite, tipInfo))
    return;
  const dc = _inStroke && composite !== "erase" ? _strokeCtx : ctx;
  const rr = Math.max(0.05, r);
  const softAlpha = !!window.brushTipSoftAlpha;
  ...
  const cosR = Math.cos(-rotation),
    sinR = Math.sin(-rotation); // world -> tip-local
  const flipXsign = window.brushTipFlipX ? -1 : 1,
    flipYsign = window.brushTipFlipY ? -1 : 1;
  ...
  const applyFalloff = softAlpha && tipMode !== "replace";
  const inner = applyFalloff
    ? _effectiveInnerFrac(rr, brushHardness, aaMode)
    : 1;
  const samples = 4,
    invSamples = 1 / (samples * samples);
  const cr = composite === "erase" ? 0 : rgb[0],
...
function _dabAA(x, y, r, rgb, alpha, composite) {
  ...
  if (_currentAAMode() === "none") {
    _dabAliased(x, y, r, rgb, alpha, composite);
    return;
  }
  const tinyGeneratedHardRound =
    r <= 1 &&
    !window._brushAirbrush &&
    !window.brushTipCanvas &&
    brushHardness >= 0.995;
  ...
  const tinyTipDab =
    r <= 1 && !window._brushAirbrush && !!window.brushTipCanvas;
  if (tinyTipDab) {
    _dabTipTinyCoverage(x, y, r, rgb, alpha, composite);
    return;
  }
...
function _getAliasedStamp(rRaw, rgb, alphaRaw, composite) {
  ...
  const tc = tmp.getContext("2d", { willReadFrequently: true });
  tc.fillStyle =
    composite === "erase" ? "black" : `rgb(${rgb[0]},${rgb[1]},${rgb[2]})`;
  tc.beginPath();
  tc.arc(w / 2, h / 2, r, 0, Math.PI * 2);
  tc.fill();
  ...
  if (window.brushTipCanvas) {
    ...
    tc.drawImage(
      ...
      0,
      0,
      window.brushTipCanvas.width || w,
      window.brushTipCanvas.height || h,
      0,
      0,
      w,
      h,
  ...
  for (let i = 0; i < d.length; i += 4) {
    if (d[i + 3] > 0) {
      // covered at all -> fully on, exact solid color
      d[i] = cr;
      d[i + 1] = cg;
      d[i + 2] = cb;
      d[i + 3] = fa;
  ...
  const stamp = { canvas: tmp, w, h };
  if (_stampCache.size >= _STAMP_CACHE_MAX)
    _stampCache.delete(_stampCache.keys().next().value); // evict oldest
...
function _dabAliased(x, y, r, rgb, alpha, composite) {
  ...
  const stamp = window.brushTipCanvas
    ? _buildTipStamp(r, rgb, alpha, composite, brushHardness)
    : _getAliasedStamp(r, rgb, alpha, composite);
  ...
  dc.globalCompositeOperation = _strokeDabComposite(composite);
  const adjustedRotation = window.brushTipCanvas
    ? _viewAdjustedTipRotation()
    : 0;
  if (
    window.brushTipCanvas &&
    (adjustedRotation || window.brushTipFlipX || window.brushTipFlipY)
  ) {
    dc.translate(x, y);
    if (adjustedRotation) dc.rotate(adjustedRotation);
    if (window.brushTipFlipX || window.brushTipFlipY)
    ...
    const latencyDrawStart =
      ...
      window.FirstDabLatencyProbe.enabled &&
      window.brushTipCanvas
        ? performance.now()
        : 0;
    dc.drawImage(stamp.canvas, -stamp.w / 2, -stamp.h / 2);
...
function _consumeStrokeDirtyRect() {
  const r = _strokeDirty;
  _strokeDirty = null;
  if (!r) return null;
  const x = Math.max(0, Math.floor(r.minX)),
    y = Math.max(0, Math.floor(r.minY));
  const ex = Math.min(activeC.width, Math.ceil(r.maxX)),
    ey = Math.min(activeC.height, Math.ceil(r.maxY));
...
function _cleanupErasedSmartOwnership() {
  ...
  if (
    ...
    layers[curLayer] &&
    layers[curLayer].type === "smart-raster" &&
    typeof clearStyleIndexWhereTransparent === "function"
  ) {
...
function _getScaledTextureCanvas() {
  ...
  if (
    ...
    Math.abs(_texCacheScale - scale) < 0.0001 &&
    _texCacheInvert === inv &&
  ...
  try {
    ...
    for (let i = 0, p = 0; i < n; i += 4, p++) {
      ...
      d[i] = 255;
      d[i + 1] = 255;
      d[i + 2] = 255; // white Ã¢â‚¬â€ color ignored by destination-in
...
function _maskRegionInPlace(dc, rx, ry, rw, rh, strength) {
  ...
  if (
    !_maskRegionInPlace._orig ||
    _maskRegionInPlace._orig.width < rw ||
    _maskRegionInPlace._orig.height < rh
  ) {
  ...
  const origCanvas = _maskRegionInPlace._orig,
    origCtx = _maskRegionInPlace._origCtx;
  ...
  if (
    !_maskRegionInPlace._masked ||
    _maskRegionInPlace._masked.width < rw ||
    _maskRegionInPlace._masked.height < rh
  ) {
  ...
  const maskedCanvas = _maskRegionInPlace._masked,
    maskedCtx = _maskRegionInPlace._maskedCtx;
...
function _applyTextureToDabDirect(dc, x, y, r, alpha) {
  ...
  const rw = Math.ceil(x + r + pad - rx),
    rh = Math.ceil(y + r + pad - ry);
...
let _texturedStrokeCanvas = null,
  _texturedStrokeCtx = null;

function _ensureTexHelper(w, h, existing, existingCtx) {
  if (existing && existing.width === w && existing.height === h)
    return { c: existing, x: existingCtx };
  const c = document.createElement("canvas");
  c.width = w;
  c.height = h;
...
function _getTexturedStrokeCanvas(srcCanvas, forceFull) {
  ...
  // Ensure output canvas.
  {
    ...
    _texturedStrokeCanvas = t.c;
    _texturedStrokeCtx = t.x;
  }
...
function _customTipGpuEligibleNow() {
  ...
  const renderer =
    window._customTipGpuRenderer && window._customTipGpuRenderer.instance
      ? window._customTipGpuRenderer.instance
      : null;
...
function _renderFallbackDabsToPrivateCanvas(fallbackDabs) {
  ...
  for (let i = 0; i < fallbackDabs.length; i++) {
    ...
    const stamp = window.brushTipCanvas
      ? _buildTipStamp(
          ...
          d.alpha,
          d.composite,
          typeof brushHardness !== "undefined" ? brushHardness : 1,
        )
      : _getAliasedStamp(d.r, d.rgb, d.alpha, d.composite);
    if (!stamp || !stamp.canvas) continue;
    const x0 = d.x - stamp.w / 2,
      y0 = d.y - stamp.h / 2;
    dc.save();
    dc.globalCompositeOperation =
      typeof _strokeDabComposite === "function"
        ? _strokeDabComposite(d.composite)
        : "source-over";
  ...
  if (
    window.brushTextureEnabled &&
    window.brushTextureCanvas &&
    typeof _getTexturedStrokeCanvas === "function"
  ) {
...
function _saveExplicitFrameToKey(ownerLayer, ownerFrame) {
  if (
    typeof isDrawingFrameHidden === "function" &&
    isDrawingFrameHidden(ownerLayer, ownerFrame)
  )
    return;
  ...
  const extended =
    typeof getExtendedLayerFrame === "function"
      ? getExtendedLayerFrame(ownerLayer, ownerFrame)
      : null;
  if (extended && typeof setExtendedLayerFrame === "function") {
    ...
    const minX = Math.min(extended.x, 0),
      minY = Math.min(extended.y, 0),
...
function _retireCustomTipOverlay(strokeId) {
  ...
  const retire = () => {
    ...
    const isNewerNow =
      (ownerNow != null && ownerNow !== strokeId) ||
      (_customTipGpuStrokeActive &&
        activeNow != null &&
        activeNow !== strokeId);
    if (isNewerNow) {
      return;
    ...
    if (
      ...
      typeof window._customTipGpuRenderer.hideLiveOverlay === "function"
    ) {
...
async function _executeCustomTipDetachedCommit(gpuTask, ctxData) {
  ...
  if (imgData && rect && rect.w > 0 && rect.h > 0) {
    ...
    rect = {
      ...
      y: 0,
      w: finishingCanvas
        ? finishingCanvas.width
        : typeof CW !== "undefined"
          ? CW
          : 1000,
      h: finishingCanvas
        ? finishingCanvas.height
        : typeof CH !== "undefined"
          ? CH
          : 1000,
    };
  ...
  const destCtx = isCurrent
    ? ctx
    : layers[ownerLayer] &&
        layers[ownerLayer].frames &&
        layers[ownerLayer].frames[ownerFrame]
      ? layers[ownerLayer].frames[ownerFrame].getContext("2d")
      : null;
  if (!destCtx) {
    if (gpuTask && typeof gpuTask.destroy === "function") gpuTask.destroy();
    return;
  ...
  if (isSmartRaster && typeof commitSmartRasterBrush === "function") {
    ownershipBefore = destCtx.getImageData(
      dirtyRect.x,
      dirtyRect.y,
      dirtyRect.w,
      dirtyRect.h,
  ...
  destCtx.globalAlpha = Math.max(
    0,
    Math.min(1, brushOpacityVal != null ? brushOpacityVal : 1),
  );
...
function _finishCustomTipOrCanvasCommit(e, strokeSession) {
  if (_customTipGpuStrokeActive) {
    ...
    const targetCanvas =
      layers[curLayer] &&
      layers[curLayer].frames &&
      layers[curLayer].frames[curFrame]
        ? layers[curLayer].frames[curFrame]
        : activeC;
    const styleId =
      typeof activeAdvancedStyleIdForPainting === "function"
    ...
    const blendModeVal =
      typeof window.brushBlendMode === "string"
        ? window.brushBlendMode
        : "normal";
    const isSmartRaster = !!(
      styleId &&
    ...
    if (
      window.pixelSelectionState &&
      window.pixelSelectionState.active &&
      window.pixelSelectionState.maskCanvas
    ) {
      if (
        window.SelectionScope &&
        typeof window.SelectionScope.isRestricted === "function" &&
        window.SelectionScope.isRestricted()
      ) {
        selectionMaskSnapshot = document.createElement("canvas");
        selectionMaskSnapshot.width =
          window.pixelSelectionState.maskCanvas.width;
        selectionMaskSnapshot.height =
          window.pixelSelectionState.maskCanvas.height;
        selectionMaskSnapshot
          .getContext("2d")
          .drawImage(window.pixelSelectionState.maskCanvas, 0, 0);
      }
    }
    ...
    try {
      if (
        ...
        if (
          typeof window._customTipGpuRenderer.presentLiveImmediately ===
          "function"
        ) {
        ...
        gpuTask = window._customTipGpuRenderer.instance.captureFinishingTask({
          paperResource,
        });
      }
    } catch (err) {
    ...
    const settled = commit.finally(() => {
      _customTipPendingCommitCount = Math.max(
        0,
        _customTipPendingCommitCount - 1,
      );
    });
...
function _emitResolvedCustomTipDab(d, options = {}) {
  ...
  if (
    window._customTipRenderer &&
    typeof window._customTipRenderer.onResolvedDab === "function"
  ) {
    window._customTipRenderer.onResolvedDab(d, options);
  }
  if (
    (_customTipGpuStrokeActive || _shouldRunCustomTipGpuDiagnostic()) &&
    window._customTipGpuRenderer &&
    typeof window._customTipGpuRenderer.onResolvedDab === "function"
  ) {
...
function _dabDirtyRadii(d) {
  let x = d.r,
    y = d.r;
  if (window.brushTipCanvas) {
    const tipW = window.brushTipCanvas.width || 1,
      tipH = window.brushTipCanvas.height || 1;
    ...
    const width =
      tipW * ((d.r * 2) / reference) * (compressWidth ? roundness : 1);
    const height =
      tipH * ((d.r * 2) / reference) * (compressWidth ? 1 : roundness);
    const cosine = Math.abs(Math.cos(_activeDabRotation)),
      sine = Math.abs(Math.sin(_activeDabRotation));
...
function _drawDabNow(d) {
  ...
  if (window.CustomBrushDebugResolvedDabs && window.brushTipCanvas) {
    window._resolvedCustomTipDrawDabNowCount =
      (window._resolvedCustomTipDrawDabNowCount || 0) + 1;
  }
  if (window.HardRoundDebugRoutingTrace) _hrRtNoteDrawDabNow();
  const customTrace = window.CustomFirstDabTrace,
    customTraceStart =
      customTrace && customTrace.enabled ? performance.now() : 0;
  if (customTrace && customTrace.enabled)
    customTrace.beginDab({
      custom: !!window.brushTipCanvas,
      radius: d.r,
      rotation: d.rotation || 0,
      roundness: d.roundness,
      tipId: customTrace.objectId(window.brushTipCanvas),
      tipVersion: window.brushTipVersion || 0,
    });
  const perf = _brushPerf(),
    perfStart = perf ? performance.now() : 0;
  _activeDabRotation = window.brushTipCanvas ? d.rotation || 0 : 0;
  _activeDabRoundness =
    window.brushTipCanvas && d.roundness != null ? d.roundness : null;
  if (window.FirstDabLatencyProbe && window.FirstDabLatencyProbe.enabled)
    window.FirstDabLatencyProbe.renderer(
      window.brushTipCanvas ? "other-custom-tip-path" : "procedural",
      { effectiveRadius: d.r },
    );
  ...
  const colorEraserBefore = _captureColorEraserDab(
    d.x,
    d.y,
    dirtyRadius.x,
    dirtyRadius.y,
  ...
  if (!_drawAutoHardRoundSegment(d)) {
    if (_currentAAMode() !== "none")
      _dabAA(d.x, d.y, d.r, d.rgb, d.alpha, d.composite);
  ...
  if (perf)
    perf.measure("dirty-rectangle-expansion", dirtyStart, {
      radiusX: dirtyRadius.x,
      radiusY: dirtyRadius.y,
      rect: _frameDirty && {
        minX: _frameDirty.minX,
        minY: _frameDirty.minY,
        maxX: _frameDirty.maxX,
        maxY: _frameDirty.maxY,
      },
    });
  if (perf)
    perf.measure("dab-rasterization", perfStart, {
      dabNumber: _strokeDabCount,
      radius: d.r,
      alpha: d.alpha,
      tip: !!window.brushTipCanvas,
  ...
  if (customTrace && customTrace.enabled) {
    customTrace.stage(
      window.brushTipCanvas
        ? "custom-tip-dab-rasterization"
        : "procedural-dab-rasterization",
      customTraceStart,
      { radius: d.r },
    );
    customTrace.endDab();
...
function _taperDistance(amount) {
  return 320 * amount;
}
function _queueDab(d) {
  ...
  if (!_replayingTaper && (_getStartTaper() > 0 || _getEndTaper() > 0))
    _strokeReplayDabs.push(Object.assign({}, d, { rgb: d.rgb.slice() }));
  if (_customTipGpuStrokeActive) {
    _customTipGpuFallbackDabs.push(
      Object.assign({}, d, { rgb: d.rgb ? d.rgb.slice() : null }),
    );
  } else {
    _drawDabNow(d);
  }
...
function _flushStrokeTail() {
  _flushTinyTipCoverageTiles();
  const startAmount = _getStartTaper(),
    endAmount = _getEndTaper();
  if ((startAmount <= 0 && endAmount <= 0) || !_strokeReplayDabs.length) {
    _strokeReplayDabs.length = 0;
    _strokeReplayBase = null;
    return;
  }
  ...
  for (let i = 1; i < _strokeReplayDabs.length; i++) {
    const previous = _strokeReplayDabs[i - 1],
      current = _strokeReplayDabs[i];
    distances[i] =
      distances[i - 1] +
      Math.hypot(current.x - previous.x, current.y - previous.y);
  }
  const totalDistance = distances[distances.length - 1];
  let startDistance = _taperDistance(startAmount),
    endDistance = _taperDistance(endAmount);
  ...
  if (totalDistance > 0) {
    for (let i = 0; i < _strokeReplayDabs.length; i++) {
      if (startDistance > 0) {
        const progress = Math.max(0, Math.min(1, distances[i] / startDistance));
        factors[i] = progress * progress * (3 - 2 * progress);
      }
      if (endDistance > 0) {
        const progress = Math.max(
          0,
          Math.min(1, (totalDistance - distances[i]) / endDistance),
        );
        factors[i] = Math.min(
          factors[i],
  ...
  if (tool === "eraser") {
    ...
    ctx.drawImage(_strokeReplayBase, 0, 0);
    ctx.restore();
  ...
  for (let i = 0; i < _strokeReplayDabs.length; i++) {
    ...
    if (_customTipGpuStrokeActive) {
      _customTipGpuFallbackDabs.push(
        Object.assign({}, replayedDab, {
          rgb: replayedDab.rgb ? replayedDab.rgb.slice() : null,
        }),
      );
    } else {
...
const _STABILIZER_EPS_SCREEN_PX = 0.3;
...
const _STABILIZER_RECOVERY_PEAK_MIN_SCREEN_PX_PER_MS = 0.2;
const _STABILIZER_RECOVERY_DECEL_RATIO = 0.45;
const _STABILIZER_RECOVERY_RESUME_RATIO = 0.8;
...
let _stabilizerRawX = 0,
  _stabilizerRawY = 0;
let _stabilizerTargetX = 0,
  _stabilizerTargetY = 0;
...
let _stabilizerRawSpeed = 0,
  _stabilizerRawPeakSpeed = 0;
let _stabilizerRawLastX = 0,
  _stabilizerRawLastY = 0,
  _stabilizerRawLastT = 0;
let _stabilizerRecoveryStartT = 0,
  _stabilizerRecoveryLastT = 0,
...
function _resetStabilization(x, y, t) {
  ...
  _stabilizerRawX = x;
  _stabilizerRawY = y;
  _stabilizerTargetX = x;
  _stabilizerTargetY = y;
  ...
  _stabilizerRawSpeed = 0;
  _stabilizerRawPeakSpeed = 0;
  _stabilizerRawLastX = x;
  _stabilizerRawLastY = y;
  _stabilizerRawLastT = _stabilizerLastSampleT;
  _stabilizerRecoveryActive = false;
  _stabilizerRecoveryStartT = _stabilizerLastAdvanceT;
  _stabilizerRecoveryLastT = _stabilizerLastAdvanceT;
  _stabilizerRecoveryTickCarry = 0;
  _stabilizerCatchupLastT = _stabilizerLastAdvanceT;
  _stabilizerCatchupTickCarry = 0;
  ...
  _stabilizerBuf = Array.from({ length: _resetWindowLen }, () => ({
    x,
    y,
    t: performance.now(),
  }));
  _stabilizerPressureBuf = Array.from(
    { length: _resetWindowLen },
    () => currentPressure,
  );
...
function _stabilizerSchedule() {
  if (_stabilizerActive && !_stabilizerRAF)
    _stabilizerRAF = requestAnimationFrame(_stabilizerStep);
}
...
function _stabilizerUpdateRawVelocity(x, y, t) {
  ...
  const distanceScreenPx =
    Math.hypot(x - _stabilizerRawLastX, y - _stabilizerRawLastY) *
    Math.max(0.05, zoom);
  ...
  _stabilizerRawPeakSpeed = Math.max(
    _stabilizerRawSpeed,
    _stabilizerRawPeakSpeed * peakDecay,
  );
  _stabilizerRawLastX = x;
  _stabilizerRawLastY = y;
  _stabilizerRawLastT = sampleT;
}
function _stabilizerUpdateRecoveryState(now) {
  const gapScreenPx = _stabilizerGapCanvas() * Math.max(0.05, zoom);
  const decelerating =
    _stabilizerRawPeakSpeed >= _STABILIZER_RECOVERY_PEAK_MIN_SCREEN_PX_PER_MS &&
    _stabilizerRawSpeed <=
      _stabilizerRawPeakSpeed * _STABILIZER_RECOVERY_DECEL_RATIO;
  if (_stabilizerRecoveryActive) {
    if (
      gapScreenPx <= _STABILIZER_CATCHUP_EXIT_SCREEN_PX ||
      _stabilizerRawSpeed >
        _stabilizerRawPeakSpeed * _STABILIZER_RECOVERY_RESUME_RATIO
    ) {
      _stabilizerRecoveryActive = false;
      _stabilizerRecoveryTickCarry = 0;
    ...
    gapScreenPx >= _STABILIZER_CATCHUP_ENTER_SCREEN_PX &&
    decelerating
  ) {
...
// change.
const _OLD_STABILIZER_TAU_MAX = 0.05; // brush-engineold.js's TAU_MAX ("fully on" old engine)
...
function _oldStabilizerAdvance(targetX, targetY, now, sampleTime) {
  ...
  const expectedLagRatio =
    (_oldStabSpeed * _OLD_STABILIZER_TAU_MAX) / Math.max(0.01, maxLagCanvas);
...
function _applyOldStabilizerFloor(x, y, amount, now, sampleTime) {
  ...
  const old = _oldStabilizerAdvance(
    _stabilizerTargetX,
    _stabilizerTargetY,
    now,
...
function _baselineCanonicalStepScreenPx() {
  ...
  return (
    _BASELINE_CANONICAL_STEP_MIN_SCREEN_PX +
    (_BASELINE_CANONICAL_STEP_MAX_SCREEN_PX -
      _BASELINE_CANONICAL_STEP_MIN_SCREEN_PX) *
      viewScale
  );
}
...
function _baselineSampleFromEvent(e, p, pressure) {
  return {
    x: p.x,
    y: p.y,
    screenX: Number.isFinite(e.clientX) ? e.clientX : p.x * zoom,
    screenY: Number.isFinite(e.clientY) ? e.clientY : p.y * zoom,
    ...
    twist: Number.isFinite(e.twist)
      ? e.twist
      : Number.isFinite(e.rotationAngle)
        ? e.rotationAngle
        : 0,
    time:
...
function _baselineNewStats() {
  return {
    rawSampleCount: 0,
    forwardedSampleCount: 0,
    exactDuplicatesRejected: 0,
    tinyMovementsConsolidated: 0,
    timeGapEmissions: 0,
    attributeChangeEmissions: 0,
    cornerEmissions: 0,
    screenDistance: { sum: 0, min: Infinity, max: 0, count: 0 },
    dt: { sum: 0, min: Infinity, max: 0, count: 0 },
  };
}
function _baselineSameSample(a, b) {
  return (
    !!a &&
    Math.abs(a.x - b.x) <= 1e-12 &&
    Math.abs(a.y - b.y) <= 1e-12 &&
    Math.abs(a.pressure - b.pressure) <= 1e-12 &&
    Math.abs(a.tiltX - b.tiltX) <= 1e-12 &&
    Math.abs(a.tiltY - b.tiltY) <= 1e-12 &&
    Math.abs(a.twist - b.twist) <= 1e-12
  );
}
function _baselineLerpSample(a, b, t) {
  const l = (x, y) => x + (y - x) * t;
  return {
    x: l(a.x, b.x),
    y: l(a.y, b.y),
...
function _baselineEmit(s, sample, out, reason) {
  if (_baselineSameSample(s.lastForwarded, sample)) return;
  if (s.lastForwarded) {
    const d = Math.hypot(
        sample.screenX - s.lastForwarded.screenX,
        sample.screenY - s.lastForwarded.screenY,
      ),
      dt = Math.max(0, sample.time - s.lastForwarded.time),
      ds = s.stats.screenDistance,
      ts = s.stats.dt;
    ds.sum += d;
    ds.min = Math.min(ds.min, d);
    ds.max = Math.max(ds.max, d);
    ds.count++;
    ts.sum += dt;
    ts.min = Math.min(ts.min, dt);
    ts.max = Math.max(ts.max, dt);
    ts.count++;
  }
  s.lastForwarded = sample;
  s.stats.forwardedSampleCount++;
  if (reason === "time") s.stats.timeGapEmissions++;
  else if (reason === "attribute") s.stats.attributeChangeEmissions++;
  else if (reason === "corner") s.stats.cornerEmissions++;
  out.push(sample);
}
function _baselineConditionerReset(sample) {
  _baselineConditionerState = {
    previousRaw: null,
    lastRaw: sample,
    lastForwarded: null,
    distanceCarry: 0,
    stats: _baselineNewStats(),
  };
  _baselineConditionerState.stats.rawSampleCount = 1;
  const out = [];
  _baselineEmit(_baselineConditionerState, sample, out, "initial");
  return out;
}
...
function _baselineConditionerSync(sample) {
  const s = _baselineConditionerState;
  if (!s) return;
  s.previousRaw = s.lastRaw;
  s.lastRaw = sample;
  s.lastForwarded = sample;
  s.distanceCarry = 0;
}
function _baselineIsCorner(a, b, c) {
  if (!a || !b) return false;
  const abx = b.screenX - a.screenX,
    aby = b.screenY - a.screenY,
    bcx = c.screenX - b.screenX,
    bcy = c.screenY - b.screenY,
    ab = Math.hypot(abx, aby),
    bc = Math.hypot(bcx, bcy);
  if (ab < 0.25 || bc < 0.25) return false;
  const cosine = Math.max(-1, Math.min(1, (abx * bcx + aby * bcy) / (ab * bc)));
  return Math.acos(cosine) >= _BASELINE_CORNER_ANGLE_RAD;
}
function _baselineConditionerPush(sample, options = {}) {
  ...
  const a = s.lastRaw,
    segmentDistance = Math.hypot(
      sample.screenX - a.screenX,
      sample.screenY - a.screenY,
    ),
    rawDt = Math.max(0, sample.time - a.time);
  if (
    segmentDistance <= 1e-12 &&
    rawDt <= 1e-9 &&
    sample.pointerId === a.pointerId &&
    _baselineSameSample(sample, a)
  ) {
    n.exactDuplicatesRejected++;
    return out;
  }
  if (_baselineIsCorner(s.previousRaw, a, sample)) {
    _baselineEmit(s, a, out, "corner");
    s.distanceCarry = 0;
  }
  if (segmentDistance > 1e-12) {
    const canonicalStep = _baselineCanonicalStepScreenPx();
    let consumed = 0;
    while (s.distanceCarry + (segmentDistance - consumed) >= canonicalStep) {
      const needed = canonicalStep - s.distanceCarry;
      consumed += needed;
      _baselineEmit(
        s,
        _baselineLerpSample(a, sample, Math.min(1, consumed / segmentDistance)),
        out,
        "distance",
      );
      s.distanceCarry = 0;
    }
    s.distanceCarry += Math.max(0, segmentDistance - consumed);
  } else n.tinyMovementsConsolidated++;
  if (
    !out.length &&
    s.lastForwarded &&
    sample.time - s.lastForwarded.time >= _BASELINE_MAX_GAP_MS
  ) {
    _baselineEmit(
      s,
      sample,
      out,
      segmentDistance <= 1e-12 ? "attribute" : "time",
    );
    s.distanceCarry = 0;
  }
  if (options.force) {
    _baselineEmit(s, sample, out, "forced");
    s.distanceCarry = 0;
  }
  s.previousRaw = a;
  s.lastRaw = sample;
...
function _baselineSampleFromStabilizedPoint(e, p, time) {
  ...
  return {
    ...
    screenX: p.x * s,
    screenY: p.y * s,
    pressure: Number.isFinite(p.pressure) ? p.pressure : 0,
    tiltX: Number.isFinite(e.tiltX) ? e.tiltX : 0,
    tiltY: Number.isFinite(e.tiltY) ? e.tiltY : 0,
    twist: Number.isFinite(e.twist)
      ? e.twist
      : Number.isFinite(e.rotationAngle)
        ? e.rotationAngle
        : 0,
    time: Number.isFinite(time) && time > 0 ? time : performance.now(),
    pointerId: e.pointerId,
    event: e,
  };
}
function _baselineConditionerFinish(cancelled = false) {
  const s = _baselineConditionerState;
  if (!s) return;
  if (window.BaselineStrokeConditionerDiagnostics?.enabled) {
    const n = s.stats,
      f = (v) => ({
    ...
    _baselineConditionerReports.push({
      cancelled,
      rawSampleCount: n.rawSampleCount,
      forwardedSampleCount: n.forwardedSampleCount,
      exactDuplicatesRejected: n.exactDuplicatesRejected,
      tinyMovementsConsolidated: n.tinyMovementsConsolidated,
      timeGapEmissions: n.timeGapEmissions,
      attributeChangeEmissions: n.attributeChangeEmissions,
      cornerEmissions: n.cornerEmissions,
      screenDistance: f(n.screenDistance),
      dt: f(n.dt),
    });
    if (_baselineConditionerReports.length > 100)
      _baselineConditionerReports.shift();
  }
  _baselineConditionerState = null;
}
window.BaselineStrokeConditionerDiagnostics = {
  enabled: false,
  enable(v = true) {
    this.enabled = !!v;
    return this.enabled;
  ...
  results() {
    return JSON.parse(JSON.stringify(_baselineConditionerReports));
  },
  latest() {
  ...
  clear() {
    _baselineConditionerReports.length = 0;
  },
};
let _stabilizerDebugLastLogT = 0;
function _stabilizePoint(x, y, t) {
  ...
  _stabilizerLastAdvanceT = performance.now();
  _stabilizerSchedule();
  const floored = _applyOldStabilizerFloor(
    avg.x,
    avg.y,
    amount,
  ...
  return {
    x: _stabilizerX,
    y: _stabilizerY,
    pressure: _stabilizerSmoothedPressure,
  };
}
...
function _stabilizerGapCanvas() {
  return Math.hypot(
    _stabilizerTargetX - _stabilizerX,
    _stabilizerTargetY - _stabilizerY,
  );
}
function _emitHardRoundStabilizedPoint(
  x,
  y,
  pressure,
  ev,
  ...
  if (window.BrushDebugStabilizerParity) {
    ...
    if (window._stabilizerParityLog.length > 200) {
      window._stabilizerParityLog.splice(
        0,
        window._stabilizerParityLog.length - 200,
      );
    }
  }
}
...
function _stabilizerEmit(x, y, now) {
  ...
  if (window._brushAirbrush && Math.hypot(x - lx, y - ly) > 0.01)
  ...
  if (_hardRoundStrokeActive && _hardRoundCore) {
    ...
    _emitHardRoundStabilizedPoint(
      x,
      y,
      _stabilizerSmoothedPressure,
      e,
      now,
      eventType,
    );
    ...
    _baselineConditionerSync(
      _baselineSampleFromStabilizedPoint(
        e,
        { x, y, pressure: _stabilizerSmoothedPressure },
        now,
      ),
    );
    lx = x;
    ly = y;
    currentPressure = _stabilizerSmoothedPressure;
    _scheduleRecomposite();
  }
...
// ---------------------------------------------------------------------
let _tipDisplayFromX = 0,
  _tipDisplayFromY = 0,
  _tipDisplayFromT = 0;
let _tipDisplayToX = 0,
  _tipDisplayToY = 0,
  _tipDisplayToT = 0;
...
function _tipDisplayCurrent(now) {
  if (_tipDisplayToT <= _tipDisplayFromT)
    return { x: _tipDisplayToX, y: _tipDisplayToY };
  const t = Math.max(
    0,
    Math.min(1, (now - _tipDisplayFromT) / (_tipDisplayToT - _tipDisplayFromT)),
  );
...
function _tipDisplayRecordAuthoritative(x, y, now) {
  ...
  const span = _tipDisplayLastEmitT
    ? Math.max(
        ...
        Math.min(_TIP_DISPLAY_MAX_SPAN_MS, now - _tipDisplayLastEmitT),
      )
    : _TIP_DISPLAY_MIN_SPAN_MS;
  _tipDisplayFromX = cur.x;
  _tipDisplayFromY = cur.y;
  _tipDisplayFromT = now;
  _tipDisplayToX = x;
  _tipDisplayToY = y;
  _tipDisplayToT = now + span;
...
function _tipDisplayCancel() {
  if (_tipDisplayRAF) {
    cancelAnimationFrame(_tipDisplayRAF);
    _tipDisplayRAF = 0;
  }
}
function _ensureStabilizerLeash() {
  if (_stabilizerLeashOverlay || !window.EditorOverlayRenderer) return;
  _stabilizerLeashOverlay = EditorOverlayRenderer.create("stabilizer-leash", {
    zIndex: 6,
    draw: function (g, geometry) {
      if (!_stabilizerActive) return;
      const anchor = geometry.worldToScreen({
        x: _stabilizerRawX,
        y: _stabilizerRawY,
      });
...
function _updateStabilizerLeash() {
  _ensureStabilizerLeash();
  if (!_stabilizerLeashOverlay) return;
  const enabled = window._tsLeashEnabled !== false;
  const atFloor = _stabilizationAmount() <= 0;
  const dist = Math.hypot(
    _stabilizerRawX - _stabilizerX,
    _stabilizerRawY - _stabilizerY,
  );
  const visible =
    enabled &&
    !atFloor &&
    _stabilizerActive &&
    dist > 0.15 / Math.max(0.05, zoom);
  _stabilizerLeashOverlay.setVisible(visible);
  if (visible) _stabilizerLeashOverlay.invalidate();
}
function _hideStabilizerLeash() {
  if (_stabilizerLeashOverlay) _stabilizerLeashOverlay.setVisible(false);
}
...
function _stabilizerNearTargetRateMultiplier() {
  ...
  const near = Math.max(
    0,
    Math.min(1, 1 - gapScreenPx / _STABILIZER_NEAR_TARGET_SCREEN_PX),
  );
...
function _stabilizerRecoveryRateMultiplier(now) {
  const gapScreenPx = _stabilizerGapCanvas() * Math.max(0.05, zoom);
  const errorStrength = Math.max(
    0,
    Math.min(
      1,
      (gapScreenPx - _STABILIZER_CATCHUP_EXIT_SCREEN_PX) /
        (_STABILIZER_RECOVERY_ERROR_FULL_SCREEN_PX -
          _STABILIZER_CATCHUP_EXIT_SCREEN_PX),
    ),
  );
  const speedRatio =
    _stabilizerRawPeakSpeed > 0
      ? _stabilizerRawSpeed / _stabilizerRawPeakSpeed
      : 1;
  const decelerationStrength = Math.max(
    0,
    Math.min(
      1,
      (_STABILIZER_RECOVERY_RESUME_RATIO - speedRatio) /
        _STABILIZER_RECOVERY_RESUME_RATIO,
    ),
  );
  const ramp = Math.max(
    0,
    Math.min(
      1,
      (now - _stabilizerRecoveryStartT) / _STABILIZER_RECOVERY_RAMP_MS,
    ),
  );
...
function _stabilizerRecoveryTicks(windowLen, now) {
  ...
  _stabilizerRecoveryLastT = now;
  _stabilizerRecoveryTickCarry +=
    ((elapsedMs * windowLen) / _STABILIZER_CATCHUP_MS) *
...
function _stabilizerCatchupTicks(dtMs, windowLen, now) {
  ...
  if (_stabilizerFinishing) {
    ...
    const timeProgress = Math.max(
      0,
      Math.min(1, elapsed / _stabilizerFinishTargetMs),
    );
    const s = timeProgress * timeProgress * (3 - 2 * timeProgress); // smoothstep
    const avgTicksPerMs = windowLen / _stabilizerFinishTargetMs;
    ticksPerMs =
      avgTicksPerMs * 0.6 + (avgTicksPerMs * 1.6 - avgTicksPerMs * 0.6) * s;
  } else {
    ticksPerMs =
      (windowLen / _STABILIZER_CATCHUP_MS) *
      _stabilizerNearTargetRateMultiplier();
  }
  _stabilizerCatchupTickCarry += elapsedMs * ticksPerMs;
  const ticks = Math.max(
    0,
    Math.min(windowLen, Math.floor(_stabilizerCatchupTickCarry)),
  );
...
function _stabilizerAdvance(dt, now) {
  ...
  const recoveryTicks = !_stabilizerFinishing
    ? _stabilizerRecoveryTicks(windowLen, now)
    : null;
  const ticks =
    recoveryTicks === null
      ? _stabilizerCatchupTicks(dtMs, windowLen, now)
      : recoveryTicks;
  ...
  for (let i = 0; i < ticks; i++) {
    _stabilizerBuf.push({
      x: _stabilizerTargetX,
      y: _stabilizerTargetY,
      t: now,
    });
    _stabilizerPressureBuf.push(_stabilizerTargetPressure);
    _stabilizerTrimBuf(windowLen);
    ...
    _stabilizerSmoothedPressure = _stabilizerPressureAverage();
    const moved =
      Math.hypot(step.x - _stabilizerX, step.y - _stabilizerY) > 1e-4;
    _stabilizerX = step.x;
    _stabilizerY = step.y;
    if (!moved) break;
  ...
  const positionConverged =
    gapCanvas * Math.max(0.05, zoom) < _STABILIZER_EPS_SCREEN_PX &&
    gapCanvas < 0.4;
  ...
  if (converged) {
    ...
    if (gapCanvas > 0.001 || pressureGap > 0) {
      _stabilizerX = _stabilizerTargetX;
      _stabilizerY = _stabilizerTargetY;
      _stabilizerSmoothedPressure = _stabilizerTargetPressure;
      _stabilizerEmit(_stabilizerX, _stabilizerY, now);
    }
    if (_stabilizerFinishing) {
...
function _stabilizerStep(now) {
  ...
  const shouldCatchUp =
    _stabilizerFinishing ||
    _stabilizerUpdateCatchupState() ||
    _stabilizerUpdateRecoveryState(now);
  if (
    !shouldCatchUp &&
    now - _stabilizerLastInputWallT < _STABILIZER_IDLE_DELAY_MS
  ) {
    _stabilizerSchedule();
    return;
  }
  const dt = Math.max(
    0.00025,
    Math.min(0.05, (now - _stabilizerLastAdvanceT) / 1000),
  );
...
function _stabilizerFinalize(x, y, pressure, event, cb) {
  ...
  const speedNorm = Math.max(
    0.5,
    Math.min(3, 0.6 + (_strokeVelocity || 0) * 1.2),
  );
  let targetMs =
    (_STABILIZER_FINISH_MIN_MS +
      gapNorm * (_STABILIZER_FINISH_MAX_MS - _STABILIZER_FINISH_MIN_MS)) /
    speedNorm;
  targetMs = Math.max(
    _STABILIZER_FINISH_MIN_MS,
    Math.min(_STABILIZER_FINISH_MAX_MS, targetMs),
  );
  _stabilizerFinishStartT = performance.now();
  _stabilizerFinishTargetMs = targetMs;
  _stabilizerFinalizeCB = () => {
    if (ownerSession !== _activeStrokeSession) {
      _traceStrokeLifecycle("stabilizer-finalize-rejected", {
        ownerSession,
        reason: "obsolete-session",
      });
      return;
    }
    cb();
...
function _resolveDabRotation(x, y) {
  ...
  if (_rotationPrevValid) {
    const dx = x - _rotationPrevX,
      dy = y - _rotationPrevY;
    if (dx || dy) _rotationDirection = Math.atan2(dy, dx);
  } else _rotationDirection = fixed;
  _rotationPrevX = x;
  _rotationPrevY = y;
  _rotationPrevValid = true;
  return window._tsRotationMode === "stroke-direction"
    ? _rotationDirection
    : fixed;
}

function _stampDab(x, y, e) {
  const startTrace = window.CustomFirstDabTrace,
    startTraceAt = startTrace && startTrace.enabled ? performance.now() : 0;
  const perf = _brushPerf(),
    paramsStart = perf ? performance.now() : 0;
  const { r, alpha } = _getEffectiveBrushParams(e);
  if (startTrace && startTrace.enabled)
    startTrace.event("brush-parameters-resolved", {
      duration: performance.now() - startTraceAt,
      radius: r,
      alpha,
    });
  if (perf)
    perf.measure("brush-parameter-resolution", paramsStart, {
      dabNumber: _strokeDabCount,
    });
  ...
  if (perf)
    perf.measure("blend-mode-setup", blendSetupStart, {
      tool,
      brushBlendMode: tool === "brush" ? window.brushBlendMode : null,
      composite,
    });
  const rotation = _resolveDabRotation(x, y);
  const scatterEnabled = !!window._tsScatterEnabled;
  const count = scatterEnabled
    ? Math.min(50, Math.max(1, Math.round(window._tsScatterCount || 1)))
    : 1;
  ...
  for (let dabIndex = 0; dabIndex < count; dabIndex++) {
    ...
    const angleJit = window.brushTipCanvas
      ? window.brushTipAngleJitter || 0
      : 0;
    if (angleJit > 0)
      dabRotation += (Math.random() * 2 - 1) * Math.PI * angleJit;
    let dabRoundness = null;
    const roundJit = window.brushTipCanvas
      ? window.brushTipRoundnessJitter || 0
      : 0;
    if (roundJit > 0) {
      const baseR =
      ...
      const minR = Math.max(
        window.brushTipMinimumRoundness || 0,
        baseR - roundJit * (baseR - (window.brushTipMinimumRoundness || 0)),
      );
...
function _airbrushIntervalMs() {
  const rate =
    typeof window !== "undefined" && window._tsAirbrushRate != null
      ? window._tsAirbrushRate
      : 0.55;
...
function _startAirbrushSpray() {
  ...
  _airbrushTimer = setInterval(() => {
    if (
      !drawing ||
      !window._brushAirbrush ||
      !window._brushContinuousSpraying
    ) {
      _stopAirbrushSpray();
      return;
    }
    ...
    if (
      performance.now() - _airbrushLastMovementTime <
      Math.max(50, _airbrushIntervalMs() * 2)
    )
      return;
    const previousSpacingRatio = _flowSpacingRatio;
    _flowSpacingRatio = _airbrushCanonicalSpacingRatio(
      _lastPointerEvent,
      currentPressure,
    );
    try {
      ...
      _flowSpacingRatio = previousSpacingRatio;
    }
    _scheduleRecomposite();
  }, _airbrushIntervalMs());
}
...
function _effectiveSpacingFrac(settings) {
  ...
  const mode = fromSettings
    ? settings["ts-spacing-mode"]
    : document.getElementById("ts-spacing-mode")?.value;
  const isAirbrush = fromSettings
    ? !!settings["ts-airbrush"]
    : !!window._brushAirbrush;
  const raw = fromSettings
    ? Number(settings["ts-spacing"]) / 100
    : typeof window !== "undefined" && window._tsSpacing != null
      ? Number(window._tsSpacing)
      : NaN;
...
function _airbrushCanonicalSpacingRatio(e, pressure) {
  const radius = _computeSpacingRadius(
    e || _lastPointerEvent,
    pressure == null ? currentPressure : pressure,
  );
...
function _walkDabArc(
  ...
  const pAt =
    pressureAt || ((t) => startPressure + (endPressure - startPressure) * t);
  if (length <= 0) {
...
function _quadArcTable(x0, y0, cx, cy, x1, y1) {
  ...
  while (stack.length) {
    const seg = stack.pop();
    const flat =
      seg.depth >= _QUAD_MAX_SPLIT_DEPTH ||
      segmentCount >= _QUAD_MAX_SEGMENTS ||
      _quadFlatnessDeviation(seg.x0, seg.y0, seg.cx, seg.cy, seg.x1, seg.y1) <=
        _QUAD_FLATNESS_TOLERANCE;
    if (flat) {
    ...
    const m0x = (seg.x0 + seg.cx) / 2,
      m0y = (seg.y0 + seg.cy) / 2;
    const m1x = (seg.cx + seg.x1) / 2,
      m1y = (seg.cy + seg.y1) / 2;
    const mx = (m0x + m1x) / 2,
      my = (m0y + m1y) / 2;
...
function _quadPointAtLength(table, distance) {
  ...
  while (lo < hi) {
    const mid = (lo + hi) >> 1;
    if (table[mid].length < distance) lo = mid + 1;
    else hi = mid;
  }
  const b = table[lo],
    a = table[lo - 1];
...
function _stampQuadCurve(
  ...
  _walkDabArc(
    len,
    (d) => _quadPointAtLength(arcTable, d),
    e,
    startPressure,
    endPressure,
  );
  ...
  let traveled = -_strokeSegCarryOver;
  while (true) {
    const sampleNow = _quadPointAtLength(
      arcTable,
      Math.max(0, Math.min(len, traveled)),
    );
...
let _curveP0 = null,
  ...
  _curvePr0 = 0,
  _curvePr1 = 0;
let _curveSubpixelConditioning = false,
  _curveBaselineSamples = null,
  _curveBaselineNext = 0,
  _curveBaselineRadius = 0;
function _resetCurve(x, y, pressure) {
  ...
  const viewScale = Math.max(0, Math.min(1, Number(zoom) || 1));
  _curveBaselineRadius = _curveSubpixelConditioning
    ? Math.max(1, Math.round(1 + 6 * (1 - viewScale)))
    : 0;
  _curveBaselineSamples = [];
  _curveBaselineNext = 0;
}
...
function _baselineSGCoefficients(radius) {
  if (_BASELINE_SG_COEFFICIENTS.has(radius))
    return _BASELINE_SG_COEFFICIENTS.get(radius);
  const n = radius * 2 + 1;
  let s2 = 0,
    s4 = 0;
  ...
  const coefficients = [];
  for (let i = -radius; i <= radius; i++)
    coefficients.push((s4 - s2 * i * i) / denominator);
...
function _curveBaselineIsCorner(samples, index, radius) {
  ...
  const a = samples[index - radius],
    b = samples[index],
    c = samples[index + radius];
  const abx = b.x - a.x,
    aby = b.y - a.y,
    bcx = c.x - b.x,
    bcy = c.y - b.y;
...
function _curveBaselineEmit(index, finalizing = false) {
  ...
  for (let j = -radius; j <= radius; j++) {
    const weight = coeffs[j + radius],
      sample = samples[index + j];
    x += sample.x * weight;
    y += sample.y * weight;
  }
...
function _curveAddPoint(x, y, pressure, e) {
  if (!_curveSubpixelConditioning) {
    _curveAddReconstructedPoint(x, y, pressure, e);
    return;
  }
  const samples = _curveBaselineSamples;
...
function _flushCurveTail(e) {
  ...
  _curveBaselineNext = 0;
  _curveBaselineRadius = 0;
  _curveSubpixelConditioning = false;
}
...
// geometry only.
const _LINE_LENGTH_EDIT_EPSILON = 0.75;
  ...
  for (let i = 1; i < samples.length; i++) {
    ...
    if (distance <= hi.distance) {
      const span = hi.distance - lo.distance,
        f = span > 0 ? (distance - lo.distance) / span : 0;
...
function _cropLinePressureProfile(newLength) {
  const gesture = _lineGesture,
    samples = gesture.pressureSamples;
  const removed = samples.reduce(
    (count, sample) => count + (sample.distance > newLength + 0.0001 ? 1 : 0),
    0,
  );
  const boundaryPressure = _linePressureAtDistance(samples, newLength);
  const retained = samples.filter(
    (sample) => sample.distance < newLength - 0.0001,
  );
  if (!retained.length || retained[0].distance > 0)
    retained.unshift({ distance: 0, pressure: samples[0].pressure });
  const last = retained[retained.length - 1];
  if (!last || Math.abs(last.distance - newLength) > 0.0001)
    retained.push({ distance: newLength, pressure: boundaryPressure });
  else last.pressure = boundaryPressure;
  gesture.pressureSamples = retained;
  gesture.recordedLength = newLength;
  currentPressure = boundaryPressure;
  _prevRawPressure = boundaryPressure;
  _lastKnownPressure = boundaryPressure;
  return removed;
}
function _editLinePressureProfile(lineEvents) {
  const gesture = _lineGesture,
    latest = lineEvents[lineEvents.length - 1];
  const sx = gesture.startPoint.x,
    sy = gesture.startPoint.y;
  ...
  let action = "rotate",
    ...
    currentEventPressure = Number(latest.event.pressure);
  if (getLinePressureMode() === "pen") {
    ...
    for (const sample of lineEvents) {
      ...
      if (distance < gesture.recordedLength - _LINE_LENGTH_EDIT_EPSILON) {
        action = "crop";
        removedSampleCount += _cropLinePressureProfile(distance);
        currentEventPressure =
          gesture.pressureSamples[gesture.pressureSamples.length - 1].pressure;
      } else if (
        distance >
        gesture.recordedLength + _LINE_LENGTH_EDIT_EPSILON
      ) {
        action = "extend";
        const pressure = _getPressure(sample.event);
        gesture.pressureSamples.push({ distance, pressure });
        _linePressureSamples.push({
          x: sample.point.x,
          y: sample.point.y,
          pressure,
        });
        gesture.recordedLength = distance;
        currentEventPressure = pressure;
        appendedSampleCount++;
      }
    }
  }
  const newLength = Math.hypot(latest.point.x - sx, latest.point.y - sy);
  gesture.currentLength = newLength;
  gesture.endPoint = { x: latest.point.x, y: latest.point.y };
  gesture.currentEventPressure = currentEventPressure;
  const samples = gesture.pressureSamples;
  gesture.lastEditDiagnostic = {
    previousLength,
    newLength,
    recordedLength: gesture.recordedLength,
    action,
    currentEventPressure,
    sampleCountBefore,
    sampleCountAfter: samples.length,
    removedSampleCount,
    appendedSampleCount,
    maxStoredDistance: samples.length
...
function _getLinePressureProfile(sx, sy, ex, ey) {
  const length = Math.max(0.0001, Math.hypot(ex - sx, ey - sy));
  if (!_lineGesture || !_lineGesture.pressureSamples.length)
    return [
      { t: 0, pressure: 1, distance: 0 },
      { t: 1, pressure: 1, distance: length },
    ];
  const profile = _lineGesture.pressureSamples.map((sample) => ({
    t: Math.max(0, Math.min(1, sample.distance / length)),
    pressure: sample.pressure,
    distance: sample.distance,
  }));
  const last = profile[profile.length - 1];
  if (last.t < 1)
    profile.push({ t: 1, pressure: last.pressure, distance: length });
  return profile;
}
function _buildLinePressureProfile(sx, sy, ex, ey) {
  ...
  return function pressureAt(t) {
    t = Math.max(0, Math.min(1, t));
    if (window.DEBUG_LINE_TOOL) _lineDiagnosticCurrentT = t;
...
function _curvePressureProfile() {
  const samples = (_lineGesture && _lineGesture.pressureSamples) || [],
    domain = Math.max(
      0.0001,
      (_lineGesture && _lineGesture.recordedLength) || 1,
    );
  return (t) =>
    _linePressureAtDistance(samples, Math.max(0, Math.min(1, t)) * domain);
}
function _strokeQuadraticProfile(p0, p1, p2, e, pressureAt) {
  const table = _quadArcTable(p0.x, p0.y, p1.x, p1.y, p2.x, p2.y),
    length = table[table.length - 1].length;
  _walkDabArc(
    length,
    (d) => {
      const point = _quadPointAtLength(table, d);
      point.t = length > 0 ? d / length : 1;
      return point;
    },
    e,
    0,
...
function _ensureCurveGuide() {
  ...
  _curveGuideOverlay = EditorOverlayRenderer.create("curve-tool-guide", {
    zIndex: 5,
    draw: function (g, geometry) {
      if (!_curveToolGesture || _curveToolGesture.phase !== "bending") return;
      const p0 = geometry.worldToScreen(_curveToolGesture.start),
        p1 = geometry.worldToScreen(_curveToolGesture.control),
        p2 = geometry.worldToScreen(_curveToolGesture.end);
      g.strokeStyle = "rgba(127,119,221,.9)";
      g.fillStyle = "#7f77dd";
      g.lineWidth = 1;
      g.setLineDash([4, 3]);
      g.beginPath();
...
function _clearCurveGuide() {
  if (_curveGuideOverlay) _curveGuideOverlay.setVisible(false);
}
function _drawCurveGuide() {
  _ensureCurveGuide();
  if (!_curveGuideOverlay) return;
  const visible = !!(
    _curveToolGesture && _curveToolGesture.phase === "bending"
  );
  _curveGuideOverlay.setVisible(visible);
  if (visible) _curveGuideOverlay.invalidate();
}
function _includeLinePreviewFrameBounds(bounds) {
  ...
  if (!_frameDirty)
    ...
    _frameDirty.minX = Math.min(_frameDirty.minX, bounds.minX);
    _frameDirty.minY = Math.min(_frameDirty.minY, bounds.minY);
    _frameDirty.maxX = Math.max(_frameDirty.maxX, bounds.maxX);
    _frameDirty.maxY = Math.max(_frameDirty.maxY, bounds.maxY);
  }
}
...
function _renderLineDrag(ex, ey, e, phase) {
  ...
  const curveBending =
    tool === "curve" &&
    _curveToolGesture &&
    _curveToolGesture.phase === "bending";
  const curveControl = curveBending ? { x: ex, y: ey } : null;
  if (curveBending) {
    ex = _curveToolGesture.end.x;
    ey = _curveToolGesture.end.y;
    _curveToolGesture.control = curveControl;
  }

  const previousEndpoint = _linePreviewPreviousEndpoint && {
    x: _linePreviewPreviousEndpoint.x,
    y: _linePreviewPreviousEndpoint.y,
  };
  const previousBounds = _linePreviewBounds && {
    minX: _linePreviewBounds.minX,
    minY: _linePreviewBounds.minY,
    maxX: _linePreviewBounds.maxX,
    maxY: _linePreviewBounds.maxY,
  };
  const transformBeforeClear =
    _strokeCtx && typeof _strokeCtx.getTransform === "function"
      ? _strokeCtx.getTransform()
      : null;
  const dabsBefore = _strokeDabCount,
    previewFrameId = ++_linePreviewFrameId;
  ...
  _smoothedPressure = usePenPressure
    ? _linePressureSamples && _linePressureSamples.length
      ? _linePressureSamples[0].pressure
      : currentPressure
    : 1; // Fixed Pressure / mouse / touch: matches the constant currentPressure=1 used below
  try {
    if (usePenPressure) {
      const pressureAt = curveBending
        ? _curvePressureProfile()
        : _buildLinePressureProfile(lineStart.x, lineStart.y, ex, ey);
      const startPressure = pressureAt(0),
        savedFlowSpacingRatio = _flowSpacingRatio;
      currentPressure = startPressure;
      _flowSpacingRatio = _initialDabSpacingRatio(e, startPressure);
      try {
        _stampDab(lineStart.x, lineStart.y, e);
      } finally {
        _flowSpacingRatio = savedFlowSpacingRatio;
      }
      currentPressure = pressureAt(1);
      if (curveBending)
        _strokeQuadraticProfile(
          lineStart,
          curveControl,
          { x: ex, y: ey },
          e,
      ...
      currentPressure = 1;
      try {
        const savedFlowSpacingRatio = _flowSpacingRatio;
        _flowSpacingRatio = _initialDabSpacingRatio(e, 1);
        try {
          _stampDab(lineStart.x, lineStart.y, e);
        } finally {
          _flowSpacingRatio = savedFlowSpacingRatio;
        ...
        _isDrawingWithPen = savedIsDrawingWithPen;
      }
    }
  } finally {
  ...
  _flushStrokeTail();
  _linePreviewBounds = _strokeDirty
    ? {
        minX: _strokeDirty.minX,
        minY: _strokeDirty.minY,
        maxX: _strokeDirty.maxX,
  ...
  if (window.DEBUG_LINE_TOOL) {
    const storedProfile = usePenPressure
      ? _getLinePressureProfile(lineStart.x, lineStart.y, ex, ey)
      : [
          { t: 0, pressure: 1 },
          { t: 1, pressure: 1 },
        ];
    const diagnostic = {
      tool: "line",
      phase: _lineGesture ? _lineGesture.phase : "unknown",
      renderPhase: phase || "preview",
      pressureMode: getLinePressureMode(),
      rawPressureSamples: _linePressureSamples.map((sample) => ({
        x: sample.x,
        y: sample.y,
        pressure: sample.pressure,
      })),
      ...
      lineSliderSize: Number(
        document.querySelector('[data-option-kind="line-size"]')?.value ||
          toolSizes.line,
      ),
      shortcutUpdatedSize: window._lastLineShortcutSize || null,
      effectiveDiameter: getBrushSize(),
      canvasScale: 1,
      zoom,
      stampCount: _strokeDabCount - dabsBefore,
      currentEventPressure: _lineGesture
        ? _lineGesture.currentEventPressure
        : null,
      storedPressureProfile: _lineGesture
        ? _lineGesture.pressureSamples.map((sample) => ({
            distance: sample.distance,
            pressure: sample.pressure,
          }))
        : storedProfile,
      profileSampleCount: _lineGesture
        ? _lineGesture.pressureSamples.length
    ...
    const records =
      window.__lineToolDiagnostics || (window.__lineToolDiagnostics = []);
  ...
  if (window.DEBUG_LINE_PREVIEW) {
    const matrix = transformBeforeClear
      ? {
          a: transformBeforeClear.a,
          b: transformBeforeClear.b,
          c: transformBeforeClear.c,
          d: transformBeforeClear.d,
          e: transformBeforeClear.e,
          f: transformBeforeClear.f,
        }
      : null;
    console.debug("[LinePreview]", {
      previewFrameId,
      pointermoveSequence: _linePreviewMoveSequence,
      generation: _linePreviewGeneration,
      canvasWidth: (_strokeCanvas && _strokeCanvas.width) || 0,
      canvasHeight: (_strokeCanvas && _strokeCanvas.height) || 0,
      transformBeforeClear: matrix,
      clearRectangle: {
        x: 0,
        y: 0,
        width: (_strokeCanvas && _strokeCanvas.width) || 0,
        height: (_strokeCanvas && _strokeCanvas.height) || 0,
      },
      previousEndpoint,
      currentEndpoint: { x: ex, y: ey },
      brushTipDiameter: getBrushSize(),
      stampCount: _strokeDabCount - dabsBefore,
      stalePreviewDiscarded: false,
      previousBounds,
      currentBounds: _linePreviewBounds,
    });
  }
}
...
function _flushDeferredKeyVisualRefreshAfterPresentation() {
  ...
  _deferredKeyVisualRefreshAfterNextPresentation = false;
  if (window._scheduleDeferredKeyVisualRefreshAfterPresentation)
    window._scheduleDeferredKeyVisualRefreshAfterPresentation();
}
function _scheduleRecomposite(options) {
  const firstDab = !!(options && options.firstDab),
    perf = _brushPerf(),
    firstDabExperiment = window.BrushFirstDabExperiment,
    legacyExperiment =
      window.KeyframeLatencyExperiment &&
      window.KeyframeLatencyExperiment.active
        ? window.KeyframeLatencyExperiment
        : window.BrushRafExperiment,
    experiment = firstDabExperiment || legacyExperiment;
  ...
  if (
    ...
    window.FirstDabLatencyProbe.beforeSchedule();
  if (perf)
    perf.point("recomposite-requested", {
      firstDab,
      rafAlreadyPending: _recompRAF,
      visibility: document.visibilityState,
      focused: document.hasFocus(),
      framePhaseMs: performance.now() % 16.67,
    });
  if (experiment)
    experiment.noteRecompositeRequest({
      firstDab,
      rafAlreadyPending: _recompRAF,
    });
  const decision = firstDabExperiment
    ? firstDabExperiment.decide({ firstDab, rafAlreadyPending: _recompRAF })
    : null;
  const immediate = decision
    ? decision.immediate
    : experiment &&
      experiment.shouldPresentImmediately({
        firstDab,
        rafAlreadyPending: _recompRAF,
      });
  if (immediate) {
    ...
    if (_recompRAF && _recompRAFHandle) {
      ...
      _recompRAF = false;
      _recompCoalescedRequests = 0;
      scheduledWork = "cancelled-and-merged";
    }
    _flushTinyTipCoverageTiles();
    const rect = drawing || _inStroke ? _consumeDirtyRect() : null;
    if (perf)
      perf.point("first-dab-immediate-recomposite", {
        mode: firstDabExperiment ? firstDabExperiment.mode : experiment.mode,
        rect,
        scheduledWork,
      });
    ...
    if (_hrTlActive()) {
      ...
      if (accepted) {
        _hrTlRecordStage("recomposite", accepted.x, accepted.y, performance.now());
        _hrTlPush("ages", "acceptedPreviewToRecomposite", performance.now() - accepted.wallTime);
      }
    }
    if (_hrLcActive()) {
      _hrLcInc("recomposite");
    ...
    _hrMtRecord(
      ...
      immediateStart,
      immediateStart + immediateDuration,
    );
    _flushDeferredKeyVisualRefreshAfterPresentation();
    if (perf)
      perf.recordDuration(
        "synchronous-first-dab-recomposite",
        immediateDuration,
        { rect, scheduledWork },
      );
    if (firstDabExperiment)
      firstDabExperiment.notePresentation({
        kind: "synchronous-first-dab",
        rect,
  ...
  if (_recompRAF) {
    _recompCoalescedRequests++;
    if (firstDabExperiment && firstDab)
      firstDabExperiment.noteScheduledDisposition("reused");
    if (experiment) experiment.noteCoalescedRequest();
    return;
  }
  _recompRAF = true;
  _recompCoalescedRequests = 0;
  const generation = _recompGeneration,
    ...
    frameIndex = curFrame,
    sessionId = _activeStrokeSession;
  _traceStrokeLifecycle("recomposite-scheduled", {
    sessionId,
    sourceLayer: layerIndex,
    sourceFrame: frameIndex,
  });
  const scheduleProfiler = _brushPerf(),
    scheduledAt = performance.now(),
    phase = scheduledAt % 16.67,
    estimatedNextDeadlineMs = 16.67 - phase;
  if (
    firstDab &&
    window.FirstDabLatencyProbe &&
    window.FirstDabLatencyProbe.enabled
  )
    window.FirstDabLatencyProbe.rafScheduled(scheduledAt);
  if (scheduleProfiler)
    scheduleProfiler.point("recomposite-raf-scheduled", {
      firstDab,
      rafAlreadyPending: false,
      visibility: document.visibilityState,
      focused: document.hasFocus(),
      framePhaseMs: phase,
      estimatedNextDeadlineMs,
      intentionallyDeferred: !!firstDab,
    });
  const rafState = experiment ? experiment.rafState() : null;
  _recompRAFHandle = requestAnimationFrame(() => {
    const callbackAt = performance.now(),
      wait = callbackAt - scheduledAt,
      coalesced = _recompCoalescedRequests;
    if (
      firstDab &&
      window.FirstDabLatencyProbe &&
      window.FirstDabLatencyProbe.enabled
    )
      window.FirstDabLatencyProbe.rafCallback(callbackAt);
    if (scheduleProfiler) {
      scheduleProfiler.point("recomposite-raf-callback-begin", {
        waitMs: wait,
        coalescedRequests: coalesced,
        framePhaseMs: callbackAt % 16.67,
        estimatedMissedUpcomingDeadline: wait > estimatedNextDeadlineMs + 1,
        anotherAppRafCallbackRanFirst: experiment
          ? experiment.anotherRafRanFirst(rafState)
          : null,
      });
      scheduleProfiler.measure("raf-wakeup-wait", scheduledAt, {
        coalescedRequests: coalesced,
      });
    }
    if (experiment)
      experiment.noteRafCallback({
        scheduledAt,
        callbackAt,
        waitMs: wait,
        coalescedRequests: coalesced,
        estimatedNextDeadlineMs,
        estimatedMissedUpcomingDeadline: wait > estimatedNextDeadlineMs + 1,
        anotherAppRafCallbackRanFirst: experiment.anotherRafRanFirst(rafState),
      });
    if (generation !== _recompGeneration) {
      _traceStrokeLifecycle("recomposite-rejected", {
        sessionId,
        reason: "generation",
        sourceLayer: layerIndex,
        sourceFrame: frameIndex,
    ...
    if (sessionId !== _activeStrokeSession) {
      _traceStrokeLifecycle("recomposite-rejected", {
        sessionId,
        reason: "obsolete-session",
        sourceLayer: layerIndex,
    ...
    _recompRAFHandle = 0;
    _recompCoalescedRequests = 0;
    if (curLayer !== layerIndex || curFrame !== frameIndex) {
      _traceStrokeLifecycle("recomposite-rejected", {
        sessionId,
        reason: "artwork-changed",
        sourceLayer: layerIndex,
        sourceFrame: frameIndex,
      });
      _flushDeferredKeyVisualRefreshAfterPresentation();
    ...
    _flushDeferredKeyVisualRefreshAfterPresentation();
    if (scheduleProfiler)
      scheduleProfiler.recordDuration(
        "scheduled-recomposite-duration",
        scheduledDuration,
        { rect, firstDab },
      );
    if (firstDabExperiment && firstDab)
      firstDabExperiment.notePresentation({
        kind: "scheduled-first-dab",
        rect,
...
function _completePostStrokePresentation(layerIndex, frameIndex) {
  _traceStrokeLifecycle("pointerup-barrier", {
    sourceLayer: layerIndex,
    sourceFrame: frameIndex,
  });
  ...
  _recompGeneration++;
  if (_recompRAFHandle) cancelAnimationFrame(_recompRAFHandle);
  _recompRAFHandle = 0;
  _recompRAF = false;
  _flushLiveColorEraserPreview();
  if (_strokeCtx && _strokeCanvas)
...
function _endStroke(pointerId) {
  ...
  if (
    ...
    pointerId !== _activeStrokePointerId
  )
    return;
  ...
  if (drawing) {
    ...
    if (_inStroke) {
      _inStroke = false;
      if (!_hardRoundStrokeWasCancelled) _commitStrokeCanvas();
    }
    _restoreSelectionScopePixels();
    _cleanupErasedSmartOwnership();
    saveActiveToKey();
  ...
  lineStart = null;
  _lineDragging = false;
  _linePreviewBounds = null;
  _linePreviewPreviousEndpoint = null;
  _linePressureSamples = [];
  _lineGesture = null;
  _curveToolGesture = null;
  _clearCurveGuide();
  _pendingDabs.length = 0;
  _curveP0 = null;
  _curveP1 = null;
...
window.finishActiveDrawingBeforeArtworkChange = function (
  nextLayer,
  nextFrame,
) {
  ...
  if (_stabilizerFinishing && _stabilizerFinalizeCB) {
    _traceStrokeLifecycle("artwork-change-finishes-stabilizer", {
      destinationLayer: nextLayer,
      destinationFrame: nextFrame,
    });
  ...
  const destinationLayer = nextLayer,
    destinationFrame = nextFrame;
  ...
  try {
    ...
    _frameDirty = null;
    _strokeDirty = null;
    curLayer = destinationLayer;
    curFrame = destinationFrame;
  }
  return true;
};
window.isDrawingActive = function () {
  return !!(
    ...
    (typeof _stabilizerFinishing !== "undefined" &&
      _stabilizerFinishing &&
      _stabilizerFinalizeCB)
  );
};
...
window.awaitPendingBrushCommits = async function () {
  while (true) {
    ...
    const pendingHardRound =
      typeof _hardRoundPendingCommitCount === "number" &&
      _hardRoundPendingCommitCount > 0;
    const pendingCustomTip =
      typeof _customTipPendingCommitCount === "number" &&
      _customTipPendingCommitCount > 0;
  ...
  if (_lastArtworkCommitError) {
    ...
    throw new Error(
      "A pending brush stroke could not be committed to the canvas: " +
...
function _getPrototypePressure(e) {
  ...
  return typeof e.pressure === "number"
...
const _RELEASE_TAIL_PRESSURE_MAX = 0.2;
...
function _contactFilteredPressure(pressure, x, y, pointerType) {
  ...
  if (Number.isFinite(pressure) && pressure > _CONTACT_PRESSURE_FLOOR)
    _lastContactPressure = pressure;
  return pressure;
}
let _lastMoveTime = 0;
let _lastMoveX = 0,
...
let _elSizeControl,
  ...
  _elStartTaper,
  _elEndTaper;
function _getSizeControl() {
  if (_elSizeControl === undefined)
    _elSizeControl = document.getElementById("ts-size-control");
  return _elSizeControl ? _elSizeControl.value : "pressure";
}
function _getFlowControl() {
  if (_elFlowControl === undefined)
    _elFlowControl = document.getElementById("ts-flow-control");
  return _elFlowControl ? _elFlowControl.value : "off";
}
function _getOpacityControl() {
  if (_elOpacityControl === undefined)
    _elOpacityControl = document.getElementById("ts-opacity-control");
  return _elOpacityControl ? _elOpacityControl.value : "pressure";
}
function _getMinSize() {
  if (_elMinSize === undefined)
    _elMinSize = document.getElementById("ts-min-size");
  return _elMinSize ? +_elMinSize.value / 100 : 0.05;
}
function _getMinFlow() {
  if (_elMinFlow === undefined)
    _elMinFlow = document.getElementById("ts-min-flow");
  return _elMinFlow ? +_elMinFlow.value / 100 : 0;
}
function _getTaperMode() {
  if (_elTaperMode === undefined)
    _elTaperMode = document.getElementById("ts-taper-mode");
  return _elTaperMode ? _elTaperMode.value : "off";
}
function _getStartTaper() {
  if (_getTaperMode() !== "percentage") return 0;
  if (_elStartTaper === undefined)
    _elStartTaper = document.getElementById("ts-start-taper");
  return _elStartTaper ? +_elStartTaper.value / 100 : 0;
}
function _getEndTaper() {
  if (_getTaperMode() !== "percentage") return 0;
  if (_elEndTaper === undefined)
    _elEndTaper = document.getElementById("ts-end-taper");
  return _elEndTaper ? +_elEndTaper.value / 100 : 0;
}
function _getPressureCurve(setting) {
  const el = document.getElementById("ts-" + setting + "-pressure-curve");
  const mode = el ? el.value : "linear";
  if (mode === "custom") {
    const custom =
      window._tsCustomPressureCurves && window._tsCustomPressureCurves[setting];
...
function _evalPressureCurveY(curveKey, x) {
  ...
  if (pts.length === 2) {
    const t = Math.max(
      0,
      Math.min(1, (x - pts[0][0]) / (pts[1][0] - pts[0][0] || 1)),
    );
  ...
  for (let i = 0; i < 24; i++) {
    const mid = (lo + hi) / 2;
    if (_bezierPointAt(pts, mid)[0] < x) lo = mid;
    else hi = mid;
  }
...
function _applyPressureCurve(p, curveKey = "linear") {
  ...
  if (Array.isArray(curveKey)) {
    ...
    for (let i = 0; i < 24; i++) {
      const mid = (lo + hi) / 2;
      if (_bezierPointAt(curveKey, mid)[0] < pressure) lo = mid;
      else hi = mid;
    }
    y = _bezierPointAt(curveKey, (lo + hi) / 2)[1];
...
function _resolveControl(ctrl, e) {
  switch (ctrl) {
    case "pressure": {
      ...
      _smoothedPressure =
        _smoothedPressure * (1 - _PRESSURE_SMOOTH) +
        currentPressure * _PRESSURE_SMOOTH;
...
function _computeEffectiveParams(e) {
  ...
  function _getPressureInfluence() {
    if (_pressureInfluence === null) {
      ...
      if (window.DEBUG_LINE_TOOL && tool === "line")
        _lineEffectivePressureSamples.push({
          t: _lineDiagnosticCurrentT,
          rawPressure: currentPressure,
          effectivePressure: _pressureInfluence,
        });
    }
  ...
  if (flowCtrl !== "off" && !_hardRoundTailCoverageOnly) {
    ...
    if (applyFlow) {
      let influence =
        flowCtrl === "pressure"
          ? _applyPressureCurve(
              _getPressureInfluence(),
              _getPressureCurve("flow"),
            )
          : _resolveControl(flowCtrl, e);
      if (flowCtrl === "pressure" && sizeCtrl === "pressure" && isPenStroke)
        influence = Math.sqrt(influence);
      const minFlow = _getMinFlow();
      baseAlpha *= Math.max(
        0,
        Math.min(1, minFlow + (1 - minFlow) * influence),
      );
      alpha = baseAlpha;
    }
  }
  ...
  if (opacityCtrl !== "off" && !_hardRoundTailCoverageOnly) {
    ...
    if (applyOpacity) {
      const influence =
        opacityCtrl === "pressure"
          ? _applyPressureCurve(
              _getPressureInfluence(),
              _getPressureCurve("opacity"),
            )
          : _resolveControl(opacityCtrl, e);
  ...
  const minimumAlpha =
    window._brushAirbrush && !window.brushTipCanvas ? 0 : 0.01;
  return {
    r: Math.max(0.05, r),
    alpha: Math.max(minimumAlpha, Math.min(1, alpha)),
  };
}
...
function _computeSpacingRadius(e, interpolatedPressure) {
  ...
  if (sizeCtrl !== "off") {
    ...
    if (applySize) {
      ...
      const influence =
        sizeCtrl === "pressure"
          ? _applyPressureCurve(rawInfluence, _getPressureCurve("size"))
          : rawInfluence;
  ...
  if (window.brushTipCanvas && window.brushTipSpacingBasis === "image-width") {
    const tipNativeW =
      window.brushTipCanvas.width || window.brushTipCanvas.naturalWidth || 1;
    const tipNativeH =
      window.brushTipCanvas.height || window.brushTipCanvas.naturalHeight || 1;
    const referenceDiameter = Number(window.brushTipReferenceDiameter);
    const spacingReference =
      Number.isFinite(referenceDiameter) && referenceDiameter > 0
        ? referenceDiameter
    ...
    const transformedTipWidth = Math.max(
      0.1,
      tipNativeW *
        ((r * 2) / spacingReference) *
        (compressWidth ? tipRoundness : 1),
    );
    return transformedTipWidth / 2;
  }
  const isHardRoundPressure =
    tool === "brush" &&
    _isDrawingWithPen &&
    !window.brushTipCanvas &&
    brushHardness >= 0.995 &&
...
let _hardRoundRendererCreatedPending = false;
let _hardRoundRendererResizePending = false;
...
if (typeof window.HardRoundDebugFirstPresentTiming === "undefined")
  window.HardRoundDebugFirstPresentTiming = false;
if (typeof window.HardRoundPresenterWarm === "undefined")
  window.HardRoundPresenterWarm = false;
...
function _hrFirstPresentRecord(strokeId) {
  ...
  let record = _hardRoundFirstPresentTiming.find(
    (item) => item.strokeId === strokeId,
  );
  if (!record && _hardRoundFirstPresentTiming.length < 5) {
    record = {
      strokeId,
      pointerDownTime: _hardRoundPointerDownAt || performance.now(),
      beginStrokeStart: null,
      beginStrokeEnd: null,
      firstRawInputTime: null,
      firstStabilizedInputTime: null,
      firstSegmentGeneratedTime: null,
      firstGpuEncodeStart: null,
      firstGpuSubmitTime: null,
      firstPresentCallTime: null,
      firstOverlayVisibleTime: null,
      rendererCreated: !!_hardRoundRendererCreatedPending,
      rendererReused: !_hardRoundRendererCreatedPending,
      warmReserveAvailableAtBegin: _hardRoundRendererPool.filter(
        (renderer) =>
          renderer.width === activeC.width &&
          renderer.height === activeC.height &&
          renderer.gpu &&
          renderer.gpu.isAvailable &&
          renderer.gpu.isAvailable(),
      ).length,
      gpuReadyAtBegin: false,
      presenterWarmAtBegin: !!window.HardRoundPresenterWarm,
      backend: null,
      previewGenerationAtBegin: null,
      overlayOwnerAtBegin: null,
      presentLivePreviewCallCount: 0,
      presentLivePreviewAcceptedCount: 0,
      cpuPreviewAcceptedCount: 0,
      presentLivePreviewRejectedCount: 0,
      overlayShownCount: 0,
      gpuClearCountThisStroke: 0,
      gpuGeometrySubmitCountThisStroke: 0,
      gpuPresentSubmitCountThisStroke: 0,
      firstPresentReason: null,
    };
...
function _hrFirstPresentNote(strokeId, name, time = performance.now()) {
  const record = _hrFirstPresentRecord(strokeId);
  if (record && record[name] == null) record[name] = time;
}
function _hrFirstPresentCount(strokeId, name, reason) {
  const record = _hrFirstPresentRecord(strokeId);
  if (!record) return;
  record[name] = (record[name] || 0) + 1;
  if (reason && record.firstPresentReason == null)
    record.firstPresentReason = reason;
}
...
window.HardRoundAnalyzeFirstPresentTiming = function () {
  const strokes = _hardRoundFirstPresentTiming.map((record) => {
    const delta = (a, b) =>
      record[a] != null && record[b] != null ? record[b] - record[a] : null;
    return Object.assign({}, record, {
      pointerDownToBeginMs: delta("pointerDownTime", "beginStrokeStart"),
      beginToFirstInputMs: delta("beginStrokeEnd", "firstRawInputTime"),
      firstInputToFirstSegmentMs: delta(
        "firstRawInputTime",
        "firstSegmentGeneratedTime",
      ),
      firstSegmentToSubmitMs: delta(
        "firstSegmentGeneratedTime",
        "firstGpuSubmitTime",
      ),
      submitToPresentCallMs: delta(
        "firstGpuSubmitTime",
        "firstPresentCallTime",
      ),
      presentCallToOverlayVisibleMs: delta(
        "firstPresentCallTime",
        "firstOverlayVisibleTime",
      ),
      pointerDownToOverlayVisibleMs: delta(
        "pointerDownTime",
        "firstOverlayVisibleTime",
      ),
    });
  ...
  return {
    presenterWarm: !!window.HardRoundPresenterWarm,
    count: strokes.length,
    strokes,
  };
};
if (typeof window.HardRoundDebugRapidPresentation === "undefined")
  window.HardRoundDebugRapidPresentation = false;
...
function _hrRapidPresentation(event, strokeId, extra) {
  ...
  _hardRoundRapidPresentationLog.push(
    Object.assign(
      {
        time: performance.now(),
        event,
        strokeId: strokeId == null ? _activeStrokeSession : strokeId,
        activeSession: _activeStrokeSession,
        overlayOwner:
        ...
        pendingCommitCount: _hardRoundPendingCommitCount,
      },
      extra || {},
    ),
  );
  if (_hardRoundRapidPresentationLog.length > 200)
    _hardRoundRapidPresentationLog.splice(
      0,
      _hardRoundRapidPresentationLog.length - 200,
    );
}
window.HardRoundAnalyzeRapidPresentation = function () {
  const log = _hardRoundRapidPresentationLog.slice(),
    byStroke = {};
  log.forEach((entry) => {
    const key = entry.strokeId == null ? "none" : String(entry.strokeId),
      bucket =
        byStroke[key] ||
        (byStroke[key] = {
          commitStarted: 0,
          readbackCompleted: 0,
          commitCompleted: 0,
          recompositeCompleted: 0,
          finalizerCompleted: 0,
        });
    if (Object.prototype.hasOwnProperty.call(bucket, entry.event))
      bucket[entry.event]++;
  });
  const incomplete = Object.entries(byStroke)
    .filter(
      ([, v]) =>
        v.commitStarted !== v.commitCompleted ||
        v.commitCompleted !== v.finalizerCompleted,
    )
    .map(([strokeId, counts]) => ({ strokeId, counts }));
  const requested = log
    .filter((entry) => entry.event === "commitStarted")
    .map((entry) => entry.strokeId);
  const resolved = log
    .filter((entry) => entry.event === "readbackCompleted")
    .map((entry) => entry.strokeId);
  const committed = log
    .filter((entry) => entry.event === "commitCompleted")
    .map((entry) => entry.strokeId);
  const duplicates = committed.filter(
    (strokeId, index) => committed.indexOf(strokeId) !== index,
  );
  const dropped = requested.filter((strokeId) => !committed.includes(strokeId));
  let outOfOrderCommitCount = 0;
  for (let index = 0; index < committed.length; index++)
    if (committed[index] !== requested[index]) outOfOrderCommitCount++;
  let outOfOrderResolveCount = 0;
  for (let index = 0; index < resolved.length; index++)
    if (resolved[index] !== requested[index]) outOfOrderResolveCount++;
  return {
    count: log.length,
    byStroke,
    incompleteStrokeCount: incomplete.length,
    incomplete,
    hardRoundCommitRequestedCount: requested.length,
    hardRoundCommitCompletedCount: committed.length,
    hardRoundCommitOrder: committed.slice(-20),
    hardRoundLastCommittedStrokeId: committed.length
      ? committed[committed.length - 1]
      : null,
    hardRoundOutOfOrderResolveCount: outOfOrderResolveCount,
    hardRoundOutOfOrderCommitCount: outOfOrderCommitCount,
    hardRoundConcurrentScratchConflictCount: 0,
    hardRoundDroppedAuthoritativeCommitCount: dropped.length,
    hardRoundDuplicateAuthoritativeCommitCount: duplicates.length,
    recentCommitLifecycle: log
      .filter(
        (entry) =>
          entry.event === "commitStarted" ||
          entry.event === "readbackCompleted" ||
          entry.event === "commitCompleted",
      )
      .slice(-60),
    log,
  };
...
if (
  typeof window !== "undefined" &&
  window.HardRoundDebugPhase11A30 === undefined
) {
...
if (
  typeof window !== "undefined" &&
  window.HardRoundDebugBypassFinishedGpuPreview === undefined
) {
...
if (
  typeof window !== "undefined" &&
  window.HardRoundDebugPhase11A32 === undefined
) {
...
if (
  typeof window !== "undefined" &&
  window.HardRoundDebugKeepGpuOverlayAfterCommit === undefined
) {
...
if (
  typeof window !== "undefined" &&
  window.HardRoundDebugPhase11A33 === undefined
) {
...
function _hrFsPush(entry) {
  ...
  if (window.HardRoundFrameStallLog.length > _HR_FS_MAX_LOG)
    window.HardRoundFrameStallLog.shift();
}
  ...
  try {
    hasCapture =
      ...
      activeC.hasPointerCapture
        ? activeC.hasPointerCapture(e.pointerId)
        : null;
  } catch (_) {}
  return {
    ...
    cursor:
      typeof activeC !== "undefined" && activeC && activeC.style
        ? activeC.style.cursor
        : null,
  };
}
function _hrFsCounts() {
  return {
    pendingSegmentCount:
      typeof _hardRoundPendingRenderSegments !== "undefined" &&
      _hardRoundPendingRenderSegments
        ? _hardRoundPendingRenderSegments.length
        : null,
    rendererSegmentCount:
      typeof _hardRoundRenderer !== "undefined" && _hardRoundRenderer
        ? _hardRoundRenderer._segmentCount
        : null,
  };
}
function _hrFsStrokeId() {
  return typeof _activeStrokeSession !== "undefined"
    ? _activeStrokeSession
    : null;
}
function _hrFsActive() {
  return !!(
    window.HardRoundDebugFrameStall &&
    typeof _inStroke !== "undefined" &&
    _inStroke
  );
}
      ...
      if (delta >= _HR_FS_STALL_THRESHOLD_MS) {
        _hrFsPush(
          Object.assign(
            ...
            _hrFsCounts(),
            _hrFsCursorSnapshot(_lastPointerEvent),
          ),
        );
      }
    }
...
function _hrFsRecordFlushPending(durationMs, drawSegmentsMs, dispatchedCount) {
  if (!_hrFsActive()) return;
  if (durationMs > _hrFsAgg.maxFlushPendingMs)
    _hrFsAgg.maxFlushPendingMs = durationMs;
  if (drawSegmentsMs != null && drawSegmentsMs > _hrFsAgg.maxDrawSegmentsMs)
    _hrFsAgg.maxDrawSegmentsMs = drawSegmentsMs;
  if (
    durationMs >= _HR_FS_STALL_THRESHOLD_MS ||
    (drawSegmentsMs != null && drawSegmentsMs >= _HR_FS_STALL_THRESHOLD_MS)
  ) {
    _hrFsPush(
      Object.assign(
        {
...
function _hrFsRecordPresentResolve(startT) {
  ...
  if (dur >= _HR_FS_STALL_THRESHOLD_MS) {
    _hrFsPush(
      Object.assign(
        {
          type: "presentLivePreviewResolve",
          timestamp: t,
          strokeId: _hrFsStrokeId(),
          presentPromiseMs: dur,
        },
        _hrFsCounts(),
        _hrFsCursorSnapshot(_lastPointerEvent),
      ),
...
window.HardRoundAnalyzeFrameStalls = function () {
  ...
  const worst = scored.map(({ r }) => ({
    ...
    pendingSegmentCount:
      r.pendingSegmentCount != null ? r.pendingSegmentCount : null,
    rendererSegmentCount:
      r.rendererSegmentCount != null ? r.rendererSegmentCount : null,
    pointerCapture: r.pointerCapture != null ? r.pointerCapture : null,
    cursor: r.cursor != null ? r.cursor : null,
  }));
  const summary = Object.assign({}, _hrFsAgg, {
    totalActiveStrokeRafFrames: _hrFsAgg.frames,
  });
  const out = { summary, worstStallWindows: worst };
  console.log("[HardRoundFrameStall] summary", summary);
  console.table
    ? console.table(worst)
    : console.log("[HardRoundFrameStall] worst windows", worst);
  return out;
};
...
function _hrMtActive() {
  return !!(
    window.HardRoundDebugMainThreadCost &&
    typeof _inStroke !== "undefined" &&
    _inStroke
  );
}
function _hrMtRecord(name, startT, endT) {
  ...
  if (!agg) {
    agg = _hrMtAgg[name] = { max: 0, over8: 0, over16: 0, over33: 0, count: 0 };
    _hrMtAgg[name] = agg;
  }
  agg.count++;
  ...
  if (duration >= _HR_MT_THRESHOLD_MS) {
    ...
    if (window.HardRoundMainThreadCostLog.length > _HR_MT_MAX_LOG)
      window.HardRoundMainThreadCostLog.shift();
  }
}
...
window.HardRoundAnalyzeMainThreadCost = function () {
  const perOp = Object.keys(_hrMtAgg).map((name) =>
    Object.assign({ name }, _hrMtAgg[name]),
  );
  ...
  const out = {
    perOperationMax: perOp,
    top10SlowestOperations: top10,
    maxSingleOperationMs: maxSingleOp,
    note,
  };
  console.log("[HardRoundMainThreadCost] per-operation max/counts", perOp);
  console.table
    ? console.table(top10)
    : console.log("[HardRoundMainThreadCost] top10", top10);
...
function _hrRtFinalizeStroke() {
  ...
  try {
    isGpuActive =
      typeof _hardRoundRenderer !== "undefined" &&
      _hardRoundRenderer &&
      _hardRoundRenderer.isGpuActive
        ? _hardRoundRenderer.isGpuActive()
        : null;
  } catch (_) {}
  const entry = Object.assign({}, _hrRtCurrentStroke, {
    isGpuActive,
    timestamp: performance.now(),
  });
  window.HardRoundRoutingTraceLog.push(entry);
  if (window.HardRoundRoutingTraceLog.length > _HR_RT_MAX_LOG)
    window.HardRoundRoutingTraceLog.shift();
...
window.HardRoundAnalyzeRoutingTrace = function () {
  ...
  const bothRoutesSameStroke = log.filter(
    (s) => s.hardRoundStrokeActive && s.legacyDabPathEntered,
  );
  const legacyOnlyStrokes = log.filter(
    (s) => !s.hardRoundStrokeActive && s.legacyDabPathEntered,
  );
  const migratedOnlyStrokes = log.filter(
    (s) => s.hardRoundStrokeActive && !s.legacyDabPathEntered,
  );
  const out = {
    strokes: log,
...
// ---------------------------------------------------------------------
if (window.HardRoundDebugLegacyDabDuringGpu === undefined)
  window.HardRoundDebugLegacyDabDuringGpu = false;
if (window.HardRoundDebugBypassTinyCoverageDuringGpu === undefined)
  window.HardRoundDebugBypassTinyCoverageDuringGpu = false;
window.HardRoundLegacyDabDuringGpuLog =
  window.HardRoundLegacyDabDuringGpuLog || [];
window.HardRoundLegacyDabDuringGpuCount =
  window.HardRoundLegacyDabDuringGpuCount || 0;
...
function _hardRoundTraceLegacyDabDuringGpu(radius) {
  if (
    !window.HardRoundDebugLegacyDabDuringGpu ||
    _hardRoundStrokeActive !== true
  )
    return;
  const entry = {
    timestamp: performance.now(),
  ...
  if (
    window.HardRoundLegacyDabDuringGpuLog.length > _HR_LEGACY_DAB_DURING_GPU_MAX
  )
    window.HardRoundLegacyDabDuringGpuLog.shift();
}
function _hardRoundShouldBypassTinyCoverageDuringGpu() {
  return (
    window.HardRoundDebugBypassTinyCoverageDuringGpu === true &&
    _hardRoundStrokeActive === true &&
    _hardRoundRendererIsGpuActive()
  );
}
...
if (window.HardRoundDebugStaleFinalizer === undefined)
  window.HardRoundDebugStaleFinalizer = false;
if (window.HardRoundDebugGuardStaleFinalizer === undefined)
  window.HardRoundDebugGuardStaleFinalizer = false;
window.HardRoundStaleFinalizerLog = window.HardRoundStaleFinalizerLog || [];
const _HR_STALE_FINALIZER_MAX = 100;
function _hrStaleFinalizerMutation(
  originStrokeId,
  mutation,
  before,
  after,
  ...
  if (window.HardRoundDebugStaleFinalizer) {
    const entry = {
      timestamp: performance.now(),
      originStrokeId,
      currentActiveStrokeSession: _activeStrokeSession,
      mutation,
      before,
      after,
      stale,
    ...
    if (window.HardRoundStaleFinalizerLog.length > _HR_STALE_FINALIZER_MAX)
      window.HardRoundStaleFinalizerLog.shift();
  }
  // Production invariant: an async continuation may finish its own
  // renderer/readback/commit work, but it must never mutate the newer
  // session's shared live state. This used to be guarded only by the
  // default-off diagnostic flag, re-enabling the proven rapid-stroke race.
...
window.HardRoundAnalyzeStaleFinalizer = function () {
  ...
  for (const entry of log)
    mutationsByName[entry.mutation] =
      (mutationsByName[entry.mutation] || 0) + 1;
  return {
    staleMutationCount: stale.length,
    firstStaleMutation: stale.length ? stale[0] : null,
    mutationsByName,
    log,
  };
};
...
function _hrPerfMarkStrokeStart(strokeId) {
  ...
  try {
    performance.mark("HR-GPU-stroke-" + strokeId + "-pointerdown");
  } catch (_) {}
}
function _hrPerfMarkRaw(strokeId) {
  ...
  try {
    performance.mark(
      "HR-GPU-stroke-" + strokeId + "-raw-" + _hrPerfMarkRawCounter,
    );
  } catch (_) {}
}
function _hrPerfMarkPreviewRaf(strokeId) {
  if (!window.HardRoundDebugPerfMarks) return;
  try {
    performance.mark("HR-GPU-stroke-" + strokeId + "-preview-raf");
  } catch (_) {}
}
function _hrPerfMarkPointerup(strokeId) {
  if (!window.HardRoundDebugPerfMarks) return;
  try {
    performance.mark("HR-GPU-stroke-" + strokeId + "-pointerup");
  } catch (_) {}
}

function _hardRoundFlushPending(renderer) {
  if (!renderer || !_hardRoundPendingRenderSegments.length) return 0;
  const _hrLcFlushStart = _hrLcActive() ? performance.now() : null;
  const _hrFsFlushStart = _hrFsActive() ? performance.now() : null;
  const pending = _hardRoundPendingRenderSegments.splice(
    0,
    _hardRoundPendingRenderSegments.length,
  );
  const pendingTimes = _hardRoundPendingRenderSegmentTimes.splice(
    0,
    pending.length,
  );
  if (_hrLcFlushStart != null && pendingTimes.length) {
    const oldest = pendingTimes[0];
    _hrLcPending(
      pending.length + _hardRoundPendingRenderSegments.length,
  ...
  if (_hrLcFlushStart != null) {
    _hrLcDuration("flushPendingMs", performance.now() - _hrLcFlushStart);
    _hrLcInc("segmentsFlushed", pending.length);
  }
  if (_hrFsDrawStart != null) {
    const _hrFsDrawMs = performance.now() - _hrFsDrawStart;
    _hrFsRecordFlushPending(
      performance.now() - _hrFsFlushStart,
      _hrFsDrawMs,
      pending.length,
    );
  }
  ...
  if (
    window.HardRoundDebugCaptureLastLiveFrame &&
...
function _hardRoundSsForMode(mode) {
  const m =
    mode || (typeof _currentAAMode === "function" ? _currentAAMode() : "none");
...
function _hardRoundGetRenderer() {
  ...
  if (readyIndex >= 0) {
    if (
      ...
      !primary._active &&
      !primary._hardRoundFinishingOwner &&
      !_hardRoundRendererPool.includes(primary)
    )
      _hardRoundRendererPool.push(primary);
    _hardRoundRenderer = _hardRoundRendererPool.splice(readyIndex, 1)[0];
    _hardRoundRendererCreatedPending = false;
    _hardRoundRendererResizePending =
      !!primary &&
      primary !== _hardRoundRenderer &&
      _hardRoundPointerdownPreparing;
  ...
  if (primary && !primary._active && !primary._hardRoundFinishingOwner) {
    ...
    if (primary.width === w && primary.height === h && primary.ss === targetSS)
      return primary;
  }
  const resized =
    !!primary &&
    (primary.width !== w || primary.height !== h || primary.ss !== targetSS);
  const pooledIndex = _hardRoundRendererPool.findIndex(
    (renderer) => !renderer._hardRoundFinishingOwner,
  );
  if (pooledIndex >= 0) {
    ...
    _hardRoundRenderer = new window.PrototypeRenderer({
      width: w,
      height: h,
      ss: targetSS,
      preferGpu: true,
  ...
  _hardRoundRendererCreatedPending =
    pooledIndex < 0 && _hardRoundPointerdownPreparing;
...
function _hardRoundHasVisibleLayerAbove(layerIndex, frameIndex) {
  for (let index = layerIndex + 1; index < layers.length; index++) {
    ...
    if (
      typeof _layerGroupChainVisible === "function" &&
      !_layerGroupChainVisible(layer)
    )
      continue;
    ...
    if (typeof getHeldKey !== "function" || getHeldKey(index, frameIndex))
      return true;
  }
  return false;
}
...
if (typeof window.HardRoundDebugSmartPointerupTiming === "undefined")
  window.HardRoundDebugSmartPointerupTiming = false;
...
function _hrSmartPointerupBegin(strokeId, smartRaster, eventTimestamp) {
  ...
  const now = performance.now(),
    record = {
      strokeId,
      smartRaster: !!smartRaster,
      pointerupEventTimestamp: eventTimestamp,
      pointerupStart: now,
      pointerupSyncEnd: null,
      pointerupSyncDuration: 0,
      ownershipBeforeMs: 0,
      endStrokeSyncSetupMs: 0,
      maskCopyMs: 0,
      smartCommitMs: 0,
      saveMs: 0,
      recompositeMs: 0,
      firstPostUpPenEvent: null,
      firstCursorApply: null,
      longestSyncOperation: null,
      totalMainThreadSyncMs: 0,
    };
  _hardRoundSmartPointerupTimingRecords.push(record);
  if (_hardRoundSmartPointerupTimingRecords.length > 20)
    _hardRoundSmartPointerupTimingRecords.shift();
  return record;
}
function _hrSmartPointerupStage(record, name, started) {
  if (record)
    record[name] = (record[name] || 0) + (performance.now() - started);
}
function _hrSmartPointerupFinish(record) {
  if (!record) return;
  record.pointerupSyncEnd = performance.now();
  record.pointerupSyncDuration =
    record.pointerupSyncEnd - record.pointerupStart;
  _hrSmartPointerupSummarize(record);
}
function _hrSmartPointerupSummarize(record) {
  if (!record) return record;
  const stages = [
    "ownershipBeforeMs",
    "endStrokeSyncSetupMs",
    "maskCopyMs",
    "smartCommitMs",
    "saveMs",
    "recompositeMs",
  ...
  const slowest = stages.reduce(
    (best, name) =>
      record[name] > (best.ms || -1) ? { name, ms: record[name] } : best,
    {},
  );
  record.longestSyncOperation = slowest.name ? slowest : null;
  record.totalMainThreadSyncMs =
    record.pointerupSyncDuration +
    record.maskCopyMs +
    record.smartCommitMs +
    record.saveMs +
    record.recompositeMs;
  return record;
}
window.HardRoundSmartPointerupTimingNote = function (type, event) {
  if (!window.HardRoundDebugSmartPointerupTiming) return;
  const record = [..._hardRoundSmartPointerupTimingRecords]
    .reverse()
    .find((item) => item.pointerupStart != null);
  if (!record) return;
  const now = performance.now();
  if (
    type === "pen-event" &&
    !record.firstPostUpPenEvent &&
    event &&
    ...
    record.firstPostUpPenEvent = {
      ...
      afterPointerupStartMs: now - record.pointerupStart,
      afterPointerupSyncEndMs:
        record.pointerupSyncEnd == null ? null : now - record.pointerupSyncEnd,
    };
  if (
    type === "cursor-apply" &&
    record.firstPostUpPenEvent &&
    !record.firstCursorApply
  )
    record.firstCursorApply = {
      performanceTimestamp: now,
      x: event && event.x,
      y: event && event.y,
      sourceEventType: event && event.sourceEventType,
      sourceEventTimestamp: event && event.sourceEventTimestamp,
      afterPointerupStartMs: now - record.pointerupStart,
      afterPointerupSyncEndMs:
        record.pointerupSyncEnd == null ? null : now - record.pointerupSyncEnd,
      afterFirstPostUpPenEventMs:
        now - record.firstPostUpPenEvent.performanceTimestamp,
    };
};
window.HardRoundAnalyzeSmartPointerupTiming = function () {
  const smart =
    [..._hardRoundSmartPointerupTimingRecords]
      .reverse()
      .find((record) => record.smartRaster) || null;
  const normal =
    [..._hardRoundSmartPointerupTimingRecords]
      .reverse()
      .find((record) => !record.smartRaster) || null;
  if (smart) _hrSmartPointerupSummarize(smart);
  if (normal) _hrSmartPointerupSummarize(normal);
  return {
    smartRaster: smart,
    normalRaster: normal,
    smartVsNormalDifference:
      smart && normal
        ? {
            pointerupSyncDuration:
              smart.pointerupSyncDuration - normal.pointerupSyncDuration,
            totalMainThreadSyncMs:
              smart.totalMainThreadSyncMs - normal.totalMainThreadSyncMs,
            firstHoverDelayMs:
              smart.firstPostUpPenEvent && normal.firstPostUpPenEvent
                ? smart.firstPostUpPenEvent.afterPointerupStartMs -
                  normal.firstPostUpPenEvent.afterPointerupStartMs
                : null,
          }
        : null,
    slowestSmartOperation: (smart && smart.longestSyncOperation) || null,
  };
};
function _commitFinishedHardRoundStroke(context) {
  const src = context.resolvedCanvas;
  if (!src) return;
  if (typeof window.pushUndoAt === "function")
    window.pushUndoAt(context.layerIndex, context.frameIndex);
  const currentDestination =
    curLayer === context.layerIndex && curFrame === context.frameIndex;
  const target = currentDestination
    ? ctx
    : context.destinationCanvas && context.destinationCanvas.getContext("2d");
  if (!target) return;
  target.save();
  target.globalAlpha = context.opacity;
  target.globalCompositeOperation = context.compositeOperation;
  target.drawImage(src, 0, 0);
  if (context.blendMode === "add-glow") {
    target.globalAlpha = context.opacity * 0.65;
    target.drawImage(src, 0, 0);
  }
  target.restore();
  if (currentDestination) {
    const key =
      layers[context.layerIndex] &&
      layers[context.layerIndex].frames[context.frameIndex];
    if (key) {
      const keyCtx = key.getContext("2d");
      keyCtx.clearRect(0, 0, key.width, key.height);
      keyCtx.drawImage(activeC, 0, 0);
    }
    recomposite(context.layerIndex, context.frameIndex);
    _hrRapidPresentation("recompositeCompleted", context.strokeId, {
      kind: "normal-raster",
    });
  }
...
function _commitFinishedSmartRasterStroke(context) {
  const mask = context.resolvedMaskCanvas;
  if (!mask) return;
  if (typeof window.pushUndoAt === "function")
    window.pushUndoAt(context.layerIndex, context.frameIndex);
  const currentDestination =
  ...
  const target = targetCanvas && targetCanvas.getContext("2d");
  if (!target) return;
  // Capture ownership at this stroke's ordered commit boundary, after all
  // older strokes have committed and before this mask changes the target.
  // Pointerup-time capture can be stale when an older resolve is pending.
  if (!context.ownershipBefore) {
    const rect = context.dirtyRect;
    const ownershipStarted = context.pointerupTiming ? performance.now() : 0;
    context.ownershipBefore =
      rect && rect.w > 0 && rect.h > 0
        ? target.getImageData(rect.x, rect.y, rect.w, rect.h)
        : target.getImageData(0, 0, targetCanvas.width, targetCanvas.height);
    _hrSmartPointerupStage(
      context.pointerupTiming,
      "ownershipBeforeMs",
      ownershipStarted,
    );
  }
  ...
  else target.drawImage(mask, 0, 0);
  target.restore();
  if (typeof window.commitSmartRasterBrushAt === "function") {
    ...
    window.commitSmartRasterBrushAt(
      context.layerIndex,
      context.frameIndex,
      mask,
      context.styleId,
    ...
    if (context.pointerupTiming)
      context.pointerupTiming.smartCommitBreakdown =
        window.HardRoundSmartCommitTimingLast || null;
  }
  if (currentDestination) {
    const saveStarted = context.pointerupTiming ? performance.now() : 0;
    const key = context.destinationCanvas;
    if (key) {
      const keyCtx = key.getContext("2d");
      keyCtx.clearRect(0, 0, key.width, key.height);
      keyCtx.drawImage(activeC, 0, 0);
...
function _hardRoundFinalizeOwnedContext(context, e) {
  _hrRapidPresentation("commitStarted", context.strokeId, {
    kind: context.smartRaster ? "smart-raster" : "normal-raster",
  });
  context.state = "finishing";
  context.renderer._hardRoundFinishingOwner = context.strokeId;
  ...
  let resolveOwnedResult, rejectOwnedResult;
  const resolution = new Promise((resolve, reject) => {
    resolveOwnedResult = resolve;
    rejectOwnedResult = reject;
  });
  const presentation = context.presentFinishedFrame
    ? _hardRoundPresentFinishedFrame(context.renderer, context.strokeId)
    : Promise.resolve();
  Promise.resolve(presentation)
    .then(() => {
      const endStrokeStarted = context.pointerupTiming ? performance.now() : 0;
      const cpuTimingCount =
        context.pointerupTiming &&
        Array.isArray(window.HardRoundCpuResolveTimingLog)
          ? window.HardRoundCpuResolveTimingLog.length
          : 0;
      // The renderer was detached at pointerup and is owned by this context.
      // This mandatory resolve is not a shared-state mutation and must never
      // be skipped merely because a newer live session has started.
      const result = context.renderer.endStroke({
        readback: context.gpuCommit,
        includeCpuMaskData: context.smartRaster,
        dirtyRect: context.dirtyRect,
      });
      _hrSmartPointerupStage(
        context.pointerupTiming,
        "endStrokeSyncSetupMs",
      ...
      if (
        context.pointerupTiming &&
        Array.isArray(window.HardRoundCpuResolveTimingLog) &&
        window.HardRoundCpuResolveTimingLog.length > cpuTimingCount
      )
        context.pointerupTiming.cpuResolveBreakdown =
          window.HardRoundCpuResolveTimingLog[
            window.HardRoundCpuResolveTimingLog.length - 1
          ] || null;
      return result;
    })
    .then(
      (result) => {
        ...
        if (context.smartRaster) {
          context.resolvedMaskCanvas = stable;
          context.resolvedMaskData = (result && result.maskData) || null;
        } else context.resolvedCanvas = stable;
  ...
  const commit = _hardRoundCommitTail
    ...
    .then((ready) => {
      ...
      _hrRapidPresentation("commitCompleted", ready.strokeId, {
        kind: ready.smartRaster ? "smart-raster" : "normal-raster",
      });
      ...
      if (!ready.gpuCommit) {
        if (_inStroke && ready.strokeId === _activeStrokeSession) {
          _inStroke = false;
          if (_strokeCtx && _strokeCanvas)
            _strokeCtx.clearRect(
              0,
              0,
              _strokeCanvas.width,
              _strokeCanvas.height,
            );
        }
      }
      ...
      // helper's existing unconditional-attempt pattern.
      {
        ...
        const isNewerOwner =
          (overlayOwner != null && overlayOwner !== ready.strokeId) ||
          (visibilityOwner != null && visibilityOwner !== ready.strokeId);
        if (isNewerOwner) {
          _hrRapidPresentation("overlayHideSuppressed", ready.strokeId, {
            reason: "older-finalizer-newer-owner",
            finalizingStrokeId: ready.strokeId,
            overlayOwner,
            overlayVisibilityOwnerStrokeId: visibilityOwner,
          ...
          const retireOverlay = () => {
            ...
            const visibilityOwnerNow =
              window.HardRoundOverlayVisibilityOwnerStrokeId;
            const activeNow = _activeStrokeSession;
            const isNewerNow =
              (overlayOwnerNow != null && overlayOwnerNow !== ready.strokeId) ||
              (visibilityOwnerNow != null &&
                visibilityOwnerNow !== ready.strokeId) ||
              (activeNow != null && activeNow !== ready.strokeId);
            if (isNewerNow) {
              _hrRapidPresentation("overlayHideSuppressed", ready.strokeId, {
                reason: "newer-stroke-started-before-paint",
                finalizingStrokeId: ready.strokeId,
                overlayOwner: overlayOwnerNow,
                overlayVisibilityOwnerStrokeId: visibilityOwnerNow,
              ...
              _hardRoundSetGpuOverlayVisible(
                false,
                "owned-finalization-retire-committed-overlay",
              ); // owned-finalization-post-paint-retire
            }
          };
          if (
            committedRevision != null &&
            ...
            window.DisplayBackend.whenRevisionPresented(
              committedRevision,
              retireOverlay,
            );
          } else {
  ...
  const settled = commit.finally(() => {
    ...
    _hardRoundPendingCommitCount = Math.max(
      0,
      _hardRoundPendingCommitCount - 1,
    );
    _hrRapidPresentation("finalizerCompleted", context.strokeId);
  });
  _hardRoundCommitTail = settled.catch((err) => {
    _lastArtworkCommitError = err;
    console.error("[Hard Round finalization]", err);
  });
  return settled;
}
...
function _hrDebugBadge(info) {
  ...
  if (!el) {
    ...
    el.style.cssText =
      "position:fixed;bottom:8px;left:8px;z-index:99999;font:11px monospace;" +
...
function _hardRoundGpuOverlay() {
  return document.getElementById("hard-round-gpu-overlay");
}
...
function _hrPresentBoundarySnapshot(reason) {
  const overlay = _hardRoundGpuOverlay();
  const overlayCs =
    overlay && typeof getComputedStyle === "function"
      ? getComputedStyle(overlay)
      : null;
  const activeCs =
    typeof activeC !== "undefined" &&
    activeC &&
    typeof getComputedStyle === "function"
      ? getComputedStyle(activeC)
      : null;
  const renderer =
    typeof _hardRoundRenderer !== "undefined" ? _hardRoundRenderer : null;
  return {
    tag: "[11B.9 PRESENT-BOUNDARY]",
    timestamp: performance.now(),
    strokeId:
      typeof _activeStrokeSession !== "undefined" ? _activeStrokeSession : null,
    reason,
...
function _hrPresentBoundaryLog(reason) {
  ...
  if (window.HardRoundPresentationBoundaryLog.length > 200) {
    window.HardRoundPresentationBoundaryLog.splice(
      0,
      window.HardRoundPresentationBoundaryLog.length - 200,
    );
  }
...
window.HardRoundLivePreviewSubmitInfo =
  window.HardRoundLivePreviewSubmitInfo || {};
window.HardRoundGetPreviewGeneration = function () {
  return _hardRoundPreviewGeneration;
};
...
function _hr1110PushRequest(entry) {
  ...
  if (window.HardRoundLivePreviewCrossoverLog.length > 300) {
    window.HardRoundLivePreviewCrossoverLog.splice(
      0,
      window.HardRoundLivePreviewCrossoverLog.length - 300,
    );
  }
  return entry;
}
...
window.HardRoundLivePreviewCrossoverSummary = function () {
  ...
  const submittedAfterPointerup = log.filter(
    (e) =>
      e.presentationSubmitted &&
      e.presentSubmitTime != null &&
      e.pointerupStartedAt != null &&
      e.presentSubmitTime > e.pointerupStartedAt,
  );
  const resolvedAfterFinishedFrameStarted = log.filter(
    (e) =>
      e.resolveTime != null &&
      e.finishedFrameStartedAt != null &&
      e.resolveTime > e.finishedFrameStartedAt,
  );
  const byStroke = {};
  log.forEach((e) => {
    byStroke[e.strokeId] = byStroke[e.strokeId] || {
      strokeId: e.strokeId,
      total: 0,
      accepted: 0,
    };
  ...
  return {
    ...
    resolvedAfterFinishedFrameStartedCount:
      resolvedAfterFinishedFrameStarted.length,
    ...
    earlyRevealJoinFixNote:
      "Phase 11B.11: this field now reads _hr1110EarlyRevealBySession, not the 11B.9 boundary log -- see comment above. If you previously saw [] here, that was the join bug, not evidence early reveal never happened.",
...
window.HardRoundAnalyzeZeroAcceptedPreviews = function () {
  ...
  const zeroAcceptedStrokes = Object.values(byStroke).filter(
    (s) => s.total > 0 && s.accepted === 0,
  );
  const results = zeroAcceptedStrokes.map((s) => {
    ...
    return {
      ...
      accepted: s.accepted,
      overlayRevealedAtBeginStroke: earlyReveal
        ? earlyReveal.requestedVisible
        : null,
...
function _hardRoundSetGpuOverlayVisible(visible, reason) {
  ...
  if (window.HardRoundDebugPresentationBoundary) {
    const before = overlay
      ? { hidden: overlay.hidden, display: overlay.style.display }
      : null;
    const willChange = overlay
  ...
  if (visible) {
    _hrFirstPresentNote(_activeStrokeSession, "firstOverlayVisibleTime");
    _hrFirstPresentCount(_activeStrokeSession, "overlayShownCount");
  }
  _hrRapidPresentation(
    visible ? "overlayShown" : "overlayHidden",
    _activeStrokeSession,
    { reason: reason || null },
  );
  _hrPresentBoundaryLog(
    "setGpuOverlayVisible(" + visible + ")" + (reason ? " via " + reason : ""),
  );
}
function _hardRoundConcealMountedOverlay(reason) {
  ...
  overlay.style.opacity = "0";
  window.HardRoundOverlayPresentedStrokeId = null;
  _hrRapidPresentation("overlayHidden", _activeStrokeSession, {
    reason: reason || "mounted-transparent",
  });
}
...
function _hr1135Snapshot(event) {
  ...
  return {
    ...
    displayBackend:
      (window.DisplayBackend && window.DisplayBackend.mode) || "canvas2d",
    activeCanvasOpacity:
      typeof activeC !== "undefined" && activeC ? activeC.style.opacity : null,
    strokeId: _activeStrokeSession,
  };
}
...
function _hr1135Store(snap) {
  ...
  if (window.HardRound1135Log.length > 30)
    window.HardRound1135Log.splice(0, window.HardRound1135Log.length - 30);
}
...
function _hardRoundReplenishWarmReserve() {
  if (
    _hardRoundReserveFillPromise ||
    typeof window === "undefined" ||
    !window.PrototypeRenderer ||
    !activeC
  )
    return _hardRoundReserveFillPromise || Promise.resolve(false);
  const w = activeC.width,
    h = activeC.height;
  const targetSS = _hardRoundSsForMode();
  _hardRoundReserveFillPromise = (async () => {
    ...
    for (const renderer of _hardRoundRendererPool) {
      ...
      if (
        ...
        continue;
      if (typeof renderer.prepareGpuPresentation === "function")
        await renderer.prepareGpuPresentation();
    }
    let readyCount = _hardRoundRendererPool.filter(
      (renderer) =>
        renderer.width === w &&
        renderer.height === h &&
        renderer.ss === targetSS &&
    ...
    while (readyCount < _HARD_ROUND_WARM_RESERVE) {
      const renderer = new window.PrototypeRenderer({
        width: w,
        height: h,
        ss: targetSS,
        preferGpu: true,
      ...
      const warmed =
        typeof renderer.prepareGpuPresentation === "function" &&
        (await renderer.prepareGpuPresentation());
      if (!warmed) break;
      _hardRoundRendererPool.push(renderer);
      readyCount++;
    }
    return readyCount >= _HARD_ROUND_WARM_RESERVE;
  })().finally(() => {
    _hardRoundReserveFillPromise = null;
  });
  return _hardRoundReserveFillPromise;
}
function _hardRoundPrewarmPresentation() {
  if (_hardRoundPresentationPrewarmPromise)
    return _hardRoundPresentationPrewarmPromise;
  const renderer = _hardRoundGetRenderer();
  if (!renderer || typeof renderer.prepareGpuPresentation !== "function")
  ...
  _hardRoundPresentationPrewarmPromise = renderer
    .prepareGpuPresentation()
    .then((warmed) => {
    ...
    .finally(() => {
      _hardRoundPresentationPrewarmPromise = null;
    });
  return _hardRoundPresentationPrewarmPromise;
}
...
window.addEventListener("brush-runtime-ready", (event) => {
  if (
    !event.detail ||
    event.detail.presetId !== "hard-round" ||
    window.brushTipCanvas ||
    window.brushTextureEnabled
  )
    return;
...
function _hardRoundEligibleNow() {
  const fn =
    typeof window !== "undefined" &&
    window.HardRoundAdapter &&
    window.HardRoundAdapter.isHardRoundEligible;
  if (!fn) return false;
  return fn({
    tool,
    isPen: _isDrawingWithPen,
...
window.HardRoundReset11B6Log = function () {
  window.HardRoundPhase11B6Log = [];
};
...
function _hr11b6CursorSnapshot(e) {
  ...
  try {
    hasCapture =
      typeof e.pointerId === "number" && activeC.hasPointerCapture
        ? activeC.hasPointerCapture(e.pointerId)
        : null;
  } catch (_) {}
  return {
...
function _hr11b6Log(stage, e, extra) {
  ...
  if (entry.pendingQueueLength > _hr11b6MaxPending)
    _hr11b6MaxPending = entry.pendingQueueLength;
  else if (
    entry.pendingQueueLength < _hr11b6MaxPending &&
    !window.HardRoundFirst11B6Regression
  ) {
    window.HardRoundFirst11B6Regression = Object.assign(
      { kind: "pendingQueueLength", maxSoFar: _hr11b6MaxPending },
      entry,
    );
  }
  if (entry.rendererSegmentCount != null) {
    if (entry.rendererSegmentCount > _hr11b6MaxRendererSegments)
      _hr11b6MaxRendererSegments = entry.rendererSegmentCount;
    else if (
      entry.rendererSegmentCount < _hr11b6MaxRendererSegments &&
      !window.HardRoundFirst11B6Regression
    ) {
      window.HardRoundFirst11B6Regression = Object.assign(
        { kind: "rendererSegmentCount", maxSoFar: _hr11b6MaxRendererSegments },
        entry,
      );
    }
  }
...
window.HardRoundAnalyze11B6 = function () {
  ...
  const cursorAnomalies = log.filter(
    (e) =>
      e.activeCHasCapture === true &&
      e.elementAtPoint &&
      e.elementAtPoint !== "active-canvas",
  );
  return {
    ...
    cursorNearFirstRegression:
      window.HardRoundFirst11B6Regression && cursorAnomalies.length
        ? cursorAnomalies.some(
            (e) =>
              Math.abs(
                e.timestamp - window.HardRoundFirst11B6Regression.timestamp,
              ) < 16,
          )
        : null,
    log,
...
function _hardRoundStampSegments(segments, e) {
  ...
  if (_hardRoundStrokeActive)
    _hrFirstPresentNote(_activeStrokeSession, "firstSegmentGeneratedTime");
  if (_hrTlActive() && segments.length) {
    const lastCore = segments[segments.length - 1];
    if (lastCore && Number.isFinite(lastCore.x1) && Number.isFinite(lastCore.y1))
      _hrTlRecordStage("coreOutput", lastCore.x1, lastCore.y1, performance.now());
  }
  const brushDiagStart = window.BrushDebugPerf ? performance.now() : 0;
  ...
  if (_hardRoundNextStampIsFirst) {
    segments[0].isStrokeStart = true;
    _hardRoundNextStampIsFirst = false;
  }
  if (_hardRoundNextStampIsLast) {
    segments[segments.length - 1].isStrokeEnd = true;
  ...
  for (const seg of segments) {
    ...
    _brushDiagPerfNote("resolved", {
      count: 1,
      radius: (resolved.r0 + resolved.r1) / 2,
      minRadius: Math.min(resolved.r0, resolved.r1),
      maxRadius: Math.max(resolved.r0, resolved.r1),
    });
    const resolvedLength = Math.hypot(
      resolved.x1 - resolved.x0,
      resolved.y1 - resolved.y0,
    );
    if (resolvedLength > 0)
  ...
  if (_hrMtAdapterStart != null)
    _hrMtRecord(
      "HardRoundAdapter.resolveSegmentRenderParams-loop",
      _hrMtAdapterStart,
      performance.now(),
    );
  ...
  if (_hrLcActive()) {
    ...
    for (let i = 0; i < renderSegs.length; i++)
      _hardRoundPendingRenderSegmentTimes.push(now);
    _hrLcInc("segmentsGenerated", renderSegs.length);
    _hrLcPending(
      _hardRoundPendingRenderSegments.length,
      _hardRoundPendingRenderSegmentTimes.length
        ? now - _hardRoundPendingRenderSegmentTimes[0]
        : 0,
    );
  }
  ...
  if (_hrMtReqPreviewStart != null)
    _hrMtRecord(
      "_hardRoundRequestLivePreview",
      _hrMtReqPreviewStart,
      performance.now(),
    );
}
...
function _hardRoundPushLiveCaptureLogEntry(entry) {
  const log =
    window.HardRoundLiveCaptureLog || (window.HardRoundLiveCaptureLog = []);
  log.push(entry);
...
function _hardRoundPreviewOrderPush(entry) {
  const log =
    window.HardRoundPreviewOrderLog || (window.HardRoundPreviewOrderLog = []);
  log.push(entry);
...
if (typeof window !== "undefined") {
  ...
  window.HardRoundAnalyzePreviewOrder = function () {
    ...
    const accepted = log
      .filter((e) => e.overlayShown)
      .sort((a, b) => a.resolveTime - b.resolveTime);
    let outOfOrder = [];
...
async function _hr11b7CompareAndRecord(
  renderer,
  strokeId,
  rendererSegmentCount,
) {
  ...
  let buf = null;
  try {
    buf = await renderer.gpu.diagResolveIntoBuffer(
      renderer._rgb,
      renderer._composite,
    );
  } catch (err) {
  ...
  if (prev && prev.strokeId === strokeId) {
    ...
    if (
      segmentsMonotonic &&
      entry.nonTransparentDelta != null &&
      entry.nonTransparentDelta < 0 &&
      !window.HardRoundFirstLiveFrameRegression
    ) {
...
window.HardRoundAnalyzeLiveFrameRegression = function () {
  ...
  return {
    ...
    suspiciousNoChangeWhileGrowing: log.filter(
      (e) =>
        e.segmentsMonotonic &&
        e.newRendererSegmentCount > e.previousRendererSegmentCount &&
        e.imageChanged === false,
    ).length,
    regressionsFound: log.filter(
      (e) =>
        e.segmentsMonotonic &&
        e.nonTransparentDelta != null &&
        e.nonTransparentDelta < 0,
    ).length,
...
function _hr1112PushLog(entry) {
  ...
  if (window.HardRoundPaintedOverlayCompareLog.length > 50) {
    window.HardRoundPaintedOverlayCompareLog.splice(
      0,
      window.HardRoundPaintedOverlayCompareLog.length - 50,
    );
  }
}
...
function _hr1112ComputeLostCoverage(expectedBuf, paintedData, w, h) {
  ...
  return {
    lostCoveragePixelCount: lostCount,
    lostCoverageBounds: maxX >= minX ? { minX, minY, maxX, maxY } : null,
  };
}
async function _hr1112CompareAndRecord(
  renderer,
  strokeId,
  rendererSegmentCount,
  previewGenerationAtAccept,
) {
  if (!renderer || !renderer.gpu || !renderer.gpu.ready) return;
  ...
  try {
    expectedBuf = await renderer.gpu.diagResolveIntoBuffer(
      renderer._rgb,
      renderer._composite,
    );
  } catch (err) {
  ...
  await new Promise((resolve) =>
    requestAnimationFrame(() => {
      rafBoundariesWaited = 1;
      resolve();
    }),
  );
  await new Promise((resolve) =>
    requestAnimationFrame(() => {
      rafBoundariesWaited = 2;
      resolve();
    }),
  );
  ...
  const supersededByGenerationChange =
    previewGenerationAtAccept !== _hardRoundPreviewGeneration;
  ...
  if (overlay) {
    try {
      if (!_hr1112PaintedCompareCanvas)
        _hr1112PaintedCompareCanvas = document.createElement("canvas");
      const cap = _hr1112PaintedCompareCanvas;
      if (cap.width !== expectedBuf.w || cap.height !== expectedBuf.h) {
        cap.width = expectedBuf.w;
        cap.height = expectedBuf.h;
      }
      ...
      paintedHashed = _hrHashBuffer(
        { data: paintedData, w: cap.width, h: cap.height },
        "11B12-painted",
      );
      if (!superseded) {
        lostCoverage = _hr1112ComputeLostCoverage(
          expectedBuf.data,
          paintedData,
          expectedBuf.w,
          expectedBuf.h,
  ...
  _hr1112PushLog({
    ...
    paintedCaptureTime,
    rafBoundariesWaited,
    expectedNonTransparent:
      expectedHashed.nonTransparent != null
        ? expectedHashed.nonTransparent
        : null,
    paintedNonTransparent:
      paintedHashed.nonTransparent != null
        ? paintedHashed.nonTransparent
        : null,
    ...
    supersededReason: superseded
      ? supersededByNewerPreview
        ? "superseded-by-newer-accepted-preview"
...
window.HardRoundAnalyzePaintedOverlayCompare = function () {
  ...
  const usable = log.filter(
    (e) => !e.superseded && e.lostCoveragePixelCount != null,
  );
  const withLostCoverage = usable.filter((e) => e.lostCoveragePixelCount > 0);
  let maxLost = 0;
  withLostCoverage.forEach((e) => {
    if (e.lostCoveragePixelCount > maxLost) maxLost = e.lostCoveragePixelCount;
  });
  return {
    totalComparisons: log.length,
...
if (typeof window.HardRoundDebugActiveCProbe === "undefined")
  window.HardRoundDebugActiveCProbe = false;
if (typeof window.HardRoundDebugInputLatency === "undefined")
  window.HardRoundDebugInputLatency = false;
...
if (typeof window.HardRoundDebugPresentationLifecycle === "undefined")
  window.HardRoundDebugPresentationLifecycle = false;
if (typeof window.HardRoundDebugLiveCadence === "undefined")
...
function _hrLcData() {
  if (!window.HardRoundLiveCadenceData) {
    window.HardRoundLiveCadenceData = {
  ...
  return window.HardRoundLiveCadenceData;
}
function _hrLcActive() {
  return typeof window !== "undefined" && !!window.HardRoundDebugLiveCadence;
}
function _hrLcInc(name, n = 1) {
  if (!_hrLcActive()) return;
  const d = _hrLcData();
  d.counts[name] = (d.counts[name] || 0) + n;
}
function _hrLcPush(bucket, name, value) {
  if (!_hrLcActive() || !Number.isFinite(value)) return;
  const d = _hrLcData();
  const root = bucket === "durations" ? d.durations : d.times;
  const arr = root[name] || (root[name] = []);
  arr.push(value);
  if (arr.length > _hrLiveCadenceLimit) arr.shift();
}
function _hrLcTime(name, t = performance.now()) {
  _hrLcPush("times", name, t);
}
function _hrLcDuration(name, ms) {
  _hrLcPush("durations", name, ms);
}
function _hrLcValue(name, value) {
  _hrLcPush("values", name, value);
}
function _hrLcPending(count, ageMs) {
  if (!_hrLcActive()) return;
  const d = _hrLcData();
  d.maxPendingSegments = Math.max(d.maxPendingSegments || 0, count || 0);
  if (Number.isFinite(ageMs))
    d.maxPendingAgeMs = Math.max(d.maxPendingAgeMs || 0, ageMs);
}
function _hrLcStats(values) {
  const arr = (values || []).filter(Number.isFinite).sort((a, b) => a - b);
  if (!arr.length) return { count: 0, avg: null, p50: null, p95: null };
...
function _hrLcIntervalStats(times) {
  const src = (times || []).filter(Number.isFinite);
  const intervals = [];
  for (let i = 1; i < src.length; i++) intervals.push(src[i] - src[i - 1]);
  const stats = _hrLcStats(intervals);
  return Object.assign(stats, {
    rateHz:
      stats.avg && stats.avg > 0 ? 1000 / stats.avg : src.length > 1 ? 0 : null,
  });
}
window.HardRoundLiveCadenceNote = function (type, detail = {}) {
  if (!_hrLcActive()) return;
  const now = performance.now();
  const d = _hrLcData();
  if (type === "gpu-resolve-into") {
    _hrLcDuration("gpuResolveIntoMs", detail.totalMs);
    _hrLcDuration("mapAsyncMs", detail.mapAsyncMs);
    _hrLcDuration("cpuPixelConversionMs", detail.pixelConversionMs);
    _hrLcDuration("putImageDataMs", detail.putImageDataMs);
    _hrLcDuration("cpuPixelConversionPutImageDataMs", detail.cpuPixelConversionPutImageDataMs);
    const fullPixels = detail.fullFramePixels;
    const dirtyPixels = detail.dirtyPixels;
    if (Number.isFinite(fullPixels) && fullPixels > 0 && Number.isFinite(dirtyPixels)) {
      d.fullFramePixels = fullPixels;
      _hrLcValue("dirtyPixels", dirtyPixels);
      _hrLcValue("dirtyToFullRatio", dirtyPixels / fullPixels);
      if (detail.partial) _hrLcInc("partialReadback");
      else _hrLcInc("fullFallback");
    }
    return;
  }
...
window.HardRoundAnalyzeLiveCadence = function () {
  const d = window.HardRoundLiveCadenceData || _hrLcData();
  const durationMs = Math.max(0, performance.now() - d.startedAt);
  const durationSeconds = durationMs / 1000;
  const count = (name) => d.counts[name] || 0;
  const rate = (name) =>
    durationSeconds > 0 ? count(name) / durationSeconds : null;
  return {
    enabled: !!window.HardRoundDebugLiveCadence,
    durationMs,
    counts: Object.assign({}, d.counts),
    ratesHz: {
      pointerrawupdate: rate("pointerrawupdate"),
      pointermove: rate("pointermove"),
    ...
    intervalsMs: {
      pointerrawupdate: _hrLcIntervalStats(d.times.pointerrawupdate),
    ...
    durationsMs: {
      previewFlight: _hrLcStats(d.durations.previewFlightMs),
      flushPending: _hrLcStats(d.durations.flushPendingMs),
      peekStroke: _hrLcStats(d.durations.peekStrokeMs),
      gpuResolveInto: _hrLcStats(d.durations.gpuResolveIntoMs),
      ...
      strokeCanvasDraw: _hrLcStats(d.durations.strokeCanvasDrawMs),
    ...
    dirtyReadback: {
      fullFramePixels: d.fullFramePixels || null,
      partialReadbackCount: count("partialReadback"),
      fullFallbackCount: count("fullFallback"),
      dirtyPixelsAvg: _hrLcStats(d.values.dirtyPixels).avg,
...
const _hrTipLagLimit = 1000;
function _hrTlActive() {
  return typeof window !== "undefined" && !!window.HardRoundDebugTipLag;
}
function _hrTlData() {
  if (!window.HardRoundTipLagData) {
    window.HardRoundTipLagData = {
      ...
      speedBins: {
        slow: { count: 0, sum: 0, max: 0 },
        medium: { count: 0, sum: 0, max: 0 },
        fast: { count: 0, sum: 0, max: 0 },
      },
      lastRaw: null,
      stabilization: null,
    };
  }
  return window.HardRoundTipLagData;
}
function _hrTlPush(root, name, value) {
  if (!_hrTlActive() || !Number.isFinite(value)) return;
  const d = _hrTlData();
...
function _hrTlPoint(x, y, time, extra) {
  return Object.assign(
    {
      x,
      y,
      time: Number.isFinite(time) ? time : performance.now(),
      wallTime: performance.now(),
      zoom: Number.isFinite(zoom) ? zoom : 1,
    },
    extra || {},
...
function _hrTlRecordRaw(x, y, time, type, extra) {
  if (!_hrTlActive()) return;
  const d = _hrTlData();
  const now = performance.now();
  const z = Math.max(0.0001, Number.isFinite(zoom) ? zoom : 1);
  const p = _hrTlPoint(x, y, time, Object.assign({ type }, extra || {}));
  if (d.lastRaw) {
    const dt = Math.max(0.001, now - d.lastRaw.wallTime);
    p.speedScreenPxPerSec =
      (Math.hypot(x - d.lastRaw.x, y - d.lastRaw.y) * z * 1000) / dt;
  } else p.speedScreenPxPerSec = 0;
  d.latest.raw = p;
  d.lastRaw = p;
}
function _hrTlRecordStage(name, x, y, time, extra) {
  if (!_hrTlActive()) return;
  const d = _hrTlData();
  const p = _hrTlPoint(x, y, time, extra);
  d.latest[name] = p;
  const raw = d.latest.raw || d.latest.coalesced;
  if (!raw) return;
  const z = Math.max(0.0001, Number.isFinite(zoom) ? zoom : 1);
  const dist = Math.hypot(p.x - raw.x, p.y - raw.y) * z;
  const age = p.wallTime - raw.wallTime;
  _hrTlPush("samples", "rawTo" + name[0].toUpperCase() + name.slice(1) + "ScreenPx", dist);
  _hrTlPush("ages", "rawTo" + name[0].toUpperCase() + name.slice(1), age);
  if (name === "coreOutput" && d.latest.stabilized) {
    const s = d.latest.stabilized;
    _hrTlPush(
      "samples",
      "stabilizedToCoreOutputScreenPx",
      Math.hypot(p.x - s.x, p.y - s.y) * z,
    );
  }
  if (name === "acceptedPreview") {
    const speed = raw.speedScreenPxPerSec || 0;
    const bucket = speed < 150 ? "slow" : speed < 700 ? "medium" : "fast";
    const b = d.speedBins[bucket];
    b.count++;
    b.sum += dist;
    b.max = Math.max(b.max, dist);
  }
}
function _hrTlRecordStabilization() {
  if (!_hrTlActive()) return;
...
window.HardRoundTipLagNote = function (stage, detail = {}) {
  if (!_hrTlActive()) return;
  if (!Number.isFinite(detail.x) || !Number.isFinite(detail.y)) return;
  _hrTlRecordStage(stage, detail.x, detail.y, detail.time, detail);
};
window.HardRoundAnalyzeTipLag = function () {
  const d = window.HardRoundTipLagData || _hrTlData();
  const sample = (name) => _hrTlStats(d.samples[name]);
  const age = (name) => _hrTlStats(d.ages[name]);
  const latestRaw = d.latest && d.latest.raw;
  const z = Math.max(0.0001, Number.isFinite(zoom) ? zoom : 1);
  const rawToDocPoint = (point) =>
    latestRaw && point && Number.isFinite(point.x) && Number.isFinite(point.y)
      ? Math.hypot(point.x - latestRaw.x, point.y - latestRaw.y) * z
      : null;
  const cursor = typeof window !== "undefined" && window.BrushCursorLatestPosition;
  const latestRawToCustomCursorScreenPx =
    latestRaw &&
    cursor &&
    Number.isFinite(latestRaw.clientX) &&
    Number.isFinite(latestRaw.clientY) &&
    Number.isFinite(cursor.renderX) &&
  ...
  const speedBin = (name) => {
    ...
    return {
      count: b.count,
      meanRawToAcceptedPreviewScreenPx: b.count ? b.sum / b.count : null,
      maxRawToAcceptedPreviewScreenPx: b.count ? b.max : null,
    };
  };
  return {
    enabled: !!window.HardRoundDebugTipLag,
    durationMs: performance.now() - d.startedAt,
    latest: d.latest,
    stabilization: d.stabilization,
    rawToStabilizedScreenPx: sample("rawToStabilizedScreenPx"),
    stabilizedToCoreOutputScreenPx: sample("stabilizedToCoreOutputScreenPx"),
    ...
    agesMs: {
      ...
      rawToAcceptedPreview: age("rawToAcceptedPreview"),
      acceptedPreviewToRecomposite: age("acceptedPreviewToRecomposite"),
      rawToRecomposite: age("rawToRecomposite"),
    },
    speedVsLag: {
      slow: speedBin("slow"),
...
function _hrNewestSampleTipRect(from, to, radius) {
  if (!from || !to) return null;
  const pad = Math.max(2, Math.ceil((Number(radius) || getBrushSize() / 2) + 3));
  ...
  const w = _strokeCanvas ? _strokeCanvas.width : activeC.width;
  const h = _strokeCanvas ? _strokeCanvas.height : activeC.height;
    ...
    rec = Object.assign(
      {
        ...
        firstRawEventType: null,
        firstRawEventTime: null,
        firstPressureAcceptedTime: null,
        firstStabilizerInputTime: null,
        firstStabilizerOutputTime: null,
        firstCoreInputTime: null,
        firstCoreGeometryTime: null,
        firstRendererGeometryTime: null,
        firstGpuSubmitTime: null,
        firstPresentCallTime: null,
        firstOverlayVisibleTime: null,
        firstGeometrySourceEvent: null,
        presentationBarrierStartTime: null,
        presentationBarrierEndTime: null,
        backend: null,
        stabilizationAmount: null,
      },
      initial,
    );
...
window.HardRoundInputLatencyNote = function (
  strokeId,
  field,
  time = performance.now(),
  extra = null,
...
window.HardRoundAnalyzeInputLatency = function () {
  return _hardRoundInputLatencyRecords.map((r) => {
    return Object.assign({}, r, {
      presentationBarrierWaitMs:
        r.presentationBarrierStartTime != null &&
        r.presentationBarrierEndTime != null
          ? r.presentationBarrierEndTime - r.presentationBarrierStartTime
          : null,
      gpuSubmitToWorkDoneMs:
        r.firstGpuSubmitTime != null && r.firstGpuWorkDoneTime != null
          ? r.firstGpuWorkDoneTime - r.firstGpuSubmitTime
          : null,
    });
  });
};

window.HardRoundIsLivePreviewCurrent = function (
  session,
  previewGeneration,
  renderer,
  strokeId,
  ...
  return (
    ...
    _hardRoundActiveContext.renderer === renderer &&
    window.HardRoundOverlayOwnerStrokeId === strokeId
  );
};
...
function _hardRoundPresentLivePreview(renderer) {
  if (!renderer || !_inStroke || !_strokeCtx || !_strokeCanvas) {
    _hrFirstPresentCount(
      ...
      !renderer ? "no-renderer" : "pending-preview-cancelled",
    );
    return;
  }
  ...
  const livePreviewId = window.HardRoundDebugLivePreviewCrossover
    ? ++_hr1110LivePreviewIdCounter
    : null;
  const _hr1110Entry =
    livePreviewId != null
      ? _hr1110PushRequest({
          ...
          resolveTime: null,
          pointerupStartedAt:
            _hr1110PointerupStartedAtBySession[session] != null
              ? _hr1110PointerupStartedAtBySession[session]
              : null,
          finishedFrameStartedAt:
            _hr1110FinishedFrameStartedAtBySession[session] != null
              ? _hr1110FinishedFrameStartedAtBySession[session]
              : null,
  ...
  if (_hrMtSetupStart != null)
    _hrMtRecord(
      "_hardRoundPresentLivePreview-sync-setup",
      _hrMtSetupStart,
      performance.now(),
    );
      ...
      const peekResult = renderer.peekStroke({
        ...
        presentationBarrierDependency: gpuLivePreview
          ? staleBaseRevision != null
            ...
            : "none-private-gpu-renderer"
          : "ordered-canvas-base",
      });
      if (peekStart == null) return peekResult;
      return Promise.resolve(peekResult).then((result) => {
        if (result) result.__hardRoundPeekStartTime = peekStart;
        return result;
      });
    })
    .then(async (result) => {
      const peekDoneAt = performance.now();
      if (_hrLcActive() && result && result.__hardRoundPeekStartTime != null)
        _hrLcDuration(
          "peekStrokeMs",
          peekDoneAt - result.__hardRoundPeekStartTime,
        );
      ...
      if (_dbgOrderEntry) {
        ...
        if (previewGeneration !== _hardRoundPreviewGeneration) {
          _dbgOrderEntry.staleAtResolve = true;
          _dbgOrderEntry.staleReason = "preview-generation-cancelled";
        } else if (session !== _activeStrokeSession) {
          _dbgOrderEntry.staleAtResolve = true;
          _dbgOrderEntry.staleReason = "session-changed";
        } else if (!_inStroke || !_strokeCtx || !_strokeCanvas) {
          _dbgOrderEntry.staleAtResolve = true;
          _dbgOrderEntry.staleReason = "stroke-ended";
        } else if (!result || !result.canvas) {
          _dbgOrderEntry.staleAtResolve = true;
          _dbgOrderEntry.staleReason = "no-result";
        }
      }
      ...
      if (_hr1110Entry) {
        ...
        _hr1110Entry.hardRoundStrokeActiveAtResolve =
          typeof _hardRoundStrokeActive !== "undefined"
            ? _hardRoundStrokeActive
            : null;
        _hr1110Entry.inStrokeAtResolve = _inStroke;
        _hr1110Entry.rendererSegmentCountAtResolve = renderer
          ? renderer._segmentCount
          : null;
        ...
        if (previewGeneration !== _hardRoundPreviewGeneration)
          _hr1110Entry.rejectReason = "preview-generation-cancelled";
        else if (session !== _activeStrokeSession)
          _hr1110Entry.rejectReason = "session-changed";
        else if (!_inStroke || !_strokeCtx || !_strokeCanvas)
          _hr1110Entry.rejectReason = "stroke-ended";
        else if (!result || !result.canvas)
          _hr1110Entry.rejectReason = "no-result";
        else if (result.presentation && result.presentation.presented === false)
          _hr1110Entry.rejectReason =
            result.presentation.reason || "presentation-rejected";
      ...
      if (previewGeneration !== _hardRoundPreviewGeneration) {
        ...
        _hr11b6Log("presentLivePreview-stale", _lastPointerEvent, {
          reason: "preview-generation-cancelled",
        });
        if (_dbgOrderEntry) _hardRoundPreviewOrderPush(_dbgOrderEntry);
        return;
      }
      if (
        ...
        _hr11b6Log("presentLivePreview-stale", _lastPointerEvent, {
          reason: "session-or-stroke-ended",
        });
        if (_dbgOrderEntry) _hardRoundPreviewOrderPush(_dbgOrderEntry);
        return;
      }
      ...
      if (
        ...
        _hrFirstPresentCount(
          session,
          "presentLivePreviewRejectedCount",
          (result.presentation && result.presentation.reason) || "other",
        );
        if (_dbgOrderEntry) {
          _dbgOrderEntry.staleAtResolve = true;
          _dbgOrderEntry.staleReason =
            (result.presentation && result.presentation.reason) ||
            "not-presented";
          _hardRoundPreviewOrderPush(_dbgOrderEntry);
        }
        return;
      }
      _hr11b6Log("presentLivePreview-accepted", _lastPointerEvent, {
        resultSegmentCount: result.segmentCount,
      });
      if (_hrTlActive() && _hrTipLagLastFlushedEndpoint)
        _hrTlRecordStage(
          "acceptedPreview",
          _hrTipLagLastFlushedEndpoint.x,
      ...
      _hrFirstPresentCount(
        ...
        gpuPreviewAccepted
          ...
          : "cpuPreviewAcceptedCount",
        gpuPreviewAccepted ? "accepted" : null,
      );
      if (gpuPreviewAccepted) {
        ...
        if (result.canvasLivePresentation || window.HardRoundDebugCanvasLivePresentation) {
          if (result.canvas) {
            const drawStart = _hrLcActive() ? performance.now() : null;
            _hardRoundSetGpuOverlayVisible(
              false,
              "presentLivePreview-canvasLivePresentationMode",
            );
          ...
          if (_dbgOrderEntry) {
            _dbgOrderEntry.overlayShown = false;
            _hardRoundPreviewOrderPush(_dbgOrderEntry);
          }
          _scheduleRecomposite();
          return;
        ...
        if (
          ...
          _hardRoundLastAcceptedLiveMetadataBySession[session] = {
            generation,
            strokeId: session,
            segmentCountAtAccept,
            acceptedAt,
          };
          const logEntry = {
            generation,
            ...
            segmentCountAtAccept,
            acceptedAt,
            readbackStartedAt: null,
            readbackCompletedAt: null,
            becameLastLive: false,
            wasStale: false,
            staleReason: null,
          };
          ...
          try {
            ...
            const buf = await renderer.gpu.diagResolveIntoBuffer(
              renderer._rgb,
              renderer._composite,
            );
            ...
            const latestGenerationForSession =
              _hardRoundLastAcceptedLiveMetadataBySession[session] &&
              _hardRoundLastAcceptedLiveMetadataBySession[session].generation;
            const isLatestGeneration =
              generation === latestGenerationForSession;
            if (stillCurrentSession && isLatestGeneration) {
              ...
              logEntry.staleReason = !stillCurrentSession
                ? "old-session"
        ...
        if (
          typeof window.HardRoundIsLivePreviewCurrent === "function" &&
          !window.HardRoundIsLivePreviewCurrent(
            session,
            previewGeneration,
            renderer,
          ...
          _hrFirstPresentCount(
            session,
            "presentLivePreviewRejectedCount",
            "stale-after-gpu-completion",
          );
          if (_dbgOrderEntry) {
            _dbgOrderEntry.staleAtResolve = true;
            _dbgOrderEntry.staleReason = "stale-after-gpu-completion";
            _hardRoundPreviewOrderPush(_dbgOrderEntry);
          }
          return;
        }
        window.HardRoundOverlayPresentedStrokeId = session;
        if (renderer._brushPerfStrokeBeginAt) {
          _brushDiagPerfNote("hard-round-first-present", {
            ms: performance.now() - renderer._brushPerfStrokeBeginAt,
          });
          renderer._brushPerfStrokeBeginAt = 0;
        }
        _hardRoundSetGpuOverlayVisible(
          true,
          "presentLivePreview-accepted-frame",
        );
        // --- diagnostic (11A.31): this generation's pixels were actually shown ---
        if (_dbgOrderEntry) {
          _dbgOrderEntry.overlayShown = true;
          _hardRoundPreviewOrderPush(_dbgOrderEntry);
        }
        return;
      }
...
function _hardRoundStartPreviewFlight(renderer, session) {
  ...
  Promise.resolve(_hardRoundPresentLivePreview(renderer)).finally(() => {
    ...
    if (flightStart != null) {
      ...
      _hrLcDuration("previewFlightMs", performance.now() - flightStart);
    }
    ...
    const needsFollowup =
      _hardRoundPreviewNeedsFollowup ||
...
function _hardRoundSchedulePreviewFrame(renderer) {
  ...
  _hardRoundPreviewRAF = requestAnimationFrame((nowMs) => {
    ...
    if (_hrMtCbStart != null)
      _hrMtRecord(
        "_hardRoundSchedulePreviewFrame-callback-total",
        _hrMtCbStart,
        performance.now(),
      );
  });
...
function _hardRoundRequestLivePreview(renderer, immediate = false) {
  ...
  _hrLcInc("previewRequest");
  _hardRoundPreviewRequestedRenderer = renderer;
  _hardRoundPreviewSession = _activeStrokeSession;
  if (_hardRoundPreviewInFlight) {
    _hrLcInc("followupRequested");
    _hardRoundPreviewNeedsFollowup = true;
    return;
  }
  if (_hardRoundPreviewRAF !== null) {
    if (!immediate && _hardRoundPreviewRAFSession === _activeStrokeSession)
      return;
  ...
  if (immediate) {
    _hardRoundStartPreviewFlight(renderer, _activeStrokeSession);
    return;
  }
...
function _hardRoundPresentFinishedFrame(
  renderer,
  originStrokeId = _activeStrokeSession,
) {
  ...
  if (window.HardRoundDebugLivePreviewCrossover) {
    _hr1110FinishedFrameStartedAtBySession[_activeStrokeSession] =
      performance.now();
  }
  return renderer.peekStroke().then((result) => {
    if (result && result.canvas) {
      ...
      if (_strokeCtx && _strokeCanvas) {
        _hrStaleFinalizerMutation(
          originStrokeId,
          "finishedPreviewStrokeScratchCanvas",
          "current-shared-stroke",
          "old-finished-preview",
          () => {
            _strokeCtx.clearRect(
              0,
              0,
...
function _ensureTipAssetId(canvas, previousCanvas) {
  ...
  if (
    previousCanvas &&
    previousCanvas._tipAssetId &&
    (canvas === previousCanvas ||
      (canvas.src && canvas.src === previousCanvas.src))
  ) {
...
window._setBrushTipShape = function (roundness, flipX, flipY) {
  window.brushTipRoundness = Math.max(
    0.01,
    Math.min(1, Number(roundness) || 1),
  );
...
function _normalizeTipAlpha(canvas) {
  ...
  let id;
  try {
    id = c2d.getImageData(0, 0, w, h);
    if (window.TipReadbackExperiment)
      window.TipReadbackExperiment.record("tip-normalization-read", {
        width: w,
        height: h,
        duration: performance.now() - readStart,
        totalDuration: performance.now() - normalizeStart,
      });
  } catch (e) {
    return canvas;
...
window.setBrushTip = function (canvas, referenceDiameter, invalidationReason) {
  ...
  const trace = window.CustomTipCacheTrace,
    previousCanvas = window.brushTipCanvas,
    previousVersion = window.brushTipVersion || 0,
    previousAlphaBuffer = _tipAlphaBuf;
  ...
  window.brushTipReferenceDiameter =
    canvas &&
    Number.isFinite(Number(referenceDiameter)) &&
    Number(referenceDiameter) > 0
      ? Number(referenceDiameter)
      : null;
  window.brushTipVersion = (window.brushTipVersion || 0) + 1;
  _tipAlphaSeedPixels =
    window.TipReadbackExperiment &&
    window.TipReadbackExperiment.mode === "D" &&
    _lastNormalizedTipPixels
      ? {
          data: _lastNormalizedTipPixels.data,
          w: _lastNormalizedTipPixels.w,
          h: _lastNormalizedTipPixels.h,
          version: window.brushTipVersion,
        }
      : null;
  _tipDabCache.clear();
  _aaDabCache.clear();
  ...
  if (trace)
    trace.invalidated({
      reason: _tipAlphaInvalidationReason,
      previousVersion,
      tipVersion: window.brushTipVersion,
      previousTipCanvasId: trace.objectId(previousCanvas, "tip-canvas"),
      tipCanvasId: trace.objectId(window.brushTipCanvas, "tip-canvas"),
      sameCanvas: previousCanvas === window.brushTipCanvas,
      alphaBufferId: trace.objectId(previousAlphaBuffer, "alpha-buffer"),
...
function _sampleVisibleCanvasColor(e) {
  ...
  if (palette && typeof palette.setForegroundFromSample === "function")
    palette.setForegroundFromSample(hex);
  ...
  if (
    ...
    layer.type === "smart-raster" &&
    window.SmartRasterLayer &&
    typeof window.SmartRasterLayer.getStyleIdAt === "function" &&
    palette &&
    typeof palette.selectAdvancedStyleById === "function"
  ) {
    const styleId = window.SmartRasterLayer.getStyleIdAt(
      curLayer,
      curFrame,
      x,
      y,
  ...
  if (
    !selectedOwnedStyle &&
    palette &&
    typeof palette.selectMatchingRgba === "function"
  )
    palette.selectMatchingRgba(pixel[0], pixel[1], pixel[2], pixel[3]);
}
function _queueVisibleCanvasColorSample(e) {
  _eyedropperLatestPosition = {
    clientX: e.clientX,
    clientY: e.clientY,
    pointerId: e.pointerId,
  };
...
function _endVisibleCanvasColorSampling(pointerId) {
  ...
  if (_eyedropperSampleRaf) {
    cancelAnimationFrame(_eyedropperSampleRaf);
    _eyedropperSampleRaf = 0;
  }
  return true;
}
    ...
    window
      ...
      .then(() => {
        ...
        if (
          ...
          _hardRoundSetGpuOverlayVisible(
            false,
            "deferred-stroke-barrier-settled",
          );
        }
      ...
      .catch((err) => {
        console.error(
          "[BrushEngine] Commit barrier failed before stroke initialization:",
          err,
        );
...
function _brushPointerDown(e) {
  ...
  const diagnosticPointerdownEntry =
    window.FirstDabLatencyProbe && window.FirstDabLatencyProbe.enabled
      ? performance.now()
      : 0;
  const customTraceEntry =
    window.CustomFirstDabTrace && window.CustomFirstDabTrace.enabled
      ? performance.now()
      : 0;
  ...
  if (
    activeGroupId ||
    panning ||
    (typeof _zoomDrag !== "undefined" && _zoomDrag) ||
    spaceHeld ||
    tool === "transform"
  )
    return;
  ...
  if (tool === "eyedropper") {
    e.preventDefault();
    _eyedropperPointerId = e.pointerId;
    _eyedropperLastAppliedRgba = null;
    activeC.setPointerCapture(e.pointerId);
    _sampleVisibleCanvasColor(e);
    return;
  }
  if (
    tool !== "brush" &&
  ...
  if (
    typeof isDrawingFrameHidden === "function" &&
    isDrawingFrameHidden(curLayer, curFrame)
  )
    return;
  ...
  if (_stabilizerFinishing && _stabilizerFinalizeCB) {
    _traceStrokeLifecycle("next-pointerdown-finishes-previous", {
      nextPointerId: e.pointerId,
    });
    _stabilizerAccelerateToCompletion();
  }
  if (drawing || _inStroke || lineStart || _colorEraserOwnership) {
    _traceStrokeLifecycle("next-pointerdown-ends-previous", {
      nextPointerId: e.pointerId,
    });
    _endStroke(_activeStrokePointerId);
  }
  ...
  if (
    ...
    _curveCommitPointerId = e.pointerId;
    _commitCurveTool(e);
    return;
  }
  if ((tool === "brush" || tool === "eraser") && window.FirstDabLatencyProbe) {
    window.FirstDabLatencyProbe.begin({
      layerType: layers[curLayer] && layers[curLayer].type,
      pointerdownAt: diagnosticPointerdownEntry,
    });
    window.FirstDabLatencyProbe.setupMeasure(
      "eventValidationAndPreventDefault",
      diagnosticPointerdownEntry,
    );
  }
  let diagnosticSetupStart =
    window.FirstDabLatencyProbe && window.FirstDabLatencyProbe.enabled
      ? performance.now()
      : 0;
  if (window.CompositionPrewarm) window.CompositionPrewarm.beforeStroke();
  if (window.FirstDabLatencyProbe)
    window.FirstDabLatencyProbe.setupMeasure(
      "pendingPrewarmCancellation",
      diagnosticSetupStart,
    );
  diagnosticSetupStart =
    window.FirstDabLatencyProbe && window.FirstDabLatencyProbe.enabled
      ? performance.now()
  ...
  if (latencyProfiler)
    latencyProfiler.startStroke({
      ...
      buffers: { stroke: !!_strokeCanvas, preview: !!_strokePreviewCanvas },
    });
  if (window.TipReadbackExperiment) window.TipReadbackExperiment.strokeStart();
  if (window.CustomFirstDabTrace)
    window.CustomFirstDabTrace.beginStroke({
      entryAt: customTraceEntry,
      tip: !!window.brushTipCanvas,
      presetId: window._activeBrushPresetId || null,
      settings: {
        size: getBrushSize(),
        ...
        opacity: brushOpacity,
        flow: brushFlow,
        pressureSize: window._brushPressureSize,
        pressureOpacity: window._brushPressureOpacity,
        zoom: typeof zoom === "number" ? zoom : null,
      },
    });
  if (window.CustomFirstDabTrace)
    window.CustomFirstDabTrace.event("stroke-state-initialization-begins");
  if (window.CustomTipCacheTrace) window.CustomTipCacheTrace.strokeStart();
  if (latencyProfiler) {
    latencyProfiler.point("pointerdown-received", {
      visibility: document.visibilityState,
      focused: document.hasFocus(),
      framePhaseMs: performance.now() % 16.67,
    });
    latencyProfiler.point("stroke-initialization-begins");
  }
  if (window.BrushRafExperiment)
    window.BrushRafExperiment.strokeBegins({
      layerIndex: curLayer,
      frameIndex: curFrame,
      layerType: layers[curLayer] && layers[curLayer].type,
    });
  if (window.BrushFirstDabExperiment)
    window.BrushFirstDabExperiment.strokeBegins({
      layerIndex: curLayer,
      frameIndex: curFrame,
      layerType: layers[curLayer] && layers[curLayer].type,
    });
  if (window.KeyframeLatencyExperiment)
    window.KeyframeLatencyExperiment.strokeBegins({
      layerIndex: curLayer,
      frameIndex: curFrame,
      layerType: layers[curLayer] && layers[curLayer].type,
    });
  if (
    (window.CustomBrushDebugRasterParity ||
      window.CustomBrushDebugResolvedDabs) &&
    window.brushTipCanvas
  ) {
  ...
  if (window.HardRoundDebugRoutingTrace) {
    _hrRtNewStroke(
      _activeStrokeSession,
      {
        ...
        airbrush: !!window._brushAirbrush,
        eligibleNowResult: _hardRoundEligibleNow(),
        hasCore: !!_hardRoundGetCore(),
        hasRenderer: !!_hardRoundGetRenderer(),
      },
      _hardRoundStrokeActive,
    );
  }
  ...
  if (window.CustomFirstDabTrace)
    window.CustomFirstDabTrace.event("pressure-input-initialization-complete", {
  ...
  if (window.CustomFirstDabTrace) {
    window.CustomFirstDabTrace.sample({
      source: "pointerdown",
    ...
    window.CustomFirstDabTrace.event("first-pointer-sample-processed", {
      x: p.x,
      y: p.y,
    });
  }
  ...
  if (latencyProfiler)
    latencyProfiler.measure("pointer-and-dynamics-init", inputStateStart);
  diagnosticSetupStart =
    window.FirstDabLatencyProbe && window.FirstDabLatencyProbe.enabled
      ? performance.now()
      : 0;
  ...
  _strokeOwnerLayer = curLayer;
  _strokeOwnerFrame = curFrame;
  ...
  if (_customTipGpuStrokeActive) {
    ...
    if (
      window._customTipGpuRenderer &&
      typeof window._customTipGpuRenderer.beginStroke === "function"
    ) {
      ...
      window._customTipGpuRenderer.beginStroke({
        ...
        height: targetH,
        strokeId: _activeStrokeSession,
        presentationOpacity: Math.max(0, Math.min(1, brushOpacity)),
      });
    }
    ...
    if (
      ...
      window.CustomTipGpuResources.getOrCreatePaperTexture(
        window.brushTextureCanvas,
        window.brushTextureVersion,
      ).catch(() => {});
    }
  } else if (
    _shouldRunCustomTipGpuDiagnostic() &&
    window._customTipGpuRenderer &&
    typeof window._customTipGpuRenderer.beginStroke === "function"
  ) {
    window._customTipGpuRenderer.beginStroke();
  ...
  if (_hardRoundStrokeActive) {
    _brushDiagPerfNote("hard-round-renderer", {
      created: _hardRoundRendererCreatedPending,
    });
    if (_hardRoundRendererResizePending)
      _brushDiagPerfNote("hard-round-resize");
    _hardRoundRendererCreatedPending = false;
    _hardRoundRendererResizePending = false;
  }
  if (_hardRoundStrokeActive) _hrPerfMarkStrokeStart(_activeStrokeSession);
  _traceStrokeLifecycle("stroke-start", {
    sourceLayer: curLayer,
    sourceFrame: curFrame,
  });
  ...
  if (tool === "line" || tool === "curve") {
    ...
    _lineGesture = {
      ...
      endPoint: { x: p.x, y: p.y },
      currentLength: 0,
      recordedLength: 0,
      pressureSamples: [{ distance: 0, pressure: storedLinePressure }],
      currentEventPressure: Number(e.pressure),
      lastEditDiagnostic: null,
    };
    if (tool === "curve")
      _curveToolGesture = {
        phase: "endpoints",
  ...
  diagnosticSetupStart =
    window.FirstDabLatencyProbe && window.FirstDabLatencyProbe.enabled
      ? performance.now()
      : 0;
  let stageStart = latencyProfiler ? performance.now() : 0;
  if (!_hardRoundStrokeActive && !_customTipGpuStrokeActive) pushUndo();
  if (window.FirstDabLatencyProbe)
    window.FirstDabLatencyProbe.setupMeasure("pushUndo", diagnosticSetupStart, {
      pushUndoSnapshotMethod:
  ...
  if (latencyProfiler)
    latencyProfiler.measure("undo-snapshot-setup", stageStart, {
  ...
  const autoCreatedKey = ensureKey({ deferVisualRefresh: true });
  if (window.FirstDabLatencyProbe)
    window.FirstDabLatencyProbe.finishEnsureKey(
      diagnosticSetupStart,
      autoCreatedKey,
    );
  if (
    ...
    latencyProfiler.measure("smart-raster-ensure-key", stageStart, {
      autoCreatedKey: !!autoCreatedKey,
      layerIndex: curLayer,
      frameIndex: curFrame,
    });
  if (window.BrushRafExperiment)
    window.BrushRafExperiment.noteKeyCheck({
      autoCreatedKey: !!autoCreatedKey,
      duration: performance.now() - stageStart,
    });
  if (window.KeyframeLatencyExperiment)
    window.KeyframeLatencyExperiment.noteKeyCheck({
      autoCreatedKey: !!autoCreatedKey,
      duration: performance.now() - stageStart,
    });
  if (latencyProfiler)
    latencyProfiler.measure("keyframe-and-session-binding", stageStart, {
      autoCreatedKey: !!autoCreatedKey,
      layerIndex: curLayer,
      frameIndex: curFrame,
    });
  ...
  if (latencyProfiler)
    latencyProfiler.measure("color-eraser-session-setup", stageStart, {
      active: tool === "eraser" && window.eraserMode === "color",
      getImageData: tool === "eraser" && window.eraserMode === "color",
    });
  diagnosticSetupStart =
    window.FirstDabLatencyProbe && window.FirstDabLatencyProbe.enabled
      ? performance.now()
  ...
  if (latencyProfiler)
    latencyProfiler.measure("taper-buffer-preparation", stageStart, {
      active: !!_strokeReplayBase,
      canvas: { width: CW, height: CH },
    });
  diagnosticSetupStart =
    window.FirstDabLatencyProbe && window.FirstDabLatencyProbe.enabled
  ...
  stageStart = latencyProfiler ? performance.now() : 0;
  _selectionScopeBase =
    tool === "eraser" && window.SelectionScope
      ? SelectionScope.captureArtwork(activeC)
      : null;
  if (window.FirstDabLatencyProbe)
    window.FirstDabLatencyProbe.setupMeasure(
      "selectionSetup",
  ...
  if (latencyProfiler)
    latencyProfiler.measure("selection-scope-setup", stageStart, {
      active: !!window.SelectionScope,
      captured: !!_selectionScopeBase,
      getImageData: !!_selectionScopeBase,
    });
  diagnosticSetupStart =
    window.FirstDabLatencyProbe && window.FirstDabLatencyProbe.enabled
      ? performance.now()
  ...
  if (window.CustomFirstDabTrace)
    window.CustomFirstDabTrace.event("spacing-path-initialization-complete", {
      carryOver: _strokeSegCarryOver,
    });
  _lastPointerEvent = e;
  if (window.FirstDabLatencyProbe)
    window.FirstDabLatencyProbe.setupMeasure(
  ...
  if (window.CustomFirstDabTrace)
    window.CustomFirstDabTrace.event("stroke-state-initialization-complete", {
      strokeCanvas: !!_strokeCanvas,
      inStroke: _inStroke,
    });
  if (latencyProfiler) {
    latencyProfiler.measure("stroke-live-buffer-activation", stageStart, {
      active: tool !== "eraser",
      canvas: {
        width: (_strokeCanvas && _strokeCanvas.width) || 0,
        height: (_strokeCanvas && _strokeCanvas.height) || 0,
    ...
    latencyProfiler.measure("pointerdown-setup-total", strokeSetupStart, {
      tool,
    });
  }
  ...
  if (window.CustomFirstDabTrace) {
    window.CustomFirstDabTrace.event("first-dab-eligibility", {
      eligible: true,
      reason: "unconditional-pointerdown-dab",
      distanceThreshold: 0,
      flowSpacingRatio: _flowSpacingRatio,
    });
    window.CustomFirstDabTrace.firstDabDispatch({
      source: "pointerdown",
      x: p.x,
      y: p.y,
    });
  ...
  if (window.FirstDabLatencyProbe)
    window.FirstDabLatencyProbe.finalizeSetup(performance.now());
  const diagnosticDabStart = window.FirstDabLatencyProbe
    ? window.FirstDabLatencyProbe.firstDabStart()
    : 0;
  const firstDabStart = latencyProfiler ? performance.now() : 0;
  ...
  try {
    if (_hardRoundStrokeActive) {
      ...
      _hrInputLatencyNote(
        ...
        "firstCoreInputTime",
        performance.now(),
        { firstGeometrySourceEvent: "pointerdown" },
      );
      const beginSeg = _hardRoundCore.beginStroke({
        x: p.x,
        y: p.y,
        pressure: prototypeBeginPressure,
        pointerType: e.pointerType,
        timeStamp: e.timeStamp || performance.now(),
      });
      if (beginSeg) {
      ...
      const gpuLiveCompatible =
        ...
        !(
          window.SelectionScope &&
          SelectionScope.isRestricted &&
          SelectionScope.isRestricted()
        ) &&
        !(layers[curLayer] && layers[curLayer].type === "smart-raster") &&
        !_hardRoundHasVisibleLayerAbove(curLayer, curFrame);
      hardRoundRenderer.preferGpu = gpuLiveCompatible;
      if (window.HardRoundDebugForceBackend === "cpu")
        hardRoundRenderer.preferGpu = false;
      else if (window.HardRoundDebugForceBackend === "gpu")
        hardRoundRenderer.preferGpu = true;
      hardRoundRenderer.presentationOpacity = Math.max(
        0,
        Math.min(1, brushOpacity),
      );
      const activeCProbeEnabled = !!window.HardRoundDebugActiveCProbe;
      const _hrProbeBefore = activeCProbeEnabled
        ? _hrHashActiveCRegion(
            0,
            0,
            activeC.width,
            activeC.height,
      ...
      _hardRoundActiveContext = {
        strokeId: _activeStrokeSession,
        renderer: hardRoundRenderer,
        layerIndex: curLayer,
        frameIndex: curFrame,
        destinationCanvas:
          (layers[curLayer] && layers[curLayer].frames[curFrame]) || null,
        opacity: Math.max(0, Math.min(1, brushOpacity)),
        blendMode: window.brushBlendMode || "normal",
        compositeOperation: _hardRoundCapturedCompositeOperation(
          window.brushBlendMode || "normal",
        ),
        styleId:
          typeof activeAdvancedStyleIdForPainting === "function"
            ? activeAdvancedStyleIdForPainting()
        ...
        smartRasterMode:
          (layers[curLayer] && layers[curLayer].renderMode) || null,
        smartRasterVersion: window.SmartRasterV4 ? "v4" : "current",
        ownershipBefore: null,
        resolvedCanvas: null,
        resolvedMaskCanvas: null,
        dirtyRect: null,
        state: "active",
        gpuCommit: false,
      ...
      if (firstPresentRecord) {
        firstPresentRecord.gpuReadyAtBegin = !!(
          hardRoundRenderer.isGpuActive && hardRoundRenderer.isGpuActive()
        );
        firstPresentRecord.backend = firstPresentRecord.gpuReadyAtBegin
          ? "webgpu"
          : "cpu";
        firstPresentRecord.previewGenerationAtBegin =
          _hardRoundPreviewGeneration;
        firstPresentRecord.overlayOwnerAtBegin =
          window.HardRoundOverlayOwnerStrokeId == null
            ? null
            : window.HardRoundOverlayOwnerStrokeId;
        if (firstPresentRecord.gpuReadyAtBegin)
          firstPresentRecord.gpuClearCountThisStroke++;
      }
      if (hardRoundBeginStarted)
        _brushDiagPerfNote("hard-round-begin", {
          ms: performance.now() - hardRoundBeginStarted,
        });
      _brushDiagPerfNote("gpu-state", {
        backend:
          hardRoundRenderer.isGpuActive && hardRoundRenderer.isGpuActive()
            ? "webgpu"
            : "cpu",
      });
      const _hrProbeAfter = activeCProbeEnabled
        ? _hrHashActiveCRegion(
            0,
            0,
            activeC.width,
      ...
      if (window.HardRoundDebugCanvasLivePresentation) {
        _hardRoundSetGpuOverlayVisible(
          false,
          "beginStroke-canvasLivePresentationMode",
        );
        _traceStrokeLifecycle(
          "hardround-overlay-show-suppressed-canvas-live-presentation",
          {
            gpuActive: !!(
              hardRoundRenderer.isGpuActive && hardRoundRenderer.isGpuActive()
            ),
        ...
        const preservePriorUntilCommit = !!(
          hardRoundRenderer.isGpuActive &&
          hardRoundRenderer.isGpuActive() &&
          (_hardRoundPendingCommitCount > 0 || basePresentationStale) &&
          priorOverlay &&
          !priorOverlay.hidden &&
          priorOverlay.style.display !== "none" &&
          priorOverlay.style.opacity !== "0"
        );
        if (!preservePriorUntilCommit) {
          if (window.HardRoundPresenterWarm)
            _hardRoundConcealMountedOverlay(
              "beginStroke-await-first-present-mounted-transparent",
            );
          else
            _hardRoundSetGpuOverlayVisible(
              false,
              "beginStroke-await-first-present",
            );
        }
        _traceStrokeLifecycle("hardround-overlay-show-deferred", {
          gpuActive: !!(
            hardRoundRenderer.isGpuActive && hardRoundRenderer.isGpuActive()
          ),
          reason: preservePriorUntilCommit
            ? "preserve-prior-until-commit"
            : "await-first-successful-present",
        });
      }
      ...
      const _hrProbeAfterOverlay = activeCProbeEnabled
        ? _hrHashActiveCRegion(
            0,
            0,
            activeC.width,
      ...
      if (activeCProbeEnabled)
        window.HardRoundActiveCProbe = {
          ...
          beforeToAfterBeginStrokeChanged:
            _hrProbeBefore.hash !== _hrProbeAfter.hash,
          afterBeginStrokeToOverlayChanged:
            _hrProbeAfter.hash !== _hrProbeAfterOverlay.hash,
          beforeToOverlayChanged:
            _hrProbeBefore.hash !== _hrProbeAfterOverlay.hash,
        };
      ...
      window.HardRoundDebugCapture = {
        rawSegmentCount: 0,
        cpuEquivalentMergedCount: 0,
        gpuDispatchedCount: 0,
        gpuDrawSegmentCalls: 0,
        gpuFlushCalls: 0,
        vertexCountGenerated: 0,
        vertexCountUploaded: 0,
        vertexCountDrawn: 0,
      };
      ...
      if (firstPresentRecord && firstPresentRecord.gpuReadyAtBegin)
        firstPresentRecord.gpuGeometrySubmitCountThisStroke++;
      ...
      const activeSS =
        ...
        window._customTipGpuRenderer &&
        window._customTipGpuRenderer.instance
          ? window._customTipGpuRenderer.instance.ss
          : window.CustomTipGpuActiveSS || null;
      window.HardRoundDebug = {
        strokeId: window._hrDebugStrokeId,
        route: _customTipGpuStrokeActive ? "custom-tip-gpu" : "legacy",
        backend: _customTipGpuStrokeActive
          ? isLive
            ? "WEBGPU-LIVE"
            : "WEBGPU-OFFSCREEN"
          : "LEGACY",
  ...
  if (window.FirstDabLatencyProbe)
    window.FirstDabLatencyProbe.firstDabEnd(diagnosticDabStart);
  if (window.CustomFirstDabTrace)
    window.CustomFirstDabTrace.event("first-dab-rasterization-complete");
  if (latencyProfiler) {
    latencyProfiler.point("first-dab-rasterization-finish");
    latencyProfiler.measure("first-dab-pipeline-total", firstDabStart, {
      dirtyRect: _frameDirty
        ? {
            minX: _frameDirty.minX,
            minY: _frameDirty.minY,
            maxX: _frameDirty.maxX,
    ...
    latencyProfiler.point("first-dab-generated");
  }
  if (window.CustomFirstDabTrace)
    window.CustomFirstDabTrace.event("recomposite-scheduling-begins");
  _scheduleRecomposite({ firstDab: true });
  if (window.CustomFirstDabTrace)
    window.CustomFirstDabTrace.event("recomposite-scheduled");
  if (window._brushAirbrush && window._brushContinuousSpraying)
    _startAirbrushSpray();
}
...
canvasArea.addEventListener("pointerdown", (e) => {
  ...
  if (
    e.target.closest &&
    e.target.closest(
      "button,input,select,textarea,[contenteditable],.tf-floating-action,#resize-canvas-settings,#camera-view-preview",
    )
  )
    return;
  _brushPointerDown(e);
...
let _linePreviewRAFPending = false,
  _linePreviewRAFHandle = 0;
let _linePreviewLatestX = 0,
  _linePreviewLatestY = 0,
  _linePreviewLatestEvent = null;
...
function _scheduleLinePreview(x, y, e) {
  _linePreviewLatestX = x;
  _linePreviewLatestY = y;
  _linePreviewLatestEvent = e;
  _linePreviewMoveSequence++;
  _linePreviewGeneration++;
...
function _cancelLinePreview() {
  ...
  if (_linePreviewRAFHandle) {
    cancelAnimationFrame(_linePreviewRAFHandle);
    _linePreviewRAFHandle = 0;
  }
  _linePreviewRAFPending = false;
  _linePreviewGeneration++;
  if (discarded && window.DEBUG_LINE_PREVIEW)
    console.debug("[LinePreview]", {
      generation: _linePreviewGeneration,
      pointermoveSequence: _linePreviewMoveSequence,
      stalePreviewDiscarded: true,
    });
}
...
function _cancelCurveTool() {
  if (!_curveToolGesture) return;
  const layer = _strokeOwnerLayer,
    frame = _strokeOwnerFrame;
  _resetCurveToolGesture();
  _strokeOwnerLayer = -1;
  _strokeOwnerFrame = -1;
  if (layer >= 0 && frame >= 0 && curLayer === layer && curFrame === frame)
    recomposite(layer, frame);
}
function _commitCurveTool(e) {
  ...
  pushUndo();
  ensureKey();
  if (!_strokeCanvas || !_inStroke) {
    _ensureStrokeCanvas();
    _inStroke = true;
  }
  _renderLineDrag(
    _curveToolGesture.control.x,
    _curveToolGesture.control.y,
    e || _lastPointerEvent || { pointerType: "mouse", pressure: 0.5 },
    "commit",
  );
  if (_inStroke) {
    _inStroke = false;
  ...
  const layer = _strokeOwnerLayer,
    frame = _strokeOwnerFrame;
  _resetCurveToolGesture();
  _completePostStrokePresentation(layer, frame);
  _strokeOwnerLayer = -1;
  _strokeOwnerFrame = -1;
  return true;
}
...
function _handleMoveEvent(e) {
  if (
    _pendingDeferredStroke &&
    e.pointerId === _pendingDeferredStroke.pointerId
  ) {
  ...
  if (
    ...
    const p = getPos(e);
    _lastPointerEvent = e;
    _curveToolGesture.control = { x: p.x, y: p.y };
    _scheduleLinePreview(p.x, p.y, e);
    return;
  }
  if ((!drawing && !_lineDragging) || activeGroupId || _strokeCompletionStarted)
    return;
  if (_activeStrokePointerId != null && e.pointerId !== _activeStrokePointerId)
    return;
  if (!(e.buttons & 1)) {
    _endStroke(e.pointerId);
    return;
  }
  if (_hardRoundStrokeActive) {
    ...
    _hrInputLatencyNote(
      ...
      "firstRawEventTime",
      performance.now(),
      { firstRawEventType: e.type || "pointermove" },
    );
  }
  e.preventDefault();
  ...
  if ((tool === "line" || tool === "curve") && _lineDragging) {
    ...
    currentPressure = retained.length
      ? retained[retained.length - 1].pressure
      : 1;
    lx = latest.point.x;
    ly = latest.point.y;
    _lastPointerEvent = latest.event;
    _scheduleLinePreview(latest.point.x, latest.point.y, latest.event);
    return;
  }
  for (const ev of events) {
    _brushDiagPerfNote("stabilized-sample");
    if (window.HardRoundDebugRoutingTrace && !_hardRoundStrokeActive)
      _hrRtNoteLegacyPathEntered();
    const newPressure = _hardRoundStrokeActive
      ? _getPrototypePressure(ev)
      : _getPressure(ev);
    if (_hardRoundStrokeActive)
      _hrInputLatencyNote(_activeStrokeSession, "firstPressureAcceptedTime");
    const raw = getPos(ev);
    ...
    const effPressure = _contactFilteredPressure(
      newPressure,
      raw.x,
      raw.y,
      ev.pointerType,
    ...
    if (_hardRoundStrokeActive)
      _hrInputLatencyNote(_activeStrokeSession, "firstStabilizerInputTime");
    const evTime =
      Number.isFinite(ev.timeStamp) && ev.timeStamp > 0
        ? ev.timeStamp
        : performance.now();
    if (_hrTlActive()) {
      _hrTlRecordStabilization();
      _hrTlRecordRaw(raw.x, raw.y, evTime, e.type || "pointermove", {
        clientX: ev.clientX,
        clientY: ev.clientY,
      });
      _hrTlData().latest.coalesced = _hrTlPoint(raw.x, raw.y, evTime, {
    ...
    if (_hardRoundStrokeActive)
      _hrFirstPresentNote(_activeStrokeSession, "firstStabilizedInputTime");
    _tipDisplayRecordAuthoritative(p.x, p.y, performance.now());
    _updateVelocity(p.x, p.y, evTime);
    if (window._brushAirbrush && Math.hypot(p.x - lx, p.y - ly) > 0.01)
      _airbrushLastMovementTime = performance.now();
...
activeC.addEventListener("pointermove", (e) => {
  if (tool === "eyedropper" && e.pointerId === _eyedropperPointerId) {
    if (e.buttons & 1) {
      e.preventDefault();
      _queueVisibleCanvasColorSample(e);
    }
    return;
  }
...
function _pointerEndStroke(e) {
  ...
  const smartPointerupTiming =
    drawing && _hardRoundStrokeActive && _hardRoundActiveContext
      ? _hrSmartPointerupBegin(
          finalizingStrokeSession,
          !!_hardRoundActiveContext.smartRaster,
          e.timeStamp,
        )
      : null;
  ...
  if (window.HardRoundDebugLivePreviewCrossover) {
    _hr1110PointerupStartedAtBySession[_activeStrokeSession] =
      performance.now();
  }
  ...
  if (window.HardRoundDebugCaptureLastLiveFrame) {
    const frozenSource =
      _hardRoundLastAcceptedLiveMetadataBySession[_activeStrokeSession];
    window.HardRoundFrozenLiveMetadata = frozenSource
      ? Object.assign({}, frozenSource)
      : null;
    window.HardRoundFrozenLastLiveFrame = window.HardRoundLastLiveFrame
      ? Object.assign({}, window.HardRoundLastLiveFrame)
      : null;
    window.HardRoundGeometryMutationLog = [];
    if (window.HardRoundDebugPhase11A30) {
      console.log(
        "[11A.30] FROZEN_LIVE_METADATA",
        window.HardRoundFrozenLiveMetadata,
      );
    }
  }
  if (_strokeCompletionStarted) return;
  ...
  if (
    tool === "curve" &&
    _curveToolGesture &&
    _curveToolGesture.phase === "endpoints"
  ) {
    const p = getPos(e);
    if (_lineGesture) _editLinePressureProfile([{ event: e, point: p }]);
    _curveToolGesture.end = { x: p.x, y: p.y };
    _curveToolGesture.control = {
      x: (lineStart.x + p.x) / 2,
      y: (lineStart.y + p.y) / 2,
    };
    _curveToolGesture.phase = "bending";
  ...
  if (tool === "line" && lineStart) {
    ...
    const retainedPressure =
      _lineGesture && _lineGesture.pressureSamples.length
        ? _lineGesture.pressureSamples[_lineGesture.pressureSamples.length - 1]
            .pressure
        : currentPressure;
    ...
    const finalizeHardRoundCoreAndRenderer = () => {
      const endpoint =
        ...
        Number.isFinite(_hardRoundCore.lastInputRaw.x) &&
        Number.isFinite(_hardRoundCore.lastInputRaw.y)
          ? _hardRoundCore.lastInputRaw
          : finalRaw;
      const finish = _hardRoundCore.finishStroke({
        x: endpoint.x,
        y: endpoint.y,
      ...
      _traceStrokeLifecycle("hardround-finishStroke", {
        segmentCount: finish.segments ? finish.segments.length : 0,
      ...
      _traceStrokeLifecycle("hardround-stampSegments", {
        queuedSegments: _hardRoundPendingRenderSegments.length,
        previewRAFPending: _hardRoundPreviewRAF,
        previewInFlight: _hardRoundPreviewInFlight,
        previewFollowupPending: _hardRoundPreviewNeedsFollowup,
      });
      ...
      // chain so a rapid next pointerdown cannot beginStroke()/reset() the
      // same GPU mask out from under this stroke.
      const renderer = _hardRoundRenderer;
      if (renderer) {
        renderer._hardRoundFinishingOwner = finalizingStrokeSession;
        if (_hardRoundRenderer === renderer) _hardRoundRenderer = null;
      }
      const flushedCount = _hardRoundFlushPending(renderer);
      _traceStrokeLifecycle("hardround-flushPending", {
        flushedSegments: flushedCount,
        overlayVisible: _hardRoundGpuOverlay()
          ? !_hardRoundGpuOverlay().hidden
          : null,
      });
      ...
      _traceStrokeLifecycle("hardround-cancelLivePreview", {
        previewGenerationAtFinish,
        previewGenerationAfterCancel: _hardRoundPreviewGeneration,
        overlayVisible: _hardRoundGpuOverlay()
          ? !_hardRoundGpuOverlay().hidden
          : null,
      });
      ...
      const ownedContext =
        _hardRoundActiveContext &&
        _hardRoundActiveContext.strokeId === finalizingStrokeSession
          ? _hardRoundActiveContext
          : null;
      _hardRoundActiveContext = null;
      if (ownedContext && renderer) {
        ownedContext.renderer = renderer;
        ownedContext.gpuCommit = !!(
          renderer.isGpuActive && renderer.isGpuActive()
        );
        ownedContext.pointerupTiming = smartPointerupTiming;
        const rendererDirty =
          typeof renderer.getStrokeDirtyRegion === "function"
            ? renderer.getStrokeDirtyRegion()
            : null;
        if (rendererDirty) {
          ownedContext.dirtyRect = {
            x: rendererDirty.x,
            y: rendererDirty.y,
            w: rendererDirty.width,
            h: rendererDirty.height,
          };
        }
      ...
      const finishHardRoundStroke = () => {
        ...
        _traceStrokeLifecycle("hardround-recomposite-after-finalize", {
          overlayVisible: _hardRoundGpuOverlay()
            ? !_hardRoundGpuOverlay().hidden
            : null,
        });
      ...
      if (renderer) {
        ...
        _traceStrokeLifecycle("hardround-finished-preview-call", {
          gpuActive: gpuCommit,
          overlayVisible: _hardRoundGpuOverlay()
            ? !_hardRoundGpuOverlay().hidden
            : null,
        ...
        const hr32Bypass = !!(
          gpuCommit && window.HardRoundDebugBypassFinishedGpuPreview
        );
        const hr32FinishedFramePromise = hr32Bypass
          ? Promise.resolve()
          : _hardRoundPresentFinishedFrame(renderer, finalizingStrokeSession);
        hr32FinishedFramePromise.then(async () => {
          _traceStrokeLifecycle("hardround-finished-preview-presented", {
            elapsedMs: performance.now() - finishedPreviewCalledAt,
            overlayVisible: _hardRoundGpuOverlay()
              ? !_hardRoundGpuOverlay().hidden
              : null,
            hr32Bypassed: hr32Bypass,
          });
          const endStrokeCalledAt = performance.now();
          _traceStrokeLifecycle("hardround-endStroke-call", {
            gpuActive: gpuCommit,
            readback: gpuCommit,
            overlayVisible: _hardRoundGpuOverlay()
              ? !_hardRoundGpuOverlay().hidden
          ...
          if (renderer) {
            ...
            if (!_hardRoundRendererPool.includes(renderer))
              _hardRoundRendererPool.push(renderer);
          }
    ...
    const finalRaw = getPos(e);
    const finalConditioned = _baselineConditionerPush(
      _baselineSampleFromEvent(e, finalRaw, currentPressure),
      { force: true },
    );
    ...
    for (const conditioned of finalConditioned) {
      ...
      _tipDisplayRecordAuthoritative(
        finalPoint.x,
        finalPoint.y,
        performance.now(),
      );
      currentPressure = finalPoint.pressure;
      lx = finalPoint.x;
      ly = finalPoint.y;
      _lastPointerEvent = e;
    }
...
function _finalizePointerEndStroke(
  e,
  originStrokeId = _activeStrokeSession,
  fromAsync = false,
) {
  if (
    _shouldRunCustomTipGpuDiagnostic() &&
    window._customTipGpuRenderer &&
    typeof window._customTipGpuRenderer.endStroke === "function"
  ) {
    window._customTipGpuRenderer.endStroke();
  }
  const mutate = (name, before, after, apply) =>
    fromAsync
      ? _hrStaleFinalizerMutation(originStrokeId, name, before, after, apply)
      : (apply(), true);
  mutate("_hardRoundStrokeActive", _hardRoundStrokeActive, false, () => {
    _hardRoundStrokeActive = false;
  }); // Phase 8C safety net
  if (window.HardRoundDebugRoutingTrace)
    mutate("routingTraceCurrentStroke", "active", "finalized", () =>
      _hrRtFinalizeStroke(),
    );
  mutate("colorEraserStrokeState", "active", "ended", () =>
    _endColorEraserStroke(),
  );
  mutate("postStrokePresentation", "pending", "complete", () =>
    _completePostStrokePresentation(_strokeOwnerLayer, _strokeOwnerFrame),
  );
  mutate("strokeCompletionObservers", "active", "finished", () => {
    ...
    const latencyProfiler = _brushPerf();
    if (latencyProfiler)
      latencyProfiler.finishStroke({
        tool,
        sourceLayer: _strokeOwnerLayer,
        sourceFrame: _strokeOwnerFrame,
      });
    if (window.CompositionPrewarm)
      window.CompositionPrewarm.noteStrokeComplete();
    if (window.BrushRafExperiment)
      window.BrushRafExperiment.strokeEnds({ dabCount: _strokeDabCount });
    if (window.BrushFirstDabExperiment)
      window.BrushFirstDabExperiment.strokeEnds({ dabCount: _strokeDabCount });
    if (window.KeyframeLatencyExperiment)
      window.KeyframeLatencyExperiment.strokeEnds({
        dabCount: _strokeDabCount,
      });
    ...
    if (window.FirstDabLatencyProbe)
      window.FirstDabLatencyProbe.strokeComplete();
  });
  mutate("_baselineConditionerState", !!_baselineConditionerState, false, () =>
    _baselineConditionerFinish(false),
  );
  mutate("_pendingDabs.length", _pendingDabs.length, 0, () => {
    _pendingDabs.length = 0;
  });
  mutate("_curveP0", _curveP0, null, () => {
    _curveP0 = null;
  });
  mutate("_curveP1", _curveP1, null, () => {
    _curveP1 = null;
  });
  mutate("_strokeSegCarryOver", _strokeSegCarryOver, 0, () => {
    _strokeSegCarryOver = 0;
  });
  mutate("_activeStrokePointerId", _activeStrokePointerId, null, () => {
    _activeStrokePointerId = null;
  });
  mutate("_strokeOwnerLayer", _strokeOwnerLayer, -1, () => {
    _strokeOwnerLayer = -1;
  });
  mutate("_strokeOwnerFrame", _strokeOwnerFrame, -1, () => {
    _strokeOwnerFrame = -1;
  });
  _hrRapidPresentation("sharedFinalizerCompleted", originStrokeId, {
    fromAsync: !!fromAsync,
  });
}
document.addEventListener("keydown", (e) => {
  ...
  if (e.key === "Escape") {
    e.preventDefault();
    _cancelCurveTool();
  } else if (e.key === "Enter" && _curveToolGesture.phase === "bending") {
    e.preventDefault();
    _commitCurveTool(_lastPointerEvent);
  }
});
activeC.addEventListener("contextmenu", (e) => {
  if (tool === "curve" && _curveToolGesture) {
    e.preventDefault();
    _cancelCurveTool();
  }
});
window.addEventListener("tool-changed", (e) => {
  if (_curveToolGesture && (!e.detail || e.detail.tool !== "curve"))
    _cancelCurveTool();
});
activeC.addEventListener("pointerup", (e) => {
  if (e.pointerId === _curveCommitPointerId) {
    _curveCommitPointerId = null;
    return;
  }
  if (_endVisibleCanvasColorSampling(e.pointerId)) {
    if (activeC.hasPointerCapture(e.pointerId))
      activeC.releasePointerCapture(e.pointerId);
    return;
  }
  _pointerEndStroke(e);
  if (activeC.hasPointerCapture(e.pointerId))
...
activeC.addEventListener("pointercancel", (e) => {
  ...
  _endVisibleCanvasColorSampling(e.pointerId);
  _endStroke(e.pointerId);
});
