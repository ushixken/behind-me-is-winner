...
test("a single centered dab resolves to opaque color at the center pixel", async () => {
  ...
  assert.strictEqual(pr, 200);
  assert.strictEqual(pg, 50);
  assert.strictEqual(pb, 50);
});
...
test("overlapping dabs in one stroke max-blend rather than darken additively", async () => {
  ...
  assert.ok(
    pa < 200,
    `overlapping same-alpha dabs must not stack past one dab's own coverage, got ${pa}`,
  );
});
...
test("cancelStroke discards accumulation without resolving", async () => {
  ...
  assert.ok(
    r.cpu.coverage.every((v) => v === 0),
    "backing store must be cleared on cancel",
  );
});
...
test("erase composite paints coverage into alpha with rgb forced to 0", async () => {
  ...
  const [pr, pg, pb, pa] = ctx._pixel(10, 10);
  assert.strictEqual(pr, 0);
  assert.strictEqual(pg, 0);
  assert.strictEqual(pb, 0);
  assert.ok(pa > 240);
});
...
test("CPU endStroke resolves only post-preview dirty pixels with full-resolve-identical output", async () => {
  ...
  assert.ok(
    result.dirtyRegion &&
      result.dirtyRegion.width < 64 &&
      result.dirtyRegion.height < 64,
    "final resolve should be dirty-region scoped",
  );
  ...
  for (let p = 0; p < optimized.length; p += 4) {
    assert.strictEqual(
      optimized[p + 3],
      ctx._data[p + 3],
      "dirty final resolve must preserve exact alpha coverage",
    );
    if (optimized[p + 3])
      assert.deepStrictEqual(
        Array.from(optimized.slice(p, p + 3)),
        Array.from(ctx._data.slice(p, p + 3)),
        "covered-pixel RGB must match full resolve",
      );
  }
});

test("short stroke on 1920x1080 retains a substantially smaller stroke-lifetime dirty region", async () => {
  const r = new PrototypeRenderer({ width: 1920, height: 1080, ss: 1 });
  r._outCtx = makeFakeCtx(1920, 1080);
  r.beginStroke();
  r.drawSegments([
    {
      x0: 100,
  ...
  const dirty = r.getStrokeDirtyRegion();
  assert.ok(dirty);
  assert.ok(
    dirty.width * dirty.height < (1920 * 1080) / 100,
    "short stroke must not degrade to full-canvas commit bounds",
  );
  await r.endStroke();
});

test("stroke dirty bounds contain every nonzero mask pixel and survive preview consumption", async () => {
  const { r, ctx } = makeRenderer(96, 72);
  r.beginStroke();
  r.drawSegments([
    {
      x0: 20,
    ...
    for (let x = 0; x < 96; x++)
      if (ctx._pixel(x, y)[3])
        assert.ok(
          ...
          `painted pixel ${x},${y} escaped dirty bounds`,
        );
});

test("stroke dirty bounds clamp at canvas edges", () => {
  const { r } = makeRenderer(80, 60);
  r.beginStroke();
  r.drawSegments([
    {
      x0: -5,
  ...
  const dirty = r.getStrokeDirtyRegion();
  assert.ok(dirty);
  assert.strictEqual(dirty.x, 0);
  assert.strictEqual(dirty.y, 0);
  assert.ok(dirty.x + dirty.width <= 80 && dirty.y + dirty.height <= 60);
});

test("large and multiple segments union their exact changed regions", () => {
  const { r } = makeRenderer(160, 120);
  r.beginStroke();
  r.drawSegments([
    {
      x0: 10,
  ...
  const first = r.getStrokeDirtyRegion();
  r.drawSegments([
    {
      x0: 110,
      y0: 80,
  ...
  const union = r.getStrokeDirtyRegion();
  assert.ok(union.x <= first.x && union.y <= first.y);
  assert.ok(union.x + union.width > 140 && union.y + union.height > 100);
  assert.ok(union.width > first.width && union.height > first.height);
});

test("CPU-owned mask data matches resolved canvas pixels in the captured dirty region", async () => {
  const { r, ctx } = makeRenderer(90, 70);
  r.beginStroke();
  r.drawSegments([
    {
      x0: 14,
  ...
  const dirty = r.getStrokeDirtyRegion(),
    result = await r.endStroke({
      readback: false,
      includeCpuMaskData: true,
      dirtyRect: dirty,
    });
  assert.ok(result.maskData instanceof Uint8ClampedArray);
  assert.strictEqual(result.maskData.length, dirty.width * dirty.height * 4);
  for (let y = 0; y < dirty.height; y++)
    for (let x = 0; x < dirty.width; x++) {
      const p = (y * dirty.width + x) * 4,
        actual = ctx._pixel(dirty.x + x, dirty.y + y);
      assert.deepStrictEqual(
        Array.from(result.maskData.slice(p, p + 4)),
        actual,
      );
    }
});
