...
test("native-flash diagnostic is default-off, bounded and event-driven", () => {
  assert.match(
    cursor,
    /BrushCursorDebugNativeFlash==='undefined'\)window\.BrushCursorDebugNativeFlash=false/,
  );
...
test("diagnostic captures target, computed cursor, visibility and capture state", () => {
  for (const field of [
    ...
    assert.match(cursor, new RegExp(field));
});
...
test("rapid presentation lifecycle counts commits without pixel diagnostics", () => {
  assert.match(
    engine,
    /HardRoundDebugRapidPresentation==='undefined'\)window\.HardRoundDebugRapidPresentation=false/,
  );
  ...
  for (const event of [
    ...
    "readbackCompleted",
    "commitCompleted",
    "recompositeCompleted",
    "finalizerCompleted",
  ])
    assert.match(engine, new RegExp("'" + event + "'"));
  const diagnosticStart = engine.indexOf(
    "if(typeof window.HardRoundDebugRapidPresentation",
  );
  const diagnostic = engine.slice(
    diagnosticStart,
    engine.indexOf("// Phase 10.1:", diagnosticStart),
  );
