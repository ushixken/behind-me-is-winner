...
const rendererSource = fs.readFileSync(path.join(__dirname, '..', '..', 'custom-tip-gpu-renderer.js'), 'utf8');
...
function runEligibleNow({
  ...
  const sandboxWindow = hasWindow ? {
    ...
    _customTipGpuRenderer: rendererInstance ? { instance: rendererInstance } : null,
  } : undefined;
  ...
  vm.runInContext(`${eligibleFnSource}\nresult = _customTipGpuEligibleNow();`, sandbox);
...
test('GPU path selection: falls back to CPU when there is no Custom Tip set', () => {
  ...
  assert.strictEqual(runEligibleNow({ tipCanvas: { width: 0, height: 0 } }), false);
});
...
test('GPU path selection: falls back to CPU when the GPU device is lost', () => {
  assert.strictEqual(runEligibleNow({ rendererInstance: { deviceLost: true } }), false);
});
...
test('stroke start: eligibility is (re)computed and beginStroke is called only when active', () => {
  const activeAssign = engine.indexOf('_customTipGpuStrokeActive = _customTipGpuEligibleNow();');
  const beginStrokeGuard = engine.indexOf('if (_customTipGpuStrokeActive) {', activeAssign);
  const beginStrokeCall = engine.indexOf("window._customTipGpuRenderer.beginStroke({", beginStrokeGuard);
  assert.ok(activeAssign >= 0 && beginStrokeGuard > activeAssign && beginStrokeCall > beginStrokeGuard,
...
test('finish: captureFinishingTask happens before the stroke is marked inactive', () => {
  ...
  const captureCall = engine.indexOf('captureFinishingTask({ paperResource });', finishStart);
  const deactivate = engine.indexOf('_customTipGpuStrokeActive = false;', finishStart);
  assert.ok(finishStart >= 0 && captureCall > finishStart && deactivate > captureCall,
...
test('finish: pending-commit count increments before the commit tail is chained', () => {
  const finishStart = engine.indexOf('function _finishCustomTipOrCanvasCommit(');
  const increment = engine.indexOf('_customTipPendingCommitCount++;', finishStart);
  const tailChain = engine.indexOf('_customTipCommitTail = settled.catch', finishStart);
  assert.ok(increment >= 0 && tailChain > increment,
...
test('authoritative commit: readback -> undo -> composite -> save -> recomposite -> destroy -> retire overlay', () => {
  const commitStart = engine.indexOf('async function _executeCustomTipDetachedCommit(');
  const commitEnd = engine.indexOf('\nfunction _finishCustomTipOrCanvasCommit', commitStart);
  ...
  assert.ok(undo > readback, 'undo snapshot must be pushed after the GPU readback resolves, against pre-commit artwork');
  assert.ok(composite > undo, 'artwork must be composited only after the undo snapshot exists');
  assert.ok(save > composite, 'keyframe must be saved only after compositing');
  assert.ok(recomposite > save, 'recomposite must be scheduled after the keyframe save');
  assert.ok(destroy > recomposite, 'the per-stroke GPU task must be destroyed after it has been fully consumed');
  assert.ok(retire > destroy, 'the overlay must only be retired after the task is destroyed and artwork committed');
});
...
test('pending count: decremented exactly once, inside .finally(), and clamped at 0', () => {
  ...
  const decrements = body.match(/_customTipPendingCommitCount\s*=\s*Math\.max\(0,\s*_customTipPendingCommitCount\s*-\s*1\)/g) || [];
  assert.strictEqual(decrements.length, 1, 'expected exactly one decrement site in the finishing function');
  assert.match(body, /\.finally\(\(\)\s*=>\s*\{\s*\n\s*_customTipPendingCommitCount\s*=\s*Math\.max\(0,/,
...
test('per-stroke GPU task: readBuffer destroy is idempotent and guarded by a destroyed flag', () => {
  ...
  assert.match(body, /destroyed:\s*false,/, 'task must start with an explicit not-destroyed flag');
  assert.match(body, /if\s*\(this\.destroyed\s*\|\|\s*!this\.readBuffer\)\s*return null;/,
    'mapAndExtractImageData must refuse to run twice against an already-destroyed/consumed buffer');
  assert.match(body, /destroy\(\)\s*\{\s*\n\s*if\s*\(!this\.destroyed\s*&&\s*this\.readBuffer\)\s*\{/,
...
test('cleanup distinguishes per-stroke destruction from intentionally cached/reusable GPU resources', () => {
  ...
  assert.match(rendererSource, /this\.readBuffer\.destroy\(\);\s*\n\s*this\.destroyed\s*=\s*true;/);
  ...
  assert.match(finishBody, /mgr\.cachedPaperTexture/, 'the finishing path must read the cached paper texture rather than recreating it');
  assert.doesNotMatch(finishBody, /cachedPaperTexture\.destroy\(\)/, 'the finishing path must not destroy the shared cached paper texture');
});
...
test('overlay retirement checks ownership both at call time and again after the commit-revision wait', () => {
  ...
  const end = engine.indexOf('\nasync function _executeCustomTipDetachedCommit', start);
  const body = engine.slice(start, end);
  const earlyGuard = body.indexOf('const isNewerOwner = (overlayOwner != null && overlayOwner !== strokeId);');
  ...
  assert.ok(earlyGuard >= 0 && earlyReturn > earlyGuard, 'must bail out early if a newer stroke already owns the overlay');
  assert.ok(lateGuard > earlyReturn && lateReturn > lateGuard && hideCall > lateReturn,
  ...
  const ownerNowAssign = body.indexOf('const ownerNow = window.CustomTipOverlayOwnerStrokeId;');
  const activeNowAssign = body.indexOf('const activeNow = _activeStrokeSession;');
  assert.ok(ownerNowAssign >= 0 && ownerNowAssign < lateGuard,
    'ownerNow must be freshly re-read from window state inside the retire() closure, not reused from the earlier snapshot');
  assert.ok(activeNowAssign >= 0 && activeNowAssign < lateGuard,
...
test('overlay ownership predicate: isolated simulation matches the production guard', () => {
  ...
  function isNewerOwner(strokeId, overlayOwner, gpuStrokeActive, activeStrokeSession) {
    return (overlayOwner != null && overlayOwner !== strokeId) ||
      (gpuStrokeActive && activeStrokeSession != null && activeStrokeSession !== strokeId);
  }

  // Stroke A finishing after B has become the current owner: A must not
  // clear/hide B's overlay.
  assert.strictEqual(isNewerOwner('A', 'B', false, null), true, 'A must be treated as stale once B owns the overlay');
...
test('overlay retirement is deferred until the committed revision has actually presented, when tracked', () => {
  ...
  const end = engine.indexOf('\nasync function _executeCustomTipDetachedCommit', start);
  const body = engine.slice(start, end);
  assert.match(body, /whenRevisionPresented\(committedRevision, retire\)/,
