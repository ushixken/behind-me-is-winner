...
const layersSource = fs.readFileSync(
  path.join(__dirname, "../../../core/layers.js"),
  "utf8",
);
...
test("production source: awaitPendingBrushCommits awaits BOTH Hard Round and Custom Tip tails", () => {
  ...
  assert.match(
    body,
    /if\(!pendingHardRound&&!pendingCustomTip\)\{\s*\n\s*break;/,
...
test("isolated model: waiting resolves once both counts are 0, and re-loops if a new commit starts mid-wait", async () => {
  ...
  model.customTipTail = rb.promise.then(() => {
    model.customTipPending = 0;
  });

  let resolved = false;
  const waiter = awaitPendingBrushCommits().then(() => {
    resolved = true;
  });
  ...
  assert.strictEqual(
    resolved,
    false,
    "must not resolve while the Custom Tip GPU commit is still pending",
  );
  ...
  assert.strictEqual(
    resolved,
    true,
    "must resolve once the pending Custom Tip GPU commit settles",
  );
});
...
test("isolated model: waits for both Hard Round and Custom Tip pending commits when both exist", async () => {
  ...
  model.customTipPending = 1;
  model.hardRoundTail = rbHR.promise.then(() => {
    model.hardRoundPending = 0;
  });
  model.customTipTail = rbCT.promise.then(() => {
    model.customTipPending = 0;
  });
  ...
  assert.strictEqual(
    resolved,
    false,
    "must not resolve with only the Custom Tip commit settled while Hard Round is still pending",
  );
...
test("production source: _brushPointerDown checks _hasPendingBrushCommits before pushUndo and stroke init, for ANY tool (including eraser)", () => {
  ...
  const pushUndoCall = body.indexOf(
  ...
  assert.ok(
    toolGate >= 0,
    "tool gate must exist and does not exclude eraser -- the barrier below therefore applies to eraser strokes too",
  );
  assert.ok(
    barrierCheck > toolGate,
    "the pending-commit barrier check must come after the tool gate, so it applies uniformly regardless of which paint tool is starting",
  );
  assert.ok(
    deferCall > barrierCheck && deferCall < pushUndoCall,
  ...
  assert.match(
    body.slice(barrierCheck, deferCall + 60),
    /return;/,
...
test("production source: _hasPendingBrushCommits reflects Custom Tip GPU pending count (not just Hard Round)", () => {
  ...
  assert.match(
    body,
    /pendingCustomTip = typeof _customTipPendingCommitCount === 'number' && _customTipPendingCommitCount > 0/,
  );
...
test("production source: deferred stroke initialization waits on awaitPendingBrushCommits before running pointerdown", () => {
  ...
  const pointerDownCall = body.indexOf(
    "_brushPointerDown(deferred.downEvent);",
    waitCall,
  );
  assert.ok(
    waitCall >= 0 && pointerDownCall > waitCall,
...
test("production source: matching moves are queued (not dropped) while a pointerdown is deferred", () => {
  ...
  assert.match(
    firstLines,
    /if\(_pendingDeferredStroke && e\.pointerId === _pendingDeferredStroke\.pointerId\)\{\s*\n\s*_pendingDeferredStroke\.queuedMoves\.push\(_clonePointerEvent\(e\)\);\s*\n\s*return;/,
...
test("production source: replay order is downEvent -> queuedMoves in order -> upEvent, then deferred state is cleared", () => {
  ...
  assert.ok(
    clearBeforeReplay >= 0 && downCall > clearBeforeReplay,
  ...
  assert.ok(
    forLoop > downCall,
    "the down event must replay before any queued moves",
  );
  assert.ok(
    moveHandle > forLoop,
    "each queued move must be replayed through the normal _handleMoveEvent, preserving array (chronological) order since queuedMoves is a plain push-ordered array",
  );
  assert.ok(
    upCheck > moveHandle,
    "the stored pointerup must only run after all queued moves have replayed",
  );
...
test("production source: a cancelled deferred stroke is not replayed, and queued-move replay checks cancellation per-iteration", () => {
  ...
  assert.match(
    body,
    /for \(const mv of deferred\.queuedMoves\) \{\s*\n\s*if \(deferred\.cancelled\) break;/,
...
test("production source: pointercancel and lostpointercapture clear deferred state safely", () => {
  const cancelStart = engine.indexOf(
    "activeC.addEventListener('pointercancel'",
  );
  const cancelEnd = engine.indexOf(
    "activeC.addEventListener('lostpointercapture'",
    cancelStart,
  );
  const cancelBody = engine.slice(cancelStart, cancelEnd);
  assert.match(
    cancelBody,
    /_pendingDeferredStroke\.cancelled = true;\s*\n\s*_pendingDeferredStroke = null;/,
  ...
  const lostEnd = engine.indexOf(
    "\n\nwindow.CustomBrushAnalyzeResolvedDabs",
    lostStart,
  );
...
test("production source: clearCurrentFrame finishes active drawing, awaits the barrier, THEN pushes undo / clears / saves / recomposites", () => {
  ...
  const end = layersSource.indexOf(
    "\ndocument.getElementById('dd-clear')",
    start,
  );
  ...
  assert.ok(
    finish >= 0 && await1 > finish,
    "must finish any active in-progress drawing before awaiting the commit barrier",
  );
  assert.ok(
    pushUndo > await1,
    "undo snapshot must not be pushed until the barrier (which drains a pending Custom Tip GPU commit) has resolved",
  );
  assert.ok(
    clear > pushUndo,
    "clear must happen only after the undo snapshot exists",
  );
...
test("isolated model: a subsequent destructive action's undo snapshot cannot begin until the barrier (and thus the prior Custom Tip commit) has resolved", async () => {
  ...
  function hasPendingBrushCommits() {
    return customTipPending > 0;
  }
  async function awaitPendingBrushCommits() {
    while (true) {
      await customTipTail;
