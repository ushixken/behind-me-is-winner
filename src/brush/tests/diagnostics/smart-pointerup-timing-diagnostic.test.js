...
test("Smart pointerup timing is default-off, bounded, and adds no readback", () => {
  assert.match(
    brush,
    /typeof window\.HardRoundDebugSmartPointerupTiming==='undefined'/,
  );
  assert.match(
    brush,
  ...
  assert.match(
    brush,
    /length>20\)_hardRoundSmartPointerupTimingRecords\.shift/,
  );
  const diagnostic = brush.slice(
    brush.indexOf("if(typeof window.HardRoundDebugSmartPointerupTiming"),
    brush.indexOf("function _commitFinishedHardRoundStroke"),
  );
...
test("existing Smart operations receive individual timing boundaries", () => {
  for (const field of [
    ...
    assert.match(brush, new RegExp("'" + field + "'"));
...
test("first post-up pen event and cursor apply are correlated", () => {
  assert.match(
    cursor,
    /HardRoundSmartPointerupTimingNote\('pen-event',event\)/,
  );
