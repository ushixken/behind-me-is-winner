...
function pointerUpFlushBranch(pendingQueue, skipFlag, flushPending, log) {
  ...
  if (skipFlag) {
    ...
    const firstPending = pendingCount
      ? Object.assign({}, pendingQueue[0])
      : null;
    const lastPending = pendingCount
      ? Object.assign({}, pendingQueue[pendingCount - 1])
      : null;
...
test("default (false) preserves normal flush: all pending segments dispatched", () => {
  ...
  let dispatched = null;
  const flushed = pointerUpFlushBranch(
    pending,
    false,
    (q) => {
      dispatched = q.splice(0, q.length);
...
test("skip=true: pending segments are NOT dispatched to the renderer", () => {
  ...
  assert.strictEqual(
    flushPendingCalled,
    false,
    "flushPending must never be invoked when skip=true",
  );
