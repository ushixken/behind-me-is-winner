...
function createCompositorEnvironment(width = 200, height = 200) {
  function createCanvas() {
    ...
    const ctx = {
      ...
      clearRect(x, y, w, h) {
        ...
        const x1 = Math.min(
          width,
          clipRect ? Math.min(x + w, clipRect.x + clipRect.w) : x + w,
        );
        const y1 = Math.min(
          height,
          clipRect ? Math.min(y + h, clipRect.y + clipRect.h) : y + h,
        );
      ...
      drawImage(srcCanvas, sx, sy) {
        ...
        const x1 = Math.min(
          width,
          clipRect ? Math.min(sx + width, clipRect.x + clipRect.w) : sx + width,
        );
        const y1 = Math.min(
          height,
          clipRect
            ? Math.min(sy + height, clipRect.y + clipRect.h)
            : sy + height,
        );
        ...
        for (let py = y0; py < y1; py++) {
          for (let px = x0; px < x1; px++) {
            ...
            if (sa <= 0 && globalCompositeOperation !== "destination-out")
              continue;
            this._blendPixel(
              destIdx,
              srcData[srcIdx],
  ...
  const layers = [{ visible: true, opacity: 1.0, frames: [activeC] }];
  let curLayer = 0;
  let curFrame = 0;
...
test("A. dirty-rect recomposite preserves display content outside dirtyRect", () => {
  ...
  assert.deepEqual(
    pUntouched1,
    pUntouched0,
    "Untouched artwork must not change after dirty recomposition elsewhere",
  );
});
...
test("B. semi-transparent artwork outside dirtyRect is not composited twice", () => {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    assert.deepEqual(
      currentPixel,
      initialPixel,
      `Untouched pixel at frame ${i} must match initial brightness`,
    );
  }
});
...
test("C. dirty recomposite result matches full recomposite pixel-for-pixel", () => {
  ...
  assert.deepEqual(
    env1.displayC._data,
    env2.displayC._data,
    "Dirty recomposite must match full recomposite across entire canvas",
  );
});
...
test("D. compC contract is preserved (ends as background + artwork)", () => {
  ...
  assert.deepEqual(
    compPixelInside,
    [200, 50, 50, 255],
    "compC contains artwork inside rect",
  );
  ...
  assert.deepEqual(
    compPixelOutside,
    [255, 255, 255, 255],
    "compC contains background outside rect",
  );
});
...
test("E. onion skin is drawn exactly once and not baked into compC", () => {
  ...
  assert.deepEqual(
    compOnion,
    [255, 255, 255, 255],
    "compC does not contain onion skin",
  );
});
...
test("F. layer opacity < 1 does not darken outside dirtyRect", () => {
  ...
  assert.deepEqual(
    afterDirty,
    baseline,
    "Layer with 50% opacity does not darken when dirty recomposite runs",
  );
});
