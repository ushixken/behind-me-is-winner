...
test("A: _prewarmLiveStrokeComposition no longer calls the real recomposite()", () => {
  ...
  assert.doesNotMatch(
    body,
    /recomposite\(/,
    "prewarm must not route through the shared compositor",
  );
  assert.match(
    body,
    /_getLiveStrokePreview\(\)/,
    "prewarm should still warm the live-preview path directly",
  );
});
...
test("B: prewarm never references the real display/composite surfaces by name", () => {
  ...
  for (const forbidden of ["displayC", "compC", "artworkCompositeC"]) {
    assert.doesNotMatch(
      ...
      `prewarm must not touch ${forbidden}`,
    );
  }
  ...
  assert.doesNotMatch(
    body,
    /activeC\s*=[^=]/,
    "prewarm must not write to activeC",
  );
});
...
test("C: prewarm still exercises the scratch/live-preview allocation path", () => {
  ...
  for (const forbidden of ["displayC", "compC", "artworkCompositeC"]) {
    assert.doesNotMatch(
      preview,
      new RegExp("\\b" + forbidden + "\\b"),
      `_getLiveStrokePreview must not touch ${forbidden}`,
    );
  }
});
