...
async function renderToGrid(w, h, segments) {
  ...
  for (let y = 0; y < h; y++)
    for (let x = 0; x < w; x++)
      grid[y * w + x] = renderer._outCtx._pixel(x, y)[3];
  return grid;
}
...
test("pixelCoveredByCapsuleForStroke: strict at an open stroke end (no half-diagonal dilation)", () => {
  ...
  const d = 0.3 + 0.4;
  const strict = CapsuleMath.pixelCoveredByCapsuleForStroke(
    d,
    0,
    0,
    0,
  ...
  assert.strictEqual(
    strict,
    0,
    "expected the strict tip test to reject a point outside the true radius",
  );
  const conservative = CapsuleMath.pixelCoveredByCapsule(
    d,
    0,
    0,
    0,
  ...
  assert.strictEqual(
    conservative,
    1,
    "sanity check: the old conservative test accepts this same point",
  );
});
...
test("pixelCoveredByCapsuleForStroke: a TAPERING interior cap (shrinking tail) keeps the conservative (dilated) test", () => {
  ...
  const missedByExact = CapsuleMath.capsuleCoverage(
    -0.5,
    0.5,
    0,
    0,
  ...
  const covered = CapsuleMath.pixelCoveredByCapsuleForStroke(
    -0.5,
    0.5,
    0,
    0,
  ...
  assert.strictEqual(
    covered,
    1,
    "expected a tapering cap to still use the conservative sweep test",
  );
});
...
test("pixelCoveredByCapsuleForStroke: a CONSTANT-radius interior joint is NOT dilated (9E.6 -- no periodic bump)", () => {
  ...
  assert.strictEqual(
    covered,
    0,
    "expected a constant-radius joint to use the floored (not dilated) test",
  );
});
...
test("pixelCoveredByCapsuleForStroke: interior segment BODY (away from caps) is NOT dilated (9E.6)", () => {
  ...
  assert.strictEqual(
    covered,
    0,
    "expected the interior body test to no longer dilate by the full half-diagonal",
  );
});
...
test("PrototypeRenderer CPU pipeline: stroke start row is exactly 1 pixel wide", async () => {
  ...
  assert.strictEqual(
    widthOfRow(grid, w, firstInkRow),
    1,
    `expected the very first ink row to be 1px wide, got ${widthOfRow(grid, w, firstInkRow)}`,
  );
});
...
test("PrototypeRenderer CPU pipeline: stroke end row is exactly 1 pixel wide", async () => {
  ...
  assert.strictEqual(
    widthOfRow(grid, w, lastInkRow),
    1,
    `expected the very last ink row to be 1px wide, got ${widthOfRow(grid, w, lastInkRow)}`,
  );
});
...
test("PrototypeRenderer CPU pipeline: stroke body (interior rows) stays connected, no gap rows", async () => {
  ...
  const grid = await renderToGrid(w, h, straightStrokeSegments(10));
  assert.ok(
    hasNoGapRows(grid, w, h),
    "expected the stroke body to have no empty rows within its own span",
  );
});
...
test("PrototypeRenderer CPU pipeline: flick-tail connectivity fix (9E.3) is preserved with tip flags set", async () => {
  ...
  assert.ok(
    hasNoGapRows(grid, w, h),
    "expected the flick tail to still have no gap rows",
  );
});
...
test("PrototypeRenderer CPU pipeline: AA-off output remains strictly binary (0/255)", async () => {
  ...
  for (let i = 0; i < grid.length; i++)
    if (grid[i] !== 0 && grid[i] !== 255) nonBinary.push(grid[i]);
  assert.strictEqual(
    nonBinary.length,
    0,
    `expected only 0/255 alpha, found ${nonBinary.length}: ${nonBinary.slice(0, 10)}`,
  );
});
...
test("PrototypeRenderer CPU pipeline: without tip flags, behavior matches pre-9E.4 (no regression for untagged callers)", async () => {
  ...
  assert.ok(
    widthOfRow(grid, w, firstInkRow) >= 1,
    "untagged callers still paint something (old conservative path)",
  );
});
