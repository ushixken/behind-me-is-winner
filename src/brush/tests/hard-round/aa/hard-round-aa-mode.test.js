...
function edgeTransitionWidth(aaMode) {
  ...
  const step = 0.02;
  let loD = null,
    hiD = null; // distances from the boundary where cov crosses 0.9 / 0.1
  ...
  assert.ok(
    loD !== null && hiD !== null,
    `aaMode=${aaMode}: coverage never crossed both thresholds`,
  );
  return hiD - loD;
}
...
test("capsuleCoverage: Off produces an aliased (near-zero-width) edge transition", () => {
  ...
  assert.ok(
    w < 0.05,
    `expected a near-instant step for Off, got transition width ${w}`,
  );
});
...
test("capsuleCoverage: none is treated the same as Off (aliased)", () => {
  ...
  assert.ok(
    w < 0.05,
    `expected a near-instant step for 'none', got transition width ${w}`,
  );
});
...
test("capsuleCoverage: Weak, Medium, Strong each produce a different (non-aliased) edge width", () => {
  ...
  assert.ok(
    weak > 0.05,
    "weak must still be smoothed (non-aliased), unlike Off",
  );
});
...
test("capsuleCoverage: Strong is smoother/wider than Medium, same angle/geometry", () => {
  ...
  assert.ok(
    strong > medium,
    `expected strong (${strong}) > medium (${medium})`,
  );
});
...
test("capsuleCoverage: Weak matches the original (pre-9E.1) unscaled band -- back-compat", () => {
  ...
  const px = bx + 0.37,
    py = by;
  const withMode = CapsuleMath.capsuleCoverage(
    px,
    py,
    ax,
    ay,
  ...
  assert.ok(
    Math.abs(withMode - noMode) < 1e-9,
    `weak (${withMode}) must equal the unscaled legacy result (${noMode})`,
  );
});
...
test("aaModeScale: monotonic Off(0) < Weak < Medium < Strong", () => {
  ...
  assert.ok(
    weak > off && medium > weak && strong > medium,
...
test("PrototypeRenderer CPU pipeline: Off remains aliased (narrowest possible resolved edge)", async () => {
  ...
  assert.ok(
    offWidth < weakWidth,
    `expected Off's resolved edge (${offWidth}) < Weak's (${weakWidth})`,
  );
});
...
test("PrototypeRenderer CPU pipeline: Weak, Medium, Strong resolve to different coverage widths", async () => {
  ...
  assert.notStrictEqual(
    weak,
    medium,
    "resolved weak and medium widths must differ",
  );
  assert.notStrictEqual(
    medium,
    strong,
    "resolved medium and strong widths must differ",
  );
  assert.notStrictEqual(
    weak,
    strong,
    "resolved weak and strong widths must differ",
  );
});
...
test("PrototypeRenderer CPU pipeline: Strong resolves smoother/wider than Medium (same angle)", async () => {
  ...
  assert.ok(
    strong >= medium,
    `expected resolved strong (${strong}) >= medium (${medium})`,
  );
});
...
test("PrototypeRenderer CPU pipeline: Medium resolves smoother/wider than Weak (same angle)", async () => {
  ...
  assert.ok(
    medium >= weak,
    `expected resolved medium (${medium}) >= weak (${weak})`,
  );
});
...
test("PrototypeRenderer CPU pipeline: capsule geometry (opaque core radius) is unchanged across aaMode", async () => {
  ...
  for (const mode of ["off", "weak", "medium", "strong"]) {
    ...
    assert.ok(
      a >= 250,
      `aaMode=${mode}: capsule interior must remain opaque (got alpha ${a})`,
    );
  }
});
