...
function hasNoGapRows(grid, w, h) {
  ...
  const first = rowHasInk.indexOf(true),
    last = rowHasInk.lastIndexOf(true);
...
function isStrictlyBinary(grid) {
  for (let i = 0; i < grid.length; i++)
    if (grid[i] !== 0 && grid[i] !== 255) return false;
  return true;
}
...
test("lightest-pressure radius is a real subpixel value from the actual adapter path", () => {
  assert.ok(
    R_LIGHT > 0 && R_LIGHT < 0.5,
    `expected a subpixel floor radius, got ${R_LIGHT}`,
  );
});
...
test("shallow-diagonal low-pressure stroke: 1px staircase, no unnecessary adjacent thickness", async () => {
  ...
  for (let x = 6; x < w - 6; x++) {
    ...
    assert.ok(
      height <= 2,
      `column ${x} is ${height}px tall, expected a thin staircase, not uniform fattening`,
    );
  }
  assert.ok(sawInk, "expected the stroke to paint something");
});
...
test("45-degree low-pressure stroke behaves correctly (1px staircase)", async () => {
  ...
  for (let y = 4; y < h - 4; y++) {
    ...
    assert.ok(
      width <= 2,
      `row ${y} is ${width}px wide on a 45-degree stroke, expected a thin staircase`,
    );
  }
  assert.ok(sawInk, "expected the stroke to paint something");
});
...
test("low-pressure stroke has no gap rows", async () => {
  ...
  assert.ok(
    ...
    "expected a continuous stroke with no gap rows",
  );
});
...
test("fast flick tail remains connected at the new floored (not dilated) body test", async () => {
  ...
  assert.ok(
    hasNoGapRows(grid, w, h),
    "expected the flick tail to stay connected (9E.3 preserved)",
  );
});
...
test("stroke start/end remain 1px at the lightest pressure", async () => {
  ...
  assert.ok(
    widthOfRow(grid, w, first) <= 1,
    `expected the start row to be <=1px, got ${widthOfRow(grid, w, first)}`,
  );
  assert.ok(
    widthOfRow(grid, w, last) <= 1,
    `expected the end row to be <=1px, got ${widthOfRow(grid, w, last)}`,
  );
});
...
test("output remains strictly binary (0/255) at the lightest pressure", async () => {
  ...
  const grid = await renderToGrid(w, h, segs);
  assert.ok(
    isStrictlyBinary(grid),
    "expected AA Off to remain strictly binary",
  );
});
...
test("increasing pressure increases width monotonically (1px -> grows beyond 1px)", async () => {
  ...
  for (const f of fractions) {
    ...
    let maxWidth = 0;
    for (let x = 6; x < w - 6; x++)
      maxWidth = Math.max(maxWidth, widthOfCol(grid, w, h, x));
    assert.ok(
      maxWidth >= prevMaxWidth,
      `expected width to grow (or hold) with pressure: ${prevMaxWidth} -> ${maxWidth} at influence ${f}`,
    );
...
test("full pressure still reaches exactly the selected brush size", () => {
  ...
  assert.ok(
    Math.abs(r - BASE_SIZE / 2) < 1e-9,
    `expected full-pressure radius ${BASE_SIZE / 2}, got ${r}`,
  );
});
...
test("Weak/Medium/Strong AA are unaffected by the 9E.6 body-floor change (capsuleCoverage path untouched)", () => {
  ["weak", "medium", "strong"].forEach((mode) => {
    ...
    assert.ok(
      cov > 0 && cov <= 1,
      `expected fractional/positive coverage for ${mode}, got ${cov}`,
    );
  });
});
