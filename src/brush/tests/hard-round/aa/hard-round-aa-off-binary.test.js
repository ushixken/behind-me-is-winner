...
test('capsuleCoverage: Off is exactly binary (0 or 1) at every sample across the edge, including sub-pixel radius', () => {
  ...
  for (let t = -3; t <= 3; t += 0.01) {
    ...
    const cov = CapsuleMath.capsuleCoverage(px, py, ax, ay, r, bx, by, r, 'off');
    if (cov !== 0 && cov !== 1) sawFraction = true;
  }
  assert.ok(!sawFraction, 'expected only 0 or 1 from capsuleCoverage with aaMode="off"');
  ...
  for (let t = -1; t <= 1; t += 0.01) {
    const cov = CapsuleMath.capsuleCoverage(t, 0, 0, 0, tinyR, 0, 0, tinyR, 'off');
    if (cov !== 0 && cov !== 1) sawFractionTiny = true;
  }
  assert.ok(!sawFractionTiny, 'expected only 0 or 1 for a sub-pixel radius with aaMode="off"');
});
...
test('capsuleCoverage: "none" matches "off" exactly, sample for sample', () => {
  ...
  for (let t = -3; t <= 3; t += 0.13) {
    const px = bx + r + t;
    const off = CapsuleMath.capsuleCoverage(px, by, ax, ay, r, bx, by, r, 'off');
    const none = CapsuleMath.capsuleCoverage(px, by, ax, ay, r, bx, by, r, 'none');
...
test('capsuleCoverage: Weak/Medium/Strong still produce fractional coverage (unaffected by 9E.2)', () => {
  ...
  for (const mode of ['weak', 'medium', 'strong']) {
    ...
    for (let t = -3; t <= 3; t += 0.05) {
      const cov = CapsuleMath.capsuleCoverage(bx + r + t, by, ax, ay, r, bx, by, r, mode);
      if (cov > 1e-9 && cov < 1 - 1e-9) sawFraction = true;
    }
    assert.ok(sawFraction, `expected ${mode} to still produce fractional (AA) coverage`);
  }
});
...
test('PrototypeRenderer CPU pipeline: Off resolves every pixel to exactly alpha 0 or 255 (horizontal capsule)', async () => {
  ...
  assert.strictEqual(nonBinary.length, 0, `expected no gray pixels, found ${nonBinary.length}: ${nonBinary.slice(0, 10)}`);
  assert.ok(alphas.some((a) => a === 255), 'expected at least some fully-opaque pixels');
});
...
test('PrototypeRenderer CPU pipeline: Off resolves every pixel to exactly alpha 0 or 255 (37deg diagonal capsule, matches reference image)', async () => {
  const cx = 32, cy = 32, len = 20, angle = 37 * Math.PI / 180;
  const dx = Math.cos(angle) * len, dy = Math.sin(angle) * len;
  const seg = {
    x0: cx - dx, y0: cy - dy, x1: cx + dx, y1: cy + dy, r0: 6, r1: 6,
    alpha0: 1, alpha1: 1, rgb: [0, 0, 0], composite: 'paint',
    hardness: 1, aaMode: 'off',
  };
  const alphas = await resolvedAlphas(seg, 64, 64);
  const nonBinary = alphas.filter((a) => a !== 0 && a !== 255);
  assert.strictEqual(nonBinary.length, 0, `expected no gray stair-step pixels on a diagonal edge, found ${nonBinary.length}: ${nonBinary.slice(0, 10)}`);
});
...
test('PrototypeRenderer CPU pipeline: "none" also resolves to exactly alpha 0 or 255', async () => {
  const seg = {
    x0: 20, y0: 32, x1: 40, y1: 32, r0: 10, r1: 10,
    alpha0: 1, alpha1: 1, rgb: [0, 0, 0], composite: 'paint',
    hardness: 1, aaMode: 'none',
  };
  const alphas = await resolvedAlphas(seg, 64, 64);
  const nonBinary = alphas.filter((a) => a !== 0 && a !== 255);
  assert.strictEqual(nonBinary.length, 0, `expected no gray pixels for aaMode="none", found ${nonBinary.length}`);
});
...
test('PrototypeRenderer CPU pipeline: Weak/Medium/Strong still resolve with gray (AA) edge pixels (unaffected by 9E.2)', async () => {
  for (const mode of ['weak', 'medium', 'strong']) {
    ...
    const alphas = await resolvedAlphas(seg, 64, 64);
    const nonBinary = alphas.filter((a) => a !== 0 && a !== 255);
    assert.ok(nonBinary.length > 0, `expected ${mode} to still resolve gray AA edge pixels`);
  }
});
...
test('PrototypeRenderer CPU pipeline: Off capsule geometry/opaque core is identical to prior aaMode behavior', async () => {
  ...
  assert.strictEqual(a, 255, 'capsule interior must remain fully opaque under Off');
});
