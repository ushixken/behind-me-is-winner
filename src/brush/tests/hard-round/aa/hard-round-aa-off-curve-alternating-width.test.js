...
function arcStroke(cx, cy, radius, startAngle, endAngle, n, r) {
  ...
  let prevX = cx + radius * Math.cos(startAngle),
    prevY = cy + radius * Math.sin(startAngle);
  for (let i = 1; i <= n; i++) {
    const t = i / n;
...
const CURVE_CASES = [
  {
    name: "gentle shallow arc, small brush",
    cx: 50,
    cy: 300,
    radius: 260,
    a0: -1.35,
  ...
  {
    name: "gentle shallow arc, slightly larger brush",
    cx: 50,
    cy: 300,
    radius: 260,
    a0: -1.35,
  ...
  {
    name: "steep arc (near-vertical tangent)",
    cx: 20,
    cy: 160,
    radius: 140,
    a0: 0.05,
...
for (const c of CURVE_CASES) {
  test(`${c.name}: stable width along a real curved stroke, no 1px/2px alternation`, async () => {
    ...
    assert.ok(
      widths.length > 0,
      "expected the curved stroke to paint something",
    );
...
test("curved stroke has no gap rows (connectivity preserved by 9E.8)", async () => {
  ...
  assert.ok(
    ...
    "expected a continuous curved stroke with no gap rows",
  );
});
...
test("sparsely-tessellated (fast flick) curved connectivity is unaffected by 9E.8", async () => {
  ...
  assert.ok(
    hasNoGapRows(grid, w, h),
    "expected the fast-flick tail to stay connected (9E.3 preserved)",
  );
});
...
test("output remains strictly binary (0/255) on a curved stroke", async () => {
  ...
  for (let i = 0; i < grid.length; i++) {
    assert.ok(
      grid[i] === 0 || grid[i] === 255,
      `expected strictly binary output, found ${grid[i]}`,
    );
  }
});
