...
test("pixelCoveredByCapsule: covers a pixel the capsule sweeps through even when its center is outside the radius", () => {
  ...
  assert.strictEqual(
    missedByExact,
    0,
    "sanity check: exact point test should miss this off-center capsule",
  );
  const covered = CapsuleMath.pixelCoveredByCapsule(
    5,
    5,
    0,
    5.6,
  ...
  assert.strictEqual(
    covered,
    1,
    "expected pixelCoveredByCapsule to catch a capsule that grazes this pixel",
  );
});
...
test("PrototypeRenderer CPU pipeline: AA-off fast-flick tapering tail has no gap rows", async () => {
  ...
  assert.ok(
    ...
    "expected the flick tail to have no empty rows within its own span",
  );
});
...
test("PrototypeRenderer CPU pipeline: connectivity fix does not reintroduce fractional/gray alpha", async () => {
  ...
  assert.strictEqual(
    nonBinary.length,
    0,
    `expected only 0/255 alpha, found ${nonBinary.length} gray pixels: ${nonBinary.slice(0, 10)}`,
  );
});
...
test("PrototypeRenderer CPU pipeline: a single round dab (zero-length segment) at a tiny radius is still filled, not dropped", async () => {
  ...
  for (let y = 0; y < h; y++)
    for (let x = 0; x < w; x++)
      if (renderer._outCtx._pixel(x, y)[3] > 0) anyOpaque = true;
  assert.ok(
    anyOpaque,
    "expected a tiny round dab to still paint at least one pixel under AA-off",
  );
});
...
test("PrototypeRenderer CPU pipeline: Off boundary dilation is small (≈1px, not several) vs an exact reference", async () => {
  ...
  async function span(aaMode) {
    ...
    let minX = w,
      maxX = 0;
    for (let x = 0; x < w; x++) {
      if (renderer._outCtx._pixel(x, 30)[3] > 0) {
        if (x < minX) minX = x;
        if (x > maxX) maxX = x;
      }
    }
  ...
  assert.ok(
    offSpan - weakSpan <= 3,
    `expected Off's span (${offSpan}) to be within ~1-2px of Weak's (${weakSpan}), not ballooned`,
  );
});
