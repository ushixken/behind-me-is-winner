...
const engineSource=fs.readFileSync(path.join(__dirname, '..', '..', '..', 'brush-engine.js'),'utf8');

test('the DOM WebGPU context is configured only by the central presenter',()=>{
  assert.equal((rendererSource.match(/^\s*this\.context\.configure\s*\(/gm)||[]).length,1);
  assert.match(rendererSource,/class HardRoundGpuPresenter[\s\S]*?this\.context\.configure\(/);
  assert.match(rendererSource,/HardRoundGpuPresenter\.acquire\(this\.w, this\.h\)/);
  assert.doesNotMatch(rendererSource,/class GpuBackend[\s\S]*?this\.outputContext\.configure\(/);
});

test('pooled GPU backends obtain one shared presenter device and pipeline family',()=>{
  assert.match(rendererSource,/const device = presenter\.device;\s*this\.device = device;/);
  assert.match(rendererSource,/this\.presentPipeline = presenter\.pipelineFor\(this\.ss\)/);
...
test('presentation rejection is explicit and propagated to the live-preview gate',()=>{
  ...
  assert.match(rendererSource,/return \{ presented: true, reason: null, canvas: this\.outputCanvas \}/);
...
test('GPU finishing contexts cannot present or toggle the shared overlay',()=>{
  assert.match(engineSource,/function _hardRoundPresentFinishedFrame[\s\S]*?if\(renderer\.isGpuActive&&renderer\.isGpuActive\(\)\)return Promise\.resolve\(\)/);
...
test('visible artwork above the active layer forces layer-aware CPU preview',()=>{
  ...
  assert.match(engineSource,/!_hardRoundHasVisibleLayerAbove\(curLayer,curFrame\)/);
