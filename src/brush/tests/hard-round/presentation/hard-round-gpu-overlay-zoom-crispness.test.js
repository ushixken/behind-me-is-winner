...
test("core-state.js holds a DOM ref to the Hard Round GPU overlay canvas", () => {
  assert.ok(
    /const hardRoundOverlayC=document\.getElementById\('hard-round-gpu-overlay'\);/.test(
      src,
    ),
  );
});
...
test("applyTransform()'s zoom image-rendering sync includes the overlay canvas alongside its siblings", () => {
  ...
  assert.ok(
    listMatch,
    "applyTransform() must have an image-rendering forEach list",
  );
  ...
  for (const required of [
    ...
    assert.ok(
      ...
      `the image-rendering forEach list must include ${required}`,
    );
  }
});
...
test("no other code path sets image-rendering on the overlay, so applyTransform() is the single source of truth for it", () => {
  ...
  assert.ok(
    !/hard-round-gpu-overlay[\s\S]{0,200}imageRendering/.test(brushEngine),
  );
