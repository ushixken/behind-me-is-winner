...
const toolsColorSource = fs.readFileSync(
  path.join(__dirname, "../../../../ui/tools-color.js"),
  "utf8",
);
...
test("normal commit preserves existing destination pixels and refreshes its key", () => {
  ...
  const end = source.indexOf(
    "function _commitFinishedSmartRasterStroke(",
    start,
  );
  ...
  assert.match(
    body,
    /keyCtx\.clearRect[\s\S]*?keyCtx\.drawImage\(activeC,0,0\)/,
  );
...
test("Smart commit preserves destination accumulation and explicit style ownership commit", () => {
  ...
  const end = source.indexOf(
    "function _hardRoundReleaseFinishingContext(",
    start,
  );
  const body = source.slice(start, end);
  ...
  assert.match(
    body,
    /commitSmartRasterBrushAt\(context\.layerIndex,context\.frameIndex/,
  );
  assert.match(body, /recomposite\(context\.layerIndex,context\.frameIndex\)/);
});
...
test("rapid contexts retain independent results and commit in stroke order", async () => {
  ...
  const enqueue = (strokeId, promise) => {
    const commit = tail.then(() => promise).then(() => order.push(strokeId));
    tail = commit.catch(() => {});
    return commit;
  };
  ...
  const p1 = enqueue(1, first),
    p2 = enqueue(2, Promise.resolve()),
    p3 = enqueue(3, Promise.resolve());
  release();
...
test("pointerup reserves the owned authoritative commit before asynchronous presentation/readback", () => {
  ...
  assert.ok(
    reserve >= 0 &&
      ...
      endStroke > presentation &&
      queue > endStroke,
    { reserve, presentation, endStroke, queue },
  );
  assert.doesNotMatch(
    body,
    /_hrStaleFinalizerMutation[\s\S]*?renderer\.endStroke/,
  );
});

test("mandatory old-stroke commit is independent of shared live scratch state", () => {
  const pointerup = source.indexOf(
...
test("ordered commits capture undo from their explicit layer/frame immediately before paint", () => {
  const normalStart = source.indexOf(
    "function _commitFinishedHardRoundStroke(",
  );
  ...
  assert.ok(
    normal.indexOf("pushUndoAt(context.layerIndex,context.frameIndex)") <
      normal.indexOf("target.drawImage(src,0,0)"),
  );
