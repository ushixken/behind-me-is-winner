...
test("reserve warmup never submits a transparent shared frame while the surface is owned or presented", () => {
  ...
  const suppressed = body.indexOf(
    "sharedPresentTrace('sharedBlankPresentSuppressed'",
    occupied,
  );
  const sharedPresent = body.indexOf(
    "await this.present([0, 0, 0], 'paint', 0, { warmup: true })",
    occupied,
  );
  assert.ok(
    occupied >= 0 && suppressed > occupied && sharedPresent > suppressed,
    { occupied, suppressed, sharedPresent },
  );
...
test("new visible presentation directly supersedes the old bridge without an intermediate blank", () => {
  ...
  const submit = (strokeId, opacity, geometry, purpose) => {
    ...
    if (
      purpose === "prewarm" &&
      (state.owner != null || state.presented != null)
    )
      return "suppressed";
  ...
  assert.equal(state.visible, true);
  assert.equal(
    state.submissions.some((item) => item.blank),
    false,
  );
});
...
test("old finalizer remains unable to alter shared presentation after newer ownership", () => {
  ...
  const presentStart = renderer.indexOf(
    "async present(rgb, composite, opacity, meta)",
  );
  const ownerGuard = renderer.indexOf(
...
test("live preview revalidates ownership immediately before swapchain access and after GPU completion", () => {
  ...
  assert.match(
    engine,
    /strokeId===_activeStrokeSession&&previewGeneration===_hardRoundPreviewGeneration/,
  );
  ...
  const beforeCheck = body.indexOf(
  ...
  assert.ok(
    beforeCheck >= 0 &&
      ...
      texture < submit &&
      submit < afterCheck,
    { beforeCheck, texture, submit, afterCheck },
  );
  assert.match(
    body,
    /if\(!currentAfterWorkDone\)return \{presented:false,reason:'stale-after-work-done'/,
  );
});
...
test("stale GPU completion cannot reveal overlay or update presented stroke id", () => {
  assert.match(
    engine,
    /renderer\.peekStroke\(\{ strokeId: session, previewGeneration, renderer/,
  );
  const guard = engine.indexOf(
    "presentLivePreviewRejectedCount','stale-after-gpu-completion'",
  );
  const presented = engine.indexOf(
    "window.HardRoundOverlayPresentedStrokeId=session",
    guard,
  );
  const reveal = engine.indexOf(
    "_hardRoundSetGpuOverlayVisible(true,'presentLivePreview-accepted-frame')",
    guard,
  );
  assert.ok(guard >= 0 && presented > guard && reveal > presented, {
    guard,
    presented,
    reveal,
  });
...
test("preview-flight ownership diagnostic is default-off and bounded", () => {
  assert.match(
    renderer,
    /HardRoundDebugPreviewFlightOwnership==='undefined'\)window\.HardRoundDebugPreviewFlightOwnership=false/,
  );
  assert.match(renderer, /if\(log\.length>100\)log\.splice/);
  for (const field of [
    "strokeId",
    "previewGeneration",
    "overlayOwnerAtFlightStart",
    "overlayOwnerBeforeSubmit",
    "overlayOwnerAfterWorkDone",
    "presentedStrokeIdBeforeSubmit",
    "presentedStrokeIdAfterWorkDone",
    "submitSequence",
    "acceptedBeforeSubmit",
    "acceptedAfterWorkDone",
    "staleBeforeSubmit",
    "staleAfterWorkDone",
  ])
