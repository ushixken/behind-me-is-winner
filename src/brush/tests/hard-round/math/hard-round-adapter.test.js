...
test("not eligible: custom tip present", () => {
  assert.strictEqual(
    isHardRoundEligible(baseCtx({ hasCustomTip: true })),
    false,
  );
});
...
test("not eligible: size control not pressure-driven", () => {
  assert.strictEqual(
    isHardRoundEligible(baseCtx({ sizeControl: "off" })),
    false,
  );
});
...
test("not eligible: scatter enabled", () => {
  assert.strictEqual(
    isHardRoundEligible(baseCtx({ scatterEnabled: true })),
    false,
  );
});
...
test("not eligible: texture enabled", () => {
  assert.strictEqual(
    isHardRoundEligible(baseCtx({ textureEnabled: true })),
    false,
  );
});
...
test("pressure 0.5 gives a smaller radius than full pressure", () => {
  const rFull = resolveEffectiveRadius({
    baseSize: 28,
    minSizeFrac: 0.05,
    curveKey: "linear",
    influence: pressureInfluence(1),
  });
  const rHalf = resolveEffectiveRadius({
    baseSize: 28,
    minSizeFrac: 0.05,
    curveKey: "linear",
    influence: pressureInfluence(0.5),
  });
  assert.ok(
    rHalf < rFull,
    `expected half-pressure radius (${rHalf}) < full-pressure radius (${rFull})`,
  );
});
...
test("no artificial taper: radius is a pure function of pressure/influence, not stroke position", () => {
  ...
  const a = resolveEffectiveRadius({
    baseSize: 20,
    minSizeFrac: 0.05,
    curveKey: "linear",
    influence: 0.6,
  });
  const b = resolveEffectiveRadius({
    baseSize: 20,
    minSizeFrac: 0.05,
    curveKey: "linear",
    influence: 0.6,
...
test("default (linear) curve uses prototype influence directly, ignoring a stale `pressure` field", () => {
  ...
  const r2 = resolveEffectiveRadius({
    baseSize: 20,
    minSizeFrac: 0,
    curveKey: "linear",
    influence: 0.7,
...
function radiiForFullPressureStroke(baseSize) {
  ...
  for (let i = 1; i < 20; i++) {
    ...
    const segs = core.pushSamples([
      { x: i * 3, y: 0, pressure: 1, pointerType: "pen", timeStamp: t },
    ]);
...
test("decreasing-pressure stroke: resolved radii stay continuous (no jump) between consecutive segments", () => {
  ...
  for (let i = 1; i < 40; i++) {
    ...
    const segs = core.pushSamples([
      { x: i * 3, y: 0, pressure, pointerType: "pen", timeStamp: t },
    ]);
    for (const seg of segs) {
      radii.push(
        resolveEffectiveRadius({
  ...
  for (let i = 1; i < radii.length; i++)
    maxJump = Math.max(maxJump, Math.abs(radii[i] - radii[i - 1]));
  ...
  assert.ok(
    maxJump < 1.5,
    `expected small continuous radius steps, got a jump of ${maxJump}`,
  );
  // And the overall trend must be downward (narrowing tail, §7).
  assert.ok(
    radii[radii.length - 1] < radii[5],
    "expected radius to trend downward as pressure releases",
  );
});
