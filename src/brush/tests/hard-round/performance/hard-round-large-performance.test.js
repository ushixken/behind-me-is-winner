...
test('compatible topmost Hard Round uses direct GPU overlay while lower layers stay layer-aware',()=>{const src=fs.readFileSync(path.join(__dirname, '..', '..', '..', 'brush-engine.js'),'utf8');assert.ok(/new window\.PrototypeRenderer\(\{width:w,height:h,ss:targetSS,preferGpu:true\}\)/.test(src));assert.ok(/const gpuLiveCompatible=[\s\S]*?!_hardRoundHasVisibleLayerAbove\(curLayer,curFrame\)/.test(src));assert.ok(/hardRoundRenderer\.preferGpu=gpuLiveCompatible/.test(src));assert.ok(/presentation\.presented!==true[\s\S]*?_hardRoundSetGpuOverlayVisible\(true,'presentLivePreview-accepted-frame'\)/.test(src));});
...
test('document-space GPU overlay is present in the transformed canvas stack',()=>{const html=fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', '..','index.html'),'utf8'),css=fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', '..','style.css'),'utf8');assert.ok(/id="hard-round-gpu-overlay"/.test(html));
  ...
  assert.ok(/#hard-round-gpu-overlay[^{]*\{[^}]*pointer-events:none;[^}]*z-index:1;[^}]*\}/.test(css));
});
