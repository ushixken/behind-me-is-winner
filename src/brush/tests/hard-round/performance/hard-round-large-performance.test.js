...
test('compatible AA-On topmost Hard Round uses direct GPU overlay while lower layers stay layer-aware',()=>{const src=fs.readFileSync(path.join(__dirname, '..', '..', '..', 'brush-engine.js'),'utf8');assert.ok(/new window\.PrototypeRenderer\(\{width:w,height:h,ss:targetSS,preferGpu:true\}\)/.test(src));assert.ok(/const gpuLiveCompatible=hardRoundAaMode!==['"]off['"]&&hardRoundAaMode!==['"]none['"]/.test(src));assert.ok(/!_hardRoundHasVisibleLayerAbove\(curLayer,curFrame\)/.test(src));assert.ok(/hardRoundRenderer\.preferGpu=gpuLiveCompatible/.test(src));assert.ok(/presentation\.presented!==true[\s\S]*?_hardRoundSetGpuOverlayVisible\(true,'presentLivePreview-accepted-frame'\)/.test(src));});
