...
async function runMigratedStroke({
  w = 64,
  h = 64,
  baseSize = 28,
  n = 12,
  ...
  let t = 8;
  for (let i = 1; i <= n; i++) {
    const segs = core.pushSamples([
      { x: 10 + i * stepPx, y: 10, pressure, pointerType: "pen", timeStamp: t },
    ]);
    stampSegments(segs);
    t += 8;
  }
...
test("Hard Round eligibility: legacy engine's procedural condition matches HardRoundAdapter.isHardRoundEligible", () => {
  ...
  assert.strictEqual(
    HardRoundAdapter.isHardRoundEligible(
      Object.assign({}, eligible, { tool: "eraser" }),
    ),
    false,
  );
  assert.strictEqual(
    HardRoundAdapter.isHardRoundEligible(
      Object.assign({}, eligible, { isPen: false }),
    ),
    false,
  );
  assert.strictEqual(
    HardRoundAdapter.isHardRoundEligible(
      Object.assign({}, eligible, { hasCustomTip: true }),
    ),
    false,
  );
  assert.strictEqual(
    HardRoundAdapter.isHardRoundEligible(
      Object.assign({}, eligible, { hardness: 0.9 }),
    ),
    false,
  );
  assert.strictEqual(
    HardRoundAdapter.isHardRoundEligible(
      Object.assign({}, eligible, { sizeControl: "fixed" }),
    ),
    false,
  );
  assert.strictEqual(
    HardRoundAdapter.isHardRoundEligible(
      Object.assign({}, eligible, { roundness: 0.5 }),
    ),
    false,
  );
  assert.strictEqual(
    HardRoundAdapter.isHardRoundEligible(
      Object.assign({}, eligible, { scatterEnabled: true }),
    ),
    false,
  );
  assert.strictEqual(
    HardRoundAdapter.isHardRoundEligible(
      Object.assign({}, eligible, { textureEnabled: true }),
    ),
    false,
  );
  assert.strictEqual(
    HardRoundAdapter.isHardRoundEligible(
      Object.assign({}, eligible, { airbrush: true }),
    ),
    false,
  );
});
...
test("begin -> drawSegments -> end lifecycle produces a resolved, opaque stroke", async () => {
  const { result } = await runMigratedStroke({
    w: 64,
    h: 64,
    baseSize: 28,
    pressure: 1,
  ...
  assert.ok(
    ...
    "at least one segment must have been drawn",
  );
});
...
test("full-pressure 28px brush remains 28px logical diameter (r ~= 14)", async () => {
  ...
  for (const r of radii) {
    assert.ok(
      Math.abs(r - 14) < 0.5,
      `expected r ~= 14 at full pressure/size 28, got ${r}`,
    );
  }
});
...
test("2px brush remains 2px logical diameter (r ~= 1) at full pressure", async () => {
  ...
  for (const r of radii) {
    assert.ok(
      Math.abs(r - 1) < 0.15,
      `expected r ~= 1 at full pressure/size 2, got ${r}`,
    );
  }
});
...
test("Phase 9C.1: mid-stroke peekStroke shows the stroke before endStroke is ever called (live-preview fix)", async () => {
  ...
  renderer.drawSegments([
    HardRoundAdapter.resolveSegmentRenderParams(beginSeg, adapterOpts),
  ]);
  ...
  assert.ok(
    midStrokePreview.canvas,
    "peekStroke must return a canvas mid-stroke",
  );
  ...
  assert.strictEqual(
    pr,
    200,
    "mid-stroke preview must already show the stroke color",
  );
  assert.ok(
    pa > 240,
    "mid-stroke preview must already be visibly opaque, not transparent",
  );

  // Accumulation must still be live afterward -- more segments, then a
  // second peek, then the real end.
  const moveSegs = core.pushSamples([
    { x: 40, y: 20, pressure: 1, pointerType: "pen", timeStamp: 8 },
  ]);
  renderer.drawSegments(
    moveSegs.map((s) =>
      HardRoundAdapter.resolveSegmentRenderParams(s, adapterOpts),
    ),
  );
  const midStrokePreview2 = await renderer.peekStroke();
  assert.ok(
    midStrokePreview2.segmentCount > midStrokePreview.segmentCount,
    "accumulation must continue after a peek",
  );
  ...
  assert.ok(
    final.canvas,
    "endStroke must still produce the final canvas after live-preview peeks",
  );
});
...
test("cancelStroke produces no committed output (backing store cleared, stroke inactive)", async () => {
  ...
  assert.strictEqual(
    zeroed,
    true,
    "backing store must be cleared after cancelStroke()",
  );
  ...
  assert.strictEqual(
    ...
    0,
    "drawSegments() after cancelStroke() must be ignored",
  );
});
...
test("GPU-ready beginStroke does not allocate or clear the CPU fallback", () => {
  ...
  assert.strictEqual(
    renderer.cpu.coverage,
    null,
    "CPU fallback must be lazy before a CPU stroke needs it",
  );
  ...
  assert.strictEqual(
    ...
    true,
    "prepared renderer must select GPU",
  );
  assert.strictEqual(gpuResetCount, 1, "GPU per-stroke state must reset once");
  assert.strictEqual(
    renderer.cpu.coverage,
    null,
    "GPU begin must not allocate the supersampled CPU coverage buffer",
  );
  renderer.cancelStroke();
});
...
test("CPU fallback storage is allocated on the first actual CPU stroke and reused", () => {
  ...
  assert.ok(
    firstCoverage instanceof Float32Array &&
      firstCoverage.length === 32 * 32 * 16,
  );
  renderer.cancelStroke();
  renderer.beginStroke();
  assert.strictEqual(
    renderer.cpu.coverage,
    firstCoverage,
    "subsequent CPU strokes must reuse fallback storage",
  );
  renderer.cancelStroke();
});
...
test("rapid-stroke finalization permanently session-gates shared mutations", () => {
  ...
  assert.doesNotMatch(
    body,
    /stale&&window\.HardRoundDebugGuardStaleFinalizer/,
    "correctness must not depend on the diagnostic guard",
  );
});
...
test("pointer-up detaches a finishing renderer before async finalization", () => {
  ...
  assert.match(
    ...
    "final pending geometry must flush into the detached owner",
  );
  assert.match(
    src,
    /_hardRoundRendererPool\.includes\(renderer\)/,
    "finished renderer must return to the reusable pool",
  );
});
...
test("migrated Hard Round path does not call _stampDab()", () => {
  ...
  const body = src.slice(start, end);
  assert.ok(
    !/_stampDab\s*\(/.test(body),
    "_hardRoundStampSegments must not call _stampDab()",
  );
  assert.ok(
...
test("Phase 9C.1: pointermove path presents a live preview via peekStroke(), not only at endStroke", () => {
  ...
  assert.ok(
    previewFnStart >= 0,
    "_hardRoundPresentLivePreview not found in brush-engine.js",
  );
  const previewFnEnd = src.indexOf("\n}\n", previewFnStart);
  const previewBody = src.slice(previewFnStart, previewFnEnd);
  assert.ok(
    /renderer\.peekStroke\(\)/.test(previewBody),
    "_hardRoundPresentLivePreview must call PrototypeRenderer.peekStroke() (not endStroke) to avoid ending the stroke",
  );
  assert.ok(
    /_strokeCtx\.drawImage\(/.test(previewBody),
    "_hardRoundPresentLivePreview must draw the peeked canvas into the live _strokeCtx preview surface",
  );
  assert.ok(
    /_scheduleRecomposite\(\)/.test(previewBody),
    "_hardRoundPresentLivePreview must request a recomposite so the preview is actually shown",
  );
});
...
test("post-commit overlay retirement defers until paint opportunity and honors newer-owner continuity", () => {
  ...
  assert.ok(
    /whenRevisionPresented/.test(body),
    "finalization must defer overlay retirement until the committed revision is actually presented",
  );
  assert.ok(
    /older-finalizer-newer-owner/.test(body),
    "immediate check must preserve older-finalizer-newer-owner continuity guard",
  );
  assert.ok(
    /newer-stroke-started-before-paint/.test(body),
    "post-paint check must cancel hide if newer stroke started before paint boundary",
  );
  assert.ok(
    /owned-finalization-retire-committed-overlay/.test(body),
    "overlay must retire via owned-finalization-retire-committed-overlay",
  );
});
