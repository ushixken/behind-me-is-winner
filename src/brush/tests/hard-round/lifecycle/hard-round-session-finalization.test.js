...
test("live GPU presentation rejects a non-owner stroke before swapchain work", () => {
  ...
  const guard = rendererSource.indexOf(
    "!previewFlightCurrent(meta,meta.renderer)",
    start,
  );
  const texture = rendererSource.indexOf(
    "this.outputContext.getCurrentTexture()",
    start,
  );
...
test("Smart Raster context captures style/destination and ordered commit captures ownership before paint", () => {
  ...
  assert.match(
    engine,
    /if\(!context\.ownershipBefore\)[\s\S]*?target\.getImageData/,
  );
...
test("Smart Raster commit API uses explicit layer and frame", () => {
  assert.match(
    panelsSource,
    /function commitSmartRasterBrushAt\(layerIndex,frameIndex/,
  );
  assert.match(
    panelsSource,
    /commitBrushMask\(layerIndex,frameIndex,maskCanvas/,
  );
  assert.match(
    panelsSource,
...
test("deferred Smart Raster commits preserve style and ownership association", async () => {
  ...
  contexts.forEach((context, index) => {
    tail = tail
      .then(() => waits[index].promise)
      .then(() => committed.push(context));
  });
  waits[2].resolve();
  waits[0].resolve();
  await Promise.resolve();
