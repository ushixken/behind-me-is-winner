...
(function () {
  ...
  function refreshSizeUI() {
    const value =
      typeof toolSizes !== "undefined" && toolSizes[tool] != null
        ? toolSizes[tool]
        : 6;
    const text = formatBrushSize(value);
    const tsVal = document.getElementById("ts-size-val");
    if (tsVal && document.activeElement !== tsVal._editingInput)
      tsVal.textContent = text;
  ...
  function applyNewSize(px) {
    ...
    const min = slider ? +slider.min : 0.1,
      max = slider ? +slider.max : 200;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (tsSize)
      tsSize.addEventListener("input", () => applyNewSize(+tsSize.value));
    if (presetSize)
      presetSize.addEventListener("input", () =>
        applyNewSize(+presetSize.value),
      );
    refreshSizeUI();
  });
})();
