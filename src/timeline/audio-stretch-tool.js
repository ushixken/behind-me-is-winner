  ...
  class AudioStretchTool {
    constructor() {
      ...
      this.button.innerHTML =
        '<svg viewBox="0 0 16 16" aria-hidden="true"><path d="M1.5 8h13M1.5 8l3-3M1.5 8l3 3M14.5 8l-3-3M14.5 8l-3 3"/></svg>';
    ...
    setActive(active, announce = true) {
      ...
      if (announce) {
        window.dispatchEvent(
          new CustomEvent("audio-timeline-tool-change", {
    ...
    modelFor(element) {
      return window.AudioClipUI?.clips.find(
        (clip) => clip.id === element?.dataset.audioClipId,
      );
    }
    ...
    clearHover() {
      if (this.hoverClip) {
        this.hoverClip.classList.remove(
          "audio-stretch-hover",
          "audio-stretch-left",
          "audio-stretch-right",
        );
    ...
    onPointerDown(event) {
      ...
      this.drag = {
        ...
        oppositeFrame:
          edge === "left" ? clip.startFrame + clip.duration : clip.startFrame,
    ...
    onDrag = (event) => {
      ...
      if (drag.edge === "right") {
        ...
        drag.startFrame = Math.max(
          0,
          Math.min(drag.oppositeFrame - MIN_DURATION, frame),
        );
        drag.duration = drag.oppositeFrame - drag.startFrame;
      }
      drag.stretchFactor =
        (drag.originalStretch * drag.duration) / drag.originalDuration;
