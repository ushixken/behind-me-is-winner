(() => {
  ...
  function sourceBounds(clip) {
    ...
    const sourceEnd = Number.isFinite(Number(clip?.sourceEnd))
      ...
      : sourceStart +
        Math.max(1, numberOr(clip?.duration, 1)) * playbackRate(clip);
    return {
      start: sourceStart + sourceOffset,
      end: Math.max(sourceStart, sourceEnd) + sourceOffset,
    };
  }

  function timelineFrameToSourceFrame(
    clip,
    timelineFrame,
    clampToTrim = false,
  ) {
    ...
    return clampToTrim
      ? Math.max(bounds.start, Math.min(bounds.end, sourceFrame))
      : sourceFrame;
  }

  function timelineFrameToSourceTime(
    clip,
    timelineFrame,
    frameRate,
    clampToTrim = false,
    ...
    return (
      timelineFrameToSourceFrame(clip, timelineFrame, clampToTrim) /
  ...
  function waveformPixelToTimelineFrame(clip, pixelX, cellWidth) {
    return (
      numberOr(clip?.startFrame) +
      numberOr(pixelX) / Math.max(1, numberOr(cellWidth, 1)) -
      0.5
    );
  }

  function waveformPixelToSourceFrame(clip, pixelX, cellWidth) {
    return timelineFrameToSourceFrame(
      clip,
      waveformPixelToTimelineFrame(clip, pixelX, cellWidth),
      true,
    );
  }
  ...
  function waveformBucketForSourceFrame(clip, sourceFrame, bucketCount) {
    ...
    const sourceTotal = Math.max(
      1,
      numberOr(clip?.sourceDuration),
      numberOr(clip?.sourceEnd) - numberOr(clip?.sourceStart),
    );
    return Math.min(
      count - 1,
      Math.max(0, Math.floor((sourceFrame / sourceTotal) * count)),
    );
  }
  ...
  function diagnoseAlignment(
    ...
    const pixelX =
      (numberOr(timelineFrame) - numberOr(clip?.startFrame)) * width +
      width / 2;
    const scrubSourceFrame = timelineFrameToSourceFrame(
      clip,
      timelineFrame,
      true,
    );
    const waveformSourceFrame = waveformPixelToSourceFrame(
      clip,
      pixelX,
      cellWidth,
    );
    ...
    return {
      ...
      sampleDelta:
        (frameDelta / Math.max(1, numberOr(frameRate, 24))) *
  ...
  window.AudioClipTimeMapping = Object.freeze({
    ...
    timelineFrameToSourceTime,
    waveformPixelToTimelineFrame,
    waveformPixelToSourceFrame,
    waveformBucketForSourceFrame,
    diagnoseAlignment,
  });
})();
