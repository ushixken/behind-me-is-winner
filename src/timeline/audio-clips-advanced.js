(() => {
  ...
  const audioWaveformHeight = () => {
    ...
    const cssValue = root
      ? parseFloat(
          getComputedStyle(root).getPropertyValue("--audio-waveform-height"),
        )
      : NaN;
  ...
  const makeWaveform = (seedText) => {
    ...
    for (let index = 0; index < 320; index++) {
      ...
      const envelope =
        0.18 + 0.82 * Math.pow(Math.sin((Math.PI * index) / 319), 0.35);
  ...
  const normalizeClip = (clip) => {
    ...
    const sourceEnd = Number.isFinite(storedSourceEnd)
      ? Math.max(sourceStart, storedSourceEnd)
      : sourceStart + duration;
    const sourceDuration = Math.max(
      sourceEnd,
      Number(clip.sourceDuration) || sourceEnd,
    );
    const sourceOffset = Math.max(
      -sourceStart,
      Math.min(sourceDuration - sourceEnd, Number(clip.sourceOffset) || 0),
    );
    return {
      id: clip.id,
      ...
      gain: Number.isFinite(clip.gain)
        ? Math.max(MIN_GAIN, Math.min(MAX_GAIN, clip.gain))
        : 0,
      showWaveform: clip.showWaveform !== false,
      stretchFactor:
        Number.isFinite(clip.stretchFactor) && clip.stretchFactor > 0
          ? clip.stretchFactor
          : 1,
      waveform: Array.isArray(clip.waveform)
        ? clip.waveform
        : makeWaveform(clip.name || clip.id || "audio"),
    };
  };
    ...
    select(id, event, clips) {
      if (event.ctrlKey || event.metaKey) {
        ...
        const ordered = [...clips].sort(
          (a, b) =>
            a.startFrame - b.startFrame || a.track.localeCompare(b.track),
        );
        ...
        if (from >= 0 && to >= 0) {
          ...
          ordered
            ...
            .forEach((clip) => this.ids.add(clip.id));
        }
      } else {
        this.ids.clear();
        this.ids.add(id);
  ...
  class WaveformRenderer {
    ...
    envelopeAt(clip, frame) {
      ...
      if (clip.fadeOutLength > 0 && frame > fadeOutStart) {
        const t = Math.max(
          0,
          Math.min(1, (clip.duration - frame) / clip.fadeOutLength),
        );
    ...
    fallbackEnvelope(clip, sourceFrom, sourceTo, sourceTotal) {
      ...
      if (!this.fallbackCache.has(samples))
        this.fallbackCache.set(samples, samples);
      ...
      const fromIndex = Math.max(
        0,
        Math.min(count - 1, Math.floor((from / total) * count)),
      );
      const toIndex = Math.max(
        fromIndex,
        Math.min(count - 1, Math.floor((to / total) * count)),
      );
      let peak = 0;
    ...
    pixelEnvelope(clip, sourceFrom, sourceTo, sourceTotal) {
      const source = clip.sourceId
        ? window.AudioSources?.get(clip.sourceId)
        : null;
      if (
        source &&
        source.pcm &&
        source.pcm.length &&
        window.AudioWaveformEnvelope
      ) {
        const total = Math.max(1, sourceTotal);
        const frameLength = Number(source.frameLength) || 0;
        const scale =
          frameLength > 0 && Number.isFinite(total) ? frameLength / total : 0;
    ...
    draw(canvas, clip) {
      ...
      if (deviceWidth > MAX_CANVAS_DEVICE_DIMENSION)
        deviceWidth = MAX_CANVAS_DEVICE_DIMENSION;
      if (deviceHeight > MAX_CANVAS_DEVICE_DIMENSION)
        deviceHeight = MAX_CANVAS_DEVICE_DIMENSION;
      if (deviceWidth * deviceHeight > MAX_CANVAS_DEVICE_AREA) {
        deviceWidth = Math.max(
          1,
          Math.floor(MAX_CANVAS_DEVICE_AREA / deviceHeight),
        );
      }
      ...
      canvas.style.imageRendering =
        deviceWidth < width * dpr ? "pixelated" : "auto";
      ...
      if (clip.showWaveform) {
        const gainScale =
          clip.gain <= MIN_GAIN ? 0 : Math.pow(10, clip.gain / 20);
        ...
        for (let col = 0; col < columns; col++) {
          ...
          const timelineFrameStart = timeMapping.waveformPixelToTimelineFrame(
            clip,
            x,
            CellW,
          );
          const timelineFrameEnd = timeMapping.waveformPixelToTimelineFrame(
            clip,
            xNext,
            CellW,
          );
          const sourceFrameStart = timeMapping.timelineFrameToSourceFrame(
            clip,
            timelineFrameStart,
            true,
          );
          const sourceFrameEnd = timeMapping.timelineFrameToSourceFrame(
            clip,
            timelineFrameEnd,
            true,
          );
          const sFromRaw =
            Math.min(sourceFrameStart, sourceFrameEnd) - clip.sourceStart;
          const sToRaw =
            Math.max(sourceFrameStart, sourceFrameEnd) - clip.sourceStart;
          ...
          const localFrame = Math.max(
            ...
            Math.min(
              clip.duration,
              (timelineFrameStart + timelineFrameEnd) / 2 - clip.startFrame,
            ),
          );
  ...
  class AudioClipRenderer {
    ...
    render() {
      ...
      this.editor.clips.forEach((clip) => {
        const row = document.querySelector(
          `.audio-timeline-track-grid[data-audio-track-id="${CSS.escape(clip.track)}"]`,
        );
    ...
    createClip(clip) {
      ...
      element.classList.toggle(
        "selected",
        this.editor.selection.ids.has(clip.id),
      );
      ...
      [
        ...
        if (type === "fade-in")
          handle.style.left = `${Math.min(100, (clip.fadeInLength / clip.duration) * 100)}%`;
        if (type === "fade-out")
          handle.style.right = `${Math.min(100, (clip.fadeOutLength / clip.duration) * 100)}%`;
      ...
      element.addEventListener("pointerdown", (event) => {
        ...
        if (edit)
          this.editor.edit.begin(
            event,
            clip.id,
            edit.dataset.audioEdit,
            element,
          );
      ...
      element.addEventListener("contextmenu", (event) =>
        this.editor.openContextMenu(event, clip.id),
      );
      return element;
    }
  }
    ...
    move = (event) => {
      ...
      if (state.type === "trim-left") {
        const delta = Math.max(
          -initial.sourceStart,
          Math.min(initial.duration - 1, frameDelta),
        );
        ...
        const maxDuration = Math.max(
          1,
          (initial.sourceDuration || initial.sourceEnd) - initial.sourceStart,
        );
        const duration = Math.max(
          1,
          Math.min(maxDuration, initial.duration + frameDelta),
        );
        clip.duration = duration;
        clip.sourceEnd = initial.sourceStart + duration;
        clip.sourceDuration = Math.max(
          initial.sourceDuration || initial.sourceEnd,
          clip.sourceEnd,
        );
        ...
        const desired = Math.max(
          ...
          Math.min(
            clip.duration,
            initial.fadeInLength + (event.clientX - state.startX) / CellW,
          ),
        );
        clip.fadeInLength = desired;
        clip.fadeOutLength = Math.min(
          initial.fadeOutLength,
          Math.max(0, clip.duration - desired),
        );
        ...
        const desired = Math.max(
          ...
          Math.min(
            clip.duration,
            initial.fadeOutLength - (event.clientX - state.startX) / CellW,
          ),
        );
        clip.fadeOutLength = desired;
        clip.fadeInLength = Math.min(
          initial.fadeInLength,
          Math.max(0, clip.duration - desired),
        );
        ...
        let gain = Math.max(
          MIN_GAIN,
          Math.min(MAX_GAIN, initial.gain + deltaDb),
        );
  ...
  class AudioClipDragController {
    ...
    begin(event, id, element) {
      ...
      const starts = new Map(
        this.editor.selectedClips().map((item) => [
          item.id,
          {
            frame: item.startFrame,
            trackIndex: Math.max(0, tracks.indexOf(item.track)),
          },
        ]),
      );
      ...
      this.state = {
        ...
        starts,
        before: this.editor.snapshot(),
        startTrack: tracks.indexOf(clip.track),
        startTimelineFrame:
          (event.clientX - rect.left + viewport.scrollLeft) / CellW,
        deltaFrame: 0,
        deltaTrack: 0,
      };
      element.setPointerCapture(event.pointerId);
    ...
    move = (event) => {
      ...
      const frame =
        (event.clientX - rect.left + state.viewport.scrollLeft) / CellW;
      const minFrame = Math.min(
        ...[...state.starts.values()].map((item) => item.frame),
      );
      state.deltaFrame = Math.max(
        -minFrame,
        Math.round(frame - state.startTimelineFrame),
      );
      ...
      state.deltaTrack = Math.max(
        -Math.min(...indices),
        Math.min(
          state.tracks.length - 1 - Math.max(...indices),
          state.deltaTrack,
        ),
      );
      this.editor.selection.ids.forEach((id) => {
        const selected = document.querySelector(
          `.audio-clip[data-audio-clip-id="${CSS.escape(id)}"]`,
        );
        if (selected)
          selected.style.transform = `translate(${state.deltaFrame * CellW}px,${state.deltaTrack * audioWaveformHeight()}px)`;
      });
      event.preventDefault();
    };
    end = (event) => {
  ...
  class AudioClipEditor {
    constructor() {
      ...
      document
        .getElementById("audio-test-clip-add")
        ?.addEventListener("pointerdown", (event) =>
          this.beginTestClipDrag(event),
        );
      document
        .getElementById("audio-timeline-labels")
        .addEventListener("pointerdown", (event) => {
          const label = event.target.closest(".audio-timeline-track-label");
          if (!label || event.target.closest("button,input")) return;
          document
            .querySelectorAll(".audio-timeline-track-label.track-selected")
            .forEach((item) => item.classList.remove("track-selected"));
          label.classList.add("track-selected");
        });
      document
        .getElementById("audio-timeline-grid")
        .addEventListener("pointerdown", (event) => {
    ...
    trackIds() {
      return window.AudioTimelineUI
        ? AudioTimelineUI.tracks.map((track) => track.id)
        : [];
    }
    find(id) {
    ...
    renderSelectionOnly() {
      document.querySelectorAll(".audio-clip").forEach((element) => {
        element.classList.toggle(
          "selected",
          this.selection.ids.has(element.dataset.audioClipId),
        );
      });
    }
    updateClipElement(element, clip) {
      ...
      if (gain)
        gain.title = (clip.gain > 0 ? "+" : "") + clip.gain.toFixed(1) + " dB";
      const fadeIn = element.querySelector(".audio-clip-fade-curve.fade-in");
      if (fadeIn)
        fadeIn.style.width = `${Math.min(100, (clip.fadeInLength / clip.duration) * 100)}%`;
      const fadeOut = element.querySelector(".audio-clip-fade-curve.fade-out");
      if (fadeOut)
        fadeOut.style.width = `${Math.min(100, (clip.fadeOutLength / clip.duration) * 100)}%`;
      const fadeInHandle = element.querySelector(
        ".audio-clip-fade-handle.left",
      );
      if (fadeInHandle)
        fadeInHandle.style.left = `${Math.min(100, (clip.fadeInLength / clip.duration) * 100)}%`;
      const fadeOutHandle = element.querySelector(
        ".audio-clip-fade-handle.right",
      );
      if (fadeOutHandle)
        fadeOutHandle.style.right = `${Math.min(100, (clip.fadeOutLength / clip.duration) * 100)}%`;
    }
    showGainTooltip(x, y, gain) {
      this.gainTooltip.textContent =
        gain <= MIN_GAIN
          ? "-∞ dB"
    ...
    hideGainTooltip() {
      this.gainTooltip.classList.remove("visible");
    }
    ...
    moveTestClipDrag = (event) => {
      ...
      document
        .querySelectorAll(".audio-timeline-track-grid.drop-target")
        .forEach((row) => row.classList.remove("drop-target"));
      const viewport = document.getElementById("audio-timeline-grid-viewport");
      const viewportBounds = viewport.getBoundingClientRect();
      const row = [
        ...document.querySelectorAll(".audio-timeline-track-grid"),
      ].find((candidate) => {
      ...
      state.targetFrame = Math.max(
        0,
        Math.round(
          (event.clientX - viewportBounds.left + viewport.scrollLeft) / CellW,
        ),
      );
    ...
    endTestClipDrag = (event) => {
      ...
      if (!state || event.pointerId !== state.pointerId) return;
      this.testClipDrag = null;
      if (state.button.hasPointerCapture(event.pointerId))
      ...
      document
        .querySelectorAll(".audio-timeline-track-grid.drop-target")
        .forEach((row) => row.classList.remove("drop-target"));
      if (state.moved && state.targetTrack)
        this.addTestClip(state.targetTrack, state.targetFrame);
    };
    addTestClip(track, startFrame) {
      ...
      const clip = normalizeClip({
        ...
        startFrame: Math.max(0, Number(startFrame) || 0),
        duration: DEFAULT_DURATION,
        sourceStart: DEFAULT_DURATION,
        sourceEnd: DEFAULT_DURATION * 2,
        sourceDuration: DEFAULT_DURATION * 3,
      });
    ...
    previewSlip(id, offset) {
      ...
      const previewOffset = Math.max(
        minimum,
        Math.min(maximum, Math.round(offset)),
      );
      const canvas = document.querySelector(
        `.audio-clip[data-audio-clip-id="${CSS.escape(id)}"] .audio-clip-waveform`,
      );
      if (canvas)
        this.renderer.waveforms.draw(canvas, {
          ...clip,
    ...
    stretchClip(id, change) {
      ...
      const nextDuration = Math.max(
        1,
        Math.round(Number(change.duration) || clip.duration),
      );
      ...
      clip.duration = nextDuration;
      clip.stretchFactor =
        Number.isFinite(nextFactor) && nextFactor > 0 ? nextFactor : 1;
      clip.fadeInLength = Math.min(clip.fadeInLength, clip.duration);
      clip.fadeOutLength = Math.min(
        clip.fadeOutLength,
        Math.max(0, clip.duration - clip.fadeInLength),
      );
      this.commit(before);
      this.render();
      return true;
    ...
    splitClip(id, absoluteFrame) {
      ...
      right.fadeOutLength = Math.min(
        original.fadeOutLength,
        Math.max(0, right.duration - right.fadeInLength),
      );
    ...
    deleteSelected() {
      ...
      this.clips = this.clips.filter(
        (clip) => !this.selection.ids.has(clip.id),
      );
      this.selection.clear();
      this.commit(before);
      this.render();
    ...
    openContextMenu(event, id) {
      ...
      waveform.textContent = clip.showWaveform
        ? "Hide Waveform"
        : "Show Waveform";
    ...
    bindContextMenu() {
      document
        .getElementById("audio-clip-ctx-waveform")
      ...
      document
        .getElementById("audio-clip-ctx-rename")
        .addEventListener("click", () => {
          const element = document.querySelector(
            `.audio-clip[data-audio-clip-id="${CSS.escape(this.contextId || "")}"]`,
          );
          this.hideContextMenu();
          if (element) this.renameInline(this.contextId, element);
        });
      document
        .getElementById("audio-clip-ctx-duplicate")
        .addEventListener("click", () => {
          this.hideContextMenu();
          this.duplicateSelected();
        });
      document
        .getElementById("audio-clip-ctx-delete")
        .addEventListener("click", () => {
          this.hideContextMenu();
          this.deleteSelected();
        });
      document
        .getElementById("audio-clip-ctx-properties")
        .addEventListener("click", () => this.hideContextMenu());
    }
    onKeyDown(event) {
