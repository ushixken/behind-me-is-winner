(() => {
    ...
    generate(buffer, count = WAVEFORM_SAMPLES) {
      ...
      for (let bucket = 0; bucket < count; bucket++) {
        ...
        const to =
          bucket === count - 1
            ? buffer.length
            : Math.min(buffer.length, from + framesPerBucket);
        let peak = 0;
        for (let frame = from; frame < to; frame++) {
          let mixed = 0;
          for (const channel of channels)
            mixed += Math.abs(channel[frame] || 0);
  ...
  class AudioImporter {
    ...
    validates(file) {
      ...
      return (
        ACCEPTED_EXTENSIONS.has(extension) || file.type.startsWith("audio/")
      );
    }
    async import(file, onStage) {
      ...
      const pending = (async () => {
        ...
        const source = {
          ...
          pcm: Array.from({ length: buffer.numberOfChannels }, (_, channel) =>
  ...
  class AudioImportDialog {
    constructor(importer) {
      ...
      this.input.accept =
        ".wav,.mp3,.ogg,.oga,.flac,audio/wav,audio/mpeg,audio/ogg,audio/flac";
      ...
      document
        .getElementById("dd-import-audio")
    ...
    audioFiles(dataTransfer) {
      return [...(dataTransfer?.files || [])].filter((file) =>
        this.importer.validates(file),
      );
    }
    onDragEnter(event) {
      if (
        !this.audioFiles(event.dataTransfer).length &&
    ...
    updateDropTrack(clientY) {
      document
        .querySelectorAll(".audio-timeline-track-grid.audio-import-target")
        .forEach((row) => row.classList.remove("audio-import-target"));
    ...
    clearDropState() {
      ...
      document
        .getElementById("img-drop-overlay")
        ?.classList.remove("active", "over-canvas");
      document
        .querySelectorAll(".audio-timeline-track-grid.audio-import-target")
        .forEach((row) => row.classList.remove("audio-import-target"));
    }
    trackAt(clientY) {
      return (
        [...document.querySelectorAll(".audio-timeline-track-grid")].find(
          (row) => {
    ...
    dropPlacement(clientX, clientY) {
      ...
      return {
        ...
        frame: Math.max(
          0,
          Math.round(
            (clientX - bounds.left + this.viewport.scrollLeft) / CellW,
          ),
        ),
      };
    }
