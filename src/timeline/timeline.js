...
function frameLabel(f) {
  return f - frameLabelOffset + 1;
}

function clampRange() {
  rangeStart = Math.max(0, Math.min(rangeStart, TOTAL - 1));
  rangeEnd = Math.max(rangeStart, Math.min(rangeEnd, TOTAL - 1));
}
function getFPS() {
  return Math.min(+fpsTl.value, MAX_FPS);
}
  ...
  if (playing) {
    ...
    playTimer = setInterval(() => {
      ...
      if (next > rangeEnd) {
        if (loopRange) {
          next = rangeStart;
          window.AudioPlaybackEngine?.seek(next);
        } else {
          clearInterval(playTimer);
          playing = false;
          window.AudioPlaybackEngine?.stop();
      ...
      if (typeof window.prepareTransformForArtworkChange === "function")
        window.prepareTransformForArtworkChange(curLayer, next);
      curFrame = next;
      window.TimelineScrubController?.reset(curFrame);
      if (window.CameraSystem && typeof CameraSystem.evaluateAt === "function")
        CameraSystem.evaluateAt(curFrame);
      artworkCompositeCtx.clearRect(0, 0, CW, CH);
      for (let i = 0; i < layers.length; i++) {
        ...
        if (k) {
          const a =
            (layers[i].opacity ?? 1) * _layerGroupChainOpacity(layers[i]);
          artworkCompositeCtx.globalAlpha = a;
          artworkCompositeCtx.drawImage(k, 0, 0);
        }
      }
      ...
      window.dispatchEvent(
        new CustomEvent("active-artwork-changed", {
          detail: { layerIndex: curLayer, frameIndex: curFrame },
        }),
      );
      updatePlayhead();
      document.getElementById("frame-info").textContent =
        frameLabel(curFrame) + " / " + frameLabel(TOTAL - 1);
...
function deleteKeyframe() {
  ...
  if (frameLabelOffset > 0) {
    ...
    if (frameLabelOffset === 0 && curFrame < 0) {
      curFrame = 0;
    }
...
document.getElementById("modal-fps-cancel").onclick = () =>
  document.getElementById("modal-fps").classList.remove("visible");
document.getElementById("modal-fps-ok").onclick = () => {
  let v = Math.max(
    1,
    Math.min(
      parseInt(document.getElementById("fps-direct-input").value) || 12,
      MAX_FPS,
    ),
  );
  fpsTl.value = v;
...
document.getElementById("modal-fps").addEventListener("click", (e) => {
  if (e.target === document.getElementById("modal-fps"))
    document.getElementById("modal-fps").classList.remove("visible");
});
...
tlScroll.addEventListener(
  "scroll",
  () => {
    if (syncingTimelineVerticalScroll) return;
    syncingTimelineVerticalScroll = true;
    tlLabelsRows.scrollTop = tlScroll.scrollTop;
    syncingTimelineVerticalScroll = false;
  },
  { passive: true },
);
tlLabelsRows.addEventListener(
  "scroll",
  () => {
    if (syncingTimelineVerticalScroll) return;
    syncingTimelineVerticalScroll = true;
    tlScroll.scrollTop = tlLabelsRows.scrollTop;
    syncingTimelineVerticalScroll = false;
  },
  { passive: true },
);
tlScroll.addEventListener(
  ...
  (event) => {
    ...
    if (event.shiftKey) tlScroll.scrollLeft += event.deltaY;
    else tlLabelsRows.scrollTop += event.deltaY;
...
function frameFromX(clientX) {
  ...
  return Math.max(
    ...
    Math.min(
      TOTAL - 1,
      Math.floor((clientX - r.left + tlScroll.scrollLeft) / CellW),
    ),
  );
}
...
function _insertFramesAtStart(amount) {
  if (amount <= 0) return;
  layers.forEach((l) => {
    const entries = Object.keys(l.frames)
      .map(Number)
      .sort((a, b) => b - a); // highest first, avoid clobbering
    const shifted = {};
    entries.forEach((f) => {
      shifted[f + amount] = l.frames[f];
    });
    l.frames = shifted;
    // Shift frameMeta in tandem so marks track their drawings
    if (l.frameMeta && Object.keys(l.frameMeta).length) {
      const shiftedMeta = {};
      Object.keys(l.frameMeta).forEach((f) => {
        shiftedMeta[+f + amount] = l.frameMeta[f];
      });
      l.frameMeta = shiftedMeta;
    }
    if (l.indexFrames && Object.keys(l.indexFrames).length) {
      const shiftedIndexFrames = {};
      Object.keys(l.indexFrames).forEach((f) => {
        shiftedIndexFrames[+f + amount] = l.indexFrames[f];
      });
      l.indexFrames = shiftedIndexFrames;
    }
    if (l.indexMeta && Object.keys(l.indexMeta).length) {
      const shiftedIndexMeta = {};
      Object.keys(l.indexMeta).forEach((f) => {
        shiftedIndexMeta[+f + amount] = l.indexMeta[f];
      });
      l.indexMeta = shiftedIndexMeta;
    }
    if (l.smartStyleFrames && Object.keys(l.smartStyleFrames).length) {
      const shiftedSmart = {};
      Object.keys(l.smartStyleFrames).forEach((f) => {
        shiftedSmart[+f + amount] = l.smartStyleFrames[f];
      });
      l.smartStyleFrames = shiftedSmart;
    }
    if (
      window.SmartRasterV4Document &&
      typeof window.SmartRasterV4Document.shiftFrames === "function"
    )
      window.SmartRasterV4Document.shiftFrames(l, amount, 0);
  });
  TOTAL += amount;
  rangeStart += amount;
  rangeEnd += amount;
...
rulerEl.addEventListener("pointerdown", (e) => {
  ...
  if (Math.abs(x - (rangeStart * CellW + CellW / 2)) < 10) {
    draggingStart = true;
    return;
  }
  if (Math.abs(x - (rangeEnd * CellW + CellW / 2)) < 10) {
    draggingEnd = true;
    return;
  }
  scrubbing = true;
  window.TimelineScrubController?.begin(
    "animation",
    e.pointerId,
    frameFromX(e.clientX),
  );
  const f = frameFromX(e.clientX);
  goToFrame(f, false, false, true);
});
...
document.addEventListener("pointermove", (e) => {
  if (scrubbing) {
    const f = frameFromX(e.clientX);
    if (f !== curFrame) goToFrame(f, false, false, true);
  }
  if (draggingStart) {
    rangeStart = Math.min(frameFromX(e.clientX), rangeEnd);
    clampRange();
    renderTimeline();
    updateStatus();
  }
  if (draggingEnd) {
    rangeEnd = Math.max(frameFromX(e.clientX), rangeStart);
    clampRange();
    renderTimeline();
    updateStatus();
  }
...
// RULER CONTEXT MENU
// ════════════════════════════════════════════════════════════════
document.getElementById("ruler-ctx-in").onclick = () => {
  rangeStart = Math.min(rulerCtxFrame, rangeEnd);
  clampRange();
  renderTimeline();
  updateStatus();
  document.getElementById("ruler-ctx-menu").classList.remove("visible");
};
document.getElementById("ruler-ctx-out").onclick = () => {
  rangeEnd = Math.max(rulerCtxFrame, rangeStart);
  clampRange();
  renderTimeline();
  updateStatus();
  document.getElementById("ruler-ctx-menu").classList.remove("visible");
};
document.getElementById("ruler-ctx-reset").onclick = () => {
  rangeStart = 0;
  rangeEnd = TOTAL - 1;
  clampRange();
  renderTimeline();
...
function timelineSelectionInteractionActive() {
  return !!(
    ...
    selectedRowLayers.size > 1 ||
    selectedKFs.size > 1 ||
    selectedTlLabelIndices.size > 1
  );
}
...
function refreshTimelineSelection() {
  ...
  if (!rows) return;
  rows
    .querySelectorAll(".tl-cell.selected")
    .forEach((cell) => cell.classList.remove("selected"));
  if (cameraTrackSelected)
    selectedFrames.forEach((frameIndex) => {
      if (cameraSelectedProperties.size)
        cameraSelectedProperties.forEach((property) => {
          const cell = rows.querySelector(
            '.camera-property-cell[data-camera-property="' +
              property +
              '"][data-camera-frame="' +
              frameIndex +
              '"]',
        ...
        const cell = rows.querySelector(
          '.camera-track-cell[data-camera-frame="' + frameIndex + '"]',
        );
        if (cell) cell.classList.add("selected");
      }
    });
  selectedRowLayers.forEach((layerIndex) =>
    selectedFrames.forEach((frameIndex) => {
      const cell = rows.querySelector(
        '.tl-cell[data-layer-idx="' +
          layerIndex +
          '"][data-frame-idx="' +
          frameIndex +
          '"]',
  ...
  rows
    .querySelectorAll(".tl-cell.cur-col")
    .forEach((cell) => cell.classList.remove("cur-col"));
  if (cameraTrackSelected && cameraSelectedProperties.size)
    cameraSelectedProperties.forEach((property) => {
      const cell = rows.querySelector(
        '.camera-property-cell[data-camera-property="' +
          property +
      ...
      if (cell) cell.classList.add("cur-col");
    });
  else {
    const current = cameraTrackSelected
      ? rows.querySelector(
          '.camera-track-cell[data-camera-frame="' + curFrame + '"]',
        )
      : rows.querySelector(
          '.tl-cell[data-layer-idx="' +
            curLayer +
  ...
  rows
    ...
    .forEach((row) => row.classList.remove("timeline-row-active"));
  if (cameraTrackSelected && cameraSelectedProperties.size)
    cameraSelectedProperties.forEach((property) => {
      const row = rows.querySelector(
        '.tl-camera-property-row[data-camera-property="' + property + '"]',
      );
      if (row) row.classList.add("timeline-row-active");
    });
  else {
    const activeRow = cameraTrackSelected
      ? rows.querySelector(".tl-camera-track-row")
      : rows.querySelector(
          '.tl-layer-track-row[data-layer-idx="' + curLayer + '"]',
        );
    if (activeRow) activeRow.classList.add("timeline-row-active");
  }
  rows
    .querySelectorAll(".kf-block.selected,.camera-kf.selected")
    .forEach((block) => block.classList.remove("selected"));
  selectedKFs.forEach((key) => {
    const block = key.startsWith("camera")
      ? rows.querySelector('.camera-kf[data-kk="' + key + '"]')
      : rows.querySelector('.kf-block[data-kk="' + key + '"]');
    if (block) block.classList.add("selected");
  });
  selectedKFs.forEach((key) => {
    const match = /^camera:(\d+)$/.exec(key);
    if (!match) return;
    rows
      .querySelectorAll(
        '.camera-property-kf[data-camera-frame="' + match[1] + '"]',
      )
      .forEach((marker) => marker.classList.add("selected"));
  });
  document
    .querySelectorAll(".tl-layer-lbl.active")
    .forEach((label) => label.classList.remove("active"));
  const cameraLabel = document.querySelector(".tl-camera-lbl");
  if (cameraLabel)
    cameraLabel.classList.toggle(
      "child-active",
      !!(cameraTrackSelected && cameraSelectedProperties.size),
    );
  if (cameraTrackSelected && cameraSelectedProperties.size)
    cameraSelectedProperties.forEach((property) => {
      const label = document.querySelector(
        '.tl-camera-property-lbl[data-camera-property="' + property + '"]',
      );
      if (label) label.classList.add("active");
    });
  else {
    const activeLabel = cameraTrackSelected
      ? document.querySelector(".tl-camera-lbl")
      : activeGroupId
        ? document.querySelector(
            '.tl-group-lbl[data-gid="' + CSS.escape(activeGroupId) + '"]',
          )
        : document.querySelector('.tl-layer-lbl[data-idx="' + curLayer + '"]');
...
function _rowSpanLayers(anchorLayerIndex, targetLayerIndex) {
  ...
  const a = layerOrder.indexOf(anchorLayerIndex),
    t = layerOrder.indexOf(targetLayerIndex);
...
document.addEventListener("pointerdown", (event) => {
  if (
    event.target.closest(
      "#timeline-area,button,input,select,textarea,[data-timeline-control]",
    )
  )
    return;
...
function selectKeyframeRange(anchor, target) {
  ...
  const firstRow = Math.min(anchorRow, targetRow),
    lastRow = Math.max(anchorRow, targetRow);
  const firstFrame = Math.min(anchor.frameIndex, target.frameIndex),
    lastFrame = Math.max(anchor.frameIndex, target.frameIndex);
  selectedKFs.clear();
  layerOrder.slice(firstRow, lastRow + 1).forEach((layerIndex) => {
    Object.keys(layers[layerIndex].frames)
      .map(Number)
      .filter(
        (frameIndex) => frameIndex >= firstFrame && frameIndex <= lastFrame,
      )
...
function _snapshotFrameMaps(layerIndices) {
  ...
  layerIndices.forEach((layerIndex) => {
    ...
    snapshot[layerIndex] = {
      ...
      v4Snapshot:
        window.SmartRasterV4Document &&
        typeof window.SmartRasterV4Document.captureLayer === "function"
          ? window.SmartRasterV4Document.captureLayer(l)
          : null,
    };
  });
  return snapshot;
...
function _restoreFrameMaps(snapshot) {
  Object.keys(snapshot).forEach((layerIndex) => {
    ...
    if (
      ...
      window.SmartRasterV4Document.restoreLayer(l, s.v4Snapshot);
  });
}
...
function startKFDrag(li, fi, e) {
  ...
  if (typeof li === "string" && li.startsWith("camera-")) {
    const property = li.slice(7),
      draggedKey = _cameraPropertyId(property, fi),
      selectedIds = selectedKFs.has(draggedKey)
        ? Array.from(selectedKFs).filter((key) =>
            /^camera-[^:]+:\d+$/.test(key),
          )
        : [draggedKey],
      before = CameraSystem.trackSnapshot(),
      items = [],
      selectedByProperty = new Map(),
      occupiedByProperty = new Map();
    selectedIds.forEach((id) => {
      const match = /^camera-([^:]+):(\d+)$/.exec(id);
      if (!match) return;
      const itemProperty = match[1],
        frame = Number(match[2]);
      if (!selectedByProperty.has(itemProperty))
        selectedByProperty.set(itemProperty, new Set());
      selectedByProperty.get(itemProperty).add(frame);
      const key = _cameraPropertyKeys(itemProperty).find(
        (candidate) => candidate.frame === frame,
      );
      if (key) items.push({ property: itemProperty, key });
    });
    selectedByProperty.forEach((frames, itemProperty) =>
      occupiedByProperty.set(
        itemProperty,
        new Set(
          _cameraPropertyKeys(itemProperty)
            .filter((key) => !frames.has(key.frame))
            .map((key) => key.frame),
        ),
      ),
    );
    dragKF = {
      ...
      property,
      properties: Array.from(selectedByProperty.keys()),
      li,
      originFi: fi,
      lockedScroll: tlScroll.scrollLeft,
      items,
      occupiedByProperty,
      before,
      appliedDelta: 0,
  ...
  if (li === "camera") {
    const draggedKey = `camera:${fi}`,
      frames = (
        selectedKFs.has(draggedKey) ? Array.from(selectedKFs) : [draggedKey]
      )
        .filter((key) => key.startsWith("camera:"))
        .map((key) => Number(key.slice(7)));
    const selected = new Set(frames),
      before = CameraSystem.trackSnapshot(),
      items = _cameraTrackKeys()
  ...
  const keys = selectedKFs.has(draggedKey)
    ? Array.from(selectedKFs)
    : [draggedKey];
  const items = keys
    .map((key) => {
      ...
      return {
        ...
        meta:
          l && l.frameMeta && l.frameMeta[frameIndex]
            ? Object.assign({}, l.frameMeta[frameIndex])
            : null,
        styleBundle:
          typeof getStyleFrameBundle === "function"
            ? getStyleFrameBundle(layerIndex, frameIndex)
            : null,
        extended:
          typeof getExtendedLayerFrame === "function" &&
          typeof cloneExtendedFrameRecord === "function"
            ? cloneExtendedFrameRecord(
                getExtendedLayerFrame(layerIndex, frameIndex),
              )
            : null,
      };
    })
  ...
  const layerIndices = Array.from(
    new Set(items.map((item) => item.layerIndex)),
  );
  const selectedOrigins = new Set(
    items.map((item) => `${item.layerIndex}:${item.frameIndex}`),
  );
  const occupied = new Set();
  layerIndices.forEach((layerIndex) =>
    Object.keys(layers[layerIndex].frames).forEach((frame) => {
  ...
  dragKF = {
    ...
    appliedDelta: 0,
    anyShift: false,
    before: _snapshotFrameMaps(allLayerIndices),
  };
  document.addEventListener("pointermove", onKFDragMove);
  document.addEventListener("pointerup", onKFDragUp);
}
function onKFDragMove(e) {
  ...
  if (dragKF.kind === "camera-property") {
    const min = Math.min(...dragKF.items.map((item) => item.key.frame)),
      max = Math.max(...dragKF.items.map((item) => item.key.frame)),
      delta = Math.max(
        -min,
        Math.min(TOTAL - 1 - max, rawTarget - dragKF.originFi),
      );
    if (
      delta === dragKF.appliedDelta ||
      dragKF.items.some((item) =>
        dragKF.occupiedByProperty
          .get(item.property)
          ?.has(item.key.frame + delta),
      )
    )
      return;
    dragKF.appliedDelta = delta;
    const keys = dragKF.before.keys.map((key) =>
      JSON.parse(JSON.stringify(key)),
    );
    dragKF.properties.forEach((property) => {
      const origins = new Set(
          dragKF.items
            .filter((item) => item.property === property)
            .map((item) => item.key.frame),
        ),
        fields = _cameraPropertyFields(property);
      keys.forEach((key) => {
        if (origins.has(key.frame))
          fields.forEach((field) => delete key[field]);
      });
    });
    dragKF.items.forEach((item) => {
      const source = item.key,
        fields = _cameraPropertyFields(item.property),
        targetFrame = source.frame + delta;
      let target = keys.find((key) => key.frame === targetFrame);
      if (!target) {
        target = {
          frame: targetFrame,
          interpolation: source.interpolation || "ease",
    ...
    dragKF.items.forEach((item) => {
      const frame = item.key.frame + delta;
      selectedKFs.add(_cameraPropertyId(item.property, frame));
      selectedFrames.add(frame);
    });
    cameraSelectedProperties = new Set(dragKF.properties);
    cameraSelectedProperty =
      dragKF.properties.length === 1 ? dragKF.properties[0] : null;
    curFrame = Math.max(0, Math.min(TOTAL - 1, dragKF.originFi + delta));
    CameraSystem.evaluateAt(curFrame);
    renderTimeline();
    return;
  }
  if (dragKF.kind === "camera") {
    const min = Math.min(...dragKF.items.map((item) => item.frame)),
      max = Math.max(...dragKF.items.map((item) => item.frame)),
      delta = Math.max(
        -min,
        Math.min(TOTAL - 1 - max, rawTarget - dragKF.originFi),
      );
    if (
      delta === dragKF.appliedDelta ||
      dragKF.items.some((item) => dragKF.occupied.has(item.frame + delta))
    )
      return;
    dragKF.appliedDelta = delta;
    const moved = dragKF.items.map((item) =>
        Object.assign({}, item, { frame: item.frame + delta }),
      ),
      origins = new Set(dragKF.items.map((item) => item.frame));
    CameraSystem.replaceTrackKeys(
      dragKF.before.keys.filter((key) => !origins.has(key.frame)).concat(moved),
    );
    selectedKFs.clear();
    selectedFrames.clear();
    moved.forEach((key) => {
      selectedKFs.add(`camera:${key.frame}`);
  ...
  if (wouldBeMin < 0) {
    ...
    dragKF.occupied = new Set(
      Array.from(dragKF.occupied, (key) => {
  ...
  const delta = Math.max(
    -minF,
    Math.min(TOTAL - 1 - maxF, rawTarget - dragKF.originFi),
  );

  if (delta === dragKF.appliedDelta) return;
  const destinations = dragKF.items.map((item) => ({
    item,
    target: item.frameIndex + delta,
  }));
  if (
    destinations.some((move) =>
      dragKF.occupied.has(`${move.item.layerIndex}:${move.target}`),
    )
  )
    return;
  dragKF.items.forEach((item) => {
    const li = item.layerIndex,
      src = item.frameIndex + dragKF.appliedDelta;
    delete layers[li].frames[src];
    if (typeof clearExtendedLayerFrame === "function")
      clearExtendedLayerFrame(li, src);
  ...
  destinations.forEach((move) => {
    ...
    if (move.item.extended && typeof setExtendedLayerFrame === "function")
      setExtendedLayerFrame(
        li,
        move.target,
        move.item.extended.canvas,
        move.item.extended.x,
        move.item.extended.y,
      );
    else if (typeof clearExtendedLayerFrame === "function")
      clearExtendedLayerFrame(li, move.target);
    if (!layers[li].frameMeta) layers[li].frameMeta = {};
    if (move.item.meta)
      layers[li].frameMeta[move.target] = Object.assign({}, move.item.meta);
    else delete layers[li].frameMeta[move.target];
    if (typeof restoreStyleFrameBundle === "function")
      restoreStyleFrameBundle(li, move.target, move.item.styleBundle);
  });
  dragKF.appliedDelta = delta;
  selectedKFs.clear();
  destinations.forEach((move) =>
    selectedKFs.add(`${move.item.layerIndex}:${move.target}`),
  );
  if (dragKF.li === curLayer) {
    curFrame = dragKF.originFi + delta;
    loadFrame(curLayer, curFrame);
  }
...
function _trimLeadingBlanks() {
  ...
  layers.forEach((l) =>
    Object.keys(l.frames).forEach((f) => {
      const n = +f;
      if (n < earliest) earliest = n;
    }),
  );
  ...
  layers.forEach((l) => {
    const shifted = {};
    Object.keys(l.frames).forEach((f) => {
      shifted[+f - trimmable] = l.frames[f];
    });
    l.frames = shifted;
    // Shift frameMeta in tandem so marks track their drawings
    if (l.frameMeta && Object.keys(l.frameMeta).length) {
      const shiftedMeta = {};
      Object.keys(l.frameMeta).forEach((f) => {
        const nf = +f - trimmable;
        if (nf >= 0) shiftedMeta[nf] = l.frameMeta[f];
      });
      l.frameMeta = shiftedMeta;
    }
    if (l.indexFrames && Object.keys(l.indexFrames).length) {
      ...
      Object.keys(l.indexFrames).forEach((f) => {
        const nf = +f - trimmable;
        if (nf >= 0) shiftedIndexFrames[nf] = l.indexFrames[f];
      });
      l.indexFrames = shiftedIndexFrames;
    }
    if (l.indexMeta && Object.keys(l.indexMeta).length) {
      ...
      Object.keys(l.indexMeta).forEach((f) => {
        const nf = +f - trimmable;
        if (nf >= 0) shiftedIndexMeta[nf] = l.indexMeta[f];
      });
      l.indexMeta = shiftedIndexMeta;
    }
    if (l.smartStyleFrames && Object.keys(l.smartStyleFrames).length) {
      ...
      Object.keys(l.smartStyleFrames).forEach((f) => {
        const nf = +f - trimmable;
        if (nf >= 0) shiftedSmart[nf] = l.smartStyleFrames[f];
      });
      l.smartStyleFrames = shiftedSmart;
    }
    if (
      ...
      window.SmartRasterV4Document.shiftFrames(l, -trimmable, 0);
  });
  TOTAL -= trimmable;
  rangeStart -= trimmable;
  rangeEnd -= trimmable;
  curFrame -= trimmable;
  ...
  if (!dragKF) {
    tlScroll.scrollLeft = Math.max(
      0,
      curFrame * CellW - tlScroll.clientWidth / 2,
    );
  }
}
...
function onKFDragUp() {
  ...
  if (
    dragKF &&
    (dragKF.kind === "camera" || dragKF.kind === "camera-property")
  ) {
    if (dragKF.appliedDelta !== 0)
      CameraSystem.commitTrack(dragKF.before, "move");
    ...
    if (frameLabelOffset > 0) _trimLeadingBlanks();
    undoStack.push({
      type: "timeline-frames",
      before: dragKF.before,
      after: _snapshotFrameMaps(dragKF.allLayerIndices),
    });
...
function renderTimeline() {
  const profiler =
      window.BrushLatencyProfiler && window.BrushLatencyProfiler.enabled
        ? window.BrushLatencyProfiler
        : null,
    total = profiler ? performance.now() : 0,
    probe =
      window.FirstDabLatencyProbe && window.FirstDabLatencyProbe.enabled
        ? window.FirstDabLatencyProbe
        : null,
    probeTotal = probe ? performance.now() : 0,
    marksBefore =
      typeof _drawingMarkLookupCount === "number" ? _drawingMarkLookupCount : 0,
    markDurationBefore =
      typeof _drawingMarkLookupDuration === "number"
        ? _drawingMarkLookupDuration
        : 0;
  let stage = probe ? performance.now() : 0;
  renderRuler();
  if (probe) probe.renderTimelineStage("renderTimelineGridDrawing", stage);
  const rowsStart = profiler ? performance.now() : 0;
  stage = probe ? performance.now() : 0;
  renderRows();
  const lookups =
      (typeof _drawingMarkLookupCount === "number"
        ? _drawingMarkLookupCount
        : marksBefore) - marksBefore,
    markDuration =
      (typeof _drawingMarkLookupDuration === "number"
        ? _drawingMarkLookupDuration
        : markDurationBefore) - markDurationBefore;
  if (probe) {
    const rowsDuration = performance.now() - stage;
    probe.renderTimelineStage(
      "renderTimelineDrawingMarks",
      stage,
      Math.min(rowsDuration, Math.max(0, markDuration)),
    );
    probe.renderTimelineStage(
      "renderTimelineFrameDrawing",
      stage,
      Math.max(0, rowsDuration - markDuration),
    );
  }
  if (profiler) {
    profiler.measure("timeline-rows-render", rowsStart, {
      drawingMarkLookups: lookups,
    });
    profiler.recordDuration("drawing-mark-lookup-work", markDuration, {
      lookups,
      updates: 0,
    });
  }
  stage = probe ? performance.now() : 0;
  renderLabelCol();
  if (probe) probe.renderTimelineStage("renderTimelineLayerDrawing", stage);
  stage = probe ? performance.now() : 0;
  updatePlayhead();
  if (probe) probe.renderTimelineStage("renderTimelinePlayhead", stage);
  stage = probe ? performance.now() : 0;
  updateStatus();
  if (probe) probe.renderTimelineStage("renderTimelineTextRendering", stage);
  stage = probe ? performance.now() : 0;
  updateRangeOverlay();
  if (probe)
    probe.renderTimelineStage("renderTimelineSelectionsHighlights", stage);
  stage = probe ? performance.now() : 0;
  updateTlHScroll();
  if (probe)
    probe.renderTimelineStage("renderTimelineScrollbarRendering", stage);
  if (probe) probe.finishRenderTimeline(probeTotal);
...
function tlZoomToSpan(newSpan, anchorFrame, anchorScreenX) {
  ...
  tlScroll.scrollLeft = Math.max(
    0,
    Math.min(maxScroll, anchorFrame * CellW - anchorScreenX),
  );
  renderTimeline();
}
...
(function () {
  ...
  window.updateTlHScroll = function updateTlHScroll() {
    ...
    const leftPx =
      maxLeft > 0 ? Math.round((tlScroll.scrollLeft / maxScroll) * maxLeft) : 0;
  ...
  let dragging = false,
    dragMode = null; // 'center' | 'left' | 'right'
  ...
  function startDrag(e, mode) {
    ...
    window.addEventListener("pointermove", globalDragMove, {
      capture: true,
      passive: false,
    });
  ...
  function applyDrag(e) {
    ...
    if (dragMode === "center") {
      ...
      tlScroll.scrollLeft = Math.max(
        0,
        Math.min(maxScroll, startScrollLeft + (dx / maxLeft) * maxScroll),
      );
      return;
    }
    ...
    if (dragMode === "left") {
      ...
      const newLeftPx = Math.max(
        0,
        Math.min(rightPx0 - MIN_HANDLE_PX, leftPx0 + dx),
      );
      ...
      const newRightPx = Math.min(
        trackW0,
        Math.max(leftPx0 + MIN_HANDLE_PX, rightPx0 + dx),
      );
  ...
  function endDrag(e) {
    ...
    try {
      if (activePointerId != null) thumb.releasePointerCapture(activePointerId);
    } catch (err) {}
    activePointerId = null;
    thumb.classList.remove("tl-thumb-drag");
    thumb.style.cursor = "";
    document.body.style.userSelect = "";
    window.removeEventListener("pointermove", globalDragMove, {
      capture: true,
    });
  ...
  thumb.addEventListener("lostpointercapture", (event) => {
    ...
    try {
      thumb.setPointerCapture(activePointerId);
    } catch (_) {}
  });
  ...
  tlScroll.addEventListener(
    ...
    (e) => {
      ...
      if (e.target && typeof e.target.closest === "function") {
        if (
          e.target.closest("#tl-labels-col") ||
          e.target.closest("#tl-controls") ||
          e.target.closest("#tl-hscroll-row")
        )
          return;
      }
...
(function () {
  ...
  window.addEventListener("keydown", _updateTlScrollTouchAction, {
    capture: true,
  });
  window.addEventListener("keyup", _updateTlScrollTouchAction, {
    capture: true,
  });
  ...
  timelineArea.addEventListener(
    "pointerdown",
    (e) => {
      if (
        e.target.closest("button,input,select,textarea,[data-timeline-control]")
      )
        return;
      ...
      try {
        timelineArea.setPointerCapture(e.pointerId);
      } catch (err) {} // so pen movement continues even outside the Timeline
    },
    { capture: true },
  );
  ...
  timelineArea.addEventListener("pointermove", (e) => {
    ...
    const newFrameWidth = Math.max(
      minFrameWidth,
      Math.min(maxFrameWidth, initialFrameWidth * zoomFactor),
    );
    ...
    tlScroll.scrollLeft = Math.max(
      0,
      Math.min(maxScroll, anchorContentX - pointerLocalX),
    );
  ...
  function endDrag(e) {
    ...
    window.timelineZoomDragActive = false;
    try {
      if (pointerId != null) timelineArea.releasePointerCapture(pointerId);
    } catch (err) {}
    pointerId = null;
    document.body.style.userSelect = "";
...
function _adaptiveRulerInterval() {
  ...
  const ideal = _niceRulerInterval(minimumSpacing / Math.max(0.0001, CellW));
  ...
  if (
    ideal > _rulerMajorInterval &&
    _rulerMajorInterval * CellW >= minimumSpacing * 0.94
  )
    return _rulerMajorInterval;
  if (ideal < _rulerMajorInterval && ideal * CellW < minimumSpacing * 1.15)
    return _rulerMajorInterval;
  return ideal;
}
function _firstAlignedRulerFrame(start, interval) {
  const label = frameLabel(start),
    remainder = ((label % interval) + interval) % interval;
  return start + ((interval - remainder) % interval);
}
function _appendRulerLabel(frame, intervalChanged) {
  const tick = document.createElement("div");
  tick.className =
    "ruler-major-tick" + (intervalChanged ? " ruler-tick-enter" : "");
...
function renderRuler() {
  const nextInterval = _adaptiveRulerInterval(),
    intervalChanged = nextInterval !== _rulerMajorInterval;
  _rulerMajorInterval = nextInterval;
  rulerEl.innerHTML = "";
  const totalW = TOTAL * CellW,
    viewLeft = tlScroll.scrollLeft,
    viewRight = viewLeft + tlScroll.clientWidth;
  ...
  const lastVisible = Math.min(
    TOTAL - 1,
    Math.ceil(viewRight / CellW) + bufferFrames,
  );
  const minorInterval =
    _rulerMajorInterval === 1
      ? 1
      : Math.max(
          1,
          _rulerMajorInterval / (_rulerMajorInterval % 5 === 0 ? 5 : 2),
        );
  ...
  rangeFill.style.left = rangeStart * CellW + "px";
  rangeFill.style.width = (rangeEnd - rangeStart + 1) * CellW + "px";
  rulerEl.appendChild(rangeFill);
  const rangeStartFill = document.createElement("div");
  rangeStartFill.className = "ruler-range-edge ruler-range-start-fill";
  rangeStartFill.style.left = rangeStart * CellW + "px";
  rangeStartFill.style.width = CellW + "px";
  rulerEl.appendChild(rangeStartFill);
  const rangeEndFill = document.createElement("div");
  rangeEndFill.className = "ruler-range-edge ruler-range-end-fill";
  rangeEndFill.style.left = rangeEnd * CellW + "px";
  rangeEndFill.style.width = CellW + "px";
  rulerEl.appendChild(rangeEndFill);
  const currentFill = document.createElement("div");
  currentFill.className = "ruler-current-fill";
  currentFill.style.left = curFrame * CellW + "px";
  currentFill.style.width = CellW + "px";
  rulerEl.appendChild(currentFill);

  const firstAligned = _firstAlignedRulerFrame(
    firstVisible,
    _rulerMajorInterval,
  );
  if (firstVisible === 0 && firstAligned !== 0)
    _appendRulerLabel(0, intervalChanged);
  for (
    let frame = firstAligned;
    frame <= lastVisible;
    frame += _rulerMajorInterval
  )
    _appendRulerLabel(frame, intervalChanged);
  ...
  ["start", "end"].forEach((which) => {
    ...
    line.id = "ruler-" + which + "-line";
    line.className = "range-marker-line";
    const frame = which === "start" ? rangeStart : rangeEnd;
    line.style.left = frame * CellW + CellW / 2 + "px";
    const label = document.createElement("div");
    label.className = "range-marker-label";
    label.id = "ruler-" + which + "-label";
    label.textContent = which === "start" ? "I" : "O";
...
function _scheduleRulerViewportRender() {
  ...
  _rulerRenderRaf = requestAnimationFrame(() => {
    _rulerRenderRaf = 0;
    renderRuler();
  });
}
tlScroll.addEventListener("scroll", _scheduleRulerViewportRender, {
  passive: true,
});

function renderRulerHighlight() {
  rulerEl
    .querySelectorAll(".ruler-current-label")
    .forEach((label) => label.remove());
  const existing = Array.from(
    rulerEl.querySelectorAll(".ruler-major-tick"),
  ).some((label) => Number(label.dataset.frame) === curFrame);
  if (!existing) {
  ...
  if (fill) {
    fill.style.left = curFrame * CellW + "px";
    fill.style.width = CellW + "px";
  }
}
...
function timelineTreeItems() {
  const all = _buildFlatGeneric({
    includeCollapsed: true,
    includeOrphanGroups: true,
  });
  return all.filter((item) => {
    if (
      item.type === "layer" &&
      (!layers[item.idx] || layers[item.idx].onTimeline === false)
    )
      return false;
    const chain =
      item.type === "group"
        ? _groupChain(item.id)
        : _groupChain(layers[item.idx].groupId);
    if (
      chain.some((id) => {
        const group = _groupById(id);
        return group && group.onTimeline === false;
      })
    )
      return false;
...
function _restoreTimelineSoloVisibility() {
  ...
  layers.forEach((item) => {
    if (_timelineSoloState.layerVisibility.has(item))
      item.visible = _timelineSoloState.layerVisibility.get(item);
  });
  groups.forEach((item) => {
    if (_timelineSoloState.groupVisibility.has(item))
      item.visible = _timelineSoloState.groupVisibility.get(item);
  });
}
function _beginTimelineSolo(type, target) {
  const same =
    _timelineSoloState &&
    _timelineSoloState.type === type &&
    _timelineSoloState.target === target;
  if (same) {
    _restoreTimelineSoloVisibility();
    _timelineSoloState = null;
    return;
  }
  if (!_timelineSoloState)
    _timelineSoloState = {
      type,
      target,
      layerVisibility: new Map(layers.map((item) => [item, item.visible])),
      groupVisibility: new Map(groups.map((item) => [item, item.visible])),
    };
  else {
    _restoreTimelineSoloVisibility();
    _timelineSoloState.type = type;
  ...
  if (type === "layer")
    ...
    const ids = _allDescendantGroupIds(target.id);
    layers.forEach((layer) => {
      layer.visible = !!(layer.groupId && ids.has(layer.groupId));
    });
  }
}
function _toggleTimelineLayerSolo(layer) {
  _beginTimelineSolo("layer", layer);
}
function _toggleTimelineGroupSolo(group) {
  _beginTimelineSolo("group", group);
}
function _timelineOnionTrace(stage, detail) {
  if (window.DEBUG_TIMELINE_ONION === true)
    console.debug("[TimelineOnion]", stage, detail || {});
}
function _applyTimelineLayerQuickAction(layer, action) {
  const index = layers.indexOf(layer);
  _timelineOnionTrace("handler", {
    action,
    layerIndex: index,
    layerName: layer && layer.name,
    before: layer && layer.onionSkin,
  });
  if (index < 0) {
    _timelineOnionTrace("invalid-layer", { action });
    return;
  }
  if (action === "lock") {
    if (typeof setLayerLocked === "function")
      setLayerLocked(index, !layer.locked);
    else layer.locked = !layer.locked;
    renderLabelCol();
    renderLayerPanel();
  } else if (action === "onion") {
    const before = layer.onionSkin === true;
    _timelineOnionTrace("4-handler-start", { layerIndex: index });
    _timelineOnionTrace("5-layer-lookup-succeeded", {
      layerIndex: index,
      layerName: layer.name,
    });
    _timelineOnionTrace("6-state-read", {
      layerIndex: index,
      onionSkin: before,
    });
    layer.onionSkin = !before;
    _timelineOnionTrace("7-state-toggled", {
      layerIndex: index,
      before,
      after: layer.onionSkin,
    });
    try {
      updateOnion();
      _timelineOnionTrace("8-renderer-invalidated", { layerIndex: index });
      renderLabelCol();
      renderLayerPanel();
      _timelineOnionTrace("9-timeline-row-refreshed", {
        layerIndex: index,
        onionSkin: layer.onionSkin,
      });
      _timelineOnionTrace("10-viewport-redrawn", { layerIndex: index });
    } catch (error) {
      console.error("[TimelineOnion] toggle failed", {
    ...
    _toggleTimelineLayerSolo(layer);
...
window.addEventListener("project-loaded", () => {
  _timelineSoloState = null;
  timelineCollapsedGroupIds.clear();
  selectedKFs.forEach((key) => {
    if (key.startsWith("camera")) selectedKFs.delete(key);
  });
  cameraTrackSelected = false;
});
...
function _makeTimelineGroupLabel(item) {
  const group = _groupById(item.id);
  if (!group) return null;
  if (group.visible == null) group.visible = true;
  if (group.locked == null) group.locked = false;
  if (group.onionSkin == null) group.onionSkin = false;
  if (group.onTimeline == null) group.onTimeline = true;
  const row = document.createElement("div");
  row.className =
    "tl-layer-lbl tl-group-lbl" +
    (!cameraTrackSelected && activeGroupId === group.id ? " active" : "");
  if (group.color && group.color !== "transparent") {
    ...
    row.style.boxShadow = `inset 3px 0 0 ${group.color}`;
  }
  row.style.height = CellH + "px";
  row.style.setProperty("--tl-tree-depth", item.depth || 0);
  ...
  toggle.textContent = hasChildren
    ? timelineCollapsedGroupIds.has(group.id)
      ? "\u25B6"
      : "\u25BC"
    : "";
  toggle.setAttribute(
    "aria-expanded",
    String(!timelineCollapsedGroupIds.has(group.id)),
  );
  toggle.title = timelineCollapsedGroupIds.has(group.id)
    ? "Expand group"
    : "Collapse group";
  toggle.onclick = (event) => {
    event.stopPropagation();
    if (!hasChildren) return;
    if (timelineCollapsedGroupIds.has(group.id))
      timelineCollapsedGroupIds.delete(group.id);
    else timelineCollapsedGroupIds.add(group.id);
    renderTimeline();
  };
  const folder = document.createElement("span");
  folder.className = "tl-group-folder";
  folder.textContent = "\uD83D\uDCC1";
  ...
  actions.className = "tl-layer-inline-actions";
  const specs = [
    [
      "solo",
      "Solo Group",
      !!(
        _timelineSoloState &&
        _timelineSoloState.type === "group" &&
        _timelineSoloState.target === group
      ),
      '<svg viewBox="0 0 16 16" aria-hidden="true"><path d="M1.5 8s2.3-4 6.5-4 6.5 4 6.5 4-2.3 4-6.5 4-6.5-4-6.5-4Z"></path><circle cx="8" cy="8" r="2"></circle></svg>',
    ],
    [
      "lock",
      "Lock Group",
      group.locked,
      '<svg viewBox="0 0 16 16" aria-hidden="true"><rect x="3.5" y="7" width="9" height="6.5" rx="1.5"></rect><path d="M5.5 7V5a2.5 2.5 0 0 1 5 0v2"></path></svg>',
    ],
    [
      "onion",
      "Group Onion Skin",
      group.onionSkin === true,
      '<svg viewBox="0 0 16 16" aria-hidden="true"><path d="M5.2 10.3A4.2 4.2 0 1 1 10.8 10.3c-.7.5-1 1-1.1 1.7H6.3c-.1-.7-.4-1.2-1.1-1.7Z"></path><path d="M6.4 14h3.2M8 0v1.2M1.7 2.6l.9.8M14.3 2.6l-.9.8"></path></svg>',
    ],
  ];
  specs.forEach(([action, title, enabled, icon]) => {
    const button = document.createElement("button");
    button.type = "button";
    button.className = "tl-layer-quick-btn" + (enabled ? " active" : "");
    ...
    button.onclick = (event) => {
      event.preventDefault();
      event.stopPropagation();
      _applyTimelineGroupQuickAction(group, action);
    };
    actions.appendChild(button);
  });
  const rubberbandZone = document.createElement("span");
  rubberbandZone.className = "tl-lbl-rbzone";
  rubberbandZone.title = "Drag to select multiple layers";
  rubberbandZone.setAttribute("aria-label", "Rubber-band layer selection area");
  rubberbandZone.onpointerdown = (event) => {
    if (event.button !== 0) return;
    event.stopPropagation();
    startTlLabelRubberBand(event);
  };
  ...
  [folder, name].forEach(
    (handle) =>
      (handle.onpointerdown = (event) => {
        ...
        event.preventDefault();
        event.stopPropagation();
        dragTlGroupId = group.id;
        row.classList.add("tl-lbl-dragging");
        document.addEventListener("pointermove", onTlGroupDragMove);
        document.addEventListener("pointerup", onTlGroupDragUp);
      }),
  );
  row.onclick = () => {
    clearCameraTimelineSelection();
...
let cameraSelectedProperties = new Set();
let cameraKeyClipboard = null;
function _cameraTrackExists() {
  return !!(
    window.CameraSystem &&
    CameraSystem.value &&
...
function _cameraTrackKeys() {
  return _cameraTrackExists() && CameraSystem.value.track
    ? CameraSystem.value.track.keys
    : [];
}
function _cameraProperties() {
  return CameraSystem.value.track.positionLinked !== false
    ? ["position", "zoom", "rotation"]
    : ["x", "y", "zoom", "rotation"];
}
function _cameraPropertyLabel(property) {
...
function _cameraPropertyKeys(property) {
  return _cameraTrackExists() ? CameraSystem.propertyKeys(property) : [];
}
function _cameraPropertyFields(property) {
  return property === "position" ? ["x", "y"] : [property];
}
function _cameraPropertyId(property, frame) {
  return "camera-" + property + ":" + frame;
}
function _cameraTimelineRowCount() {
  return _cameraTrackExists()
    ? 1 + (cameraTrackExpanded ? _cameraProperties().length : 0)
    : 0;
}
function _cameraKeyAt(frame) {
  return _cameraTrackKeys().find((key) => key.frame === frame) || null;
}
function _selectedCameraFrames() {
  return Array.from(
    new Set(
      Array.from(selectedKFs)
        .filter((key) => key.startsWith("camera"))
...
function _selectedCameraProperty() {
  if (cameraSelectedProperties.size === 1)
    return Array.from(cameraSelectedProperties)[0];
  return cameraSelectedProperty;
}
function clearCameraTimelineSelection() {
  const hadSelection =
    cameraTrackSelected || _selectedCameraFrames().length > 0;
  cameraTrackSelected = false;
  cameraSelectedProperty = null;
  cameraSelectedProperties.clear();
  selectedKFs.forEach((key) => {
    if (key.startsWith("camera")) selectedKFs.delete(key);
  });
  if (
    kfSelectionAnchor &&
    String(kfSelectionAnchor.trackId || "").startsWith("camera")
  )
    kfSelectionAnchor = null;
  return hadSelection;
}
...
function selectTimelineKey(trackId, frame, event) {
  if (trackId !== "camera") {
    const layerIndex = Number(trackId),
      id = `${layerIndex}:${frame}`;
    cameraTrackSelected = false;
    cameraSelectedProperty = null;
    if (_selectedCameraFrames().length) {
      selectedKFs.clear();
      kfSelectionAnchor = null;
    }
    if (
      ...
      event.shiftKey &&
      kfSelectionAnchor &&
      kfSelectionAnchor.layerIndex != null
    ) {
      selectKeyframeRange(kfSelectionAnchor, { layerIndex, frameIndex: frame });
      selectedRowLayers = new Set(
        _rowSpanLayers(kfSelectionAnchor.layerIndex, layerIndex),
      );
    } else if (event && (event.ctrlKey || event.metaKey)) {
      if (selectedKFs.has(id)) selectedKFs.delete(id);
      else selectedKFs.add(id);
      selectedRowLayers.add(layerIndex);
      kfSelectionAnchor = { layerIndex, frameIndex: frame };
    } else {
      if (!selectedKFs.has(id)) {
        selectedKFs.clear();
        selectedKFs.add(id);
      }
      selectedRowLayers = new Set([layerIndex]);
      kfSelectionAnchor = { layerIndex, frameIndex: frame };
    }
    return;
  }
  const frames = _cameraTrackKeys()
      .map((key) => key.frame)
      .sort((a, b) => a - b),
    id = `camera:${frame}`;
  frame = Number(frame);
  cameraTrackSelected = true;
  cameraSelectedProperty = null;
  cameraSelectedProperties.clear();
  selectedRowLayers.clear();
  if (
    ...
    event.shiftKey &&
    kfSelectionAnchor &&
    kfSelectionAnchor.trackId === "camera"
  ) {
    const lo = Math.min(kfSelectionAnchor.frameIndex, frame),
      hi = Math.max(kfSelectionAnchor.frameIndex, frame);
    selectedKFs.clear();
    frames
      .filter((value) => value >= lo && value <= hi)
      .forEach((value) => selectedKFs.add(`camera:${value}`));
  } else if (event && (event.ctrlKey || event.metaKey)) {
    if (selectedKFs.has(id)) selectedKFs.delete(id);
    else selectedKFs.add(id);
...
function _addOrUpdateCameraKey() {
  if (!_cameraTrackExists()) return;
  CameraSystem.addOrUpdateKey(curFrame);
  selectedKFs.clear();
  selectedKFs.add(`camera:${curFrame}`);
  selectedFrames.clear();
  selectedFrames.add(curFrame);
  kfSelectionAnchor = { trackId: "camera", frameIndex: curFrame };
  cameraTrackSelected = true;
  renderTimeline();
}
function _deleteSelectedCameraKeys() {
  const frames = _selectedCameraFrames();
  if (!window.CameraSystem || !frames.length) return false;
  const property = _selectedCameraProperty();
  if (property) CameraSystem.deletePropertyKeys(property, frames);
  else CameraSystem.deleteKeys(frames);
  selectedKFs.clear();
  selectedFrames.clear();
  selectedFrames.add(curFrame);
  kfSelectionAnchor = null;
...
function _copySelectedCameraKeys() {
  const selected = new Set(_selectedCameraFrames()),
    property = _selectedCameraProperty(),
    keys = (
      property ? _cameraPropertyKeys(property) : _cameraTrackKeys()
    ).filter((key) => selected.has(key.frame));
  if (!keys.length) return false;
  const first = Math.min(...keys.map((key) => key.frame)),
    fields = property
      ? _cameraPropertyFields(property)
      : ["x", "y", "zoom", "rotation"];
  cameraKeyClipboard = keys.map((key) => {
    const copy = {
      offset: key.frame - first,
      property,
      interpolation: key.interpolation || "linear",
      bezier: key.bezier ? JSON.parse(JSON.stringify(key.bezier)) : null,
    };
    fields.forEach((field) => {
...
function _pasteCameraKeys(frame) {
  if (!window.CameraSystem || !cameraKeyClipboard || !cameraKeyClipboard.length)
    return false;
  const before = CameraSystem.trackSnapshot(),
    property = cameraKeyClipboard[0].property || null,
    fields = property
      ? _cameraPropertyFields(property)
      : ["x", "y", "zoom", "rotation"],
    keys = before.keys.map((key) => JSON.parse(JSON.stringify(key))),
    added = [];
  cameraKeyClipboard.forEach((source) => {
    const targetFrame = Math.min(TOTAL - 1, Math.max(0, frame + source.offset));
    let target = keys.find((key) => key.frame === targetFrame);
    if (!target) {
      target = {
        frame: targetFrame,
    ...
    added.push({ frame: targetFrame });
  });
  CameraSystem.replaceTrackKeys(keys, "paste", before);
  selectedKFs.clear();
  added.forEach((key) =>
    selectedKFs.add(
      property ? _cameraPropertyId(property, key.frame) : `camera:${key.frame}`,
    ),
  );
  selectedFrames.clear();
  added.forEach((key) => selectedFrames.add(key.frame));
  cameraTrackSelected = true;
  cameraSelectedProperty = property;
  return true;
}
function _duplicateCameraKeys() {
  if (!_copySelectedCameraKeys()) return false;
  const frames = _selectedCameraFrames(),
    property = cameraKeyClipboard[0].property || null,
    target = Math.min(TOTAL - 1, Math.min(...frames) + 1),
    targets = cameraKeyClipboard.map((key) => target + key.offset),
    occupied = new Set(
      (property ? _cameraPropertyKeys(property) : _cameraTrackKeys()).map(
        (key) => key.frame,
      ),
    );
  if (targets.some((frame) => frame >= TOTAL || occupied.has(frame)))
    return false;
  return _pasteCameraKeys(target);
}
function _cameraShortcut(action) {
  if (!cameraTrackSelected && !_selectedCameraFrames().length) return false;
  if (action === "copy") return _copySelectedCameraKeys();
  if (action === "cut") {
    if (!_copySelectedCameraKeys()) return false;
    return _deleteSelectedCameraKeys();
  }
  if (action === "paste") return _pasteCameraKeys(curFrame);
  if (action === "duplicate") return _duplicateCameraKeys();
  if (action === "delete") return _deleteSelectedCameraKeys();
  return false;
}
function _selectCameraTrack(selectCurrentKey) {
  if (!_cameraTrackExists()) return;
  cameraTrackSelected = true;
  cameraSelectedProperty = null;
  cameraSelectedProperties.clear();
  ...
  selectedRowLayers.clear();
  kfSelectionAnchor = null;
  if (selectCurrentKey && _cameraKeyAt(curFrame)) {
    selectedKFs.add(`camera:${curFrame}`);
    kfSelectionAnchor = { trackId: "camera", frameIndex: curFrame };
  }
  renderTimeline();
}
function _selectAllCameraTrackKeys() {
  if (!_cameraTrackExists()) return false;
  const keys = _cameraTrackKeys();
  cameraTrackSelected = true;
  cameraSelectedProperty = null;
  cameraSelectedProperties.clear();
  selectedKFs.clear();
  ...
  kfSelectionAnchor = keys.length
    ? { trackId: "camera", frameIndex: keys[0].frame }
    : null;
  renderTimeline();
  return true;
}
function _selectCameraProperties(properties) {
  if (!_cameraTrackExists()) return false;
  const valid = _cameraProperties().filter((property) =>
    properties.includes(property),
  );
  if (!valid.length) return false;
  cameraTrackExpanded = true;
  cameraTrackSelected = true;
  cameraSelectedProperties = new Set(valid);
  cameraSelectedProperty = valid.length === 1 ? valid[0] : null;
  selectedKFs.clear();
  selectedFrames.clear();
  selectedRowLayers.clear();
  valid.forEach((property) =>
    _cameraPropertyKeys(property).forEach((key) => {
      selectedKFs.add(_cameraPropertyId(property, key.frame));
      selectedFrames.add(key.frame);
    }),
  );
  const firstFrame = Math.min(...Array.from(selectedFrames));
  kfSelectionAnchor = Number.isFinite(firstFrame)
    ? { trackId: "camera-" + valid[0], frameIndex: firstFrame }
    : null;
  renderTimeline();
  return true;
}
function _selectAllCameraPropertyKeys(property) {
  return _selectCameraProperties([property]);
}
function _activateCameraFromTimeline() {
  if (tool !== "camera") setTool("camera", "Camera");
  _selectCameraTrack(true);
}
async function _removeCameraTrack() {
  if (!_cameraTrackExists()) return;
  const keyCount = _cameraTrackKeys().length,
    confirmed = await siteConfirm(
      keyCount
        ? "Remove Camera and delete all " +
            keyCount +
            " Camera keyframe" +
            (keyCount === 1 ? "" : "s") +
            "?"
        : "Remove Camera from this document?",
      { title: "Remove Camera", okText: "Remove Camera", danger: true },
    );
  if (!confirmed) return;
  CameraSystem.remove();
  clearCameraTimelineSelection();
...
function _makeCameraTrackLabel() {
  const row = document.createElement("div");
  row.className =
    "tl-layer-lbl tl-camera-lbl" +
    (cameraSelectedProperties.size
      ? " child-active"
      : cameraTrackSelected
        ? " active"
        : "");
  ...
  disclosure.className = "tl-camera-disclosure";
  disclosure.textContent = cameraTrackExpanded ? "▼" : "▶";
  disclosure.title = cameraTrackExpanded
    ? "Collapse Camera properties"
    : "Expand Camera properties";
  disclosure.setAttribute("aria-expanded", String(cameraTrackExpanded));
  disclosure.onpointerdown = (event) => {
    event.preventDefault();
    event.stopPropagation();
  };
  disclosure.onclick = (event) => {
    event.preventDefault();
    event.stopPropagation();
    cameraTrackExpanded = !cameraTrackExpanded;
    if (!cameraTrackExpanded) {
      cameraSelectedProperty = null;
      cameraSelectedProperties.clear();
      selectedKFs.forEach((key) => {
        if (/^camera-[^:]+:/.test(key)) selectedKFs.delete(key);
      });
    }
    renderTimeline();
  };
  const name = document.createElement("span");
  name.className = "tl-lbl-name";
  name.textContent = "Camera";
  const actions = document.createElement("span");
  actions.className = "tl-layer-inline-actions";
  const add = document.createElement("button");
  add.type = "button";
  add.className = "tl-layer-quick-btn";
  add.textContent = "+";
  add.title = "Add or Update Camera Key";
  add.setAttribute("aria-label", add.title);
  add.dataset.timelineControl = "camera-key";
  ...
  add.onclick = (event) => {
    event.preventDefault();
    event.stopPropagation();
    _addOrUpdateCameraKey();
  };
  const separateKey = document.createElement("button");
  separateKey.type = "button";
  separateKey.className =
    "tl-layer-quick-btn tl-camera-separate-key" +
    (CameraSystem.separateKey ? " separate-key-on" : "");
  separateKey.innerHTML =
    '<svg viewBox="0 0 16 16" aria-hidden="true"><path d="M4 3.5 6.5 6 4 8.5 1.5 6 4 3.5Zm8 4L14.5 10 12 12.5 9.5 10 12 7.5Z"></path></svg>';
  separateKey.title = CameraSystem.separateKey
    ? "Separate Key: only edited properties are keyed"
    : "Separate Key off: all Camera transforms are keyed";
  separateKey.setAttribute("aria-label", separateKey.title);
  separateKey.setAttribute("aria-pressed", String(CameraSystem.separateKey));
  separateKey.dataset.timelineControl = "camera-separate-key";
  separateKey.onpointerdown = (event) => {
    event.preventDefault();
    event.stopPropagation();
  };
  separateKey.onclick = (event) => {
    event.preventDefault();
    event.stopPropagation();
    const enabled = CameraSystem.setSeparateKey(!CameraSystem.separateKey);
    separateKey.classList.toggle("separate-key-on", enabled);
    separateKey.setAttribute("aria-pressed", String(enabled));
    separateKey.title = enabled
      ? "Separate Key: only edited properties are keyed"
      : "Separate Key off: all Camera transforms are keyed";
    separateKey.setAttribute("aria-label", separateKey.title);
  };
  ...
  remove.type = "button";
  remove.className = "tl-layer-quick-btn tl-camera-remove-btn";
  remove.innerHTML =
    '<svg viewBox="0 0 16 16" aria-hidden="true"><path d="M3.5 4.5h9M6 4.5V3h4v1.5M5 6.5v6M8 6.5v6M11 6.5v6M4.5 4.5l.5 9h6l.5-9"></path></svg>';
  remove.title = "Remove Camera";
  remove.setAttribute("aria-label", remove.title);
  remove.dataset.timelineControl = "remove-camera";
  ...
  actions.append(add, separateKey, remove);
  row.append(disclosure, name, actions);
  row.onclick = _activateCameraFromTimeline;
  return row;
}
function _addOrUpdateCameraPropertyKey(property) {
  if (!_cameraTrackExists()) return;
  CameraSystem.addOrUpdatePropertyKey(property, curFrame);
  cameraTrackSelected = true;
  cameraSelectedProperty = property;
  cameraSelectedProperties = new Set([property]);
  selectedKFs.clear();
  selectedKFs.add(_cameraPropertyId(property, curFrame));
  selectedFrames.clear();
  selectedFrames.add(curFrame);
  kfSelectionAnchor = { trackId: "camera-" + property, frameIndex: curFrame };
  renderTimeline();
}

function _makeCameraPropertyLabel(property) {
  const row = document.createElement("div");
  row.className =
    "tl-layer-lbl tl-camera-property-lbl" +
    (cameraSelectedProperties.has(property) ? " active" : "");
  row.style.height = CellH + "px";
  row.dataset.cameraProperty = property;
  const spacer = document.createElement("span");
  spacer.className = "tl-camera-property-indent";
  const name = document.createElement("span");
  name.className = "tl-lbl-name";
  name.textContent = _cameraPropertyLabel(property);
  const actions = document.createElement("span");
  actions.className = "tl-layer-inline-actions";
  const key = document.createElement("button");
  key.type = "button";
  key.className =
    "tl-layer-quick-btn tl-camera-property-key-btn" +
    (_cameraPropertyKeys(property).some((item) => item.frame === curFrame)
      ? " active"
      : "");
  key.textContent = "◆";
  key.title = "Add or Update " + _cameraPropertyLabel(property) + " Key";
  key.setAttribute("aria-label", key.title);
  key.dataset.timelineControl = "camera-property-key";
  ...
  key.onclick = (event) => {
    event.preventDefault();
    event.stopPropagation();
    _addOrUpdateCameraPropertyKey(property);
  };
  actions.appendChild(key);
  row.append(spacer, name, actions);
  row.onclick = (event) => {
    if (tool !== "camera") setTool("camera", "Camera");
    const properties = new Set(cameraSelectedProperties);
    ...
    if (!properties.size) properties.add(property);
    _selectCameraProperties(Array.from(properties));
  };
  return row;
}
window.CameraTimeline = {
  exists: _cameraTrackExists,
  remove: _removeCameraTrack,
  handleShortcut: _cameraShortcut,
  addOrUpdateKey: _addOrUpdateCameraKey,
  deleteSelected: _deleteSelectedCameraKeys,
  selectTrack: _selectCameraTrack,
  selectAllTrack: _selectAllCameraTrackKeys,
  selectProperty: _selectAllCameraPropertyKeys,
  getSelectedFrames() {
    return _selectedCameraFrames();
  },
  get selectedProperty() {
    return cameraSelectedProperty;
  },
  get selectedProperties() {
    return Array.from(cameraSelectedProperties);
  },
  get selected() {
    return cameraTrackSelected || _selectedCameraFrames().length > 0;
  },
};
function renderLabelCol() {
  const el = document.getElementById("tl-labels-rows");
  el.innerHTML = "";
  if (_cameraTrackExists()) {
    const properties = _cameraProperties(),
      valid = new Set(properties);
    cameraSelectedProperties = new Set(
      Array.from(cameraSelectedProperties).filter((property) =>
        valid.has(property),
      ),
    );
    cameraSelectedProperty =
      cameraSelectedProperties.size === 1
        ? Array.from(cameraSelectedProperties)[0]
        : null;
    selectedKFs.forEach((key) => {
      const match = /^camera-([^:]+):/.exec(key);
      if (match && !valid.has(match[1])) selectedKFs.delete(key);
    });
    el.appendChild(_makeCameraTrackLabel());
    if (cameraTrackExpanded)
      properties.forEach((property) =>
        el.appendChild(_makeCameraPropertyLabel(property)),
      );
  } else clearCameraTimelineSelection();
  const visibleItems = timelineTreeItems();
  const visibleIndices = visibleItems
    .filter((item) => item.type === "layer")
    .map((item) => item.idx);
  visibleItems.forEach((item) => {
    if (item.type === "group") {
      const groupRow = _makeTimelineGroupLabel(item);
      if (groupRow) el.appendChild(groupRow);
      return;
    }
    const i = item.idx,
      l = layers[i];
    ...
    lbl.className =
      "tl-layer-lbl" +
      (!cameraTrackSelected && !activeGroupId && i === curLayer
        ? " active"
        : isMultiSel
          ? " multi-sel"
          : "");
    ...
    lbl.dataset.idx = i;
    lbl.title =
      "Click to select. Shift+click range, Ctrl+click individual. Drag name to Hide zone to remove from timeline. Drag empty space to rubber-band select.";
    lbl.innerHTML =
      '<span class="tl-lbl-draghandle" draggable="true"><span class="tl-lbl-name">' +
      l.name +
      '</span></span><span class="tl-layer-inline-actions"><button type="button" class="tl-layer-quick-btn' +
      (_timelineSoloState &&
      _timelineSoloState.type === "layer" &&
      _timelineSoloState.target === l
        ? " active"
        : "") +
      '" data-action="solo" title="Solo Mode" aria-label="Solo Mode" aria-pressed="' +
      String(
        !!(
          _timelineSoloState &&
          _timelineSoloState.type === "layer" &&
      ...
      '"><svg viewBox="0 0 16 16" aria-hidden="true"><path d="M1.5 8s2.3-4 6.5-4 6.5 4 6.5 4-2.3 4-6.5 4-6.5-4-6.5-4Z"></path><circle cx="8" cy="8" r="2"></circle></svg></button><button type="button" class="tl-layer-quick-btn' +
      (l.locked ? " active" : "") +
      '" data-action="lock" title="Lock Layer" aria-label="Lock Layer" aria-pressed="' +
      String(!!l.locked) +
      '"><svg viewBox="0 0 16 16" aria-hidden="true"><rect x="3.5" y="7" width="9" height="6.5" rx="1.5"></rect><path d="M5.5 7V5a2.5 2.5 0 0 1 5 0v2"></path></svg></button><button type="button" class="tl-layer-quick-btn timeline-onion-toggle' +
      (l.onionSkin === true ? " active" : "") +
      '" data-action="onion" data-timeline-control="onion-skin" title="Onion Skin" aria-label="Onion Skin" aria-pressed="' +
      String(l.onionSkin === true) +
      '"><svg viewBox="0 0 16 16" aria-hidden="true"><path d="M5.2 10.3A4.2 4.2 0 1 1 10.8 10.3c-.7.5-1 1-1.1 1.7H6.3c-.1-.7-.4-1.2-1.1-1.7Z"></path><path d="M6.4 14h3.2M8 0v1.2M1.7 2.6l.9.8M14.3 2.6l-.9.8"></path></svg></button></span><span class="tl-lbl-rbzone" title="Drag to select multiple layers" aria-label="Rubber-band layer selection area"></span>';
    const handle = lbl.querySelector(".tl-lbl-draghandle");

    lbl.addEventListener("click", (ev) => {
      if (
        ev.target.closest(
          "button,input,select,textarea,[data-timeline-control]",
        )
      )
        return;
      ...
      if (ev.shiftKey && tlLabelLastClicked != null) {
        ...
        const lo = Math.min(vi, lastVi),
          hi = Math.max(vi, lastVi);
        for (let x = lo; x <= hi; x++)
          selectedTlLabelIndices.add(visibleIndices[x]);
        renderLabelCol();
    ...
    lbl.querySelectorAll(".tl-layer-quick-btn").forEach((actionButton) => {
      if (actionButton.dataset.action === "onion") {
        ...
        actionButton.addEventListener("pointerdown", (event) => {
          ...
          _timelineOnionTrace("pointerdown", {
            layerIndex: i,
            pointerId: event.pointerId,
            ...
            capture: actionButton.hasPointerCapture(event.pointerId),
          });
        });
        actionButton.addEventListener("pointermove", (event) => {
          ...
          if (
            Math.hypot(
              event.clientX - pressed.startX,
              event.clientY - pressed.startY,
            ) > TAP_SLOP
          )
            pressed.moved = true;
        });
        actionButton.addEventListener("pointerup", (event) => {
          ...
          const rect = actionButton.getBoundingClientRect(),
            inside =
              event.clientX >= rect.left - TAP_SLOP &&
              event.clientX <= rect.right + TAP_SLOP &&
              event.clientY >= rect.top - TAP_SLOP &&
              event.clientY <= rect.bottom + TAP_SLOP &&
              !pressed.moved,
            nodeStillConnected =
              pressed.node === actionButton && actionButton.isConnected;
          _timelineOnionTrace("pointerup", {
            layerIndex: i,
            pointerId: event.pointerId,
            inside,
            moved: pressed.moved,
          ...
          clearPressed();
          if (inside && nodeStillConnected)
            _applyTimelineLayerQuickAction(l, "onion");
        });
        actionButton.addEventListener("pointercancel", (event) => {
          event.stopPropagation();
          _timelineOnionTrace("pointercancel", {
            layerIndex: i,
            pointerId: event.pointerId,
          });
          clearPressed();
        ...
        actionButton.addEventListener("lostpointercapture", () => {
          if (pressed)
            _timelineOnionTrace("lostpointercapture", {
              layerIndex: i,
        ...
        actionButton.addEventListener("click", (event) => {
          event.preventDefault();
          event.stopPropagation();
          _timelineOnionTrace("click", { layerIndex: i, detail: event.detail });
          if (event.detail === 0) _applyTimelineLayerQuickAction(l, "onion");
        });
        return;
      }
      ...
      actionButton.addEventListener("pointerup", (event) => {
        ...
        if (actionButton.hasPointerCapture(event.pointerId))
          actionButton.releasePointerCapture(event.pointerId);
        _applyTimelineLayerQuickAction(l, actionButton.dataset.action);
      });
      actionButton.addEventListener("pointercancel", (event) => {
        if (event.pointerId === actionPointerId) actionPointerId = null;
      });
      actionButton.addEventListener("lostpointercapture", () => {
        actionPointerId = null;
      });
...
function onTlGroupDragMove(ev) {
  ...
  const rect = tlHideZone.getBoundingClientRect(),
    over =
...
function onTlGroupDragUp(ev) {
  document.removeEventListener("pointermove", onTlGroupDragMove);
  document.removeEventListener("pointerup", onTlGroupDragUp);
  if (dragTlGroupId === null) return;
  const rect = tlHideZone.getBoundingClientRect(),
    over =
      ev.clientX >= rect.left &&
  ...
  document
    .querySelectorAll(".tl-lbl-dragging")
    .forEach((element) => element.classList.remove("tl-lbl-dragging"));
  if (over) {
    const group = _groupById(dragTlGroupId);
    if (group) group.onTimeline = false;
    renderLayerPanel();
    renderTimeline();
  }
  dragTlGroupId = null;
...
function onTlLabelDragUp(ev) {
  ...
  document
    .querySelectorAll(".tl-lbl-dragging")
    .forEach((el) => el.classList.remove("tl-lbl-dragging"));
  const r = tlHideZone.getBoundingClientRect();
  const over =
    ev.clientX >= r.left &&
    ev.clientX <= r.right &&
  ...
  if (over) {
    const toHide =
      selectedTlLabelIndices.size > 0
        ? [...selectedTlLabelIndices]
        : [dragTlLabelIdx];
    toHide.forEach((i) => {
      if (layers[i]) layers[i].onTimeline = false;
    });
    selectedTlLabelIndices.clear();
    renderLayerPanel();
    renderTimeline();
...
function startTlLabelRubberBand(ev) {
  if (ev.button !== 0) return;
  tlLbSelecting = true;
  tlLbStartX = ev.clientX;
  tlLbStartY = ev.clientY;
  if (!tlLbBoxEl) {
    tlLbBoxEl = document.createElement("div");
    tlLbBoxEl.style.cssText =
      "position:fixed;border:1px solid var(--accent);background:color-mix(in srgb,var(--accent) 50%,transparent);z-index:9998;pointer-events:none;display:none;border-radius:2px;";
    document.body.appendChild(tlLbBoxEl);
  }
  tlLbBoxEl.style.left = tlLbStartX + "px";
  tlLbBoxEl.style.top = tlLbStartY + "px";
  tlLbBoxEl.style.width = "0";
  tlLbBoxEl.style.height = "0";
  tlLbBoxEl.style.display = "block";
  document.addEventListener("pointermove", onTlLbMove);
  document.addEventListener("pointerup", onTlLbUp);
}
function _updateTlLabelAutoScroll(clientY) {
  const el = document.getElementById("tl-labels-rows");
  if (!el) {
    _stopAutoScroll();
    return;
  ...
  if (clientY < r.top + zone) {
    _autoScrollSpeed = -Math.max(2, Math.round((zone - (clientY - r.top)) / 4));
    _startAutoScroll(el);
  } else if (clientY > r.bottom - zone) {
    _autoScrollSpeed = Math.max(
      2,
      Math.round((zone - (r.bottom - clientY)) / 4),
    );
    _startAutoScroll(el);
  } else {
    _stopAutoScroll();
...
function onTlLbMove(ev) {
  ...
  const x = Math.min(ev.clientX, tlLbStartX),
    y = Math.min(ev.clientY, tlLbStartY);
  const w = Math.abs(ev.clientX - tlLbStartX),
    h = Math.abs(ev.clientY - tlLbStartY);
  tlLbBoxEl.style.left = x + "px";
  tlLbBoxEl.style.top = y + "px";
  tlLbBoxEl.style.width = w + "px";
  tlLbBoxEl.style.height = h + "px";
  const bx1 = x,
    by1 = y,
    bx2 = x + w,
    by2 = y + h;
...
function onTlLbUp() {
  ...
  if (
    selectedTlLabelIndices.size > 0 &&
    !selectedTlLabelIndices.has(curLayer)
  ) {
...
function _makeCameraPropertyRow(property, totalW, timelineFps) {
  const row = document.createElement("div");
  row.className =
    "tl-row tl-layer-track-row tl-camera-property-row" +
    (cameraSelectedProperties.has(property) ? " timeline-row-active" : "");
  ...
  for (let frame = 0; frame < TOTAL; frame++) {
    const cell = document.createElement("div"),
      id = _cameraPropertyId(property, frame),
      key = keys.find((item) => item.frame === frame);
    cell.className =
      "tl-cell camera-property-cell" +
      (Math.floor(frame / timelineFps) % 2 === 1 ? " second-band" : "") +
      (cameraSelectedProperties.has(property) && frame === curFrame
        ? " cur-col"
        : "") +
      (cameraSelectedProperties.has(property) && selectedFrames.has(frame)
        ? " selected"
        : "");
    cell.style.cssText =
      "left:" +
      ...
      CellH +
      "px;";
    cell.dataset.cameraProperty = property;
    ...
    cell.addEventListener("pointerdown", (event) => {
      ...
      if (
        event.shiftKey &&
        kfSelectionAnchor &&
        kfSelectionAnchor.trackId === "camera-" + property
      ) {
        const lo = Math.min(kfSelectionAnchor.frameIndex, frame),
          hi = Math.max(kfSelectionAnchor.frameIndex, frame);
        selectedFrames.clear();
        for (let value = lo; value <= hi; value++) selectedFrames.add(value);
        selectedKFs.clear();
        keys
          .filter((item) => item.frame >= lo && item.frame <= hi)
          .forEach((item) =>
            selectedKFs.add(_cameraPropertyId(property, item.frame)),
          );
      } else if (event.ctrlKey || event.metaKey) {
        if (selectedFrames.has(frame)) selectedFrames.delete(frame);
        else selectedFrames.add(frame);
        if (key) {
          if (selectedKFs.has(id)) selectedKFs.delete(id);
          else selectedKFs.add(id);
        }
        kfSelectionAnchor = {
          trackId: "camera-" + property,
          frameIndex: frame,
        };
      } else {
        const dragSelected = event.button === 0 && key && selectedKFs.has(id);
        if (!dragSelected) {
          selectedKFs.clear();
          if (key) selectedKFs.add(id);
        }
        selectedFrames.clear();
        selectedFrames.add(frame);
        kfSelectionAnchor = {
        ...
        if (event.button === 0) {
          if (dragSelected) startKFDrag("camera-" + property, frame, event);
          else tlSelDrag = { startF: frame, trackId: "camera-" + property };
        }
      }
      if (event.button !== 2) goToFrame(frame, false, false, true);
      renderTimeline();
    });
    row.appendChild(cell);
  }
  keys
    .filter((key) => key.frame < TOTAL)
    .forEach((key) => {
      const id = _cameraPropertyId(property, key.frame),
        marker = document.createElement("div");
      marker.className =
        "camera-kf camera-property-kf" +
        (selectedKFs.has(id) || selectedKFs.has("camera:" + key.frame)
          ? " selected"
          : "");
      marker.style.left = key.frame * CellW + CellW / 2 + "px";
      marker.style.top = CellH / 2 + "px";
      marker.dataset.cameraFrame = key.frame;
      marker.dataset.cameraProperty = property;
      marker.dataset.kk = id;
      marker.title =
        _cameraPropertyLabel(property) + " Key F" + frameLabel(key.frame);
      marker.addEventListener("pointerdown", (event) => {
        ...
        if (
          ...
          const lo = Math.min(kfSelectionAnchor.frameIndex, key.frame),
        ...
        selectedKFs.forEach((selected) => {
          if (selected.startsWith("camera-" + property + ":"))
            selectedFrames.add(Number(selected.split(":")[1]));
        });
        if (event.button !== 2) {
          goToFrame(key.frame, false, false, true);
          if (event.button === 0)
            startKFDrag("camera-" + property, key.frame, event);
        }
        renderTimeline();
      });
...
function renderRows() {
  const rowWrap = document.getElementById("tl-rows-wrap");
  rowWrap.innerHTML = "";
  ...
  ro.id = "range-overlay";
  rowWrap.appendChild(ro);
  const totalW = TOTAL * CellW,
    timelineFps = Math.max(1, getFPS());
  rowWrap.style.width = totalW + "px";

  if (_cameraTrackExists()) {
    const cameraRow = document.createElement("div");
    cameraRow.className =
      "tl-row tl-camera-track-row tl-layer-track-row" +
      (cameraTrackSelected ? " timeline-row-active" : "");
    cameraRow.style.width = totalW + "px";
    cameraRow.style.height = CellH + "px";
    cameraRow.style.position = "relative";
    for (let frame = 0; frame < TOTAL; frame++) {
      const cell = document.createElement("div");
      cell.className =
        "tl-cell camera-track-cell" +
        (Math.floor(frame / timelineFps) % 2 === 1 ? " second-band" : "") +
        (cameraTrackSelected && frame === curFrame ? " cur-col" : "") +
        (cameraTrackSelected && selectedFrames.has(frame) ? " selected" : "");
      cell.style.cssText =
        "left:" +
        frame * CellW +
        "px;position:absolute;width:" +
      ...
      cell.addEventListener("pointerdown", (event) => {
        ...
        if (
          ...
          selectedKFs.clear();
          _cameraTrackKeys()
            .filter((key) => key.frame >= lo && key.frame <= hi)
            .forEach((key) => selectedKFs.add("camera:" + key.frame));
        } else if (event.ctrlKey || event.metaKey) {
          if (selectedFrames.has(frame)) selectedFrames.delete(frame);
          else selectedFrames.add(frame);
          const key = _cameraKeyAt(frame),
            id = "camera:" + frame;
          if (key) {
            if (selectedKFs.has(id)) selectedKFs.delete(id);
            else selectedKFs.add(id);
          ...
          const key = _cameraKeyAt(frame),
            id = "camera:" + frame,
            dragSelectedKey = event.button === 0 && key && selectedKFs.has(id);
          if (!dragSelectedKey) {
            selectedKFs.clear();
            if (key) selectedKFs.add(id);
          }
          ...
          if (dragSelectedKey) {
            ...
            if (frames.length) {
              ...
              for (let value = lo; value <= hi; value++)
                selectedFrames.add(value);
            }
          } else selectedFrames.add(frame);
          kfSelectionAnchor = { trackId: "camera", frameIndex: frame };
          if (event.button === 0) {
            if (dragSelectedKey) startKFDrag("camera", frame, event);
            else tlSelDrag = { startF: frame, trackId: "camera" };
          }
        }
        if (event.button === 2) {
      ...
      cell.addEventListener("contextmenu", () => {
        ...
        const key = _cameraKeyAt(frame);
        selectedKFs.clear();
        if (key) selectedKFs.add("camera:" + frame);
        kfSelectionAnchor = { trackId: "camera", frameIndex: frame };
        refreshTimelineSelection();
      });
    ...
    _cameraTrackKeys()
      .filter((key) => key.frame < TOTAL)
      .forEach((key) => {
        const id = `camera:${key.frame}`,
          marker = document.createElement("div");
        marker.className =
          "camera-kf" + (selectedKFs.has(id) ? " selected" : "");
        marker.style.left = key.frame * CellW + CellW / 2 + "px";
        marker.style.top = CellH / 2 + "px";
        marker.dataset.cameraFrame = key.frame;
        marker.dataset.kk = id;
        marker.title = "Camera Key F" + frameLabel(key.frame);
        marker.addEventListener("pointerdown", (event) => {
          event.stopPropagation();
          if (tool !== "camera") setTool("camera", "Camera");
          ...
          if (!event.shiftKey && !event.ctrlKey && !event.metaKey) {
            ...
            if (selectedKFs.has(id)) selectedFrames.add(key.frame);
            else selectedFrames.delete(key.frame);
          }
          if (event.button === 2) {
            refreshTimelineSelection();
            return;
        ...
        marker.addEventListener("contextmenu", () => {
          ...
          selectedKFs.clear();
          selectedKFs.add(id);
          kfSelectionAnchor = { trackId: "camera", frameIndex: key.frame };
          refreshTimelineSelection();
        });
    ...
    if (cameraTrackExpanded)
      _cameraProperties().forEach((property) =>
        rowWrap.appendChild(
          _makeCameraPropertyRow(property, totalW, timelineFps),
        ),
      );
  }
  ...
  timelineTreeItems().forEach((item) => {
    if (item.type === "group") {
      const groupRow = document.createElement("div"),
        group = _groupById(item.id);
      groupRow.className = "tl-row tl-group-track-row";
      groupRow.style.width = totalW + "px";
      groupRow.style.height = CellH + "px";
      groupRow.dataset.groupId = item.id;
      if (group && group.color && group.color !== "transparent") {
        const hex = group.color,
          r = parseInt(hex.slice(1, 3), 16),
          g = parseInt(hex.slice(3, 5), 16),
          b = parseInt(hex.slice(5, 7), 16);
        groupRow.style.background = `rgba(${r},${g},${b},0.22)`;
      }
      rowWrap.appendChild(groupRow);
      return;
    }
    const i = item.idx,
      l = layers[i];
    const row = document.createElement("div");
    row.className =
      "tl-row tl-layer-track-row" +
      (!cameraTrackSelected && i === curLayer ? " timeline-row-active" : "") +
      (l.color && l.color !== "transparent" ? " has-layer-color" : "");
    row.style.width = totalW + "px";
    row.style.position = "relative";
    ...
    if (l.color && l.color !== "transparent") {
      ...
      const r = parseInt(hex.slice(1, 3), 16),
        g = parseInt(hex.slice(3, 5), 16),
        b = parseInt(hex.slice(5, 7), 16);
      row.style.background = `rgba(${r},${g},${b},0.25)`;
      row.style.setProperty("--timeline-layer-color", l.color);
    }
    ...
    for (let f = 0; f < TOTAL; f++) {
      ...
      if (!cameraTrackSelected && f === curFrame && i === curLayer)
        cls += " cur-col";
      ...
      cell.addEventListener("pointerdown", (ev) => {
        ...
        if (ev.shiftKey) {
          const anchor = kfSelectionAnchor || {
            layerIndex: curLayer,
            frameIndex: curFrame,
          };
          selectKeyframeRange(anchor, { layerIndex: i, frameIndex: f });
          selectedRowLayers = new Set(_rowSpanLayers(anchor.layerIndex, i));
          const lo = Math.min(anchor.frameIndex, f),
            hi = Math.max(anchor.frameIndex, f);
          selectedFrames.clear();
          for (let ff = lo; ff <= hi; ff++) selectedFrames.add(ff);
          if (i !== curLayer)
            switchLayer(i, {
          ...
          if (selectedFrames.has(f)) selectedFrames.delete(f);
          else selectedFrames.add(f);
          ...
          tlSelDrag = { startF: f, anchorLayer: i };
          if (i !== curLayer)
            switchLayer(i, {
              skipTimelineRender: true,
              skipLoadFrame: true,
    ...
    // Extend bars
    const kfs = Object.keys(l.frames)
      .map(Number)
      .sort((a, b) => a - b);
    for (let ki = 0; ki < kfs.length - 1; ki++) {
      const f1 = kfs[ki],
        f2 = kfs[ki + 1];
      if (f1 >= TOTAL) continue;
      const bar = document.createElement("div");
      bar.className = "kf-extend";
      bar.style.left = f1 * CellW + CellW / 2 + "px";
      bar.style.width = (Math.min(f2, TOTAL - 1) - f1) * CellW + "px";
      bar.style.top = CellH / 2 - 1 + "px";
      row.appendChild(bar);
    }
    ...
    kfs
      ...
      .forEach((f) => {
        ...
        const kk = `${i}:${f}`;
        let cls = "kf-block";
        if (selectedKFs.has(kk)) cls += " selected";
        if (
          typeof isDrawingFrameHidden === "function" &&
          isDrawingFrameHidden(i, f)
        )
          cls += " drawing-hidden";
        block.className = cls;
        block.style.cssText =
          ...
          (f * CellW + 3) +
          "px;top:4px;bottom:4px;width:" +
          (CellW - 6) +
          "px;";
        block.style.setProperty(
          "--kf-mark-color",
          getMarkDef(getDrawingMark(i, f)).color,
        );
        block.title =
          l.name +
          " F" +
        ...
        if (
          ...
          x.className = "kf-hidden-x";
          x.textContent = "X";
          block.appendChild(x);
        }
        block.dataset.layerIdx = i;
        block.dataset.kk = kk;
        block.addEventListener("pointerdown", (ev) => {
          ...
          if (
            ...
            const lo = Math.min(kfSelectionAnchor.frameIndex, f),
              hi = Math.max(kfSelectionAnchor.frameIndex, f);
            selectedFrames.clear();
            for (let ff = lo; ff <= hi; ff++) selectedFrames.add(ff);
          } else {
            if (!selectedKFs.has(kk)) {
              selectedKFs.clear();
              selectedKFs.add(kk);
            }
            selectedRowLayers = new Set([i]);
  ...
  let tlRbSelecting = false,
    tlRbStartX = 0,
    tlRbStartY = 0,
    tlRbBoxEl = null;
  if (!rowWrap.dataset.selectionHandlersBound)
    rowWrap.addEventListener("pointerdown", (ev) => {
      ...
      if (ev.target.classList.contains("tl-cell") || ev.target === rowWrap) {
        ...
        if (!ev.shiftKey && !ev.ctrlKey && !ev.metaKey) {
          tlRbSelecting = true;
          tlRbStartX = ev.clientX;
          tlRbStartY = ev.clientY;
          if (!tlRbBoxEl) {
            tlRbBoxEl = document.createElement("div");
            tlRbBoxEl.style.cssText =
              "position:fixed;border:1px solid var(--accent);background:color-mix(in srgb,var(--accent) 50%,transparent);z-index:9998;pointer-events:none;border-radius:2px;display:none;";
            document.body.appendChild(tlRbBoxEl);
          }
          tlRbBoxEl.style.left = tlRbStartX + "px";
          tlRbBoxEl.style.top = tlRbStartY + "px";
          tlRbBoxEl.style.width = "0";
          tlRbBoxEl.style.height = "0";
          tlRbBoxEl.style.display = "block";
          const onRbMove = (mev) => {
            if (!tlRbSelecting) return;
            const x = Math.min(mev.clientX, tlRbStartX),
              y = Math.min(mev.clientY, tlRbStartY);
            const w = Math.abs(mev.clientX - tlRbStartX),
              h = Math.abs(mev.clientY - tlRbStartY);
            tlRbBoxEl.style.left = x + "px";
            tlRbBoxEl.style.top = y + "px";
            tlRbBoxEl.style.width = w + "px";
            tlRbBoxEl.style.height = h + "px";
          };
          const onRbUp = () => {
            tlRbSelecting = false;
  ...
  if (!rowWrap.dataset.selectionHandlersBound)
    rowWrap.addEventListener("pointermove", (e) => {
      ...
      const f2 = Math.max(
        ...
        Math.min(
          TOTAL - 1,
          Math.floor((e.clientX - r.left + tlScroll.scrollLeft) / CellW),
        ),
      );

      if (
        typeof tlSelDrag.trackId === "string" &&
        tlSelDrag.trackId.startsWith("camera-")
      ) {
        const anchorProperty = tlSelDrag.trackId.slice(7),
          under = document.elementFromPoint(e.clientX, e.clientY),
          targetRow = under && under.closest("[data-camera-property]"),
          targetProperty =
            (targetRow && targetRow.dataset.cameraProperty) || anchorProperty,
          properties = _cameraProperties(),
          anchorIndex = properties.indexOf(anchorProperty),
          targetIndex = properties.indexOf(targetProperty),
          selectedProperties =
          ...
          lo = Math.min(tlSelDrag.startF, f2),
          hi = Math.max(tlSelDrag.startF, f2);
        selectedFrames.clear();
        for (let frame = lo; frame <= hi; frame++) selectedFrames.add(frame);
        selectedKFs.clear();
        selectedProperties.forEach((property) =>
          _cameraPropertyKeys(property)
            .filter((key) => key.frame >= lo && key.frame <= hi)
            .forEach((key) =>
              selectedKFs.add(_cameraPropertyId(property, key.frame)),
            ),
        ...
        cameraTrackSelected = true;
        cameraSelectedProperties = new Set(selectedProperties);
        cameraSelectedProperty =
          selectedProperties.length === 1 ? selectedProperties[0] : null;
        curFrame = f2;
        CameraSystem.evaluateAt(curFrame);
        refreshTimelineSelection();
        return;
      ...
      if (tlSelDrag.trackId === "camera") {
        const lo = Math.min(tlSelDrag.startF, f2),
          hi = Math.max(tlSelDrag.startF, f2);
        selectedFrames.clear();
        for (let frame = lo; frame <= hi; frame++) selectedFrames.add(frame);
        selectedKFs.clear();
      ...
      if (rowEl && rowEl.dataset.layerIdx !== undefined)
        li = parseInt(rowEl.dataset.layerIdx);
      ...
      if (kfSelectionAnchor) {
        selectKeyframeRange(kfSelectionAnchor, {
          layerIndex: li,
          frameIndex: f2,
        });
      }

      const frameChanged = f2 !== curFrame;
      curFrame = f2;
      if (frameChanged) loadFrame(curLayer, curFrame);
      refreshTimelineSelection();
    });
...
function updateRangeOverlay() {
  const ro = document.getElementById("range-overlay");
  if (!ro) return;
  const left = rangeStart * CellW,
    width = (rangeEnd - rangeStart + 1) * CellW,
    totalH = (timelineTreeItems().length + _cameraTimelineRowCount()) * CellH;
  ro.style.cssText =
    ...
    "px;height:" +
    totalH +
    "px;border-left:2px solid rgba(29,158,117,0.6);border-right:2px solid rgba(226,75,74,0.6);pointer-events:none;z-index:6;";
}

function updatePlayhead() {
  const ph = document.getElementById("playhead");
  if (!ph) return;
  const playheadFrame = Number.isFinite(window.SharedPlayhead?.position)
    ? window.SharedPlayhead.position
    : curFrame;
  const left = playheadFrame * CellW + CellW / 2 - 1;
  ph.style.left = left + "px";
  ph.style.height =
    (timelineTreeItems().length + _cameraTimelineRowCount()) * CellH + "px";
  ph.style.top = "0";
  window.dispatchEvent(
    new CustomEvent("timeline-playhead-updated", {
      detail: { frame: playheadFrame, left },
    }),
  );
  ...
  if (!window._tlEdgeZooming && !window._tlZooming) {
    ...
    if (left > visible - 40 || left < tlScroll.scrollLeft + 20)
      tlScroll.scrollLeft = left - tlScroll.clientWidth / 2;
  }
}

function _statusKeyframeTarget() {
  let layerIndex = curLayer,
    frameIndex = curFrame;
  if (selectedKFs.size) {
    let key = null;
    if (kfSelectionAnchor) {
      const anchorKey =
        kfSelectionAnchor.layerIndex + ":" + kfSelectionAnchor.frameIndex;
      if (selectedKFs.has(anchorKey)) key = anchorKey;
    }
    key = key || selectedKFs.values().next().value;
    const split = String(key).split(":");
    if (split.length === 2) {
      layerIndex = Number(split[0]);
      frameIndex = Number(split[1]);
    }
  }
...
function updateStatus() {
  const target = _statusKeyframeTarget(),
    layer = layers[target.layerIndex];
  const keyframes =
    layer && layer.frames
      ? Object.keys(layer.frames)
          .map(Number)
          .sort((a, b) => a - b)
      : [];
  ...
  if (keyframeIndex < 0) {
    for (let index = keyframes.length - 1; index >= 0; index--) {
      if (keyframes[index] <= target.frameIndex) {
        keyframeIndex = index;
        break;
      }
    }
  ...
  document.getElementById("stat-kf").textContent =
    "KF: " + (keyframeIndex >= 0 ? keyframeIndex + 1 : "—");
  document.getElementById("stat-frame").textContent =
    "F: " + (keyframe === null ? "—" : frameLabel(keyframe));
  document.getElementById("stat-range").textContent =
    "Range: " + frameLabel(rangeStart) + "–" + frameLabel(rangeEnd);
  let exposure = "—";
  if (keyframe !== null) {
...
function syncOpacityControls() {
  ...
  if (activeGroupId) {
    ...
    if (cl && cl.stencil && cl.stencil !== "none") {
      const displayStencil =
        cl.stencil === "inside" || cl.stencil === "group-inside"
          ? "inside"
          : cl.stencil === "outside" || cl.stencil === "group-outside"
            ? "outside"
            : "none";
  ...
  layerOpVal.textContent = pct + "%";
  const displayStencil =
    l.stencil === "inside" || l.stencil === "group-inside"
      ? "inside"
      : l.stencil === "outside" || l.stencil === "group-outside"
        ? "outside"
        : "none";
  layerStencilSel.value = displayStencil;
...
layerStencilSel.onchange = () => {
  ...
  if (v === "none") {
    ...
    l.clipTo = null;
    l.clipToGroup = null;
  } else if (l.clipToGroup) {
    l.stencil = v === "outside" ? "group-outside" : "group-inside";
  } else {
    l.stencil = v;
  }
  recomposite(curLayer, curFrame);
...
function showLayerColorPicker(layerIdx, dotEl) {
  ...
  popup.style.cssText =
    "position:fixed;z-index:9999;background:var(--bg2);border:1px solid var(--border2);border-radius:8px;padding:8px;box-shadow:0 8px 24px rgba(0,0,0,0.5);display:flex;flex-direction:column;gap:6px;min-width:130px;";
  ...
  title.style.cssText =
    "font-size:10px;color:var(--text2);font-weight:600;letter-spacing:.06em;text-transform:uppercase;margin-bottom:2px;";
  ...
  clearBtn.style.cssText =
    "width:20px;height:20px;border-radius:50%;border:1.5px solid var(--border2);cursor:pointer;background:repeating-conic-gradient(#999 0% 25%,#ddd 0% 50%) 0 0/10px 10px;flex-shrink:0;";
  clearBtn.onclick = () => {
    layers[layerIdx].color = "transparent";
    popup.remove();
    layerColorPickerEl = null;
    renderLayerPanel();
    renderTimeline();
  };
  ...
  LCOLORS.forEach((c) => {
    ...
    sw.style.cssText =
      "width:20px;height:20px;border-radius:50%;border:1.5px solid var(--border2);cursor:pointer;background:" +
      c +
      ";flex-shrink:0;";
    if (layers[layerIdx].color === c) sw.style.outline = "2px solid #fff";
    sw.onclick = () => {
      layers[layerIdx].color = c;
      popup.remove();
      layerColorPickerEl = null;
      renderLayerPanel();
      renderTimeline();
  ...
  customLabel.style.cssText = "font-size:10px;color:var(--text2);";
  customLabel.textContent = "Custom";
  const customIn = document.createElement("input");
  customIn.type = "color";
  customIn.value =
    layers[layerIdx].color && layers[layerIdx].color !== "transparent"
      ? layers[layerIdx].color
      : "#7F77DD";
  customIn.style.cssText =
    "width:32px;height:22px;border:1px solid var(--border2);border-radius:4px;cursor:pointer;padding:1px;background:var(--bg3);";
  customIn.oninput = (e) => {
    layers[layerIdx].color = e.target.value;
    renderLayerPanel();
    renderTimeline();
  };
...
function showGroupColorPicker(grp, dotEl) {
  ...
  customIn.type = "color";
  customIn.value =
    grp.color && grp.color !== "transparent" ? grp.color : "#7F77DD";
  customIn.style.cssText =
    "width:32px;height:22px;border:1px solid var(--border2);border-radius:4px;cursor:pointer;padding:1px;background:var(--bg3);";
  customIn.oninput = (e) => {
    grp.color = e.target.value;
    renderLayerPanel();
    renderTimeline();
  };
  customRow.appendChild(customLabel);
...
function startEyeDrag(layerIdx, ev) {
  ...
  eyeGhost.style.left = ev.clientX + "px";
  eyeGhost.style.top = ev.clientY + "px";
...
function onEyeDragMove(ev) {
  ...
  eyeGhost.style.top = ev.clientY + "px";
  document
    .querySelectorAll(".layer-row[data-idx]")
    .forEach((r) => r.classList.remove("eye-drop-target"));
  document
    .querySelectorAll(".layer-group-row[data-gid]")
    .forEach((r) => r.classList.remove("eye-drop-target"));
  ...
  if (layerRow) {
    ...
    const dragFlatPos = flat.findIndex(
      (it) => it.type === "layer" && it.idx === eyeDragLayerIdx,
    );
    const nextItem = dragFlatPos >= 0 ? flat[dragFlatPos + 1] : null;
    let isDirectlyBelow =
      nextItem && nextItem.type === "layer" && nextItem.idx === idx;
    ...
    if (
      isDirectlyBelow &&
      draggedInGroup &&
      nextItem._layerRef.groupId !== draggedLayer.groupId
    )
      isDirectlyBelow = false;
    // A layer outside any group can never target a layer that's inside a group (outside-to-inside blocked too).
    if (isDirectlyBelow && !draggedInGroup && nextItem._layerRef.groupId)
      isDirectlyBelow = false;
    if (!isNaN(idx) && isDirectlyBelow) {
      eyeDragTargetIdx = idx;
      eyeDragTargetGid = null;
      layerRow.classList.add("eye-drop-target");
    } else {
      eyeDragTargetIdx = null;
      eyeDragTargetGid = null;
    }
  } else if (groupRow) {
    ...
    const isDirectlyBelow =
      nextItem && nextItem.type === "group" && nextItem.id === gid;
    // A layer inside a group can never target a group header (that's always outside its own group).
    if (grp && isDirectlyBelow && !draggedInGroup) {
      eyeDragTargetGid = gid;
      eyeDragTargetIdx = null;
      groupRow.classList.add("eye-drop-target");
    } else {
      eyeDragTargetGid = null;
    }
  } else {
...
function onEyeDragUp() {
  ...
  const targetLayerRow = document.querySelector(
    ".layer-row.eye-drop-target[data-idx]",
  );
  const targetGroupRow = document.querySelector(
    ".layer-group-row.eye-drop-target[data-gid]",
  );
  document
    .querySelectorAll(".layer-row[data-idx]")
    .forEach((r) => r.classList.remove("eye-drop-target"));
  ...
  if (targetGroupRow) {
    ...
    if (
      l.clipToGroup === gid &&
      (l.stencil === "group-inside" || l.stencil === "group-outside")
    ) {
      l.clipToGroup = null;
      l.stencil = "none";
    } else {
      l.clipToGroup = gid;
      l.clipTo = null;
      if (l.stencil !== "group-inside" && l.stencil !== "group-outside")
        l.stencil = "group-inside";
    }
...
function _findGroupEyeDropTarget(gid) {
  const flat = _buildFlatDisplayItems();
  const headerPos = flat.findIndex(
    (it) => it.type === "group" && it.id === gid,
  );
...
function onEyeDragGroupMove(ev) {
  eyeGhost.style.left = ev.clientX + "px";
  eyeGhost.style.top = ev.clientY + "px";
  eyeGhost._lastX = ev.clientX;
  eyeGhost._lastY = ev.clientY;
  document
    .querySelectorAll(".layer-row[data-idx]")
    .forEach((r) => r.classList.remove("eye-drop-target"));
  document
...
function onEyeDragGroupUp() {
  ...
  document
    .querySelectorAll(".layer-row[data-idx]")
    .forEach((r) => r.classList.remove("eye-drop-target", "eye-blocked"));
  document
    .querySelectorAll(".layer-group-row[data-gid]")
    .forEach((r) => r.classList.remove("eye-drop-target", "eye-blocked"));
  if (targetGroupRow) {
    ...
    if (g) {
      if (g.clipToGroup === gid && g.stencil && g.stencil !== "none") {
        ...
        g.clipToGroup = gid;
        if (g.stencil !== "inside" && g.stencil !== "outside")
          g.stencil = "inside";
      }
      recomposite(curLayer, curFrame);
      renderLayerPanel();
    ...
    if (!isNaN(idx)) {
      ...
      if (g) {
        if (
          g.clipTo === idx &&
          (g.stencil === "inside" || g.stencil === "outside")
        ) {
          g.clipTo = null;
          g.stencil = "none";
        } else {
...
function _layerGroupChainVisible(l) {
  ...
  for (const gid of _groupChain(l.groupId)) {
    const g = _groupById(gid);
...
function _groupBlockEnd(flat, headerPos) {
  ...
  while (i < flat.length) {
    ...
    if (
      it.type === "layer" &&
      it._layerRef &&
      it._layerRef.groupId &&
      idSet.has(it._layerRef.groupId)
    ) {
      i++;
      continue;
    }
    if (it.type === "group" && idSet.has(it.id)) {
      i++;
      continue;
    }
    break;
...
function _buildFlatGeneric(opts) {
  ...
  for (let i = layers.length - 1; i >= 0; i--) {
    ...
    flat.push({
      ...
      idx: i,
      _layerRef: l,
      depth: l.groupId ? _groupDepth(l.groupId) + 1 : 0,
    });
  }
  if (opts.includeOrphanGroups) {
    ...
    groups.forEach((g) => {
      if (!renderedGroups.has(g.id) && !_groupHasLayersInSubtree(g.id))
        emitAncestors(g.id);
    });
  }
...
function showDropLine(rowEl, position) {
  ...
  if (!layerDropLineEl || !layerDropLineEl.parentNode) {
    ...
    layerDropLineEl.style.cssText =
      "position:absolute;left:0;right:0;height:2px;background:var(--red);z-index:100;pointer-events:none;border-radius:2px;box-shadow:0 0 4px rgba(226,75,74,0.6);";
...
function _createAutoScrollController(listEl) {
  ...
  return {
    update(clientY) {
      ...
      const zone = 40;
      if (clientY < r.top + zone) {
        speed = -Math.max(2, Math.round((zone - (clientY - r.top)) / 4));
        if (!raf) raf = requestAnimationFrame(step);
      } else if (clientY > r.bottom - zone) {
        speed = Math.max(2, Math.round((zone - (r.bottom - clientY)) / 4));
        if (!raf) raf = requestAnimationFrame(step);
      } else {
        speed = 0;
      }
...
function startRowDrag(opts) {
  if (
    opts.downEv.pointerType === "pen"
      ? !(opts.downEv.buttons & 1)
      : opts.downEv.button !== 0
  )
    return;
  let active = false;
  const startX = opts.downEv.clientX,
    startY = opts.downEv.clientY;
  let dropTarget = null;
  ...
  const onMove = (ev) => {
    ...
    if (row) {
      ...
      const rows = [...opts.listEl.querySelectorAll(opts.rowSelector)].filter(
        (r) => opts.getRowId(r) !== opts.dragId,
      );
      if (rows.length) {
        let nearest = rows[0],
          nd = Infinity;
  ...
  const finish = (cancelled) => {
    ...
    if (!cancelled && wasActive && dropTarget)
      opts.onDrop(dropTarget.id, dropTarget.before);
  };
...
function startLayerRubberBand(ev) {
  ...
  if (!lbBoxEl) {
    ...
    lbBoxEl.style.cssText =
      "position:fixed;border:1px solid var(--accent);background:color-mix(in srgb,var(--accent) 50%,transparent);z-index:9998;pointer-events:none;display:none;border-radius:2px;";
    document.body.appendChild(lbBoxEl);
  }
  lbBoxEl.style.left = lbStartX + "px";
  lbBoxEl.style.top = lbStartY + "px";
  lbBoxEl.style.width = "0";
  lbBoxEl.style.height = "0";
  lbBoxEl.style.display = "block";
...
function onLBMove(ev) {
  ...
  const x = Math.min(ev.clientX, lbStartX),
    y = Math.min(ev.clientY, lbStartY);
  const w = Math.abs(ev.clientX - lbStartX),
    h = Math.abs(ev.clientY - lbStartY);
  lbBoxEl.style.left = x + "px";
  lbBoxEl.style.top = y + "px";
  lbBoxEl.style.width = w + "px";
  lbBoxEl.style.height = h + "px";
...
function renderLayerPanel() {
  ...
  activeC.style.cursor = activeGroupId
    ? "not-allowed"
    : typeof _baseCursorCSS === "function"
      ? _baseCursorCSS()
      : "crosshair";
  ...
  groups.forEach((g) => {
    if (g.parentId && !groups.find((g2) => g2.id === g.parentId))
      g.parentId = null;
  });
  ...
  for (const entry of flat) {
    ...
    const isCurActive =
      i === curLayer &&
      !activeGroupId &&
      (!hasMultiSel || selectedLayerIndices.has(curLayer));
    const row = document.createElement("div");
    row.className =
      "layer-row" +
      (isCurActive ? " active" : "") +
    ...
    if (entry.depth > 1)
      row.style.paddingLeft = 18 + (entry.depth - 1) * 16 + "px";
    if (l.color && l.color !== "transparent") {
      const hex = l.color;
      const r = parseInt(hex.slice(1, 3), 16),
        g = parseInt(hex.slice(3, 5), 16),
      ...
      row.addEventListener("mouseenter", () => {
        row.style.background = `rgba(${r},${g},${b},0.32)`;
      });
    ...
    const dotStyle =
      l.color && l.color !== "transparent"
        ? "background:" + l.color
        : "background:repeating-conic-gradient(#999 0% 25%,#ddd 0% 50%) 0 0/8px 8px;border:1px solid var(--border2);";
    const clipArrow =
      l.clipTo != null || l.clipToGroup != null
        ? `<span class="layer-clip-arrow" title="Clipped ${l.stencil === "outside" || l.stencil === "group-outside" ? "outside" : "inside"} ${l.clipToGroup != null ? "group" : "layer"} below">${l.stencil === "outside" || l.stencil === "group-outside" ? "⬇out" : "⬇in"}</span>`
        : "";
    ...
    const opTag =
      layerOwnOp < 1
        ? `<span style="font-size:9px;color:var(--text2);flex-shrink:0;">${effectiveOp}%</span>`
        : "";
    ...
    row.querySelector(".layer-rb-zone").addEventListener("pointerdown", (e) => {
      if (
        ...
        e.target.classList.contains("layer-unhide")
      )
        return;
      if (e.pointerType === "pen" ? !(e.buttons & 1) : e.button !== 0) return;
      e.preventDefault();
    ...
    row.addEventListener("click", (e) => {
      if (e.target.classList.contains("layer-dot")) {
        e.stopPropagation();
        showLayerColorPicker(i, e.target);
        return;
      }
      if (e.target.classList.contains("layer-unhide")) {
        e.stopPropagation();
        l.onTimeline = true;
        renderLayerPanel();
        renderTimeline();
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        const posOf = (idx) =>
          order.findIndex((o) => o.type === "layer" && o.idx === idx);
        const posOfGroup = (gid) =>
          order.findIndex((o) => o.type === "group" && o.id === gid);
        const posA =
          groupShiftAnchor != null
            ? posOfGroup(groupShiftAnchor)
            : posOf(layerShiftAnchor ?? curLayer);
        const posB = posOf(i);
        if (posA === -1 || posB === -1) {
    ...
    if (eyeEl) {
      eyeEl.addEventListener("pointerdown", (e) => {
        ...
        const onMove = (mv) => {
          if (
            Math.abs(mv.clientX - startX) > 5 ||
            Math.abs(mv.clientY - startY) > 5
          ) {
            dragged = true;
            document.removeEventListener("pointermove", onMove);
            startEyeDrag(i, mv);
    ...
    if (dotEl)
      dotEl.addEventListener("click", (e) => {
        e.stopPropagation();
        showLayerColorPicker(i, e.currentTarget);
      });
    ...
    row.addEventListener("contextmenu", (e) => {
      ...
      layerCtxMenu.style.left =
        Math.min(e.clientX, window.innerWidth - 160) + "px";
      layerCtxMenu.style.top =
  ...
  listEl.addEventListener(
    "pointerdown",
    (e) => {
      const onRow = e.target.closest(
        ".layer-row[data-idx],.layer-group-row[data-gid]",
      );
...
function _reanchorAllStencils() {
  ...
  layers.forEach((l, newIdx) => {
    ...
    const flatPos = flat.findIndex(
      (it) => it.type === "layer" && it.idx === newIdx,
    );
    ...
    if (!below) {
      ...
      l.clipToGroup = below.id;
      l.clipTo = null;
      if (l.stencil === "inside" || l.stencil === "outside")
        l.stencil = "group-inside";
...
function _startAutoScroll(el) {
  ...
  const step = () => {
    if (!_autoScrollEl || _autoScrollSpeed === 0) {
      _autoScrollRAF = null;
      return;
    }
    _autoScrollEl.scrollTop += _autoScrollSpeed;
...
function startLayerDrag(idx, downEv) {
  if (
    downEv.pointerType === "pen" ? !(downEv.buttons & 1) : downEv.button !== 0
  )
    return;
  ...
  const onMove = (ev) => {
    ...
    let row =
      el &&
      (el.closest(".layer-row[data-idx]") ||
        el.closest(".layer-group-row[data-gid]"));
    ...
    if (
      row &&
      !row.hasAttribute("data-gid") &&
      parseInt(row.dataset.idx) === idx
    )
      row = null;
    if (row) {
      const rr = row.getBoundingClientRect();
      ...
      layerDropTarget = {
        idx: isGrpRow ? row.dataset.gid : parseInt(row.dataset.idx),
        isGroup: isGrpRow,
        position: pos,
        dropMode,
      };
      if (dropMode === "into") {
        ...
        hideDropLine();
        document
          .querySelectorAll(".layer-group-row")
      ...
      const allRows = [
        ...document.querySelectorAll(
          "#layers-list .layer-row[data-idx], #layers-list .layer-group-row[data-gid]",
        ),
      ].filter(
        (r) =>
          !(!r.hasAttribute("data-gid") && parseInt(r.dataset.idx) === idx),
      );
      if (allRows.length) {
        let nearest = allRows[0],
        ...
        const clampPos =
          ev.clientY < (nr.top + nr.bottom) / 2 ? "before" : "after";
        const isGrp = clampRow.hasAttribute("data-gid");
        layerDropTarget = {
          idx: isGrp ? clampRow.dataset.gid : parseInt(clampRow.dataset.idx),
          isGroup: isGrp,
          position: clampPos,
          dropMode: "reorder",
        };
  ...
  const onUp = (ev) => {
    ...
    if (layerDropTarget.idx != null) {
      ...
      const selfDrop =
        !layerDropTarget.isGroup && layerDropTarget.idx === dragIdx;
      if (!selfDrop) {
        // Always use the full flat list (includes collapsed children) so nothing gets lost
        const flatFull = _buildFlatDisplayItemsAll();
        const dragFlatIdx = flatFull.findIndex(
          (it) => it.type === "layer" && it.idx === dragIdx,
        );
        if (dragFlatIdx >= 0) {
          let insertSlot,
            joinGroupId = null;
          if (layerDropTarget.isGroup && layerDropTarget.dropMode === "into") {
            ...
            const grpFlatIdx = flatFull.findIndex(
              (it) => it.type === "group" && it.id === layerDropTarget.idx,
            );
            if (grpFlatIdx < 0) {
              manualDragIdx = null;
              manualDragActive = false;
            ...
            const targetFlatIdx = layerDropTarget.isGroup
              ...
              : flatFull.findIndex(
                  (it) => it.type === "layer" && it.idx === layerDropTarget.idx,
                );
            if (targetFlatIdx < 0) {
              manualDragIdx = null;
              manualDragActive = false;
            ...
            let effectiveSlot =
              pos === "before" ? targetFlatIdx : targetFlatIdx + 1;
            if (layerDropTarget.isGroup && pos === "after") {
              const grpHeaderIdx = flatFull.findIndex(
                (it) => it.type === "group" && it.id === layerDropTarget.idx,
              );
              if (grpHeaderIdx >= 0)
                effectiveSlot = _groupBlockEnd(flatFull, grpHeaderIdx);
            }
          ...
          // an item back into the same slot is a harmless no-op.
          {
            ...
            const adjSlot = Math.max(
              0,
              Math.min(newFlat.length, insertSlot - removedBefore),
            );
            ...
            if (joinGroupId !== null) {
              ...
              const newPos = newFlat.findIndex(
                (it) => it.type === "layer" && it._layerRef === draggedLayer,
              );
                ...
                if (itemBelow && itemBelow.type === "layer") {
                  ...
                  if (belowGroup !== inferredGroup) {
                    ...
                    if (!layerDropTarget.isGroup) {
                      inferredGroup =
                        pos === "after" ? belowGroup : inferredGroup;
                    } else {
            ...
            const newLayersOrdered = newFlat
              .filter((it) => it.type === "layer")
              .map((it) => it._layerRef);
  ...
  function _cleanupLayerDrag(ev) {
    onUp(ev);
    document.removeEventListener("pointermove", onMove);
    document.removeEventListener("pointerup", _cleanupLayerDrag);
    document.removeEventListener("pointercancel", _cleanupLayerDrag);
  }
  document.addEventListener("pointermove", onMove);
...
function startAddLayerDrag(downEv) {
      ...
      if (insertSlot != null) {
        if (joinGroupId !== null) {
          ...
          if (itemBelow && itemBelow.type === "layer") {
            const belowGroup = itemBelow._layerRef.groupId ?? null;
            if (
              belowGroup !== inferredGroup &&
              !layerDropTarget.isGroup &&
        ...
        let layersAbove = 0;
        for (let fi = 0; fi < insertSlot; fi++)
          if (flatFull[fi].type === "layer") layersAbove++;
  ...
  function _cleanupPtrDrag(ev) {
    onUp(ev);
    document.removeEventListener("pointermove", onMove);
    document.removeEventListener("pointerup", _cleanupPtrDrag);
    document.removeEventListener("pointercancel", _cleanupPtrDrag);
    try {
      downEv.target.releasePointerCapture(ev.pointerId);
    } catch (err) {}
  }
  document.addEventListener("pointermove", onMove);
...
function makeGroupRow(grp, depth) {
  ...
  const isChildActive =
    ...
    !activeGroupId &&
    layers[curLayer] &&
    layers[curLayer].groupId === grp.id;
  ...
  row.className =
    ...
    (grp.onTimeline === false ? " tl-hidden" : "");
  ...
  if (grp.color && grp.color !== "transparent") {
    ...
    row.addEventListener("mouseleave", () => {
      row.style.background = isChildActive
        ? `rgba(${r},${g},${b},0.18)`
        : `rgba(${r},${g},${b},0.22)`;
    });
  }
  ...
  const hasChildren =
    layers.some((l) => l.groupId === grp.id) ||
    groups.some((g) => g.parentId === grp.id);
  const chevron = grp.collapsed ? "▶" : "▼";
  const toggleStyle = hasChildren
    ? ""
    : "visibility:hidden;pointer-events:none;";
  const opPct = Math.round((grp.opacity ?? 1) * 100);
  const opTag =
    opPct < 100
      ? `<span style="font-size:9px;color:var(--text2);flex-shrink:0;">${opPct}%</span>`
      : "";
  const nameBold = isActive || isMultiSel;
  const groupHiddenFromTimeline = grp.onTimeline === false;
  const grpDotStyle =
    grp.color && grp.color !== "transparent"
      ? "background:" + grp.color
      : "background:repeating-conic-gradient(#999 0% 25%,#ddd 0% 50%) 0 0/8px 8px;border:1px solid var(--border2);";
  const grpClipArrow =
    (grp.clipToGroup != null || grp.clipTo != null) &&
    grp.stencil &&
    grp.stencil !== "none"
      ? `<span class="layer-clip-arrow" title="Clipped ${grp.stencil === "outside" ? "outside" : "inside"} ${grp.clipToGroup != null ? "group" : "layer"} below">${grp.stencil === "outside" ? "⬇out" : "⬇in"}</span>`
      : "";
  row.innerHTML =
  ...
  row.querySelector(".layer-group-toggle").onclick = (e) => {
    e.stopPropagation();
    grp.collapsed = !grp.collapsed;
    renderLayerPanel();
    renderTimeline();
  };
  const groupUnhide = row.querySelector(".layer-unhide");
  if (groupUnhide)
    groupUnhide.onclick = (event) => {
      event.stopPropagation();
      grp.onTimeline = true;
      renderLayerPanel();
      renderTimeline();
  ...
  if (grpDotEl)
    grpDotEl.addEventListener("click", (e) => {
      e.stopPropagation();
      showGroupColorPicker(grp, e.currentTarget);
    });
  // Drag-zone: whole zone starts reorder (excluding toggle)
  row.querySelector(".layer-drag-zone").addEventListener("pointerdown", (e) => {
    if (
      e.target.classList.contains("layer-group-toggle") ||
      e.target.classList.contains("layer-dot") ||
      e.target.classList.contains("layer-unhide")
    )
      return;
    if (e.target.closest(".layer-group-name")?.contentEditable === "true")
      return;
    if (e.pointerType === "pen" ? !(e.buttons & 1) : e.button !== 0) return;
    e.preventDefault();
    e.stopPropagation();
  ...
  row.addEventListener("click", (e) => {
    if (
      e.target.classList.contains("layer-group-vis") ||
      e.target.classList.contains("layer-group-toggle") ||
      ...
      return;
    if (row.querySelector(".layer-group-name").contentEditable === "true")
      return;
    if (e.ctrlKey || e.metaKey) {
  ...
  row.addEventListener("contextmenu", (e) => {
    e.preventDefault();
    e.stopPropagation();
    _layerCtxTargetIdx = null;
    _layerCtxTargetGid = grp.id;
    layerCtxMenu.classList.remove("mode-layer");
    layerCtxMenu.classList.add("mode-group");
    layerCtxMenu.style.left =
      Math.min(e.clientX, window.innerWidth - 160) + "px";
    layerCtxMenu.style.top =
      Math.min(e.clientY, window.innerHeight - 120) + "px";
...
function startGroupDrag(gid, downEv) {
  ...
  const onMove = (ev) => {
    ...
    if (row) {
      ...
      const targetIsSelfOrDescendant =
        isGrpRow && descendantIds.has(row.dataset.gid);
      let pos, dropMode;
  ...
  const onUp = () => {
    ...
    if (
      layerDropTarget.idx != null &&
      !(layerDropTarget.isGroup && descendantIds.has(layerDropTarget.idx))
    ) {
      const pos = layerDropTarget.position;
      ...
      if (targetFlatIdx >= 0) {
        ...
        const ownFlatIdxs = new Set(
          flatFull
            .filter(
              (it) =>
                (it.type === "group" && descendantIds.has(it.id)) ||
                (it.type === "layer" &&
                  layers[it.idx] &&
                  descendantIds.has(layers[it.idx].groupId)),
            )
            .map((it) => it.flatPos),
        );
        if (!ownFlatIdxs.has(targetFlatIdx)) {
          ...
          flatFull.forEach((it, fi) => {
            if (it.type === "group" && descendantIds.has(it.id))
              movingFlatIdxs.add(fi);
            if (
              it.type === "layer" &&
              layers[it.idx] &&
              descendantIds.has(layers[it.idx].groupId)
          ...
          const removedBefore = [...movingFlatIdxs].filter(
            (fi) => fi < insertSlot,
          ).length;
          const adjSlot = Math.max(
            0,
            Math.min(remaining.length, insertSlot - removedBefore),
          );
          const newFlat = [
            ...remaining.slice(0, adjSlot),
            ...moving,
            ...remaining.slice(adjSlot),
          ];
          ...
          if (dropMode === "into") {
            ...
            if (targetLayer && pos === "after" && newParent !== null) {
              ...
              const targetFlatPos = flatFull.findIndex(
                (it) => it.type === "layer" && it.idx === layerDropTarget.idx,
              );
              let ni = targetFlatPos + 1;
              while (
                ni < flatFull.length &&
                ((flatFull[ni].type === "group" &&
                  descendantIds.has(flatFull[ni].id)) ||
                  (flatFull[ni].type === "layer" &&
                    layers[flatFull[ni].idx] &&
                    descendantIds.has(layers[flatFull[ni].idx].groupId)))
              )
                ni++;
              ...
              if (!nextReal) {
                ...
                newParent = layers[nextReal.idx]
                  ? layers[nextReal.idx].groupId || null
                  : null;
          ...
          groups.forEach((g) => {
            if (!seenGroups.has(g.id)) newGroups.push(g);
          });
          ...
          curLayer =
            newCurIdx >= 0
              ? newCurIdx
              : Math.max(0, Math.min(layers.length - 1, curLayer));
          loadFrame(curLayer, curFrame);
        }
      }
    }
...
function showGroupOpacityPopup(grp, cx, cy) {
  ...
  const sl = popup.querySelector("input[type=range]");
  const vl = popup.querySelector("span:last-child");
  sl.oninput = () => {
    grp.opacity = sl.value / 100;
  ...
  setTimeout(() => {
    function co(ev) {
      if (!popup.contains(ev.target)) {
        popup.remove();
        document.removeEventListener("click", co, true);
      }
    }
...
function _doAddLayer(placement, layerType) {
  ...
  if (targetGroupId) {
    let topIdx = -1;
    layers.forEach((l, i) => {
      if (l.groupId === targetGroupId && i > topIdx) topIdx = i;
    });
    ...
    let topIdx = -1;
    layers.forEach((l, i) => {
      if (l.groupId && fullIds.has(l.groupId) && i > topIdx) topIdx = i;
    });
    insertAt = topIdx >= 0 ? topIdx + 1 : layers.length;
  ...
  if (
    ...
    typeof window.PaletteDocker.refresh === "function"
  )
    window.PaletteDocker.refresh();
}

let _pendingAddLayerType = "bitmap";
function _addLayerBtnClick(layerType) {
  _pendingAddLayerType =
    layerType === "smart-raster" ? "smart-raster" : "bitmap";
  if (activeGroupId) {
    const grp = groups.find((g) => g.id === activeGroupId);
    document.getElementById("modal-add-layer-placement-msg").textContent =
      'Where do you want to add the new layer relative to "' +
      (grp ? grp.name : "the group") +
      '"?';
    document.querySelector(
      'input[name="add-layer-placement"][value="inside"]',
    ).checked = true;
    document
      .getElementById("modal-add-layer-placement")
...
document
  .getElementById("add-smart-raster-btn")
  .addEventListener("click", () => {
    _addLayerBtnClick("smart-raster");
  });
document.getElementById("modal-add-layer-placement-cancel").onclick = () =>
  document
    .getElementById("modal-add-layer-placement")
    .classList.remove("visible");
document.getElementById("modal-add-layer-placement-ok").onclick = () => {
  const placement =
    document.querySelector('input[name="add-layer-placement"]:checked')
      ?.value || "inside";
  document
    .getElementById("modal-add-layer-placement")
    .classList.remove("visible");
...
document
  ...
  .addEventListener("click", (e) => {
    if (e.target === document.getElementById("modal-add-layer-placement"))
      document
        .getElementById("modal-add-layer-placement")
        .classList.remove("visible");
  });
...
function _computeNewGroupParentId() {
  ...
  const selectedLayers =
    selectedLayerIndices.size > 0
      ? selectedLayerIndices
      : selectedTlLabelIndices;
  if (selectedLayers.size > 0) {
    selectedLayers.forEach((i) => {
      if (layers[i]) parentCandidates.add(layers[i].groupId || null);
    });
  }
  if (selectedGroupIds.size > 0) {
    selectedGroupIds.forEach((gid) => {
      ...
      if (!g) return;
      const hasSelectedAncestor = _groupChain(gid)
        .slice(0, -1)
        .some((aid) => selectedGroupIds.has(aid));
  ...
  if (selectedLayers.size === 0 && selectedGroupIds.size === 0) {
    parentCandidates.add(
      layers[curLayer] ? layers[curLayer].groupId || null : null,
    );
  }
...
function _createGroupFromSelection(parentId) {
  ...
  if (toGroupLayers.size === 0 && selectedGroupIds.size === 0)
    toGroupLayers.add(curLayer);
  toGroupLayers.forEach((i) => {
    if (layers[i]) layers[i].groupId = id;
  });
  selectedLayerIndices.clear();
  selectedTlLabelIndices.clear();
  selectedGroupIds.clear();
...
document.getElementById("add-group-btn").onclick = () => {
  ...
  if (selectedLayerIndices.size === 0) {
    ...
    else if (selectedGroupIds.size === 0 && activeGroupId)
      singleGroupId = activeGroupId;
  }
  if (singleGroupId) {
...
document.getElementById("modal-group-action-cancel").onclick = () =>
  document.getElementById("modal-group-action").classList.remove("visible");
document.getElementById("modal-group-action").addEventListener("click", (e) => {
  if (e.target === document.getElementById("modal-group-action"))
    document.getElementById("modal-group-action").classList.remove("visible");
});
document.getElementById("modal-group-action-ok").onclick = () => {
  ...
  const action =
    document.querySelector('input[name="group-action"]:checked')?.value ||
    "wrap";
