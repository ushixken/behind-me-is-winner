...
function refreshTimelineSelection(){
  ...
  if(cameraTrackSelected)selectedFrames.forEach(frameIndex=>{if(cameraSelectedProperties.size)cameraSelectedProperties.forEach(property=>{const cell=rows.querySelector('.camera-property-cell[data-camera-property="'+property+'"][data-camera-frame="'+frameIndex+'"]');if(cell)cell.classList.add('selected');});else{const cell=rows.querySelector('.camera-track-cell[data-camera-frame="'+frameIndex+'"]');if(cell)cell.classList.add('selected');}});
  ...
  rows.querySelectorAll('.tl-cell.cur-col').forEach(cell=>cell.classList.remove('cur-col'));if(cameraTrackSelected&&cameraSelectedProperties.size)cameraSelectedProperties.forEach(property=>{const cell=rows.querySelector('.camera-property-cell[data-camera-property="'+property+'"][data-camera-frame="'+curFrame+'"]');if(cell)cell.classList.add('cur-col');});else{const current=cameraTrackSelected?rows.querySelector('.camera-track-cell[data-camera-frame="'+curFrame+'"]'):rows.querySelector('.tl-cell[data-layer-idx="'+curLayer+'"][data-frame-idx="'+curFrame+'"]');if(current)current.classList.add('cur-col');}
  rows.querySelectorAll('.timeline-row-active').forEach(row=>row.classList.remove('timeline-row-active'));if(cameraTrackSelected&&cameraSelectedProperties.size)cameraSelectedProperties.forEach(property=>{const row=rows.querySelector('.tl-camera-property-row[data-camera-property="'+property+'"]');if(row)row.classList.add('timeline-row-active');});else{const activeRow=cameraTrackSelected?rows.querySelector('.tl-camera-track-row'):rows.querySelector('.tl-layer-track-row[data-layer-idx="'+curLayer+'"]');if(activeRow)activeRow.classList.add('timeline-row-active');}
  rows.querySelectorAll('.kf-block.selected,.camera-kf.selected').forEach(block=>block.classList.remove('selected'));selectedKFs.forEach(key=>{const block=key.startsWith('camera')?rows.querySelector('.camera-kf[data-kk="'+key+'"]'):rows.querySelector('.kf-block[data-kk="'+key+'"]');if(block)block.classList.add('selected');});selectedKFs.forEach(key=>{const match=/^camera:(\d+)$/.exec(key);if(!match)return;rows.querySelectorAll('.camera-property-kf[data-camera-frame="'+match[1]+'"]' ).forEach(marker=>marker.classList.add('selected'));});
  document.querySelectorAll('.tl-layer-lbl.active').forEach(label=>label.classList.remove('active'));const cameraLabel=document.querySelector('.tl-camera-lbl');if(cameraLabel)cameraLabel.classList.toggle('child-active',!!(cameraTrackSelected&&cameraSelectedProperties.size));if(cameraTrackSelected&&cameraSelectedProperties.size)cameraSelectedProperties.forEach(property=>{const label=document.querySelector('.tl-camera-property-lbl[data-camera-property="'+property+'"]');if(label)label.classList.add('active');});else{const activeLabel=cameraTrackSelected?document.querySelector('.tl-camera-lbl'):(activeGroupId?document.querySelector('.tl-group-lbl[data-gid="'+CSS.escape(activeGroupId)+'"]'):document.querySelector('.tl-layer-lbl[data-idx="'+curLayer+'"]'));if(activeLabel)activeLabel.classList.add('active');}
...
function startKFDrag(li,fi,e){
  ...
  if(typeof li==='string'&&li.startsWith('camera-')){
    const property=li.slice(7),draggedKey=_cameraPropertyId(property,fi),selectedIds=selectedKFs.has(draggedKey)?Array.from(selectedKFs).filter(key=>/^camera-[^:]+:\d+$/.test(key)):[draggedKey],before=CameraSystem.trackSnapshot(),items=[],selectedByProperty=new Map(),occupiedByProperty=new Map();selectedIds.forEach(id=>{const match=/^camera-([^:]+):(\d+)$/.exec(id);if(!match)return;const itemProperty=match[1],frame=Number(match[2]);if(!selectedByProperty.has(itemProperty))selectedByProperty.set(itemProperty,new Set());selectedByProperty.get(itemProperty).add(frame);const key=_cameraPropertyKeys(itemProperty).find(candidate=>candidate.frame===frame);if(key)items.push({property:itemProperty,key});});selectedByProperty.forEach((frames,itemProperty)=>occupiedByProperty.set(itemProperty,new Set(_cameraPropertyKeys(itemProperty).filter(key=>!frames.has(key.frame)).map(key=>key.frame))));dragKF={kind:'camera-property',property,properties:Array.from(selectedByProperty.keys()),li,originFi:fi,lockedScroll:tlScroll.scrollLeft,items,occupiedByProperty,before,appliedDelta:0,anyShift:false};document.addEventListener('pointermove',onKFDragMove);document.addEventListener('pointerup',onKFDragUp);return;
  }
  if(li==='camera'){
...
function onKFDragMove(e){
  ...
  if(dragKF.kind==='camera-property'){
    const min=Math.min(...dragKF.items.map(item=>item.key.frame)),max=Math.max(...dragKF.items.map(item=>item.key.frame)),delta=Math.max(-min,Math.min(TOTAL-1-max,rawTarget-dragKF.originFi));if(delta===dragKF.appliedDelta||dragKF.items.some(item=>dragKF.occupiedByProperty.get(item.property)?.has(item.key.frame+delta)))return;dragKF.appliedDelta=delta;const keys=dragKF.before.keys.map(key=>JSON.parse(JSON.stringify(key)));dragKF.properties.forEach(property=>{const origins=new Set(dragKF.items.filter(item=>item.property===property).map(item=>item.key.frame)),fields=_cameraPropertyFields(property);keys.forEach(key=>{if(origins.has(key.frame))fields.forEach(field=>delete key[field]);});});dragKF.items.forEach(item=>{const source=item.key,fields=_cameraPropertyFields(item.property),targetFrame=source.frame+delta;let target=keys.find(key=>key.frame===targetFrame);if(!target){target={frame:targetFrame,interpolation:source.interpolation||'ease',bezier:source.bezier?JSON.parse(JSON.stringify(source.bezier)):null};keys.push(target);}fields.forEach(field=>{if(Object.prototype.hasOwnProperty.call(source,field))target[field]=source[field];});});CameraSystem.replaceTrackKeys(keys.filter(key=>'x'in key||'y'in key||'zoom'in key||'rotation'in key));selectedKFs.clear();selectedFrames.clear();dragKF.items.forEach(item=>{const frame=item.key.frame+delta;selectedKFs.add(_cameraPropertyId(item.property,frame));selectedFrames.add(frame);});cameraSelectedProperties=new Set(dragKF.properties);cameraSelectedProperty=dragKF.properties.length===1?dragKF.properties[0]:null;curFrame=Math.max(0,Math.min(TOTAL-1,dragKF.originFi+delta));CameraSystem.evaluateAt(curFrame);renderTimeline();return;
  }
...
function _makeTimelineGroupLabel(item){
  ...
  row.onclick=()=>{clearCameraTimelineSelection();selectedTlLabelIndices.clear();selectedLayerIndices.clear();selectedGroupIds.clear();selectedGroupIds.add(group.id);activeGroupId=group.id;renderLayerPanel();renderTimeline();};
  return row;
}
...
function _selectedCameraProperty(){if(cameraSelectedProperties.size===1)return Array.from(cameraSelectedProperties)[0];return cameraSelectedProperty;}
function clearCameraTimelineSelection(){const hadSelection=cameraTrackSelected||_selectedCameraFrames().length>0;cameraTrackSelected=false;cameraSelectedProperty=null;cameraSelectedProperties.clear();selectedKFs.forEach(key=>{if(key.startsWith('camera'))selectedKFs.delete(key);});if(kfSelectionAnchor&&String(kfSelectionAnchor.trackId||'').startsWith('camera'))kfSelectionAnchor=null;return hadSelection;}
...
function selectTimelineKey(trackId,frame,event){
  ...
  const frames=_cameraTrackKeys().map(key=>key.frame).sort((a,b)=>a-b),id=`camera:${frame}`;frame=Number(frame);cameraTrackSelected=true;cameraSelectedProperty=null;cameraSelectedProperties.clear();selectedRowLayers.clear();
...
function _selectCameraTrack(selectCurrentKey){if(!_cameraTrackExists())return;cameraTrackSelected=true;cameraSelectedProperty=null;cameraSelectedProperties.clear();selectedKFs.clear();selectedFrames.clear();selectedFrames.add(curFrame);selectedRowLayers.clear();kfSelectionAnchor=null;if(selectCurrentKey&&_cameraKeyAt(curFrame)){selectedKFs.add(`camera:${curFrame}`);kfSelectionAnchor={trackId:'camera',frameIndex:curFrame};}renderTimeline();}
function _selectAllCameraTrackKeys(){if(!_cameraTrackExists())return false;const keys=_cameraTrackKeys();cameraTrackSelected=true;cameraSelectedProperty=null;cameraSelectedProperties.clear();selectedKFs.clear();selectedFrames.clear();selectedRowLayers.clear();keys.forEach(key=>{selectedKFs.add('camera:'+key.frame);selectedFrames.add(key.frame);});kfSelectionAnchor=keys.length?{trackId:'camera',frameIndex:keys[0].frame}:null;renderTimeline();return true;}
function _selectCameraProperties(properties){if(!_cameraTrackExists())return false;const valid=_cameraProperties().filter(property=>properties.includes(property));if(!valid.length)return false;cameraTrackExpanded=true;cameraTrackSelected=true;cameraSelectedProperties=new Set(valid);cameraSelectedProperty=valid.length===1?valid[0]:null;selectedKFs.clear();selectedFrames.clear();selectedRowLayers.clear();valid.forEach(property=>_cameraPropertyKeys(property).forEach(key=>{selectedKFs.add(_cameraPropertyId(property,key.frame));selectedFrames.add(key.frame);}));const firstFrame=Math.min(...Array.from(selectedFrames));kfSelectionAnchor=Number.isFinite(firstFrame)?{trackId:'camera-'+valid[0],frameIndex:firstFrame}:null;renderTimeline();return true;}
function _selectAllCameraPropertyKeys(property){return _selectCameraProperties([property]);}
...
function renderRows(){
  ...
  for(let frame=0;frame<TOTAL;frame++){
    ...
    cell.addEventListener('pointerdown',event=>{
      ...
      cameraTrackSelected=true;cameraSelectedProperty=null;cameraSelectedProperties.clear();selectedRowLayers.clear();
    ...
    cell.addEventListener('contextmenu',()=>{
      cameraTrackSelected=true;cameraSelectedProperty=null;cameraSelectedProperties.clear();selectedRowLayers.clear();selectedFrames.clear();selectedFrames.add(frame);
  ...
  _cameraTrackKeys().filter(key=>key.frame<TOTAL).forEach(key=>{
    ...
    marker.addEventListener('contextmenu',()=>{
      cameraTrackSelected=true;cameraSelectedProperty=null;cameraSelectedProperties.clear();selectedRowLayers.clear();selectedFrames.clear();selectedFrames.add(key.frame);
  ...
  if(!rowWrap.dataset.selectionHandlersBound)rowWrap.addEventListener('pointermove',e=>{
    ...
    if(typeof tlSelDrag.trackId==='string'&&tlSelDrag.trackId.startsWith('camera-')){const anchorProperty=tlSelDrag.trackId.slice(7),under=document.elementFromPoint(e.clientX,e.clientY),targetRow=under&&under.closest('[data-camera-property]'),targetProperty=targetRow&&targetRow.dataset.cameraProperty||anchorProperty,properties=_cameraProperties(),anchorIndex=properties.indexOf(anchorProperty),targetIndex=properties.indexOf(targetProperty),selectedProperties=anchorIndex<0||targetIndex<0?[anchorProperty]:properties.slice(Math.min(anchorIndex,targetIndex),Math.max(anchorIndex,targetIndex)+1),lo=Math.min(tlSelDrag.startF,f2),hi=Math.max(tlSelDrag.startF,f2);selectedFrames.clear();for(let frame=lo;frame<=hi;frame++)selectedFrames.add(frame);selectedKFs.clear();selectedProperties.forEach(property=>_cameraPropertyKeys(property).filter(key=>key.frame>=lo&&key.frame<=hi).forEach(key=>selectedKFs.add(_cameraPropertyId(property,key.frame))));cameraTrackSelected=true;cameraSelectedProperties=new Set(selectedProperties);cameraSelectedProperty=selectedProperties.length===1?selectedProperties[0]:null;curFrame=f2;CameraSystem.evaluateAt(curFrame);refreshTimelineSelection();return;}
