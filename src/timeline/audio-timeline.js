(() => {
  ...
  class AudioTimelineHeader {
    ...
    render() {
      this.button.setAttribute(
        "aria-expanded",
        String(!this.section.collapsed),
      );
      this.button.querySelector(".audio-timeline-disclosure").textContent = this
        .section.collapsed
        ? "▶"
        : "▼";
    }
  ...
  class AudioTimelineTrackRow {
    constructor(section, track, index) {
      this.section = section;
      this.track = track;
      this.index = index;
    }
    ...
    createLabel() {
      ...
      handle.type = "button";
      handle.className = "audio-track-drag-handle";
      handle.title = "Reorder Track";
      handle.setAttribute("aria-label", `Reorder ${this.track.name}`);
      handle.textContent = "⋮⋮";
      ...
      name.addEventListener("dblclick", (event) => {
        event.stopPropagation();
        this.section.renameTrack(this.track.id, name);
      });
      const future = document.createElement("span");
      future.className = "audio-track-future-controls";
      future.setAttribute("aria-hidden", "true");
      const remove = document.createElement("button");
      remove.type = "button";
      remove.className = "audio-track-delete";
      remove.title = "Delete Track";
      remove.textContent = "×";
      remove.setAttribute("aria-label", `Delete ${this.track.name}`);
      remove.addEventListener("click", (event) => {
        event.stopPropagation();
        this.section.deleteTrack(this.track.id);
      });
  ...
  class AudioTimelineViewport {
    constructor(section, labels, viewport, grid) {
      ...
      labels.addEventListener("scroll", () =>
        this.syncVertical(labels, viewport),
      );
      viewport.addEventListener("scroll", () => {
        ...
        if (
          !this.syncing &&
          timelineScroll &&
          viewport.scrollLeft !== timelineScroll.scrollLeft
        ) {
      ...
      viewport.addEventListener("lostpointercapture", (event) => {
        if (this.scrubPointerId != null)
          window.TimelineScrubController?.end(this.scrubPointerId);
        this.scrubPointerId = null;
    ...
    frameFromPointer(event) {
      ...
      return Math.max(
        ...
        Math.min(
          TOTAL - 1,
          (event.clientX - rect.left + this.viewport.scrollLeft - CellW / 2) /
            CellW,
        ),
      );
    }
    ...
    beginScrub(event) {
      ...
      const continuous =
        window.TimelineScrubController?.updateAudio(position, (frame) =>
          goToFrame(frame, false, false, true),
        ) ?? position;
      if (!(typeof playing === "boolean" && playing))
        window.AudioPlaybackEngine?.frameChanged(continuous, {
          playing: false,
          scrubbing: true,
        });
      event.preventDefault();
    ...
    updateGeometry() {
      ...
      const frame = Number.isFinite(window.SharedPlayhead?.position)
        ? window.SharedPlayhead.position
        : typeof curFrame === "number"
          ? curFrame
          : 0;
      ...
      if (timelineBody && timelineScroll) {
        const labelWidth = Math.max(
          ...
          timelineScroll.getBoundingClientRect().left -
            timelineBody.getBoundingClientRect().left,
        );
  ...
  class AudioTimelineDivider {
    ...
    start(event) {
      ...
      this.startHeight = this.section.collapsed
        ? HEADER_HEIGHT
        : this.section.height;
    ...
    move(event) {
      ...
      if (nextHeight <= HEADER_HEIGHT) this.section.setCollapsed(true, false);
        ...
        if (this.section.collapsed) this.section.setCollapsed(false, false);
        this.section.setHeight(nextHeight, false);
      }
      event.preventDefault();
    }
  ...
  class AudioTimelineSection {
    constructor() {
      ...
      document
        .getElementById("tl-scroll")
        .addEventListener("scroll", () => this.viewport.syncHorizontal());
      window.addEventListener("timeline-rendered", () =>
        this.viewport.updateGeometry(),
      );
      window.addEventListener("timeline-playhead-updated", () =>
        this.viewport.updateGeometry(),
      );
      ...
      if (window.ResizeObserver) {
        ...
        new ResizeObserver(() => this.applyHeight()).observe(
          document.getElementById("tl-body"),
        );
      }
    ...
    addTrack() {
      this.tracks.push({
        id: `audio-track-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
        name: this.nextTrackName(),
      });
      this.persistTracks();
      this.viewport.render(this.tracks);
    }
    ...
    renameTrack(id, label) {
      ...
      if (!track || label.querySelector("input")) return;
      ...
      const finish = (save) => {
        ...
        if (save && value) track.name = value;
        this.persistTracks();
    ...
    bindReorder(handle, index) {
      handle.addEventListener("pointerdown", (event) => {
        ...
        handle
          .closest(".audio-timeline-track-label")
          .classList.add("reordering");
        event.preventDefault();
      });
      handle.addEventListener("pointermove", (event) => {
        if (!this.trackDrag || event.pointerId !== this.trackDrag.pointerId)
          return;
        const under = document.elementFromPoint(event.clientX, event.clientY);
        const row = under && under.closest(".audio-timeline-track-label");
        if (!row || !this.viewport.labels.contains(row)) return;
        this.trackDrag.to = Number(row.dataset.audioTrackIndex);
        this.viewport.labels
          .querySelectorAll(".reorder-target")
          .forEach((item) => item.classList.remove("reorder-target"));
        row.classList.add("reorder-target");
      });
      const finish = (event) => {
        if (!this.trackDrag || event.pointerId !== this.trackDrag.pointerId)
          return;
        const { from, to } = this.trackDrag;
        this.trackDrag = null;
    ...
    persistTracks() {
      localStorage.setItem(STORAGE_TRACKS, JSON.stringify(this.tracks));
    }
    ...
    setHeight(value, persist = true) {
      this.height = Math.max(
        HEADER_HEIGHT,
        Math.min(this.maximumHeight(), value),
      );
      this.applyHeight();
    ...
    applyHeight() {
      const height = this.collapsed
        ? HEADER_HEIGHT
        : Math.min(this.height, this.maximumHeight());
    ...
    setTracks(tracks) {
      this.tracks = Array.isArray(tracks)
  ...
  window.AudioTimelineUI = {
    ...
    get tracks() {
      return section.tracks.map((track) => ({ ...track }));
    },
  };
})();
