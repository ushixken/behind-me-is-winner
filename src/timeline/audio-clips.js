(() => {
  ...
  class AudioClipRenderer {
    ...
    createClip(clip) {
      ...
      element.addEventListener("pointerdown", (event) =>
        this.editor.drag.begin(event, clip.id, element),
      );
  ...
  class AudioClipDragController {
    ...
    begin(event, id, element) {
      ...
      document
        ...
        .forEach((item) =>
          item.classList.toggle(
            "selected",
            this.editor.selection.ids.has(item.dataset.audioClipId),
          ),
        );
      ...
      this.editor.selectedClips().forEach((item) =>
        starts.set(item.id, {
    ...
    onMove = (event) => {
      ...
      const timelineFrame =
        (event.clientX - rect.left + drag.viewport.scrollLeft) / CellW;
      let deltaFrame = Math.round(timelineFrame - drag.startTimelineFrame);
      const minFrame = Math.min(
        ...[...drag.starts.values()].map((start) => start.frame),
      );
      ...
      const indices = [...drag.starts.values()].map(
        (start) => start.trackIndex,
      );
      deltaTrack = Math.max(
        -Math.min(...indices),
        Math.min(drag.trackIds.length - 1 - Math.max(...indices), deltaTrack),
      );
      ...
      const targetRow = document.querySelector(
        `.audio-timeline-track-grid[data-audio-track-id="${CSS.escape(drag.trackIds[targetIndex])}"]`,
      );
      if (targetRow) targetRow.classList.add("drop-target");
      this.editor.selection.ids.forEach((selectedId) => {
        const selected = document.querySelector(
          `.audio-clip[data-audio-clip-id="${CSS.escape(selectedId)}"]`,
        );
        if (selected) {
  ...
  class AudioClipEditor {
    constructor() {
      ...
      document
        ...
        .addEventListener("click", () => this.addTestClip());
      document
    ...
    addTestClip() {
      ...
      const selectedTrack = document.querySelector(
        ".audio-timeline-track-label.track-selected",
      );
      const track = selectedTrack
        ? selectedTrack.dataset.audioTrackId
        : tracks[0];
