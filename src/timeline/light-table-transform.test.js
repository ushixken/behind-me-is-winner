...
function loadTransformHelpers() {
  const transformSrc = fs.readFileSync(
    path.join(__dirname, "..", "transform", "transform-tool.js"),
    "utf8",
  );
...
function hitTestSingleRef(p, w, h, transform) {
  ...
  const boxCenter = {
    x: w / 2 + transform.positionX,
    y: h / 2 + transform.positionY,
  };
...
test("1. Hit-test at 0 degrees: interior click returns move and pivot click returns pivot", () => {
  ...
  assert.strictEqual(
    hitPivot.mode,
    "pivot",
    "Click directly on center pivot handle returns pivot",
  );
  ...
  assert.strictEqual(
    hitMove.mode,
    "move",
    "Interior click away from handles must be move",
  );
});
...
test("2. Hit-test at 45 degrees: interior click returns move and outside click returns rotate", () => {
  ...
  assert.strictEqual(
    hitInterior.mode,
    "move",
    "Interior click at 45 deg must be move",
  );
  ...
  assert.strictEqual(
    hitOutside.mode,
    "rotate",
    "Outside click at 45 deg must be rotate",
  );
});
...
test("3. Hit-test at 90 degrees: rotated interior point returns move", () => {
  ...
  assert.strictEqual(
    hitInside.mode,
    "move",
    "Local interior point at 90 deg must be move",
  );
});
...
test("4. Hit-test corner handle at 45 degrees returns scale", () => {
  ...
  assert.strictEqual(
    hit.mode,
    "scale",
    "Corner handle at 45 deg must be scale",
  );
});
...
test("5. Custom off-center pivot rotation maintains exact world pivot location without drift", () => {
  ...
  assert.ok(
    Math.abs(pivotAfter45.x - startPivotWorld.x) < 1e-6,
    `Pivot X drifted after 45 deg: ${pivotAfter45.x} vs ${startPivotWorld.x}`,
  );
  assert.ok(
    Math.abs(pivotAfter45.y - startPivotWorld.y) < 1e-6,
    `Pivot Y drifted after 45 deg: ${pivotAfter45.y} vs ${startPivotWorld.y}`,
  );
  ...
  assert.ok(
    Math.abs(pivotAfter90.x - startPivotWorld.x) < 1e-6,
    `Pivot X drifted after 90 deg: ${pivotAfter90.x} vs ${startPivotWorld.x}`,
  );
  assert.ok(
    Math.abs(pivotAfter90.y - startPivotWorld.y) < 1e-6,
    `Pivot Y drifted after 90 deg: ${pivotAfter90.y} vs ${startPivotWorld.y}`,
  );
  ...
  assert.ok(
    Math.abs(pivotAfter135.x - startPivotWorld.x) < 1e-6,
    `Pivot X drifted across repeated gestures: ${pivotAfter135.x} vs ${startPivotWorld.x}`,
  );
  assert.ok(
    Math.abs(pivotAfter135.y - startPivotWorld.y) < 1e-6,
    `Pivot Y drifted across repeated gestures: ${pivotAfter135.y} vs ${startPivotWorld.y}`,
  );
});
...
test("6. Custom off-center pivot scale maintains exact world pivot location without drift", () => {
  ...
  assert.ok(
    Math.abs(pivotAfterScale15.x - startPivotWorld.x) < 1e-6,
    `Pivot X drifted after 1.5x scale: ${pivotAfterScale15.x} vs ${startPivotWorld.x}`,
  );
  assert.ok(
    Math.abs(pivotAfterScale15.y - startPivotWorld.y) < 1e-6,
    `Pivot Y drifted after 1.5x scale: ${pivotAfterScale15.y} vs ${startPivotWorld.y}`,
  );
  ...
  assert.ok(
    Math.abs(pivotAfterScale20.x - startPivotWorld.x) < 1e-6,
    `Pivot X drifted after 2.0x scale: ${pivotAfterScale20.x} vs ${startPivotWorld.x}`,
  );
  assert.ok(
    Math.abs(pivotAfterScale20.y - startPivotWorld.y) < 1e-6,
    `Pivot Y drifted after 2.0x scale: ${pivotAfterScale20.y} vs ${startPivotWorld.y}`,
  );
  ...
  assert.ok(
    Math.abs(pivotAfterScale25.x - startPivotWorld.x) < 1e-6,
    `Pivot X drifted across repeated scales: ${pivotAfterScale25.x} vs ${startPivotWorld.x}`,
  );
  assert.ok(
    Math.abs(pivotAfterScale25.y - startPivotWorld.y) < 1e-6,
    `Pivot Y drifted across repeated scales: ${pivotAfterScale25.y} vs ${startPivotWorld.y}`,
  );
});
...
test("8. Verification of light-table.js integration", () => {
  ...
  assert.match(
    ltSrc,
    /_tfSetStateForPivot\(pivot,\s*st\.rotation,\s*newScale,\s*t,\s*pivotLocal,\s*box,\s*newScale\)/,
  );
  assert.match(
    ltSrc,
    /_tfSetStateForPivot\(pivot,\s*refNewRot,\s*st\.scaleX,\s*t,\s*pivotLocal,\s*box,\s*st\.scaleY\)/,
  );
});
...
test("9. Dedicated Light Table Transform tool mode lifecycle and tool restoration", () => {
  ...
  const brushSrc = fs.readFileSync(
  ...
  assert.match(
    tfSrc,
    /if\(window\.LightTable&&window\.LightTable\.transformMode\)\s*return;/,
  );
