(() => {
  ...
  class AudioWaveformHeightController {
    constructor() {
      ...
      this.control.innerHTML =
        '<span class="audio-height-groove"><span class="audio-height-thumb"></span></span>';
      ...
      this.control.addEventListener("lostpointercapture", (event) =>
        this.finish(event),
      );
    }
    ...
    apply(value, persist = true) {
      ...
      window.dispatchEvent(
        new CustomEvent("audio-waveform-height-change", {
  ...
  window.AudioWaveformHeight = {
    ...
    restore: (state) =>
      controller.apply(state?.audioWaveformHeight ?? DEFAULT_HEIGHT),
  };
})();
