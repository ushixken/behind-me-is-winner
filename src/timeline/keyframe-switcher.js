...
(function () {
  ...
  function _refreshMarkButtons(currentMarkId) {
    ...
    if (hiddenBtn) {
      hiddenBtn.classList.toggle("dm-btn-active", hidden);
      hiddenBtn.disabled = !hasKey;
    }
  ...
  markBtns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const id = btn.dataset.mark;
      if (typeof setDrawingMark === "function")
        setDrawingMark(curLayer, curFrame, id);
      refreshMarks();
  ...
  if (hiddenBtn) {
    hiddenBtn.addEventListener("click", () => {
      ...
      if (
        ...
        !setDrawingFrameHidden(curLayer, curFrame, !before)
      )
        return;
      undoStack.push({
        type: "drawing-frame-hidden",
        layer: curLayer,
        frame: curFrame,
        before,
        after: !before,
  ...
  try {
    ...
    if (saved && typeof saved === "object") {
      const v = {
        keyframe: !!saved.keyframe,
        breakdown: !!saved.breakdown,
        inbetween: !!saved.inbetween,
      };
  ...
  function _saveFlipThrough() {
    try {
      localStorage.setItem(STORE_KEY_FLIPTHROUGH, JSON.stringify(_flipThrough));
    } catch (e) {}
  }
  ...
  function _initFlipThroughChips() {
    ...
    ftChips.forEach((chip) => {
      ...
      if (def && def[mark])
        chip.style.setProperty("--kfsw-mark-color", def[mark].color);
    });
  ...
  chkFtKf.addEventListener("change", () =>
    _onFlipThroughChange("keyframe", chkFtKf),
  );
  chkFtBd.addEventListener("change", () =>
    _onFlipThroughChange("breakdown", chkFtBd),
  );
  chkFtIb.addEventListener("change", () =>
    _onFlipThroughChange("inbetween", chkFtIb),
  );
  ...
  function keyframeTimes(li) {
    ...
    if (!layer) return [];
    return Object.keys(layer.frames)
      .map(Number)
      .sort((a, b) => a - b);
  }
  ...
  function filteredFrameTimes(li) {
    const times = keyframeTimes(li).filter(
      (f) =>
        !(
          typeof isDrawingFrameHidden === "function" &&
    ...
    if (
      _flipThrough.keyframe &&
      _flipThrough.breakdown &&
      _flipThrough.inbetween
    )
      return times;
  ...
  function _visibleStepTarget(start, direction) {
    for (let f = start; f >= rangeStart && f <= rangeEnd; f += direction) {
      if (
        !(
          typeof isDrawingFrameHidden === "function" &&
          isDrawingFrameHidden(curLayer, f)
        )
      )
        return f;
    }
  ...
  function navigate(direction) {
    ...
    if (direction > 0 && curFrame >= rangeEnd) {
      const target = chkBypass.checked
        ? rangeStart
        : _visibleStepTarget(rangeStart, 1);
      if (target !== null) goToFrame(target);
      refresh();
      return;
    }
    if (direction < 0 && curFrame <= rangeStart) {
      const target = chkBypass.checked
        ? rangeEnd
        : _visibleStepTarget(rangeEnd, -1);
      if (target !== null) goToFrame(target);
      refresh();
      return;
    }
    ...
    if (
      !chkBypass.checked &&
      typeof isDrawingFrameHidden === "function" &&
      isDrawingFrameHidden(curLayer, target)
    ) {
      const visible = _visibleStepTarget(target, direction);
      if (visible === null) {
        refresh();
        return;
      }
  ...
  function _commitStep() {
    ...
    spinStep.value = v;
    try {
      localStorage.setItem(STORE_KEY_STEP, v);
    } catch (e) {}
    refresh();
  }
  ...
  chkBypass.onchange = () => {
    try {
      localStorage.setItem(STORE_KEY_BYPASS, chkBypass.checked ? "1" : "0");
    } catch (e) {}
  ...
  document.addEventListener("keyup", (e) => {
    if (_matchFlipperBypass(e)) _releaseBypassHold();
  });
  ...
  if (kfswPanel && typeof ResizeObserver !== "undefined") {
    ...
    function _applyLayout() {
      ...
      if (!isDocked) {
        const stateFullyHidden =
          kfswPanel.classList.contains("kfsw-hide-state");
        if (stateFullyHidden) {
    ...
    kfswPanel.addEventListener(
      "pointerdown",
      (e) => {
        if (
          e.target &&
          e.target.classList &&
          e.target.classList.contains("fp-float-resize")
        ) {
  ...
  if (dmPanel && typeof ResizeObserver !== "undefined") {
    ...
    function _dmApplyLayout() {
      ...
      const buttons = Array.from(dmButtons.children).filter(
        (button) => getComputedStyle(button).display !== "none",
      );
      const buttonRowWidth =
        buttons.reduce((width, button) => width + button.offsetWidth, 0) +
      ...
      if (dmPanel.style.minWidth !== safeMinimumCss)
        dmPanel.style.minWidth = safeMinimumCss;

      if (dmLabelWrap && getComputedStyle(dmLabelWrap).display !== "none") {
        const labelGap = 5;
        _dmKnownLabelWidth = Math.max(
          _dmKnownLabelWidth,
          (dmSwatch ? dmSwatch.offsetWidth : 0) +
            labelGap +
            (dmLabel ? dmLabel.scrollWidth : 0),
        );
      }
