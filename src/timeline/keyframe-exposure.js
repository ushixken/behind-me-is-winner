...
(function () {
  ...
  try {
    ...
    if (savedAmount >= 1)
      spinAmount.value = Math.max(1, Math.min(999, savedAmount));
  } catch (e) {}
  try {
  ...
  function selectedExposureFrame(li) {
    if (typeof selectedKFs !== "undefined" && selectedKFs && selectedKFs.size) {
      const frames = Array.from(selectedKFs)
        .map((id) => {
  ...
  function insertHoldFrame(li, at) {
    const layer = layers[li];
    if (!layer) return;
    const kf = Object.keys(layer.frames)
      .map(Number)
    ...
    kf.forEach((f) => {
      if (f >= at) {
        ...
        if (nf < TOTAL) {
          ...
          if (layer.frameMeta[f])
            layer.frameMeta[nf] = Object.assign({}, layer.frameMeta[f]);
          else delete layer.frameMeta[nf];
          if (
            typeof restoreStyleFrameBundle === "function" &&
            typeof getStyleFrameBundle === "function"
          )
            restoreStyleFrameBundle(li, nf, getStyleFrameBundle(li, f));
        }
  ...
  function normalExposure(forcePositive) {
    ...
    if (shift > 0) {
      ...
      for (let i = 0; i < shift; i++)
        insertHoldFrame(curLayer, targetFrame + 1);
      setStatus(`✔ Frame ${targetFrame + 1}: inserted ${shift} hold frame(s).`);
    } else {
      const nextFrame = keyframeTimes(curLayer).find(
        (frame) => frame > targetFrame,
      );
      const removable =
        nextFrame === undefined ? 0 : Math.max(0, nextFrame - targetFrame - 1);
      ...
      for (let i = 0; i < actualShift; i++)
        removeHoldFrame(curLayer, targetFrame + 1);
      setStatus(
        `✔ Frame ${targetFrame + 1}: removed ${actualShift} hold frame(s).`,
      );
    }
  ...
  function bypassExposure(forward) {
    ...
    if (!layer.frames[curFrame]) {
      setStatus(
        `⚠ No keyframe at frame ${curFrame + 1}. Seek to a keyframe first.`,
      );
      return;
    }
    ...
    const exposure = Math.max(
      1,
      Math.min(999, parseInt(spinAmount.value) || 2),
    );
    ...
    if (forward) {
      ...
      if (next === undefined) {
        setStatus("⚠ No next keyframe found.");
        return;
      }
      neighbour = next;
      ...
      if (!prev.length) {
        setStatus("⚠ No previous keyframe found.");
        return;
      }
    ...
    if (neededShift === 0) {
      setStatus(
        `✔ Gap to ${dirLabel} keyframe is already ${exposure} — nothing to do.`,
      );
      return;
    }
    ...
    if (neededShift > 0) {
      ...
      setStatus(
        `✔ Bypass: inserted ${neededShift} hold frame(s). Gap to ${dirLabel} keyframe is now ${exposure}.`,
      );
    } else {
      ...
      setStatus(
        `✔ Bypass: removed ${absShift} hold frame(s). Gap to ${dirLabel} keyframe is now ${exposure}.`,
      );
    }
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (matchBind(e, "increaseExposure")) {
      e.preventDefault();
      applyExposure(true);
  ...
  spinAmount.onchange = () => {
    ...
    spinAmount.value = v;
    try {
      localStorage.setItem(STORE_KEY_AMOUNT, v);
    } catch (e) {}
  };
  ...
  if (kfexpPanel && typeof ResizeObserver !== "undefined") {
    const HEIGHT_SEQ = ["kfexp-hide-hint", "kfexp-hide-info", "kfexp-compact"];
    ...
    const ro = new ResizeObserver(() => {
      ...
      if (!compact) {
        ...
        HEIGHT_SEQ.forEach((cls) => {
          if (cls !== "kfexp-compact") kfexpPanel.classList.remove(cls);
        });
