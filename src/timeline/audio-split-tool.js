(() => {
  ...
  class AudioSplitTool {
    ...
    onPointerMove(event) {
      ...
      const model = window.AudioClipUI?.clips.find(
        (item) => item.id === clip.dataset.audioClipId,
      );
      if (!model) return;
    ...
    onPointerDown(event) {
      ...
      if (!clip) return;
      window.AudioClipUI?.splitAt(
        clip.dataset.audioClipId,
        this.frameAtPointer(event),
      );
      this.clearGuide();
    }
