(() => {
  ...
  function clearArea() {
    ...
    return {
      left: 0,
      top: 0,
      clearW: canvasArea.clientWidth,
      clearH: canvasArea.clientHeight,
    };
  }
  ...
  document.addEventListener("click", (event) => {
    const button =
      event.target.closest && event.target.closest("#btn-camera-view");
    if (!button) return;
  ...
  function setMagnification(nextMagnification, clientX, clientY) {
    ...
    const pointerX = Number.isFinite(clientX)
      ? clientX - rect.left
      : oldGeometry.area.left + oldGeometry.area.clearW / 2;
    const pointerY = Number.isFinite(clientY)
      ? clientY - rect.top
      : oldGeometry.area.top + oldGeometry.area.clearH / 2;
    nextMagnification = Math.max(0.1, Math.min(8, nextMagnification));
    ...
    const nextCenteredLeft =
      oldGeometry.area.left + (oldGeometry.area.clearW - nextWidth) / 2;
    const nextCenteredTop =
      oldGeometry.area.top + (oldGeometry.area.clearH - nextHeight) / 2;
  ...
  function zoomBy(direction, clientX, clientY) {
    return setMagnification(
      magnification * (direction > 0 ? 1.2 : 1 / 1.2),
      clientX,
      clientY,
    );
  }
  ...
  host.addEventListener(
    ...
    (event) => {
      ...
      event.stopPropagation();
      setMagnification(
        magnification * Math.exp(-event.deltaY * 0.0015),
        event.clientX,
        event.clientY,
      );
    },
  ...
  function beginPointerNavigation(event, forceCtrl) {
    ...
    event.stopPropagation();
    const zoomDrag =
      primaryDrag && (forceCtrl || event.ctrlKey || previewCtrlHeld);
    navigation = {
  ...
  canvasArea.addEventListener("pointermove", (event) => {
    ...
    if (navigation.kind === "zoom") {
      const next =
        ...
        Math.pow(2, (event.clientX - navigation.startX) / 300);
  ...
  function endNavigation(event) {
    ...
    event.stopPropagation();
    try {
      if (canvasArea.hasPointerCapture(event.pointerId))
        canvasArea.releasePointerCapture(event.pointerId);
    } catch (_) {}
    navigation = null;
    host.style.cursor = presenting ? "grab" : "default";
  }
  ...
  canvasArea.addEventListener("pointercancel", endNavigation, {
    capture: true,
  });
  canvasArea.addEventListener("lostpointercapture", (event) => {
    if (navigation && event.pointerId === navigation.pointerId) {
      navigation = null;
      host.style.cursor = presenting ? "grab" : "default";
    }
  });
